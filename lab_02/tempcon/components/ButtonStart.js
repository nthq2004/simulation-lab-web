...
export class NormallyOpenPushButton extends BaseComponent {
    ...
    _drawContactMechanism() {
        ...
        // 固定触头（COM，左侧）
        const comX = cx2 - 20;
        this.group.add(new Konva.Rect({ x: comX - colW/2, y: cy2 + 14, width: colW, height: 8, fill: '#c0a020', stroke: '#8a7010', strokeWidth: 0.8, cornerRadius: 1 }));
        this.group.add(new Konva.Line({ points: [comX, cy2+16, comX, cy2+ch-4], stroke: '#c0a020', strokeWidth: 2.5, lineCap: 'round' }));

        // 固定触头（NO，右侧）- NO 型使用 NO 标注
        const noX  = cx2 + 20;
        this.group.add(new Konva.Rect({ x: noX - colW/2, y: cy2 + 14, width: colW, height: 8, fill: '#c0a020', stroke: '#8a7010', strokeWidth: 0.8, cornerRadius: 1 }));
        this.group.add(new Konva.Line({ points: [noX, cy2+16, noX, cy2+ch-4], stroke: '#c0a020', strokeWidth: 2.5, lineCap: 'round' }));
        ...
        this.group.add(this._contactBridge, this._springGroup, this._dotCom, this._dotNo);
    ...
    _drawIndicatorLed() {
        ...
        // 标注
        this.group.add(new Konva.Text({ x: cx2-10, y: cy2+7, width: 20, text: 'NO', fontSize: 7, fill: '#546e7a', align: 'center' }));
    ...
    _drawTerminals() {
        ...
        this.group.add(tNoBlock);
        this.group.add(new Konva.Line({ points: [noX+4, this._contactY + this._contactH - 16, noX+4, ty+4], stroke: '#c0a020', strokeWidth: 1.5, dash: [3,2] }));
