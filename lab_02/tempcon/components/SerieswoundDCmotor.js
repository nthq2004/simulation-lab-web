...
export class SeriesWoundDCMotor extends BaseComponent {
    ...
    _drawStatorShell() {
        ...
        this.group.add(new Konva.Circle({ x: cx, y: cy, radius: R+12, fill: '#37474f', stroke: '#263238', strokeWidth: 2.5 }));
        for (let i = 0; i < 4; i++) {
            const a = (i/4)*Math.PI*2 + Math.PI/4;
            this.group.add(new Konva.Circle({ x: cx+(R+9)*Math.cos(a), y: cy+(R+9)*Math.sin(a), radius: 5, fill: '#263238' }));
        }
        this.group.add(new Konva.Circle({ x: cx, y: cy, radius: R, fill: '#0a1520' }));
        this.group.add(new Konva.Text({ x: cx-R, y: cy-R-22, width: R*2, text: '串励直流电动机', fontSize: 9, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
    }
    ...
    _drawSeriesFieldWinding() {
        ...
        for (let i = 0; i < p*2; i++) {
            ...
            // 铁芯极靴
            this.group.add(new Konva.Arc({ x: cx, y: cy, innerRadius: ir-8, outerRadius: ir+2, angle: poleArc*180/Math.PI*1.4, rotation: (poleAngle-poleArc*0.7)*180/Math.PI-90, fill: '#455a64', stroke: '#2a3a44', strokeWidth: 0.5 }));
            ...
            // 极性标记（N/S）
            const mr  = (ir+or)/2;
            this.group.add(new Konva.Text({ x: cx+mr*Math.cos(poleAngle-Math.PI/2)-5, y: cy+mr*Math.sin(poleAngle-Math.PI/2)-6, width: 10, text: isN?'N':'S', fontSize: 9, fontStyle: 'bold', fill: '#fff', align: 'center' }));
        }
        // 标注
        this.group.add(new Konva.Text({ x: cx-R, y: cy+R+14, width: R*2, text: `Rse=${this.Rse}Ω（粗线少匝，I=I_a）`, fontSize: 7.5, fontFamily: 'Courier New, monospace', fill: '#ffa726', align: 'center' }));
    }
    ...
    _drawCommutator() {
        ...
        [{ a: -Math.PI/2, col: '#ffcc80' }, { a: Math.PI/2, col: '#fff9c4' }].forEach(({ a, col }) => {
            ...
            this.group.add(new Konva.Rect({ x: bx2-5, y: by2-4, width: 10, height: 8, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1, cornerRadius: 1 }));
    ...
    _drawShaft() {
        const cx = this._motorCX, cy = this._motorCY, R = this._statorR;
        this.group.add(new Konva.Ellipse({ x: cx+R+6, y: cy, radiusX: 8, radiusY: R*0.48, fill: '#546e7a', stroke: '#37474f', strokeWidth: 1.5 }));
        this.group.add(new Konva.Rect({ x: cx+R+8, y: cy-5, width: 26, height: 10, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1 }));
        this.group.add(new Konva.Ellipse({ x: cx-R-6, y: cy, radiusX: 8, radiusY: R*0.48, fill: '#546e7a', stroke: '#37474f', strokeWidth: 1.5 }));
    }
    ...
    _drawCircuitSchematic() {
        ...
        this.group.add(new Konva.Text({ x: sx+2, y: sy+2, width: sw-4, text: '串励接法', fontSize: 7.5, fill: '#ffa726', align: 'center' }));

        const mx = sx + sw/2;
        // V_s → Rse → Ra(E_b) → GND 串联路径
        this.group.add(new Konva.Text({ x: sx+3, y: sy+14, text: 'V_s', fontSize: 7, fontStyle: 'bold', fill: '#ef9a9a' }));
        this.group.add(new Konva.Line({ points: [sx+3, sy+22, mx, sy+22], stroke: '#ffa726', strokeWidth: 1.5 }));
        this.group.add(new Konva.Text({ x: sx+3, y: sy+25, text: 'Rse（串励）', fontSize: 6.5, fill: '#ffa726' }));
        this.group.add(new Konva.Line({ points: [mx, sy+33, mx, sy+44], stroke: '#ffa726', strokeWidth: 1.5 }));
        this.group.add(new Konva.Text({ x: sx+3, y: sy+36, text: 'Ra + E_b', fontSize: 6.5, fill: '#ef9a9a' }));
        this.group.add(new Konva.Line({ points: [mx, sy+44, sx+sw-3, sy+44], stroke: '#ffa726', strokeWidth: 1.5 }));
        this.group.add(new Konva.Text({ x: sx+3, y: sy+47, text: 'I_a = I_se = I', fontSize: 6.5, fill: '#fff59d' }));
        this.group.add(new Konva.Text({ x: sx+3, y: sy+57, text: 'T∝I²（双重增磁）', fontSize: 6.5, fill: '#ffa726' }));
        this._schIText = new Konva.Text({ x: sx+3, y: sy+67, text: 'I=0A', fontSize: 7, fontFamily: 'Courier New, monospace', fill: '#fff59d' });
        this.group.add(this._schIText);
    }

    // ── 动态层 ───────────────────────────────
    _drawForceLayer()   { this._forceGroup = new Konva.Group(); this.group.add(this._forceGroup); }
    _drawFlowLayer()    { this._flowGroup  = new Konva.Group(); this.group.add(this._flowGroup);  }
    ...
    _drawInstrHead() {
        ...
        this.group.add(new Konva.Text({ x: hx+8, y: hy+17, width: hw-16, text: `${this.Vs}V  串励接法`, fontSize: 7, fill: '#78909c', align: 'center' }));
        this.group.add(new Konva.Text({ x: hx+8, y: hy+27, width: hw-16, text: 'SERIES DC MOTOR', fontSize: 7, fill: '#90a4ae', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this.group.add(ring, this._lcdBg, this._speedArc, this._warnRingLcd, this._lcdMain, this._lcdUnit, this._lcdI, this._lcdTem, this._lcdEb);
    }
    ...
    _drawSpeedTorqueCurve() {
        ...
        this.group.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'n-T 双曲线特性（ω∝1/√T）', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
        ...
        if (curvePts.length > 2) {
            this.group.add(new Konva.Line({ points: curvePts, stroke: '#ffa726', strokeWidth: 1.5, opacity: 0.5, lineJoin: 'round' }));
        }

        // 飞车危险区（左侧高速区）
        this.group.add(new Konva.Rect({ x: ox, y: oy-ah+2, width: aw*0.12, height: ah-4, fill: 'rgba(239,83,80,0.15)', cornerRadius: 1 }));
        this.group.add(new Konva.Text({ x: ox+1, y: cy2+16, text: '飞车\n危险', fontSize: 6.5, fill: '#ef9a9a', lineHeight: 1.3 }));
        ...
        this._charMaxT = maxT;
        this.group.add(bg, titleBg, this._charPoint);
    }
    ...
    _drawLoadPanel() {
        ...
        this.group.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '负载阻力矩 T_L', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
        ...
        this.group.add(bg, titleBg, this._loadBar, this._loadValText, this._flyWarnBox, this._flyWarnText);
        this._dynText = new Konva.Text({ x: px+4, y: py+44, width: pw-8, text: '', fontSize: 8, fill: '#66bb6a', align: 'center' });
        this.group.add(this._dynText);
    }
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'n(rpm)  I(A)  T_em(N·m)', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));

        const h3 = (wh-13)/3;
        this._wavMids = [wy+13+h3*0.5, wy+13+h3*1.5, wy+13+h3*2.5];
        this._wavMids.forEach(my => {
            this.group.add(new Konva.Line({ points: [wx+2, my, wx+ww-2, my], stroke: 'rgba(200,200,200,0.09)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        this.group.add(new Konva.Text({ x: wx+4, y: wy+13+4, text: 'n', fontSize: 8, fill: '#ffa726' }));
        this.group.add(new Konva.Text({ x: wx+4, y: wy+13+h3+4, text: 'I', fontSize: 8, fill: '#fff59d' }));
        this.group.add(new Konva.Text({ x: wx+4, y: wy+13+h3*2+4, text: 'T', fontSize: 8, fill: '#ffcc80' }));
        ...
        this.group.add(bg, titleBg, this._wLineN, this._wLineI, this._wLineTem, this._wNLbl, this._wILbl, this._wTLbl);
        this._wavH3 = h3;
    }
