 ...
 *  wire_p     — 正极
 *  wire_n     — 负极
 *  wire_ma_p  — 4-20mA 温度输出正极
 *  wire_ma_n  — 4-20mA 温度输出负极
 */
export class PTCThermistor extends BaseComponent {
    constructor(config, sys) {
        ...
        this.addPort(this._probeCX - 12, this.height - 4, 'p',    'wire', 'P+');
        this.addPort(this._probeCX + 12, this.height - 4, 'n',    'wire', 'P−');
    ...
    _tickPhysics(dt) {
        ...
        // 电路参数
        const Rtotal = this.resistance + this.Rload;
        this.current  = this.Vsupply / Rtotal;
        this.voltage  = this.Vsupply * this.resistance / Rtotal;
        this.power    = this.current * this.current * this.resistance;

        // 工作状态判断
        if (this.current > this.Ilimit) {
            this.state = 'limiting';
        } else if (this.temperature >= this.Tswitch && this.resistance > this.Rmin * 100) {
    ...
    _tickDisplay() {
        ...
        if (this._lcdR2)    this._lcdR2.text(this._fmtR(this.resistance));
