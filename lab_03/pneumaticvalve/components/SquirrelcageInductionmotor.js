...
export class SquirrelCageInductionMotor extends BaseComponent {
    ...
    _drawStatorIron() {
        ...
        // 截面图标注
        this.group.add(new Konva.Text({ x: cx-this._statorRo, y: cy-this._statorRo-22, width: this._statorRo*2, text: '三相异步电动机（截面图）', fontSize: 9, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawStatorWindings() {
        ...
        phases.forEach(ph => {
            ...
            ph.angles.forEach((a, side) => {
                ...
                grp.add(g2);
                // 引线
                this.group.add(new Konva.Line({ points: [cx+(r-bh/2)*Math.cos(a-Math.PI/2), cy+(r-bh/2)*Math.sin(a-Math.PI/2), cx+(Ri-4)*Math.cos(a-Math.PI/2), cy+(Ri-4)*Math.sin(a-Math.PI/2)], stroke: ph.color, strokeWidth: 1.5, opacity: 0.5 }));
            });
        ...
        phases.forEach((ph, i) => {
            ...
            const r = Ro + 18;
            this.group.add(new Konva.Text({ x: cx + r*Math.cos(a)-8, y: cy + r*Math.sin(a)-6, text: ph.name, fontSize: 10, fontStyle: 'bold', fill: ph.color }));
        });
    }
    ...
    _drawCageRotor() {
        ...
        // 标注
        this.group.add(new Konva.Text({ x: cx-22, y: cy+this._rotorRo+6, text: '鼠笼转子', fontSize: 8.5, fill: '#c0a020', fontStyle: 'bold' }));
    ...
    _drawShaft() {
        ...
        this.group.add(new Konva.Rect({ x: cx+Ro+8, y: cy-5, width: 24, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this.group.add(new Konva.Ellipse({ x: cx+Ro+8, y: cy, radiusX: 8, radiusY: this._statorRi*0.48, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1.5 }));
        this.group.add(new Konva.Ellipse({ x: cx-Ro-8, y: cy, radiusX: 8, radiusY: this._statorRi*0.48, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1.5 }));
        this.group.add(new Konva.Text({ x: cx+Ro+10, y: cy+8, text: '输出轴', fontSize: 7.5, fill: '#607d8b' }));
    }
    ...
    _drawInducedCurrentLayer() {
        ...
        this.group.add(this._inducedGroup);
    }
    ...
    _drawSideView() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            this.group.add(new Konva.Rect({ x: bx, y: sy+sh*0.12, width: 3, height: sh*0.78, fill: '#2c5b7a', cornerRadius: 1 }));
        }
        // 前后端盖（椭圆形）
        const capRY = sh*0.39;
        ...
        // 轴（伸出左端）
        this.group.add(new Konva.Rect({ x: sx-16, y: cy2-5, width: 22, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        // 接线盒（顶部）
        const jbX = cx2-15, jbY = sy+sh*0.12-14;
        this.group.add(new Konva.Rect({ x: jbX, y: jbY, width: 30, height: 14, fill: '#3a5a78', stroke: '#2c5b7a', strokeWidth: 1.5, cornerRadius: [3,3,0,0] }));
        this.group.add(new Konva.Text({ x: jbX+2, y: jbY+3, width: 26, text: 'U V W', fontSize: 7.5, fill: '#80c0d4', align: 'center' }));
        // 铭牌
        this.group.add(new Konva.Rect({ x: cx2-20, y: cy2-10, width: 40, height: 20, fill: '#1a3a50', cornerRadius: 2 }));
        this.group.add(new Konva.Text({ x: cx2-20, y: cy2-8, width: 40, text: `${this.ratedPower}kW\n${this.ratedVoltage}V`, fontSize: 7.5, fill: '#80c0d4', align: 'center', lineHeight: 1.3 }));
        // 支脚
        [-sh*0.28, sh*0.28].forEach(dy => {
            this.group.add(new Konva.Rect({ x: sx+8, y: cy2+dy-4, width: sw-16, height: 8, fill: '#3a5a78', cornerRadius: 2 }));
            this.group.add(new Konva.Rect({ x: sx+10, y: cy2+dy+4, width: sw-20, height: 10, fill: '#2c5b7a', cornerRadius: [0,0,2,2] }));
        });
        this.group.add(body, leftCap, rightCap, this._fanGroup);
        this.group.add(new Konva.Text({ x: sx, y: sy-16, width: sw, text: '侧视图', fontSize: 8.5, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawTorqueSpeedCurve() {
        ...
        this.group.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'T-n 机械特性曲线（转矩-转速）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        [0, this.ratedTorque, this.maxTorque].forEach(T => {
            ...
            this.group.add(new Konva.Text({ x: ox-24, y: ty-4, width: 22, text: T===0?'0':Math.round(T)+'', fontSize: 6, fill: '#37474f', align: 'right' }));
        });
        ...
        if (tsCurvePts.length > 2) {
            this.group.add(new Konva.Line({ points: tsCurvePts, stroke: '#4fc3f7', strokeWidth: 2, lineJoin: 'round', opacity: 0.85 }));
        }
        ...
        this.group.add(new Konva.Circle({ x: nRatedX, y: tRatedY, radius: 4, fill: '#66bb6a', stroke: '#2e7d32', strokeWidth: 1 }));
        this.group.add(new Konva.Text({ x: nRatedX+4, y: tRatedY-10, text: 'N点', fontSize: 7, fill: '#66bb6a' }));
        // 最大转矩点
        const nMaxX = ox + (this.syncSpeed*(1-this.slipMaxTorque)/nMax)*(aw-2);
        const tMaxY = oy - (this.maxTorque/tMax)*(ah-4);
        this.group.add(new Konva.Circle({ x: nMaxX, y: tMaxY, radius: 4, fill: '#ffa726', stroke: '#e65100', strokeWidth: 1 }));
        this.group.add(new Konva.Text({ x: nMaxX-18, y: tMaxY-12, text: 'T_max', fontSize: 7, fill: '#ffa726' }));
        // 同步速垂线
        const n1X = ox + (this.syncSpeed/nMax)*(aw-2);
        this.group.add(new Konva.Line({ points: [n1X, oy-ah+2, n1X, oy], stroke: 'rgba(255,255,255,0.1)', strokeWidth: 0.8, dash: [3,3] }));
        // 稳定/不稳定区标注
        this.group.add(new Konva.Text({ x: ox+4, y: oy-ah+14, text: '不稳定区', fontSize: 7, fill: 'rgba(239,83,80,0.55)' }));
        this.group.add(new Konva.Text({ x: nMaxX+8, y: oy-ah+14, text: '稳定运行区', fontSize: 7, fill: 'rgba(102,187,106,0.55)' }));
        ...
        this.group.add(bg, titleBg, this._workPoint, this._workHLine, this._workVLine);
    }
    ...
    _drawLCDPanel() {
        ...
        this.group.add(ring, this._lcdBg, this._speedArc, this._lcdMain, this._lcdUnit, this._lcdSlip, this._lcdTorque, this._lcdPF, bg, titleBg);
    }
    ...
    _drawLoadPanel() {
        ...
        this.group.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '负载调节 T_L', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
        ...
        this.group.add(bg, titleBg, this._loadBar, this._loadValText, this._loadPctText, hit);
    }
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '三相定子电流  Iu(t)  Iv(t)  Iw(t)  ── 转速 n(t)', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        lbls.forEach((l, i) => { this.group.add(new Konva.Text({ x: wx+4, y: wy+14+h4*i+4, text: l, fontSize: 8, fill: cols[i] })); });
        ...
        this.group.add(bg, titleBg, this._wLineIu, this._wLineIv, this._wLineIw, this._wLineN, this._wIuLbl, this._wNLbl);
        this._wavH4 = h4;
    }
