import { Workflow, CircuitSolver, PneumaticSolver, DigitalSolver, MicrocontrollerSolver, 
    MCS51Solver, Show, perfMonitor, EventBus, EquipmentPool, ThermalSolver } from './export.js';
import { LeakDetector, AirBottle, PressRegulator, PressMeter, TeeConnector,
    ...
    DCPower, VariResistor, Resistor, OpAmp, Ground, Monitor,
    VoltageRelay, ACPower, Oscilloscope,
    Capacitor, JFET, Diode, Transistor, NpnTempSensor, RealResistor,
    RealVariResistor, CoolingSystem, FuelOilHeater, PressRelay, AudioVisualAlarm,
    NormallyClosedPushButton, NormallyOpenPushButton} from './export.js';
import { componentConfigs } from './project/pressguage.js';
...
export class ControlSystem {
    ...
    _initTempSlider() {
        ...
        slider.addEventListener('input', () => {
            ...
            // 仅控制调压阀设定压力，压力表由气路求解器更新
            const reg = this.comps['reg'];
            if (reg) {
                reg.setPressure = press / 1000; // kPa → MPa
                reg.update();
            }
            this.requestRedraw();
        });
