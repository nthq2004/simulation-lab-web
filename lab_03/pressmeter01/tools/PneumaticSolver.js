...
export class PneumaticSolver {
    ...
    _syncDevices() {
        ...
        Object.values(this.sys.comps).forEach(device => {
            if (targetTypes.includes(device.type)) {
                ...
                if (device.update) {
                    if (device.type === 'regulator') {
                        ...
                        device.update();
                    } else if (device.type === 'bourdon_tube' || device.type === 'diaphragm_gauge') {
                        // 气路求解器内部单位为 MPa，压力表显示单位为 kPa
                        device.update(currentP * 1000);
                    } else {
            ...
            else if (device.special === 'pressrelay') {
                const inPortId = `${device.id}_pipe_i`;
            ...
            else if (device.special === 'bubble_level') {
                device.updateFlow();
            }
