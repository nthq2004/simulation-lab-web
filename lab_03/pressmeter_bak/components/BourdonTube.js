...
export class BourdonTube extends BaseComponent {
    ...
    _rebuildMechanism() {
        ...
        if (frac > 0.02) {
            ...
            const disp = Math.sqrt((tipX - geo0.tipX)**2 + (tipY - geo0.tipY)**2);
            this._mechGroup.add(new Konva.Text({
                x: this._cx - this._R * 0.40, y: this._socketY - 4,
                width: this._R * 0.80,
                text: `管端位移: ${disp.toFixed(1)}px`,
    ...
    _drawBourdontube(frac, tipX, tipY, tipAng, curveR) {
        ...
        if (frac > 0.03) {
            ...
            // 零压标签（在管体中部外侧）
            const midAng0 = (angF + ang0) / 2;
            this._mechGroup.add(new Konva.Text({
                x: tcx + (rOuter0 + 14) * Math.cos(midAng0) - 22,
                y: tcy - (rOuter0 + 14) * Math.sin(midAng0) - 7,
                text: '零压位置', fontSize: 8, fill: 'rgba(100,200,255,0.55)', fontStyle: 'bold',
            }));
        }
        ...
        // ── 管端截面变化示意（椭圆→圆形）──────────────────
        if (frac > 0.01) {
            const secCX = this._cx - this._R * 0.68;
            const secCY = this._cy + this._R * 0.48;
            const secR  = this._R * 0.11;
            this._mechGroup.add(new Konva.Circle({
                x: secCX, y: secCY, radius: secR + 2,
                fill: '#f5f3ef', stroke: '#78909c', strokeWidth: 1.5,
            }));
            this._mechGroup.add(new Konva.Circle({
                x: secCX, y: secCY, radius: secR,
                fill: '#fafaf8', stroke: '#cfd8dc', strokeWidth: 0.5,
            }));
            const rx = secR * 0.75;
            const ry = secR * (0.28 + frac * 0.60);
            this._mechGroup.add(new Konva.Ellipse({
                x: secCX, y: secCY,
                radiusX: rx, radiusY: ry,
                fill: `rgba(64,200,255,${0.30 + frac * 0.55})`,
                stroke: '#00bcd4', strokeWidth: 1.5,
            }));
            this._mechGroup.add(new Konva.Text({
                x: secCX - secR, y: secCY + secR + 3,
                width: secR * 2, text: '管截面',
                fontSize: 7, fill: '#546e7a', align: 'center', fontStyle: 'bold',
            }));
            this._mechGroup.add(new Konva.Line({
                points: [secCX + secR + 2, secCY, tipX - 6, tipY + 4],
                stroke: '#00bcd4', strokeWidth: 0.8, opacity: 0.5, dash: [2, 3],
            }));
        }
    }
    ...
    _rebuildNeedle() {
        ...
        if (frac > 0.02) {
            ...
            // 角度数值
            const totalDeg = (this._dialAngEnd * frac).toFixed(0);
            const midA = (startA + endA) / 2;
            this._needleGroup.add(new Konva.Text({
                x: cx + (arcR + 10) * Math.cos(midA) - 16,
                y: cy + (arcR + 10) * Math.sin(midA) - 6,
                width: 32, text: `${totalDeg}°`,
                fontSize: 8, fill: '#ffc832', align: 'center', fontStyle: 'bold',
            }));
        }
    }
    ...
    _drawComponentLabels() {
        ...
        // 定义标注：[文本, 目标点x, 目标点y, 标签x, 标签y, 颜色]
        const items = [
            { text: '波登管', tx: cx - R*0.40, ty: cy - R*0.10, lx: 4, ly: cy - R*0.75, color: '#2196F3' },
            { text: '连杆',   tx: cx + R*0.05, ty: cy + R*0.28, lx: 4, ly: cy + R*0.02, color: '#4CAF50' },
            { text: '扇形齿轮', tx: cx + R*0.30, ty: cy + R*0.15, lx: W - 74, ly: cy + R*0.02, color: '#FF9800' },
            { text: '小齿轮', tx: cx, ty: cy, lx: W - 62, ly: cy - R*0.50, color: '#9C27B0' },
            { text: '指针',   tx: cx - R*0.40, ty: cy - R*0.50, lx: W - 62, ly: cy - R*0.75, color: '#F44336' },
            { text: '游丝',   tx: cx + R*0.10, ty: cy + R*0.05, lx: W - 74, ly: cy + R*0.38, color: '#607D8B' },
        ];
        ...
        if (labelY + 14 < H) {
            this.group.add(new Konva.Text({
                ...
                fontSize: 9, fill: '#37474F', align: 'center', fontStyle: 'bold',
            }));
        }
    }
