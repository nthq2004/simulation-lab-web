...
export class Workflow {
    ...
    _onStepPass() {
        ...
        // 记录步骤到学员会话
        if (this.sys && this.sys.sessionManager) {
            this.sys.sessionManager.logStep(idx, mode, true);
        }
        // 事件发射：通知外部步骤通过
        if (this.sys && this.sys.eventBus) {
            this.sys.eventBus.emit('workflow:step', {
            ...
            this.sys.eventBus.emit('session:action', {
                action: 'workflow:step:pass',
                idx,
                mode,
                timestamp: Date.now(),
            });
    ...
    showQuiz(config) {
        ...
        `;
            ...
            submitBtn.onclick = () => {
                ...
                // 记录到学员会话
                if (this.sys && this.sys.sessionManager) {
                    this.sys.sessionManager.logQuiz(
                        config.question ? config.question.slice(0, 40) : '',
                        isCorrect,
                        selectedIndices
                    );
                }
