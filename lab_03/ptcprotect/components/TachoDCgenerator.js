...
export class DCTachogenerator extends BaseComponent {
    ...
    _drawCrossSection() {
        ...
        this.group.add(new Konva.Rect({ x: mLeft, y: mTop, width: mW, height: mH, fill: '#1c2b38', stroke: '#37474f', strokeWidth: 1.5, cornerRadius: 2 }));
        
        // 永磁定子（两对极示意）
        const poleW = 8, poleH = mH * 0.55;
        const poleY = mCY - poleH/2;
        this.group.add(new Konva.Rect({ x: mLeft+4, y: poleY, width: poleW, height: poleH, fill: '#e53935', stroke: '#b71c1c', strokeWidth: 0.8, opacity: 0.8 }));
        this.group.add(new Konva.Text({ x: mLeft+4, y: poleY+poleH/2-4, width: poleW, text: 'N', fontSize: 8, fill: '#fff', align: 'center' }));
        this.group.add(new Konva.Rect({ x: mRight-12, y: poleY, width: poleW, height: poleH, fill: '#1e88e5', stroke: '#0d47a1', strokeWidth: 0.8, opacity: 0.8 }));
        this.group.add(new Konva.Text({ x: mRight-12, y: poleY+poleH/2-4, width: poleW, text: 'S', fontSize: 8, fill: '#fff', align: 'center' }));
        
        // 电枢铁芯（小惯量）
        const rR = Math.round(mH * 0.28);
        this._rotorR = rR;
        this.group.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: rR*0.95, radiusY: rR*0.28, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.8 }));
        ...
        for (let i = 0; i < 10; i++) {
            ...
            this.group.add(new Konva.Circle({ x: wx, y: wy, radius: 3, fill: '#ffb74d', stroke: '#e65100', strokeWidth: 0.5 }));
        }
        
        // 换向器（示意）
        const commY = mBot - 8;
        const commW = 14;
        this.group.add(new Konva.Rect({ x: mCX-commW/2, y: commY-3, width: commW, height: 6, fill: '#d4a017', stroke: '#8d6e63', strokeWidth: 0.8, cornerRadius: 1 }));
        
        // 电刷（碳刷，信号输出）
        this.group.add(new Konva.Rect({ x: mCX-commW/2-6, y: commY-1.5, width: 6, height: 3, fill: '#3e2723', stroke: '#1b0f0a', strokeWidth: 0.5 }));
        this.group.add(new Konva.Rect({ x: mCX+commW/2, y: commY-1.5, width: 6, height: 3, fill: '#3e2723', stroke: '#1b0f0a', strokeWidth: 0.5 }));
        
        // 气隙
        this.group.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: rR*0.96, radiusY: rR*0.28, fill: '#06101a', stroke: '#1a3040', strokeWidth: 0.3 }));
        
        // 输出端子
        const termX = mRight + 6;
        const termY = mCY;
        this.group.add(new Konva.Circle({ x: termX, y: termY-8, radius: 4, fill: '#ef5350' }));
        this.group.add(new Konva.Text({ x: termX+4, y: termY-12, text: 'U+', fontSize: 7, fill: '#ef5350', fontStyle: 'bold' }));
        this.group.add(new Konva.Circle({ x: termX, y: termY+8, radius: 4, fill: '#90caf9' }));
        this.group.add(new Konva.Text({ x: termX+4, y: termY+4, text: 'U-', fontSize: 7, fill: '#90caf9', fontStyle: 'bold' }));
        
        // 导线
        const outX = this._csX + this._csW + 8;
        this.group.add(new Konva.Line({ points: [termX, termY-8, outX, this._csCY+20], stroke: '#ef5350', strokeWidth: 1.2, dash: [3,3] }));
        this.group.add(new Konva.Line({ points: [termX, termY+8, outX, this._csCY+40], stroke: '#90caf9', strokeWidth: 1.2, dash: [3,3] }));
        
        // 输入轴（较细）
        this.group.add(new Konva.Rect({ x: mCX-2.5, y: mBot, width: 5, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 0.8 }));
    ...
    _drawOutputCharacteristic() {
        ...
        this.group.add(new Konva.Text({ x: cx+4, y: cy+2, width: cw-8, text: '输出电压特性曲线 Uout = f(ω)', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this.group.add(new Konva.Text({ x: ox-18, y: oy-ah-3, text: 'Uout (V)', fontSize: 7, fill: '#80cbc4', rotation: -90 }));
        this.group.add(new Konva.Text({ x: ox+aw-10, y: oy+5, text: 'ω (rpm)', fontSize: 7, fill: '#80cbc4' }));
        ...
        for (let i = 0; i <= 4; i++) {
            ...
            this.group.add(new Konva.Line({ points: [x, oy-2, x, oy+2], stroke: '#546e7a', strokeWidth: 0.5 }));
            if (i % 2 === 0) {
                this.group.add(new Konva.Text({ x: x-5, y: oy+3, text: `${Math.round(rpm)}`, fontSize: 6, fill: '#546e7a' }));
            }
            this.group.add(new Konva.Line({ points: [ox-2, y, ox+2, y], stroke: '#546e7a', strokeWidth: 0.5 }));
            if (i % 2 === 0) {
                this.group.add(new Konva.Text({ x: ox-20, y: y-4, text: `${v.toFixed(0)}`, fontSize: 6, fill: '#546e7a', align: 'right' }));
            }
        }
        ...
        this.group.add(this._idealLine);
        
        // 实际特性线（考虑负载和温度）
        this._actualLine = new Konva.Line({ points: [], stroke: '#ffd54f', strokeWidth: 2 });
        this.group.add(this._actualLine);
        
        // 当前工作点
        this._opPoint = new Konva.Circle({ x: ox, y: oy, radius: 6, fill: '#ff7043', stroke: '#fff', strokeWidth: 1.5 });
        this.group.add(this._opPoint);
        
        // 图例
        this.group.add(new Konva.Line({ points: [cx+10, cy+25, cx+30, cy+25], stroke: '#64b5f6', strokeWidth: 1.5, dash: [6,4] }));
        this.group.add(new Konva.Text({ x: cx+33, y: cy+21, text: '理想特性', fontSize: 7, fill: '#64b5f6' }));
        this.group.add(new Konva.Line({ points: [cx+10, cy+35, cx+30, cy+35], stroke: '#ffd54f', strokeWidth: 2 }));
        this.group.add(new Konva.Text({ x: cx+33, y: cy+31, text: '实际特性', fontSize: 7, fill: '#ffd54f' }));
        this.group.add(new Konva.Circle({ x: cx+20, y: cy+47, radius: 4, fill: '#ff7043' }));
        this.group.add(new Konva.Text({ x: cx+33, y: cy+43, text: '当前工作点', fontSize: 7, fill: '#ff7043' }));
    ...
    _drawMeterPanel() {
        ...
        this.group.add(new Konva.Text({ x: mx+4, y: my+2, width: mw-8, text: '测速仪表 | 实时监测', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        cells.forEach(({ label, id, unit, color, formatter }, i) => {
            ...
            this.group.add(new Konva.Rect({ x, y, width: cellW-6, height: 46, fill: '#0d1520', stroke: '#1a3040', strokeWidth: 0.8, cornerRadius: 3 }));
            this.group.add(new Konva.Text({ x, y: y+4, width: cellW-6, text: label, fontSize: 8, fill: '#546e7a', align: 'center' }));
            ...
            this.group.add(new Konva.Text({ x, y: y+36, width: cellW-6, text: unit, fontSize: 7, fill: '#37474f', align: 'center' }));
        ...
        this.group.add(this._statusIndicator, this._statusText);
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '输出电压波形 (Uout)    转速波形 (ω)', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        
        const ox = wx + 20, oy = wy + wh - 18, aw = ww - 40, ah = wh - 48;
        
        // 坐标轴
        this.group.add(new Konva.Line({ points: [ox, oy-ah, ox, oy, ox+aw, oy], stroke: '#37474f', strokeWidth: 0.8 }));
        this.group.add(new Konva.Text({ x: ox-14, y: oy-ah, text: 'U(V)', fontSize: 7, fill: '#80cbc4' }));
        this.group.add(new Konva.Text({ x: ox+aw-10, y: oy+5, text: 't(s)', fontSize: 7, fill: '#80cbc4' }));
        ...
        this.group.add(this._waveVoltage, this._waveSpeed);
        
        // 图例
        this.group.add(new Konva.Line({ points: [wx+10, wy+20, wx+25, wy+20], stroke: '#ef5350', strokeWidth: 1.5 }));
        this.group.add(new Konva.Text({ x: wx+28, y: wy+16, text: '输出电压', fontSize: 7, fill: '#ef5350' }));
        this.group.add(new Konva.Line({ points: [wx+85, wy+20, wx+100, wy+20], stroke: '#4fc3f7', strokeWidth: 1.2 }));
        this.group.add(new Konva.Text({ x: wx+103, y: wy+16, text: '转速', fontSize: 7, fill: '#4fc3f7' }));
