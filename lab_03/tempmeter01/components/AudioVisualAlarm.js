...
export class AudioVisualAlarm extends BaseComponent {
    ...
    _drawLampLens() {
        ...
        // 灯笼棱纹（装饰性环形）
        for (let i = 1; i <= 4; i++) {
            this.group.add(new Konva.Circle({ x: cx, y: cy, radius: R*i/4, fill: 'none', stroke: `rgba(255,255,255,0.10)`, strokeWidth: 0.8 }));
        }
        // 高光反射（顶部左侧弧形）
        ...
        this.group.add(this._glowCircle, this._lensOuter, this._lensInner, this._lampCore, this._lensHighlight);
    }
    ...
    _drawBody() {
        ...
        // 顶面高光
        this.group.add(new Konva.Rect({ x: bx+2, y: by+2, width: bw-4, height: 5, fill: 'rgba(255,255,255,0.15)', cornerRadius: [3,3,0,0] }));
        // 底面暗影
        this.group.add(new Konva.Rect({ x: bx+2, y: by+bh-7, width: bw-4, height: 5, fill: 'rgba(0,0,0,0.22)', cornerRadius: [0,0,3,3] }));
        ...
        for (let r = 0; r < 3; r++) {
            for (let c = 0; c < 4; c++) {
                this.group.add(new Konva.Circle({ x: grillX+5+c*8, y: grillY+5+r*7, radius: 2.5, fill: '#1a0000', opacity: 0.5 }));
            }
        }
        this.group.add(new Konva.Text({ x: grillX, y: grillY+25, width: 36, text: `${this.soundVolume}dB`, fontSize: 7.5, fill: 'rgba(255,255,255,0.35)', align: 'center' }));

        // 接线盒指示（右下角）
        const termLblX = bx + bw - 38;
        this.group.add(new Konva.Text({ x: termLblX, y: by+bh*0.52, width: 32, text: 'IN\n▼', fontSize: 8, fill: 'rgba(255,255,255,0.40)', align: 'center', lineHeight: 1.2 }));

        this.group.add(body, nameBg, this._alarmText, this._statusBar, this._statusFill, this._statusLbl);
    }
    ...
    _drawResetButton() {
        ...
        this.group.add(this._btnText,_btnOut,this._btnBody);
    }
    ...
    _drawTerminals() {
        ...
        // 端子块背景
        this.group.add(new Konva.Rect({ x: this._bodyX, y: ty, width: this._bodyW, height: 20, fill: '#263238', stroke: '#1a2634', strokeWidth: 1, cornerRadius: [0,0,3,3] }));
        // A 端子
        ...
        this.group.add(this._termABlock,  this._termBBlock);
    }
    ...
    _drawDynamicLayers() {
        ...
        this.group.add(this._soundGroup, this._rippleGroup);
    }
