...
export class BourdonTube extends BaseComponent {
    constructor(config, sys) {
        ...
        this._tubeCy = this._cy + this._R * 0.10;
    ...
    _drawDial() {
        ...
        this.group.add(new Konva.Circle({
            x: cx, y: cy, radius: R,
            fill: '#f4f0e8', stroke: '#d8d0c0', strokeWidth: 1,
        }));
        // 表盘轻微污渍/纸质感（同心淡圆）
        this.group.add(new Konva.Circle({
            x: cx, y: cy, radius: R * 0.96,
            fill: 'transparent', stroke: '#e8e0d0', strokeWidth: 0.5,
        }));
        ...
        for (let i = 0; i <= totalTicks; i++) {
            ...
            const rOuter = R * 0.93;
            const rInner = isMajor ? R * 0.78 : (isMid ? R * 0.86 : R * 0.90);
            const sw     = isMajor ? 1.5 : 0.8;

            // 危险区（后20%）用红色
            const pct = i / totalTicks;
            const col = pct >= 0.80 ? '#c03020' : pct >= 0.60 ? '#a06010' : '#606870';
            ...
            if (isMajor) {
                ...
                this.group.add(new Konva.Text({
                    x: cx + rTxt * Math.cos(angRad) - 14,
                    y: cy + rTxt * Math.sin(angRad) - 7,
                    width: 28, height: 14,
                    text: String(val),
                    fontSize: Math.max(7, R * 0.10),
                    fill: pct >= 0.80 ? '#a02010' : '#303840',
        ...
        // 量程单位
        this.group.add(new Konva.Text({
            x: cx - R * 0.30, y: cy + R * 0.28,
            width: R * 0.60, height: 14,
            text: this.rangeUnit,
            fontSize: Math.max(7, R * 0.095),
            fill: '#505a64', align: 'center', fontStyle: 'bold',
            letterSpacing: 1,
        }));

        // 危险区弧（红色警示弧）
        const aStart = -(this._dialAngStart) + this._dialAngEnd * 0.80;
        const aEnd   = -(this._dialAngStart) + this._dialAngEnd * 1.00;
        const rArc   = R * 0.95;
        this.group.add(new Konva.Arc({
            x: cx, y: cy,
            innerRadius: rArc - 1, outerRadius: rArc + 2,
            angle: (aEnd - aStart),
            rotation: aStart,
            fill: '#c03020', opacity: 0.6,
        }));
    }
    ...
    _rebuildMechanism() {
        ...
        // ── 计算管端几何 ──
        const {tipX, tipY, tipAng, lkX, lkY, sectorAng} = this._calcGeometry(frac);

        // ── 绘制波登管（C形弧）──
        this._drawBourdontube(frac, tipX, tipY, tipAng);
    ...
    _calcGeometry(frac) {
        ...
        // 自由端坐标（在波登管弧上）
        const tipX = tcx + tR * Math.cos(tipAng);
        const tipY = tcy - tR * Math.sin(tipAng);   // SVG y轴向下
        ...
        return { tipX, tipY, tipAng, lkX, lkY, sectorAng };
    }

    _drawBourdontube(frac, tipX, tipY, tipAng) {
        const tcx  = this._tubeCx, tcy = this._tubeCy;
        const tR   = this._tubeR;
        const angF = this._tubeAngFixed;
        const ang0 = this._tubeAngFree0;
        ...
        // 波登管壁厚 → 内外半径
        const wall = tR * 0.072;
        const rOuter = tR + wall;
        const rInner = tR - wall;

        // 将数学角度转为 SVG arc 参数
        // SVG arc：从固定端 → 顺时针（大弧）→ 自由端
        const toSVG = (r, a) => ({
            x: (tcx + r * Math.cos(a)).toFixed(2),
            y: (tcy - r * Math.sin(a)).toFixed(2),   // 注意 y 翻转
        });

        const sfO = toSVG(rOuter, angF);
        const stO = toSVG(rOuter, tipAngCur);
        const sfI = toSVG(rInner, angF);
        const stI = toSVG(rInner, tipAngCur);

        // 管壁填充色：低压→钢蓝，高压→铜橙
        const r = Math.round(60  + frac * 150);
        const g = Math.round(100 - frac * 40);
        const b = Math.round(180 - frac * 120);
        const wallFill = `rgba(${r},${g},${b},0.28)`;

        // 管截面路径（外弧 + 封端 + 内弧回）
        const outerArc = `M${sfO.x},${sfO.y} A${rOuter},${rOuter} 0 1,0 ${stO.x},${stO.y}`;
        const innerArc = `L${stI.x},${stI.y} A${rInner},${rInner} 0 1,1 ${sfI.x},${sfI.y} Z`;
        const tubePath = outerArc + innerArc;

        // 管体填充
        this._mechGroup.add(new Konva.Path({
            data: tubePath,
            fill: wallFill, stroke: 'none',
        }));

        // 外壁轮廓线
        this._mechGroup.add(new Konva.Path({
            data: `M${sfO.x},${sfO.y} A${rOuter},${rOuter} 0 1,0 ${stO.x},${stO.y}`,
            fill: 'none',
            stroke: frac >= 0.80 ? '#b04030' : '#5a7090',
            strokeWidth: 1.4,
        }));
        // 内壁轮廓线
        this._mechGroup.add(new Konva.Path({
            data: `M${sfI.x},${sfI.y} A${rInner},${rInner} 0 1,0 ${stI.x},${stI.y}`,
            fill: 'none',
            stroke: frac >= 0.80 ? '#903020' : '#486080',
            strokeWidth: 0.8,
        }));
        ...
        this._mechGroup.add(new Konva.Ellipse({
            ...
            radiusX: hlR, radiusY: hlR * 1.6,
            fill: '#7090b0', stroke: '#4a6a8a', strokeWidth: 0.8, opacity: 0.7,
        }));
        this._mechGroup.add(new Konva.Ellipse({
            x: tipX, y: tipY,
            radiusX: hlR, radiusY: hlR * 1.6,
            fill: '#809ab8', stroke: '#5a7a9a', strokeWidth: 0.8, opacity: 0.7,
        }));

        // 压力流体粒子（在管腔中心线上滚动，压力 > 5% 时显示）
        if (frac > 0.05) {
            const steps = 6;
            for (let i = 0; i < steps; i++) {
                ...
                const px = tcx + tR * Math.cos(angPt);
                const py = tcy - tR * Math.sin(angPt);
                const alpha = 0.25 + 0.50 * Math.sin(fi * Math.PI);
                this._mechGroup.add(new Konva.Circle({
                    x: px, y: py,
                    radius: wall * 0.38,
                    fill: `rgba(${r + 60},${g + 60},${b + 60},${alpha})`,
                }));
            }
        }
    }
    ...
    _drawSectorGear(sectorAng) {
        ...
        for (let i = 0; i <= teethN; i++) {
            ...
            this._mechGroup.add(new Konva.Line({
                points: [rx, ry, tx, ty],
                stroke: '#7080b0', strokeWidth: 1.5, lineCap: 'round',
            }));
        }
