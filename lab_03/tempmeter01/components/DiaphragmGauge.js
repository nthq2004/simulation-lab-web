...
export class DiaphragmGauge extends BaseComponent {
    constructor(config, sys) {
        ...
        this.rangeMax   = config.rangeMax   || 25;       // 量程上限（kPa）
        ...
        this._maxDefl   = this._capsuleH * 0.28;
    ...
    _drawCapsuleBody() {
        ...
        this.group.add(new Konva.Rect({
            ...
            fillLinearGradientStartPoint: { x: 0, y: 0 },
            fillLinearGradientEndPoint:   { x: w - 4, y: 0 },
            fillLinearGradientColorStops: [
                0,   '#7a6a28',
                0.3, '#c0a040',
                0.6, '#d8b848',
        ...
        this.group.add(new Konva.Rect({
            ...
            fill: '#12121a', stroke: '#2a2a38', strokeWidth: 0.5,
        }));
        // 连接颈（膜盒与表壳之间的短颈，黄铜色）
        const neckW = this._socketW * 0.80;
        this.group.add(new Konva.Rect({
            x: this._cx - neckW / 2,
            y: this._cy + this._R + 2,
            width: neckW,
            height: this._capsuleY - (this._cy + this._R + 2) + 2,
            fillLinearGradientStartPoint: { x: 0, y: 0 },
            fillLinearGradientEndPoint:   { x: neckW, y: 0 },
            fillLinearGradientColorStops: [
                0,   '#6a5a20',
                0.4, '#b89030',
                0.6, '#c8a038',
        ...
        this.group.add(new Konva.Circle({
            ...
            radius: 2.5,
            fill: '#0a0a12', stroke: '#3a3a48', strokeWidth: 0.5,
        }));
    }
    ...
    _drawDial() {
        ...
        for (let i = 0; i <= totalTicks; i++) {
            ...
            // 主刻度数字
            if (isMajor) {
                const val  = Math.round(frac * this.rangeMax);
                const rTxt = R * 0.67;
        ...
        // "DIAPHRAGM" 类型标注（区别于波登管）
        this.group.add(new Konva.Text({
            x: cx - R * 0.45, y: cy - R * 0.50,
            width: R * 0.90, height: 12,
            text: 'DIAPHRAGM',
            fontSize: Math.max(6, R * 0.080),
            fill: '#8090a0', align: 'center', letterSpacing: 1,
        }));
        ...
        this.group.add(new Konva.Arc({
            x: cx, y: cy,
            innerRadius: R * 0.94 - 1, outerRadius: R * 0.94 + 2,
            angle: aEnd - aStart,
            rotation: aStart,
    ...
    _calcGeometry(frac) {
        ...
        const rodTopY = topMemY;                   // 推杆上端跟随上膜片
    ...
    _drawDiaphragm(frac, geo) {
        ...
        if (frac > 0.10) {
            ...
            this._mechGroup.add(new Konva.Line({
                points: [arrowX, baseY, arrowX, curY + 4],
                stroke: `rgba(${r},${g},${b},0.60)`,
                strokeWidth: 1, lineCap: 'round',
            }));
            this._mechGroup.add(new Konva.Line({
                points: [arrowX - 3, curY + 6, arrowX, curY + 1, arrowX + 3, curY + 6],
                stroke: `rgba(${r},${g},${b},0.60)`,
    ...
    _drawPushRod(frac, geo) {
        const { topMemY, rodTopY } = geo;
        const cx = this._memCx;
        // 推杆从膜片中心向上穿出上盖，伸入表盘区域
        const rodBottomY = this._memTopY0;   // 零压参考基准（上盖穿孔处）
        const rodTopActY = topMemY - 2;      // 推杆上端（连杆铰接点）
        ...
        const guideH = 10;
        this._mechGroup.add(new Konva.Rect({
            x: cx - 2.5, y: rodBottomY - guideH / 2,
