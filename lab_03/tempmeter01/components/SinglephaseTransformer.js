...
export class SinglePhaseTransformer extends BaseComponent {
    ...
    _drawCore() {
        ...
        // 轭铁高光
        this.group.add(new Konva.Rect({ x: cx+2, y: cy+2, width: cw-4, height: 4, fill: 'rgba(255,255,255,0.12)', cornerRadius: [1,1,0,0] }));
        this.group.add(new Konva.Rect({ x: cx+2, y: cy+ch-yk+2, width: cw-4, height: 4, fill: 'rgba(255,255,255,0.08)' }));

        // 铁芯标注
        this.group.add(new Konva.Text({ x: cx+lw+4, y: lly+lh/2-6, text: '铁芯', fontSize: 9, fontStyle: 'bold', fill: 'rgba(255,255,255,0.3)', align: 'center', width: cw-lw*3 }));

        this.group.add(topYoke, botYoke, leftLeg, rightLeg, this._midLegRect);
    }
    ...
    _drawCoreLaminations() {
        ...
        for (let i = 2; i < yk-1; i += 3) {
            this.group.add(new Konva.Line({ points: [cx+2, cy+i, cx+cw-2, cy+i], stroke: 'rgba(0,0,0,0.15)', strokeWidth: 0.7 }));
        }
        // 下轭铁叠片纹
        for (let i = 2; i < yk-1; i += 3) {
            this.group.add(new Konva.Line({ points: [cx+2, cy+ch-yk+i, cx+cw-2, cy+ch-yk+i], stroke: 'rgba(0,0,0,0.15)', strokeWidth: 0.7 }));
        }
        // 左腿叠片纹
        for (let i = 2; i < lh-1; i += 3) {
            this.group.add(new Konva.Line({ points: [cx+2, lly+i, cx+lw-2, lly+i], stroke: 'rgba(0,0,0,0.15)', strokeWidth: 0.7 }));
        }
        // 右腿叠片纹
        for (let i = 2; i < lh-1; i += 3) {
            this.group.add(new Konva.Line({ points: [cx+cw-lw+2, lly+i, cx+cw-2, lly+i], stroke: 'rgba(0,0,0,0.15)', strokeWidth: 0.7 }));
        }
        // 中心柱叠片纹
        for (let i = 2; i < lh-1; i += 3) {
            this.group.add(new Konva.Line({ points: [this._midLegX+2, lly+i, this._midLegX+this._legW-2, lly+i], stroke: 'rgba(0,0,0,0.18)', strokeWidth: 0.7 }));
        }
    }
    ...
    _drawLeakageFluxLayer() {
        ...
        this.group.add(this._leakGroup);
    }
    ...
    _drawWindings() {
        ...
        // 原绕组引线
        this.group.add(new Konva.Line({ points: [w1X1, coilY, w1X1-12, coilY, w1X1-12, midY-18], stroke: '#ff8f00', strokeWidth: 2, lineCap: 'round' }));
        this.group.add(new Konva.Line({ points: [w1X1, coilY+coilH, w1X1-12, coilY+coilH, w1X1-12, midY+18], stroke: '#ff8f00', strokeWidth: 2, lineCap: 'round' }));
        // 端子螺栓
        this.group.add(new Konva.Circle({ x: w1X1-12, y: midY-18, radius: 4.5, fill: '#37474f', stroke: '#263238', strokeWidth: 1 }));
        this.group.add(new Konva.Circle({ x: w1X1-12, y: midY+18, radius: 4.5, fill: '#37474f', stroke: '#263238', strokeWidth: 1 }));
        // 原绕组标注
        this.group.add(new Konva.Text({ x: w1X1-32, y: coilY-18, text: `N₁=${this.N1}`, fontSize: 9, fontStyle: 'bold', fill: '#ff8f00' }));
        this.group.add(new Konva.Text({ x: w1X1-28, y: coilY+coilH+4, text: '原边', fontSize: 8.5, fill: '#ff8f00' }));
        ...
        // 副绕组引线
        this.group.add(new Konva.Line({ points: [w2X2, coilY, w2X2+12, coilY, w2X2+12, midY-18], stroke: '#42a5f5', strokeWidth: 2, lineCap: 'round' }));
        this.group.add(new Konva.Line({ points: [w2X2, coilY+coilH, w2X2+12, coilY+coilH, w2X2+12, midY+18], stroke: '#42a5f5', strokeWidth: 2, lineCap: 'round' }));
        // 端子螺栓
        this.group.add(new Konva.Circle({ x: w2X2+12, y: midY-18, radius: 4.5, fill: '#37474f', stroke: '#263238', strokeWidth: 1 }));
        this.group.add(new Konva.Circle({ x: w2X2+12, y: midY+18, radius: 4.5, fill: '#37474f', stroke: '#263238', strokeWidth: 1 }));
        // 副绕组标注
        this.group.add(new Konva.Text({ x: w2X2+16, y: coilY-18, text: `N₂=${this.N2}`, fontSize: 9, fontStyle: 'bold', fill: '#42a5f5' }));
        this.group.add(new Konva.Text({ x: w2X2+16, y: coilY+coilH+4, text: '副边', fontSize: 8.5, fill: '#42a5f5' }));
        ...
        this.group.add(this._priCurrentGroup, this._secCurrentGroup);
    }
    ...
    _drawEquivCircuit() {
        ...
        // 磁通符号 Φ
        this.group.add(new Konva.Text({ x: cx2-8, y: cy2, text: 'Φ', fontSize: 13, fontStyle: 'bold', fill: 'rgba(79,195,247,0.6)' }));
        ...
        // 变比
        this.group.add(new Konva.Text({ x: cx2-18, y: cy2+28, width: 36, text: `k=${this.k.toFixed(2)}`, fontSize: 8, fontFamily: 'Courier New, monospace', fill: 'rgba(255,255,255,0.35)', align: 'center' }));
        // 频率
        this.group.add(new Konva.Text({ x: cx2-18, y: cy2+40, width: 36, text: `${this.frequency}Hz`, fontSize: 8, fontFamily: 'Courier New, monospace', fill: 'rgba(255,255,255,0.25)', align: 'center' }));

        this.group.add(this._phiLabel);
    }
    ...
    _drawInstrHead() {
        ...
        this.group.add(new Konva.Text({ x: hx+7, y: hy+17, width: hw-14, text: `${this.V1rated}/${this.V2rated}V`, fontSize: 7, fill: '#78909c', align: 'center' }));
        this.group.add(new Konva.Text({ x: hx+7, y: hy+27, width: hw-14, text: `${this.ratedKVA}kVA  ${this.frequency}Hz`, fontSize: 7, fill: '#90a4ae', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this.group.add(ring, this._lcdBg, this._loadArc, this._lcdMain, this._lcdUnit, this._lcdEta, this._lcdV1, this._lcdV2);
    }
    ...
    _drawKnobs() {
        ...
        this.group.add(base, this._knobRotor, new Konva.Text({ x: kx-18, y: ky+21, width: 36, text: '负载调节', fontSize: 8.5, fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'V₁(原边)  V₂(副边)  I₁(原边)  I₂(副边)', fontSize: 8, fontStyle: 'bold', fill: '#4dd0e1', align: 'center' }));
        ...
        lbls.forEach((l, i) => { this.group.add(new Konva.Text({ x: wx+4, y: wy+13+h4*i+4, text: l, fontSize: 8, fill: cols[i] })); });
        ...
        this.group.add(bg, titleBg, this._wLineV1, this._wLineV2, this._wLineI1, this._wLineI2, this._wV1Lbl, this._wV2Lbl, this._wI1Lbl, this._wI2Lbl);
        this._wavH4 = h4;
    }
    ...
    _drawLoadPanel() {
        ...
        this.group.add(new Konva.Text({ x: hx+2, y: panY+2, width: hw-4, text: '负载参数', fontSize: 8, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
        ...
        this.group.add(bg, this._panelI1, this._panelI2, this._panelP, this._panelPfe);
    }
