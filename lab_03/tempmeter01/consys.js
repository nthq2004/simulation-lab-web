...
import { LeakDetector, AirBottle, PressRegulator, PressMeter, TeeConnector, StopValve, Pump, Cooler, Engine, WaterTankSystem, WaterTankTwoPos, WaterTankLevelControl, DiffTransmitter, BubbleLevelTransmitter, PIDController, OvenSystem, ElecValve, LVDTPressureSensor, TempTransmitter, PressTransmitter, SmartPressTransmitter, Rosemount475, VoltageTransmitter, DCPower, AmpMeter, VariResistor, Resistor, Multimeter, OpAmp, Ground, Monitor, ProcessCalibrator, VoltageRelay, ACPower, Oscilloscope_tri, Oscilloscope, SignalGenerator, Capacitor, JFET, Diode, Transistor, NpnTempSensor, RealResistor, RealVariResistor, CoolingSystem, FuelOilHeater, PressRelay, AudioVisualAlarm, NormallyClosedPushButton, NormallyOpenPushButton, BimetallicThermometer, MercuryThermometer, TempMeter, IRThermometer, HandheldTempMeter } from './export.js';
...
export class ControlSystem {
    ...
    init() {
        ...
        const componentConfigs = [
            ...
            // ── 双金属温度计 ──
            { Class: BimetallicThermometer, id: 'bimetal', x: 250, y: 50,
              width: 280, height: 320, tempMin: -40, tempMax: 60,
              initTemp: 0, label: 'TI-101' },
            // ── 水银温度计 ──
            { Class: MercuryThermometer, id: 'mercury', x: 750, y: 50,
              width: 120, height: 460, tempMin: -10, tempMax: 110,
              initTemp: 0, label: 'TT-201', mode: 'industrial' },
            // ── 感温包驱动温度表 ──
            { Class: TempMeter, id: 'tempmeter', x: 1200, y: 180,
              radius: 80, min: 0, max: 120, title: '感温包温度表' },
            // ── 手持式红外测温仪 ──
            { Class: IRThermometer, id: 'handheld', x: 1420, y: 100,
              width: 230, height: 340, initTemp: 0, label: 'TIR-301' },
    ...
    _initTempSlider() {
        ...
        slider.addEventListener('input', () => {
            ...
            if (this.comps['bimetal'] && this.comps['bimetal'].setTemperature) {
                this.comps['bimetal'].setTemperature(temp);
            }
            if (this.comps['mercury'] && this.comps['mercury'].setTemperature) {
                this.comps['mercury'].setTemperature(temp);
            }
            if (this.comps['tempmeter'] && this.comps['tempmeter'].update) {
                this.comps['tempmeter'].update(temp);
            }
            if (this.comps['handheld'] && this.comps['handheld'].update) {
                this.comps['handheld'].update(temp);
            }
            this.requestRedraw();
        });
    }
