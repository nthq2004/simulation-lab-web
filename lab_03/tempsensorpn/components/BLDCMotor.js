...
export class BLDCMotor extends BaseComponent {
    ...
    _drawMotorSection() {
        ...
        [['H1',52],['H2',66],['H3',80]].forEach(([l,dy]) => {
            this.group.add(new Konva.Line({ points: [ecx+sOuter*0.5,ecy+dy,termX,ecy+dy], stroke:'#ffd54f', strokeWidth:1.5, dash:[3,3] }));
            this.group.add(new Konva.Circle({ x:termX, y:ecy+dy, radius:3, fill:'#ffd54f' }));
            this.group.add(new Konva.Text({ x:termX+5, y:ecy+dy-5, text:l, fontSize:8, fill:'#ffd54f' }));
        });
        ...
        this.group.add(new Konva.Text({ x:ex-28, y:ecy-26, text:'+', fontSize:10, fill:'#ef5350', fontStyle:'bold' }));
        this.group.add(new Konva.Text({ x:ex-28, y:ecy+16, text:'−', fontSize:10, fill:'#90caf9', fontStyle:'bold' }));
    ...
    _drawMagnetLayer() {
        ...
        this.group.add(this._magnetGroup);
    }
    ...
    _drawInverterTopology() {
        ...
        this.group.add(new Konva.Rect({ x:ix, y:iy, width:iw, height:ih, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:ix, y:iy, width:iw, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:ix+4, y:iy+2, width:iw-8, text:'三相逆变器（六步PWM换向）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        // 直流母线
        const busY_p = iy + 20, busY_n = iy + ih - 14;
        this.group.add(new Konva.Line({ points:[ix+8,busY_p,ix+iw-8,busY_p], stroke:'#ef5350', strokeWidth:2.5 }));
        this.group.add(new Konva.Line({ points:[ix+8,busY_n,ix+iw-8,busY_n], stroke:'#90caf9', strokeWidth:2.5 }));
        this.group.add(new Konva.Text({ x:ix+8, y:busY_p-10, text:`+${this.ratedVoltage}V`, fontSize:8, fill:'#ef5350' }));
        this.group.add(new Konva.Text({ x:ix+8, y:busY_n+3,  text:'GND', fontSize:8, fill:'#90caf9' }));
        ...
        for (let ph = 0; ph < 3; ph++) {
            ...
            // 相线（母线中点引出）
            this.group.add(new Konva.Line({ points:[cx3,busY_p+4,cx3,busY_n-4], stroke:'#546e7a', strokeWidth:1 }));
            // 相输出线
            this.group.add(new Konva.Line({ points:[cx3,midY,ix+iw+4,midY-24+ph*24], stroke:col, strokeWidth:1.5, dash:[3,3] }));
            ...
            // 相标注
            this.group.add(new Konva.Text({ x:cx3-6, y:midY-6, text:phLabels[ph], fontSize:10, fill:col, fontStyle:'bold' }));
        }

        // PWM 占空比指示条
        const barY = iy + ih - 28;
        this.group.add(new Konva.Text({ x:ix+8, y:barY-10, text:'PWM 占空比:', fontSize:7.5, fill:'#546e7a' }));
        this.group.add(new Konva.Rect({ x:ix+8, y:barY, width:iw-16, height:8, fill:'#0a0a18', cornerRadius:2 }));
        ...
        this.group.add(this._pwmBar, this._pwmTxt);
    }
    ...
    _drawCommutationTable() {
        ...
        this.group.add(new Konva.Text({ x:cx+4, y:cy+2, width:cw-8, text:'六步换向表（霍尔状态→导通相）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        [['步骤',0.06],['Hall',0.22],['高侧',0.42],['低侧',0.62],['状态',0.80]].forEach(([h,xr]) => {
            this.group.add(new Konva.Text({ x:cx+cw*xr, y:hY, text:h, fontSize:7.5, fill:'#546e7a', fontStyle:'bold' }));
        });
        this.group.add(new Konva.Line({ points:[cx+4, hY+10, cx+cw-4, hY+10], stroke:'#1a3040', strokeWidth:0.8 }));
        ...
        steps.forEach(({ hall, step, hi, lo, color }, i) => {
            ...
            this.group.add(rowBg);
            this.group.add(new Konva.Text({ x:cx+cw*0.06, y:ry+2, text:`${step}`, fontSize:9, fontStyle:'bold', fill:color }));
            this.group.add(new Konva.Text({ x:cx+cw*0.18, y:ry+2, text:hall,    fontSize:8, fill:'#ffd54f', fontFamily:'Courier New, monospace' }));
            this.group.add(new Konva.Text({ x:cx+cw*0.40, y:ry+2, text:hi,      fontSize:9, fontStyle:'bold', fill:'#ef9a9a' }));
            this.group.add(new Konva.Text({ x:cx+cw*0.60, y:ry+2, text:lo,      fontSize:9, fontStyle:'bold', fill:'#90caf9' }));
            const stDot= new Konva.Circle({ x:cx+cw*0.85, y:ry+rowH/2-1, radius:5, fill:'#263238', stroke:color, strokeWidth:1 });
            this.group.add(stDot);
            this._commRows.push({ bg: rowBg, dot: stDot, color });
        });

        // 当前换向步骤指针
        this._commPointer = new Konva.Text({ x:cx+4, y:cy+28, text:'▶', fontSize:10, fill:'#ffd54f' });
        this.group.add(this._commPointer);
        this._commRowH = rowH;
    ...
    _drawTNCurve() {
        ...
        this.group.add(new Konva.Text({ x:tx+4, y:ty+2, width:tw-8, text:'T-n 特性曲线', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        duties.forEach((D, di) => {
            ...
            if (pts.length > 4)
                this.group.add(new Konva.Line({ points:pts, stroke:dColors[di], strokeWidth:1.2, lineJoin:'round', opacity:0.65 }));
        });
        this.group.add(new Konva.Text({ x:ox+aw*0.6, y:ty+14, text:'D=100%', fontSize:6.5, fill:'#ef9a9a' }));
        this.group.add(new Konva.Text({ x:ox+aw*0.6, y:ty+23, text:'D=25%',  fontSize:6.5, fill:'#90caf9' }));

        // 额定工作点
        const rNX = ox + (this.ratedSpeed*2*Math.PI/60 / this.syncOmegaMax)*aw;
        const rTY = oy - (this.ratedTorque/this.maxTorque)*(ah-4);
        this.group.add(new Konva.Circle({ x:rNX, y:rTY, radius:3, fill:'#ffd54f', stroke:'#f9a825', strokeWidth:1 }));
        this.group.add(new Konva.Text({ x:rNX+3, y:rTY-9, text:'N', fontSize:6.5, fill:'#ffd54f' }));
    ...
    _drawEfficiencyMap() {
        ...
        this.group.add(new Konva.Rect({ x:mx, y:my, width:mw, height:mh, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:mx, y:my, width:mw, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:mx+4, y:my+2, width:mw-8, text:'效率 MAP（η%）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ox = mx+12, oy = my+mh-12, aw = mw-18, ah = mh-26;
        this.group.add(new Konva.Line({ points:[ox,oy-ah,ox,oy,ox+aw,oy], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Text({ x:ox-10, y:oy-ah, text:'T', fontSize:7, fill:'#80cbc4' }));
        this.group.add(new Konva.Text({ x:ox+aw+2, y:oy+2, text:'n', fontSize:7, fill:'#80cbc4' }));
        ...
        [[0.60,'#ef5350'],[0.75,'#ffa726'],[0.85,'#ffd54f'],[0.92,'#66bb6a'],[0.95,'#4fc3f7']].forEach(([eta, col]) => {
            ...
            if (pts.length > 4) {
                this.group.add(new Konva.Line({ points:pts, stroke:col, strokeWidth:1, lineJoin:'round', opacity:0.5 }));
                if (pts.length >= 4)
                    this.group.add(new Konva.Text({ x:pts[0]+2, y:pts[1]-8, text:`${(eta*100).toFixed(0)}%`, fontSize:6.5, fill:col }));
            }
        });

        // 动态工作点
        this._mapPoint = new Konva.Circle({ x:ox+aw*0.5, y:oy-ah*0.5, radius:5, fill:'#fff', stroke:'#000', strokeWidth:1.5 });
        this.group.add(this._mapPoint);
    ...
    _drawControlPanel() {
        ...
        this.group.add(this._modeBtn, this._modeTxt);
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({ x:wx+4, y:wy+1, width:ww-8, text:'反电动势（U/V/W 梯形波）   三相电流（U/V/W）   电磁转矩 T   霍尔信号 H1/H2/H3', fontSize:8, fill:'#80cbc4', align:'center' }));
        ...
        ['BEMF', 'I(uvw)', 'T_em', 'Hall'].forEach((l, i) => {
            this.group.add(new Konva.Text({ x:wx+4, y:wy+12+hCh*i+3, text:l, fontSize:8, fill:['#e53935','#ef9a9a','#ffd54f','#ffd54f'][i] }));
        });
        this.group.add(this._wLEU, this._wLEV, this._wLEW, this._wLIU, this._wLIV, this._wLIW, this._wLT, this._wLH1, this._wLH2, this._wLH3);
        this._wavHCh = hCh;
    }
