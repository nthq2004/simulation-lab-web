...
export class IRThermometer extends BaseComponent {
    constructor(config, sys) {
        ...
        this._portSignal = {
            x: W * 0.34,
            y: this._handle.y + this._handle.h + 4,
        };
        this._portAlarm  = { x: W * 0.52, y: this._handle.y + this._handle.h + 4 };
        this._portVCC    = { x: W * 0.70, y: this._handle.y + this._handle.h + 4 };
        this._portGND    = { x: W * 0.25, y: this._handle.y + this._handle.h + 4 };
        ...
        // 端口注册
        this.addPort(this._portSignal.x, this._portSignal.y, 'pin_signal', 'wire', 'SIG');
        this.addPort(this._portAlarm.x,  this._portAlarm.y,  'pin_alarm',  'wire', 'ALM');
        this.addPort(this._portVCC.x,    this._portVCC.y,    'pin_vcc',    'wire', 'VCC');
        this.addPort(this._portGND.x,    this._portGND.y,    'pin_gnd',    'wire', 'GND');
    }
    ...
    _drawBody() {
        ...
        this.group.add(new Konva.Rect({
            ...
            fill: '#1c2028', stroke: '#0e1216', strokeWidth: 0.8,
            cornerRadius: 3,
        }));
        // 电池仓标识
        ...
        [0.25, 0.75].forEach(fx => {
            this.group.add(new Konva.Circle({
                ...
                radius: 2.5, fill: '#282e38', stroke: '#0a0e14', strokeWidth: 0.6,
            }));
            this.group.add(new Konva.Line({
        ...
        this.group.add(new Konva.Text({
            ...
            fontSize: 7, fill: 'rgba(120,160,200,0.60)',
            align: 'center', fontFamily: 'Courier New',
        }));
    }
    ...
    _rebuildLCD() {
        ...
        this._lcdGroup.add(new Konva.Text({
            x: lcd.x + 6, y: subY,
            text: `ε=${this.emissivity.toFixed(2)}`,
            fontSize: subFontSize, fill: '#00a080',
            fontFamily: 'Courier New',
        }));

        // D:S 值
        this._lcdGroup.add(new Konva.Text({
            x: lcd.x + lcd.w * 0.28, y: subY,
            text: `D:S ${this.dsRatio}:1`,
            fontSize: subFontSize, fill: '#0090a0',
            fontFamily: 'Courier New',
        }));
