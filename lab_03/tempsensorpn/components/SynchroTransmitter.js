...
export class SynchroTransmitter extends BaseComponent {
    ...
    _drawSynchroSection() {
        ...
        this.group.add(new Konva.Ring({ x: ecx, y: ecy, innerRadius: slipR1, outerRadius: slipR2, fill: 'rgba(200,120,50,0.3)', stroke: '#a06020', strokeWidth: 0.8 }));
        this.group.add(new Konva.Text({ x: ecx-20, y: ecy-slipR2-12, text: '集电环', fontSize: 7, fill: '#c87832' }));

        // 励磁引线（左侧端子）
        const termX = ex - 10;
        [['R1','#ef9a9a',-20],['R2','#90caf9',20]].forEach(([l,c,dy]) => {
            this.group.add(new Konva.Line({ points: [termX, ecy+dy, ex+8, ecy+dy], stroke: c, strokeWidth: 2, dash: [3,3] }));
            this.group.add(new Konva.Circle({ x: termX, y: ecy+dy, radius: 3.5, fill: c }));
            this.group.add(new Konva.Text({ x: ex-25, y: ecy+dy-5, text: l, fontSize: 8, fill: c, fontStyle: 'bold' }));
        });

        // 定子输出端子（右侧）
        const outX = ex + ew + 10;
        [['S1','#e53935',-28],['S2','#43a047',0],['S3','#1e88e5',28]].forEach(([l,c,dy]) => {
            this.group.add(new Konva.Line({ points: [ecx+sOuter*0.65, ecy+dy, outX, ecy+dy], stroke: c, strokeWidth: 2 }));
            this.group.add(new Konva.Circle({ x: outX, y: ecy+dy, radius: 3.5, fill: c }));
            this.group.add(new Konva.Text({ x: outX+5, y: ecy+dy-6, text: l, fontSize: 9, fill: c, fontStyle: 'bold' }));
        });

        // 轴标注（底部）
        this.group.add(new Konva.Rect({ x: ecx-5, y: ey+eh, width: 10, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this.group.add(new Konva.Text({ x: ecx-16, y: ey+eh+12, text: '角度输入轴', fontSize: 7, fill: '#546e7a' }));
    }
    ...
    _drawRotorLayer() {
        ...
        this.group.add(this._rotorAngleLine);
    }
    ...
    _drawExcitCircuit() {
        ...
        this.group.add(new Konva.Text({ x:ex+4, y:ey+2, width:ew-8, text:'励磁回路（R1-R2）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        // 上下导线
        this.group.add(new Konva.Line({ points:[x0,y0,x1,y0], stroke:'#ef9a9a', strokeWidth:1.5 }));
        this.group.add(new Konva.Line({ points:[x0,y1,x1,y1], stroke:'#90caf9', strokeWidth:1.5 }));
        // 左侧竖线（电压源）
        this.group.add(new Konva.Line({ points:[x0,y0,x0,y1], stroke:'#37474f', strokeWidth:1 }));
        // 电压源符号（正弦波）
        const srcPts = [];
        for (let i = 0; i <= 20; i++) {
        ...
        this.group.add(new Konva.Circle({ x:x0, y:(y0+y1)/2, radius:(y1-y0)*0.30, fill:'#0a1a28', stroke:'#ef9a9a', strokeWidth:1.5 }));
        this.group.add(new Konva.Line({
        ...
        this.group.add(new Konva.Text({ x:x0-(y1-y0)*0.32, y:(y0+y1)/2-6, text:'~\nU_R', fontSize:8, fill:'#ef9a9a', align:'center', lineHeight:1.2 }));

        // 电阻 R（锯齿）
        const rX1=xm-20, rX2=xm;
        this._drawResistorH(rX1, y0, rX2-rX1, 6, '#ffa726');
        this.group.add(new Konva.Text({ x:rX1, y:y0-14, width:rX2-rX1, text:`R\n${this.R_rotor}Ω`, fontSize:7, fill:'#ffa726', align:'center', lineHeight:1.3 }));

        // 电感 L（线圈）
        const lX1=rX2+4, lX2=x1-4;
        this._drawInductorH(lX1, y0, lX2-lX1, 5, '#ffd54f');
        this.group.add(new Konva.Text({ x:lX1, y:y0-14, width:lX2-lX1, text:`L\n${this.L_rotor}H`, fontSize:7, fill:'#ffd54f', align:'center', lineHeight:1.3 }));

        // 右侧线圈（自整角机转子绕组）
        this.group.add(new Konva.Line({ points:[x1,y0,x1,y1], stroke:'#37474f', strokeWidth:1 }));
        this.group.add(new Konva.Rect({ x:x1-16, y:(y0+y1)/2-(y1-y0)*0.30, width:12, height:(y1-y0)*0.60, fill:'#1a1000', stroke:'#c87832', strokeWidth:1, cornerRadius:2 }));
        this.group.add(new Konva.Text({ x:x1-22, y:(y0+y1)/2-8, text:'励磁\n绕组', fontSize:6.5, fill:'#c87832', lineHeight:1.3 }));
        ...
        this.group.add(this._excVLabel, this._excILabel, this._excCosPhi);
    }
    ...
    _drawAmplitudeCurve() {
        ...
        this.group.add(new Konva.Rect({ x:ax, y:ay, width:aw, height:ah, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:ax, y:ay, width:aw, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:ax+4, y:ay+2, width:aw-8, text:'角度-幅值特性（E_Si vs θ）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ox=ax+14, oy=ay+ah-12, aw2=aw-20, ah2=ah-26;
        // 坐标轴
        this.group.add(new Konva.Line({ points:[ox,oy-ah2,ox,oy,ox+aw2,oy], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Text({ x:ox-13, y:oy-ah2, text:'E(V)', fontSize:7, fill:'#80cbc4' }));
        this.group.add(new Konva.Text({ x:ox+aw2+2, y:oy+2, text:'θ(°)', fontSize:7, fill:'#80cbc4' }));

        // 0°/90°/180°/270°/360° 刻度
        [0,90,180,270,360].forEach(deg => {
            const x2=ox+(deg/360)*aw2;
            this.group.add(new Konva.Line({ points:[x2,oy,x2,oy+3], stroke:'#37474f', strokeWidth:0.7 }));
            this.group.add(new Konva.Text({ x:x2-8, y:oy+4, text:`${deg}°`, fontSize:6, fill:'#37474f', width:16, align:'center' }));
        });
        ...
        this.group.add(new Konva.Line({ points:[ox,zero_line,ox+aw2,zero_line], stroke:'#37474f', strokeWidth:0.5, dash:[3,3] }));
        ...
        offsets.forEach((off, i) => {
            ...
            this.group.add(new Konva.Line({ points:pts, stroke:cols[i], strokeWidth:1.5, lineJoin:'round', opacity:0.7 }));
        });

        // 零位线
        this.group.add(new Konva.Line({ points:[ox,oy-ah2/2,ox+aw2,oy-ah2/2], stroke:'#37474f', strokeWidth:0.5, dash:[2,4] }));
        this.group.add(new Konva.Text({ x:ox+2, y:oy-ah2/2-8, text:'0', fontSize:6.5, fill:'#546e7a' }));

        // 图例
        [['S1','#e53935'],['S2','#43a047'],['S3','#1e88e5']].forEach(([l,c],i) => {
            this.group.add(new Konva.Line({ points:[ax+6, ay+14+i*9+3, ax+16, ay+14+i*9+3], stroke:c, strokeWidth:1.8 }));
            this.group.add(new Konva.Text({ x:ax+18, y:ay+14+i*9-1, text:l, fontSize:7, fill:c }));
        });
    ...
    _drawPolarDisplay() {
        ...
        this.group.add(new Konva.Text({ x:px+4, y:py+2, width:pw-8, text:'极坐标（角度编码可视化）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        [0.25,0.5,0.75,1.0].forEach(r => {
            this.group.add(new Konva.Circle({ x:ocx, y:ocy, radius:R*r, fill:'rgba(0,0,0,0)', stroke:'#1a3040', strokeWidth:0.6 }));
        });
        for (let i = 0; i < 12; i++) {
            const a = (i/12)*Math.PI*2;
            this.group.add(new Konva.Line({ points:[ocx,ocy,ocx+R*Math.cos(a),ocy+R*Math.sin(a)], stroke:'#1a3040', strokeWidth:0.5 }));
        }
        // 角度标注
        [0,90,180,270].forEach(d => {
            const a = (d-90)*Math.PI/180;
            this.group.add(new Konva.Text({ x:ocx+(R+6)*Math.cos(a)-6, y:ocy+(R+6)*Math.sin(a)-6, text:`${d}°`, fontSize:6.5, fill:'#37474f' }));
        });
        this.group.add(new Konva.Text({ x:ocx-8, y:py+ph-14, text:'θ 编码方向（顺时针）', fontSize:7, fill:'#546e7a' }));
        ...
        this.group.add(this._polDecoded, this._polVector, this._polAngleText, this._polDecodedText);
    ...
    _drawPhasorDiagram() {
        ...
        this.group.add(new Konva.Rect({ x:px, y:py, width:pw, height:ph, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:px, y:py, width:pw, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:px+4, y:py+2, width:pw-8, text:'三相电压相量图', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        this.group.add(this._phaPhi, this._phaS1, this._phaS2, this._phaS3);
    }
    ...
    _drawScottT() {
        ...
        this.group.add(new Konva.Text({ x:sx+4, y:sy+2, width:sw-8, text:'Scott-T 变换（三相→正交双相）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ocx=sx+sw*0.50, ocy=sy+sh*0.58;
        const R  =Math.min(sw,sh)*0.28;

        // α-β 坐标轴（正交）
        this.group.add(new Konva.Line({ points:[sx+6,ocy,sx+sw-6,ocy], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Line({ points:[ocx,sy+14,ocx,sy+sh-6], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Text({ x:sx+sw-14, y:ocy+3, text:'α', fontSize:8, fill:'#ef9a9a' }));
        this.group.add(new Konva.Text({ x:ocx+3, y:sy+14, text:'β', fontSize:8, fill:'#90caf9' }));
        ...
        // 变换公式标注
        this.group.add(new Konva.Text({ x:sx+4, y:sy+sh-30, text:'α = K·sin(θ)·U_m·sin(ωt)', fontSize:7, fill:'#ef9a9a' }));
        this.group.add(new Konva.Text({ x:sx+4, y:sy+sh-20, text:'β = K·cos(θ)·U_m·sin(ωt)', fontSize:7, fill:'#90caf9' }));
        ...
        this.group.add(this._scAlphaProj, this._scBetaProj, this._scAlpha, this._scBeta, this._scIs);
    }
    ...
    _drawErrorAnalysis() {
        ...
        this.group.add(new Konva.Rect({ x:ex, y:ey, width:ew, height:eh, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:ex, y:ey, width:ew, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:ex+4, y:ey+2, width:ew-8, text:'误差信号（CX→CT 随动系统）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        // 误差电压波形区
        const ox=ex+12, oy=ey+eh*0.55, aw=ew-18, ah=eh*0.30;
        this.group.add(new Konva.Line({ points:[ox,oy-ah,ox,oy+ah*0.3,ox+aw,oy+ah*0.3], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Text({ x:ox-12, y:oy-ah, text:'e', fontSize:7, fill:'#80cbc4' }));
        this.group.add(new Konva.Text({ x:ox+aw+2, y:oy+ah*0.3+2, text:'t', fontSize:7, fill:'#80cbc4' }));

        this._errWaveLine = new Konva.Line({ points:[], stroke:'#ef5350', strokeWidth:1.8, lineJoin:'round' });
        this.group.add(this._errWaveLine);
        ...
        this.group.add(this._errAngleText, this._errAmplText, this._errLinearText, this._errRxLabel);
    }
    ...
    _drawControlPanel() {
        ...
        this.group.add(new Konva.Rect({x:px,y:py,width:pw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0]}));
        this.group.add(new Konva.Text({x:px+4,y:py+2,width:pw-8,text:'参数控制',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        this.group.add(new Konva.Text({x:fLX,y:fBY-2,text:'频率:',fontSize:7.5,fill:'#546e7a'}));
        this._freqBtns={};
        freqs.forEach((f,i) => {
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({x:wx+4,y:wy+1,width:ww-8,text:'励磁 U_R   定子 e_S1/e_S2/e_S3（调幅波，幅值携带角度）   误差信号 e_error',fontSize:8,fill:'#80cbc4',align:'center'}));
        ...
        ['U_R','e_S1/S2/S3','（S1红/S2绿/S3蓝）','e_error'].forEach((l,i)=>{
            this.group.add(new Konva.Text({x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#ffa726','#e53935','#80cbc4','#ef5350'][i]}));
        });
        this.group.add(this._wLUR, this._wLES1, this._wLES2, this._wLES3, this._wLErr);
        this._wavHCh=hCh;
    }
