        ...
        sys.stepsArray[0] = [
            // 1. 电桥供电基础回路
            {
                ...
                act: async () => {
                    ...
                    sys.showComp.showTooltip('pt', '1. PT100测温电桥', { color: '#2ecc71' });
            ...
            // 修正后的步骤 13
            {
                ...
                act: async () => {
                    // 1. 设置状态
                    sys.comps.pt.currentResistance = 119.25;
                    sys.comps.pt.update();

                    // 2. 显示反馈

                    // 确保 ID 传递准确
                    sys.showComp.showTooltip('r1', '模拟 50 度 (Rt=119.25Ω)', { color: '#e82f0e' });
                ...
                check: () => Math.abs(sys.comps.pt.currentResistance - 119.25) < 0.5
            },
            ...
            // 14. 100度性能验证
            {
                ...
                act: async () => {
                    await new Promise(r => setTimeout(r, 500));
                    sys.comps.pt.currentResistance = 138.51;
                    sys.comps.pt.update(); // 触发组件状态更新
                    sys.showComp.showTooltip('varires', '模拟100度(Rt=138.51Ω)', { color: '#e82f0e' });
                    await new Promise(r => setTimeout(r, 4000));
                    sys.showComp.clearAllTooltips(); // 演示结束，恢复视角
                },
                check: () => Math.abs(sys.comps.pt.currentResistance - 138.51) < 0.5
            }
        ];
    }
