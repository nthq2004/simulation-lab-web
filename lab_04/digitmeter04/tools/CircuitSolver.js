...
export class CircuitSolver {
    ...
    _solve() {
        ...
        // ── 恒流源（cc_source）每端口最多一个额外电压方程 ──
        let ccEqCount = 0;
        ccDevs.forEach(dev => {
            const hasCom = this.portToCluster.has(`${dev.id}_wire_com`);
            for (let i = 1; i <= 4; i++) {
                if (this.portToCluster.has(`${dev.id}_wire_i${i}`) && hasCom)
                    ccEqCount++;
            }
        });

        // ---总的方程数量。
        const totalSize = mSize + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + ccEqCount;
    ...
    _updateDeviceCurrents(devices, results) {
        ...
        mf47Devs.forEach(dev => {
            const vV = getV(dev.id, 'v') ?? 0;
            const vMA = getV(dev.id, 'mA') ?? 0;
            const vCOM = getV(dev.id, 'COM') ?? 0;
            const R = dev.getInputImpedance();
            const group = dev._range?.group;
            if (group === 'DCmA') {
                dev.physCurrent = (vMA - vCOM) / Math.max(0.1, R);
            } else {
                dev.physCurrent = (vV - vCOM) / Math.max(0.1, R);
            }
        });
