    ...
    stampCCSources(ctx, G, B, ccDevs, ccVIdx) {
        ...
        ccDevs.forEach(dev => {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                const targetA = CURRENTS[i];
                const rReq = ctx.getEquivalentResistance(ctx.clusters[cP], ctx.clusters[cN], ctx.clusters);
                if (rReq * targetA > COMPLIANCE_V || rReq > 1000000) {
                    this._addV(ctx, G, B, cP, cN, COMPLIANCE_V, ccVIdx++);
                } else {
                    ccVIdx++;
                    this._addI(ctx, B, cP, cN, targetA);
                }
            }
        });
        return ccVIdx;
