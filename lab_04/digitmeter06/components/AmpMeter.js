 ...
 * 模拟电流表（模拟仪表面板）
 *
 * 概述：在面板上渲染一个带刻度、指针和嵌入式 LCD 的电流表，
 * 支持数值到角度的映射、可配置量程、以及端子导出位置。
 * 视觉样式参考传统指针式仪表（270°量程近似，实际使用 -120° ~ +120°）。
 *
 * 参数与属性说明（构造器接收 `config`）：
 *  - config.radius: 仪表半径（像素），默认 70
 *  - min / max: 仪表量程下限/上限（mA）
 *  - title: 仪表标题文本
 *
 * 行为说明：
 *  - `update(mA)`：外部驱动方法，传入当前电流值（mA），组件完成限幅、指针旋转与 LCD 文本更新
 *  - `valueToAngle(value)`：将量程数值映射到表盘角度（-120°..+120°）
 */
export class AmpMeter extends BaseComponent {
    ...
    init() {
        // 初始化并绘制静态层次元素，顺序从底到顶以保证覆盖关系正确
        this._drawShell();   // 外框与金属质感
        this._drawZones();   // 安全/告警色块（绿/黄/红）
        this._drawTicks();   // 刻度与数字
        this._drawPointer(); // 指针（动态旋转）
        this._drawCenter();  // 指针轴心
        this._drawLcd();     // 嵌入式 LCD 显示
        this._drawname();    // 仪表名称
    }
    /* ===============================
       数值 → 角度（唯一映射）
    ...
    _drawZones() {
        ...
        zones.forEach(z => {
            ...
            this._staticGroup.add(new Konva.Arc({ x: 0, y: 0, innerRadius: this.radius - 12 * this.scale, outerRadius: this.radius, angle: angle, rotation: rotation, fill: z.color, opacity: 0.65 }));
        });
    }
    ...
    _drawTicks() {
        ...
        for (let i = 0; i <= totalSteps; i++) {
            ...
            if (isMajor) {
                ...
                this._staticGroup.add(new Konva.Text({ x: this.textRadius * Math.cos(textRad) - 14 * this.scale, y: this.textRadius * Math.sin(textRad) - 6 * this.scale, width: 28 * this.scale, align: 'center', text: v.toString(), fontSize: 11 * this.scale, fill: '#000' }));
            }
        }
    }
