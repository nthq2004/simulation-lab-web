...
export class ACContactor extends BaseComponent {
    ...
    _drawElectromagnet() {
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: coreX, y: limbY + limbH * 0.4, width: coreW, text: '静铁芯（E形）', fontSize: 8, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: coreX, y: gapTop + 1, width: coreW, text: '─── 气隙 δ ───', fontSize: 7, fill: '#37474f', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: ex + 4, y: limbY + yoke/2 - 6, text: 'E', fontSize: 11, fontStyle: 'bold', fill: '#ef9a9a' }));
    ...
    _drawCoilWinding() {
        ...
        // 线圈接线端（A1/A2）
        const termX = ex + 10;
        this._staticGroup.add(new Konva.Line({ points: [coilX1, coilY1+4, termX, coilY1+4, termX, this._emCY - 20], stroke: '#ff8f00', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [coilX1, coilY2-4, termX, coilY2-4, termX, this._emCY + 20], stroke: '#ff8f00', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Circle({ x: termX, y: this._emCY - 20, radius: 3, fill: '#37474f', stroke: '#546e7a', strokeWidth: 0.5 }));
        this._staticGroup.add(new Konva.Circle({ x: termX, y: this._emCY + 20, radius: 3, fill: '#37474f', stroke: '#546e7a', strokeWidth: 0.5 }));
        this._staticGroup.add(new Konva.Text({ x: termX - 14, y: this._emCY - 26, text: 'A1', fontSize: 8, fill: '#ff8f00' }));
        this._staticGroup.add(new Konva.Text({ x: termX - 14, y: this._emCY + 22, text: 'A2', fontSize: 8, fill: '#ff8f00' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: coilX2 + 3, y: coilY1 + coilH/2 - 14, text: '主线圈\nN匝', fontSize: 8, fill: '#ff8f00', lineHeight: 1.4 }));

        this._coilY1 = coilY1; this._coilY2 = coilY2;
        this._coilX1 = coilX1; this._coilX2 = coilX2;
        this._staticGroup.add(this._coilGlow, this._coilGroup);
    }
    ...
    _drawShadeRings() {
        ...
        // 磁通分区标注
        this._staticGroup.add(new Konva.Text({ x: lps[0] + 1, y: gapTop + 2, width: limb - 2, text: 'Φb', fontSize: 7, fill: '#ef9a9a', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: lps[2] + 1, y: gapTop + 2, width: limb - 2, text: 'Φb', fontSize: 7, fill: '#ef9a9a', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: lps[1] + 1, y: gapTop + 2, width: limb - 2, text: 'Φa', fontSize: 7, fill: '#ffd54f', align: 'center' }));
    }
    ...
    _drawMainContacts() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx+4, y: cy+2, width: cw-8, text: `主触点（${this.polePairs}P  ${this.ratedCurrent}A）`, fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        for (let i = 1; i <= this.polePairs; i++) {
            ...
            // 上固定触点
            this._staticGroup.add(new Konva.Rect({ x: px-10, y: cy+23, width: 20, height: fixH, fill: '#90a4ae', stroke: '#546e7a', strokeWidth: 0.8, cornerRadius: 2 }));
            // 下固定触点
            this._staticGroup.add(new Konva.Rect({ x: px-10, y: cy+ch-26, width: 20, height: fixH, fill: '#90a4ae', stroke: '#546e7a', strokeWidth: 0.8, cornerRadius: 2 }));
            ...
            // 极标注
            this._staticGroup.add(new Konva.Text({ x: px-6, y: cy+14, text: `L${i}`, fontSize: 7, fill: '#4fc3f7' }));
            this._staticGroup.add(new Konva.Text({ x: px-6, y: cy+ch-12, text: `T${i}`, fontSize: 7, fill: '#4fc3f7' }));
        }

        // 行程指示条
        this._staticGroup.add(new Konva.Text({ x: cx+2, y: cy+ch-50, text: '行程:', fontSize: 7, fill: '#37474f' }));
        this._staticGroup.add(new Konva.Rect({ x: cx+28, y: cy+ch-50, width: cw-36, height: 7, fill: '#0a0a18', cornerRadius: 2 }));
        ...
        // 灭弧罩（示意框）
        this._staticGroup.add(new Konva.Rect({ x: cx+4, y: cy+36, width: cw-8, height: ch-72, fill: 'rgba(0,0,0,0)', stroke: '#263238', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: cx+4, y: cy+36, text: '灭弧罩', fontSize: 7, fill: '#263238' }));
    ...
    _drawAuxContacts() {
        ...
        const drawAux = (label, termTop, termBot, isNO, yOffset) => {
            ...
            this._staticGroup.add(new Konva.Rect({ x: ax, y: ty, width: aw, height: ah/2, fill: '#0d1520', stroke: '#1a3040', strokeWidth: 1, cornerRadius: 3 }));
            this._staticGroup.add(new Konva.Text({ x: ax+2, y: ty+2, width: aw-4, text: label, fontSize: 7.5, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

            // 上固定触点
            this._staticGroup.add(new Konva.Rect({ x: hcx-hw/2, y: ty+13, width: hw, height: 6, fill: '#78909c', stroke: '#455a64', strokeWidth: 0.6, cornerRadius: 1 }));
            // 下固定触点
            this._staticGroup.add(new Konva.Rect({ x: hcx-hw/2, y: ty+ah/2-10, width: hw, height: 6, fill: '#78909c', stroke: '#455a64', strokeWidth: 0.6, cornerRadius: 1 }));
            // 端子标注
            this._staticGroup.add(new Konva.Text({ x: hcx-hw/2-2, y: ty+10, text: termTop, fontSize: 7, fill: '#4fc3f7' }));
            this._staticGroup.add(new Konva.Text({ x: hcx-hw/2-2, y: ty+ah/2-10, text: termBot, fontSize: 7, fill: '#4fc3f7' }));
            ...
            this._staticGroup.add(bridge, statusDot);
    ...
    _drawForceGraph() {
        ...
        this._staticGroup.add(new Konva.Text({ x: fx+4, y: fy+2, width: fw-8, text: 'F_em 吸力波形（分磁环消振）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-10, y: oy-ah, text: 'F', fontSize: 7, fill: '#80cbc4' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [ox, springY, ox+aw, springY], stroke: '#ef5350', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: springY-9, text: 'F_spring', fontSize: 6.5, fill: '#ef5350' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: ptsNoShade,    stroke: '#ef5350', strokeWidth: 1.2, lineJoin: 'round', opacity: 0.45, dash: [3,3] }));
        this._staticGroup.add(new Konva.Line({ points: ptsWithShade,  stroke: '#66bb6a', strokeWidth: 1.5, lineJoin: 'round', opacity: 0.55 }));

        // 图例
        this._staticGroup.add(new Konva.Line({ points: [ox+aw*0.55, oy-ah+8,  ox+aw*0.72, oy-ah+8],  stroke: '#66bb6a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw*0.73, y: oy-ah+4, text: '有分磁环', fontSize: 6.5, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Line({ points: [ox+aw*0.55, oy-ah+18, ox+aw*0.72, oy-ah+18], stroke: '#ef5350', strokeWidth: 1.2, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw*0.73, y: oy-ah+14, text: '无分磁环', fontSize: 6.5, fill: '#ef5350' }));

        // 动态工作点
        this._forcePoint = new Konva.Circle({ x: ox, y: oy, radius: 4, fill: '#ffd54f', stroke: '#f9a825', strokeWidth: 1.5 });
        this._staticGroup.add(this._forcePoint);
    ...
    _drawLCDPanel() {
        ...
        if (indH > 18) {
            ...
            this._staticGroup.add(this._stateIndicator, this._stateText);
        }
    }
    ...
    _drawControlPanel() {
        ...
        // 线圈电压调节
        const slY = py + 44;
        this._staticGroup.add(new Konva.Text({ x: px+4, y: slY-11, text: `线圈电压 Uc (额定 ${this.ratedVoltageCoil}V):`, fontSize: 8, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Rect({ x: px+4, y: slY, width: pw-32, height: 9, fill: '#0a0a18', cornerRadius: 2 }));
        ...
        this._interactGroup.add(this._voltBar, this._voltValTxt, hitVolt);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'Uc 线圈电压   Ic 线圈电流   Φa/Φb 磁通   F_em 吸力', fontSize: 8, fill: '#80cbc4', align: 'center' }));
        ...
        ['Uc', 'Ic', 'Φb', 'F'].forEach((l, i) => {
            this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+12+h4*i+3, text: l, fontSize: 8, fill: ['#ff8f00','#ffd54f','#ef9a9a','#66bb6a'][i] }));
        });

        this._staticGroup.add(this._wLineU, this._wLineI, this._wLineFlux, this._wLineF);
        this._wavH4 = h4;
    }
