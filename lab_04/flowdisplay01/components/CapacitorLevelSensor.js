...
export class CapacitiveLevelSensor extends BaseComponent {
    ...
    _drawTankBody() {
        ...
        [tw*0.2, tw*0.5, tw*0.8].forEach(dx => {
            this._staticGroup.add(new Konva.Circle({ x: tx+dx, y: ty+wall/2, radius: 3, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Circle({ x: tx+dx-0.8, y: ty+wall/2-0.8, radius: 1, fill: 'rgba(255,255,255,0.2)' }));
        });
        // 内腔
        ...
        this._staticGroup.add(outer, topCap, this._innerCav, bottom, this._hiLine, this._loLine);
    }
    ...
    _drawElectrodeProbe() {
        ...
        this._staticGroup.add(insulate, electrode, topConn);
    }
    ...
    _drawCrossSection() {
        ...
        lbls.forEach(l => this._staticGroup.add(new Konva.Text({ x: l.x, y: l.y, text: l.text, fontSize: 7.5, fill: l.color, lineHeight: 1.4 })));
        ...
        this._staticGroup.add(bg, this._secGasArc, this._secLiqArc, outerRing, innerElec, this._secEpsLabel, this._secCapLabel, this._secFreqLabel, this._secLvLabel);
    }
    ...
    _drawInstrumentHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+10, y: hy+19, width: hw-20, text: 'CAPACITIVE  4-20mA', fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._capArc, this._lcdMain, this._lcdUnit, this._lcdCap, this._lcdCurrent, this._lcdEps);
    }
    ...
    _drawOscilloscope() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: oy+2, width: ow-8, text: '振荡器输出  (C↑→f↓，C↓→f↑)', fontSize: 8, fontStyle: 'bold', fill: '#7986cb', align: 'center' }));

        // 网格
        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [ox, oy+oh*i/3, ox+ow, oy+oh*i/3], stroke: 'rgba(121,134,203,0.08)', strokeWidth: 0.5 }));
        for (let i = 1; i < 6; i++) this._staticGroup.add(new Konva.Line({ points: [ox+ow*i/6, oy, ox+ow*i/6, oy+oh], stroke: 'rgba(121,134,203,0.06)', strokeWidth: 0.5 }));

        // 基准线
        this._oscMidY = oy + oh * 0.54;
        this._staticGroup.add(new Konva.Line({ points: [ox+2, this._oscMidY, ox+ow-2, this._oscMidY], stroke: 'rgba(121,134,203,0.15)', strokeWidth: 0.5, dash: [4,4] }));
        ...
        this._staticGroup.add(bg, titleBg, this._oscLine, this._oscCapLabel, this._oscFrqLabel, this._oscMaLabel);
    }
