...
export class DpFlowIndicator extends BaseComponent {
    ...
    _rebuildDynamic() {
        ...
        this._drawFluidFlow();
        this._drawOrificeJet();
        this._drawPressureZones();
        this._drawVortices();
        this._drawUTubeLevels();
        this._drawFloatLinkage();
        this._drawPointer();
        this._drawParticles();
    }
    ...
    _drawFluidFlow() {
        ...
        this._dynamicGroup.add(new Konva.Rect({
            x: this._pipeX0, y: y - r,
            width: orX - orW/2 - this._pipeX0, height: r * 2,
            fill: this.medium === 'air' ? `rgba(200,230,255,${baseAlpha*0.35})` : `rgba(20,100,180,${baseAlpha})`,
        }));
        // 下游液体（低压，颜色略浅）
        this._dynamicGroup.add(new Konva.Rect({
            x: orX + orW/2, y: y - r,
            width: this._pipeX1 - (orX + orW/2), height: r * 2,
            fill: col,
        }));
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._dynamicGroup.add(new Konva.Line({
                points: [wx, y - r*0.5, wx, y + r*0.5],
                stroke: `rgba(100,190,255,${wa})`, strokeWidth: 1.8, lineCap: 'round',
            }));
        }
    }

    // ── 孔板射流束（Vena Contracta 效果）────────────────────
    _drawOrificeJet() {
        const y   = this._pipeY, r = this._pipeR;
        const orX = this._orX, orW = this._orW;
        const oh  = this._orHole;
        ...
        // 射流束：孔口→收缩点（Vena Contracta，约 0.6D 下游）→扩散
        // 收缩最小截面 ≈ 0.6 × 孔口半径
        const vcX  = orX + orW/2 + this._pipeR * 1.2;   // VC 位置 x
        const vcOh = oh * (0.60 + q * 0.08);             // 收缩后半径（流量越大收缩越小）
        ...
        this._dynamicGroup.add(new Konva.Line({
            points: [
                orX + orW/2, y - oh,
                vcX, y - vcOh,
                orX + orW/2 + this._pipeR * 3.5, y - r * 0.60,
            ],
            stroke: `rgba(80,180,255,${jetAlpha})`, strokeWidth: 1.5,
        ...
        this._dynamicGroup.add(new Konva.Line({
            points: [
                orX + orW/2, y,
                vcX + this._pipeR * 0.4, y,
            ],
            stroke: `rgba(160,220,255,${jetAlpha * 1.2})`, strokeWidth: vcOh * 1.2,
            lineCap: 'round',
        }));
        ...
        if (q > 0.15) {
            ...
            this._dynamicGroup.add(new Konva.Line({
                points: [vcX, y + vcOh + 2, vcX, y + r - 2],
                stroke: `rgba(100,200,255,${vcAlpha * 0.5})`, strokeWidth: 0.8, dash: [2,3],
            }));
            this._dynamicGroup.add(new Konva.Text({
                x: vcX - 16, y: y + r + 4, width: 32,
                text: 'VC', fontSize: 6.5, fill: `rgba(100,200,255,${vcAlpha})`, align: 'center',
            }));
        }
    }
    ...
    _drawPressureZones() {
        ...
        // 高压区（孔板上游，略红）
        this._dynamicGroup.add(new Konva.Rect({
            x: this._tapHX - this._pipeR, y: y - r + 1,
            width: this._pipeR * 2, height: r * 2 - 2,
            fill: `rgba(220,80,60,${pAlpha})`,
        }));
        // 低压区（孔板下游，略蓝）
        this._dynamicGroup.add(new Konva.Rect({
            x: this._tapLX - this._pipeR, y: y - r + 1,
            width: this._pipeR * 2, height: r * 2 - 2,
            fill: `rgba(40,100,220,${pAlpha})`,
        }));
    }
    ...
    _drawVortices() {
        this._vortices.forEach(v => {
            ...
            this._dynamicGroup.add(new Konva.Arc({
                x: v.x, y: v.y,
                innerRadius: v.r * 0.5, outerRadius: v.r,
                angle: 270, rotation: v.angle,
                stroke: `rgba(100,180,255,${alpha})`, strokeWidth: 1.0,
            }));
        });
    }
    ...
    _drawUTubeLevels() {
        ...
        // 高压侧液柱（液面下降 = 高压压低液面）
        const hLevel = this._uLevelH;   // 0=满（液面在顶），1=空（液面在底）
        const lLevel = this._uLevelL;   // 0=空，1=满（液面在顶）

        // 左管（高压侧）液体：从底部到 (1-hLevel)*uh 高度
        const hFillH = (1 - hLevel) * uh;
        if (hFillH > 2) {
            this._dynamicGroup.add(new Konva.Rect({
                x: hpX - tr, y: cy - hFillH,
                width: tr * 2, height: hFillH,
                fill: fluidFill,
            }));
            // 液面（水平线）
            this._dynamicGroup.add(new Konva.Line({
                points: [hpX - tr, cy - hFillH, hpX + tr, cy - hFillH],
                stroke: fluidStroke, strokeWidth: 1.5, lineCap: 'round',
            }));
            // 液面波纹
            if (this._flow > 0.05) {
                const wPhase = (Date.now() / 400) % (Math.PI * 2);
                this._dynamicGroup.add(new Konva.Line({
                    points: [hpX - tr + 1, cy - hFillH, hpX + tr - 1, cy - hFillH],
                    stroke: `rgba(120,200,255,${0.15 + Math.abs(Math.sin(wPhase)) * 0.20})`,
                    strokeWidth: 2, lineCap: 'round',
                }));
            }
        }

        // 右管（低压侧）液体：液面升高 lLevel*uh
        const lFillH = lLevel * uh;
        if (lFillH > 2) {
            this._dynamicGroup.add(new Konva.Rect({
                x: lpX - tr, y: cy - lFillH,
                width: tr * 2, height: lFillH,
                fill: fluidFill,
            }));
            this._dynamicGroup.add(new Konva.Line({
                points: [lpX - tr, cy - lFillH, lpX + tr, cy - lFillH],
                stroke: fluidStroke, strokeWidth: 1.5, lineCap: 'round',
            }));
            if (this._flow > 0.05) {
                const wPhase = (Date.now() / 400 + Math.PI) % (Math.PI * 2);
                this._dynamicGroup.add(new Konva.Line({
                    points: [lpX - tr + 1, cy - lFillH, lpX + tr - 1, cy - lFillH],
                    stroke: `rgba(120,200,255,${0.15 + Math.abs(Math.sin(wPhase)) * 0.20})`,
                    strokeWidth: 2, lineCap: 'round',
                }));
            }
        ...
        this._dynamicGroup.add(new Konva.Line({
            points: [hpX, cy + 2, hpX, cy + 14, lpX, cy + 14, lpX, cy + 2],
            stroke: fluidFill, strokeWidth: tr * 2, lineCap: 'round', lineJoin: 'round',
        }));
        ...
        if (Math.abs(deltaH) > 4) {
            ...
            // 差值线
            this._dynamicGroup.add(new Konva.Line({
                points: [midX, y1, midX, y2],
                stroke: 'rgba(200,180,80,0.45)', strokeWidth: 1, dash: [3,3],
            }));
            // 双箭头
            [y1, y2].forEach(yy => {
                this._dynamicGroup.add(new Konva.Line({
                    points: [midX - 4, yy + (yy === y1 ? 5 : -5), midX, yy, midX + 4, yy + (yy === y1 ? 5 : -5)],
                    stroke: 'rgba(200,180,80,0.45)', strokeWidth: 1, lineCap: 'round', lineJoin: 'round',
                }));
            });
            this._dynamicGroup.add(new Konva.Text({
                x: midX + 4, y: (y1 + y2) / 2 - 5, width: 24,
                text: 'Δh', fontSize: 7, fill: 'rgba(200,180,60,0.55)',
            }));
        }
    }
    ...
    _drawFloatLinkage() {
        ...
        this._dynamicGroup.add(new Konva.Rect({
            x: lpX - tr * 0.8, y: floatY - 5,
            width: tr * 1.6, height: 8,
            fill: '#c0a030', stroke: '#907818', strokeWidth: 0.8, cornerRadius: 1,
        }));
        ...
        this._dynamicGroup.add(new Konva.Line({
            points: [lpX, floatY, lpX + 18, floatY, pivX - 10, pivY + 8, pivX, pivY],
            stroke: 'rgba(160,150,80,0.55)', strokeWidth: 1.8,
            lineCap: 'round', lineJoin: 'round', tension: 0.35,
        }));

        // 连杆活动节点
        [[lpX + 18, floatY], [pivX - 10, pivY + 8]].forEach(([px, py]) => {
            this._dynamicGroup.add(new Konva.Circle({
                x: px, y: py, radius: 2.5,
                fill: '#b09020', stroke: '#808010', strokeWidth: 0.7,
            }));
        });
    }
    ...
    _drawPointer() {
        ...
        // 指针杆
        this._dynamicGroup.add(new Konva.Line({
            points: [cx - PTL * cosR, cy - PTL * sinR, cx + PL * cosR, cy + PL * sinR],
            stroke: '#181820', strokeWidth: 2.0, lineCap: 'round',
        }));
        // 红色尖端
        this._dynamicGroup.add(new Konva.Line({
            points: [cx + (PL - R*0.16)*cosR, cy + (PL - R*0.16)*sinR, cx + PL*cosR, cy + PL*sinR],
            stroke: '#e03020', strokeWidth: 2.0, lineCap: 'round',
        }));
        // 蓝色尾部
        this._dynamicGroup.add(new Konva.Line({
            points: [cx - R*0.05*cosR, cy - R*0.05*sinR, cx - PTL*cosR, cy - PTL*sinR],
            stroke: '#2060b0', strokeWidth: 3.0, lineCap: 'round',
        }));

        // 轴心（黄铜圆）
        this._dynamicGroup.add(new Konva.Circle({
            x: cx, y: cy, radius: R * 0.062,
            fillRadialGradientStartPoint:  { x: -R*0.02, y: -R*0.02 },
            fillRadialGradientEndPoint:    { x: 0, y: 0 },
            fillRadialGradientStartRadius: R * 0.01,
            fillRadialGradientEndRadius:   R * 0.062,
        ...
        this._dynamicGroup.add(new Konva.Text({
            x: cx + (PL + 10) * cosR - 18, y: cy + (PL + 10) * sinR - 6,
            width: 36, text: qRead,
            fontSize: 8, fontFamily: 'monospace', fontStyle: 'bold',
            fill: '#d03020', align: 'center',
    ...
    _drawParticles() {
        ...
        this._particles.forEach(p => {
            ...
            this._dynamicGroup.add(new Konva.Circle({
                ...
                fill: `rgba(${inJet ? '160,220,255' : '80,160,220'},${alpha})`,
            }));
        });
    }
