...
export class HallSpeedSensor extends BaseComponent {
    ...
    _drawGear() {
        ...
        this._staticGroup.add(this._gearGroup);
        ...
        this._staticGroup.add(this._dirArrow);
    }
    ...
    _drawHallIC() {
        ...
        pinColors.forEach((col, i) => {
            const px = ix + 4 + i * 9;
            const py = iy + ih + 2;
            this._staticGroup.add(new Konva.Rect({ x: px-2, y: py, width: 4, height: 12, fill: col, cornerRadius: 1 }));
            this._staticGroup.add(new Konva.Text({ x: px-6, y: py+14, text: pinNames[i], fontSize: 6.5, fill: col }));
            // 输出引脚 LED
            if (i === 2) {
                this._outLed = new Konva.Circle({ x: px, y: py - 4, radius: 3, fill: '#1a1a1a' });
                this._staticGroup.add(this._outLed);
            }
        });
        ...
        this._staticGroup.add(this._fieldLine, body, sheen, sensorFace, label, hallLabel);
    }

    // ── 磁场可视化层 ──────────────────────────
    _drawBFieldViz() {
        this._bFieldGroup = new Konva.Group();
        this._staticGroup.add(this._bFieldGroup);
        ...
        this._staticGroup.add(new Konva.Text({ x: bx - 2, y: by - 12, text: 'B', fontSize: 9, fontStyle: 'bold', fill: '#ff8f00' }));
        this._staticGroup.add(this._bBarBg, this._bBarFill);
        ...
        this._staticGroup.add(this._threshLine);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 3; i++) {
            this._staticGroup.add(new Konva.Line({ points: [hx, hy+6+i*10, hx+hw, hy+6+i*10], stroke: 'rgba(255,255,255,0.14)', strokeWidth: 0.8 }));
        }
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+17, width: hw-16, text: 'HALL  NPN-OC', fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._rpmArc, this._lcdMain, this._lcdUnit, this._lcdFreq, this._lcdVcc, this._lcdPulse);
    }
    ...
    _drawCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: cy2+3, width: cw-4, text: 'NPN OC 等效', fontSize: 7.5, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
        ...
        // VCC 线
        this._staticGroup.add(new Konva.Line({ points: [x1, yTop, x1, yTop + 4], stroke: '#e53935', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: x1 - 6, y: yTop - 2, text: 'VCC', fontSize: 7, fill: '#e53935' }));

        // 上拉电阻
        const rH = 10;
        for (let i = 0; i < 3; i++) {
            this._staticGroup.add(new Konva.Line({ points: [x1 + (i%2===0?-3:3), yTop+4+i*3, x1 + (i%2===0?3:-3), yTop+4+i*3+3], stroke: '#ffd54f', strokeWidth: 1 }));
        }
        this._staticGroup.add(new Konva.Text({ x: x1 + 4, y: yTop + 6, text: 'R', fontSize: 7, fill: '#ffd54f' }));

        // 集电极节点
        const yC = yTop + 14;
        this._staticGroup.add(new Konva.Line({ points: [x1, yTop+rH+4, x1, yC], stroke: '#e53935', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, yC, midX - 4, yC], stroke: '#e53935', strokeWidth: 1 }));

        // NPN 三极管（简化画法）
        const tx = midX, ty = yC + 2;
        // base
        this._staticGroup.add(new Konva.Line({ points: [tx - 6, ty, tx - 6, ty + 12], stroke: '#80cbc4', strokeWidth: 2 }));
        // emitter, collector
        this._staticGroup.add(new Konva.Line({ points: [tx - 6, ty + 3, tx + 4, ty - 3], stroke: '#80cbc4', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [tx - 6, ty + 9, tx + 4, ty + 15], stroke: '#80cbc4', strokeWidth: 1 }));
        // collector (to C node)
        this._staticGroup.add(new Konva.Line({ points: [tx + 4, ty - 3, tx + 4, yC], stroke: '#80cbc4', strokeWidth: 1 }));
        // Hall IC 驱动基极
        this._staticGroup.add(new Konva.Line({ points: [x2, ty + 6, tx - 6, ty + 6], stroke: '#43a047', strokeWidth: 1, dash: [2,2] }));
        this._staticGroup.add(new Konva.Text({ x: x2 - 8, y: ty + 2, text: 'OUT', fontSize: 6.5, fill: '#43a047' }));

        // 发射极到 GND
        this._staticGroup.add(new Konva.Line({ points: [tx + 4, ty + 15, tx + 4, yBot], stroke: '#607d8b', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [tx - 3, yBot, tx + 11, yBot], stroke: '#607d8b', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: tx - 2, y: yBot + 2, text: 'GND', fontSize: 7, fill: '#607d8b' }));

        // 输出状态指示
        this._circOutDot = new Konva.Circle({ x: midX - 4, y: yC, radius: 3, fill: '#1a1a1a' });
        this._staticGroup.add(bg, this._circOutDot);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '输出波形  ── 数字方波  ── 霍尔模拟电压', fontSize: 8, fontStyle: 'bold', fill: '#66bb6a', align: 'center' }));

        // 网格
        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/3, wx+ww, wy+wh*i/3], stroke: 'rgba(100,200,100,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 5; i++) this._staticGroup.add(new Konva.Line({ points: [wx+ww*i/5, wy, wx+ww*i/5, wy+wh], stroke: 'rgba(100,200,100,0.05)', strokeWidth: 0.5 }));
        ...
        // 通道标签
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'CH1  数字', fontSize: 8, fontStyle: 'bold', fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+4, text: 'CH2  模拟', fontSize: 8, fontStyle: 'bold', fill: '#ff8f00' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineDigital, this._wLineAnalog, this._wFreqLbl, this._wRpmLbl, this._wBLbl);
    }
