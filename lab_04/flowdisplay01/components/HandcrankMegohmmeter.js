...
export class HandCrankMegohmmeter extends BaseComponent {
    ...
    _drawCase() {
        ...
        // 外壳高光
        this._staticGroup.add(new Konva.Rect({ x: cx+4, y: cy+4, width: cw-8, height: 6, fill: 'rgba(255,255,255,0.18)', cornerRadius: [6,6,0,0] }));
        // 底部铭牌
        this._staticGroup.add(new Konva.Rect({ x: cx+12, y: cy+ch-26, width: cw-24, height: 18, fill: '#2a1400', cornerRadius: 2 }));
        ...
        this._staticGroup.add(body, panel, panelBorder, this._nameplateText);
    }
    ...
    _drawMeterFace() {
        ...
        // 表盘玻璃反光（椭圆高光）
        this._staticGroup.add(new Konva.Ellipse({ x: cx-R*0.22, y: cy-R*0.3, radiusX: R*0.25, radiusY: R*0.12, fill: 'rgba(255,255,255,0.35)' }));
        // 表盘下半弧形遮罩（遮挡不需要显示的部分）
        this._staticGroup.add(new Konva.Arc({ x: cx, y: cy, innerRadius: 0, outerRadius: R+1, angle: 65, rotation: 157.5, fill: '#f5f0e8' }));

        // 品牌文字
        this._staticGroup.add(new Konva.Text({ x: cx-40, y: cy+R*0.05, width: 80, text: 'MΩ', fontSize: 18, fontStyle: 'bold', fill: '#8a3a00', align: 'center' }));
        ...
        this._staticGroup.add(faceOuter, faceInner, faceMain, this._rangeText);
    }
    ...
    _drawScale() {
        ...
        // 刻度盘弧线
        this._staticGroup.add(new Konva.Arc({ x: cx, y: cy, innerRadius: R*0.85, outerRadius: R*0.88, angle: this._arcSpanDeg, rotation: ARC_START_DEG-90, fill: '#8a3a00', opacity: 0.6 }));
        ...
        keyLabels.forEach(({ v, label }) => {
            ...
            this._staticGroup.add(new Konva.Text({ x: lx-12, y: ly2-6, width: 24, text: label, fontSize, fontStyle: 'bold', fill: '#3a1a00', align: 'center' }));
        });
        ...
        this._staticGroup.add(new Konva.Arc({ x: cx, y: cy, innerRadius: R*0.85, outerRadius: R*0.90, angle: dangerEnd-dangerStart, rotation: dangerStart-90, fill: 'rgba(200,30,30,0.3)' }));
        ...
        this._staticGroup.add(new Konva.Arc({ x: cx, y: cy, innerRadius: R*0.85, outerRadius: R*0.90, angle: safeEnd-safeStart, rotation: safeStart-90, fill: 'rgba(30,150,30,0.25)' }));

        // 刻度值标注 MΩ
        this._staticGroup.add(new Konva.Text({ x: cx-24, y: cy-R*0.5, width: 48, text: 'MΩ', fontSize: 9, fontStyle: 'bold', fill: '#5a3000', align: 'center' }));
    }
    ...
    _drawNeedle() {
        ...
        this._staticGroup.add(this._needleGroup);
    }
    ...
    _drawCrankAssembly() {
        ...
        // 摇把底座（固定圆盘）
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R+6, fill: '#8a3a00', stroke: '#5a2000', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R+4, fill: '#c05a00', stroke: '#8a3a00', strokeWidth: 1 }));
        ...
        this._staticGroup.add(this._crankGroup);

        // 摇把标注
        this._staticGroup.add(new Konva.Text({ x: cx-20, y: cy+R+8, width: 40, text: '手摇把', fontSize: 8, fill: '#c05a00', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: cx-20, y: cy+R+18, width: 40, text: '120r/min', fontSize: 7.5, fontFamily: 'Courier New, monospace', fill: '#8a5000', align: 'center' }));
    }
    ...
    _drawGenerator() {
        ...
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: r+4, fill: '#3a2000', stroke: '#2a1000', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: r,   fill: '#1a0d00' }));
        ...
        // 输出线圈（固定）
        this._staticGroup.add(new Konva.Rect({ x: cx-r*0.12, y: cy-r*0.92, width: r*0.24, height: r*1.84, fill: 'none', stroke: '#ff8f00', strokeWidth: 2, opacity: 0.6 }));
        this._staticGroup.add(this._genGroup);
        this._staticGroup.add(new Konva.Text({ x: cx-16, y: cy+r+4, width: 32, text: '发电机', fontSize: 7.5, fill: '#c05a00', align: 'center' }));
    }
    ...
    _drawTerminals() {
        ...
        terms.forEach(({ x, label, color, desc }) => {
            // 端子柱
            this._staticGroup.add(new Konva.Rect({ x: x-10, y: ty, width: 20, height: 28, fill: color, stroke: this._darken(color), strokeWidth: 1.5, cornerRadius: [2,2,0,0] }));
            // 端子螺丝
            this._staticGroup.add(new Konva.Circle({ x, y: ty+10, radius: 7, fill: '#f5f0e8', stroke: '#5a3000', strokeWidth: 1 }));
            this._staticGroup.add(new Konva.Line({ points: [x-5, ty+10, x+5, ty+10], stroke: '#3a2000', strokeWidth: 2 }));
            this._staticGroup.add(new Konva.Line({ points: [x, ty+5, x, ty+15], stroke: '#3a2000', strokeWidth: 2 }));
            // 标签
            this._staticGroup.add(new Konva.Text({ x: x-10, y: ty+20, width: 20, text: label, fontSize: 10, fontStyle: 'bold', fill: '#f5f0e8', align: 'center' }));
            this._staticGroup.add(new Konva.Text({ x: x-20, y: ty+30, width: 40, text: desc, fontSize: 7, fill: '#8a5000', align: 'center' }));
        });
    }
    ...
    _drawRangeSelector() {
        ...
        this._staticGroup.add(new Konva.Circle({ x: sx, y: sy, radius: R+4, fill: '#5a2000', stroke: '#3a1000', strokeWidth: 1.5 }));
        ...
        this.ranges.forEach((v, i) => {
            ...
            this._staticGroup.add(new Konva.Text({ x: lx2-16, y: ly2-7, width: 32, text: v+'V', fontSize: 7.5, fontStyle: isAct?'bold':'normal', fill: isAct?'#ff8f00':'#8a5000', align: 'center' }));
        });
        ...
        this._staticGroup.add(this._rangeKnob);
        this._staticGroup.add(new Konva.Text({ x: sx-20, y: sy+R+6, width: 40, text: '量程', fontSize: 7.5, fill: '#8a5000', align: 'center' }));
    }
    ...
    _drawDigitalDisplay() {
        ...
        this._staticGroup.add(bg, this._dispRx, this._dispRpm, this._dispV);
    }
    ...
    _drawRxPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '被测绝缘电阻 R_x 调节（拖拽设定）', fontSize: 8, fill: '#c05a00', align: 'center' }));

        // 滑块（水平，对数刻度）
        const barX = px+6, barY = py+20, barW = pw-12, barH = 12;
        this._staticGroup.add(new Konva.Rect({ x: barX, y: barY, width: barW, height: barH, fill: '#0a0500', cornerRadius: 3 }));
        ...
        this._staticGroup.add(bg, titleBg, this._rxBar, this._rxSlider, this._rxVal);
    }
    ...
    _drawSparkLayer() {
        ...
        this._staticGroup.add(this._sparkGroup);
    }
