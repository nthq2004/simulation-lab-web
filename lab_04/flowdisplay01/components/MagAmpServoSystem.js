...
export class MagAmpServoSystem extends BaseComponent {
    ...
    _init() {
        ...
        this._staticGroup.add(this._dynAC, this._dynMech, this._dynMotor, this._dynSignal);
    ...
    _drawMagAmp() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:b.x+2,y:b.y+2,width:b.w,height:b.h, fill:'rgba(0,0,0,0.14)', cornerRadius:5 }));
        ...
        // 磁放大器标题
        this._staticGroup.add(new Konva.Text({ x:b.x+4, y:b.y+5, text:'磁放大器', fontSize:8, fontStyle:'bold', fill:'#70b0d8' }));
        this._staticGroup.add(new Konva.Text({ x:b.x+4, y:b.y+15, text:'Mag-Amp', fontSize:7, fill:'#406080' }));
        ...
        [b.y+b.h*0.28, b.y+b.h*0.65].forEach((cy, idx) => {
            ...
            // 标注
            this._staticGroup.add(new Konva.Text({ x:cx-8, y:cy-r*0.55-11, text:'W₁', fontSize:7, fill:'#7090a0' }));
        });

        // 输入输出端子标注
        this._staticGroup.add(new Konva.Text({ x:b.x-36, y:b.y+b.h*0.26, text:'给定', fontSize:8, fill:'#80a0c0' }));
        this._staticGroup.add(new Konva.Arrow({ x:b.x-2,y:b.y+b.h*0.28, points:[0,0,6,0], pointerLength:4,pointerWidth:3, fill:'#6090c0',stroke:'#6090c0',strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Text({ x:b.x-36, y:b.y+b.h*0.60, text:'反馈', fontSize:8, fill:'#c09030' }));
        this._staticGroup.add(new Konva.Arrow({ x:b.x-2,y:b.y+b.h*0.62, points:[0,0,6,0], pointerLength:4,pointerWidth:3, fill:'#c09030',stroke:'#c09030',strokeWidth:1.2 }));

        this._staticGroup.add(new Konva.Text({ x:b.x+b.w-14, y:b.y+b.h*0.24, text:'a', fontSize:9, fontStyle:'bold', fill:'#60d080' }));
        this._staticGroup.add(new Konva.Text({ x:b.x+b.w-14, y:b.y+b.h*0.60, text:'b', fontSize:9, fontStyle:'bold', fill:'#d06060' }));
    }

    // ── 触发电路 ─────────────────────────────
    _drawTrigger(t, label, color) {
        this._staticGroup.add(new Konva.Rect({ x:t.x+1,y:t.y+1,width:t.w,height:t.h, fill:'rgba(0,0,0,0.12)', cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({
            x:t.x, y:t.y, width:t.w, height:t.h,
            fill:'#1e2e20', stroke:'#2e4030', strokeWidth:1.5, cornerRadius:4,
        }));
        this._staticGroup.add(new Konva.Text({ x:t.x+3, y:t.y+3, text:label, fontSize:7, fontStyle:'bold', fill:color }));
        // 锯齿脉冲示意
        ...
        pts.push(wx1,wy);
        this._staticGroup.add(new Konva.Line({ points:pts, stroke:color, strokeWidth:1.2, lineCap:'round', lineJoin:'round' }));
        this._staticGroup.add(new Konva.Text({ x:t.x+3, y:t.y+t.h*0.78, text:'α相控', fontSize:7, fill:color }));
    }

    // ── 交流电子开关（二极管+晶闸管，非整流桥）──
    _drawACSwitch(s, scrLabel, dLabel, color, nodeLabel) {
        this._staticGroup.add(new Konva.Rect({ x:s.x+1,y:s.y+1,width:s.w,height:s.h, fill:'rgba(0,0,0,0.12)', cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({
            x:s.x, y:s.y, width:s.w, height:s.h,
            fill:'#221a10', stroke:'#3a2e18', strokeWidth:1.5, cornerRadius:4,
        }));
        this._staticGroup.add(new Konva.Text({ x:s.x+3, y:s.y+3, text:scrLabel, fontSize:8, fontStyle:'bold', fill:color }));
        ...
        // 阻断线
        this._staticGroup.add(new Konva.Line({ points:[sx+r*0.7,cy-r*0.7,sx+r*0.7,cy+r*0.7], stroke:'#e09040', strokeWidth:1.2 }));
        // 门极
        this._staticGroup.add(new Konva.Line({ points:[sx+r*0.7,cy+r*0.4,sx+r*1.4,cy+r*0.9], stroke:'#e0e040', strokeWidth:1 }));
        this._staticGroup.add(new Konva.Circle({ x:sx+r*1.4,y:cy+r*0.9, radius:2, fill:'#e0e040' }));
        ...
        // 节点标注
        this._staticGroup.add(new Konva.Text({ x:cx-4, y:s.y-12, text:nodeLabel, fontSize:9, fontStyle:'bold', fill:'#d0b060' }));
        this._staticGroup.add(new Konva.Text({ x:s.x+3, y:s.y+s.h-12, text:dLabel, fontSize:7, fill:'#6080c0' }));
    }
    ...
    _drawMotor() {
        ...
        // 外圆
        this._staticGroup.add(new Konva.Rect({ x:m.x+2,y:m.y+2,width:m.w,height:m.h, fill:'rgba(0,0,0,0.12)', cornerRadius:m.w/2 }));
        this._staticGroup.add(new Konva.Rect({
        ...
        // 定子绕组标注（上下）
        this._staticGroup.add(new Konva.Text({ x:m.x+m.w*0.12, y:m.y+m.h*0.14, text:'Ⅰ', fontSize:9, fontStyle:'bold', fill:'#3060a0' }));
        this._staticGroup.add(new Konva.Text({ x:m.x+m.w*0.12, y:m.y+m.h*0.72, text:'Ⅱ', fontSize:9, fontStyle:'bold', fill:'#a04030' }));
        // 电机标注
        this._staticGroup.add(new Konva.Text({ x:m.x+m.w*0.18, y:m.y+m.h*0.43, text:'M', fontSize:11, fontStyle:'bold', fill:'#3a5068' }));
        this._staticGroup.add(new Konva.Text({ x:m.x, y:m.y+m.h+4, text:'两相电机', fontSize:7.5, fill:'#506070' }));
    ...
    _drawGearBox() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:g.x+1,y:g.y+1,width:g.w,height:g.h, fill:'rgba(0,0,0,0.14)', cornerRadius:3 }));
        this._staticGroup.add(new Konva.Rect({
        ...
        // 大齿轮
        this._staticGroup.add(new Konva.Circle({ x:gcx, y:gcy-g.h*0.20, radius:r1, fill:'#7888a0', stroke:'#5a6878', strokeWidth:1.5 }));
        for (let i=0;i<10;i++) {
        ...
        // 小齿轮（输出轴侧）
        this._staticGroup.add(new Konva.Circle({ x:gcx, y:gcy+g.h*0.20, radius:r2, fill:'#8898b0', stroke:'#5a6878', strokeWidth:1.2 }));
        for (let i=0;i<6;i++) {
        ...
        // 减速比标注
        this._staticGroup.add(new Konva.Text({ x:g.x+2, y:g.y+g.h-14, text:`i=${this.gearRatio}`, fontSize:7.5, fill:'#8090a8' }));
        this._staticGroup.add(new Konva.Text({ x:g.x, y:g.y-12, text:'减速器', fontSize:7.5, fill:'#708090' }));
        // 轴连线（电机→减速器，减速器→凸轮）
        const mx = this._motorBox.x + this._motorBox.w;
        const my = this._motorBox.y + this._motorBox.h/2;
        this._staticGroup.add(new Konva.Line({ points:[mx,my,g.x,gcy], stroke:'#8090a0', strokeWidth:3 }));
        this._staticGroup.add(new Konva.Line({ points:[g.x+g.w,gcy, this._camBox.x, this._camBox.y+this._camBox.h/2], stroke:'#8090a0', strokeWidth:2.5 }));
    }
    ...
    _drawCam() {
        ...
        this._staticGroup.add(new Konva.Circle({ x:cx, y:cy, radius:r*0.20, fill:'#4a5868', stroke:'#3a4858', strokeWidth:1 }));
        this._staticGroup.add(new Konva.Text({ x:c.x, y:c.y+c.h+3, text:'凸轮', fontSize:8, fill:'#708090' }));
    ...
    _drawLVDT() {
        ...
        // 外壳
        this._staticGroup.add(new Konva.Rect({ x:l.x+1,y:l.y+1,width:l.w,height:l.h, fill:'rgba(0,0,0,0.14)', cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({
        ...
        // LVDT 标注
        this._staticGroup.add(new Konva.Text({ x:l.x+4, y:l.y+l.h*0.88, text:'LVDT', fontSize:8, fontStyle:'bold', fill:'#5090c0' }));
        this._staticGroup.add(new Konva.Text({ x:l.x+4, y:l.y+l.h*0.94, text:'差动变压器', fontSize:7, fill:'#406080' }));
        ...
        // 弹簧连线（凸轮→铁心）
        this._staticGroup.add(new Konva.Text({ x:this._springBox.x, y:this._springBox.y-14, text:'压紧弹簧', fontSize:8, fill:'#7a8890' }));

        // 输出轴标注
        this._staticGroup.add(new Konva.Text({ x:l.x, y:l.y+l.h+4, text:'输出轴', fontSize:8, fill:'#6080a0' }));
    }
    ...
    _drawFeedbackLoop() {
        ...
        this._staticGroup.add(new Konva.Text({ x:magMidX+4, y:fbY-10, text:'V_lvdt（反馈）', fontSize:7.5, fill:'#b08020' }));
    }
    ...
    _drawControlPanel() {
        const W = this.width, H = this.height;
        const panY = H*0.89;
        this._staticGroup.add(new Konva.Rect({ x:0,y:panY,width:W,height:H*0.11, fill:'#0c1420',stroke:'#1c2840',strokeWidth:1,cornerRadius:3 }));
