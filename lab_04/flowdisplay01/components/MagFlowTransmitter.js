...
export class MagFlowTransmitter extends BaseComponent {
    ...
    _drawJunctionBox() {
        ...
        this._staticGroup.add(jBox, jBoxSheen, lCap, rCap, this._jNameplate, this._jTagText, this._jTypeText);
    }
    ...
    _drawSensorTube() {
        ...
        this._staticGroup.add(tubeOuter, this._tubeInner, tubeSheenT, tubeSheenB, this._particleGroup);
    }
    ...
    _drawFlange(x, y, w, h, side) {
        ...
        holes.forEach(ratio => {
            const hx = x + w / 2;
            const hy = y + h * ratio;
            this._staticGroup.add(new Konva.Circle({ x: hx, y: hy, radius: 3, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Circle({ x: hx - 0.8, y: hy - 0.8, radius: 1, fill: 'rgba(255,255,255,0.2)' }));
        });
        this._staticGroup.add(flange);
    }
    ...
    _drawCoilAssembly() {
        ...
        this._staticGroup.add(topCoil, topCoilLabel, botCoil, botCoilLabel, this._magLine);
    }
    ...
    _drawElectrodeMarkers() {
        ...
        elecX.forEach((ex, idx) => {
            ...
            this._staticGroup.add(elecOuter, elecOuter2);
            if (idx === 0) this._elecDotL = this._elecDot;
            else           this._elecDotR = this._elecDot;
            this._staticGroup.add(this._elecDot);
        });
    }
