...
export class PMSMMotor extends BaseComponent {
    ...
    _drawMotorSection() {
        ...
        this._staticGroup.add(new Konva.Arc({x:ecx,y:ecy,innerRadius:encR-3,outerRadius:encR+3,angle:30,rotation:-15,fill:'#ffd54f',opacity:0.7}));
        this._staticGroup.add(new Konva.Text({x:ecx+encR*0.7,y:ecy-encR*0.3,text:'Enc',fontSize:7,fill:'#ffd54f'}));
        ...
        [['U','#e53935',-28],['V','#43a047',0],['W','#1e88e5',28]].forEach(([l,c,dy])=>{
            this._staticGroup.add(new Konva.Line({points:[ecx+sOuter*0.6,ecy+dy,termX,ecy+dy],stroke:c,strokeWidth:2}));
        ...
        [['EncA',54],['EncB',68],['EncZ',82]].forEach(([l,dy])=>{
            this._staticGroup.add(new Konva.Line({points:[ecx+encR*0.7,ecy+dy,termX,ecy+dy],stroke:'#ffd54f',strokeWidth:1.2,dash:[3,3]}));
            this._staticGroup.add(new Konva.Circle({x:termX,y:ecy+dy,radius:3,fill:'#ffd54f'}));
            this._staticGroup.add(new Konva.Text({x:termX+5,y:ecy+dy-5,text:l,fontSize:7.5,fill:'#ffd54f'}));
        });
        ...
        this._staticGroup.add(new Konva.Text({x:ex-26,y:ecy-26,text:'+',fontSize:10,fill:'#ef5350',fontStyle:'bold'}));
        this._staticGroup.add(new Konva.Text({x:ex-26,y:ecy+16,text:'−',fontSize:10,fill:'#90caf9',fontStyle:'bold'}));
    ...
    _drawFOCDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({x:fx+4,y:fy+2,width:fw-8,text:'FOC 磁场定向控制框图（速度外环→dq 电流内环→SVPWM）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        blocks.forEach(({ lbl, x, w, col, bg }) => {
            this._staticGroup.add(new Konva.Rect({x,y:y0-14,width:w,height:28,fill:bg,stroke:col,strokeWidth:1,cornerRadius:3}));
            this._staticGroup.add(new Konva.Text({x,y:y0-12,width:w,text:lbl,fontSize:7,fill:col,align:'center',lineHeight:1.35}));
        });
        ...
        this._staticGroup.add(new Konva.Text({x:fx+fw/2-20,y:y0+fh*0.38-10,text:'n 速度反馈',fontSize:7,fill:'#ffd54f',opacity:0.8}));
        ...
        this._staticGroup.add(new Konva.Text({x:fx+300,y:y0+fh*0.24-9,text:'id/iq 电流反馈',fontSize:7,fill:'#a5d6a7',opacity:0.7}));
        ...
        this._staticGroup.add(new Konva.Text({x:fx+blocks[9].x-16,y:y0+16,text:'θe↗',fontSize:7,fill:'#ffd54f',opacity:0.6}));
        ...
        this._staticGroup.add(this._focIdLabel, this._focIqLabel, this._focMod);
    }
    ...
    _drawDQPlane() {
        ...
        this._staticGroup.add(new Konva.Rect({x:dx,y:dy,width:dw,height:dh,fill:'#010d18',stroke:'#1a3040',strokeWidth:1.5,cornerRadius:4}));
        this._staticGroup.add(new Konva.Rect({x:dx,y:dy,width:dw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0]}));
        this._staticGroup.add(new Konva.Text({x:dx+4,y:dy+2,width:dw-8,text:'id-iq 电流平面（MTPA + 弱磁轨迹）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));

        const ocx = dx+dw*0.55, ocy = dy+dh*0.52;
        const scl = dw*0.38/this.maxCurrent;  // px/A

        // 坐标轴
        this._staticGroup.add(new Konva.Line({points:[dx+8,ocy,dx+dw-6,ocy],stroke:'#37474f',strokeWidth:0.8}));
        this._staticGroup.add(new Konva.Line({points:[ocx,dy+14,ocx,dy+dh-6],stroke:'#37474f',strokeWidth:0.8}));
        this._staticGroup.add(new Konva.Text({x:dx+dw-16,y:ocy+2,text:'id',fontSize:8,fill:'#ffd54f'}));
        this._staticGroup.add(new Konva.Text({x:ocx+3,y:dy+15,text:'iq',fontSize:8,fill:'#80cbc4'}));

        // 电流极限圆
        const iMaxPx = this.maxCurrent*scl;
        this._staticGroup.add(new Konva.Circle({x:ocx,y:ocy,radius:iMaxPx,fill:'rgba(102,187,106,0.06)',stroke:'#66bb6a',strokeWidth:1,dash:[4,3]}));
        this._staticGroup.add(new Konva.Text({x:ocx-iMaxPx-2,y:ocy-8,text:`Is_max`,fontSize:6.5,fill:'#66bb6a'}));
        ...
        this._staticGroup.add(this._voltEllipse);
        this._staticGroup.add(new Konva.Text({x:ctrX-10,y:ocy-ellB-10,text:'Vs_lim',fontSize:6.5,fill:'#4fc3f7'}));
        ...
        if (this.motorType==='IPM') {
            ...
            if (mtpaPts.length>4)
                this._staticGroup.add(new Konva.Line({points:mtpaPts,stroke:'#ffa726',strokeWidth:1.5,lineJoin:'round',opacity:0.75}));
            this._staticGroup.add(new Konva.Text({x:ocx-iMaxPx*0.5,y:ocy-iMaxPx*0.6,text:'MTPA',fontSize:7,fill:'#ffa726'}));
        }

        // id=0 线（SPM 最优线）
        this._staticGroup.add(new Konva.Line({points:[ocx,ocy,ocx,dy+14],stroke:'#ffd54f',strokeWidth:0.8,dash:[3,3],opacity:0.5}));
        this._staticGroup.add(new Konva.Text({x:ocx+3,y:dy+14,text:'id=0',fontSize:6.5,fill:'#ffd54f',opacity:0.7}));

        // 永磁体磁链点（-ψf/Ld 处）
        const psiX = ocx - (this.psiF/this.Ld)*scl;
        if (psiX>dx+6) {
            this._staticGroup.add(new Konva.Circle({x:psiX,y:ocy,radius:3.5,fill:'#ef9a9a',stroke:'#b71c1c',strokeWidth:0.8}));
            this._staticGroup.add(new Konva.Text({x:psiX-8,y:ocy+5,text:'-ψf/Ld',fontSize:6.5,fill:'#ef9a9a'}));
        }
        ...
        this._staticGroup.add(this._dqIdLine,this._dqIqLine,this._dqPoint);
    ...
    _drawPhasorDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({x:px+4,y:py+2,width:pw-8,text:'相量图（d-q 旋转坐标系）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        this._staticGroup.add(new Konva.Text({x:px+pw-16,y:ocy+2,text:'d',fontSize:8,fill:'#ffd54f'}));
        this._staticGroup.add(new Konva.Text({x:ocx+3,y:py+14,text:'q',fontSize:8,fill:'#80cbc4'}));
        ...
        [['#ef9a9a','ψ_f（永磁链）'],['#ffd54f','Is（定子电流）'],
         ...
         ['#ffffff','Us（定子电压）'],['#66bb6a','ud  '],['#4fc3f7','uq']].forEach(([col,lbl],i)=>{
            ...
            this._staticGroup.add(new Konva.Text({x:lgX+12,y:lgY+i*9-1,text:lbl,fontSize:6.5,fill:col}));
        });

        this._phOCX=ocx; this._phOCY=ocy; this._phR=R;
        this._staticGroup.add(this._phaPsiF,this._phaId,this._phaIq,this._phaIs,this._phaUd,this._phaUq,this._phaUs);
    }
    ...
    _drawTNCurve() {
        ...
        this._staticGroup.add(new Konva.Text({x:tx+4,y:ty+2,width:tw-8,text:'T-n 特性（恒转矩+弱磁区）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        this._staticGroup.add(new Konva.Line({points:[nBaseX,oy-ah,nBaseX,oy],stroke:'#ffa726',strokeWidth:0.8,dash:[4,3]}));
        this._staticGroup.add(new Konva.Text({x:nBaseX-3,y:oy+3,text:'nb',fontSize:6.5,fill:'#ffa726'}));
        ...
        this._staticGroup.add(new Konva.Line({points:ctPts, stroke:'#ef9a9a',strokeWidth:1.5,opacity:0.5}));
        this._staticGroup.add(new Konva.Line({points:ctPts2,stroke:'#66bb6a',strokeWidth:1.5,opacity:0.7}));
        this._staticGroup.add(new Konva.Rect({x:ox,y:oy-Trated/Tmax*(ah-4),width:nBaseX-ox,height:Trated/Tmax*(ah-4),fill:'rgba(102,187,106,0.08)'}));
        this._staticGroup.add(new Konva.Text({x:ox+4,y:oy-(Tmax/Tmax)*(ah-4)-10,text:'T_max（峰值）',fontSize:6.5,fill:'#ef9a9a'}));
        this._staticGroup.add(new Konva.Text({x:ox+4,y:oy-(Trated/Tmax)*(ah-4)-10,text:'T_rated',fontSize:6.5,fill:'#66bb6a'}));
        ...
        if (fwPts.length>4)
            this._staticGroup.add(new Konva.Line({points:fwPts,stroke:'#4fc3f7',strokeWidth:1.5,lineJoin:'round',opacity:0.75}));
        ...
        if (pRatedPts.length>4)
            this._staticGroup.add(new Konva.Line({points:pRatedPts,stroke:'#80cbc4',strokeWidth:1,lineJoin:'round',opacity:0.4,dash:[3,3]}));

        // 区域标注
        this._staticGroup.add(new Konva.Text({x:ox+4,y:oy-ah+4,text:'↑ 恒转矩区（MTPA）',fontSize:6.5,fill:'#66bb6a'}));
        this._staticGroup.add(new Konva.Text({x:nBaseX+4,y:ty+16,text:'弱磁\n恒功率区',fontSize:6.5,fill:'#4fc3f7',lineHeight:1.3}));
    ...
    _drawTorqueDecomposition() {
        ...
        this._staticGroup.add(new Konva.Rect({x:tx,y:ty,width:tw,height:th,fill:'#010d18',stroke:'#1a3040',strokeWidth:1.5,cornerRadius:4}));
        this._staticGroup.add(new Konva.Rect({x:tx,y:ty,width:tw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0]}));
        this._staticGroup.add(new Konva.Text({x:tx+4,y:ty+2,width:tw-8,text:'转矩分解（永磁+磁阻 vs id）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        this._staticGroup.add(new Konva.Text({x:ox+aw+2,y:oy+2,text:'id',fontSize:7,fill:'#ffd54f'}));
        ...
        // 零转矩基准线
        this._staticGroup.add(new Konva.Line({points:[ox,oy,ox+aw,oy],stroke:'#37474f',strokeWidth:0.5}));

        this._staticGroup.add(new Konva.Line({points:tPm_pts, stroke:'#ef9a9a',strokeWidth:1.5,lineJoin:'round',opacity:0.75}));
        if (this.motorType==='IPM') {
            this._staticGroup.add(new Konva.Line({points:tRel_pts,stroke:'#4fc3f7',strokeWidth:1.5,lineJoin:'round',opacity:0.65,dash:[4,3]}));
            this._staticGroup.add(new Konva.Line({points:tTot_pts,stroke:'#66bb6a',strokeWidth:2,  lineJoin:'round',opacity:0.85}));
        }
        ...
        if (this.motorType==='IPM') {
            ...
            this._staticGroup.add(new Konva.Line({points:[xMTPA,oy-ah,xMTPA,oy],stroke:'#ffa726',strokeWidth:0.9,dash:[3,3]}));
            this._staticGroup.add(new Konva.Text({x:xMTPA-4,y:oy+3,text:'MTPA',fontSize:6,fill:'#ffa726'}));
        }

        // 动态工作点
        this._tdPoint = new Konva.Circle({x:ox+aw,y:oy-ah*0.5,radius:5,fill:'#ffd54f',stroke:'#f9a825',strokeWidth:1.5});
        this._staticGroup.add(this._tdPoint);
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Rect({x:px,y:py,width:pw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0]}));
        this._staticGroup.add(new Konva.Text({x:px+4,y:py+2,width:pw-8,text:'FOC 参数控制',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+1,width:ww-8,text:'三相电流（正弦）   BEMF（正弦，区别于BLDC梯形波）   dq电流 id/iq   转矩分解 Tpm/Trel',fontSize:8,fill:'#80cbc4',align:'center'}));
        ...
        ['ia/ib/ic','BEMF','id/iq','T分解'].forEach((l,i)=>{
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#e53935','#ef9a9a','#ffd54f','#66bb6a'][i]}));
        });
        this._staticGroup.add(this._wLIa,this._wLIb,this._wLIc,this._wLEa,this._wLId,this._wLIq,this._wLTpm,this._wLTrel,this._wLT);
        this._wavHCh=hCh;
    }
