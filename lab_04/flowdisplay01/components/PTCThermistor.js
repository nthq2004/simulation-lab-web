...
export class PTCThermistor extends BaseComponent {
    constructor(config, sys) {
        ...
        this.addPort(this._probeCX + 12, this.height - 4, 'r',    'wire');
    }
    ...
    _drawProbeHousing() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: headX+5, y: py+headH/2-10, width: headW-10, height: 20, fill: '#0a1520', cornerRadius: 2 }));
        ...
        [[headX+8,py+8],[headX+headW-8,py+8],[headX+8,py+headH-8],[headX+headW-8,py+headH-8]].forEach(([bx,by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 3.5, fill: '#0d1520' }));
        });
        ...
        // 管体高光
        this._staticGroup.add(new Konva.Rect({ x: cx2-tubeW/2+2, y: tubeY, width: 3, height: tubeH, fill: 'rgba(255,255,255,0.15)' }));
        ...
        // 圆片高光
        this._staticGroup.add(new Konva.Ellipse({ x: cx2-discRX*0.25, y: discY+discRY*0.6, radiusX: discRX*0.22, radiusY: discRY*0.18, fill: 'rgba(255,255,255,0.25)' }));
        ...
        for (let i = 0; i <= 5; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [scaleX, fy, scaleX+7, fy], stroke: '#37474f', strokeWidth: i===0||i===5?1.2:0.7 }));
            if (i%2===0) this._staticGroup.add(new Konva.Text({ x: scaleX+9, y: fy-5, text: `${Math.round(T)}°`, fontSize: 7.5, fill: '#546e7a' }));
        }
        ...
        this._staticGroup.add(new Konva.Line({ points: [scaleX-4, tsY, scaleX+22, tsY], stroke: 'rgba(255,213,79,0.55)', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Text({ x: scaleX+24, y: tsY-5, text: `Ts=${this.Tswitch}°C`, fontSize: 6.5, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Line({ points: [scaleX-4, tcY, scaleX+22, tcY], stroke: 'rgba(239,83,80,0.55)', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Text({ x: scaleX+24, y: tcY-5, text: `Tc=${this.Tcurie}°C`, fontSize: 6.5, fill: '#ef9a9a' }));
        ...
        this._staticGroup.add(head, tube, this._discGlow, this._discEllipse, this._topElec, this._botElec, this._typeLbl);
    }
    ...
    _drawPTCElement() {
        ...
        this._staticGroup.add(this._ceramicGroup);
        ...
        this._staticGroup.add(this._tempIndicator);
    }

    // ── 铁电晶格相变动画层 ────────────────────
    _drawLatticeLayer() {
        this._latticeGroup = new Konva.Group();
        this._staticGroup.add(this._latticeGroup);
    }
    ...
    _drawLeads() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2-24, y: this.height-18, text: 'P+', fontSize: 7.5, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+14, y: this.height-18, text: 'P−', fontSize: 7.5, fill: '#90caf9' }));
        this._staticGroup.add(this._lead1, this._lead2);
    }
    ...
    _drawRTCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'R-T 特性（"浴盆"曲线 — PTC 居里点突变）', fontSize: 8, fontStyle: 'bold', fill: '#f57f17', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-16, y: cy2+14, text: 'R(Ω)', fontSize: 7, fill: '#f57f17' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+cw-14, y: oy+2, text: 'T(°C)', fontSize: 7, fill: '#f57f17' }));
        ...
        [0, 25, 50, 80, 100, 130, 150].forEach(T => {
            ...
            this._staticGroup.add(new Konva.Line({ points: [tx, oy, tx, oy+3], stroke: '#37474f', strokeWidth: 0.8 }));
            if (T%50===0||T===25) this._staticGroup.add(new Konva.Text({ x: tx-8, y: oy+4, width: 16, text: T.toString(), fontSize: 6, fill: '#37474f', align: 'center' }));
        });
        // 电阻轴（对数）
        ...
        rValues.forEach(R => {
            ...
            this._staticGroup.add(new Konva.Line({ points: [ox-3, ry, ox, ry], stroke: '#37474f', strokeWidth: 0.8 }));
            const rStr = R>=1e6?(R/1e6)+'M':R>=1000?(R/1000)+'k':R.toString();
            this._staticGroup.add(new Konva.Text({ x: ox-20, y: ry-4, width: 18, text: rStr, fontSize: 6, fill: '#37474f', align: 'right' }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points: curvePts, stroke: '#f57f17', strokeWidth: 2, lineJoin: 'round', opacity: 0.85 }));

        // Tswitch 和 Tcurie 标注
        const tsX = ox + (this.Tswitch-tMin)/(tMax-tMin)*(aw-2);
        const tcX = ox + (this.Tcurie-tMin)/(tMax-tMin)*(aw-2);
        this._staticGroup.add(new Konva.Line({ points: [tsX, oy-ah+2, tsX, oy], stroke: 'rgba(255,213,79,0.4)', strokeWidth: 1, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: tsX-14, y: cy2+14, text: `T_sw\n${this.Tswitch}°C`, fontSize: 6.5, fill: '#ffd54f', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Line({ points: [tcX, oy-ah+2, tcX, oy], stroke: 'rgba(239,83,80,0.4)', strokeWidth: 1, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: tcX-14, y: cy2+14+12, text: `T_c\n${this.Tcurie}°C`, fontSize: 6.5, fill: '#ef9a9a', lineHeight: 1.3 }));

        // 区域着色（三段）
        this._staticGroup.add(new Konva.Rect({ x: ox, y: cy2+13, width: tsX-ox, height: ah-4-13+cy2-cy2, fill: 'rgba(66,165,245,0.06)', cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: cy2+14, text: 'NTC区', fontSize: 6.5, fill: 'rgba(66,165,245,0.5)' }));
        this._staticGroup.add(new Konva.Rect({ x: tsX, y: cy2+13, width: tcX-tsX, height: oy-cy2-13, fill: 'rgba(239,83,80,0.08)', cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: tsX+2, y: cy2+14, text: '突变区', fontSize: 6.5, fill: 'rgba(239,83,80,0.55)' }));
        ...
        this._staticGroup.add(bg, titleBg, this._rtPoint, this._rtHLine, this._rtVLine, this._rtLabel);
    }
    ...
    _drawProtectCircuit() {
        ...
        const titleBg = new Konva.Rect({ x: cx2, y: cy2, width: cw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] });
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: '自恢复保护电路仿真', fontSize: 8, fontStyle: 'bold', fill: '#f57f17', align: 'center' }));

        const lx = cx2+8, rx = cx2+cw-8, midY = cy2+ch/2+4;

        // Vs → PTC → Rload → GND
        this._staticGroup.add(new Konva.Text({ x: lx, y: midY-28, text: `Vs=${this.Vsupply}V`, fontSize: 8, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Line({ points: [lx+30, midY-22, lx+52, midY-22], stroke: '#ef9a9a', strokeWidth: 1.5 }));

        // PTC 符号（正方形加斜线，IEC）
        const ptcX = lx+52, ptcY = midY-30;
        this._staticGroup.add(new Konva.Rect({ x: ptcX, y: ptcY, width: 24, height: 16, fill: 'none', stroke: '#f57f17', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [ptcX, ptcY+16, ptcX+24, ptcY], stroke: '#f57f17', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: ptcX+2, y: ptcY+2, text: 'PTC', fontSize: 7, fill: '#f57f17' }));
        this._staticGroup.add(new Konva.Text({ x: ptcX+26, y: ptcY+3, text: '+T', fontSize: 7, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Line({ points: [ptcX+24, midY-22, ptcX+50, midY-22, ptcX+50, midY-10], stroke: '#ffd54f', strokeWidth: 1.5 }));
        ...
        this._staticGroup.add(new Konva.Rect({ x: rlX, y: rlY, width: 10, height: 22, fill: 'none', stroke: '#66bb6a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: rlX+12, y: rlY+5, text: `RL\n${this.Rload}Ω`, fontSize: 7, fill: '#66bb6a', lineHeight: 1.2 }));
        this._staticGroup.add(new Konva.Line({ points: [rlX+5, rlY+22, rlX+5, midY+20], stroke: '#66bb6a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [lx+28, midY+20, rlX+5, midY+20], stroke: '#b0bec5', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: lx+28, y: midY+22, text: '⏚ GND', fontSize: 7.5, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Line({ points: [lx+30, midY-22, lx+28, midY-22, lx+28, midY+20], stroke: '#ef9a9a', strokeWidth: 1.5 }));
        ...
        this._interactGroup.add(bg, titleBg, this._circVptcLbl, this._circILbl, this._circStateLbl, trigBtn, trigLbl, resetBtn, resetLbl);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [hx, hy+7+i*10, hx+hw, hy+7+i*10], stroke: 'rgba(255,255,255,0.14)', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+17, width: hw-14, text: 'PTC THERMISTOR', fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+27, width: hw-14, text: `Ts=${this.Tswitch}°C  Tc=${this.Tcurie}°C`, fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        [['mA+','#ffd54f',14],['mA−','#90a4ae',34]].forEach(([lbl,col,ty]) => {
            this._staticGroup.add(new Konva.Text({ x: hx+6, y: hy+ty-3, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._tempArc, this._lcdMain, this._lcdUnit, this._lcdR2, this._lcdI, this._lcdState2);
    }
    ...
    _drawStatePanel() {
        ...
        if (btnPanH > 14) {
            ...
            this._staticGroup.add(new Konva.Text({ x: hx+2, y: btnPanY+2, width: hw-4, text: '型号预设', fontSize: 7.5, fill: '#37474f', align: 'center' }));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'T(t)  R(t)  I(t)  P(t)', fontSize: 8, fontStyle: 'bold', fill: '#f57f17', align: 'center' }));
        ...
        ['T(°C)','R(Ω)','I(A)','P(W)'].forEach((lbl,i) => {
            this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+13+h4*i+4, text: lbl, fontSize: 8, fill: cols[i] }));
        });
        ...
        this._staticGroup.add(bg, titleBg, this._wLineT, this._wLineR, this._wLineI, this._wLineP, this._wTLbl, this._wRLbl, this._wILbl, this._wPLbl);
        this._wavH4 = h4;
    }
