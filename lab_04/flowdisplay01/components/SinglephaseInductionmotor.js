...
export class SinglePhaseInductionMotor extends BaseComponent {
    ...
    _drawStatorIron() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._staticGroup.add(new Konva.Circle({ x: cx+(Ro+8)*Math.cos(a), y: cy+(Ro+8)*Math.sin(a), radius: 5, fill: '#263238' }));
        }
        ...
        for (let i = 0; i < 24; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [cx+Ri*Math.cos(a), cy+Ri*Math.sin(a), cx+Ro*Math.cos(a), cy+Ro*Math.sin(a)], stroke: 'rgba(0,0,0,0.14)', strokeWidth: 0.7 }));
        }
        const slotN = 24;
        ...
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: Ri-4, fill: '#0a1520' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-Ro, y: cy-Ro-22, width: Ro*2, text: '单相异步电动机（截面图）', fontSize: 9, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawMainAuxWindings() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx + Ro + 14, y: cy - 8, text: 'M\n主绕组', fontSize: 8, fill: '#ff8f00', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx - Ro - 46, y: cy - 8, text: 'A\n辅助绕组', fontSize: 8, fill: '#66bb6a', lineHeight: 1.3 }));

        // 绕组标注（底部）
        this._staticGroup.add(new Konva.Rect({ x: cx-30, y: cy-Ro-8, width: 24, height: 8, fill: 'rgba(255,143,0,0.15)' }));
        this._staticGroup.add(new Konva.Text({ x: cx-30, y: cy-Ro-7, text: '主绕组(M)', fontSize: 6.5, fill: '#ff8f00' }));
        this._staticGroup.add(new Konva.Rect({ x: cx+6, y: cy-Ro-8, width: 24, height: 8, fill: 'rgba(102,187,106,0.15)' }));
        this._staticGroup.add(new Konva.Text({ x: cx+6, y: cy-Ro-7, text: '辅绕组(A)', fontSize: 6.5, fill: '#66bb6a' }));
    }
    ...
    _drawShaft() {
        ...
        this._staticGroup.add(new Konva.Ellipse({ x: cx+Ro+6, y: cy, radiusX: 7, radiusY: Ro*0.44, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Rect({ x: cx+Ro+8, y: cy-5, width: 22, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Ellipse({ x: cx-Ro-6, y: cy, radiusX: 7, radiusY: Ro*0.44, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1.5 }));
    }

    _drawMagFieldLayer() { this._magGroup = new Konva.Group(); this._staticGroup.add(this._magGroup); }
    ...
    _drawCircuitDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: '电路图（双值电容 电容起动-运行）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        const lx = cx2+10, rx = cx2+cw-10, midY = cy2+halfCH/2+8;
        const vL = cy2+18, vN = cy2+halfCH-8;

        // 电源 L/N 纵线
        this._staticGroup.add(new Konva.Line({ points: [lx, vL, lx, vN], stroke: '#ef9a9a', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Line({ points: [rx, vL, rx, vN], stroke: '#90caf9', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Text({ x: lx-8, y: vL-2, text: 'L', fontSize: 8.5, fontStyle: 'bold', fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: rx+2, y: vL-2, text: 'N', fontSize: 8.5, fontStyle: 'bold', fill: '#90caf9' }));

        // 主绕组（M）支路（中上）
        const m1Y = cy2 + 22, m2Y = cy2 + halfCH - 10;
        const mX1 = lx + Math.round(cw*0.22), mX2 = rx - Math.round(cw*0.22);
        this._staticGroup.add(new Konva.Line({ points: [lx, m1Y, mX1, m1Y], stroke: '#ff8f00', strokeWidth: 1.5 }));
        // 主绕组矩形符号
        this._staticGroup.add(new Konva.Rect({ x: mX1, y: m1Y-5, width: mX2-mX1, height: 10, fill: 'none', stroke: '#ff8f00', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: mX1+4, y: m1Y-3, text: 'M（主绕组）', fontSize: 7.5, fill: '#ff8f00' }));
        this._staticGroup.add(new Konva.Line({ points: [mX2, m1Y, rx, m1Y], stroke: '#ff8f00', strokeWidth: 1.5 }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [lx, aY, crX-2, aY], stroke: '#66bb6a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [crX-2, aY-6, crX-2, aY+6], stroke: '#66bb6a', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Line({ points: [crX+2, aY-6, crX+2, aY+6], stroke: '#66bb6a', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Text({ x: crX-10, y: aY-14, width: 20, text: `Cr\n${this.Cr}μF`, fontSize: 6.5, fill: '#66bb6a', align: 'center', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Line({ points: [crX+2, aY, crX+8, aY], stroke: '#66bb6a', strokeWidth: 1.5 }));

        // 起动电容 Cs + 离心开关（串联）
        const csX = crX + Math.round(cw*0.16);
        const ksX = csX + Math.round(cw*0.14);
        this._staticGroup.add(new Konva.Line({ points: [crX+8, aY, csX-2, aY], stroke: '#ffd54f', strokeWidth: 1.5 }));
        // Cs（虚线框，起动阶段有效）
        this._staticGroup.add(new Konva.Rect({ x: csX-2, y: aY-8, width: ksX-csX-2, height: 16, fill: 'none', stroke: '#ffd54f', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Line({ points: [csX-2, aY-5, csX-2, aY+5], stroke: '#ffd54f', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Line({ points: [csX+2, aY-5, csX+2, aY+5], stroke: '#ffd54f', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Text({ x: csX-8, y: aY-18, width: 16, text: `Cs\n${this.Cs}μF`, fontSize: 6.5, fill: '#ffd54f', align: 'center', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: kX-4, y: aY-14, width: 22, text: 'K\n离心', fontSize: 6.5, fill: '#ffd54f', align: 'center', lineHeight: 1.3 }));
        this._staticGroup.add(this._centrifLineLeft, this._centrifContactLine, this._centrifLineRight);

        // 辅助绕组（A）
        const aW1 = kX + 18;
        const aW2 = rx - Math.round(cw*0.06);
        this._staticGroup.add(new Konva.Line({ points: [aW1, aY, aW1+4, aY], stroke: '#66bb6a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Rect({ x: aW1+4, y: aY-5, width: aW2-aW1-4, height: 10, fill: 'none', stroke: '#66bb6a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: aW1+6, y: aY-3, text: 'A（辅助绕组）', fontSize: 7.5, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Line({ points: [aW2, aY, rx, aY], stroke: '#66bb6a', strokeWidth: 1.5 }));

        // 离心开关状态指示
        this._centrifStatusText = new Konva.Text({ x: kX-14, y: aY+8, width: 44, text: '闭合', fontSize: 7.5, fill: '#ffd54f', align: 'center' });
        this._staticGroup.add(this._centrifStatusText);
    ...
    _drawDirectionSwitch() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: swY+2, width: cw-8, text: '正反转控制（切换辅助绕组极性）', fontSize: 8, fontStyle: 'bold', fill: '#ffd54f', align: 'center' }));
        ...
        for (let i = 0; i < 2; i++) {
            ...
            this._staticGroup.add(new Konva.Circle({ x: lx2, y: swCY, radius: 4, fill: '#ffd54f' }));
            this._staticGroup.add(new Konva.Text({ x: lx2-8, y: swCY+6, width: 16, text: 'COM', fontSize: 6.5, fill: '#ffd54f', align: 'center' }));
        }
        // 正转端（+）
        this._staticGroup.add(new Konva.Circle({ x: swCX-30, y: swCY-16, radius: 3.5, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Circle({ x: swCX+30, y: swCY+16, radius: 3.5, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: swCX-44, y: swCY-21, text: '正转', fontSize: 7, fill: '#66bb6a' }));
        // 反转端（-）
        this._staticGroup.add(new Konva.Circle({ x: swCX-30, y: swCY+16, radius: 3.5, fill: '#ffa726' }));
        this._staticGroup.add(new Konva.Circle({ x: swCX+30, y: swCY-16, radius: 3.5, fill: '#ffa726' }));
        this._staticGroup.add(new Konva.Text({ x: swCX+34, y: swCY+11, text: '反转', fontSize: 7, fill: '#ffa726' }));
        ...
        this._staticGroup.add(this._swBlade1, this._swBlade2);
        ...
        this._staticGroup.add(bg, titleBg, this._dirText);
    ...
    _drawTorqueSpeedCurve() {
        ...
        const titleBg = new Konva.Rect({ x: cx2, y: cy2, width: cw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] });
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'T-n 特性（正向+反向磁场合成）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-14, y: cy2+13, text: 'T', fontSize: 7, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+cw-12, y: oy+2, text: 'n', fontSize: 7, fill: '#80cbc4' }));
        ...
        [0, 500, 1000, 1500, this.syncSpeed].forEach(n => {
            ...
            if (n % 500 === 0 || n===this.syncSpeed) this._staticGroup.add(new Konva.Text({ x: nx-10, y: oy+4, width: 20, text: n===this.syncSpeed?'n₁':n+'', fontSize: 6, fill: '#37474f', align: 'center' }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points: fwPts, stroke: '#66bb6a', strokeWidth: 1.5, lineJoin: 'round', opacity: 0.6 }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: oy-ah+14, text: 'T_f(正向)', fontSize: 7, fill: 'rgba(102,187,106,0.7)' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: rvPts, stroke: '#ffa726', strokeWidth: 1, dash: [4,3], lineJoin: 'round', opacity: 0.5 }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw*0.5, y: oy-ah+14, text: 'T_b(反向)', fontSize: 7, fill: 'rgba(255,167,38,0.7)' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: netPts, stroke: '#4fc3f7', strokeWidth: 2, lineJoin: 'round', opacity: 0.9 }));
    ...
    _drawLCDPanel() {
        ...
        params.forEach(({label,id,unit,color},i)=>{
            ...
            this._staticGroup.add(new Konva.Rect({x:cx3,y:cy3,width:cellW,height:22,fill:'#0d1520',cornerRadius:2}));
            this._staticGroup.add(new Konva.Text({x:cx3+2,y:cy3+2,text:label,fontSize:7,fill:'#37474f'}));
            ...
            this._staticGroup.add(new Konva.Text({x:cx3+2,y:cy3+14,width:cellW-4,text:unit,fontSize:7,fill:'#1a252f',align:'right'}));
            this._lcdCells[id]=val;
            this._staticGroup.add(val);
        });
        this._staticGroup.add(ring, this._lcdBg, this._speedArc, this._lcdMain, this._lcdUnit, this._lcdSlip, this._lcdTorque, this._lcdCsw, bg, titleBg);
    }
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: slY-10, text: '负载 T_L:', fontSize: 8, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Rect({ x: px+4, y: slY, width: slW, height: 10, fill: '#0a0a18', cornerRadius: 2 }));
        ...
        this._interactGroup.add(bg, titleBg, this._loadBarCtrl, this._loadValCtrl, hitLoad);
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'Im 主绕组  Ia 辅助绕组  n 转速', fontSize: 8, fill: '#80cbc4', align: 'center' }));

        const h3 = (wh-12)/3;
        this._wavMids = [wy+12+h3*0.5, wy+12+h3*1.5, wy+12+h3*2.5];
        this._wavMids.forEach(my => this._staticGroup.add(new Konva.Line({ points: [wx+2,my,wx+ww-2,my], stroke: 'rgba(200,200,200,0.07)', strokeWidth: 0.5, dash: [4,3] })));
        ...
        ['Im','Ia','n'].forEach((l,i)=>{
            ...
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+12+h3*i+4,text:l,fontSize:8,fill:cols[i]}));
        });

        this._staticGroup.add(bg, titleBg, this._wLineIm, this._wLineIa, this._wLineN);
        this._wavH3 = h3;
    }
