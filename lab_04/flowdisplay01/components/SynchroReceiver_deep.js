...
export class SynchroReceiver extends BaseComponent {
    ...
    _drawCrossSection() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: mLeft, y: mTop, width: mW, height: mH, fill: '#1c2b38', stroke: '#37474f', strokeWidth: 1.5, cornerRadius: 3 }));
        
        // 定子铁芯（圆环形）
        const statorR = Math.min(mW, mH) * 0.42;
        const statorW = statorR * 1.8;
        const statorH = statorR * 0.9;
        this._staticGroup.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: statorW, radiusY: statorH, fill: '#455a64', stroke: '#263238', strokeWidth: 1 }));
        ...
        windingAngles.forEach((angle, idx) => {
            ...
            this._staticGroup.add(new Konva.Line({ points: [wx+8, wy, termX, termY], stroke: windingColors[idx], strokeWidth: 1, dash: [3,2] }));
            this._staticGroup.add(new Konva.Circle({ x: termX, y: termY, radius: 3.5, fill: windingColors[idx] }));
            this._staticGroup.add(new Konva.Text({ x: termX+5, y: termY-4, text: `S${idx+1}`, fontSize: 7, fill: windingColors[idx], fontStyle: 'bold' }));
        });
        
        // 转子铁芯（凸极或隐极）
        const rotorR = statorR * 0.55;
        this._staticGroup.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: rotorR, radiusY: rotorR * 0.55, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(this._rotorCoil);
        
        // 滑环（转子励磁输入）
        const slipRingY = mBot - 15;
        this._staticGroup.add(new Konva.Rect({ x: mCX-8, y: slipRingY-3, width: 16, height: 6, fill: '#d4a017', stroke: '#8d6e63', strokeWidth: 0.8, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Line({ points: [mCX-8, slipRingY, mLeft-8, slipRingY], stroke: '#ffa726', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Text({ x: mLeft-25, y: slipRingY-5, text: '励磁输入', fontSize: 6.5, fill: '#ffa726' }));
        
        // 输出轴（转角输入）
        this._staticGroup.add(new Konva.Rect({ x: mCX-3, y: mBot, width: 6, height: 12, fill: '#78909c', stroke: '#546e7a', strokeWidth: 0.8 }));
        
        // 误差电压输出端子（转子输出）
        const outX = mRight + 8;
        const outY = ecy + 30;
        this._staticGroup.add(new Konva.Line({ points: [mCX+rotorR*0.6, mCY+rotorH*0.3, outX, outY], stroke: '#ce93d8', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Circle({ x: outX, y: outY, radius: 3.5, fill: '#ce93d8' }));
        this._staticGroup.add(new Konva.Text({ x: outX+5, y: outY-4, text: 'Z1', fontSize: 7, fill: '#ce93d8', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Line({ points: [mCX+rotorR*0.6, mCY+rotorH*0.3+15, outX, outY+20], stroke: '#ce93d8', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Circle({ x: outX, y: outY+20, radius: 3.5, fill: '#ce93d8' }));
        this._staticGroup.add(new Konva.Text({ x: outX+5, y: outY+16, text: 'Z2', fontSize: 7, fill: '#ce93d8', fontStyle: 'bold' }));
    ...
    _drawErrorCharacteristic() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx+4, y: cy+2, width: cw-8, text: '误差特性曲线 U_out = K·sin(Δθ)', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-20, y: oy-ah+10, text: 'U_out', fontSize: 7, fill: '#80cbc4', rotation: -90 }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw-15, y: oy+5, text: 'Δθ (°)', fontSize: 7, fill: '#80cbc4' }));
        
        // 刻度
        for (let deg = -180; deg <= 180; deg += 45) {
            const x = ox + (deg + 180) / 360 * aw;
            const y0 = oy;
            this._staticGroup.add(new Konva.Line({ points: [x, y0-3, x, y0+3], stroke: '#546e7a', strokeWidth: 0.5 }));
            if (Math.abs(deg) % 90 === 0) {
                this._staticGroup.add(new Konva.Text({ x: x-6, y: y0+5, text: `${deg}`, fontSize: 6, fill: '#546e7a' }));
            }
        }
        ...
        this._staticGroup.add(this._sinCurve);
        
        // 工作点标记
        this._workPoint = new Konva.Circle({ x: ox + aw/2, y: oy, radius: 6, fill: '#ff7043', stroke: '#fff', strokeWidth: 1.5 });
        this._staticGroup.add(this._workPoint);
        
        // 零位电压指示线（水平虚线）
        this._nullLine = new Konva.Line({ points: [ox, oy-2, ox+aw, oy-2], stroke: '#ffa726', strokeWidth: 1, dash: [5,5] });
        this._staticGroup.add(this._nullLine);
        
        // 精度区域（±精度线）
        const accDeg = this.accuracy / 60;  // 角分转度
        const accX1 = ox + (accDeg + 180) / 360 * aw;
        const accX2 = ox + (-accDeg + 180) / 360 * aw;
        this._staticGroup.add(new Konva.Line({ points: [accX1, oy-ah, accX1, oy+5], stroke: '#ef5350', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Line({ points: [accX2, oy-ah, accX2, oy+5], stroke: '#ef5350', strokeWidth: 0.8, dash: [4,3] }));
    ...
    _drawMeterPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: mx+4, y: my+2, width: mw-8, text: '运行数据', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        cells.forEach(({ label, id, unit, color, formatter }, i) => {
            ...
            this._staticGroup.add(new Konva.Rect({ x, y, width: cellW-6, height: 32, fill: '#0d1520', stroke: '#1a3040', strokeWidth: 0.6, cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x, y: y+3, width: cellW-6, text: label, fontSize: 7, fill: '#546e7a', align: 'center' }));
            ...
            this._staticGroup.add(new Konva.Text({ x, y: y+26, width: cellW-6, text: unit, fontSize: 6, fill: '#37474f', align: 'center' }));
        ...
        this._staticGroup.add(this._syncIndicator, this._syncText);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '三相输入电压 (S1-S3, S2-S4, S3-S5)    误差电压波形', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        for (let i = 0; i < 3; i++) {
            const yOff = (i - 1) * h3;
            this._staticGroup.add(new Konva.Line({ points: [ox, oy+yOff, ox+aw, oy+yOff], stroke: 'rgba(100,100,100,0.3)', strokeWidth: 0.5 }));
        }
        ...
        this._staticGroup.add(this._waveU1, this._waveU2, this._waveU3, this._waveErr);
        
        // 图例
        const legX = wx + 10, legY = wy + 20;
        [['U_S1S3', '#ef5350'], ['U_S2S4', '#4caf50'], ['U_S3S5', '#2196f3'], ['U_error', '#ffd54f']].forEach(([lbl, col], i) => {
            this._staticGroup.add(new Konva.Line({ points: [legX + i*70, legY, legX + i*70 + 12, legY], stroke: col, strokeWidth: 1.5 }));
            this._staticGroup.add(new Konva.Text({ x: legX + i*70 + 14, y: legY-4, text: lbl, fontSize: 6, fill: col }));
        });
