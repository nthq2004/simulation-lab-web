...
export class UltrasonicLevelSensor extends BaseComponent {
    ...
    _drawSensorHead() {
        ...
        [-flangeW*0.38, flangeW*0.38].forEach(dx => {
            this._staticGroup.add(new Konva.Circle({ x: cx+dx, y: sy+6, radius: 3.5, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
        });
        ...
        this._staticGroup.add(new Konva.Text({ x: cx - bodyW/2 + 2, y: bodyY + 5, text: 'TX', fontSize: 8, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Text({ x: cx - bodyW/2 + 2, y: bodyY + 17, text: 'RX', fontSize: 8, fill: '#546e7a' }));

        this._probeCX = cx;
        this._probeY  = probeY;
        this._probeR  = probeR;

        this._staticGroup.add(outerRing, innerRing, this._probeFace, this._emitLed, this._recvLed);
    ...
    _drawBeamLayer() {
        ...
        this._staticGroup.add(this._beamGroup);
    }
    ...
    _drawTank() {
        ...
        for (let i = 0; i <= 5; i++) {
            const ly = ty + (th * i) / 5;
            this._staticGroup.add(new Konva.Line({ points: [tx+tw, ly, tx+tw+8, ly], stroke: '#78909c', strokeWidth: 0.8 }));
            this._staticGroup.add(new Konva.Text({ x: tx+tw+10, y: ly-5, text: `${100-i*20}%`, fontSize: 8, fill: '#607d8b' }));
        }
        // 盲区警示线（虚线）
        ...
        this._staticGroup.add(outer, this._tankInner, bottom, this._blindLine);
    }
    ...
    _drawTankDynamic() {
        ...
        this._staticGroup.add(this._liquidRect, this._liquidSurf, this._hiAlarmLine, this._loAlarmLine, this._tankBeamGroup, this._surfaceRipple, this._distArrow);
    }
    ...
    _drawAScan() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: oy+2, width: ow-8, text: 'A-Scan  回波信号', fontSize: 8, fontStyle: 'bold', fill: '#00bcd4', align: 'center' }));

        // 网格线
        for (let i = 1; i < 4; i++) {
            this._staticGroup.add(new Konva.Line({ points: [ox, oy+oh*i/4, ox+ow, oy+oh*i/4], stroke: 'rgba(0,188,212,0.08)', strokeWidth: 0.5 }));
        }
        for (let i = 1; i < 5; i++) {
            this._staticGroup.add(new Konva.Line({ points: [ox+ow*i/5, oy, ox+ow*i/5, oy+oh], stroke: 'rgba(0,188,212,0.06)', strokeWidth: 0.5 }));
        }
        ...
        this._staticGroup.add(bg, titleBg, this._txMarker, this._echoMarker, this._oscBlindRect, this._ascanLine, this._oscTofLabel, this._oscDistLabel);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 3; i++) {
            this._staticGroup.add(new Konva.Line({ points: [hx, hy+8+i*10, hx+hw, hy+8+i*10], stroke: 'rgba(255,255,255,0.16)', strokeWidth: 0.8 }));
        }
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+10, y: hy+19, width: hw-20, text: `${this.freqKHz}kHz ULTRASONIC`, fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(midRing, this._lcdBg, this._lvArc, this._lcdMain, this._lcdUnit, this._lcdDist, this._lcdCurrent, this._lcdTemp);
    }
    ...
    _drawBottomPanel() {
        ...
        this._staticGroup.add(bg, this._panelLv, this._panelDist, this._panelSt, this._panelTof);
    }
