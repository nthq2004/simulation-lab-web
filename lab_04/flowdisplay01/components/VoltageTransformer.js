...
export class VoltageTransformer extends BaseComponent {
    ...
    _drawCoreAndWindings() {
        ...
        // 外框
        this._staticGroup.add(new Konva.Rect({ x: ex, y: ey, width: ew, height: eh, fill: '#0d1a24', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 5 }));
        this._staticGroup.add(new Konva.Text({ x: ex, y: ey - 14, width: ew, text: '铁芯与绕组截面（叠片硅钢）', fontSize: 9, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
        ...
        // 顶轭
        this._staticGroup.add(new Konva.Rect({ x: frameX, y: frameY, width: frameW, height: limb, fill: coreCol, stroke: coreEdge, strokeWidth: 1.5 }));
        // 底轭
        this._staticGroup.add(new Konva.Rect({ x: frameX, y: frameY + frameH - limb, width: frameW, height: limb, fill: coreCol, stroke: coreEdge, strokeWidth: 1.5 }));
        // 左柱
        this._staticGroup.add(new Konva.Rect({ x: frameX, y: frameY, width: limb, height: frameH, fill: '#546e7a', stroke: coreEdge, strokeWidth: 1.5 }));
        // 右柱（中柱——绕组所在）
        const midLimbX = frameX + frameW - limb;
        this._staticGroup.add(new Konva.Rect({ x: midLimbX, y: frameY, width: limb, height: frameH, fill: '#546e7a', stroke: coreEdge, strokeWidth: 1.5 }));
        ...
        [[frameX, frameY, frameW, limb],
         ...
         [midLimbX, frameY, limb, frameH]].forEach(([rx, ry, rw, rh]) => {
            for (let i = 2; i < rh; i += 3) {
                this._staticGroup.add(new Konva.Line({ points: [rx+1, ry+i, rx+rw-1, ry+i], stroke: 'rgba(0,0,0,0.14)', strokeWidth: 0.6 }));
            }
        });

        // 铁芯窗口（左右两个窗口）
        this._staticGroup.add(new Konva.Rect({ x: frameX+limb, y: frameY+limb, width: frameW-limb*2, height: frameH-limb*2, fill: '#08111a' }));

        // 标注极性（铁芯窗口内）
        this._staticGroup.add(new Konva.Text({ x: frameX+limb+2, y: frameY+limb+2, width: frameW-limb*2-4, text: 'Φ', fontSize: 13, fontStyle: 'bold', fill: '#ffd54f', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: frameX, y: frameY+frameH*0.45, width: frameW, text: '硅钢叠片铁芯', fontSize: 8, fill: '#78909c', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [w1X1, w1Y1+4,  pri_termX, w1Y1+4,  pri_termX, ecy-28], stroke: '#e53935', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [w1X1, w1Y2-4,  pri_termX, w1Y2-4,  pri_termX, ecy+28], stroke: '#e53935', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Circle({ x: pri_termX, y: ecy-28, radius: 3.5, fill: '#e53935', stroke: '#b71c1c', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Circle({ x: pri_termX, y: ecy+28, radius: 3.5, fill: '#e53935', stroke: '#b71c1c', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: pri_termX-20, y: ecy-34, text: 'A\n(HV)', fontSize: 7.5, fill: '#ef9a9a', lineHeight: 1.3, align: 'right' }));
        this._staticGroup.add(new Konva.Text({ x: pri_termX-20, y: ecy+24, text: 'N\n(HV)', fontSize: 7.5, fill: '#ef9a9a', lineHeight: 1.3, align: 'right' }));
        this._staticGroup.add(new Konva.Text({ x: w1X1+2, y: w1Y1+w1H*0.38, text: `N₁\n≈${this.ratedTurns1}\n匝`, fontSize: 8, fill: '#ef9a9a', lineHeight: 1.4 }));

        // 一次绕组发光（随励磁电流变化）
        this._w1Glow = new Konva.Rect({ x: w1X1-2, y: w1Y1-2, width: w1X2-w1X1+4, height: w1H+4, fill: 'rgba(229,57,53,0)', cornerRadius: 2 });
        this._staticGroup.add(this._w1Glow);
        this._staticGroup.add(this._w1Group);
        ...
        this._staticGroup.add(new Konva.Line({ points: [w2X2, w2Y1+4,  sec_termX, w2Y1+4,  sec_termX, ecy-28], stroke: '#1565c0', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [w2X2, w2Y2-4,  sec_termX, w2Y2-4,  sec_termX, ecy+28], stroke: '#1976d2', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Circle({ x: sec_termX, y: ecy-28, radius: 3.5, fill: '#1565c0', stroke: '#0d47a1', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Circle({ x: sec_termX, y: ecy+28, radius: 3.5, fill: '#1976d2', stroke: '#0d47a1', strokeWidth: 0.8 }));
        // 接地符号（二次侧 n 端接地）
        [0,1,2].forEach(i => this._staticGroup.add(new Konva.Line({ points: [sec_termX-(i+1)*3, ecy+34+(i*4), sec_termX+(i+1)*3, ecy+34+(i*4)], stroke: '#4caf50', strokeWidth: 1.5-i*0.4 })));
        this._staticGroup.add(new Konva.Text({ x: sec_termX+6, y: ecy-34, text: 'a\n(LV)', fontSize: 7.5, fill: '#90caf9', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Text({ x: sec_termX+6, y: ecy+24, text: 'n\n(⏚)', fontSize: 7.5, fill: '#90caf9', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Text({ x: w2X1+1, y: w2Y1+w2H*0.38, text: `N₂\n≈${this.ratedTurns2}\n匝`, fontSize: 8, fill: '#90caf9', lineHeight: 1.4 }));

        // 第三绕组（开口三角，示意）
        const w3Y1 = ecy + 52;
        this._staticGroup.add(new Konva.Line({ points: [w2X2, w3Y1, sec_termX, w3Y1, sec_termX, ecy+64], stroke: '#ff8a65', strokeWidth: 1.5, lineCap: 'round', dash: [4,3] }));
        this._staticGroup.add(new Konva.Line({ points: [w2X2, w3Y1+14, sec_termX, w3Y1+14, sec_termX, ecy+96], stroke: '#ff8a65', strokeWidth: 1.5, lineCap: 'round', dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: w2X1+1, y: w3Y1+2, text: '开口△\n(可选)', fontSize: 7, fill: '#ff8a65', lineHeight: 1.3 }));

        // 绕组发光
        this._w2Glow = new Konva.Rect({ x: w2X1-2, y: w2Y1-2, width: w2X2-w2X1+4, height: w2H+4, fill: 'rgba(21,101,192,0)', cornerRadius: 2 });
        this._staticGroup.add(this._w2Glow);
        this._staticGroup.add(this._w2Group);

        // 匝数比标注
        this._staticGroup.add(new Konva.Text({ x: ex+4, y: ey+eh-14, width: ew-8, text: `K=${this.ratioK}:1  N₁:N₂≈${this.ratedTurns1}:${this.ratedTurns2}`, fontSize: 8, fill: '#4fc3f7', align: 'center' }));
    ...
    _drawEquivCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ex+4, y: ey+2, width: ew-8, text: 'T 形等效电路（折算至一次侧）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        // 上导线
        this._staticGroup.add(new Konva.Line({ points: [x0, y0, x1, y0], stroke: '#546e7a', strokeWidth: 1.5 }));
        // 下导线（公共中线 N）
        this._staticGroup.add(new Konva.Line({ points: [x0, y1, x1, y1], stroke: '#546e7a', strokeWidth: 1.5 }));
        // 左侧竖线（U1 端）
        this._staticGroup.add(new Konva.Line({ points: [x0, y0, x0, y1], stroke: '#ef9a9a', strokeWidth: 1.2 }));
        // 右侧竖线（U2' 端）
        this._staticGroup.add(new Konva.Line({ points: [x1, y0, x1, y1], stroke: '#90caf9', strokeWidth: 1.2 }));

        // ── R1（矩形锯齿）──
        const r1X = x0 + 12, r1W = 28;
        this._drawResistor(r1X, y0, r1W, 8, '#ef9a9a');
        this._staticGroup.add(new Konva.Text({ x: r1X, y: y0-14, text: `R₁\n${this.R1}Ω`, fontSize: 7, fill: '#ef9a9a', align: 'center', width: r1W, lineHeight: 1.3 }));

        // ── X1（电感线圈）──
        const x1X = r1X + r1W + 6, x1W = 28;
        this._drawInductor(x1X, y0, x1W, 7, '#ffd54f');
        this._staticGroup.add(new Konva.Text({ x: x1X, y: y0-14, text: `jX₁\n${this.X1}Ω`, fontSize: 7, fill: '#ffd54f', align: 'center', width: x1W, lineHeight: 1.3 }));

        // ── 励磁支路节点 ──
        this._staticGroup.add(new Konva.Circle({ x: xm, y: y0, radius: 3, fill: '#80cbc4' }));

        // Rm（竖向，并联）
        const rmY0 = y0 + 6, rmH = (y1 - y0) * 0.42;
        this._drawResistor(xm - 14, rmY0, rmH, 8, '#66bb6a', true);
        this._staticGroup.add(new Konva.Text({ x: xm - 28, y: rmY0 + rmH * 0.3, text: `Rm\n${(this.Rm/1e3).toFixed(0)}kΩ`, fontSize: 7, fill: '#66bb6a', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Line({ points: [xm-14, rmY0+rmH, xm-14, y1, xm+14, y1, xm+14, rmY0+rmH], stroke: '#546e7a', strokeWidth: 1 }));

        // jXm（竖向，并联）
        const xmY0 = y0 + 6;
        this._drawInductor(xm + 6, xmY0, rmH, 7, '#ef9a9a', true);
        this._staticGroup.add(new Konva.Text({ x: xm + 18, y: xmY0 + rmH * 0.3, text: `jXm\n${(this.Xm/1e3).toFixed(0)}kΩ`, fontSize: 7, fill: '#ef9a9a', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Line({ points: [xm, y0, xm, rmY0], stroke: '#546e7a', strokeWidth: 1 }));

        // ── R2'（折算）──
        const r2X = xm + 40, r2W = 28;
        this._drawResistor(r2X, y0, r2W, 8, '#90caf9');
        this._staticGroup.add(new Konva.Text({ x: r2X, y: y0-14, text: `R₂'\n${this.R2p}Ω`, fontSize: 7, fill: '#90caf9', align: 'center', width: r2W, lineHeight: 1.3 }));

        // ── X2'（折算）──
        const x2X = r2X + r2W + 6, x2W = 28;
        this._drawInductor(x2X, y0, x2W, 7, '#80cbc4');
        this._staticGroup.add(new Konva.Text({ x: x2X, y: y0-14, text: `jX₂'\n${this.X2p}Ω`, fontSize: 7, fill: '#80cbc4', align: 'center', width: x2W, lineHeight: 1.3 }));

        // ── 负载 ZL'（右侧，竖向）──
        const zlY0 = y0 + 6, zlH = (y1 - y0) * 0.78;
        this._drawResistor(x1 - 4, zlY0, zlH, 7, '#ffa726', true);
        this._staticGroup.add(new Konva.Text({ x: x1 + 4, y: zlY0 + zlH * 0.3, text: `Z_L'\n(负载)`, fontSize: 7, fill: '#ffa726', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, y0, x1, zlY0], stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, zlY0+zlH, x1, y1], stroke: '#546e7a', strokeWidth: 1 }));

        // 端口标注
        this._staticGroup.add(new Konva.Text({ x: x0-10, y: ey+14, text: 'U₁', fontSize: 9, fill: '#ef9a9a', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Text({ x: x1-12, y: ey+14, text: "U₂'", fontSize: 9, fill: '#90caf9', fontStyle: 'bold' }));

        // 电流方向箭头
        this._staticGroup.add(new Konva.Arrow({ points: [x0+4, y0-8, x0+20, y0-8], stroke: '#ef9a9a', fill: '#ef9a9a', strokeWidth: 1, pointerLength: 4, pointerWidth: 3 }));
        this._staticGroup.add(new Konva.Text({ x: x0+6, y: y0-18, text: 'I₁', fontSize: 7.5, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Arrow({ points: [x2X+x2W+4, y0-8, x2X+x2W+20, y0-8], stroke: '#90caf9', fill: '#90caf9', strokeWidth: 1, pointerLength: 4, pointerWidth: 3 }));
        this._staticGroup.add(new Konva.Text({ x: x2X+x2W+6, y: y0-18, text: "I₂'", fontSize: 7.5, fill: '#90caf9' }));
        ...
        this._staticGroup.add(this._i0Arrow);
        this._staticGroup.add(new Konva.Text({ x: xm+5, y: y0+8, text: 'I₀', fontSize: 7.5, fill: '#66bb6a' }));
    }
    ...
    _drawInductor(x, y, len, r, color, vertical = false) {
        ...
        if (!vertical) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [x, y, x, y + 2], stroke: color, strokeWidth: 1.5 }));
            this._staticGroup.add(new Konva.Line({ points: [x, y + len - 2, x, y + len], stroke: color, strokeWidth: 1.5 }));
        }
    }
    ...
    _drawPhasorDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '相量图（动态）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [ocx, py+16, ocx, py+ph-6], stroke: '#1a3040', strokeWidth: 0.7 }));
        this._staticGroup.add(new Konva.Text({ x: ocx+4, y: py+16, text: '+jQ', fontSize: 7, fill: '#37474f' }));
        this._staticGroup.add(new Konva.Text({ x: px+pw-14, y: ocy+3, text: '+P', fontSize: 7, fill: '#37474f' }));
        ...
        [[ocx+R*0.3, ocy-ph*0.28, '#ef9a9a','U₁'],
         ...
         [ocx+R*0.3, ocy-ph*0.04, '#ffd54f','ΔU（误差）']].forEach(([lx,ly,col,lbl]) => {
            this._staticGroup.add(new Konva.Line({ points: [lx, ly, lx+12, ly], stroke: col, strokeWidth: 2 }));
            this._staticGroup.add(new Konva.Text({ x: lx+14, y: ly-4, text: lbl, fontSize: 7, fill: col }));
        });

        this._phOCX = ocx; this._phOCY = ocy; this._phR = R;
        this._staticGroup.add(this._phaU1, this._phaU2k, this._phaI0, this._phaDU);
    }
    ...
    _drawBHCurve() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: bx, y: by, width: bw, height: bh, fill: '#010d18', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 4 }));
        this._staticGroup.add(new Konva.Rect({ x: bx, y: by, width: bw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: bx+4, y: by+2, width: bw-8, text: 'B-H 励磁特性（硅钢片）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-18, y: oy-ah, text: 'B(T)', fontSize: 7, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw+2, y: oy+2, text: 'H', fontSize: 7, fill: '#80cbc4' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: bhPts, stroke: '#4fc3f7', strokeWidth: 2, lineJoin: 'round', opacity: 0.75 }));

        // 饱和点虚线
        const BsatY = oy - (this.Bsat / (this.Bsat * 1.1)) * (ah-2);
        this._staticGroup.add(new Konva.Line({ points: [ox, BsatY, ox+aw, BsatY], stroke: '#ef5350', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: BsatY-10, text: `Bsat=${this.Bsat}T`, fontSize: 6.5, fill: '#ef5350' }));

        // 额定工作点虚线
        const HratedX = ox + (this.Hrated / hMax) * aw;
        const BratedY = oy - (this.Brated / (this.Bsat * 1.1)) * (ah-2);
        this._staticGroup.add(new Konva.Line({ points: [HratedX, oy, HratedX, BratedY], stroke: '#ffd54f', strokeWidth: 0.8, dash: [3,3] }));
        this._staticGroup.add(new Konva.Line({ points: [ox, BratedY, HratedX, BratedY], stroke: '#ffd54f', strokeWidth: 0.8, dash: [3,3] }));

        // 动态工作点
        this._bhPoint  = new Konva.Circle({ x: HratedX, y: BratedY, radius: 4, fill: '#ffd54f', stroke: '#f9a825', strokeWidth: 1.5 });
        this._staticGroup.add(this._bhPoint);
    ...
    _drawErrorCurve() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: ex, y: ey, width: ew, height: eh, fill: '#010d18', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 4 }));
        this._staticGroup.add(new Konva.Rect({ x: ex, y: ey, width: ew, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: ex+4, y: ey+2, width: ew-8, text: `误差特性（${this.accuracy}级）`, fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        const ox = ex+14, oy = ey+eh/2+4, aw = ew-20, ah = (eh-22)/2-4;
        this._staticGroup.add(new Konva.Line({ points: [ox, ey+14, ox, ey+eh-10, ox+aw, ey+eh-10], stroke: '#37474f', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw+2, y: ey+eh-12, text: 'S(VA)', fontSize: 6.5, fill: '#37474f' }));
        this._staticGroup.add(new Konva.Line({ points: [ox, oy, ox+aw, oy], stroke: '#37474f', strokeWidth: 0.6, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox-12, y: oy-4, text: '0%', fontSize: 6.5, fill: '#37474f' }));

        // 精度等级误差带（±fu%）
        const lim  = this._accuracyLimits[this.accuracy] || { fu: 0.5, delta: 20 };
        const fuY0 = oy - (lim.fu / 3.0) * ah;  // +fu%
        const fuY1 = oy + (lim.fu / 3.0) * ah;  // -fu%
        this._staticGroup.add(new Konva.Rect({ x: ox, y: fuY0, width: aw, height: fuY1-fuY0, fill: 'rgba(102,187,106,0.12)', stroke: '#66bb6a', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: fuY0-10, text: `+${lim.fu}%（${this.accuracy}级限）`, fontSize: 6.5, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: fuY1+2, text: `-${lim.fu}%`, fontSize: 6.5, fill: '#66bb6a' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: errPts, stroke: '#ef9a9a', strokeWidth: 1.5, lineJoin: 'round', opacity: 0.7 }));

        // 额定点
        const nRX = ox + (this.ratedVA / (this.ratedVA * 1.5)) * aw;
        this._staticGroup.add(new Konva.Line({ points: [nRX, ey+14, nRX, ey+eh-10], stroke: '#ffd54f', strokeWidth: 0.7, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: nRX-8, y: ey+eh-10, text: 'Sn', fontSize: 6.5, fill: '#ffd54f' }));

        // 动态工作点
        this._errPoint = new Konva.Circle({ x: nRX, y: oy, radius: 4, fill: '#ef5350', stroke: '#c62828', strokeWidth: 1.5 });
        this._staticGroup.add(this._errPoint);
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+4, y: ly+2, width: lw-8, text: '测量仪表', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: px, y: py, width: pw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '参数调节', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        sliders.forEach(({ label, key, color, get, set, display }, si) => {
            ...
            const slY  = py + 38;
            this._staticGroup.add(new Konva.Text({ x: slX, y: slY - 12, text: label, fontSize: 7.5, fill: '#546e7a' }));
        ...
        const btnY = py + ph - 22;
        this._staticGroup.add(new Konva.Text({ x: px+4, y: btnY - 10, text: '精度等级：', fontSize: 8, fill: '#546e7a' }));
        this._accBtns = {};
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'U₁ 一次电压   U₂×K 折算二次电压   ΔU 误差电压   I₀ 励磁电流   B 磁通密度', fontSize: 8, fill: '#80cbc4', align: 'center' }));
        ...
        ['U₁', 'U₂×K', 'ΔU', 'I₀', 'B'].forEach((l, i) => {
            this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+12+h5*i+3, text: l, fontSize: 8, fill: ['#ef9a9a','#90caf9','#ffd54f','#66bb6a','#4fc3f7'][i] }));
        });
        this._staticGroup.add(this._wLU1, this._wLU2k, this._wLErr, this._wLI0, this._wLB);
        this._wavH5 = h5;
    }
