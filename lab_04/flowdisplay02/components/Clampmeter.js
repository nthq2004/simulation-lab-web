...
export class ClampMeter extends BaseComponent {
    ...
    _drawBody() {
        ...
        for (let i = 6; i < bh-4; i += 5) {
            this._staticGroup.add(new Konva.Line({ points: [bx+1, by+i, bx+5, by+i], stroke: '#2a3040', strokeWidth: 2.5, opacity: 0.6 }));
            this._staticGroup.add(new Konva.Line({ points: [bx+bw-1, by+i, bx+bw-5, by+i], stroke: '#2a3040', strokeWidth: 2.5, opacity: 0.6 }));
        }
        // 正面深色区（LCD + 按键区）
        ...
        [0.28, 0.72].forEach((xRatio, i) => {
            ...
            const ty = by + bh - 18;
            this._staticGroup.add(new Konva.Rect({ x: tx-8, y: ty, width: 16, height: 18, fill: i===0?'#1a1a1a':'#c8a000', stroke: '#0a0a0a', strokeWidth: 1, cornerRadius: [1,1,0,0] }));
            this._staticGroup.add(new Konva.Circle({ x: tx, y: ty+8, radius: 5, fill: '#2a2a2a', stroke: '#555', strokeWidth: 1 }));
            this._staticGroup.add(new Konva.Text({ x: tx-12, y: by+bh-10, width: 24, text: i===0?'COM':'V/Ω', fontSize: 7.5, fill: i===0?'#666':'#c8a000', align: 'center' }));
        });

        this._staticGroup.add(neck, body, face);
    }
    ...
    _drawJawBase() {
        ...
        // 铁芯内高光
        this._staticGroup.add(new Konva.Arc({ x: cx, y: cy, innerRadius: R-cw+2, outerRadius: R-cw+6, angle: 178, rotation: startAngle-90, fill: 'rgba(100,150,200,0.08)' }));
        ...
        [[startAngle-90, 'left'], [(startAngle+180)-90, 'right']].forEach(([a_deg, side]) => {
            ...
            this._staticGroup.add(new Konva.Circle({ x: fx, y: fy, radius: cw/2+1, fill: '#3a4a60', stroke: '#1a2030', strokeWidth: 1 }));
        });

        this._staticGroup.add(this._coreFixed);
    }
    ...
    _drawJawMoving() {
        ...
        this._staticGroup.add(this._jawGroup);
    }
    ...
    _drawCoreDetail() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._staticGroup.add(new Konva.Ellipse({ x: coilX, y: cy3, radiusX: 4, radiusY: coilR, fill: 'none', stroke: '#c0a020', strokeWidth: 2, opacity: 0.7 }));
        }
        this._staticGroup.add(new Konva.Text({ x: coilX-12, y: cy + R*0.55, width: 24, text: `N₂\n${this.N2}匝`, fontSize: 7, fill: '#c0a020', align: 'center', lineHeight: 1.3 }));
        ...
        // 互感器比例标注
        this._staticGroup.add(new Konva.Text({ x: cx-30, y: cy+R+8, width: 60, text: `1:${this.N2}\n互感器`, fontSize: 7.5, fill: '#4a6a8a', align: 'center', lineHeight: 1.3 }));
    }
    ...
    _drawMeterFace() {
        ...
        // 表盘背景
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R+4, fill: '#1a2030', stroke: '#0d1020', strokeWidth: 1.5 }));
        const face = new Konva.Circle({ x: cx, y: cy, radius: R, fill: '#e8ddb8' });
        this._staticGroup.add(face);
        ...
        // 表盘玻璃反光
        this._staticGroup.add(new Konva.Ellipse({ x: cx-R*0.2, y: cy-R*0.3, radiusX: R*0.22, radiusY: R*0.10, fill: 'rgba(255,255,255,0.28)' }));

        this._staticGroup.add(this._rangeUnitText, this._rangeIText);
    }
    ...
    _drawDigitalDisplay() {
        ...
        this._staticGroup.add(bg, this._dispI, this._dispUnit, this._dispFreq, this._dispPhase);
    }
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '量程  /  测量相', fontSize: 8, fill: '#4a6a8a', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: adjY+2, width: pw-8, text: '各相电流调节', fontSize: 8, fill: '#4a6a8a', align: 'center' }));

        this.phases.forEach((ph, i) => {
            const barY = adjY + 14 + i * Math.round((adjH-14)/3);
            const barW = pw - 50;
            this._staticGroup.add(new Konva.Text({ x: px+4, y: barY+2, text: ph.label, fontSize: 7.5, fill: ph.color }));
            this._staticGroup.add(new Konva.Rect({ x: px+26, y: barY+2, width: barW, height: 8, fill: '#0a0a10', cornerRadius: 2 }));
            ...
            this._interactGroup.add(adjBg, bar, hitBar, valTxt);
        });
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'i(t) 感应电流波形', fontSize: 8, fill: '#33cc55', align: 'center' }));

        const midY = wy + wh/2;
        this._staticGroup.add(new Konva.Line({ points: [wx+2, midY, wx+ww-2, midY], stroke: 'rgba(100,200,100,0.15)', strokeWidth: 0.5 }));
        this._wLineI = new Konva.Line({ points: [], stroke: '#33ff66', strokeWidth: 1.5, lineJoin: 'round' });

        this._wavMidY = midY;
        this._staticGroup.add(bg, titleBg, this._wLineI);
    }
    ...
    _drawJawTrigger() {
        ...
        this._staticGroup.add(new Konva.Text({ x: trigX-4, y: trigY+30, width: 30, text: '扳机', fontSize: 7.5, fill: '#c05a00' }));
        ...
        this._interactGroup.add(this._trigBtn, this._trigGroup);
        ...
        this._interactGroup.add(jawHit);
    }
