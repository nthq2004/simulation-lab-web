...
export class DisplacerLevelSensor extends BaseComponent {
    ...
    _drawTank() {
        ...
        for (let i = 0; i <= 4; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [tx+tw, ly, tx+tw+7, ly], stroke: '#78909c', strokeWidth: 0.8 }));
            this._staticGroup.add(new Konva.Text({ x: tx+tw+9, y: ly-5, text: `${100-i*25}%`, fontSize: 8, fill: '#607d8b' }));
        }
        // 报警线
        ...
        this._staticGroup.add(outer, topCap, this._innerCav, this._hiLine, this._loLine);
    }
    ...
    _drawSpringHanger() {
        ...
        this._staticGroup.add(beam, rod, this._rodLine, this._springLbl);
    }
    ...
    _drawDisplacerGroup() {
        ...
        this._staticGroup.add(this._displacerGroup);
    }
    ...
    _drawTankDynamic() {
        ...
        this._staticGroup.add(this._springGroup);
    }
    ...
    _drawLVDT() {
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+2, y: ly+3, width: lw-4, text: 'LVDT 差动变压器', fontSize: 8, fontStyle: 'bold', fill: '#e57373', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-bodyW/2+2, y: outY, text: `V_A`, fontSize: 8, fill: '#90caf9' }));
        this._staticGroup.add(new Konva.Text({ x: cx-bodyW/2+2, y: outY+10, text: `V_B`, fontSize: 8, fill: '#a5d6a7' }));
        this._staticGroup.add(new Konva.Text({ x: cx-bodyW/2+2, y: outY+20, text: `ΔV`, fontSize: 8, fill: '#e57373' }));
        ...
        this._staticGroup.add(bg, titleBg, shell, prim, secA, secB, this._coreGroup, this._fieldLinesGroup, this._vALabel, this._vBLabel, this._vOutLabel, this._coreDispLabel);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [hx, hy+7+i*10, hx+hw, hy+7+i*10], stroke: 'rgba(255,255,255,0.16)', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+19, width: hw-16, text: 'DISPLACER  LVDT', fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._lvArc, this._lcdMain, this._lcdUnit, this._lcdBuoy, this._lcdCurr, this._lcdDisp);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: oy+2, width: ow-8, text: 'LVDT 输出信号  ── V_A  ── V_B  ── ΔV=V_A−V_B', fontSize: 8, fontStyle: 'bold', fill: '#ef9a9a', align: 'center' }));

        // 网格
        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [ox, oy+oh*i/3, ox+ow, oy+oh*i/3], stroke: 'rgba(239,154,154,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 5; i++) this._staticGroup.add(new Konva.Line({ points: [ox+ow*i/5, oy, ox+ow*i/5, oy+oh], stroke: 'rgba(239,154,154,0.05)', strokeWidth: 0.5 }));
        ...
        [this._wMidA, this._wMidB, this._wMidD].forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [ox+2, my, ox+ow-2, my], stroke: 'rgba(200,200,200,0.1)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        this._staticGroup.add(bg, titleBg, this._wLineA, this._wLineB, this._wLineD, this._wLblA, this._wLblB, this._wLblD);
    }
