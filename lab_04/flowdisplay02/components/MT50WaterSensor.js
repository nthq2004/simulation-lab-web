...
export class MT50WaterSensor extends BaseComponent {
    ...
    _drawProbeLongSection() {
        ...
        // 尺寸标注
        this._staticGroup.add(new Konva.Text({ x: px+pw+4, y: py+wallT, text: `R₂=${(this.outerRadius*1000).toFixed(1)}mm`, fontSize: 7.5, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Text({ x: cx+coreT+4, y: py+ph/2-6, text: `R₁=${(this.innerRadius*1000).toFixed(1)}mm`, fontSize: 7.5, fill: '#a0a020' }));

        // 法兰（左侧管口）
        [py + wallT + 14, py + ph - wallT - 30].forEach(fY => {
            this._staticGroup.add(new Konva.Rect({ x: px - 16, y: fY, width: 16, height: 20, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1, cornerRadius: [2,0,0,2] }));
            this._staticGroup.add(new Konva.Circle({ x: px-8, y: fY+6, radius: 3, fill: '#37474f' }));
            this._staticGroup.add(new Konva.Circle({ x: px-8, y: fY+14, radius: 3, fill: '#37474f' }));
        });

        this._staticGroup.add(new Konva.Text({ x: px-42, y: py+ph/2-28, text: '燃油\n进口', fontSize: 8, fill: '#ffa726', lineHeight: 1.4 }));
        this._staticGroup.add(new Konva.Text({ x: px-42, y: py+ph/2+10, text: '燃油\n出口', fontSize: 8, fill: '#a1887f', lineHeight: 1.4 }));

        // 流向箭头
        this._staticGroup.add(new Konva.Line({ points: [px-16, py+ph/2-18, px-1, py+ph/2-18], stroke: '#ffa726', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [px-4, py+ph/2-22, px-1, py+ph/2-18, px-4, py+ph/2-14], stroke: '#ffa726', strokeWidth: 1.5, lineJoin: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [px-1, py+ph/2+18, px-16, py+ph/2+18], stroke: '#795548', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [px-12, py+ph/2+14, px-16, py+ph/2+18, px-12, py+ph/2+22], stroke: '#795548', strokeWidth: 1.5, lineJoin: 'round' }));

        // 标注文字
        this._staticGroup.add(new Konva.Text({ x: cx-22, y: py+ph/2-6, text: '内电极 A', fontSize: 8, fontStyle: 'bold', fill: '#ffd54f', align: 'center', width: 44 }));
        this._staticGroup.add(new Konva.Text({ x: px+pw-44, y: py+8, text: '外电极 B', fontSize: 8, fontStyle: 'bold', fill: '#90a4ae' }));
        this._staticGroup.add(new Konva.Text({ x: px+wallT+2, y: py+ph/2-6, text: '测量腔', fontSize: 8, fill: '#4fc3f7' }));

        this._staticGroup.add(outer, topCap, botCap, this._innerRod, connector, insTop, insBot);
    ...
    _drawFluidLayer() {
        ...
        this._staticGroup.add(this._fluidGroup, this._fieldGroup2);
    }

    // ── 电场线层（动态）─────────────────────
    _drawFieldLinesLayer() {
        this._fieldLayer = new Konva.Group();
        this._staticGroup.add(this._fieldLayer);
    }
    ...
    _drawCrossSection() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+2, y: sy+2, width: sw-4, text: '同轴截面', fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));
        ...
        // 内电极高光
        this._staticGroup.add(new Konva.Circle({ x: cx-R1px*0.3, y: cy-R1px*0.3, radius: R1px*0.2, fill: 'rgba(255,230,100,0.4)' }));

        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx-12, y: cy-R1px-12, text: 'A(+)', fontSize: 7, fill: '#ffd54f', align: 'center', width: 24 }));
        this._staticGroup.add(new Konva.Text({ x: cx+R2px+2, y: cy-5, text: 'B(−)', fontSize: 7, fill: '#90a4ae' }));
        ...
        this._staticGroup.add(bg, titleBg, shell, this._secOuter, this._secFluid, this._secFieldGroup, inner, this._secEpsLabel, this._secCapLabel);
    }
    ...
    _drawEquivCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: cy2+2, width: cw-4, text: '等效电路', fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));
        ...
        // 上下导线
        this._staticGroup.add(new Konva.Line({ points: [x1, y1, mx, y1, mx, my-6], stroke: '#4fc3f7', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, y2, mx, y2, mx, my+6], stroke: '#4fc3f7', strokeWidth: 1 }));
        // 极板符号
        this._staticGroup.add(new Konva.Line({ points: [mx-10, my-6, mx+10, my-6], stroke: '#ffd54f', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Line({ points: [mx-10, my+6, mx+10, my+6], stroke: '#90a4ae', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Line({ points: [x2, y1, mx, y1], stroke: '#4fc3f7', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x2, y2, mx, y2], stroke: '#4fc3f7', strokeWidth: 1 }));

        // A、B 端子标注
        this._staticGroup.add(new Konva.Text({ x: x1-2, y: y1-10, text: 'A', fontSize: 9, fontStyle: 'bold', fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: x1-2, y: y2, text: 'B', fontSize: 9, fontStyle: 'bold', fill: '#90a4ae' }));
        ...
        // 振荡框
        this._staticGroup.add(new Konva.Rect({ x: cx2+4, y: cy2+ch-28, width: cw-8, height: 20, fill: '#0d1a30', stroke: '#1a3040', strokeWidth: 1, cornerRadius: 2 }));
        ...
        this._staticGroup.add(bg, titleBg, this._circCxLabel, this._circOscLabel);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [hx, hy+6+i*10, hx+hw, hy+6+i*10], stroke: 'rgba(255,255,255,0.12)', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+18, width: hw-16, text: 'WATER CONTENT SENSOR', fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+27, width: hw-16, text: 'Coaxial Capacitive', fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        [['CAP-A', '#ffd54f', 20], ['CAP-B', '#90a4ae', 44]].forEach(([lbl, col, ty]) => {
            this._staticGroup.add(new Konva.Rect({ x: hx+4, y: hy+ty-7, width: hw-8, height: 14, fill: 'rgba(255,255,255,0.025)', cornerRadius: 2 }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._alarmArc, this._waterArc, this._lcdMain, this._lcdUnit, this._lcdCap, this._lcdEps, this._lcdTemp);
    }
    ...
    _drawWaveformArea() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'C(t) 电容变化  振荡器输出', fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/3, wx+ww, wy+wh*i/3], stroke: 'rgba(79,195,247,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [wx+ww*i/4, wy, wx+ww*i/4, wy+wh], stroke: 'rgba(79,195,247,0.05)', strokeWidth: 0.5 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'C (pF)', fontSize: 8, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+5, text: 'OSC', fontSize: 8, fill: '#4fc3f7' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineCap, this._wLineOsc, this._wCapLbl, this._wOscLbl);
    }
