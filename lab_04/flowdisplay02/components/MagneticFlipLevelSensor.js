...
export class MagneticFlipLevelGauge extends BaseComponent {
    ...
    _drawScaleRuler() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx + tw + 4, y: hiY - 8, text: 'HH', fontSize: 7, fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Text({ x: sx + tw + 4, y: loY + 1, text: 'LL', fontSize: 7, fill: '#ffa726' }));
    ...
    _drawIndicatorColumn() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: cx, y: cy, width: 4, height: ch, fill: '#cfd8dc' }));
        this._staticGroup.add(new Konva.Rect({ x: cx + cw - 4, y: cy, width: 4, height: ch, fill: '#cfd8dc' }));

        this._staticGroup.add(new Konva.Text({ x: cx, y: cy - 18, width: cw, text: '翻板柱', fontSize: 8.5, fontStyle: 'bold', fill: '#37474f', align: 'center' }));

        this._staticGroup.add(frame, topCap, botCap);
    }
    ...
    _drawMainChamber() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx, y: cy - 18, width: cw, text: '主腔', fontSize: 8.5, fontStyle: 'bold', fill: '#37474f', align: 'center' }));

        this._staticGroup.add(outer, inner);
    }
    ...
    _drawConnectingFlanges() {
        ...
        this._staticGroup.add(topFlange, topBolt1, topBolt2, topPipe);
        this._staticGroup.add(botFlange, botBolt1, botBolt2, botPipe);
    }
    ...
    _drawReedArray() {
        ...
        this._staticGroup.add(reedHousing);
    ...
    _drawFloatMagnet() {
        ...
        this._staticGroup.add(this._floatGroup);
    }
    ...
    _drawFlipChips() {
        ...
        this._staticGroup.add(this._levelArrow);
    }
    ...
    _drawTransmitter() {
        ...
        this._staticGroup.add(new Konva.Text({ x: tx+2, y: ty+4, width: tw-4, text: '干簧管远传变送器', fontSize: 8.5, fontStyle: 'bold', fill: '#e8eaf6', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: tx+2, y: ty+13, width: tw-4, text: 'Reed Array · 4~20mA', fontSize: 7, fill: '#9fa8da', align: 'center' }));

        // 电路示意图区域
        const circuitY = ty + 52;
        this._staticGroup.add(new Konva.Rect({ x: tx+4, y: circuitY, width: tw-8, height: 44, fill: '#0a1520', stroke: '#1a3040', strokeWidth: 1, cornerRadius: 2 }));
        
        // 电阻网络示意图
        this._staticGroup.add(new Konva.Text({ x: tx+8, y: circuitY+4, text: '精密电阻网络', fontSize: 6.5, fill: '#8bc34a' }));
        this._staticGroup.add(new Konva.Text({ x: tx+8, y: circuitY+16, text: 'R1 ─┬─ R2 ─┬─ R3 ─┬─ ...', fontSize: 7, fill: '#ffd54f', fontFamily: 'monospace' }));
        this._staticGroup.add(new Konva.Text({ x: tx+8, y: circuitY+28, text: '    ↓     ↓     ↓', fontSize: 6, fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Text({ x: tx+8, y: circuitY+38, text: ' [簧]   [簧]   [簧]', fontSize: 6, fill: '#4fc3f7' }));

        // 接线端子区
        const termY = ty + th - 42;
        this._staticGroup.add(new Konva.Rect({ x: tx+4, y: termY, width: tw-8, height: 38, fill: '#0a1520', stroke: '#1a3040', strokeWidth: 1, cornerRadius: 2 }));

        [['I+ (4-20mA)', '#ffd54f', termY+8], ['COM (-)', '#90a4ae', termY+24]].forEach(([lbl, col, ly]) => {
            this._staticGroup.add(new Konva.Rect({ x: tx+6, y: ly-5, width: tw-12, height: 12, fill: 'rgba(255,255,255,0.03)', cornerRadius: 1 }));
            this._staticGroup.add(new Konva.Text({ x: tx+9, y: ly-2, text: lbl, fontSize: 7.5, fontStyle: 'bold', fill: col }));
        });
        ...
        this._staticGroup.add(new Konva.Circle({ x: this._chamX + this._chamW, y: connY, radius: 4, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1 }));

        this._staticGroup.add(body, nameBg, this._txLcd, this._txCurrText, this._txPctText, this._txResText);
    }
    ...
    _drawMeterDisplay() {
        ...
        this._staticGroup.add(new Konva.Text({ x: mx+2, y: my+4, width: mw-4, text: '干簧管远传读数', fontSize: 7.5, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));
        ...
        this._staticGroup.add(bg, this._meterLvPct, this._meterLvMM, this._meterCurrent, this._meterAlarm);
    }
    ...
    _drawDrainValve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-16, y: cy+16, text: '排污阀', fontSize: 7.5, fill: '#78909c' }));
        this._staticGroup.add(valve, stem, hand);
    }
