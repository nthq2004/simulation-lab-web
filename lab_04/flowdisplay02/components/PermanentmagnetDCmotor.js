...
export class PermanentMagnetDCMotor extends BaseComponent {
    ...
    _drawStatorShell() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 5, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Circle({ x: bx - 1, y: by - 1, radius: 1.5, fill: 'rgba(255,255,255,0.25)' }));
        }
        // 定子内壁（圆柱内面）
        const statorBore = new Konva.Circle({ x: cx, y: cy, radius: R, fill: '#0d1a28' });

        // 外壳高光
        this._staticGroup.add(new Konva.Arc({ x: cx, y: cy, innerRadius: R+4, outerRadius: R+10, angle: 60, rotation: -150, fill: 'rgba(255,255,255,0.08)' }));

        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx - R - 10, y: cy - R - 24, text: '定子（永磁体）', fontSize: 9, fontStyle: 'bold', fill: '#80cbc4' }));

        this._staticGroup.add(statorOuter, statorInner, statorBore);
    }
    ...
    _drawPermanentMagnets() {
        ...
        for (let i = 0; i < p * 2; i++) {
            ...
            this._staticGroup.add(new Konva.Text({ x: lx - 5, y: ly - 5, width: 10, text: isN ? 'N' : 'S', fontSize: 10, fontStyle: 'bold', fill: '#ffffff', align: 'center' }));
            this._staticGroup.add(pole);
        }
    }
    ...
    _drawMagneticFieldLines() {
        ...
        this._staticGroup.add(this._fieldLinesGroup);
    }
    ...
    _drawCommutatorBrushes() {
        ...
        brushData.forEach(({ angle, label, color }) => {
            ...
            this._staticGroup.add(brush, spark);
        });
        this._staticGroup.add(this._comGroup);

        // 换向器中心轴盘
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: comR - 6, fill: '#546e7a', stroke: '#37474f', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: cx - 16, y: cy + comR + 6, text: '换向器', fontSize: 8, fill: '#78909c' }));
    }
    ...
    _drawShaftEnd() {
        ...
        // 轴延伸（右侧）
        this._staticGroup.add(new Konva.Rect({ x: cx + R + 8, y: cy - 6, width: 30, height: 12, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Rect({ x: cx + R + 36, y: cy - 4, width: 16, height: 8, fill: '#90a4ae', stroke: '#607d8b', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: cx + R + 8, y: cy + 10, text: '输出轴', fontSize: 7.5, fill: '#607d8b' }));

        // 端盖
        this._staticGroup.add(new Konva.Ellipse({ x: cx + R + 6, y: cy, radiusX: 8, radiusY: R * 0.55, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Ellipse({ x: cx - R - 6, y: cy, radiusX: 8, radiusY: R * 0.55, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1.5 }));
    }
    ...
    _drawInputWires() {
        ...
        // 引线标注
        this._staticGroup.add(new Konva.Text({ x: cx - R - 28, y: cy - 22, text: 'V+', fontSize: 9, fontStyle: 'bold', fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: cx - R - 28, y: cy + 14, text: 'V−', fontSize: 9, fontStyle: 'bold', fill: '#90caf9' }));
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+17, width: hw-16, text: `${this.ratedVoltage}V  ${this.ratedPower}W`, fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+26, width: hw-16, text: 'PERMANENT MAGNET DC', fontSize: 7, fill: '#90a4ae', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._speedArc, this._lcdMain, this._lcdUnit, this._lcdVolt, this._lcdCurr, this._lcdBackEMF);
    }
    ...
    _drawKnobs() {
        ...
        this._interactGroup.add(base, this._knobRotor, new Konva.Text({ x: kx-16, y: ky+22, width: 32, text: '电压旋钮', fontSize: 8.5, fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawCharCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'n-T 机械特性', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, charLine, this._charPoint);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '电枢电流 I(A)  ── 转速 n(rpm)', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/3, wx+ww, wy+wh*i/3], stroke: 'rgba(255,167,38,0.07)', strokeWidth: 0.5 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'I (A)', fontSize: 8, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+5, text: 'n (rpm)', fontSize: 8, fill: '#ffa726' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineI, this._wLineN, this._wILbl, this._wNLbl);
    }
