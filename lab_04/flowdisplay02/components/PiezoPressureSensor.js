...
export class PiezoelectricPressureSensor extends BaseComponent {
    ...
    _drawChamber() {
        ...
        [[cx+10, cy+10], [cx+cw-10, cy+10], [cx+10, cy+ch-10], [cx+cw-10, cy+ch-10]].forEach(([bx, by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 4, fill: '#263238', stroke: '#1a252f', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Circle({ x: bx-1, y: by-1, radius: 1.2, fill: 'rgba(255,255,255,0.22)' }));
        });
        // 分隔密封槽（中部）
        ...
        // P1/P2 标注
        this._staticGroup.add(new Konva.Text({ x: cx + 8, y: cy + 8, text: 'P₁  (正压腔)', fontSize: 9, fontStyle: 'bold', fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: cx + 8, y: cy + ch/2 + 12, text: 'P₂  (负压腔)', fontSize: 9, fontStyle: 'bold', fill: '#90caf9' }));

        this._staticGroup.add(body, this._topCav, this._botCav);
    }
    ...
    _drawPressurePorts() {
        ...
        ].forEach(n => this._staticGroup.add(n));
        ...
        // 压力传导线（虚线）
        this._staticGroup.add(new Konva.Line({ points: [cx - 1, p1Y, cx + 6, this._crystCY - this._crystH/2 - 4], stroke: '#ef5350', strokeWidth: 1, dash: [3,2], opacity: 0.6 }));
        this._staticGroup.add(new Konva.Line({ points: [cx - 1, p2Y, cx + 6, this._crystCY + this._crystH/2 + 4], stroke: '#1565c0', strokeWidth: 1, dash: [3,2], opacity: 0.6 }));
    }
    ...
    _drawCrystalBase() {
        ...
        [-cw/2 - clampW/2 + 2, cw/2 + clampW/2 - 2].forEach(xOff => {
            for (let dy of [-clampH/3, clampH/3]) {
                this._staticGroup.add(new Konva.Circle({ x: cx + xOff, y: cy + dy, radius: 4, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
            }
        });
        // 顶部电极（金色）
        ...
        // 电极引线
        this._staticGroup.add(new Konva.Line({ points: [cx + cw/2 + 2, cy - ch/2 - 2, this._headX - 4, this._headY + 20], stroke: '#ffd54f', strokeWidth: 1.5, dash: [3,2] }));
        this._staticGroup.add(new Konva.Line({ points: [cx + cw/2 + 2, cy + ch/2 + 2, this._headX - 4, this._headY + 44], stroke: '#ffd54f', strokeWidth: 1.5, dash: [3,2] }));
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx - 30, y: cy - ch/2 - 22, text: '压电晶片（PZT）', fontSize: 9, fontStyle: 'bold', fill: '#80cbc4' }));

        this._staticGroup.add(this._topElectrode, this._botElectrode);
    }

    // ── 电荷层（动态，每帧重绘）────────────
    _drawChargeLayer() {
        this._chargeGroup = new Konva.Group();
        this._staticGroup.add(this._chargeGroup);
    }
    ...
    _drawCrystalDeform() {
        ...
        this._staticGroup.add(this._crystalFill, this._crystalLine, this._crystalTopLine, this._crystalBotLine);
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+17, width: hw-16, text: 'PIEZO  DIFF', fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._chargeArc, this._lcdMain, this._lcdUnit, this._lcdDP, this._lcdVoc, this._lcdDeform);
    }
    ...
    _drawEquivCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: '等效电路  Q=d·A·ΔP　V_oc=Q/C_s', fontSize: 8, fontStyle: 'bold', fill: '#7986cb', align: 'center' }));
        ...
        // 压电源（左侧，菱形标记）
        this._staticGroup.add(new Konva.Line({ points: [x1, y1, x1, y2], stroke: '#7986cb', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1-8, (y1+y2)/2, x1, (y1+y2)/2-10, x1+8, (y1+y2)/2, x1, (y1+y2)/2+10, x1-8, (y1+y2)/2], closed: true, stroke: '#7986cb', fill: '#0d1a30', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: x1+12, y: (y1+y2)/2-5, text: 'Q', fontSize: 9, fontStyle: 'bold', fill: '#7986cb' }));

        // 自身电容 Cs
        const capX = midX;
        this._staticGroup.add(new Konva.Line({ points: [x1, y1, capX, y1], stroke: '#7986cb', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [capX-8, y1+4, capX+8, y1+4], stroke: '#ffd54f', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Line({ points: [capX-8, y1+10, capX+8, y1+10], stroke: '#ffd54f', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Line({ points: [capX, y1+10, capX, y2], stroke: '#7986cb', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: capX+10, y: y1+5, text: 'Cs', fontSize: 8, fill: '#ffd54f' }));

        // 导线连接到输出
        this._staticGroup.add(new Konva.Line({ points: [x2, y1, x2, y2], stroke: '#7986cb', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, y2, x2, y2], stroke: '#7986cb', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, y1, capX-0, y1], stroke: '#7986cb', strokeWidth: 1 }));
        ...
        this._staticGroup.add(bg, titleBg, this._circVout, this._circQlbl);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'Q(t) 电荷输出  ΔP(t) 差压', fontSize: 8, fontStyle: 'bold', fill: '#7986cb', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/3, wx+ww, wy+wh*i/3], stroke: 'rgba(121,134,203,0.08)', strokeWidth: 0.5 }));
        for (let i = 1; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [wx+ww*i/4, wy, wx+ww*i/4, wy+wh], stroke: 'rgba(121,134,203,0.05)', strokeWidth: 0.5 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'Q (pC)', fontSize: 8, fill: '#7986cb' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+6, text: 'ΔP (kPa)', fontSize: 8, fill: '#ef9a9a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineQ, this._wLineDP, this._wQLbl, this._wDPLbl);
    }
    ...
    _drawBottomPanel() {
        ...
        this._staticGroup.add(bg, this._panelDP, this._panelQ, this._panelSt, this._panelDeform);
    }
