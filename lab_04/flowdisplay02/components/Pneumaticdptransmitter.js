...
export class PneumaticDPTransmitter extends BaseComponent {
    ...
    _drawCapsuleBody() {
        ...
        [[cx+5,cy+5],[cx+cw-5,cy+5],[cx+5,cy+ch-5],[cx+cw-5,cy+ch-5]].forEach(([bx,by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 3.5, fill: '#263238' }));
        });
        // 铭牌
        this._staticGroup.add(new Konva.Rect({ x: cx+4, y: cy+ch/2-12, width: cw-8, height: 24, fill: '#1e2a36', cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: cx+4, y: cy+ch/2-10, width: cw-8, text: 'DP\nCAP', fontSize: 8, fontStyle: 'bold', fill: 'rgba(255,255,255,0.28)', align: 'center', lineHeight: 1.4 }));

        // 压力接口法兰
        [[cy+ch*0.28, '#ef9a9a', 'P₁'], [cy+ch*0.72, '#90caf9', 'P₂']].forEach(([fy, col, lbl]) => {
            this._staticGroup.add(new Konva.Rect({ x: cx-12, y: fy-10, width: 14, height: 20, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1, cornerRadius: [2,0,0,2] }));
            this._staticGroup.add(new Konva.Text({ x: cx-24, y: fy-5, text: lbl, fontSize: 8.5, fontStyle: 'bold', fill: col }));
        });
    ...
    _drawPressureChambers() {
        ...
        this._staticGroup.add(this._p1Cav, this._p2Cav);
    }
    ...
    _drawDiaphragmLayer() {
        ...
        this._staticGroup.add(this._diaphragmLayer);
    }
    ...
    _drawConnectingRod() {
        ...
        this._staticGroup.add(this._connectRod, this._rodArrow);
    }
    ...
    _drawLeverAssembly() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: px-12, y: py+12, width: 24, height: 4, fill: '#37474f' }));
        ...
        this._staticGroup.add(this._leverArm, this._pivotDot, this._leverAngleLbl);
        this._staticGroup.add(new Konva.Text({ x: px-14, y: py+18, text: '杠杆支点', fontSize: 7.5, fill: '#546e7a' }));
    }
    ...
    _drawNozzleFlapper() {
        ...
        // 喷嘴供气管（从上方供气）
        this._staticGroup.add(new Konva.Rect({ x: nx+5, y: ny-36, width: 8, height: 20, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: nx+2, y: ny-48, text: '供气', fontSize: 7.5, fill: '#80cbc4' }));
        ...
        this._staticGroup.add(nozzleBody, this._nozzleHole, this._flapperRect, this._gapGroup, this._backPressLine, this._backPressLbl);
        this._staticGroup.add(new Konva.Text({ x: nx+2, y: ny+20, text: '喷嘴', fontSize: 7.5, fill: '#78909c' }));
        this._staticGroup.add(new Konva.Text({ x: nx-14, y: ny+20, text: '挡板', fontSize: 7.5, fill: '#90a4ae' }));
    }
    ...
    _drawAmplifier() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ax+2, y: ay+4, width: aw-4, text: '气动放大器', fontSize: 8.5, fontStyle: 'bold', fill: '#e3f2fd', align: 'center' }));
        ...
        // 供气管（顶部）
        this._staticGroup.add(new Konva.Rect({ x: ax+aw/2-5, y: ay-14, width: 10, height: 14, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: ax+aw/2-18, y: ay-22, text: '140kPa仪表风', fontSize: 7.5, fill: '#80cbc4' }));

        // 供气流动粒子
        this._supplyFlowGroup = new Konva.Group();

        // 输出管（右侧）
        this._staticGroup.add(new Konva.Rect({ x: ax+aw, y: ay+ah/2-5, width: 18, height: 10, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(body, nameBar, this._ampChamber, this._ampGainBar, this._ampGainLbl, this._supplyFlowGroup, this._ampOutLbl);
    }
    ...
    _drawFeedbackBellows() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._staticGroup.add(fl);
        }
        this._staticGroup.add(this._fbBody);
        ...
        this._staticGroup.add(this._fbArrow);
        ...
        this._staticGroup.add(this._fbLbl, this._fbForceLbl);
    }
    ...
    _drawInstrumentLines() {
        ...
        this._staticGroup.add(this._ampToFbLine, this._outFlowGroup);
    }
    ...
    _drawLCDPanel() {
        ...
        for (let i = 0; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [lx, ly+7+i*10, lx+lw, ly+7+i*10], stroke: 'rgba(255,255,255,0.12)', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+7, y: ly+17, width: lw-14, text: 'PNEUMATIC  DP', fontSize: 7, fill: '#78909c', align: 'center' }));
        ...
        [['气动输出','#a5d6a7',18],['mA+','#ffd54f',42],['mA−','#90a4ae',62]].forEach(([lbl,col,ty]) => {
            this._staticGroup.add(new Konva.Text({ x: lx+6, y: ly+ty-3, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
        ...
        this._staticGroup.add(jBox, plate, lcap, rcap, this._idText, body, ring, this._lcdBg, this._outArc, this._lcdDP, this._lcdMain, this._lcdUnit, this._lcdMA, this._lcdGap);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '输出气压(t)  ── kPa 与差压 ΔP 线性关系', fontSize: 8, fontStyle: 'bold', fill: '#66bb6a', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'ΔP', fontSize: 8, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+5, text: 'P_out', fontSize: 8, fill: '#66bb6a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineDP, this._wLineOut, this._wDPLbl, this._wOutLbl);
    }
    ...
    _drawStepLabels() {
        ...
        steps.forEach(({ x, y, text }) => {
            this._staticGroup.add(new Konva.Text({ x: x-24, y: y, width: 48, text, fontSize: 8, fill: '#546e7a', align: 'center' }));
        });
        // 连接箭头
