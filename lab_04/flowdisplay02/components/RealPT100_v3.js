...
export class PT100Sensor3 extends BaseComponent {
    ...
    _drawProbeHousing() {
        ...
        [[px+8,py+8],[px+pw-8,py+8],[px+8,py+headH-8],[px+pw-8,py+headH-8]].forEach(([bx,by]) => {
            ...
            this._staticGroup.add(new Konva.Circle({ x: bx-1, y: by-1, radius: 1.3, fill: 'rgba(255,255,255,0.25)' }));
        });
        // 接线盒铭牌
        this._staticGroup.add(new Konva.Rect({ x: px+6, y: py+headH/2-10, width: pw-12, height: 20, fill: '#1e2a36', cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: px+6, y: py+headH/2-8, width: pw-12, text: 'PT100\nRTD', fontSize: 7.5, fontStyle: 'bold', fill: '#90caf9', align: 'center', lineHeight: 1.3 }));
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._staticGroup.add(new Konva.Circle({ x: fx, y: fy, radius: 3.5, fill: '#37474f' }));
        }
        ...
        // 保护管高光
        this._staticGroup.add(new Konva.Rect({ x: cx2-tubeW/2+2, y: tubeY, width: 4, height: tubeH, fill: 'rgba(255,255,255,0.18)', cornerRadius: [0,0,2,2] }));
        // 管端（测量端，圆头）
        const tipY  = tubeY + tubeH - 6;
        this._staticGroup.add(new Konva.Ellipse({ x: cx2, y: tipY+6, radiusX: tubeW/2, radiusY: 6, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1 }));
        ...
        // 标注：量程
        this._staticGroup.add(new Konva.Text({ x: px+pw+4, y: py+headH+flangeH+4, text: `${this.tempRangeHi}°C`, fontSize: 7.5, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: px+pw+4, y: py+ph-14, text: `${this.tempRangeLo}°C`, fontSize: 7.5, fill: '#90caf9' }));
        // 量程线
        this._staticGroup.add(new Konva.Line({ points: [px+pw+2, py+headH+flangeH, px+pw+12, py+headH+flangeH], stroke: '#ef9a9a', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Line({ points: [px+pw+2, py+ph-6, px+pw+12, py+ph-6], stroke: '#90caf9', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(head, flange, tube, this._heatGlowRect);
    }
    ...
    _drawPlatinumElement() {
        ...
        // 铝氧化物陶瓷基体
        this._staticGroup.add(new Konva.Rect({ x: cx2-ew/2, y: elemY, width: ew, height: elemH, fill: '#f5f5f5', stroke: '#e0e0e0', strokeWidth: 0.5, cornerRadius: 1 }));
        ...
        for (let i = 0; i < lineCount; i++) {
            ...
            this._staticGroup.add(platLine);
        }
        // 铂丝引出线
        this._staticGroup.add(new Konva.Line({ points: [cx2, elemY, cx2, this._probeTubeY + 8], stroke: '#c0a020', strokeWidth: 1.2, opacity: 0.5 }));
        ...
        // 铂丝标注
        this._staticGroup.add(new Konva.Text({ x: cx2-12, y: elemY+elemH+3, width: 24, text: '铂丝\n绕制', fontSize: 7, fill: 'rgba(192,160,32,0.7)', align: 'center', lineHeight: 1.3 }));
    }
    ...
    _drawRTCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'R-T 特性曲线（Callendar-Van Dusen）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-10, y: cy2+14, text: 'R(Ω)', fontSize: 7, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+cw-12, y: oy+2, text: 'T(°C)', fontSize: 7, fill: '#80cbc4' }));
        // T轴刻度
        [-200, 0, 200, 400, 600, 850].forEach(T => {
            ...
            this._staticGroup.add(new Konva.Text({ x: tx-8, y: oy+4, width: 16, text: T.toString(), fontSize: 6, fill: '#37474f', align: 'center' }));
        });
        // R轴刻度
        [0, 100, 200, 300, 400].forEach(R => {
            ...
            this._staticGroup.add(new Konva.Text({ x: ox-20, y: ry-4, width: 18, text: R.toString(), fontSize: 6, fill: '#37474f', align: 'right' }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points: curvePts, stroke: '#4fc3f7', strokeWidth: 1.8, lineJoin: 'round', opacity: 0.8 }));
    ...
    _drawWireModePanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '接线方式 / 误差分析', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._errorText, this._modeDesc);
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+17, width: hw-14, text: 'PT100  IEC 60751', fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._tempArc, this._lcdMain, this._lcdUnit, this._lcdR, this._lcdMA, this._lcdWire);
    }
    ...
    _drawWaveform() {
        ...
        const titleBg = new Konva.Rect({ x: wx, y: wy, width: ww, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] });
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'T(t)温度趋势  R(t)电阻趋势', fontSize: 8, fontStyle: 'bold', fill: '#ff8f00', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+13+4, text: 'T(°C)', fontSize: 8, fill: '#ff8f00' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+13+h2+4, text: 'R(Ω)', fontSize: 8, fill: '#4fc3f7' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineT, this._wLineR, this._wTLbl, this._wRLbl);
        this._wavH2 = h2;
    }
