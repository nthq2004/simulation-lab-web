...
export class Thermocouple extends BaseComponent {
    ...
    _drawProtectionTube() {
        ...
        [[headX+8, py+10],[headX+headW-8, py+10],[headX+8, py+headH-10],[headX+headW-8, py+headH-10]].forEach(([bx,by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 4, fill: '#263238' }));
            this._staticGroup.add(new Konva.Circle({ x: bx-1, y: by-1, radius: 1.5, fill: 'rgba(255,255,255,0.22)' }));
        });
        // 接线盒铭牌
        this._staticGroup.add(new Konva.Rect({ x: headX+8, y: py+headH/2-10, width: headW-16, height: 20, fill: '#1e2a36', cornerRadius: 2 }));
        ...
        this._staticGroup.add(this._typeLbl);
        ...
        for (let i = 2; i < flangeH-1; i += 3) {
            this._staticGroup.add(new Konva.Line({ points: [headX-4, flangeY+i, headX-4+flangeW, flangeY+i], stroke: '#37474f', strokeWidth: 0.5, opacity: 0.5 }));
        }
        ...
        // 套管高光
        this._staticGroup.add(new Konva.Rect({ x: cx2-tubeW/2+2, y: tubeY, width: 4, height: tubeH, fill: 'rgba(255,255,255,0.18)', cornerRadius: [0,0,1,1] }));
        // 管端（半圆头）
        this._staticGroup.add(new Konva.Ellipse({ x: cx2, y: tubeY+tubeH, radiusX: tubeW/2, radiusY: 7, fill: isSteel?'#607d8b':'#bcaaa4', stroke: isSteel?'#37474f':'#a1887f', strokeWidth: 1 }));
        ...
        this._staticGroup.add(head, flange, tube, this._heatGlowEllipse);
    }
    ...
    _drawWires() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2-24, y: this.height-18, text: 'mV+', fontSize: 7.5, fill: tc?.posWire||'#9e9e9e', align: 'right', width: 20 }));
        this._staticGroup.add(new Konva.Text({ x: cx2+6,  y: this.height-18, text: 'mV−', fontSize: 7.5, fill: tc?.negWire||'#bdbdbd' }));

        this._staticGroup.add(this._wire1, this._wire1Label, this._wire2, this._wire2Label, this._extWire1, this._extWire2);
    }
    ...
    _drawColdJunction() {
        ...
        this._staticGroup.add(new Konva.Text({ x: modX+2, y: modY+2, width: modW-4, text: 'CJC\n冷端补偿', fontSize: 7, fill: '#80cbc4', align: 'center', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: this._probeX, y: cy2+6, text: '冷端', fontSize: 7.5, fill: '#90caf9' }));

        this._staticGroup.add(modBg, this._cjcTempText, this._coldJunction, this._coldJunction2);
    }
    ...
    _drawSeebeckArrow() {
        ...
        this._staticGroup.add(this._seebeckArrow, this._seebeckLabel);
    }
    ...
    _drawETCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'E-T 特性曲线（各型号对比）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-16, y: cy2+14, text: 'E(mV)', fontSize: 7, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+cw-14, y: oy+2, text: 'T(°C)', fontSize: 7, fill: '#80cbc4' }));
        ...
        [0, 200, 400, 600, 800, 1000, 1200].forEach(T => {
            ...
            if (T % 400 === 0) this._staticGroup.add(new Konva.Text({ x: tx-8, y: oy+4, width: 16, text: T.toString(), fontSize: 6, fill: '#37474f', align: 'center' }));
        });
        [0, 20, 40, 60].forEach(E => {
            ...
            this._staticGroup.add(new Konva.Text({ x: ox-16, y: ry-4, width: 14, text: E.toString(), fontSize: 6, fill: '#37474f', align: 'right' }));
        });
        ...
        typeOrder.forEach(type => {
            ...
            if (pts.length > 2) {
                this._staticGroup.add(new Konva.Line({ points: pts, stroke: tc.color, strokeWidth: this.tcType === type ? 2.2 : 1.2, lineJoin: 'round', opacity: this.tcType === type ? 0.9 : 0.4, listening: false }));
            }
            // 型号标签（末端）
            if (pts.length >= 2) {
                this._staticGroup.add(new Konva.Text({ x: pts[pts.length-2]+2, y: pts[pts.length-1]-6, text: type, fontSize: 7, fill: tc.color, opacity: 0.7 }));
            }
        });
        ...
        this._staticGroup.add(bg, titleBg, this._etPoint, this._etHLine, this._etVLine, this._etLabel);
    }
    ...
    _drawTypeSelector() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+4, y: sy+2, width: sw-8, text: '分度号选择 / EMF 输出', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._emfText, this._cjcText, this._seebeckText);
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+17, width: hw-14, text: 'THERMOCOUPLE', fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+27, width: hw-14, text: `${this.tempRangeLo}~${this.tempRangeHi}°C`, fontSize: 7, fill: '#90a4ae', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._tempArc, this._lcdMain, this._lcdUnit, this._lcdEmf, this._lcdMA, this._lcdType);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'T(t)  E(mV)', fontSize: 8, fontStyle: 'bold', fill: '#ff5722', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+13+4, text: 'T(°C)', fontSize: 8, fill: '#ff5722' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+13+h2+4, text: 'E(mV)', fontSize: 8, fill: '#ffd54f' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineT, this._wLineE, this._wTLbl, this._wELbl);
        this._wavH2 = h2;
    }
