...
export class ResolverTransducer extends BaseComponent {
    ...
    _drawPneumaticCylinder() {
        ...
        // 气压进口（左侧）
        this._staticGroup.add(new Konva.Rect({ x: cx2-14, y: cy2+ch/2-6, width: 14, height: 12, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: cx2-28, y: cy2+ch/2-14, text: '气压\n进口', fontSize: 7.5, fill: '#80cbc4', lineHeight: 1.3 }));
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx2, y: cy2-20, width: cw, text: '气动\n驱动', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center', lineHeight: 1.3 }));

        this._staticGroup.add(body, topCap, botCap, this._cylCav, this._pressureZone, this._piston, this._pressLabel);
    }
    ...
    _drawPushRod() {
        ...
        this._staticGroup.add(this._pushRod);
    }
    ...
    _drawResolverHousing() {
        ...
        // 外壳圆环
        this._staticGroup.add(new Konva.Circle({ x: cx2, y: cy2, radius: Ro + 12, fill: '#455a64', stroke: '#263238', strokeWidth: 2.5 }));
        // 内壁
        this._staticGroup.add(new Konva.Circle({ x: cx2, y: cy2, radius: Ro, fill: '#0d1a28' }));
        // 外壳高光
        this._staticGroup.add(new Konva.Arc({ x: cx2, y: cy2, innerRadius: Ro+4, outerRadius: Ro+10, angle: 70, rotation: -150, fill: 'rgba(255,255,255,0.08)' }));
        // 安装耳
        for (let i = 0; i < 4; i++) {
            const a = (i/4)*Math.PI*2 + Math.PI/4;
            this._staticGroup.add(new Konva.Circle({ x: cx2+(Ro+9)*Math.cos(a), y: cy2+(Ro+9)*Math.sin(a), radius: 5, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
        }
        // 铭牌
        this._staticGroup.add(new Konva.Rect({ x: cx2-22, y: cy2+Ro*0.6, width: 44, height: 14, fill: '#1e2a36', cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: cx2-22, y: cy2+Ro*0.6+2, width: 44, text: 'RESOLVER', fontSize: 7.5, fill: 'rgba(255,255,255,0.35)', align: 'center' }));

        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx2-Ro, y: cy2-Ro-22, width: Ro*2, text: '旋转变压器', fontSize: 9, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
    }
    ...
    _drawStatorWindings() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+Ro+14, y: cy2-6, text: 'REF\n激励', fontSize: 7.5, fill: '#ff8f00', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2-Ro-42, y: cy2-6, text: 'SIN\n正弦', fontSize: 7.5, fill: '#42a5f5', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+Ro+14, y: cy2+10, text: 'COS\n余弦', fontSize: 7.5, fill: '#66bb6a', lineHeight: 1.3 }));
    }
    ...
    _drawAngleDial() {
        ...
        for (let deg = 0; deg < 360; deg += 10) {
            ...
            if (isMaj) {
                const lr = r2 + 8;
                this._staticGroup.add(new Konva.Text({ x: cx2+lr*Math.cos(rad)-8, y: cy2+lr*Math.sin(rad)-5, width: 16, text: deg.toString(), fontSize: 7, fill: '#607d8b', align: 'center' }));
            }
        }
        ...
        this._staticGroup.add(this._dialPointer, this._dialAngleLbl);
    }

    // ── 磁耦合场动画层 ───────────────────────
    _drawCouplingFieldLayer() {
        this._coupGroup = new Konva.Group();
        this._staticGroup.add(this._coupGroup);
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+17, width: hw-14, text: `${this.excFreq}Hz  ${this.rdcBits}bit`, fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+27, width: hw-14, text: 'RESOLVER', fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        [['COS+','#66bb6a',18],['COS−','#388e3c',38]].forEach(([lbl,col,ty]) => {
            this._staticGroup.add(new Konva.Text({ x: hx+6, y: hy+ty+44-7+2, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
        [['RDC-A','#ffd54f',18],['RDC-B','#ffd54f',38]].forEach(([lbl,col,ty]) => {
            this._staticGroup.add(new Konva.Text({ x: hx+6, y: hy+ty+44+20, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._angleArc, this._lcdMain, this._lcdUnit, this._lcdPress, this._lcdFreq, this._lcdRDC);
    }
    ...
    _drawRDCModule() {
        ...
        this._staticGroup.add(new Konva.Text({ x: rx+2, y: ry+3, width: rw-4, text: 'RDC 解算器', fontSize: 8.5, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: rx+2, y: ry+11, width: rw-4, text: `${this.rdcBits}-bit`, fontSize: 7, fill: '#37474f', align: 'center' }));

        // 输入端（SIN/COS 信号输入）
        const inY = ry + 24;
        ['SIN 输入', 'COS 输入'].forEach((lbl, i) => {
            this._staticGroup.add(new Konva.Rect({ x: rx+4, y: inY+i*16, width: rw-8, height: 13, fill: '#0d2030', cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: rx+5, y: inY+i*16+2, text: lbl, fontSize: 8, fill: '#4fc3f7' }));
        });
        ...
        // 跟踪环路可视化
        const trackY = ry + 62;
        this._staticGroup.add(new Konva.Rect({ x: rx+4, y: trackY, width: rw-8, height: 30, fill: '#020c14', stroke: '#1a3040', strokeWidth: 0.8, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: rx+5, y: trackY+2, text: '跟踪环路', fontSize: 7, fill: '#546e7a' }));
        // 跟踪误差条
        this._staticGroup.add(new Konva.Rect({ x: rx+5, y: trackY+12, width: rw-10, height: 8, fill: '#0d2030', cornerRadius: 2 }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [rx+rw/2, trackY+12, rx+rw/2, trackY+20], stroke: '#37474f', strokeWidth: 1 }));  // 零点线

        // 输出显示
        const outY = trackY + 36;
        this._staticGroup.add(new Konva.Rect({ x: rx+4, y: outY, width: rw-8, height: 40, fill: '#020c14', stroke: '#1a3040', strokeWidth: 0.8, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: rx+5, y: outY+2, text: '数字输出', fontSize: 7, fill: '#ffd54f' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: rx+4, y: outY+40, width: rw-8, text: `分辨率: ${resolutionDeg.toFixed(3)}°`, fontSize: 7, fill: '#546e7a', align: 'center' }));

        // 速度输出（d/dt）
        const velY = outY + 54;
        this._staticGroup.add(new Konva.Rect({ x: rx+4, y: velY, width: rw-8, height: 22, fill: '#020c14', stroke: '#1a3040', strokeWidth: 0.8, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: rx+5, y: velY+2, text: 'dθ/dt', fontSize: 7, fill: '#80cbc4' }));
        ...
        this._staticGroup.add(bg, titleBg, this._rdcSinVal, this._rdcCosVal, this._rdcErrorBar, this._rdcErrorBarNeg, this._rdcAngleDisp, this._rdcAngleDeg, this._rdcVelDisp);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'REF(载波)  SIN输出  COS输出  θ(t)角度', fontSize: 8, fontStyle: 'bold', fill: '#4dd0e1', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineRef, this._wLineSin, this._wLineCos, this._wLineTheta, this._wRefLbl, this._wSinLbl, this._wCosLbl, this._wThetaLbl);
        this._wavH4 = h4;
    }
