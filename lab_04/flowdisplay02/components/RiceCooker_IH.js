...
export class IHRiceCooker extends BaseComponent {
    ...
    _init() {
        ...
        this._drawLabel();
        // 动态层
        this._igbtGroup = new Konva.Group(); this._staticGroup.add(this._igbtGroup);
        this._ntcGroup  = new Konva.Group(); this._staticGroup.add(this._ntcGroup);
        this._coilGroup = new Konva.Group(); this._staticGroup.add(this._coilGroup);
        this._waveGroup = new Konva.Group(); this._staticGroup.add(this._waveGroup);
    ...
    _drawLid() {
        ...
        this._staticGroup.add(this._lidNtcDot);
    ...
    _drawPowerBoard() {
        ...
        this._staticGroup.add(new Konva.Text({ x: pfcX+2, y: compY-4, text: 'L', fontSize: 8, fill: '#ffcc80', fontStyle: 'bold' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: capX+1, y: compY-3, text: 'C', fontSize: 8, fill: '#8090ff', fontStyle: 'bold' }));
        ...
        this._staticGroup.add(this._dcBusLabel);
    }
    ...
    _drawRectBridge(bx, by, size) {
        ...
        // 内部 × 对角线（四只二极管简化）
        this._staticGroup.add(new Konva.Line({ points: [bx-s+2,by-s+2, bx+s-2,by+s-2], stroke:'#60a0ff', strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Line({ points: [bx+s-2,by-s+2, bx-s+2,by+s-2], stroke:'#60a0ff', strokeWidth:0.8 }));
        // 标注
        this._staticGroup.add(new Konva.Text({ x: bx-6, y: by+s+2, text: 'Bridge', fontSize: 7, fill: '#80b0e0' }));
    }
    ...
    _drawCtrlBoard() {
        ...
        // MCU 引脚简化
        for (let i = 0; i < 5; i++) {
            this._staticGroup.add(new Konva.Rect({ x: mcuX-3, y: compY-8+i*4, width: 3, height: 2, fill: '#a0a0a0' }));
            this._staticGroup.add(new Konva.Rect({ x: mcuX+22, y: compY-8+i*4, width: 3, height: 2, fill: '#a0a0a0' }));
        }
        this._staticGroup.add(new Konva.Text({ x: mcuX+3, y: compY-6, text: 'MCU', fontSize: 8, fill: '#60a0ff', fontStyle: 'bold' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ntcX+1, y: compY-13, text: 'Rp', fontSize: 7, fill: '#ffcc80' }));
        // 节点
        this._staticGroup.add(new Konva.Circle({
        ...
        this._staticGroup.add(new Konva.Text({ x: ntcX, y: compY+5, text: 'NTC', fontSize: 7, fill: '#ff8080' }));
        // 连线 Rp→节点→NTC
        this._staticGroup.add(new Konva.Line({ points:[ntcX+5,compY-6,ntcX+5,compY], stroke:'#c0e0c0',strokeWidth:1 }));
        this._staticGroup.add(new Konva.Line({ points:[ntcX+5,compY,ntcX+5,compY+4], stroke:'#c0e0c0',strokeWidth:1 }));
        // ADC 连线 → MCU
        this._staticGroup.add(new Konva.Line({
            points: [ntcX+5, compY, mcuX+22, compY-2],
            stroke: '#40c040', strokeWidth: 1, dash: [3,2],
        }));
        this._staticGroup.add(new Konva.Text({ x: ntcX+12, y: compY-8, text: 'ADC', fontSize: 7, fill: '#40c040' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: drvX+1, y: compY-8, text: 'DRV', fontSize: 7.5, fill: '#ffaa30', fontStyle: 'bold' }));
        // 光耦隔离符号
        this._staticGroup.add(new Konva.Line({
        ...
        this._staticGroup.add(new Konva.Text({ x: mcuX+24, y: compY-4, text: 'PWM', fontSize: 7, fill: '#ffaa30' }));
        // 驱动输出 → 功率板（跨区连线用端口表示）
        this._staticGroup.add(new Konva.Line({
            points: [drvX+18, compY, cb.x+cb.w, compY],
            stroke: '#ff8020', strokeWidth: 1.5,
        }));
        this._staticGroup.add(new Konva.Text({ x: cb.x+cb.w-16, y: compY-8, text: 'Vg', fontSize: 7, fill: '#ff8020' }));
        ...
        this._staticGroup.add(this._ntcLabel);
    }
    ...
    _updateHeaterGlow() {
        ...
        if (!this._coilHeatRect) {
            ...
            this._staticGroup.add(this._coilHeatRect);
        }
