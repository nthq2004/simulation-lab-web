...
export class SolenoidValve extends BaseComponent {
    ...
    _drawTerminals() {
        [this._termL1, this._termL2].forEach((t, i) => {
            ...
            const sr = t.w * 0.28;
            this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: sr, fill: '#999', stroke: '#666', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Line({ points: [cx-sr*0.7,cy, cx+sr*0.7,cy], stroke:'#444', strokeWidth:1, lineCap:'round' }));
            this._staticGroup.add(new Konva.Line({ points: [cx,cy-sr*0.7, cx,cy+sr*0.7], stroke:'#444', strokeWidth:1, lineCap:'round' }));
            // 标注
            this._staticGroup.add(new Konva.Text({
    ...
    _drawStatusIndicator() {
        ...
        this._staticGroup.add(this._statusDot, this._statusText, this._coilDot);
    }
