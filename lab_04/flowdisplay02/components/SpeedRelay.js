...
export class SpeedRelay extends BaseComponent {
    ...
    _drawHousing() {
        ...
        // 外壳圆形
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R+14, fill: '#37474f', stroke: '#263238', strokeWidth: 2.5 }));
        for (let i = 0; i < 4; i++) {
            const a = (i/4)*Math.PI*2 + Math.PI/4;
            this._staticGroup.add(new Konva.Circle({ x: cx+(R+10)*Math.cos(a), y: cy+(R+10)*Math.sin(a), radius: 5.5, fill: '#263238', stroke: '#1a2634', strokeWidth: 0.5 }));
        }
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R, fill: '#0d1a28' }));
        // 轴孔标注
        this._staticGroup.add(new Konva.Text({ x: cx-R, y: cy-R-22, width: R*2, text: '速度继电器（截面图）', fontSize: 9, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: cx-R-14, y: cy, text: '←正转', fontSize: 8, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: cx+R+2, y: cy, text: '反转→', fontSize: 8, fill: '#ffa726' }));
    }
    ...
    _drawStator() {
        ...
        this._staticGroup.add(this._statorGroup);
    }
    ...
    _drawRotor() {
        ...
        // 轴延伸
        this._staticGroup.add(new Konva.Rect({ x: cx-4, y: cy+R+6, width: 8, height: 16, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: cx-12, y: cy+R+24, width: 24, text: '转轴', fontSize: 7.5, fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawSprings() {
        ...
        for (let s = 0; s < 2; s++) {
            ...
            this._staticGroup.add(spGrp);
        }
        this._staticGroup.add(new Konva.Text({ x: cx-R-40, y: cy+R*0.6, text: '弹簧', fontSize: 8, fill: '#78909c' }));
    }
    ...
    _drawVortexLayer() {
        ...
        this._staticGroup.add(this._vortexGroup);
    }
    ...
    _drawContactMechanism() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: cy2+2, width: cw-4, text: '触点机构', fontSize: 8.5, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(this._statorLinkLine);
        ...
        contactDefs.forEach(({ id, label, type, side, y, color }) => {
            ...
            const fixedY  = gy;
            this._staticGroup.add(new Konva.Rect({ x: fixedX-3, y: fixedY-6, width: 6, height: 12, fill: '#c0a020', stroke: '#8a7010', strokeWidth: 0.8, cornerRadius: 1 }));
            this._staticGroup.add(new Konva.Line({ points: [fixedX, fixedY-6, fixedX, cy2+14], stroke: color, strokeWidth: 1.5, dash: [3,2], opacity: 0.5 }));
            ...
            this._staticGroup.add(led, lbl, stateText);
        });

        this._staticGroup.add(bg, titleBg, this._contactBarGroup);

        // 分隔线（正/反转组之间）
        const divY = cy2 + ch/2;
        this._staticGroup.add(new Konva.Line({ points: [cx2+4, divY, cx2+cw-4, divY], stroke: '#1a3040', strokeWidth: 1, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: divY-10, width: cw-4, text: '—— 正转触点 ——', fontSize: 7.5, fill: '#66bb6a', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: divY+3, width: cw-4, text: '—— 反转触点 ——', fontSize: 7.5, fill: '#ffa726', align: 'center' }));
    }
    ...
    _drawSpeedTorqueCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'n-θ 特性（迟滞回路）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        // 坐标系
        const ox = cx2+12, oy = cy2+ch/2, aw = cw-18, ah = (ch-22)/2;
        // 零点横线（n=0分界）
        this._staticGroup.add(new Konva.Line({ points: [ox, oy, ox+aw, oy], stroke: '#37474f', strokeWidth: 1 }));
        // 纵轴
        this._staticGroup.add(new Konva.Line({ points: [ox, cy2+13, ox, cy2+ch-6], stroke: '#37474f', strokeWidth: 1 }));
        // 轴标签
        this._staticGroup.add(new Konva.Text({ x: ox-8, y: cy2+13, text: 'θ', fontSize: 8, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+cw-14, y: oy+2, text: 'n', fontSize: 8, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: cy2+13, text: '+正转', fontSize: 7, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: cy2+ch-14, text: '−反转', fontSize: 7, fill: '#ffa726' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [ox, oy, nActX, oy-ah*0.6, fullX, fullY_up], stroke: '#66bb6a', strokeWidth: 1.5, lineJoin: 'round', opacity: 0.7 }));
        // 下降曲线（迟滞）
        this._staticGroup.add(new Konva.Line({ points: [fullX, fullY_up, nResX, oy-ah*0.35, ox, oy], stroke: '#66bb6a', strokeWidth: 1, dash: [4,3], lineJoin: 'round', opacity: 0.45 }));

        // 反转特性（镜像，向下）
        this._staticGroup.add(new Konva.Line({ points: [ox, oy, nActX, oy+ah*0.6, fullX, oy+ah-4], stroke: '#ffa726', strokeWidth: 1.5, lineJoin: 'round', opacity: 0.7 }));
        this._staticGroup.add(new Konva.Line({ points: [fullX, oy+ah-4, nResX, oy+ah*0.35, ox, oy], stroke: '#ffa726', strokeWidth: 1, dash: [4,3], lineJoin: 'round', opacity: 0.45 }));

        // n_act / n_reset 标注
        this._staticGroup.add(new Konva.Line({ points: [nActX, cy2+13, nActX, cy2+ch-6], stroke: 'rgba(239,83,80,0.35)', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Text({ x: nActX-8, y: cy2+14, text: 'n_a', fontSize: 7, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Line({ points: [nResX, cy2+13, nResX, cy2+ch-6], stroke: 'rgba(255,167,38,0.30)', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Text({ x: nResX-8, y: cy2+14, text: 'n_r', fontSize: 7, fill: '#ffa726' }));
        ...
        this._staticGroup.add(bg, titleBg, this._workPointUp, this._workPointDown);
    }
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+4, y: ly+2, width: lw-8, text: '参数显示', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._lcdN, this._lcdTheta, this._lcdState);
    }
    ...
    _drawSpeedSlider() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+4, y: sy+2, width: sw-8, text: '转速调节（向右=正转，向左=反转）', fontSize: 8, fontStyle: 'bold', fill: '#ffd54f', align: 'center' }));
        ...
        // 零点线
        this._staticGroup.add(new Konva.Line({ points: [midX, barY-3, midX, barY+barH+3], stroke: '#37474f', strokeWidth: 1.5 }));
        // 正转区标注（右侧）
        this._staticGroup.add(new Konva.Text({ x: midX+4, y: barY-10, text: '→ 正转', fontSize: 7.5, fill: '#66bb6a' }));
        // 反转区标注（左侧）
        this._staticGroup.add(new Konva.Text({ x: barX, y: barY-10, text: '反转 ←', fontSize: 7.5, fill: '#ffa726' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [nActXFwd, barY-2, nActXFwd, barY+barH+2], stroke: 'rgba(239,83,80,0.5)', strokeWidth: 1, dash: [2,2] }));
        this._staticGroup.add(new Konva.Line({ points: [nActXRev, barY-2, nActXRev, barY+barH+2], stroke: 'rgba(239,83,80,0.5)', strokeWidth: 1, dash: [2,2] }));
        this._staticGroup.add(new Konva.Text({ x: nActXFwd-8, y: barY+barH+3, width: 16, text: `${this.nAct}`, fontSize: 7, fill: '#ef9a9a', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._speedBarFwd, this._speedBarRev, this._speedSlider, this._speedValText);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'n(t) 转速曲线  ── θ(t) 偏转角曲线', fontSize: 8, fill: '#80cbc4', align: 'center' }));

        const midY = wy + wh/2;
        this._staticGroup.add(new Konva.Line({ points: [wx+2, midY, wx+ww-2, midY], stroke: 'rgba(100,200,200,0.12)', strokeWidth: 0.5, dash: [4,3] }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+12+4, text: 'n', fontSize: 8, fill: '#4dd0e1' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: midY+4, text: 'θ', fontSize: 8, fill: '#ffd54f' }));
        this._wavMidN = wy + wh*0.28;
        this._wavMidD = midY + wh*0.24;

        this._staticGroup.add(bg, titleBg, this._wLineN, this._wLineD);
    }
