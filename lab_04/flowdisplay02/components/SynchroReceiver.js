...
export class SynchroReceiver extends BaseComponent {
    ...
    _drawReceiverSection() {
        ...
        statorAxes.forEach((a, i) => {
            ...
            this._staticGroup.add(dot, dotB);
            // 轴标注
            this._staticGroup.add(new Konva.Text({
        ...
        // 集电环（转子输出引出）
        this._staticGroup.add(new Konva.Ring({ x: ecx, y: ecy, innerRadius: rotorR*0.28, outerRadius: rotorR*0.38, fill: 'rgba(200,120,50,0.25)', stroke: '#a06020', strokeWidth: 0.8 }));

        // ── 端子接线 ──
        // 定子输入端子（右侧，来自发送机）
        const inX = ex + ew + 10;
        [['S1','#e53935',-28],['S2','#43a047',0],['S3','#1e88e5',28]].forEach(([l,c,dy]) => {
            this._staticGroup.add(new Konva.Line({ points:[ecx+sOuter*0.65,ecy+dy,inX,ecy+dy], stroke:c, strokeWidth:2 }));
            this._staticGroup.add(new Konva.Circle({ x:inX, y:ecy+dy, radius:3.5, fill:c }));
            this._staticGroup.add(new Konva.Text({ x:inX+5, y:ecy+dy-6, text:l, fontSize:9, fill:c, fontStyle:'bold' }));
        });

        // 转子输出端子（左侧，误差信号输出）
        const outX = ex - 10;
        [['R1','#ef9a9a',-20],['R2','#90caf9',20]].forEach(([l,c,dy]) => {
            this._staticGroup.add(new Konva.Line({ points:[outX,ecy+dy,ex+8,ecy+dy], stroke:c, strokeWidth:2 }));
            this._staticGroup.add(new Konva.Circle({ x:outX, y:ecy+dy, radius:3.5, fill:c }));
            this._staticGroup.add(new Konva.Text({ x:ex-26, y:ecy+dy-5, text:l, fontSize:8, fill:c, fontStyle:'bold' }));
        });

        // PSD 输出端子（左侧，直流）
        this._staticGroup.add(new Konva.Line({ points:[outX,ecy+58,ex+8,ecy+58], stroke:'#ffd54f', strokeWidth:2, dash:[3,3] }));
        this._staticGroup.add(new Konva.Circle({ x:outX, y:ecy+58, radius:3.5, fill:'#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x:ex-30, y:ecy+53, text:'PSD\n出', fontSize:7, fill:'#ffd54f', lineHeight:1.2 }));

        // 随动轴（底部）
        this._staticGroup.add(new Konva.Rect({ x:ecx-5, y:ey+eh, width:10, height:10, fill:'#66bb6a', stroke:'#2e7d32', strokeWidth:1 }));
        this._staticGroup.add(new Konva.Text({ x:ecx-18, y:ey+eh+12, text:'随动电机驱动', fontSize:7, fill:'#66bb6a' }));

        // 接线说明（右下）
        this._staticGroup.add(new Konva.Rect({ x:ex+4, y:ey+eh-30, width:ew-8, height:26, fill:'#0a1520', stroke:'#1a3040', strokeWidth:0.8, cornerRadius:2 }));
        this._staticGroup.add(new Konva.Text({ x:ex+6, y:ey+eh-28, text:'接线：TX.S1→CT.S1\nTX.S2→CT.S2  TX.S3→CT.S3', fontSize:7, fill:'#546e7a', lineHeight:1.4 }));
    }
    ...
    _drawRotorLayer() {
        ...
        this._staticGroup.add(this._fieldArrow);
        this._staticGroup.add(new Konva.Text({
    ...
    _drawErrorCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x:ex+4, y:ey+2, width:ew-8, text:'误差特性 e_out = K·U·sin(Δθ)', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ox=ex+14, oy=ey+eh*0.54, aw=ew-20, ah=eh*0.38;
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah,ox,oy+ah,ox,oy,ox+aw,oy], stroke:'#37474f', strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:ox-13,y:oy-ah,text:'e_out', fontSize:6.5, fill:'#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw+2,y:oy+2,text:'Δθ', fontSize:6.5, fill:'#80cbc4' }));

        // -180°/0°/+180° 标注
        [[-180,-1],[0,0],[90,0.5],[180,1]].forEach(([d,xr]) => {
            const x2 = ox + (d+180)/360*aw;
            this._staticGroup.add(new Konva.Line({ points:[x2,oy-3,x2,oy+3], stroke:'#37474f', strokeWidth:0.7 }));
            this._staticGroup.add(new Konva.Text({ x:x2-8,y:oy+5, text:`${d}°`, fontSize:5.5, fill:'#37474f', width:16, align:'center' }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points:sinPts, stroke:'#ef5350', strokeWidth:2, lineJoin:'round', opacity:0.8 }));
        ...
        this._staticGroup.add(new Konva.Line({ points:linPts, stroke:'#4fc3f7', strokeWidth:1.5, lineJoin:'round', opacity:0.7, dash:[5,3] }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw*0.55,y:oy-ah*0.7,text:'实际', fontSize:6.5, fill:'#ef5350' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw*0.55,y:oy-ah*0.55,text:'线性近似', fontSize:6.5, fill:'#4fc3f7' }));

        // 零位 ± 区域（绿色）
        const nullX = ox + (0+180)/360*aw;
        this._staticGroup.add(new Konva.Rect({ x:nullX-aw*0.08, y:oy-ah, width:aw*0.16, height:ah*2, fill:'rgba(102,187,106,0.08)', stroke:'#66bb6a', strokeWidth:0.6, dash:[3,3] }));
        this._staticGroup.add(new Konva.Text({ x:nullX-12, y:oy-ah+2, text:'线性区', fontSize:6, fill:'#66bb6a' }));

        // 动态工作点
        this._errCrvPoint = new Konva.Circle({ x:nullX, y:oy, radius:5, fill:'#ffd54f', stroke:'#f9a825', strokeWidth:1.5 });
        this._staticGroup.add(this._errCrvPoint);
    ...
    _drawSystemBlock() {
        ...
        this._staticGroup.add(new Konva.Text({ x:bx+4, y:by+2, width:bw-8, text:'随动系统闭环结构（CX-TX → CT → PSD → Amp → 随动电机）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        blocks.forEach(({ lbl, x, w, col, bg }) => {
            this._staticGroup.add(new Konva.Rect({ x, y:y0-13, width:w, height:26, fill:bg, stroke:col, strokeWidth:1, cornerRadius:3 }));
            this._staticGroup.add(new Konva.Text({ x, y:y0-11, width:w, text:lbl, fontSize:6.5, fill:col, align:'center', lineHeight:1.35 }));
        });
        ...
        for (let i = 0; i < blocks.length-1; i++) {
            ...
            this._staticGroup.add(new Konva.Arrow({ points:[x1,y0,x2,y0], stroke:arrowColors[i], fill:arrowColors[i], strokeWidth:1, pointerLength:4, pointerWidth:3 }));
        }
        ...
        this._staticGroup.add(new Konva.Text({ x:bx+bw*0.55, y:fbY-9, text:'θ_CT 反馈（机械轴）', fontSize:7, fill:'#4fc3f7', opacity:0.9 }));
        ...
        this._staticGroup.add(this._blkErrLabel, this._blkStateLabel);
    }
    ...
    _drawPhasorDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({ x:px+4, y:py+2, width:pw-8, text:'磁场与转子相量', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        this._staticGroup.add(this._phaArc,this._phaPhi_S,this._phaR_axis,this._phaEout,this._phaAngleLabel);
    }
    ...
    _drawPSDPanel() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:px, y:py, width:pw, height:ph, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({ x:px, y:py, width:pw, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:px+4, y:py+2, width:pw-8, text:'相敏解调器（PSD）原理', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        boxes.forEach(({ lbl, x, col, bg }, i) => {
            this._staticGroup.add(new Konva.Rect({ x, y:boxY, width:boxW, height:boxH, fill:bg, stroke:col, strokeWidth:1, cornerRadius:3 }));
            this._staticGroup.add(new Konva.Text({ x, y:boxY+2, width:boxW, text:lbl, fontSize:6.5, fill:col, align:'center', lineHeight:1.3 }));
            if (i < boxes.length-1)
                this._staticGroup.add(new Konva.Arrow({ points:[x+boxW+1,boxY+boxH/2,x+boxW+gap,boxY+boxH/2], stroke:col, fill:col, strokeWidth:1, pointerLength:4, pointerWidth:3 }));
        });
        ...
        // 调幅波（上半）
        this._staticGroup.add(new Konva.Text({ x:wx, y:wY0-2, text:'e_out（调幅）', fontSize:6.5, fill:'#ef5350' }));
        const amPts=[];
        for (let i=0;i<60;i++) {
        ...
        this._staticGroup.add(new Konva.Line({ points:amPts, stroke:'#ef5350', strokeWidth:1.5, lineJoin:'round', opacity:0.65 }));
        this._staticGroup.add(new Konva.Line({ points:[wx,wY0+wH/2,wx+ww,wY0+wH/2], stroke:'#37474f', strokeWidth:0.5 }));

        // 解调后直流（下半）
        const dcY=wY0+wH+6;
        this._staticGroup.add(new Konva.Text({ x:wx, y:dcY-2, text:'V_DC（解调）', fontSize:6.5, fill:'#ffd54f' }));
        ...
        this._staticGroup.add(new Konva.Line({ points:[wx,dcY,wx,dcY+wH+4,wx+ww,dcY+wH+4,wx+ww,dcY], stroke:'#37474f', strokeWidth:0.5 }));
        this._staticGroup.add(new Konva.Line({ points:[wx,dcY+wH/2,wx+ww,dcY+wH/2], stroke:'#37474f', strokeWidth:0.4, dash:[3,3] }));
        this._staticGroup.add(this._psdDCLine, this._psdDCLabel);
    ...
    _drawDynamicTrace() {
        ...
        this._staticGroup.add(new Konva.Text({ x:dx+4, y:dy+2, width:dw-8, text:'动态跟踪响应曲线', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        this._staticGroup.add(this._dynTXLine, this._dynCTLine, this._dynErrLine);
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:px,y:py,width:pw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:px+4,y:py+2,width:pw-8,text:'随动系统控制',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x:wx+4,y:wy+1,width:ww-8,text:'定子输入 e_S1   误差输出 e_out（调幅波）   PSD直流 V_DC   角差 Δθ',fontSize:8,fill:'#80cbc4',align:'center' }));
        ...
        ['e_S1（定子输入）','e_out（误差调幅波）','V_DC（PSD解调）','Δθ（角差）'].forEach((l,i) => {
            this._staticGroup.add(new Konva.Text({ x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#e53935','#ef9a9a','#ffd54f','#4fc3f7'][i] }));
        });
        this._staticGroup.add(this._wLES1,this._wLEout,this._wLVDC,this._wLDelta);
        this._wavHCh=hCh;
    }
