...
export class TorqueSynchroReceiver extends BaseComponent {
    ...
    _drawTRSection() {
        ...
        for (let p = 0; p < 2; p++) {
            this._staticGroup.add(new Konva.Arc({ x: ecx, y: ecy, innerRadius: rotorR*0.72, outerRadius: rotorR*0.95, angle: 90, rotation: p*180-45, fill: '#455a64', stroke: '#263238', strokeWidth: 1 }));
        }
        this._rotorR  = rotorR;
        this._sInner  = sInner;
        this._sOuter  = sOuter;
        ...
        // 集电环（TR 需要集电环引入转子励磁）
        this._staticGroup.add(new Konva.Ring({ x: ecx, y: ecy, innerRadius: rotorR*0.26, outerRadius: rotorR*0.37, fill: 'rgba(200,120,50,0.28)', stroke: '#a06020', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: ecx - 18, y: ecy - rotorR*0.37 - 12, text: '集电环（励磁引入）', fontSize: 7, fill: '#c87832' }));

        // ── 端子 ──
        // 励磁端子（左侧，R1/R2）
        const excX = ex - 10;
        [['R1','#ef9a9a',-20],['R2','#90caf9',20]].forEach(([l,c,dy]) => {
            this._staticGroup.add(new Konva.Line({ points:[excX,ecy+dy,ex+8,ecy+dy], stroke:c, strokeWidth:2 }));
            this._staticGroup.add(new Konva.Circle({ x:excX, y:ecy+dy, radius:3.5, fill:c }));
            this._staticGroup.add(new Konva.Text({ x:ex-26, y:ecy+dy-5, text:l, fontSize:8, fill:c, fontStyle:'bold' }));
        });
        // 励磁标注（★ 力矩式 TR 特征）
        this._staticGroup.add(new Konva.Text({ x:ex-28, y:ecy+32, width:24, text:'励磁↑\n同TX', fontSize:6.5, fill:'#c87832', lineHeight:1.3, align:'center' }));
        ...
        // 输出轴（底部，驱动指针）
        this._staticGroup.add(new Konva.Rect({ x:ecx-6, y:ey+eh, width:12, height:14, fill:'#66bb6a', stroke:'#2e7d32', strokeWidth:1.5 }));
        this._staticGroup.add(new Konva.Text({ x:ecx-18, y:ey+eh+15, text:'指针/负载', fontSize:7, fill:'#66bb6a' }));
        ...
        this._staticGroup.add(this._outOfStepOverlay, this._outOfStepLabel);

        // 接线说明
        this._staticGroup.add(new Konva.Rect({ x:ex+3, y:ey+eh-30, width:ew-6, height:26, fill:'#0a1520', stroke:'#1a3040', strokeWidth:0.8, cornerRadius:2 }));
        this._staticGroup.add(new Konva.Text({ x:ex+5, y:ey+eh-28, text:'★ TR 特征：R1/R2 通励磁\n定子 S1/S2/S3 接 TX 输出', fontSize:7, fill:'#ffa726', lineHeight:1.4 }));
    }
    ...
    _drawFieldLayer() {
        ...
        this._staticGroup.add(this._fieldGroup, this._fieldArrow);
    }
    ...
    _drawRotorLayer() {
        ...
        this._staticGroup.add(this._rotorFieldArrow);
    }
    ...
    _drawTorqueAngleCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x:tx+4,y:ty+2,width:tw-8,text:'力矩-角度特性 T = Ks·sin(Δθ)',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));

        const ox=tx+14, oy=ty+th*0.56, aw=tw-20, ah=th*0.38;
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah,ox,oy+ah*0.6,ox,oy,ox+aw,oy], stroke:'#37474f', strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:ox-13,y:oy-ah,text:'T(N·m)',fontSize:6.5,fill:'#80cbc4' }));
        ...
        this._staticGroup.add(new Konva.Line({ points:sinPts, stroke:'#66bb6a', strokeWidth:2, lineJoin:'round', opacity:0.8 }));

        // ±90° 稳定极限竖线
        [90,-90].forEach(d => {
            const x2=ox+(d+180)/360*aw;
            this._staticGroup.add(new Konva.Line({ points:[x2,ty+14,x2,oy+ah*0.4],stroke:'#ef5350',strokeWidth:0.8,dash:[3,3] }));
            this._staticGroup.add(new Konva.Text({ x:x2-4,y:oy+ah*0.4+2,text:`${d>0?'+':''}${d}°`,fontSize:6,fill:'#ef5350' }));
        });

        // 失步区（|Δθ|>90°）填色
        const x90p=ox+(90+180)/360*aw, x90n=ox+(-90+180)/360*aw;
        this._staticGroup.add(new Konva.Rect({ x:x90p,y:oy-ah,width:ox+aw-x90p,height:ah*2,fill:'rgba(239,83,80,0.08)' }));
        this._staticGroup.add(new Konva.Rect({ x:ox,y:oy-ah,width:x90n-ox,height:ah*2,fill:'rgba(239,83,80,0.08)' }));
        this._staticGroup.add(new Konva.Text({ x:x90p+2,y:ty+16,text:'失步\n区',fontSize:6.5,fill:'#ef5350',lineHeight:1.3 }));

        // 线性区（±30°，蓝色标注）
        const x30p=ox+(30+180)/360*aw, x30n=ox+(-30+180)/360*aw;
        this._staticGroup.add(new Konva.Rect({ x:x30n,y:oy-ah,width:x30p-x30n,height:ah*2,fill:'rgba(102,187,106,0.07)' }));
        this._staticGroup.add(new Konva.Text({ x:x30n+2,y:ty+16,text:'线性\n工作区',fontSize:5.5,fill:'#66bb6a',lineHeight:1.3 }));
        ...
        this._staticGroup.add(this._taLoadLine, this._taWorkPoint, this._taErrLabel);
    ...
    _drawDampingPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x:dx+4,y:dy+2,width:dw-8,text:'阻尼特性分析',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));

        // 阻尼比量表（半圆）
        const gcx=dx+dw*0.38, gcy=dy+dh*0.58;
        const gR =Math.round(Math.min(dw,dh)*0.23);
        // 颜色区域：欠阻尼/临界/过阻尼
        this._staticGroup.add(new Konva.Arc({ x:gcx,y:gcy,innerRadius:gR-6,outerRadius:gR+6,angle:90,rotation:-180,fill:'rgba(79,195,247,0.20)',stroke:'#4fc3f7',strokeWidth:0.7 }));
        this._staticGroup.add(new Konva.Arc({ x:gcx,y:gcy,innerRadius:gR-6,outerRadius:gR+6,angle:30,rotation:-90,fill:'rgba(102,187,106,0.25)',stroke:'#66bb6a',strokeWidth:0.7 }));
        this._staticGroup.add(new Konva.Arc({ x:gcx,y:gcy,innerRadius:gR-6,outerRadius:gR+6,angle:60,rotation:-60,fill:'rgba(255,167,38,0.20)',stroke:'#ffa726',strokeWidth:0.7 }));
        // 标注
        this._staticGroup.add(new Konva.Text({ x:gcx-gR-10,y:gcy-4,text:'欠阻尼\nζ<1',fontSize:6,fill:'#4fc3f7',lineHeight:1.3 }));
        this._staticGroup.add(new Konva.Text({ x:gcx+gR+3, y:gcy-4,text:'过阻尼\nζ>1',fontSize:6,fill:'#ffa726',lineHeight:1.3 }));
        this._staticGroup.add(new Konva.Text({ x:gcx-6,    y:gcy-gR-12,text:'ζ=1\n临界',fontSize:6,fill:'#66bb6a',lineHeight:1.3 }));
        ...
        this._staticGroup.add(this._dampNeedle, this._dampZetaLabel);
        ...
        params.forEach(({ lbl, val, col }, i) => {
            this._staticGroup.add(new Konva.Text({ x:tX,y:tY+i*18+2,text:lbl+':',fontSize:7.5,fill:'#546e7a' }));
    ...
    _drawPhasorDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({ x:px+4,y:py+2,width:pw-8,text:'相量图（Φ_S × Φ_R = 力矩）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));
        ...
        this._staticGroup.add(this._torqueArc,this._deltaArc,this._phiS,this._phiR,this._deltaLabel);
    }
    ...
    _drawDynamicResponse() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:dx,y:dy,width:dw,height:dh,fill:'#010d18',stroke:'#1a3040',strokeWidth:1.5,cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({ x:dx,y:dy,width:dw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:dx+4,y:dy+2,width:dw-8,text:'动态跟踪响应曲线（角度 + 角速度）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x:ox-12,y:oy-ah,text:'θ(°)',fontSize:7,fill:'#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw+2,y:oy+2,text:'t',fontSize:7,fill:'#80cbc4' }));

        // 零线
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah/2,ox+aw,oy-ah/2],stroke:'#37474f',strokeWidth:0.5,dash:[2,4] }));
        ...
        this._staticGroup.add(this._dynTXLine,this._dynTRLine,this._dynOmLine);
    ...
    _drawTorqueDecomp() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:tx,y:ty,width:tw,height:th,fill:'#010d18',stroke:'#1a3040',strokeWidth:1.5,cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({ x:tx,y:ty,width:tw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:tx+4,y:ty+2,width:tw-8,text:'实时转矩分解（T_sync + T_damp + T_load）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));

        const ox=tx+12, oy=ty+th*0.55, aw=tw-18, ah=th*0.38;
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah,ox,oy+ah*0.5,ox,oy,ox+aw,oy],stroke:'#37474f',strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:ox-13,y:oy-ah,text:'T(N·m)',fontSize:6.5,fill:'#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw+2,y:oy+2,text:'t',fontSize:6.5,fill:'#80cbc4' }));
        ...
        this._staticGroup.add(this._tdSyncLine,this._tdDampLine,this._tdLoadLine,this._tdNetLine);
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:px,y:py,width:pw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:px+4,y:py+2,width:pw-8,text:'系统参数控制',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x:wx+4,y:wy+1,width:ww-8,text:'励磁 u_R   同步转矩 T_sync   阻尼转矩 T_damp   角差 Δθ（失步阈值±90°）',fontSize:8,fill:'#80cbc4',align:'center' }));
        ...
        ['励磁 u_R','T_sync','T_damp','Δθ（°）'].forEach((l,i) => {
            this._staticGroup.add(new Konva.Text({ x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#ef9a9a','#66bb6a','#4fc3f7','#ffd54f'][i] }));
        });
        this._staticGroup.add(this._wLUR,this._wLSync,this._wLDamp,this._wLDelta);
        this._wavHCh=hCh;
    }
