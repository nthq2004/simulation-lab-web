...
export class VT20ViscositySensor extends BaseComponent {
    ...
    _drawOilChamber() {
        ...
        [[px+8,py+8],[px+pw-8,py+8],[px+8,py+ph-8],[px+pw-8,py+ph-8]].forEach(([bx,by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 4.5, fill: '#263238', stroke: '#1a252f', strokeWidth: 0.5 }));
        ...
        this._staticGroup.add(outer, this._oilCav);
    }
    ...
    _drawFuelPorts() {
        ...
        // 箭头
        this._staticGroup.add(new Konva.Line({ points: [px-30, cy-16, px-4, cy-16], stroke: '#ffa726', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [px-8, cy-20, px-2, cy-16, px-8, cy-12], stroke: '#ffa726', strokeWidth: 1.5, lineJoin: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [px-4, cy+16, px-30, cy+16], stroke: '#795548', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [px-24, cy+12, px-30, cy+16, px-24, cy+20], stroke: '#795548', strokeWidth: 1.5, lineJoin: 'round' }));

        this._staticGroup.add(new Konva.Text({ x: px-46, y: cy-28, text: '燃油\n进口', fontSize: 8, fill: '#ffa726', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Text({ x: px-46, y: cy+10, text: '燃油\n出口', fontSize: 8, fill: '#a1887f', lineHeight: 1.3 }));

        this._staticGroup.add(portIn, portOut);
    }
    ...
    _drawPiezoElements() {
        ...
        this._staticGroup.add(this._pzDriveGroup, this._pzSenseGroup);

        // 扭矩传递杆
        this._torsionGroup = new Konva.Group();
        this._staticGroup.add(this._torsionGroup);
    }
    ...
    _drawPendulumBase() {
        ...
        this._staticGroup.add(innerBase, ring, shaft);
        ...
        this._staticGroup.add(new Konva.Text({ x: cx - 24, y: cy - R - 20, text: '旋转钟摆', fontSize: 8.5, fontStyle: 'bold', fill: '#80cbc4' }));
    }
    ...
    _drawPendulumDynamic() {
        ...
        this._staticGroup.add(this._pendulumGroup);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [hx, hy+7+i*10, hx+hw, hy+7+i*10], stroke: 'rgba(255,255,255,0.14)', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+18, width: hw-16, text: 'VISCOSITY SENSOR', fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+27, width: hw-16, text: `f=${this.driveFreq}Hz  Torsional`, fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        [['DRV+','#42a5f5',14],['DRV−','#1565c0',34],['SNS+','#ffa726',58],['SNS−','#e65100',78]].forEach(([lbl,col,ty]) => {
            ...
            this._staticGroup.add(new Konva.Text({ x: hx+7, y: hy+ty-4, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._phaseArc, this._lcdMain, this._lcdUnit, this._lcdPhase, this._lcdFreq, this._lcdTemp);
    }
    ...
    _drawKnobs() {
        ...
        this._interactGroup.add(base, this._knobRotor, new Konva.Text({ x: kx-18, y: ky+20, width: 36, text: '黏度旋钮', fontSize: 8.5, fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawWaveformArea() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '电压波形  ── V_in (驱动)  ── V_out (感测)  ← 相位差 φ', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));

        // 网格
        for (let i = 1; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/4, wx+ww, wy+wh*i/4], stroke: 'rgba(255,167,38,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 5; i++) this._staticGroup.add(new Konva.Line({ points: [wx+ww*i/5, wy, wx+ww*i/5, wy+wh], stroke: 'rgba(255,167,38,0.05)', strokeWidth: 0.5 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'V_in  驱动', fontSize: 8, fill: '#42a5f5' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+4, text: 'V_out 感测', fontSize: 8, fill: '#ffa726' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineIn, this._wLineOut, this._phaseMarker, this._wInLbl, this._wOutLbl, this._wPhaseLbl);
    }
    ...
    _drawLissajous() {
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+2, y: ly+2, width: lw-4, text: 'V_in vs V_out', fontSize: 8, fontStyle: 'bold', fill: '#ffd54f', align: 'center' }));

        const lmx = lx + lw/2, lmy = ly + lh/2;
        this._staticGroup.add(new Konva.Line({ points: [lx+2, lmy, lx+lw-2, lmy], stroke: 'rgba(200,200,200,0.1)', strokeWidth: 0.5 }));
        this._staticGroup.add(new Konva.Line({ points: [lmx, ly+14, lmx, ly+lh-2], stroke: 'rgba(200,200,200,0.1)', strokeWidth: 0.5 }));
        this._staticGroup.add(new Konva.Text({ x: lx+2, y: lmy+2, text: 'X:V_in', fontSize: 7, fill: '#546e7a' }));
