...
export class EddyCurrentTachometer extends BaseComponent {
    constructor(config, sys) {
        ...
        this.addPort(this._shaftPort.x, this._shaftPort.y, 'shaft', 'signal', 'n');
    }
    ...
    _drawStaticParts() {
        ...
        if (this._showInternal) {
            ...
            this._drawMechLabels();
            this._drawCouplingArrows();
        }
        this._drawShaftPortLabel();
    }
    ...
    _drawCase() {
        ...
        this._staticGroup.add(new Konva.Rect({
            ...
            fillLinearGradientColorStops: [
                0, '#3c3c44', 0.35, '#2e2e36', 0.7, '#262630', 1, '#1e1e28',
            ],
            stroke: '#545460', strokeWidth: 1.5,
            cornerRadius: c.rx,
        }));
        // 顶部高光条
        this._staticGroup.add(new Konva.Rect({
            x: c.x + 4, y: c.y + 3, width: c.w - 8, height: c.h * 0.07,
            fill: 'rgba(255,255,255,0.055)', cornerRadius: [c.rx, c.rx, 0, 0],
        }));
        // 内嵌凹槽边框
        this._staticGroup.add(new Konva.Rect({
            x: c.x + 7, y: c.y + 7, width: c.w - 14, height: c.h - 14,
            fill: 'transparent', stroke: 'rgba(255,255,255,0.03)',
    ...
    _drawNameplate() {
        ...
        this._staticGroup.add(new Konva.Rect({
            ...
            fillLinearGradientColorStops: [
                0, '#18202e', 0.5, '#202a3e', 1, '#18202e',
            ],
            stroke: '#283848', strokeWidth: 0.8,
        ...
        this._staticGroup.add(new Konva.Text({
            ...
            fill: '#80c8e8', verticalAlign: 'middle',
        }));
        this._staticGroup.add(new Konva.Text({
            x: np.x + np.w * 0.40, y: np.y, width: np.w * 0.57, height: np.h,
            text: '磁感应式转速表', fontSize: fs * 0.72,
            fill: '#607888', align: 'right', verticalAlign: 'middle',
        }));
    }
    ...
    _drawUnitLabel() {
        ...
        this._staticGroup.add(new Konva.Text({
            x: cx - r * 0.46, y: cy + r * 0.24,
            width: r * 0.92, height: fs * 1.5,
            text: 'r/min', fontSize: fs, fontStyle: 'bold',
    ...
    _drawMechFrame() {
        ...
        const fw = W * 0.92, fh = this._mechH + 6;
        this._staticGroup.add(new Konva.Rect({
            x: fx, y: fy, width: fw, height: fh,
            fill: '#141620', stroke: '#28303e',
        ...
        this._staticGroup.add(new Konva.Line({
            ...
            stroke: 'rgba(100,120,160,0.18)', strokeWidth: 0.8,
    ...
    _drawMagnetStatic() {
        ...
        // 软轴（波浪线，从磁铁向下到底部端口）
        this._drawFlexCable(cx, cy + mr + 12, this._shaftPort.x, portY - 4);
    }

    /**
     * 绘制软轴（Flexible Cable，波浪折线）
     */
    _drawFlexCable(x0, y0, x1, y1) {
        const len   = Math.sqrt((x1 - x0) ** 2 + (y1 - y0) ** 2);
        const dx    = (x1 - x0) / len, dy = (y1 - y0) / len;
        const px    = -dy, py = dx;
        const waves = 8;
        const amp   = this.width * 0.018;
        const pts   = [x0, y0];
        for (let i = 1; i <= waves * 2; i++) {
            const t    = i / (waves * 2);
            const sign = i % 2 === 0 ? 1 : -1;
            const fade = Math.sin(t * Math.PI);   // 首尾收拢
            pts.push(
                x0 + dx * len * t + px * amp * sign * fade,
                y0 + dy * len * t + py * amp * sign * fade,
            );
        }
        pts.push(x1, y1);
        this._staticGroup.add(new Konva.Line({
            points: pts, stroke: '#607080', strokeWidth: 2.2,
        ...
        for (let i = 1; i < segs; i++) {
            const t  = i / segs;
            const lx = x0 + dx * len * t;
            const ly = y0 + dy * len * t;
            const hw = this.width * 0.008;
            this._staticGroup.add(new Konva.Line({
                points: [lx - px * hw, ly - py * hw, lx + px * hw, ly + py * hw],
                stroke: '#485868', strokeWidth: 1.0, lineCap: 'round',
            }));
        }
    }
    ...
    _drawShaftPortLabel() {
        const W  = this.width;
        const fs = Math.max(6, W * 0.038);
        this._staticGroup.add(new Konva.Text({
            x: this._shaftPort.x - 16, y: this._shaftPort.y - 16,
            width: 32, height: fs * 1.3,
            text: 'shaft', fontSize: fs * 0.82,
            fill: '#607888', align: 'center',
        }));
    }
