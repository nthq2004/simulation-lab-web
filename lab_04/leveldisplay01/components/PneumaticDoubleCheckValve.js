...
export class PneumaticDoubleCheckValve extends BaseComponent {
    ...
    _drawFlanges() {
        ...
        defs.forEach(({ f, isLeft, color, tag }) => {
            ...
            [f.y + f.h*0.15, f.y + f.h*0.85].forEach(by => {
                this.group.add(new Konva.Circle({ x: boltX, y: by, radius: f.h*0.10, fill:'#444', stroke:'#333', strokeWidth:0.5 }));
            });
            // 色标环
    ...
    _drawIsoSymbol() {
        ...
        // IN / OUT 标注
        this.group.add(new Konva.Text({ x: sx - 4,          y: sy + isoH + 3, text: 'IN',  fontSize: 7, fontStyle: 'bold', fill: '#ef9a9a' }));
        this.group.add(new Konva.Text({ x: sx + isoW - 14,  y: sy + isoH + 3, text: 'OUT', fontSize: 7, fontStyle: 'bold', fill: '#90caf9' }));
    }
