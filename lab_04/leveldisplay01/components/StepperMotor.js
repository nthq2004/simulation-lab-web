...
export class StepperMotor extends BaseComponent {
    ...
    _drawMotorSection() {
        ...
        this.group.add(new Konva.Ring({ x: ecx, y: ecy, innerRadius: rotorR*0.55, outerRadius: rotorR*0.78, fill: '#3e2723', stroke: '#5d4037', strokeWidth: 0.8 }));
        this.group.add(new Konva.Text({ x: ecx - 12, y: ecy - 6, text: '转子\n永磁', fontSize: 7, fill: '#a1887f', lineHeight: 1.3 }));
        ...
        this.group.add(this._rotorRefDot);

        // 三相接线端子（右侧）
        const termX = ex + ew + 10;
        [['A','#e53935',-20],['B','#43a047',20]].forEach(([l,c,dy]) => {
            this.group.add(new Konva.Line({ points: [ecx+sOuter*0.65, ecy+dy, termX, ecy+dy], stroke: c, strokeWidth: 2 }));
        ...
        [['PUL','#ffd54f',-28],['DIR','#80cbc4',-8],['ENA','#66bb6a',12]].forEach(([l,c,dy]) => {
            this.group.add(new Konva.Line({ points: [ctrlX, ecy+dy, ex, ecy+dy], stroke: c, strokeWidth: 1.5, dash: [3,3] }));
            this.group.add(new Konva.Circle({ x: ctrlX, y: ecy+dy, radius: 3, fill: c }));
            this.group.add(new Konva.Text({ x: ex-28, y: ecy+dy-5, text: l, fontSize: 7, fill: c }));
        });
    ...
    _drawRotorTeethLayer() {
        ...
        this.group.add(this._teethGroup);
    }
    ...
    _drawExcitationTable() {
        ...
        this.group.add(new Konva.Text({ x:sx+4, y:sy+2, width:sw-8, text:'励磁序列（当前模式）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        // 表头
        const hY = sy + 16;
        [['步#',0.08],['iA',0.30],['iB',0.56],['磁场↗',0.75]].forEach(([h,xr]) => {
            this.group.add(new Konva.Text({ x:sx+sw*xr, y:hY, text:h, fontSize:7.5, fill:'#546e7a', fontStyle:'bold' }));
        });
        this.group.add(new Konva.Line({ points:[sx+4, hY+10, sx+sw-4, hY+10], stroke:'#1a3040', strokeWidth:0.8 }));
        ...
        for (let i = 0; i < 8; i++) {
            ...
            this.group.add(rowBg, tStep, tIA, tIB, dot);
        ...
        this.group.add(this._seqPointer);
    }
    ...
    _drawTFCurve() {
        ...
        this.group.add(new Konva.Text({ x:tx+4, y:ty+2, width:tw-8, text:'T-f 转矩-频率特性', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        this.group.add(new Konva.Text({ x:ox+aw+2, y:oy+2, text:'f', fontSize:7, fill:'#80cbc4' }));
        ...
        [500,1000,2000,4000,fMax].forEach(f => {
            ...
            this.group.add(new Konva.Line({ points:[x,oy,x,oy+3], stroke:'#37474f', strokeWidth:0.7 }));
            this.group.add(new Konva.Text({ x:x-8, y:oy+4, text:`${f>=1000?(f/1000)+'k':f}`, fontSize:6, fill:'#37474f', width:16, align:'center' }));
        });

        // 保持转矩水平线
        const holdY = oy - (1.0)*(ah-4);
        this.group.add(new Konva.Line({ points:[ox,holdY,ox+aw,holdY], stroke:'#546e7a', strokeWidth:0.8, dash:[4,3] }));
        this.group.add(new Konva.Text({ x:ox+2, y:holdY-9, text:'T_hold', fontSize:6.5, fill:'#546e7a' }));
        ...
        this.group.add(new Konva.Line({ points:pullOutPts, stroke:'#ef5350', strokeWidth:2, lineJoin:'round', opacity:0.85 }));
        ...
        this.group.add(new Konva.Line({ points:pullInPts, stroke:'#ffa726', strokeWidth:1.5, lineJoin:'round', opacity:0.7 }));

        // 共振频率标线
        const fResX = ox+(this.resonFreq/fMax)*aw;
        if (fResX > ox && fResX < ox+aw) {
            this.group.add(new Konva.Line({ points:[fResX,ty+14,fResX,oy], stroke:'#ffd54f', strokeWidth:0.8, dash:[3,3] }));
            this.group.add(new Konva.Text({ x:fResX-3, y:ty+14, text:'f_res', fontSize:6.5, fill:'#ffd54f' }));
        }

        // 区域填色（稳定运行区）
        if (pullInPts.length>4)
            this.group.add(new Konva.Line({ points:[...pullInPts, ox+aw,oy, ox,oy], closed:true, fill:'rgba(102,187,106,0.06)', stroke:'none' }));

        // 图例
        [[tx+6,'#ef5350','失步转矩（Pull-out）'],[tx+6,'#ffa726','起动转矩（Pull-in）']].forEach(([x,col,lbl],i)=>{
            this.group.add(new Konva.Line({ points:[x,ty+14+i*9+3,x+12,ty+14+i*9+3], stroke:col, strokeWidth:1.8 }));
            this.group.add(new Konva.Text({ x:x+14, y:ty+14+i*9-1, text:lbl, fontSize:6.5, fill:col }));
        });

        // 动态工作点
        this._tfPoint = new Konva.Circle({ x:ox, y:oy, radius:5, fill:'#66bb6a', stroke:'#2e7d32', strokeWidth:1.5 });
        this.group.add(this._tfPoint);
    ...
    _drawPhasorViz() {
        ...
        this.group.add(new Konva.Text({ x:px+4, y:py+2, width:pw-8, text:'定子磁场相量（A/B 相合成）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ocx=px+pw*0.50, ocy=py+ph*0.58;
        const R  =Math.min(pw,ph)*0.30;

        // 坐标轴（A/B 方向）
        this.group.add(new Konva.Line({ points:[px+6,ocy,px+pw-6,ocy], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Line({ points:[ocx,py+14,ocx,py+ph-6], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Text({ x:px+pw-14,y:ocy+3, text:'A', fontSize:8, fill:'#e53935' }));
        this.group.add(new Konva.Text({ x:ocx+3,y:py+14, text:'B', fontSize:8, fill:'#43a047' }));

        // 电流极限圆
        this.group.add(new Konva.Circle({ x:ocx, y:ocy, radius:R, fill:'rgba(255,255,255,0.03)', stroke:'#37474f', strokeWidth:0.8, dash:[4,3] }));

        // 整步 4 个稳定位置点
        const stepPts = [[1,0],[0,1],[-1,0],[0,-1]];
        stepPts.forEach(([a,b]) => {
            this.group.add(new Konva.Circle({ x:ocx+a*R, y:ocy-b*R, radius:4, fill:'#263238', stroke:'#37474f', strokeWidth:1 }));
        });
        // 双相激励 4 点
        [[1,1],[-1,1],[-1,-1],[1,-1]].forEach(([a,b]) => {
            this.group.add(new Konva.Circle({ x:ocx+a*R/Math.sqrt(2), y:ocy-b*R/Math.sqrt(2), radius:3, fill:'#1a2a3a', stroke:'#37474f', strokeWidth:0.8 }));
        });
        ...
        this.group.add(this._pvIA, this._pvIB, this._pvIs, this._pvRotor);
    }
    ...
    _drawPositionTrace() {
        ...
        this.group.add(new Konva.Rect({ x:tx, y:ty, width:tw, height:th, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:tx, y:ty, width:tw, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:tx+4, y:ty+2, width:tw-8, text:'位置跟踪（指令步数 vs 实际位置）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ox=tx+14, oy=ty+th-12, aw=tw-20, ah=th-26;
        this.group.add(new Konva.Line({ points:[ox,oy-ah,ox,oy,ox+aw,oy], stroke:'#37474f', strokeWidth:0.8 }));
        this.group.add(new Konva.Text({ x:ox-12, y:oy-ah, text:'steps', fontSize:7, fill:'#80cbc4' }));
        ...
        this.group.add(this._ptCmdLine, this._ptActLine, this._ptErrLine);
    ...
    _drawOutOfStepPanel() {
        ...
        this.group.add(new Konva.Rect({ x:ox, y:oy, width:ow, height:oh, fill:'#010d18', stroke:'#1a3040', strokeWidth:1.5, cornerRadius:4 }));
        this.group.add(new Konva.Rect({ x:ox, y:oy, width:ow, height:13, fill:'#0a1a28', cornerRadius:[4,4,0,0] }));
        this.group.add(new Konva.Text({ x:ox+4, y:oy+2, width:ow-8, text:'失步检测与振动分析', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        // 电角度偏差仪表（圆形）
        const cx2=ox+ow*0.35, cy2=oy+oh*0.52;
        const R2 =Math.round(Math.min(ow,oh)*0.22);
        // 圆弧背景（0~180°区）
        this.group.add(new Konva.Arc({ x:cx2, y:cy2, innerRadius:R2-6, outerRadius:R2+6, angle:180, rotation:-180, fill:'rgba(102,187,106,0.15)', stroke:'#66bb6a', strokeWidth:0.8 }));
        // 90°~180° 危险区
        this.group.add(new Konva.Arc({ x:cx2, y:cy2, innerRadius:R2-6, outerRadius:R2+6, angle:90, rotation:-180, fill:'rgba(255,167,38,0.20)' }));
        // >90° 失步区
        this.group.add(new Konva.Arc({ x:cx2, y:cy2, innerRadius:R2-6, outerRadius:R2+6, angle:90, rotation:-90, fill:'rgba(239,83,80,0.25)' }));
        this.group.add(new Konva.Text({ x:cx2-R2, y:cy2+R2+3, width:R2*2, text:'电角偏差', fontSize:7, fill:'#546e7a', align:'center' }));
        this.group.add(new Konva.Text({ x:cx2+R2*0.5, y:cy2-R2*0.25, text:'90°', fontSize:6.5, fill:'#ffa726' }));
        this.group.add(new Konva.Text({ x:cx2-R2-14, y:cy2-8, text:'180°', fontSize:6.5, fill:'#ef5350' }));
        ...
        this.group.add(this._osNeedle, this._osAngleLabel);
        ...
        this.group.add(this._osStatusBg, this._osStatusText, this._osLostLabel);

        // 振动指示条
        const vibY=oy+oh*0.78;
        this.group.add(new Konva.Text({ x:ox+6, y:vibY-10, text:'振动幅度:', fontSize:7.5, fill:'#546e7a' }));
        this.group.add(new Konva.Rect({ x:ox+6, y:vibY, width:ow-12, height:8, fill:'#0a0a18', cornerRadius:2 }));
        this._osVibBar  = new Konva.Rect({ x:ox+6, y:vibY, width:0, height:8, fill:'#ffd54f', cornerRadius:2 });
        this._osVibBarW = ow-12;
        this.group.add(this._osVibBar);

        // 共振警告
        this._osResWarn = new Konva.Text({ x:ox+6, y:vibY+11, text:'', fontSize:7, fill:'#ffa726' });
        this.group.add(this._osResWarn);
    }
    ...
    _drawControlPanel() {
        ...
        this.group.add(new Konva.Rect({x:px,y:py,width:pw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0]}));
        this.group.add(new Konva.Text({x:px+4,y:py+2,width:pw-8,text:'驱动控制',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        this.group.add(new Konva.Text({x:px+4,y:mbY-10,text:'励磁方式：',fontSize:7.5,fill:'#546e7a'}));
        this._modeBtns={};
        ...
        this.group.add(new Konva.Text({x:px+4,y:mY-10,text:'细分数：',fontSize:7.5,fill:'#546e7a'}));
        this._microBtns={};
        micros.forEach((m,i)=>{
    ...
    _drawWaveform() {
        ...
        this.group.add(new Konva.Text({x:wx+4,y:wy+1,width:ww-8,text:'A相电流 iA   B相电流 iB   电磁转矩 T   位置误差（失步检测）',fontSize:8,fill:'#80cbc4',align:'center'}));
        ...
        ['iA','iB','T(N·m)','误差'].forEach((l,i)=>{
            this.group.add(new Konva.Text({x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#e53935','#43a047','#ffd54f','#ef5350'][i]}));
        });
        this.group.add(this._wLIA, this._wLIB, this._wLTq, this._wLErr);
        this._wavHCh=hCh;
    }
