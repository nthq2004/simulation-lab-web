...
export class ACServoInduction extends BaseComponent {
    ...
    _drawCrossSection() {
        ...
        // 转子铁芯
        this._staticGroup.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: rR*0.98, radiusY: rR*0.26, fill: '#37474f', stroke: '#263238', strokeWidth: 1, opacity: 0.4 }));
        this._staticGroup.add(new Konva.Text({ x: mCX-22, y: mCY-6, text: '鼠笼导条', fontSize: 6.5, fill: '#ef9a9a', width: 44, align: 'center' }));
        ...
        for (let i = 0; i < 32; i++) {
            ...
            this._staticGroup.add(new Konva.Arc({ x: encX+encR, y: encY, innerRadius: encR*0.5, outerRadius: encR*0.85, angle: 360/32-2, rotation: i*360/32-90, fill: col, opacity: 0.7 }));
        }
        this._staticGroup.add(new Konva.Text({ x: encX, y: encY+encR+3, width: encR*2, text: `增量编码器\n${this.encoderCPR}线`, fontSize: 7, fill: '#ffd54f', align: 'center', lineHeight: 1.3 }));
    ...
    _drawRotorLayer() {
        ...
        this._staticGroup.add(this._rotorRing);
    ...
    _drawControlBlock() {
        ...
        this._staticGroup.add(new Konva.Text({ x:cx+4, y:cy+2, width:cw-8, text:'IFOC 磁场定向控制（转差频率法 + 转子磁链观测）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        mainBlocks.forEach(({ lbl, x, w, col, bg }) => {
            this._staticGroup.add(new Konva.Rect({ x, y:y0-12, width:w, height:24, fill:bg, stroke:col, strokeWidth:1, cornerRadius:3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x:cx+cw*0.55, y:fbY2-9, text:'ω 速度反馈', fontSize:6, fill:'#4fc3f7', opacity:0.7 }));

        // 电流反馈线
        this._staticGroup.add(new Konva.Line({ points:[cx+306,y0-12, cx+306,y0-ch*0.18, cx+266,y0-ch*0.18, cx+266,y0-12], stroke:'#ef9a9a', strokeWidth:1, dash:[3,3], opacity:0.6 }));
        this._staticGroup.add(new Konva.Text({ x:cx+280, y:y0-ch*0.18-9, text:'id/iq 电流反馈', fontSize:6, fill:'#ef9a9a', opacity:0.65 }));

        // 磁链反馈线
        this._staticGroup.add(new Konva.Line({ points:[cx+352,y0, cx+352,y0+12, cx+182,y0+12, cx+182,y0], stroke:'#80cbc4', strokeWidth:0.8, dash:[3,3], opacity:0.55 }));
        this._staticGroup.add(new Konva.Text({ x:cx+260, y:y0+14, text:'ψr 磁链观测', fontSize:6, fill:'#80cbc4', opacity:0.7 }));
        ...
        this._staticGroup.add(this._ctrlErrLabel, this._ctrlSlipLabel, this._ctrlFluxLabel);
    }
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({x:lx+4,y:ly+2,width:lw-8,text:'伺服运行仪表（感应电机 IFOC）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+1,width:ww-8,text:'速度 ω   转矩 T   q轴电流 iq   d轴电流 id   转子磁链 ψr   制动器气隙 δ',fontSize:7.5,fill:'#80cbc4',align:'center'}));
        ...
        this._staticGroup.add(this._wLSpd,this._wLTq,this._wLIq,this._wLId,this._wLFlux,this._wLBrkG);
        this._wavHCh=hCh;
    }
