...
export class ACServoWithBrake extends BaseComponent {
    ...
    _drawCrossSection() {
        ...
        // 前后端盖
        this._staticGroup.add(new Konva.Rect({ x: mLeft, y: mTop, width: 10, height: mH, fill: '#263238', stroke: '#37474f', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Rect({ x: mRight-10, y: mTop, width: 10, height: mH, fill: '#263238', stroke: '#37474f', strokeWidth: 1 }));

        // 定子铁芯（叠片，示意）
        const stW = Math.round(mW * 0.75), stH = Math.round(mH * 0.72);
        const stL = mCX - stW/2, stT = mCY - stH/2;
        this._staticGroup.add(new Konva.Rect({ x: stL, y: stT, width: stW, height: stH, fill: '#455a64', stroke: '#263238', strokeWidth: 1 }));
        for (let i = 2; i < stH; i += 3)
            this._staticGroup.add(new Konva.Line({ points: [stL+1, stT+i, stL+stW-1, stT+i], stroke: 'rgba(0,0,0,0.12)', strokeWidth: 0.6 }));
        ...
        for (let i = 0; i < finCount; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [mLeft-6, fy, mLeft, fy], stroke: '#37474f', strokeWidth: 3 }));
            this._staticGroup.add(new Konva.Line({ points: [mRight, fy, mRight+6, fy], stroke: '#37474f', strokeWidth: 3 }));
        }

        // 出线端子（上侧）
        const termY = mTop - 2;
        const termX = mLeft + mW * 0.3;
        this._staticGroup.add(new Konva.Rect({ x: termX, y: termY-8, width: 30, height: 10, fill: '#263238', stroke: '#37474f', strokeWidth: 1, cornerRadius: 2 }));
        [['U','#e53935',0],['V','#43a047',10],['W','#1e88e5',20]].forEach(([l,c,dx]) => {
            this._staticGroup.add(new Konva.Circle({ x: termX+5+dx, y: termY-3, radius: 3, fill: c }));
        });
        this._staticGroup.add(new Konva.Text({ x: termX, y: termY-18, text: 'UVW 出线', fontSize: 7, fill: '#546e7a' }));
        ...
        for (let i = 0; i < 16; i++) {
            ...
            this._staticGroup.add(new Konva.Arc({ x: encX+encR, y: encY, innerRadius: encR*0.4, outerRadius: encR*0.85, angle: 360/16-1, rotation: i*360/16-90, fill: col, opacity: 0.7 }));
        }
        ...
        this._staticGroup.add(new Konva.Line({ points: [encX+encR*2, encY, termR, ecy+56], stroke: '#ffd54f', strokeWidth: 1.5, dash: [3,3] }));
        this._staticGroup.add(new Konva.Circle({ x: termR, y: ecy+56, radius: 3.5, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: termR+4, y: ecy+50, text: 'Enc', fontSize: 8, fill: '#ffd54f' }));

        // UVW 端子连线
        [['U','#e53935',-30,0],['V','#43a047',0,1],['W','#1e88e5',30,2]].forEach(([l,c,dy]) => {
            this._staticGroup.add(new Konva.Line({ points: [mLeft+mW*0.42, mTop-2, termR, ecy+dy], stroke: c, strokeWidth: 1.5, dash: [3,3] }));
            this._staticGroup.add(new Konva.Circle({ x: termR, y: ecy+dy, radius: 3.5, fill: c }));
            this._staticGroup.add(new Konva.Text({ x: termR+4, y: ecy+dy-5, text: l, fontSize: 9, fill: c, fontStyle: 'bold' }));
        });

        // 直流母线（左侧）
        this._staticGroup.add(new Konva.Line({ points: [ex-14, ecy-22, mLeft+4, ecy-22], stroke: '#ef5350', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Line({ points: [ex-14, ecy+22, mLeft+4, ecy+22], stroke: '#90caf9', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Circle({ x: ex-14, y: ecy-22, radius: 3, fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Circle({ x: ex-14, y: ecy+22, radius: 3, fill: '#90caf9' }));
        this._staticGroup.add(new Konva.Text({ x: ex-27, y: ecy-28, text: '+', fontSize: 10, fill: '#ef5350', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Text({ x: ex-27, y: ecy+18, text: '−', fontSize: 10, fill: '#90caf9', fontStyle: 'bold' }));
    ...
    _drawBrakeSection() {
        ...
        for (let i = 0; i < coilTurns; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [bLeft+6,ty, bRight-6,ty, bRight-6,ty+(coilH-4)/coilTurns*0.85, bLeft+6,ty+(coilH-4)/coilTurns*0.85], stroke: ['#c87832','#e09040'][i%2], strokeWidth: 1.5, lineJoin: 'round' }));
        }
    ...
    _drawRotorLayer() {
        ...
        for (let i = 0; i < nPoles; i++) {
            ...
            this._staticGroup.add(mag);
        }
        this._staticGroup.add(this._rotorEllipse);
        // 参考点（旋转指示）
    ...
    _drawControlBlock() {
        ...
        this._staticGroup.add(new Konva.Text({ x:cx+4, y:cy+2, width:cw-8, text:'三环串级 FOC 控制框图（位置环→速度环→电流环→SVPWM）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        mainBlocks.forEach(({ lbl, x, w, col, bg, yOff=0 }) => {
            this._staticGroup.add(new Konva.Rect({ x, y:y0-13+yOff, width:w, height:26, fill:bg, stroke:col, strokeWidth:1, cornerRadius:3 }));
            this._staticGroup.add(new Konva.Text({ x, y:y0-11+yOff, width:w, text:lbl, fontSize:6.5, fill:col, align:'center', lineHeight:1.35 }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points:[cx+cw-18,y0+13, cx+cw-18,fbY1, cx+58,fbY1, cx+58,y0+13], stroke:'#ffd54f', strokeWidth:1, dash:[4,3], opacity:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:cx+cw/2-25, y:fbY1-9, text:'θ 位置反馈（编码器）', fontSize:7, fill:'#ffd54f', opacity:0.9 }));

        // 速度反馈线
        const fbY2 = y0 + ch*0.25;
        this._staticGroup.add(new Konva.Line({ points:[cx+cw-18,y0+13, cx+cw-18,fbY2, cx+148,fbY2, cx+148,y0+13], stroke:'#4fc3f7', strokeWidth:1, dash:[3,3], opacity:0.65 }));
        this._staticGroup.add(new Konva.Text({ x:cx+cw*0.65, y:fbY2-9, text:'ω 速度反馈', fontSize:6.5, fill:'#4fc3f7', opacity:0.7 }));

        // 电流反馈线（最内层，仅到电流 PI）
        this._staticGroup.add(new Konva.Line({ points:[cx+320,y0-13, cx+320,y0-ch*0.18, cx+236,y0-ch*0.18, cx+236,y0-13], stroke:'#ef9a9a', strokeWidth:1, dash:[3,3], opacity:0.6 }));
        this._staticGroup.add(new Konva.Text({ x:cx+270, y:y0-ch*0.18-9, text:'id/iq 电流反馈', fontSize:6.5, fill:'#ef9a9a', opacity:0.65 }));

        // θ_e 路径（到 Park 变换）
        this._staticGroup.add(new Konva.Line({ points:[cx+cw-18,y0, cx+cw-18,y0-14, cx+320,y0-14], stroke:'#ffd54f', strokeWidth:0.8, dash:[3,3], opacity:0.5 }));
        this._staticGroup.add(new Konva.Text({ x:cx+cw*0.72, y:y0-22, text:'θe（转子位置）', fontSize:6, fill:'#ffd54f', opacity:0.6 }));

        // 制动器联锁线（到 PMSM 模块）
        this._staticGroup.add(new Konva.Line({ points:[cx+406+20,y0, cx+406+20,y0+ch*0.42, cx+cx+10,y0+ch*0.42], stroke:'#ffa726', strokeWidth:0.8, dash:[4,4], opacity:0.45 }));
        this._staticGroup.add(new Konva.Text({ x:cx+cw*0.5, y:y0+ch*0.42-9, text:'BRK 制动器联锁', fontSize:6.5, fill:'#ffa726', opacity:0.55 }));
        ...
        this._staticGroup.add(this._ctrlErrLabel, this._ctrlSpdLabel, this._ctrlModLabel);
    }
    ...
    _drawSCurveChart() {
        ...
        segXs.forEach(x=>this._staticGroup.add(new Konva.Line({points:[x,sy+14,x,oy],stroke:'#37474f',strokeWidth:0.6,dash:[3,3]})));
        ['Jerk↑','匀加速','Jerk↓','匀速','Jerk↓','匀减速','Jerk↑'].forEach((lbl,i)=>{
            const xl=ox+(([0,t1,t2,t3,t4,t5,t6][i]+[t1,t2,t3,t4,t5,t6,1][i])/2)*aw;
            this._staticGroup.add(new Konva.Text({x:xl-10,y:sy+14,width:20,text:lbl,fontSize:5.5,fill:'#546e7a',align:'center'}));
        });
        this._staticGroup.add(new Konva.Text({x:ox+aw*0.5,y:sy+sh-22,text:`Jmax=${this.profileMaxJerk}rpm/s²`,fontSize:7,fill:'#4fc3f7',align:'center',width:sw-14}));
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({x:lx+4,y:ly+2,width:lw-8,text:'伺服运行仪表',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+1,width:ww-8,text:'速度 ω   转矩 T   q轴电流 iq   d轴电流 id   制动器气隙 δ',fontSize:8,fill:'#80cbc4',align:'center'}));
        ...
        ['ω(rpm)','T(N·m)','iq(A)','δ制动'].forEach((l,i)=>{
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#4fc3f7','#ffd54f','#80cbc4','#ffa726'][i]}));
        });
        this._staticGroup.add(this._wLSpd,this._wLTq,this._wLIq,this._wLId,this._wLBrkG);
        this._wavHCh=hCh;
    }
