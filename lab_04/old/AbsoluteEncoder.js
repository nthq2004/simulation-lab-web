...
export class AbsoluteEncoder extends BaseComponent {
    ...
    _init() {
        ...
        this._staticGroup.add(this._diskLayer, this._highlightLayer);
        this._drawReadHead();
        this._drawShaft();
        this._drawInstrDial();
    ...
    _drawDiskBackground() {
        ...
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R + 14, fill: 'none', stroke: '#546e7a', strokeWidth: 0.5, dash: [4,4] }));

        // 安装孔
        for (let i = 0; i < 4; i++) {
            const a = (i / 4) * Math.PI * 2 + Math.PI / 4;
            const bx = cx + (R+9) * Math.cos(a), by = cy + (R+9) * Math.sin(a);
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 4.5, fill: '#263238', stroke: '#1a252f', strokeWidth: 0.8 }));
            this._staticGroup.add(new Konva.Circle({ x: bx-1, y: by-1, radius: 1.4, fill: 'rgba(255,255,255,0.28)' }));
        }
    ...
    _drawReadHead() {
        ...
        this._staticGroup.add(this._readAngleIndicator);
    }
    ...
    _drawShaft() {
        ...
        // 转轴延伸到外壳下方
        this._staticGroup.add(new Konva.Line({ points: [cx, cy + this._diskR + 14, cx, cy + this._diskR + 36], stroke: '#78909c', strokeWidth: 6, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [cx - 8, cy + this._diskR + 36, cx + 8, cy + this._diskR + 36], stroke: '#546e7a', strokeWidth: 2 }));
        this._staticGroup.add(new Konva.Text({ x: cx - 18, y: cy + this._diskR + 40, text: '输入轴', fontSize: 9, fill: '#607d8b' }));
        // 拖拽提示
        this._staticGroup.add(new Konva.Text({ x: cx - 26, y: cy + this._diskR + 56, text: '⟳ 拖动旋转', fontSize: 9, fill: '#546e7a' }));
    }
    ...
    _drawInstrDial() {
        ...
        this._staticGroup.add(new Konva.Text({ x: dx+2, y: dy+3, width: dw-4, text: `绝对编码器  ${this.bits}bit  ÷${this.steps}`, fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));
        ...
        this._staticGroup.add(bg, this._dialAngle, this._dialTurns, this._dialPos, this._dialRes);
    }
    ...
    _drawCodeRegisters() {
        ...
        this._staticGroup.add(new Konva.Text({ x: rx + 4, y: ry + 3, text: '码字寄存器', fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7' }));

        // 格雷码标题
        this._staticGroup.add(new Konva.Text({ x: rx + 4, y: ry + 16, text: 'GRAY:', fontSize: 8.5, fontFamily: 'Courier New, monospace', fill: '#80cbc4' }));
        // 二进制标题
        this._staticGroup.add(new Konva.Text({ x: rx + 4, y: ry + 40, text: 'BIN: ', fontSize: 8.5, fontFamily: 'Courier New, monospace', fill: '#ffd54f' }));
        // 十六进制
        this._staticGroup.add(new Konva.Text({ x: rx + 4, y: ry + 62, text: 'HEX: ', fontSize: 8.5, fontFamily: 'Courier New, monospace', fill: '#a5d6a7' }));
        ...
        this._staticGroup.add(bg, this._hexLabel, this._decLabel);
    }
    ...
    _drawSSIWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+4, y: sy+2, width: sw-8, text: `SSI 同步串行接口  CLK=${this.ssiFreqKHz}kHz  DATA=格雷码`, fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [sx, sy+sh*i/3, sx+sw, sy+sh*i/3], stroke: 'rgba(79,195,247,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 5; i++) this._staticGroup.add(new Konva.Line({ points: [sx+sw*i/5, sy, sx+sw*i/5, sy+sh], stroke: 'rgba(79,195,247,0.05)', strokeWidth: 0.5 }));

        this._ssiMidClk  = sy + sh * 0.24;
        this._ssiMidData = sy + sh * 0.72;

        [this._ssiMidClk, this._ssiMidData].forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [sx+2, my, sx+sw-2, my], stroke: 'rgba(200,200,200,0.10)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+4, y: sy+16, text: 'CLK', fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7' }));
        this._staticGroup.add(new Konva.Text({ x: sx+4, y: sy+sh/2+4, text: 'DATA', fontSize: 8, fontStyle: 'bold', fill: '#ff8f00' }));
        ...
        this._staticGroup.add(bg, titleBg, this._ssiLineClk, this._ssiLineData, this._ssiPositionLbl, this._ssiBitLbl);
    }
