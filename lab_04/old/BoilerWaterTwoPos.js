...
export class BoilerWaterLevelTwoPositionControl extends BaseComponent {
    ...
    _drawMotorPump() {
        ...
        this._staticGroup.add(new Konva.Text({ x: mx+4, y: my-10, width: mw-8, text: 'U  V  W', fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        // 联轴器螺栓
        this._staticGroup.add(new Konva.Circle({ x: cx-3, y: coupleY+4, radius: 2.5, fill: '#37474f' }));
        this._staticGroup.add(new Konva.Circle({ x: cx+3, y: coupleY+4, radius: 2.5, fill: '#37474f' }));
        ...
        // 泵出口（右侧）
        this._staticGroup.add(new Konva.Rect({ x: mx+mw, y: pumpY+pumpH*0.38, width: 14, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        // 泵进口（底部）
        this._staticGroup.add(new Konva.Rect({ x: cx-5, y: pumpY+pumpH-4, width: 10, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: cx-10, y: pumpY+pumpH+8, width: 20, text: '进水', fontSize: 7.5, fill: '#90a4ae', align: 'center' }));

        // 标注
        this._staticGroup.add(new Konva.Text({ x: mx, y: my-24, width: mw, text: '电机', fontSize: 8.5, fontStyle: 'bold', fill: '#42a5f5', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: mx, y: pumpY+pumpH+18, width: mw, text: '水泵', fontSize: 8.5, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(motor, plate, termBox, couple, pump, volute, this._impellerGroup, this._motorLed, this._motorLabel);
    }
    ...
    _drawSupplyPipe() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cvX-8, y: y-18, text: 'CV', fontSize: 7, fontStyle: 'bold', fill: '#ffd54f' }));

        // 流动粒子（动态层）
        this._flowGroup = new Konva.Group();
        this._flowPipeX1 = x1; this._flowPipeX2 = x2; this._flowPipeY = y;

        this._staticGroup.add(pipe, cv, this._flowGroup);
    }
    ...
    _drawBoiler() {
        ...
        // 铭牌
        this._staticGroup.add(new Konva.Rect({ x: bx+8, y: by+bh/2-14, width: bw-16, height: 28, fill: '#1e2a36', stroke: '#37474f', strokeWidth: 0.5, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: bx+8, y: by+bh/2-12, width: bw-16, text: '锅    炉', fontSize: 10, fontStyle: 'bold', fill: 'rgba(255,255,255,0.25)', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: bx+8, y: by+bh/2, width: bw-16, text: 'BOILER', fontSize: 8, fill: 'rgba(255,255,255,0.15)', align: 'center' }));

        // 安全阀（顶部）
        const svX = bx + bw - 22;
        const svY = by + 16;
        this._staticGroup.add(new Konva.Rect({ x: svX-4, y: svY-12, width: 8, height: 14, fill: '#ef5350', stroke: '#b71c1c', strokeWidth: 1, cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Rect({ x: svX-7, y: svY-14, width: 14, height: 4, fill: '#c62828', cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: svX-8, y: svY-24, text: 'SV', fontSize: 7, fontStyle: 'bold', fill: '#ef9a9a' }));

        // 蒸汽出口（顶部中央）
        this._staticGroup.add(new Konva.Rect({ x: bx+bw/2-8, y: by-8, width: 16, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: bx+bw/2-12, y: by-20, text: '蒸汽出口', fontSize: 7.5, fill: '#ef9a9a' }));

        // 支脚
        [-bw/3, bw/3].forEach(ox => {
            this._staticGroup.add(new Konva.Rect({ x: bx+bw/2+ox-6, y: by+bh-8, width: 12, height: 16, fill: '#37474f', stroke: '#263238', strokeWidth: 1, cornerRadius: [0,0,2,2] }));
            this._staticGroup.add(new Konva.Rect({ x: bx+bw/2+ox-14, y: by+bh+6, width: 28, height: 5, fill: '#263238', cornerRadius: 1 }));
        });
        ...
        for (let i = 0; i < 3; i++) {
            ...
            this._staticGroup.add(hbar);
        }
        this._staticGroup.add(new Konva.Text({ x: bx+4, y: by+bh-50, text: '加热元件', fontSize: 7.5, fill: '#ef9a9a' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: bx+bw+2, y: this._bHiLineY-6, text: 'H', fontSize: 8, fontStyle: 'bold', fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Text({ x: bx+bw+2, y: this._bLoLineY-1, text: 'L', fontSize: 8, fontStyle: 'bold', fill: '#ffa726' }));

        this._staticGroup.add(body, topHead, botHead, this._boilerInner, hiLine, loLine);
    }
    ...
    _drawBoilerWater() {
        ...
        this._staticGroup.add(this._boilerWater, this._boilerSurf, this._bubbleGroup, this._steamGroup2);
    }
    ...
    _drawConnectingPipes() {
        ...
        // 上连通管
        this._staticGroup.add(new Konva.Rect({ x: bx-2, y: hiY-5, width: fx-bx+4, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: bx+2, y: hiY-15, text: '上连通', fontSize: 7, fill: '#80cbc4' }));
        // 下连通管
        this._staticGroup.add(new Konva.Rect({ x: bx-2, y: loY-5, width: fx-bx+4, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: bx+2, y: loY+6, text: '下连通', fontSize: 7, fill: '#80cbc4' }));
    }
    ...
    _drawFloatChamber() {
        ...
        // 玻璃高光
        this._staticGroup.add(new Konva.Rect({ x: this._fcInX+1, y: this._fcInY, width: 4, height: this._fcInH, fill: 'rgba(255,255,255,0.15)' }));
        // 顶底端盖
        ...
        this._staticGroup.add(new Konva.Rect({ x: fx+fw/2-8, y: drainY, width: 16, height: 10, fill: '#455a64', stroke: '#263238', strokeWidth: 1, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: fx+fw/2-12, y: drainY+12, text: '排污阀', fontSize: 7, fill: '#78909c' }));

        // 标注
        this._staticGroup.add(new Konva.Text({ x: fx, y: fy-18, width: fw, text: '浮子室', fontSize: 9, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        // 水位刻度
        for (let i = 0; i <= 5; i++) {
            const ly = this._fcInY + this._fcInH*(1 - i/5);
            this._staticGroup.add(new Konva.Line({ points: [fx+fw, ly, fx+fw+6, ly], stroke: '#78909c', strokeWidth: 0.8 }));
            this._staticGroup.add(new Konva.Text({ x: fx+fw+8, y: ly-5, text: `${i*20}%`, fontSize: 7, fill: '#607d8b' }));
        }
        // 高低水位线
        ...
        this._staticGroup.add(new Konva.Line({ points: [this._fcInX, hiLineY, this._fcInX+this._fcInW, hiLineY], stroke: 'rgba(239,83,80,0.5)', strokeWidth: 1, dash: [4,3] }));
        this._staticGroup.add(new Konva.Line({ points: [this._fcInX, loLineY, this._fcInX+this._fcInW, loLineY], stroke: 'rgba(255,152,0,0.5)', strokeWidth: 1, dash: [4,3] }));

        this._staticGroup.add(body, inner, topCap, botCap);
        ...
        this._staticGroup.add(this._floatChamWater, this._floatChamSurf);
    }
    ...
    _drawFloatMechanism() {
        ...
        this._staticGroup.add(this._floatRod, this._linkRod, this._floatGroup, this._sectorGroup);

        // 铰接点标注
        this._staticGroup.add(new Konva.Text({ x: this._pivotX-14, y: this._pivotY+6, text: '铰接', fontSize: 7.5, fill: '#78909c' }));
        this._staticGroup.add(new Konva.Text({ x: this._pivotX-22, y: this._pivotY-30, text: '扇形调节板', fontSize: 7.5, fontStyle: 'bold', fill: '#90a4ae' }));
    }
    ...
    _drawMagneticMechanism() {
        ...
        // 同极斥力说明
        this._staticGroup.add(new Konva.Text({ x: px2+14, y: ry-28, text: '同极斥力', fontSize: 7.5, fontStyle: 'bold', fill: '#ffd54f' }));
        this._repulseArrow = new Konva.Line({ points: [px2+60, ry-4, px2+40, ry-4], stroke: '#ffd54f', strokeWidth: 1.5, lineCap: 'round' });
        this._staticGroup.add(new Konva.Line({ points: [rx-36, ry, px2+60, ry], stroke: '#546e7a', strokeWidth: 0.8, dash: [3,3], opacity: 0.5 }));
        ...
        this._staticGroup.add(this._switchArm, this._springGroup, this._repulseGroup, this._repulseArrow);
    }
    ...
    _drawSwitchContacts() {
        ...
        this._staticGroup.add(new Konva.Text({ x: mx2+2, y: by3+2, width: cw-4, text: '开关量输出', fontSize: 8, fontStyle: 'bold', fill: '#90caf9', align: 'center' }));
        ...
        terms.forEach(({ label, desc, color, dy }) => {
            const ty = by3 + dy;
            this._staticGroup.add(new Konva.Rect({ x: mx2+4, y: ty-6, width: cw-8, height: 13, fill: 'rgba(255,255,255,0.025)', cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: mx2+7, y: ty-3, text: `${label} (${desc})`, fontSize: 9, fontStyle: 'bold', fill: color }));
        ...
        this._staticGroup.add(box, boxTitle);
    }
    ...
    _drawBottomPanel() {
        ...
        this._staticGroup.add(bg, this._panelMain, this._panelSub, this._panelStatus);
    }
