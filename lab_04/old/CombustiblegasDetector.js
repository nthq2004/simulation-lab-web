...
export class CombustibleGasDetector extends BaseComponent {
    ...
    _drawSensorHousing() {
        ...
        // 防爆标记
        this._staticGroup.add(new Konva.Text({ x: sx + sw - 44, y: sy + sh - 14, text: 'Ex d II CT4', fontSize: 7.5, fill: 'rgba(249,168,37,0.6)' }));
        // 安装孔
        [[sx + 10, sy + 10], [sx + sw - 10, sy + 10], [sx + 10, sy + sh - 10], [sx + sw - 10, sy + sh - 10]].forEach(([bx, by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 4.5, fill: '#1a1a1a', stroke: '#333', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Circle({ x: bx - 1, y: by - 1, radius: 1.2, fill: 'rgba(255,255,255,0.2)' }));
        });
        this._staticGroup.add(body, this._sensInner, nameBar);
    }
    ...
    _drawDiffusionMesh() {
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: sx - 32, y: sy + sh / 2 - 40, text: '扩散\n铜网', fontSize: 7.5, fill: '#bf8c00', lineHeight: 1.4 }));
        // 进气箭头
        this._staticGroup.add(new Konva.Line({ points: [sx - 38, sy + sh / 2 - 10, sx - 20, sy + sh / 2 - 10], stroke: '#ffa726', strokeWidth: 1.8, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [sx - 24, sy + sh / 2 - 14, sx - 18, sy + sh / 2 - 10, sx - 24, sy + sh / 2 - 6], stroke: '#ffa726', strokeWidth: 1.8, lineJoin: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [sx - 38, sy + sh / 2 + 10, sx - 20, sy + sh / 2 + 10], stroke: '#546e7a', strokeWidth: 1.5, lineCap: 'round', dash: [3, 2] }));
        this._staticGroup.add(new Konva.Text({ x: sx - 52, y: sy + sh / 2 - 14, text: '气体\n进入', fontSize: 7, fill: '#ffa726', lineHeight: 1.3 }));
        this._staticGroup.add(meshFrame);
    }
    ...
    _drawBeadCavity() {
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx1 - 18, y: cy - R - 18, text: '探测珠 (Active)', fontSize: 8, fontStyle: 'bold', fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: cx2 - 20, y: cy - R - 18, text: '补偿珠 (Ref.)', fontSize: 8, fontStyle: 'bold', fill: '#90caf9' }));
        this._staticGroup.add(cavity, divider);
    }
    ...
    _drawBeadElements() {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const r2 = R * 0.62;
            this._staticGroup.add(new Konva.Arc({ x: cx1, y: cy, innerRadius: r2 - 2, outerRadius: r2 + 2, angle: 50, rotation: a * 180 / Math.PI, fill: 'rgba(200,180,100,0.5)' }));
        }
        // 催化剂涂层标识
        this._staticGroup.add(new Konva.Circle({ x: cx1, y: cy, radius: R * 0.45, fill: 'none', stroke: 'rgba(255,215,0,0.35)', strokeWidth: 1.5 }));
        // 高温标注
        ...
        for (let i = 0; i < 6; i++) {
            const a = (i / 6) * Math.PI * 2;
            const r2 = R * 0.62;
            this._staticGroup.add(new Konva.Arc({ x: cx2, y: cy, innerRadius: r2 - 2, outerRadius: r2 + 2, angle: 50, rotation: a * 180 / Math.PI, fill: 'rgba(100,150,200,0.45)' }));
        }
        this._staticGroup.add(new Konva.Text({ x: cx2 - 14, y: cy - R - 22, text: '500°C', fontSize: 8.5, fontFamily: 'Courier New, monospace', fill: '#90caf9' }));
        ...
        this._staticGroup.add(this._detGlow, this._detBead, this._detTempText);
        this._staticGroup.add(this._comGlow, this._comBead);
        this._staticGroup.add(this._rdLine, this._rcLine, this._rdLabel, this._rcLabel);
    }
    ...
    _drawGasParticleLayer() {
        ...
        this._staticGroup.add(this._gasGroup, this._reactionGroup);
    }
    ...
    _drawBridgeCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: bx + 4, y: by + 2, width: bw - 8, text: '惠斯顿电桥  Wheatstone Bridge', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        // Vs 供电（顶）和地（底）
        this._staticGroup.add(new Konva.Line({ points: [midX, topY, midX - 2, topY + 6, midX + 2, topY + 6, midX, topY], closed: true, fill: '#ef9a9a', stroke: 'none' }));
        this._staticGroup.add(new Konva.Text({ x: midX - 8, y: topY - 8, text: 'Vs', fontSize: 8.5, fontStyle: 'bold', fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Line({ points: [midX - 6, botY + 2, midX + 6, botY + 2, midX - 3, botY + 7, midX + 3, botY + 7], stroke: '#607d8b', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: midX - 4, y: botY + 9, text: '⏚', fontSize: 9, fill: '#607d8b' }));
        ...
        // 连线
        this._staticGroup.add(new Konva.Line({ points: [nodes.top[0], nodes.top[1], nodes.left[0], nodes.left[1]], stroke: '#546e7a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [nodes.top[0], nodes.top[1], nodes.right[0], nodes.right[1]], stroke: '#546e7a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [nodes.left[0], nodes.left[1], nodes.bot[0], nodes.bot[1]], stroke: '#546e7a', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Line({ points: [nodes.right[0], nodes.right[1], nodes.bot[0], nodes.bot[1]], stroke: '#546e7a', strokeWidth: 1.5 }));
        // 中桥（差分输出）
        this._staticGroup.add(new Konva.Line({ points: [nodes.left[0] + 5, nodes.left[1], nodes.right[0] - 5, nodes.right[1]], stroke: '#ffd54f', strokeWidth: 1.2, dash: [3, 2] }));
        ...
        // R1 R2（固定平衡电阻）
        this._staticGroup.add(new Konva.Rect({ x: nodes.left[0] - 14, y: midY + 10, width: 28, height: 12, fill: '#1a1a1a', stroke: '#546e7a', strokeWidth: 1, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: nodes.left[0] - 12, y: midY + 12, width: 24, text: 'R1', fontSize: 8, fontFamily: 'Courier New, monospace', fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Rect({ x: nodes.right[0] - 14, y: midY + 10, width: 28, height: 12, fill: '#1a1a1a', stroke: '#546e7a', strokeWidth: 1, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: nodes.right[0] - 12, y: midY + 12, width: 24, text: 'R2', fontSize: 8, fontFamily: 'Courier New, monospace', fill: '#78909c', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._vBridgeLine, this._rdBox, this._rdTxt, this._rcBox, this._rcTxt, this._vBridgeLabel);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [hx, hy + 6 + i * 10, hx + hw, hy + 6 + i * 10], stroke: 'rgba(255,255,255,0.12)', strokeWidth: 0.7 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: hx + 8, y: hy + 17, width: hw - 16, text: 'COMBUSTIBLE GAS', fontSize: 7, fill: '#78909c', align: 'center' }));
        const gasInfo = this._gasDB[this.gasType] || this._gasDB['CH4'];
        this._staticGroup.add(new Konva.Text({ x: hx + 8, y: hy + 27, width: hw - 16, text: `${gasInfo.name}  LEL=${gasInfo.lel}%vol`, fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        [['V+', '#ef5350', 14], ['GND', '#607d8b', 34], ['4-20', '#ffd54f', 58], ['ALM1', '#ffa726', 78], ['ALM2', '#ef5350', 98]].forEach(([lbl, col, ty]) => {
            this._staticGroup.add(new Konva.Rect({ x: hx + 4, y: hy + ty - 7, width: hw - 8, height: 13, fill: 'rgba(255,255,255,0.02)', cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: hx + 7, y: hy + ty - 3, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(new Konva.Circle({ x: lcx, y: lcy, radius: R + 4, fill: '#0d1020', stroke: '#1a1f30', strokeWidth: 1 }));
        const gasInfo = this._gasDB[this.gasType] || this._gasDB['CH4'];
        ...
        this._staticGroup.add(ring, this._lcdBg, this._concArc, this._lcdMain, this._lcdUnit, this._lcdGasLbl, this._lcdCurr, this._lcdTemp);
    }
    ...
    _drawWaveforms() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx + 4, y: wy + 2, width: ww - 8, text: '浓度 %LEL  电桥输出 V_bridge', fontSize: 8, fontStyle: 'bold', fill: '#ffd54f', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy + wh * i / 3, wx + ww, wy + wh * i / 3], stroke: 'rgba(255,213,79,0.06)', strokeWidth: 0.5 }));

        this._wavMidConc = wy + wh * 0.26;
        this._wavMidBridge = wy + wh * 0.74;
        [this._wavMidConc, this._wavMidBridge].forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [wx + 2, my, wx + ww - 2, my], stroke: 'rgba(200,200,200,0.1)', strokeWidth: 0.5, dash: [4, 3] }));
        });
        ...
        [this.alm1LEL, this.alm2LEL].forEach((lel, i) => {
            ...
            this._staticGroup.add(new Konva.Line({ points: [wx + 2, this._wavMidConc - ratio * amp, wx + ww - 2, this._wavMidConc - ratio * amp], stroke: col, strokeWidth: 0.8, dash: [3, 3] }));
            this._staticGroup.add(new Konva.Text({ x: wx + ww - 26, y: this._wavMidConc - ratio * amp - 9, text: `L${i + 1}`, fontSize: 7, fill: col.replace('0.35', '0.7') }));
        });
        ...
        this._staticGroup.add(new Konva.Text({ x: wx + 4, y: wy + 16, text: '浓度 %LEL', fontSize: 8, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: wx + 4, y: wy + wh / 2 + 4, text: 'V_bridge', fontSize: 8, fill: '#ef9a9a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineConc, this._wLineBridge, this._wConcLbl, this._wBridgeLbl);
    }
