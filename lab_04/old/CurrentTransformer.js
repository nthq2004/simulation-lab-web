...
export class CurrentTransformer extends BaseComponent {
    ...
    _drawCoreAndWindings() {
        ...
        this._staticGroup.add(this._busBar);
        ...
        // P1/P2 端子圆点
        this._staticGroup.add(new Konva.Circle({ x: ex + 14, y: ecy, radius: 5, fill: '#b8860b', stroke: '#8b6914', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Circle({ x: ex + ew - 14, y: ecy, radius: 5, fill: '#b8860b', stroke: '#8b6914', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: ex + 6, y: ecy - 15, text: 'P1', fontSize: 8, fill: '#ffd54f', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Text({ x: ex + ew - 22, y: ecy - 15, text: 'P2', fontSize: 8, fill: '#ffd54f', fontStyle: 'bold' }));
        ...
        this._staticGroup.add(new Konva.Circle({ x: secTermX, y: ecy - 16, radius: 3.5, fill: '#1565c0', stroke: '#0d47a1', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Circle({ x: secTermX, y: ecy + 16, radius: 3.5, fill: '#1976d2', stroke: '#0d47a1', strokeWidth: 0.8 }));

        // 接地符号（S2 端）
        [0, 1, 2].forEach(i => this._staticGroup.add(new Konva.Line({
            points: [secTermX - (i+1)*3, ecy+22+(i*4), secTermX + (i+1)*3, ecy+22+(i*4)],
            stroke: '#4caf50', strokeWidth: 1.5 - i * 0.4,
        })));

        this._staticGroup.add(new Konva.Text({ x: secTermX + 4, y: ecy - 22, text: 'S1', fontSize: 8, fill: '#90caf9', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Text({ x: secTermX + 4, y: ecy + 12, text: 'S2\n(⏚)', fontSize: 8, fill: '#90caf9', lineHeight: 1.3 }));
        this._staticGroup.add(new Konva.Text({
    ...
    _drawEquivCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ex+4, y: ey+2, width: ew-8, text: 'T 形等效电路（折算至二次侧，恒流源模型）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [x0, y0, x0, y1], stroke: '#ffd54f', strokeWidth: 1.2 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, y0, x1, y1], stroke: '#90caf9', strokeWidth: 1.2 }));

        // 恒流源符号（左侧，圆圈+箭头）
        const csR = vH * 0.22;
        const csCX= x0, csCY = (y0 + y1) / 2;
        this._staticGroup.add(new Konva.Circle({ x: csCX, y: csCY, radius: csR, fill: '#0d1a28', stroke: '#ffd54f', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Arrow({
            points: [csCX, csCY + csR * 0.5, csCX, csCY - csR * 0.5],
            stroke: '#ffd54f', fill: '#ffd54f', strokeWidth: 1.5, pointerLength: 4, pointerWidth: 3,
        }));
        this._staticGroup.add(new Konva.Text({ x: csCX - 18, y: ey + 14, text: "I₁'=I₁/K", fontSize: 7.5, fill: '#ffd54f' }));

        // 励磁支路节点
        this._staticGroup.add(new Konva.Circle({ x: xm, y: y0, radius: 3, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Line({ points: [xm, y0, xm, y0 + 6], stroke: '#546e7a', strokeWidth: 1 }));

        // Rm（竖向）
        const rmY0 = y0 + 6, rmH = vH * 0.40;
        this._drawResistor(xm - 14, rmY0, rmH, 7, '#66bb6a', true);
        this._staticGroup.add(new Konva.Text({ x: xm - 32, y: rmY0 + rmH * 0.3, text: `Rm\n${this.Rm}Ω`, fontSize: 7, fill: '#66bb6a', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: xm + 18, y: rmY0 + rmH * 0.3, text: `jXm\n${this.Xm}Ω`, fontSize: 7, fill: '#ef9a9a', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(this._i0ArrowEq);
        this._staticGroup.add(new Konva.Text({ x: xm + 5, y: y0 + 10, text: 'I₀', fontSize: 7.5, fill: '#66bb6a' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [x1, y0, x1, zbY0], stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [x1, zbY0 + zbH, x1, y1], stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: x1 + 5, y: zbY0 + zbH * 0.25, text: `Zb\n(负载\n${this.Zb_rated.toFixed(1)}Ω)`, fontSize: 7, fill: '#ffa726', lineHeight: 1.35 }));

        // ★ 开路/短路状态动态标注
        this._zbLabel = new Konva.Text({ x: x1 + 5, y: zbY0 + zbH * 0.6, text: '正常', fontSize: 7.5, fill: '#66bb6a', fontStyle: 'bold' });
        this._staticGroup.add(this._zbLabel);

        // 电流方向
        this._staticGroup.add(new Konva.Arrow({ points: [xm+4, y0-8, xm+22, y0-8], stroke: '#90caf9', fill: '#90caf9', strokeWidth: 1, pointerLength: 4, pointerWidth: 3 }));
        this._staticGroup.add(new Konva.Text({ x: xm+6, y: y0-18, text: 'I₂', fontSize: 7.5, fill: '#90caf9' }));

        // 二次端电压标注
        this._u2Label = new Konva.Text({ x: x1 - 10, y: ey + 14, text: 'U₂', fontSize: 9, fill: '#ffa726', fontStyle: 'bold' });
        this._staticGroup.add(this._u2Label);
        ...
        this._staticGroup.add(this._zbOpenMark);
    }
    ...
    _drawPhasorDiagram() {
        ...
        this._staticGroup.add(new Konva.Line({ points: [ocx, py+16, ocx, py+ph-4], stroke: '#1a3040', strokeWidth: 0.7 }));
        ...
        [['#ffd54f','I₁（一次电流）'],
         ...
         ['#ef9a9a','ΔI（误差电流）']].forEach(([col, lbl], i) => {
            this._staticGroup.add(new Konva.Line({ points: [lgX, py+18+i*10, lgX+12, py+18+i*10], stroke: col, strokeWidth: 2 }));
            this._staticGroup.add(new Konva.Text({ x: lgX+14, y: py+14+i*10, text: lbl, fontSize: 7, fill: col }));
        });

        this._phOCX = ocx; this._phOCY = ocy; this._phR = R;
        this._staticGroup.add(this._phaI1, this._phaI2, this._phaI0, this._phaDI);
    }
    ...
    _drawBHCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: bx+4, y: by+2, width: bw-8, text: 'B-H 励磁特性（CT 工作在线性区极低端）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Rect({ x: ox, y: oy-ah, width: aw, height: oy-ah-BsatY, fill: 'rgba(239,83,80,0.08)' }));
        this._staticGroup.add(new Konva.Line({ points: [ox, BsatY, ox+aw, BsatY], stroke: '#ef5350', strokeWidth: 0.9, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: BsatY-10, text: `饱和 Bsat=${this.Bsat}T`, fontSize: 6.5, fill: '#ef5350' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: [HratedX, oy, HratedX, BratedY], stroke: '#66bb6a', strokeWidth: 0.9, dash: [3,3] }));
        this._staticGroup.add(new Konva.Line({ points: [ox, BratedY, HratedX, BratedY], stroke: '#66bb6a', strokeWidth: 0.9, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: BratedY-10, text: `额定 Brated=${this.Brated}T`, fontSize: 6.5, fill: '#66bb6a' }));

        // 放大框（线性区局部放大示意）
        const zoomX = ox + aw * 0.50, zoomY = oy - ah * 0.20;
        const zoomW = aw * 0.46, zoomH = ah * 0.18;
        this._staticGroup.add(new Konva.Rect({ x: zoomX, y: zoomY, width: zoomW, height: zoomH, fill: '#0a1520', stroke: '#37474f', strokeWidth: 0.8, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: zoomX+2, y: zoomY+2, text: '← CT 工作区\n   （线性区极低端）', fontSize: 6.5, fill: '#66bb6a', lineHeight: 1.4 }));
    ...
    _drawErrorCurve() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: ex, y: ey, width: ew, height: eh, fill: '#010d18', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 4 }));
        this._staticGroup.add(new Konva.Rect({ x: ex, y: ey, width: ew, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: ex+4, y: ey+2, width: ew-8, text: `误差特性（${this.accuracy}级  ALF=${this.ALF}）`, fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox+aw+2, y: ey+eh-12, text: 'I/In', fontSize: 6.5, fill: '#37474f' }));
        this._staticGroup.add(new Konva.Text({ x: ox-14, y: oy-4, text: '0', fontSize: 6.5, fill: '#37474f' }));

        // 精度等级误差带
        const lim  = this._accuracyLimits[this.accuracy] || { fi: 0.5, delta: 20 };
        const fiY0 = oy - (lim.fi / 6.0) * ah;
        const fiY1 = oy + (lim.fi / 6.0) * ah;
        this._staticGroup.add(new Konva.Rect({ x: ox, y: fiY0, width: aw, height: fiY1-fiY0, fill: 'rgba(102,187,106,0.10)', stroke: '#66bb6a', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: fiY0-10, text: `+${lim.fi}%`, fontSize: 6.5, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: ox+2, y: fiY1+2,  text: `-${lim.fi}%`, fontSize: 6.5, fill: '#66bb6a' }));
        ...
        this._staticGroup.add(new Konva.Line({ points: errPts, stroke: '#ef9a9a', strokeWidth: 1.5, lineJoin: 'round', opacity: 0.75 }));

        // ALF 竖线
        const alfX = ox + (this.ALF / (this.ALF * 1.2)) * aw;
        this._staticGroup.add(new Konva.Line({ points: [alfX, ey+14, alfX, ey+eh-10], stroke: '#ffa726', strokeWidth: 0.8, dash: [4,3] }));
        this._staticGroup.add(new Konva.Text({ x: alfX-6, y: ey+eh-10, text: `ALF\n=${this.ALF}`, fontSize: 6, fill: '#ffa726', lineHeight: 1.3 }));

        // 额定工作点（I/In=1）
        const inX = ox + (1.0 / (this.ALF * 1.2)) * aw;
        this._staticGroup.add(new Konva.Line({ points: [inX, ey+14, inX, ey+eh-10], stroke: '#ffd54f', strokeWidth: 0.7, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: inX-6, y: ey+eh-10, text: 'In', fontSize: 6.5, fill: '#ffd54f' }));
    ...
    _drawControlPanel() {
        ...
        const abY = py + ph - 22;
        this._staticGroup.add(new Konva.Text({ x: px+4, y: abY-10, text: '精度等级：', fontSize: 8, fill: '#546e7a' }));
        this._accBtns = {};
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'I₁ 一次电流   I₂×K 折算二次电流   I₀ 励磁电流   ΔI 误差电流   U₂ 二次端电压   B 磁通密度', fontSize: 8, fill: '#80cbc4', align: 'center' }));
        ...
        ['I₁','I₂×K','I₀','ΔI','U₂','B'].forEach((l,i)=>{
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+12+h6*i+3,text:l,fontSize:8,fill:['#ffd54f','#90caf9','#66bb6a','#ef9a9a','#ffa726','#4fc3f7'][i]}));
        });
        this._staticGroup.add(this._wLI1, this._wLI2k, this._wLI0, this._wLDI, this._wLU2, this._wLB);
        this._wavH6 = h6;
    }
