...
export class DCContactor extends BaseComponent {
    ...
    _drawElectromagnet() {
        ...
        // 底轭
        this._staticGroup.add(new Konva.Rect({ x: coreX, y: yokeY, width: coreW, height: yoke, fill: coreCol, stroke: coreEdge, strokeWidth: 1.5 }));
        // 左右柱（整块，无叠片纹）
        [coreX, coreX + coreW - limb].forEach(lx => {
            this._staticGroup.add(new Konva.Rect({ x: lx, y: yokeY - limbH, width: limb, height: limbH, fill: '#546e7a', stroke: coreEdge, strokeWidth: 1.5 }));
        });
        // 极面标注
        this._staticGroup.add(new Konva.Text({ x: coreX, y: yokeY - limbH + 4, text: 'N', fontSize: 11, fontStyle: 'bold', fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: coreX + coreW - limb, y: yokeY - limbH + 4, text: 'S', fontSize: 11, fontStyle: 'bold', fill: '#90caf9' }));
        this._staticGroup.add(new Konva.Text({ x: coreX, y: yokeY + yoke*0.35, width: coreW, text: '静铁芯（U形，整块软钢）', fontSize: 8, fill: '#78909c', align: 'center' }));

        // 无叠片纹说明
        this._staticGroup.add(new Konva.Text({ x: ex + 4, y: yokeY - 12, width: ew - 8, text: '⚡ 直流：无需叠片铁芯', fontSize: 7.5, fill: '#ffd54f', align: 'left' }));

        // 气隙
        const gapY  = yokeY - limbH - Math.round(eh * 0.06);
        const gapH  = Math.round(eh * 0.06);
        this._staticGroup.add(new Konva.Rect({ x: coreX, y: gapY, width: coreW, height: gapH, fill: '#06101a' }));
        this._staticGroup.add(new Konva.Text({ x: coreX, y: gapY + 1, width: coreW, text: '─ 气隙 δ ─', fontSize: 7, fill: '#37474f', align: 'center' }));
    ...
    _drawCoilWinding() {
        ...
        // 接线端（+/-）
        const termX = ex + 10;
        this._staticGroup.add(new Konva.Line({ points: [coilCX - coilOutR, coilY1 + 4, termX, coilY1 + 4, termX, this._emCY - 18], stroke: '#e53935', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Line({ points: [coilCX - coilOutR, coilY2 - 4, termX, coilY2 - 4, termX, this._emCY + 18], stroke: '#1565c0', strokeWidth: 2, lineCap: 'round' }));
        this._staticGroup.add(new Konva.Circle({ x: termX, y: this._emCY - 18, radius: 3, fill: '#e53935', stroke: '#b71c1c', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Circle({ x: termX, y: this._emCY + 18, radius: 3, fill: '#1565c0', stroke: '#0d47a1', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: termX - 12, y: this._emCY - 24, text: '+  A1', fontSize: 8, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: termX - 12, y: this._emCY + 22, text: '−  A2', fontSize: 8, fill: '#90caf9' }));
        ...
        // 参数标注
        this._staticGroup.add(new Konva.Text({ x: coilCX + coilOutR + 3, y: coilY1 + coilH*0.2, text: `N=${this.coilTurns}\nR=${this.coilR}Ω\nτ=${(this.coilTau*1000).toFixed(0)}ms`, fontSize: 7.5, fill: '#ef9a9a', lineHeight: 1.5 }));
        // 时间常数说明
        this._staticGroup.add(new Konva.Text({ x: ex + 4, y: coilY2 + 4, width: ew - 8, text: `i(t)=I_ss(1−e^{−t/τ})  τ=${(this.coilTau*1000).toFixed(0)}ms`, fontSize: 7, fill: '#80cbc4', align: 'center' }));

        this._coilY1   = coilY1;
        this._coilY2   = coilY2;
        this._coilCX   = coilCX;
        this._coilOutR = coilOutR;
        this._staticGroup.add(this._coilBobbin, this._coilGlow, this._coilGroup);
    }
    ...
    _drawArmatureLayer() {
        ...
        this._staticGroup.add(this._armatureGroup);
    }
    ...
    _drawFdCurve() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: fx, y: fy, width: fw, height: fh, fill: '#010d18', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 4 }));
        this._staticGroup.add(new Konva.Rect({ x: fx, y: fy, width: fw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: fx+4, y: fy+2, width: fw-8, text: 'F-δ 气隙特性曲线', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: ox-16, y: oy-ah, text: 'F(N)', fontSize: 7, fill: '#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x: ox+aw+2, y: oy+2, text: 'δ(mm)', fontSize: 7, fill: '#80cbc4' }));

        // 刻度
        [1,2,3,4,5,6].forEach(d => {
            const nx = ox + (d/6.5) * aw;
            this._staticGroup.add(new Konva.Line({ points: [nx, oy, nx, oy+3], stroke: '#37474f', strokeWidth: 0.7 }));
            this._staticGroup.add(new Konva.Text({ x: nx-5, y: oy+4, text: d+'', fontSize: 6, fill: '#37474f', align: 'center' }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points: fPts,      stroke: '#66bb6a', strokeWidth: 1.8, lineJoin: 'round', opacity: 0.7 }));
        this._staticGroup.add(new Konva.Line({ points: fPtsHalf,  stroke: '#4fc3f7', strokeWidth: 1.2, lineJoin: 'round', opacity: 0.5, dash: [4,3] }));

        // 弹簧特性（线性，从 0 到 δmax）
        const springPts = [ox, oy - (this.springForce/this._calcForce(I_ss,0.1e-3))*(ah-2)*0.15, ox+aw, oy - 0];
        this._staticGroup.add(new Konva.Line({ points: springPts, stroke: '#ef5350', strokeWidth: 1.2, lineJoin: 'round', opacity: 0.6, dash: [3,3] }));

        // 图例
        const lgY = fy + 16;
        [[fw*0.55,'#66bb6a','F(I_ss)'], [fw*0.55,'#4fc3f7','F(0.65×I_ss)'], [fw*0.55,'#ef5350','弹簧反力']].forEach(([xr, col, lbl], i) => {
            this._staticGroup.add(new Konva.Line({ points: [fx+xr, lgY+i*10+3, fx+xr+14, lgY+i*10+3], stroke: col, strokeWidth: i===2?1.2:i===1?1.2:1.8, dash: i>0?[3,3]:[] }));
            this._staticGroup.add(new Konva.Text({ x: fx+xr+16, y: lgY+i*10-1, text: lbl, fontSize: 6.5, fill: col }));
        });

        // 动态工作点
        this._fdPoint  = new Konva.Circle({ x: ox, y: oy, radius: 4.5, fill: '#ffd54f', stroke: '#f9a825', strokeWidth: 1.5 });
        this._staticGroup.add(this._fdPoint);
    ...
    _drawMainContacts() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: cx, y: cy, width: cw, height: ch, fill: '#0d1520', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 4 }));
        this._staticGroup.add(new Konva.Rect({ x: cx, y: cy, width: cw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: cx+4, y: cy+2, width: cw-8, text: `主触点（${this.polePairs}P  ${this.ratedCurrent}A DC）磁吹弧`, fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        const step   = cw / (this.polePairs + 1);
        const busCol = '#78909c', busEdge = '#455a64';
        const fixH   = 10, bridgeH = 8;

        this._staticGroup.add(new Konva.Rect({ x: cx+4, y: cy+16, width: cw-8, height: 7, fill: busCol, stroke: busEdge, strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Rect({ x: cx+4, y: cy+ch-16, width: cw-8, height: 7, fill: busCol, stroke: busEdge, strokeWidth: 1 }));
        ...
        for (let i = 1; i <= this.polePairs; i++) {
            ...
            // 上固定触头（银钨合金）
            this._staticGroup.add(new Konva.Rect({ x: px-10, y: cy+23, width: 20, height: fixH, fill: '#cfd8dc', stroke: '#546e7a', strokeWidth: 0.8, cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: px-9, y: cy+25, text: 'AgW', fontSize: 6, fill: '#607d8b' }));
            // 下固定触头
            this._staticGroup.add(new Konva.Rect({ x: px-10, y: cy+ch-26, width: 20, height: fixH, fill: '#cfd8dc', stroke: '#546e7a', strokeWidth: 0.8, cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: px-9, y: cy+ch-24, text: 'AgW', fontSize: 6, fill: '#607d8b' }));

            this._staticGroup.add(new Konva.Line({ points: [px, cy+16, px, cy+23], stroke: '#78909c', strokeWidth: 4 }));
            this._staticGroup.add(new Konva.Line({ points: [px, cy+ch-16, px, cy+ch-26], stroke: '#78909c', strokeWidth: 4 }));
            ...
            this._staticGroup.add(bridgeGroup);

            this._staticGroup.add(new Konva.Text({ x: px-6, y: cy+14, text: `P${i}`, fontSize: 7, fill: '#ef9a9a' }));
            this._staticGroup.add(new Konva.Text({ x: px-6, y: cy+ch-12, text: `N${i}`, fontSize: 7, fill: '#90caf9' }));
        }

        // 磁吹弧线圈（绕在触点旁，串联于主回路）
        const blowX = cx + 6, blowW = cw - 12;
        this._staticGroup.add(new Konva.Rect({ x: blowX, y: cy+ch-52, width: blowW, height: 14, fill: '#0a1520', stroke: '#1e3a5f', strokeWidth: 0.8, dash: [4,3], cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: blowX+2, y: cy+ch-50, text: `磁吹线圈（${this.blowCoilTurns}匝，串联）→ 磁场 B ↑`, fontSize: 7, fill: '#4fc3f7' }));
        ...
        for (let g = 0; g < this.arcGridCount; g++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [gridSX, gy, gridEX, gy], stroke: '#1e3a5f', strokeWidth: 0.7, opacity: 0.5 }));
        }
        this._staticGroup.add(new Konva.Text({ x: cx + 4, y: cy + 34, text: `灭弧栅（${this.arcGridCount}片）`, fontSize: 7, fill: '#1e3a5f' }));
        this._staticGroup.add(new Konva.Rect({ x: gridSX, y: gridY, width: gridEX-gridSX, height: gridH, fill: 'rgba(0,0,0,0)', stroke: '#1e3a5f', strokeWidth: 0.6, dash: [3,3] }));

        // 行程条
        this._staticGroup.add(new Konva.Text({ x: cx+4, y: cy+ch-62, text: '行程:', fontSize: 7, fill: '#37474f' }));
        this._staticGroup.add(new Konva.Rect({ x: cx+30, y: cy+ch-63, width: cw-38, height: 7, fill: '#0a0a18', cornerRadius: 2 }));
        this._travelBar  = new Konva.Rect({ x: cx+30, y: cy+ch-63, width: 0, height: 7, fill: '#66bb6a', cornerRadius: 2 });
        this._travelBarW = cw - 38;
        this._staticGroup.add(this._travelBar);
    ...
    _drawAuxContacts() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: ax, y: ay, width: aw, height: ah, fill: '#0d1520', stroke: '#1a3040', strokeWidth: 1.5, cornerRadius: 4 }));
        this._staticGroup.add(new Konva.Rect({ x: ax, y: ay, width: aw, height: 13, fill: '#0a1a28', cornerRadius: [4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: ax+4, y: ay+2, width: aw-8, text: '辅助触点', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        const drawAux = (label, t1, t2, isNO, xOff, yOff) => {
            const tx = ax + xOff, ty = ay + yOff;
            const hw = aw * 0.40, hcx = tx + hw / 2 + 6;
            this._staticGroup.add(new Konva.Rect({ x: tx, y: ty, width: hw + 12, height: ah / 2 - 4, fill: '#0a1020', stroke: '#1a2540', strokeWidth: 0.8, cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: tx + 2, y: ty + 2, text: label, fontSize: 7, fill: '#546e7a' }));
            this._staticGroup.add(new Konva.Rect({ x: hcx - 12, y: ty + 13, width: 24, height: 5, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.5, cornerRadius: 1 }));
            this._staticGroup.add(new Konva.Rect({ x: hcx - 12, y: ty + ah/2 - 14, width: 24, height: 5, fill: '#607d8b', stroke: '#37474f', strokeWidth: 0.5, cornerRadius: 1 }));
            this._staticGroup.add(new Konva.Text({ x: tx + 2, y: ty + 11, text: t1, fontSize: 6.5, fill: '#4fc3f7' }));
            this._staticGroup.add(new Konva.Text({ x: tx + 2, y: ty + ah/2 - 15, text: t2, fontSize: 6.5, fill: '#4fc3f7' }));
            ...
            this._staticGroup.add(bridge, dot);
    ...
    _drawLCDPanel() {
        ...
        if (indH > 16) {
            ...
            this._staticGroup.add(new Konva.Rect({ x: lx+4, y: indY, width: lw-8, height: indH, fill: '#0d1520', cornerRadius: 3 }));
    ...
    _drawControlPanel() {
        ...
        // 线圈电压调节
        const slY = py + 44;
        this._staticGroup.add(new Konva.Text({ x: px+4, y: slY - 11, text: `线圈电压 Uc (额定 ${this.ratedVoltageCoil}V DC):`, fontSize: 8, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Rect({ x: px+4, y: slY, width: (pw-16)/2, height: 8, fill: '#0a0a18', cornerRadius: 2 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: slX2, y: slY2 - 11, text: `主回路电流 I_main:`, fontSize: 8, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Rect({ x: slX2, y: slY2, width: (pw-16)/2 - 12, height: 8, fill: '#0a0a18', cornerRadius: 2 }));
        ...
        this._interactGroup.add(this._voltBar, this._voltValTxt, hitV, this._iMainBar, this._iMainValTxt, hitI);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+1, width: ww-8, text: 'Ic 线圈电流(RL暂态)   F_em 吸力   δ 气隙   U_arc 弧压', fontSize: 8, fill: '#80cbc4', align: 'center' }));
        ...
        ['Ic', 'F', 'δ', 'Uarc'].forEach((l, i) => {
            this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+12+h4*i+3, text: l, fontSize: 8, fill: ['#ffd54f','#66bb6a','#4fc3f7','#ff8a65'][i] }));
        });
        this._staticGroup.add(this._wLineI, this._wLineF, this._wLineGap, this._wLineArc);
        this._wavH4 = h4;
    }
