...
export class DCServoWithBrake extends BaseComponent {
    ...
    _drawCrossSection() {
        ...
        // 外壳（圆筒形）
        this._staticGroup.add(new Konva.Rect({ x: mLeft, y: mTop, width: mW, height: mH, fill: '#1c2b38', stroke: '#37474f', strokeWidth: 2, cornerRadius: 3 }));
        
        // 前后端盖（带轴承示意）
        this._staticGroup.add(new Konva.Rect({ x: mLeft, y: mTop, width: 8, height: mH, fill: '#263238', stroke: '#37474f', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Rect({ x: mRight-8, y: mTop, width: 8, height: mH, fill: '#263238', stroke: '#37474f', strokeWidth: 1 }));

        // 定子磁极（永磁体或励磁绕组）
        const poleW = 12, poleH = mH * 0.65;
        const poleY = mCY - poleH/2;
        // N 极（红）
        this._staticGroup.add(new Konva.Rect({ x: mLeft+8, y: poleY, width: poleW, height: poleH, fill: '#e53935', stroke: '#b71c1c', strokeWidth: 1, opacity: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: mLeft+8, y: poleY+poleH/2-5, width: poleW, text: 'N', fontSize: 10, fill: '#fff', align: 'center', fontStyle: 'bold' }));
        // S 极（蓝）
        this._staticGroup.add(new Konva.Rect({ x: mRight-20, y: poleY, width: poleW, height: poleH, fill: '#1e88e5', stroke: '#0d47a1', strokeWidth: 1, opacity: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: mRight-20, y: poleY+poleH/2-5, width: poleW, text: 'S', fontSize: 10, fill: '#fff', align: 'center', fontStyle: 'bold' }));

        // 电枢铁芯（转子）
        const rR = Math.round(mH * 0.32);
        this._rotorR = rR;
        this._staticGroup.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: rR*0.95, radiusY: rR*0.28, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1 }));
        ...
        for (let i = 0; i < 12; i++) {
            ...
            this._staticGroup.add(new Konva.Circle({ x: wx, y: wy, radius: 4, fill: '#ffb74d', stroke: '#e65100', strokeWidth: 0.5 }));
        }
        
        // 换向器（后端部，铜片环）
        const commY = mBot - 12;
        const commW = 20;
        this._staticGroup.add(new Konva.Rect({ x: mCX-commW/2, y: commY-4, width: commW, height: 8, fill: '#d4a017', stroke: '#8d6e63', strokeWidth: 1, cornerRadius: 1 }));
        for (let i = 0; i < 10; i++) {
            const segW = commW / 10;
            this._staticGroup.add(new Konva.Line({ points: [mCX-commW/2 + i*segW, commY-4, mCX-commW/2 + i*segW, commY+4], stroke: '#5d4037', strokeWidth: 0.5 }));
        }
        this._staticGroup.add(new Konva.Text({ x: mCX-8, y: commY-12, text: '换向器', fontSize: 6, fill: '#d4a017', width: 16, align: 'center' }));

        // 电刷（碳刷，两侧）
        this._staticGroup.add(new Konva.Rect({ x: mCX-commW/2-8, y: commY-2, width: 8, height: 4, fill: '#3e2723', stroke: '#1b0f0a', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Rect({ x: mCX+commW/2, y: commY-2, width: 8, height: 4, fill: '#3e2723', stroke: '#1b0f0a', strokeWidth: 0.8 }));
        this._staticGroup.add(new Konva.Text({ x: mCX-commW/2-10, y: commY-8, text: '电刷', fontSize: 6, fill: '#5d4037' }));

        // 气隙
        this._staticGroup.add(new Konva.Ellipse({ x: mCX, y: mCY, radiusX: rR*0.96, radiusY: rR*0.28, fill: '#06101a', stroke: '#1a3040', strokeWidth: 0.5 }));

        // 散热孔
        for (let i = 0; i < 4; i++) {
            const hy = mTop + mH*0.2 + i*(mH*0.6/3);
            this._staticGroup.add(new Konva.Rect({ x: mLeft-3, y: hy, width: 3, height: 6, fill: '#37474f', cornerRadius: 1 }));
            this._staticGroup.add(new Konva.Rect({ x: mRight, y: hy, width: 3, height: 6, fill: '#37474f', cornerRadius: 1 }));
        }

        // 电枢接线端子（正负极）
        const termX = mLeft + 15;
        const termY = mTop - 6;
        this._staticGroup.add(new Konva.Rect({ x: termX, y: termY-6, width: 24, height: 12, fill: '#263238', stroke: '#37474f', strokeWidth: 1, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Circle({ x: termX+6, y: termY, radius: 4, fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Text({ x: termX+2, y: termY-4, text: '+', fontSize: 8, fill: '#fff', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Circle({ x: termX+18, y: termY, radius: 4, fill: '#90caf9' }));
        this._staticGroup.add(new Konva.Text({ x: termX+14, y: termY-4, text: '−', fontSize: 8, fill: '#fff', fontStyle: 'bold' }));

        // 编码器（后部）
        const encX = mRight + 5, encY = mCY;
        const encR = Math.round(mH*0.20);
        this._staticGroup.add(new Konva.Circle({ x: encX+encR, y: encY, radius: encR, fill: '#0d1a24', stroke: '#ffd54f', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: encX, y: encY+encR+3, width: encR*2, text: `编码器\n${this.encoderCPR}线`, fontSize: 7, fill: '#ffd54f', align: 'center', lineHeight: 1.2 }));

        // 信号线
        const termR = ex + ew + 10;
        this._staticGroup.add(new Konva.Line({ points: [encX+encR*2, encY, termR, ecy], stroke: '#ffd54f', strokeWidth: 1.5, dash: [3,3] }));
        this._staticGroup.add(new Konva.Circle({ x: termR, y: ecy, radius: 3.5, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: termR+4, y: ecy-5, text: 'Enc', fontSize: 8, fill: '#ffd54f' }));

        // 电枢电源线
        const armPosTerm = ex - 8;
        const armNegTerm = ex - 8;
        this._staticGroup.add(new Konva.Line({ points: [termX+6, termY-6, armPosTerm, ecy-22], stroke: '#ef5350', strokeWidth: 1.5, dash: [3,3] }));
        this._staticGroup.add(new Konva.Line({ points: [termX+18, termY-6, armNegTerm, ecy+22], stroke: '#90caf9', strokeWidth: 1.5, dash: [3,3] }));
        this._staticGroup.add(new Konva.Circle({ x: armPosTerm, y: ecy-22, radius: 3.5, fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Circle({ x: armNegTerm, y: ecy+22, radius: 3.5, fill: '#90caf9' }));
        this._staticGroup.add(new Konva.Text({ x: armPosTerm-8, y: ecy-28, text: '+', fontSize: 9, fill: '#ef5350', fontStyle: 'bold' }));
        this._staticGroup.add(new Konva.Text({ x: armNegTerm-8, y: ecy+18, text: '−', fontSize: 9, fill: '#90caf9', fontStyle: 'bold' }));

        // 输出轴
        this._staticGroup.add(new Konva.Rect({ x: mCX-4, y: mBot, width: 8, height: 12, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
    ...
    _drawBrakeSection() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: bLeft, y: bTop, width: bW, height: bH, fill: '#1c2020', stroke: '#37474f', strokeWidth: 1.5, cornerRadius: 3 }));
        this._staticGroup.add(new Konva.Text({ x: bLeft, y: bTop-12, width: bW, text: '制动器', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));

        // 线圈
        const coilY = bTop + bH*0.18;
        const coilH = bH*0.28;
        this._staticGroup.add(new Konva.Rect({ x: bLeft+4, y: coilY, width: bW-8, height: coilH, fill: '#1a1000', stroke: '#ffa726', strokeWidth: 1, cornerRadius: 2 }));
        for (let i = 0; i < 8; i++) {
            const ty = coilY + 2 + i*(coilH-4)/8;
            this._staticGroup.add(new Konva.Line({ points: [bLeft+6,ty, bRight-6,ty, bRight-6,ty+(coilH-4)/8*0.85, bLeft+6,ty+(coilH-4)/8*0.85], stroke: ['#c87832','#e09040'][i%2], strokeWidth: 1.5, lineJoin: 'round' }));
        }
        this._staticGroup.add(new Konva.Text({ x: bLeft+2, y: coilY+coilH*0.35, width: bW-4, text: '励磁\n线圈', fontSize: 7.5, fill: '#ffa726', align: 'center', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Line({ points: spPts, stroke: '#80cbc4', strokeWidth: 1.5, lineJoin: 'round' }));
        this._staticGroup.add(new Konva.Text({ x: bLeft+4, y: spTop + spH*0.3, text: '弹簧\n(预压)', fontSize: 7, fill: '#80cbc4', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(this._brakeArmatureRect);
        this._brakeArmatureY0 = spTop;
        this._brakeArmatureY1 = spTop - 6;

        // 摩擦片
        const frY = spBot + 4;
        this._staticGroup.add(new Konva.Rect({ x: bLeft+4, y: frY, width: bW-8, height: 8, fill: '#3e2723', stroke: '#795548', strokeWidth: 1, cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: bLeft+2, y: frY-10, width: bW-4, text: '摩擦片', fontSize: 7, fill: '#a1887f', align: 'center' }));
        ...
        this._staticGroup.add(this._brakeGapLine, this._brakeGapLabel);

        // 制动器接线
        const brkTermX = ex + ew + 10;
        [['#ef9a9a',80,'+BRK'],['#90caf9',96,'−BRK']].forEach(([col,dy,lbl]) => {
            this._staticGroup.add(new Konva.Line({ points: [bCX, bTop+coilH*0.5, brkTermX, this._csCY+dy], stroke: col, strokeWidth: 1.5, dash: [3,3] }));
            this._staticGroup.add(new Konva.Circle({ x: brkTermX, y: this._csCY+dy, radius: 3.5, fill: col }));
            this._staticGroup.add(new Konva.Text({ x: brkTermX+4, y: this._csCY+dy-5, text: lbl, fontSize: 7.5, fill: col }));
        });

        this._brakeCoilGlow = new Konva.Rect({ x: bLeft+3, y: coilY-1, width: bW-6, height: coilH+2, fill: 'rgba(255,167,38,0)', cornerRadius: 2 });
        this._staticGroup.add(this._brakeCoilGlow);
    ...
    _drawBrakeAnimLayer() {
        ...
        this._staticGroup.add(this._brakeAnimGroup);
    }
    ...
    _drawControlBlock() {
        ...
        this._staticGroup.add(new Konva.Text({ x:cx+4, y:cy+2, width:cw-8, text:'直流伺服三环控制（位置环→速度环→电流环→PWM）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        mainBlocks.forEach(({ lbl, x, w, col }) => {
            this._staticGroup.add(new Konva.Rect({ x, y:y0-12, width:w, height:24, fill:'#0a1020', stroke:col, strokeWidth:1, cornerRadius:3 }));
            this._staticGroup.add(new Konva.Text({ x, y:y0-10, width:w, text:lbl, fontSize:6.5, fill:col, align:'center', lineHeight:1.3 }));
        });

        // 前向链路
        [[cx+42,cx+50],[cx+84,cx+92],[cx+126,cx+134],[cx+168,cx+176],[cx+210,cx+218],[cx+252,cx+260]].forEach(([x1,x2])=>{
            this._staticGroup.add(new Konva.Arrow({ points:[x1,y0,x2,y0], stroke:'#4fc3f7', fill:'#4fc3f7', strokeWidth:1, pointerLength:4, pointerWidth:3 }));
        });

        // 位置反馈
        const fbY1 = y0 + ch*0.40;
        this._staticGroup.add(new Konva.Line({ points:[cx+cw-18,y0+12, cx+cw-18,fbY1, cx+52,fbY1, cx+52,y0+12], stroke:'#ffd54f', strokeWidth:1, dash:[4,3], opacity:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:cx+cw/2-35, y:fbY1-9, text:'θ 位置反馈（编码器）', fontSize:6.5, fill:'#ffd54f', opacity:0.9 }));

        // 速度反馈
        const fbY2 = y0 + ch*0.25;
        this._staticGroup.add(new Konva.Line({ points:[cx+cw-18,y0+12, cx+cw-18,fbY2, cx+94,fbY2, cx+94,y0+12], stroke:'#4fc3f7', strokeWidth:1, dash:[3,3], opacity:0.7 }));
        this._staticGroup.add(new Konva.Text({ x:cx+cw*0.55, y:fbY2-9, text:'ω 速度反馈', fontSize:6.5, fill:'#4fc3f7', opacity:0.7 }));

        // 电流反馈
        this._staticGroup.add(new Konva.Line({ points:[cx+260,y0, cx+260,y0+12, cx+136,y0+12, cx+136,y0], stroke:'#ef9a9a', strokeWidth:1, dash:[3,3], opacity:0.6 }));
        this._staticGroup.add(new Konva.Text({ x:cx+190, y:y0+14, text:'ia 电流反馈', fontSize:6.5, fill:'#ef9a9a', opacity:0.65 }));
        ...
        this._staticGroup.add(this._ctrlErrLabel, this._ctrlSpdLabel, this._ctrlCurLabel);
    }
    ...
    _drawTraceChart() {
        ...
        this._staticGroup.add(new Konva.Text({ x:tx+4, y:ty+2, width:tw-8, text:'位置跟踪曲线（给定 θ* vs 实际 θ）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x:ox-10, y:oy-ah, text:'θ', fontSize:7, fill:'#80cbc4' }));
        ...
        this._staticGroup.add(this._tracePosRefLine, this._tracePosLine, this._traceErrLine);
    ...
    _drawSCurveChart() {
        ...
        this._staticGroup.add(new Konva.Text({ x:sx+4, y:sy+2, width:sw-8, text:'S 曲线速度规划（Jerk 限制）', fontSize:8, fontStyle:'bold', fill:'#80cbc4', align:'center' }));

        const ox=sx+12, oy=sy+sh-10, aw=sw-18, ah=sh-26;
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah,ox,oy,ox+aw,oy], stroke:'#37474f', strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:ox-10, y:oy-ah, text:'v', fontSize:7, fill:'#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw+2, y:oy+2, text:'t', fontSize:7, fill:'#80cbc4' }));
        ...
        this._staticGroup.add(new Konva.Line({ points:pts, stroke:'#4fc3f7', strokeWidth:1.8, lineJoin:'round', opacity:0.6 }));

        this._staticGroup.add(new Konva.Text({ x:ox+aw*0.5, y:sy+sh-22, text:`Jmax=${this.profileMaxJerk}rpm/s²  Amax=${this.profileMaxAcc}rpm/s`, fontSize:6.5, fill:'#4fc3f7', align:'center', width:sw-14 }));

        this._scrvPoint = new Konva.Circle({ x:ox, y:oy, radius:5, fill:'#66bb6a', stroke:'#2e7d32', strokeWidth:1.5 });
        this._staticGroup.add(this._scrvPoint);
    ...
    _drawBrakePanel() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:bx, y:by, width:bw, height:bh, fill:'#100a00', stroke:'#ffa726', strokeWidth:1.5, cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({ x:bx, y:by, width:bw, height:13, fill:'#1a1000', cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:bx+4, y:by+2, width:bw-8, text:'制动器状态', fontSize:8, fontStyle:'bold', fill:'#ffa726', align:'center' }));
        ...
        this._staticGroup.add(this._brakeIconBg, this._brakeIconText);
        ...
        cells.forEach(({lbl,id,unit,color},i)=>{
            ...
            this._staticGroup.add(new Konva.Rect({x:cx3,y:cy3,width:cellW,height:cellH,fill:'#1a1000',cornerRadius:2,stroke:'#2a1500',strokeWidth:0.5}));
            this._staticGroup.add(new Konva.Text({x:cx3+2,y:cy3+2,text:lbl,fontSize:6.5,fill:'#5d4037'}));
            ...
            this._staticGroup.add(new Konva.Text({x:cx3+2,y:cy3+13,width:cellW-4,text:unit,fontSize:6,fill:'#1a0a00',align:'right'}));
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({x:lx+4,y:ly+2,width:lw-8,text:'直流伺服运行仪表',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Text({x:px+4,y:py+2,width:pw-8,text:'运动控制操作',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        this._interactGroup.add(brkBtn, this._brkBtnText);
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+1,width:ww-8,text:'速度 ω   转矩 T   电枢电流 ia   电枢电压 ua   制动器气隙 δ',fontSize:7.5,fill:'#80cbc4',align:'center'}));
        ...
        ['ω(rpm)','T(N·m)','ia(A)','ua(V)','δ制动'].forEach((l,i)=>{
            ...
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:7,fill:colors[i]}));
        });
        this._staticGroup.add(this._wLSpd,this._wLTq,this._wLIa,this._wLUa,this._wLBrkG);
        this._wavHCh=hCh;
    }
