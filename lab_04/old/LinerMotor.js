...
export class LinearMotor extends BaseComponent {
    ...
    _drawLinearMotorSection() {
        ...
        const railH = lh * 0.12;
        this._staticGroup.add(new Konva.Rect({ x: lx+4, y: railY, width: lw-8, height: railH, fill: '#263238', stroke: '#37474f', strokeWidth: 1.5, cornerRadius: 2 }));
        // 导轨纹（工字钢截面示意）
        for (let i = 0; i < 12; i++) {
            const x2 = lx + 8 + i * (lw-16) / 12;
            this._staticGroup.add(new Konva.Line({ points:[x2, railY, x2, railY+railH], stroke:'rgba(255,255,255,0.08)', strokeWidth:0.8 }));
        }
        this._staticGroup.add(new Konva.Text({ x: lx+6, y: railY+railH/3, text: '导轨 / 机座', fontSize: 8, fill: '#546e7a' }));
        ...
        if (this.motorType === 'LIM') {
            // 铝板（LIM 次级）
            this._staticGroup.add(new Konva.Rect({ x: lx+4, y: secY, width: lw-8, height: secH, fill: '#5d7a8a', stroke: '#78909c', strokeWidth: 1, cornerRadius: 1 }));
            // 涡流图案（示意）
            for (let i = 0; i < 8; i++) {
                const cx2 = lx + 20 + i*(lw-40)/8;
                this._drawEdgyCurrent(cx2, secY + secH/2, secH*0.35);
            }
            this._staticGroup.add(new Konva.Text({ x: lx+6, y: secY+secH*0.3, text: '次级铝板（感应涡流产生推力）', fontSize: 8, fill: '#b0bec5' }));
        } else {
            // 永磁阵列（PMLM 次级）
            ...
            for (let i = 0; i < nPoles; i++) {
                ...
                if (poleW > 16)
                    this._staticGroup.add(new Konva.Text({ x: lx+4+i*poleW+poleW/2-4, y: secY+secH*0.3, text: label, fontSize: 9, fill: '#1a252f', fontStyle: 'bold' }));
            }
            this._staticGroup.add(new Konva.Text({ x: lx+6, y: secY-12, text: '次级永磁阵列（N-S 交替，极距 τ）', fontSize: 8, fill: '#ef9a9a' }));
        }
        ...
        const gapH = lh * 0.09;
        this._staticGroup.add(new Konva.Rect({ x: lx+4, y: gapY, width: lw-8, height: gapH, fill: '#06101a', opacity: 0.9 }));
        this._staticGroup.add(new Konva.Text({ x: lx+lw/2-12, y: gapY+1, text: '← 气隙 δ →', fontSize: 7, fill: '#37474f' }));
        ...
        this._staticGroup.add(this._primaryGroup, this._posArrow, this._posLabel, this._velArrow, this._velLabel);

        // ── 行程尺（底部刻度）──
        const scaleY = ly + lh - 10;
        this._staticGroup.add(new Konva.Line({ points:[lx+4,scaleY,lx+lw-4,scaleY], stroke:'#37474f', strokeWidth:0.8 }));
        for (let i = 0; i <= 10; i++) {
            const sx = lx+4 + (i/10)*(lw-8);
            this._staticGroup.add(new Konva.Line({ points:[sx,scaleY,sx,scaleY+(i%5===0?5:3)], stroke:'#37474f', strokeWidth:0.7 }));
            if (i%5===0)
                this._staticGroup.add(new Konva.Text({ x:sx-8,y:scaleY+6, text:`${(i/10*this.maxStroke).toFixed(1)}m`, fontSize:6, fill:'#37474f' }));
        }
        // 目标位置指示线
        this._targetLine = new Konva.Line({ points:[lx+4,priY-2,lx+4,scaleY+8], stroke:'#ffd54f',strokeWidth:1,dash:[4,3],opacity:0.7 });
        this._staticGroup.add(this._targetLine);
    }
    ...
    _drawEdgyCurrent(cx, cy, r) {
        ...
        this._staticGroup.add(new Konva.Line({ points:pts, stroke:'rgba(200,220,255,0.18)', strokeWidth:1.2, lineJoin:'round', closed:true }));
    }
    ...
    _drawPrimaryMoverLayer() {
        ...
        this._staticGroup.add(this._moverEffectGroup);
    }
    ...
    _drawFVCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x:fx+4,y:fy+2,width:fw-8, text:'F-v 推力-速度特性', fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));
        ...
        this._staticGroup.add(new Konva.Text({ x:ox-12,y:oy-ah,text:'F(N)',fontSize:7,fill:'#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw+2,y:oy+2,text:'v(m/s)',fontSize:7,fill:'#80cbc4' }));

        // v_s 同步速度线
        const vsX = ox + (this.syncSpeed/this.maxSpeed)*aw;
        this._staticGroup.add(new Konva.Line({ points:[vsX,oy-ah,vsX,oy],stroke:'#ffd54f',strokeWidth:0.7,dash:[3,3] }));
        this._staticGroup.add(new Konva.Text({ x:vsX-4,y:oy+3,text:'v_s',fontSize:6.5,fill:'#ffd54f' }));
        ...
        this._staticGroup.add(new Konva.Line({ points:limPts,    stroke:'#ef9a9a',strokeWidth:1.5,lineJoin:'round',opacity:0.65,dash:[4,3] }));
        this._staticGroup.add(new Konva.Line({ points:limEndPts, stroke:'#66bb6a',strokeWidth:1.8,lineJoin:'round',opacity:0.75 }));

        // 图例
        this._staticGroup.add(new Konva.Line({ points:[fx+6,fy+14,fx+16,fy+14],stroke:'#ef9a9a',strokeWidth:1.5,dash:[4,3] }));
        this._staticGroup.add(new Konva.Text({ x:fx+18,y:fy+10,text:'无端部效应',fontSize:6.5,fill:'#ef9a9a' }));
        this._staticGroup.add(new Konva.Line({ points:[fx+6,fy+23,fx+16,fy+23],stroke:'#66bb6a',strokeWidth:1.5 }));
        this._staticGroup.add(new Konva.Text({ x:fx+18,y:fy+19,text:'含端部效应',fontSize:6.5,fill:'#66bb6a' }));

        this._fvWorkPt = new Konva.Circle({ x:ox,y:oy,radius:5,fill:'#ffd54f',stroke:'#f9a825',strokeWidth:1.5 });
        this._staticGroup.add(this._fvWorkPt);
    ...
    _drawFSCurve() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:fx,y:fy,width:fw,height:fh,fill:'#010d18',stroke:'#1a3040',strokeWidth:1.5,cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({ x:fx,y:fy,width:fw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:fx+4,y:fy+2,width:fw-8,text:'F-s 推力-转差率',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));

        const ox=fx+14, oy=fy+fh-12, aw=fw-20, ah=fh-26;
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah,ox,oy,ox+aw,oy],stroke:'#37474f',strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:ox-12,y:oy-ah,text:'F(N)',fontSize:7,fill:'#80cbc4' }));
        this._staticGroup.add(new Konva.Text({ x:ox+aw+2,y:oy+2,text:'s',fontSize:7,fill:'#80cbc4' }));
        ...
        this._staticGroup.add(new Konva.Line({ points:fsPts,stroke:'#4fc3f7',strokeWidth:2,lineJoin:'round',opacity:0.75 }));

        // 临界转差率标注
        const sm = this.LIM_R2 / Math.sqrt(this.LIM_R1**2+(this.LIM_X1+this.LIM_X2)**2);
        const smX = ox+(1-sm)*aw;
        this._staticGroup.add(new Konva.Line({ points:[smX,oy-ah,smX,oy],stroke:'#ffa726',strokeWidth:0.8,dash:[3,3] }));
        this._staticGroup.add(new Konva.Text({ x:smX-4,y:oy+3,text:'s_m',fontSize:6.5,fill:'#ffa726' }));

        this._fsWorkPt = new Konva.Circle({ x:ox,y:oy,radius:5,fill:'#66bb6a',stroke:'#2e7d32',strokeWidth:1.5 });
        this._staticGroup.add(this._fsWorkPt);
    ...
    _drawDQPlane() {
        ...
        this._staticGroup.add(new Konva.Text({ x:dx+4,y:dy+2,width:dw-8,text:'d-q 电流平面（PMLM）',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));

        const ocx=dx+dw*0.55, ocy=dy+dh*0.55;
        const R=Math.min(dw,dh)*0.32;

        this._staticGroup.add(new Konva.Line({ points:[dx+6,ocy,dx+dw-6,ocy],stroke:'#37474f',strokeWidth:0.7 }));
        this._staticGroup.add(new Konva.Line({ points:[ocx,dy+14,ocx,dy+dh-6],stroke:'#37474f',strokeWidth:0.7 }));
        this._staticGroup.add(new Konva.Text({ x:dx+dw-12,y:ocy+3,text:'id',fontSize:8,fill:'#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x:ocx+3,y:dy+14,text:'iq',fontSize:8,fill:'#80cbc4' }));

        // 电流极限圆
        const iMaxPx = this.ratedCurrent*1.5 * (R/(this.ratedCurrent*1.5));
        this._staticGroup.add(new Konva.Circle({ x:ocx,y:ocy,radius:R,fill:'rgba(102,187,106,0.05)',stroke:'#66bb6a',strokeWidth:0.8,dash:[4,3] }));
        ...
        if (this.PMLM_Ld !== this.PMLM_Lq) {
            ...
            if (mtpaPts.length>4)
                this._staticGroup.add(new Konva.Line({ points:mtpaPts,stroke:'#ffa726',strokeWidth:1.5,lineJoin:'round',opacity:0.7 }));
        }
        // id=0 线
        this._staticGroup.add(new Konva.Line({ points:[ocx,ocy,ocx,dy+14],stroke:'#ffd54f',strokeWidth:0.7,dash:[3,3],opacity:0.5 }));
        ...
        this._staticGroup.add(this._dqIdLine,this._dqIqLine,this._dqWorkPt);
    ...
    _drawDynamicResponse() {
        ...
        this._staticGroup.add(new Konva.Rect({ x:dx,y:dy,width:dw,height:dh,fill:'#010d18',stroke:'#1a3040',strokeWidth:1.5,cornerRadius:4 }));
        this._staticGroup.add(new Konva.Rect({ x:dx,y:dy,width:dw,height:13,fill:'#0a1a28',cornerRadius:[4,4,0,0] }));
        this._staticGroup.add(new Konva.Text({ x:dx+4,y:dy+2,width:dw-8,text:'位移 x(t) / 速度 v(t)',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center' }));

        const ox=dx+12, oy=dy+dh-12, aw=dw-18, ah=dh-26;
        this._staticGroup.add(new Konva.Line({ points:[ox,oy-ah,ox,oy,ox+aw,oy],stroke:'#37474f',strokeWidth:0.8 }));
        this._staticGroup.add(new Konva.Text({ x:ox-10,y:oy-ah,text:'x/v',fontSize:7,fill:'#80cbc4' }));
        ...
        this._staticGroup.add(this._dynXLine,this._dynVLine,this._dynFLine);
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x:px+4,y:modeBY-2,text:'控制模式:',fontSize:7.5,fill:'#546e7a' }));
        this._modeBtns={};
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x:wx+4,y:wy+1,width:ww-8,text:'三相电流 iU/iV/iW   电磁推力 F_em   速度 v   位移 x',fontSize:8,fill:'#80cbc4',align:'center' }));
        ...
        ['iU/iV/iW','F_em','v(m/s)','x(m)'].forEach((l,i)=>{
            this._staticGroup.add(new Konva.Text({ x:wx+4,y:wy+12+hCh*i+3,text:l,fontSize:8,fill:['#e53935','#66bb6a','#4fc3f7','#ffd54f'][i] }));
        });
        this._staticGroup.add(this._wLIU,this._wLIV,this._wLIW,this._wLF,this._wLV,this._wLX);
        this._wavHCh=hCh;
    }
