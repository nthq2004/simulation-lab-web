...
export class MARK6OilMistDetector extends BaseComponent {
    ...
    _drawDetectorBody() {
        ...
        [[cx+8,cy+8],[cx+cw-8,cy+8],[cx+8,cy+ch-8],[cx+cw-8,cy+ch-8]].forEach(([bx,by]) => {
            this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 4, fill: '#0d1020', stroke: '#263238', strokeWidth: 0.5 }));
            this._staticGroup.add(new Konva.Circle({ x: bx-1, y: by-1, radius: 1.3, fill: 'rgba(255,255,255,0.18)' }));
        });
        // 高光条
        ...
        this._staticGroup.add(body, this._measCav, nameBar);
    }
    ...
    _drawOpticalPaths() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-10, y: ch1Y-tubeH/2-14, text: 'CH1  测量通道', fontSize: 8.5, fontStyle: 'bold', fill: '#42a5f5' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-10, y: ch2Y+tubeH/2+4, text: 'CH2  参考通道（清洁空气）', fontSize: 8.5, fontStyle: 'bold', fill: '#66bb6a' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: procX+4, y: procY+3, width: 48, text: 'DSP\n信号处理', fontSize: 7, fill: '#80cbc4', align: 'center', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(this._ch1Tube, this._led1Glow, led1, led1Lbl, pd1, pd1Lbl);
        this._staticGroup.add(this._ch2Tube, this._led2Glow, led2, led2Lbl, pd2, pd2Lbl);
        this._staticGroup.add(procBox);
    }
    ...
    _drawSamplingTubes() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: this._cellX-32, y: cy-26, width: 16, height: 16, fill: '#ef5350', stroke: '#c62828', strokeWidth: 1.5, cornerRadius: [2,0,0,2] }));
        this._staticGroup.add(new Konva.Text({ x: this._cellX-48, y: cy-30, text: '油雾\n进口', fontSize: 7.5, fill: '#ef9a9a', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(new Konva.Rect({ x: this._cellX-32, y: cy+10, width: 16, height: 16, fill: '#546e7a', stroke: '#37474f', strokeWidth: 1.5, cornerRadius: [2,0,0,2] }));
        this._staticGroup.add(new Konva.Text({ x: this._cellX-46, y: cy+10, text: '气体\n出口', fontSize: 7.5, fill: '#90a4ae', lineHeight: 1.3 }));

        this._staticGroup.add(inPipe, outPipe);
    }
    ...
    _drawOilMistLayer() {
        ...
        this._staticGroup.add(this._oilMistGroup, this._lightBeamGroup);
    }
    ...
    _drawInstrHead() {
        ...
        for (let i = 0; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [hx, hy+7+i*10, hx+hw, hy+7+i*10], stroke: 'rgba(255,255,255,0.12)', strokeWidth: 0.8 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+18, width: hw-16, text: 'MARK 6  OIL MIST', fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+27, width: hw-16, text: `CAN ${this.canBaudrate}kbps  NODE:${this.nodeId}`, fontSize: 7, fill: '#90a4ae', align: 'center' }));
        ...
        [['CAN-H','#ef9a9a',14],['CAN-L','#90caf9',34],['V+','#e53935',58],['GND','#546e7a',78]].forEach(([lbl,col,ty]) => {
            this._staticGroup.add(new Konva.Rect({ x: hx+4, y: hy+ty-7, width: hw-8, height: 13, fill: 'rgba(255,255,255,0.025)', cornerRadius: 2 }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._concArc, this._lcdMain, this._lcdUnit, this._lcdCH12, this._lcdStatus, this._lcdNode);
    }
    ...
    _drawCANPanel() {
        ...
        for (let i = 0; i < this._canLogMax; i++) {
            ...
            this._staticGroup.add(t2);
        }

        // CAN-H / CAN-L 总线活动条
        const busY = cy2 + ch - 12;
        this._staticGroup.add(new Konva.Rect({ x: cx2+4, y: busY-4, width: cw-8, height: 6, fill: '#0d2030', cornerRadius: 2 }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: busY+4, text: 'CAN-H', fontSize: 6.5, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: cx2+35, y: busY+4, text: 'CAN-L', fontSize: 6.5, fill: '#90caf9' }));

        this._staticGroup.add(bg, titleBg, this._canHBus, this._canLBus);
    }
    ...
    _drawWaveforms() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '油雾浓度 %LEL  ── CH1  ── CH2', fontSize: 8, fontStyle: 'bold', fill: '#4dd0e1', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/3, wx+ww, wy+wh*i/3], stroke: 'rgba(77,208,225,0.06)', strokeWidth: 0.5 }));
        for (let i = 1; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [wx+ww*i/4, wy, wx+ww*i/4, wy+wh], stroke: 'rgba(77,208,225,0.05)', strokeWidth: 0.5 }));

        this._wavMidCH1 = wy + wh * 0.26;
        this._wavMidCH2 = wy + wh * 0.74;
        [this._wavMidCH1, this._wavMidCH2].forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [wx+2, my, wx+ww-2, my], stroke: 'rgba(200,200,200,0.10)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points: [wx+2, this._wavMidCH1 - preAlarmRatio*wavAmp, wx+ww-2, this._wavMidCH1 - preAlarmRatio*wavAmp], stroke: 'rgba(255,213,79,0.35)', strokeWidth: 0.8, dash: [3,3] }));
        this._staticGroup.add(new Konva.Line({ points: [wx+2, this._wavMidCH1 - alarmRatio*wavAmp,    wx+ww-2, this._wavMidCH1 - alarmRatio*wavAmp],    stroke: 'rgba(239,83,80,0.35)',  strokeWidth: 0.8, dash: [3,3] }));
        this._staticGroup.add(new Konva.Text({ x: wx+ww-24, y: this._wavMidCH1 - preAlarmRatio*wavAmp - 9, text: 'PRE', fontSize: 7, fill: 'rgba(255,213,79,0.7)' }));
        this._staticGroup.add(new Konva.Text({ x: wx+ww-24, y: this._wavMidCH1 - alarmRatio*wavAmp - 9,    text: 'ALM', fontSize: 7, fill: 'rgba(239,83,80,0.7)' }));

        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'CH1 测量', fontSize: 8, fill: '#42a5f5' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+5, text: 'CH2 参考', fontSize: 8, fill: '#66bb6a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineCH1, this._wLineCH2, this._wCH1Lbl, this._wCH2Lbl);
    }
