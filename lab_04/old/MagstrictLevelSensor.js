...
export class MagnetoStrictiveLevelSensor extends BaseComponent {
    ...
    _drawTankBody() {
        ...
        // 顶部高光
        this._staticGroup.add(new Konva.Rect({ x: tx, y: ty, width: tw, height: 4, fill: 'rgba(255,255,255,0.10)', cornerRadius: [4,4,0,0] }));
        ...
        for (let i = 0; i <= 10; i++) {
            ...
            const isMajor = i % 5 === 0;
            this._staticGroup.add(new Konva.Line({ points: [tx+tw, ly, tx+tw+(isMajor?10:5), ly], stroke: '#78909c', strokeWidth: isMajor?1.2:0.7 }));
            if (isMajor) this._staticGroup.add(new Konva.Text({ x: tx+tw+12, y: ly-6, text: `${100-i*10}%`, fontSize: 8, fill: '#607d8b' }));
        }
        // 报警线
        ...
        this._staticGroup.add(outer, topCap, this._innerCav, botCap, this._hiLine, this._loLine);
    }
    ...
    _drawElectronicsHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-hw/2+6, y: hy+17, width: hw-12, text: 'MAGNETO LEVEL', fontSize: 7, fill: '#78909c', align: 'center' }));
        // LED 状态灯
        ...
        [[cx-hw/2-6, '#546e7a'], [cx+hw/2-2, '#546e7a']].forEach(([lx, col]) => {
            this._staticGroup.add(new Konva.Rect({ x: lx, y: hy+8, width: 8, height: 12, fill: col, stroke: '#37474f', strokeWidth: 1, cornerRadius: 2 }));
        });
        // 连接到仪表头的导线
        ...
        this._staticGroup.add(headBody, plate, this._headIdText, this._statusLed);
    }
    ...
    _drawWaveguideRod() {
        ...
        // 波导管外套管（保护管，浅灰）
        this._staticGroup.add(new Konva.Rect({ x: rx-5, y: rt, width: 10, height: rb-rt, fill: '#b0bec5', stroke: '#90a4ae', strokeWidth: 0.5, cornerRadius: 2 }));
        // 磁致伸缩芯线（中央金色细线）
        this._waveguideWire = new Konva.Line({ points: [rx, rt, rx, rb], stroke: '#c0a020', strokeWidth: 2, lineCap: 'round' });
        // 端部吸波阻尼（底部黑块，防止回波反射）
        this._staticGroup.add(new Konva.Rect({ x: rx-6, y: rb-8, width: 12, height: 10, fill: '#1a1a1a', stroke: '#263238', strokeWidth: 0.5, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: rx-18, y: rb+4, text: '阻尼端', fontSize: 7.5, fill: '#546e7a' }));
        // 顶端发射线圈
        this._staticGroup.add(new Konva.Rect({ x: rx-7, y: rt-2, width: 14, height: 14, fill: '#f57f17', stroke: '#e65100', strokeWidth: 1, cornerRadius: 3 }));
        this._staticGroup.add(new Konva.Text({ x: rx-16, y: rt-14, text: '发射线圈', fontSize: 7.5, fill: '#ff8f00' }));
        // 接收拾振器（波导管顶端，稍低于发射线圈）
        this._pickupCoil = new Konva.Rect({ x: rx-5, y: rt+15, width: 10, height: 8, fill: '#26c6da', stroke: '#0097a7', strokeWidth: 1, cornerRadius: 2 });
        this._staticGroup.add(new Konva.Text({ x: rx+8, y: rt+15, text: '拾振器', fontSize: 7.5, fill: '#4dd0e1' }));
        this._staticGroup.add(this._waveguideWire, this._pickupCoil);

        // 标注：波导管
        this._staticGroup.add(new Konva.Text({ x: rx-20, y: (rt+rb)/2-8, text: '波\n导\n管', fontSize: 7.5, fill: '#80cbc4', lineHeight: 1.4 }));
    }
    ...
    _drawLiquidLayer() {
        ...
        this._staticGroup.add(this._liquidRect, this._liquidSurf, this._rippleGroup);
    }
    ...
    _drawFloatMagnet() {
        ...
        this._staticGroup.add(this._floatGroup, this._floatLabel);

        // 双浮子（界面测量用）
        if (this.multiFloat) {
            this._floatGroup2 = this._floatGroup.clone();
            this._floatGroup2.y(this._rodTop + 60);
            this._staticGroup.add(this._floatGroup2);
        }
    }
    ...
    _drawPulseLayer() {
        ...
        this._staticGroup.add(this._pulseGroup);
    }

    // ── 扭转波层（动态）──────────────────
    _drawTofWaveLayer() {
        this._tofWaveGroup = new Konva.Group();
        this._staticGroup.add(this._tofWaveGroup);
    }
    ...
    _drawTofOscilloscope() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: oy+2, width: ow-8, text: 'TOF 时序  发射脉冲 ── 扭转波回波', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [ox, oy+oh*i/3, ox+ow, oy+oh*i/3], stroke: 'rgba(128,203,196,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 5; i++) this._staticGroup.add(new Konva.Line({ points: [ox+ow*i/5, oy, ox+ow*i/5, oy+oh], stroke: 'rgba(128,203,196,0.05)', strokeWidth: 0.5 }));

        this._tofMidPulse = oy + oh * 0.24;
        this._tofMidEcho  = oy + oh * 0.72;
        [this._tofMidPulse, this._tofMidEcho].forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [ox+2, my, ox+ow-2, my], stroke: 'rgba(200,200,200,0.10)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: oy+16, text: '发射', fontSize: 8, fill: '#ffd54f' }));
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: oy+oh/2+4, text: '回波', fontSize: 8, fill: '#4dd0e1' }));
        ...
        this._staticGroup.add(bg, titleBg, this._tofLinePulse, this._tofLineEcho, this._tofDeltaMarker, this._tofTofLabel, this._tofPosLabel);
    }
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+18, width: hw-16, text: 'MAGNETO  4-20mA', fontSize: 7, fill: '#78909c', align: 'center' }));
        ...
        this._staticGroup.add(jBox, plate, lcap, rcap, this._idText, body, ring, this._lcdBg, this._lvArc, this._lcdMain, this._lcdUnit, this._lcdPct, this._lcdCurr, this._lcdTof);
    }
    ...
    _drawPositionWave() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '液位 L(t)  mm', fontSize: 8, fontStyle: 'bold', fill: '#66bb6a', align: 'center' }));
        for (let i = 1; i < 3; i++) this._staticGroup.add(new Konva.Line({ points: [px, py+ph*i/3, px+pw, py+ph*i/3], stroke: 'rgba(102,187,106,0.07)', strokeWidth: 0.5 }));

        const midY = py + ph * 0.55;
        this._staticGroup.add(new Konva.Line({ points: [px+2, midY, px+pw-2, midY], stroke: 'rgba(200,200,200,0.1)', strokeWidth: 0.5, dash: [4,3] }));
        ...
        this._staticGroup.add(bg, titleBg, this._posLine, this._posCurrLbl);
    }
