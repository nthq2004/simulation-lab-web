...
export class MarineDieselGenerator extends BaseComponent {
    ...
    _drawBase() {
        ...
        // 底架高光（顶面）
        this._staticGroup.add(new Konva.Rect({ x: bx, y: by, width: bw, height: 5, fill: 'rgba(255,255,255,0.22)', cornerRadius: [0,0,0,0] }));
        // 减振器（底部，圆形）
        const vibPts = [bw*0.12, bw*0.38, bw*0.62, bw*0.88];
        vibPts.forEach(vx => {
            // 减振垫
            this._staticGroup.add(new Konva.Ellipse({ x: vx, y: by+bh-4, radiusX: 18, radiusY: 8, fill: '#4a4a4a', stroke: '#2a2a2a', strokeWidth: 1 }));
            this._staticGroup.add(new Konva.Ellipse({ x: vx, y: by+bh-8, radiusX: 18, radiusY: 6, fill: '#2a2a2a' }));
        });
        // 横向加强筋
        for (let i = 1; i < 4; i++) {
            this._staticGroup.add(new Konva.Line({ points: [bw*i/4, by+5, bw*i/4, by+bh-8], stroke: '#6a7f8c', strokeWidth: 2 }));
        }
    ...
    _drawAlternator() {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [sx, ay+4, sx, ay+ah-4], stroke: '#5a8fa0', strokeWidth: 1.5, opacity: 0.5 }));
        }
        // 顶部接线盒
        const jbW = Math.round(aw * 0.36), jbH = Math.round(ah * 0.14);
        const jbX = ax + (aw - jbW)/2;
        this._staticGroup.add(new Konva.Rect({ x: jbX, y: ay-jbH+4, width: jbW, height: jbH, fill: '#3a4a54', stroke: '#263238', strokeWidth: 1.5, cornerRadius: [3,3,0,0] }));
        this._staticGroup.add(new Konva.Text({ x: jbX+2, y: ay-jbH+6, width: jbW-4, text: 'ALTERNATOR', fontSize: 7, fill: 'rgba(255,255,255,0.5)', align: 'center' }));

        // 出线孔（左侧，三相 + N）
        for (let i = 0; i < 4; i++) {
            this._staticGroup.add(new Konva.Circle({ x: ax+6, y: ay+12+i*16, radius: 3.5, fill: '#1a1a1a', stroke: '#333', strokeWidth: 0.8 }));
        }

        // 铭牌
        const npX = ax+aw/2-22, npY = ay+ah/2+8;
        this._staticGroup.add(new Konva.Rect({ x: npX, y: npY, width: 44, height: 20, fill: '#2a3a44', cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: npX+2, y: npY+3, width: 40, text: `${this.ratedPower}kW\n${this.ratedVoltage}V`, fontSize: 7, fill: '#90caf9', align: 'center', lineHeight: 1.3 }));

        // 标注
        this._staticGroup.add(new Konva.Text({ x: ax, y: ay-18, width: aw, text: '同步发电机', fontSize: 8.5, fontStyle: 'bold', fill: '#37474f', align: 'center' }));

        this._staticGroup.add(altBody, frontEndCap, rearEndCap);
    }
    ...
    _drawAltFan() {
        ...
        // 风扇罩
        this._staticGroup.add(new Konva.Circle({ x: ax+8, y: cy, radius: fanR+4, fill: 'none', stroke: '#4a6a7a', strokeWidth: 2 }));
        // 风扇叶片（动态旋转组）
    ...
    _drawCoupling() {
        ...
        // 螺栓
        for (let i = -1; i <= 1; i += 2) {
            this._staticGroup.add(new Konva.Circle({ x: this._coupX + this._coupW/2, y: cy + i*r1*0.5, radius: 2.5, fill: '#37474f' }));
        }
        this._staticGroup.add(leftFlange, midBody, rightFlange);
    }
    ...
    _drawEngineBody() {
        ...
        // 顶部深色带（进气管安装区）
        this._staticGroup.add(new Konva.Rect({ x: ex, y: ey, width: ew, height: Math.round(eh*0.14), fill: '#5a9aaa', stroke: '#4a8a9a', strokeWidth: 0, cornerRadius: [4,6,0,0] }));
        // 顶面高光
        this._staticGroup.add(new Konva.Rect({ x: ex+4, y: ey+2, width: ew-8, height: 4, fill: 'rgba(255,255,255,0.18)', cornerRadius: [2,2,0,0] }));
        ...
        // 油底壳（底部深色加厚区）
        this._staticGroup.add(new Konva.Rect({ x: ex+6, y: ey+eh-Math.round(eh*0.22), width: ew-12, height: Math.round(eh*0.22), fill: '#4a8090', stroke: '#3a6070', strokeWidth: 1, cornerRadius: [0,0,3,3] }));
        // 机油底壳螺栓
        for (let i = 0; i < 4; i++) {
            this._staticGroup.add(new Konva.Circle({ x: ex+20+i*(ew-30)/4, y: ey+eh-8, radius: 3.5, fill: '#2a3a44', stroke: '#1a2a34', strokeWidth: 0.5 }));
        }
        ...
        for (let i = 0; i < N; i++) {
            ...
            [[cvX+5, coverY+5],[cvX+coverW-7, coverY+5],[cvX+5, coverY+coverH-5],[cvX+coverW-7, coverY+coverH-5]].forEach(([bx,by]) => {
                this._staticGroup.add(new Konva.Circle({ x: bx, y: by, radius: 2.5, fill: '#4a8090' }));
            });
            this._staticGroup.add(cover, coverInner);
        }
        this._staticGroup.add(engBody);
    }
    ...
    _drawCylinderHeads() {
        ...
        for (let i = 0; i < N; i++) {
            ...
            this._staticGroup.add(head, rockerCover, injDot);
        }
    }
    ...
    _drawInletManifold() {
        ...
        for (let i = 0; i < this.cylinders; i++) {
            ...
            // 空滤外壳
            this._staticGroup.add(new Konva.Ellipse({ x: fx, y: fTopY + filterR*0.5, radiusX: filterR*0.45, radiusY: filterR*0.5, fill: '#4a6a7a', stroke: '#3a5a6a', strokeWidth: 1 }));
            this._staticGroup.add(new Konva.Rect({ x: fx-filterR*0.45, y: fTopY+filterR*0.5, width: filterR*0.9, height: filterR*0.8, fill: '#4a6a7a', stroke: '#3a5a6a', strokeWidth: 1 }));
            this._staticGroup.add(new Konva.Ellipse({ x: fx, y: fTopY + filterR*1.3, radiusX: filterR*0.45, radiusY: filterR*0.3, fill: '#3a5a6a', stroke: '#2a4a5a', strokeWidth: 1 }));
        }
        // 进气总管
        this._staticGroup.add(new Konva.Rect({ x: manifoldX, y: manifoldY, width: manifoldW, height: manifoldH, fill: '#5a9aaa', stroke: '#4a8a9a', strokeWidth: 1.5, cornerRadius: 2 }));
    }
    ...
    _drawExhaustPipes() {
        ...
        // 排气集管（水平钢管）
        this._staticGroup.add(new Konva.Rect({ x: exhX, y: exhY, width: 12, height: exhH, fill: '#3a3a3a', stroke: '#1a1a1a', strokeWidth: 1.5, cornerRadius: 2 }));
        // 绝热包扎（浅灰色）
        this._staticGroup.add(new Konva.Rect({ x: exhX-1, y: exhY-1, width: 14, height: exhH+2, fill: 'rgba(200,200,200,0.3)', cornerRadius: 2 }));

        // 出口管（连接到涡轮增压器）
        this._staticGroup.add(new Konva.Rect({ x: exhX+10, y: exhY+exhH/2-5, width: this._tcX-exhX-10, height: 10, fill: '#3a3a3a', stroke: '#1a1a1a', strokeWidth: 1 }));

        // 动态排气烟雾层
        this._exhaustGroup = new Konva.Group();
        this._staticGroup.add(this._exhaustGroup);
    }
    ...
    _drawTurbocharger() {
        ...
        // 压气机蜗壳（内圈）
        this._staticGroup.add(new Konva.Circle({ x: cx2, y: cy2, radius: R*0.7, fill: '#2a4a5a', stroke: '#1a3a4a', strokeWidth: 1 }));
        ...
        this._staticGroup.add(compressor, this._tcGlow, this._tcGroup, inletPipe, outletPipe);
        this._staticGroup.add(new Konva.Text({ x: cx2-20, y: ty+R*2+4, text: '涡轮增压器', fontSize: 8.5, fill: '#37474f' }));
    }
    ...
    _drawFuelSystem() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: fpX, y: fpY, width: 14, height: fpH, fill: '#3a5a6a', stroke: '#2a4a5a', strokeWidth: 1, cornerRadius: 2 }));
        this._staticGroup.add(new Konva.Text({ x: fpX-2, y: fpY+fpH+2, text: '油泵', fontSize: 7.5, fill: '#546e7a' }));
        ...
        for (let i = 0; i < N; i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points: [fpX + 7, fpY + i * fpH/N, cx2, headY], stroke: '#2a4a5a', strokeWidth: 1, opacity: 0.5 }));
        }
    }
    ...
    _drawFlywheel() {
        ...
        // 飞轮罩壳（轮廓）
        this._staticGroup.add(new Konva.Ellipse({ x: fwX, y: fwCY, radiusX: 18, radiusY: fwR+8, fill: '#3a4a54', stroke: '#263238', strokeWidth: 2 }));
        // 飞轮盘面
        ...
        this._staticGroup.add(this._flywheelGroup);
        this._staticGroup.add(new Konva.Text({ x: fwX-16, y: fwCY + fwR + 6, text: '飞轮', fontSize: 7.5, fill: '#546e7a' }));
    }
    ...
    _drawStarterMotor() {
        ...
        this._staticGroup.add(new Konva.Rect({ x: smX, y: smY, width: 28, height: smH, fill: '#2a3a44', stroke: '#1a2634', strokeWidth: 1.5, cornerRadius: 3 }));
        this._staticGroup.add(new Konva.Ellipse({ x: smX, y: smY+smH/2, radiusX: 10, radiusY: smH/2-2, fill: '#1a2634', stroke: '#0d1520', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: smX-2, y: smY+smH+2, text: '起动电机', fontSize: 7, fill: '#546e7a' }));
    }
    ...
    _drawControlPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+pw*0.3-7, y: btnY+7, text: 'START', fontSize: 5.5, fill: '#4caf50' }));
        this._staticGroup.add(new Konva.Text({ x: px+pw*0.7-6, y: btnY+7, text: 'STOP', fontSize: 5.5, fill: '#ef5350' }));

        // 点击事件
        this._btnStart.on('click tap', () => this.start());
        this._btnStop.on('click tap',  () => this.stop());

        this._interactGroup.add(panel, this._screen, this._screenLine1, this._screenLine2, this._btnStart, this._btnStop);
    }
    ...
    _drawDynamicLayers() {
        ...
        this._staticGroup.add(this._pistonFireGroup, this._vibGroup);
    }
    ...
    _drawParamDisplays() {
        ...
        params.forEach(({ label, id, color }, i) => {
            ...
            this._staticGroup.add(new Konva.Rect({ x: px2, y: py, width: wPerCell-2, height: this._baseH-4, fill: 'rgba(0,0,0,0.18)', cornerRadius: 2 }));
            ...
            this._staticGroup.add(new Konva.Text({ x: px2+2, y: py+11, width: wPerCell-6, text: label, fontSize: 7, fill: 'rgba(255,255,255,0.3)', align: 'center' }));
