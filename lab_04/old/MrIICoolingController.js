...
export class MrIICoolingController extends BaseComponent {
    ...
    _drawShell() {
        ...
        [[s.x+14, s.y+12], [s.x+s.w-14, s.y+12],
         [s.x+14, s.y+s.h-12], [s.x+s.w-14, s.y+s.h-12]].forEach(([x, y]) => {
            this._staticGroup.add(new Konva.Circle({ x, y, radius: sR, fill:'#8090a8', stroke:'#4a5468', strokeWidth:0.6 }));
            this._staticGroup.add(new Konva.Line({ points:[x-sR*0.65,y,x+sR*0.65,y], stroke:'#2a3040', strokeWidth:0.9, lineCap:'round' }));
            this._staticGroup.add(new Konva.Line({ points:[x,y-sR*0.65,x,y+sR*0.65], stroke:'#2a3040', strokeWidth:0.9, lineCap:'round' }));
        });
    ...
    _drawTerminalBlock() {
        ...
        terms.forEach(({ frac, label, color }) => {
            ...
            this._staticGroup.add(new Konva.Circle({ x: px, y: py+3.5, radius: 2.2, fill:'#1a1e28' }));
            this._staticGroup.add(new Konva.Text({
