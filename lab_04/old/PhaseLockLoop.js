...
export class PhaseLockLoop extends BaseComponent {
    ...
    _drawBackground() {
        ...
        // 网格
        for (let gx=20;gx<W-10;gx+=22) this._staticGroup.add(new Konva.Line({
            points:[gx,8,gx,H-8], stroke:'rgba(30,60,100,0.08)', strokeWidth:0.5,
        }));
        for (let gy=20;gy<H-10;gy+=22) this._staticGroup.add(new Konva.Line({
    ...
    _drawBlock_PFD() {
        ...
        this._staticGroup.add(this._upLabel, this._dnLabel);
    }
    ...
    _drawBlock_CP() {
        ...
        this._staticGroup.add(this._cpCurrentText);
    }
    ...
    _drawBlock_LF() {
        ...
        this._staticGroup.add(this._vctrlText);
    }
    ...
    _drawResistorV(cx, cy, name, val) {
        ...
        this._staticGroup.add(new Konva.Line({ points:[cx,cy-h/2,cx,cy-h/2-6], stroke:'#80a040', strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Line({ points:[cx,cy+h/2,cx,cy+h/2+6], stroke:'#80a040', strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Text({ x:cx+6, y:cy-6, text:`${name}\n${val}`, fontSize:6.5, fill:'#7a9040', fontFamily:'monospace', lineHeight:1.3 }));
    }

    _drawCapV(cx, cy, name, val, color='#60c080') {
        const gap=5, w=14;
        this._staticGroup.add(new Konva.Line({ points:[cx,cy-gap-6,cx,cy-gap], stroke:color, strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Line({ points:[cx-w/2,cy-gap,cx+w/2,cy-gap], stroke:color, strokeWidth:2.2 }));
        this._staticGroup.add(new Konva.Line({ points:[cx-w/2,cy+gap,cx+w/2,cy+gap], stroke:color, strokeWidth:2.2 }));
        this._staticGroup.add(new Konva.Line({ points:[cx,cy+gap,cx,cy+gap+6], stroke:color, strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Text({ x:cx+8, y:cy-6, text:`${name}\n${val}`, fontSize:6.5, fill:color, fontFamily:'monospace', lineHeight:1.3 }));
    }
    ...
    _drawBlock_VCO() {
        ...
        this._staticGroup.add(this._vcoFreqText);
    }
    ...
    _drawLCTank(cx, cy) {
        ...
        for (let i=0;i<turns;i++) {
            ...
            this._staticGroup.add(new Konva.Line({ points:pts, stroke:'#c090ff', strokeWidth:2, lineCap:'round', lineJoin:'round' }));
        }
        this._staticGroup.add(new Konva.Text({ x:cx-6, y:cy-22, text:'L', fontSize:8, fill:'#c090ff', fontFamily:'monospace' }));

        // 变容二极管 Cvar（可变电容）
        const cvx=cx, cvy=cy+8;
        this._staticGroup.add(new Konva.Line({ points:[cvx-16,cvy,cvx-4,cvy], stroke:'#a070ff', strokeWidth:1.5 }));
        this._staticGroup.add(new Konva.Line({ points:[cvx-4,cvy-8,cvx-4,cvy+8], stroke:'#a070ff', strokeWidth:2.5 }));
        this._staticGroup.add(new Konva.Line({ points:[cvx+4,cvy-8,cvx+4,cvy+8], stroke:'#a070ff', strokeWidth:2.5 }));
        // 变容二极管箭头
        this._staticGroup.add(new Konva.Line({ points:[cvx,cvy-6,cvx+4,cvy], stroke:'#ff80ff', strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Line({ points:[cvx,cvy+6,cvx+4,cvy], stroke:'#ff80ff', strokeWidth:1.2 }));
        this._staticGroup.add(new Konva.Line({ points:[cvx+4,cvy,cvx+16,cvy], stroke:'#a070ff', strokeWidth:1.5 }));
        this._staticGroup.add(new Konva.Text({ x:cx-4, y:cvy+10, text:'Cvar', fontSize:7, fill:'#a070ff', fontFamily:'monospace' }));
        ...
        this._staticGroup.add(new Konva.Text({ x:this._blocks[3].x+2, y:this._topY+4, text:'Vctrl→', fontSize:7, fill:'#80e080', fontFamily:'monospace' }));
    }
    ...
    _drawTopLabels() {
        ...
        this._staticGroup.add(this._lockStateText);
        ...
        this._staticGroup.add(this._progressBar);
    }
    ...
    _drawStatusPanel() {
        ...
        this._staticGroup.add(this._stateDot);
    }
