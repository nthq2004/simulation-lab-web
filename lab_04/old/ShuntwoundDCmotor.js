...
export class ShuntWoundDCMotor extends BaseComponent {
    ...
    _drawStatorShell() {
        ...
        // 外壳
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R + 12, fill: '#455a64', stroke: '#263238', strokeWidth: 2.5 }));
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R + 12, fill: 'rgba(255,255,255,0.06)', stroke: 'none' }));
        // 安装耳
        for (let i = 0; i < 4; i++) {
            const a = (i/4) * Math.PI * 2 + Math.PI/4;
            this._staticGroup.add(new Konva.Circle({ x: cx + (R+9)*Math.cos(a), y: cy + (R+9)*Math.sin(a), radius: 5, fill: '#37474f', stroke: '#263238', strokeWidth: 0.5 }));
        }
        // 定子内壁
        this._staticGroup.add(new Konva.Circle({ x: cx, y: cy, radius: R, fill: '#0d1a28' }));
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx - R, y: cy - R - 22, width: R*2, text: '并励直流电动机', fontSize: 9, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
    }
    ...
    _drawFieldWinding() {
        ...
        for (let i = 0; i < p * 2; i++) {
            ...
            this._staticGroup.add(new Konva.Text({ x: mlx-5, y: mly-6, width: 10, text: isN ? 'N' : 'S', fontSize: 9, fontStyle: 'bold', fill: '#fff', align: 'center' }));
        }
        this._staticGroup.add(new Konva.Text({ x: cx - R, y: cy + R + 14, width: R*2, text: `励磁: Rf=${this.Rf}Ω  If=${this._If.toFixed(2)}A`, fontSize: 7.5, fontFamily: 'Courier New, monospace', fill: '#ff8f00', align: 'center' }));
    }
    ...
    _drawCommutator() {
        ...
        [{ a: -Math.PI/2, col: '#ef9a9a' }, { a: Math.PI/2, col: '#90caf9' }].forEach(({ a, col }) => {
            ...
            this._staticGroup.add(new Konva.Rect({ x: bx-5, y: by-4, width: 10, height: 8, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1, cornerRadius: 1 }));
    ...
    _drawShaft() {
        const cx = this._motorCX, cy = this._motorCY, R = this._statorR;
        this._staticGroup.add(new Konva.Ellipse({ x: cx+R+6, y: cy, radiusX: 8, radiusY: R*0.50, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Rect({ x: cx+R+8, y: cy-5, width: 28, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Ellipse({ x: cx-R-6, y: cy, radiusX: 8, radiusY: R*0.50, fill: '#607d8b', stroke: '#455a64', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: cx+R+8, y: cy+8, text: '输出轴', fontSize: 7.5, fill: '#607d8b' }));
    }
    ...
    _drawElecSchematic() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+2, y: sy+2, width: sw-4, text: '并励接法', fontSize: 7.5, fill: '#80cbc4', align: 'center' }));

        // 简化电路线
        const mx = sx + sw/2;
        // 电枢支路
        this._staticGroup.add(new Konva.Text({ x: sx+3, y: sy+15, text: '─Ra─E_b─', fontSize: 6.5, fill: '#ef9a9a' }));
        this._schIaText = new Konva.Text({ x: sx+3, y: sy+25, text: `Ia=0A`, fontSize: 6.5, fontFamily: 'Courier New, monospace', fill: '#ef9a9a' });
        // 励磁支路
        this._staticGroup.add(new Konva.Text({ x: sx+3, y: sy+36, text: `─Rf─`, fontSize: 6.5, fill: '#ff8f00' }));
        this._staticGroup.add(new Konva.Text({ x: sx+3, y: sy+46, text: `If=${this._If.toFixed(2)}A`, fontSize: 6.5, fontFamily: 'Courier New, monospace', fill: '#ff8f00' }));

        this._staticGroup.add(this._schIaText);
    }
    ...
    _drawInputFlowLayer() {
        ...
        this._staticGroup.add(this._inputFlowGroup);
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+17, width: hw-16, text: `${this.Vs}V  并励接法`, fontSize: 7, fill: '#78909c', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+27, width: hw-16, text: 'SHUNT DC MOTOR', fontSize: 7, fill: '#90a4ae', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._speedArc, this._transientRing, this._lcdMain, this._lcdUnit, this._lcdIa, this._lcdTem, this._lcdEb);
    }
    ...
    _drawLoadPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '负载阻力矩调节  T_L (N·m)', fontSize: 8, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
        ...
        this._staticGroup.add(this._loadValText);
        ...
        this._staticGroup.add(bg, titleBg, this._loadBar, this._loadValText);
        ...
        this._staticGroup.add(this._dynStatusText);
    }
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '动态响应：n(rpm)  ── Ia(A)  ── T_em(N·m)', fontSize: 8, fontStyle: 'bold', fill: '#66bb6a', align: 'center' }));
        ...
        // 通道标签
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+14+4, text: 'n', fontSize: 8, fill: '#66bb6a' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+14+h3+4, text: 'Ia', fontSize: 8, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+14+h3*2+4, text: 'Tem', fontSize: 8, fill: '#80cbc4' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineN, this._wLineIa, this._wLineTem, this._wNLbl, this._wIaLbl, this._wTLbl);
        this._wavH3 = h3;
    }
