...
export class ACTachogenerator extends BaseComponent {
    ...
    _drawGeneratorShell() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            this._staticGroup.add(new Konva.Circle({ x: bx - 1, y: by - 1, radius: 1.2, fill: 'rgba(255,255,255,0.25)' }));
        }
        // 定子铁芯（带槽）
        ...
        // 气隙
        this._staticGroup.add(new Konva.Ring({ x: cx, y: cy, innerRadius: R * 0.58, outerRadius: R * 0.62, fill: 'rgba(100,150,200,0.08)' }));

        this._staticGroup.add(outerShell, outerRing, statorRing);
    }
    ...
    _drawStaticCoils() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx - 10, y: cy - R * 0.95, text: '励磁', fontSize: 8, fontStyle: 'bold', fill: '#ef9a9a' }));
        ...
        this._staticGroup.add(new Konva.Text({ x: cx + R * 0.62, y: cy + 4, text: '输出', fontSize: 8, fontStyle: 'bold', fill: '#90caf9' }));
    }
    ...
    _drawEquivCircuit() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: cy2+3, width: cw-4, text: '等效电路', fontSize: 8, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));

        const y1 = cy2 + 16, y2 = cy2 + ch - 8;
        const x1 = cx2 + 8, x2 = cx2 + cw * 0.48, x3 = cx2 + cw - 10;

        // 励磁回路（顶部）
        this._staticGroup.add(new Konva.Line({ points: [x1, y1, x3, y1], stroke: '#c62828', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: x1+2, y: y1-8, text: 'V_ref', fontSize: 7, fill: '#ef9a9a' }));
        // 励磁电感线圈
        for (let i = 0; i < 4; i++) {
            this._staticGroup.add(new Konva.Arc({ x: x1 + 20 + i * 9, y: y1, innerRadius: 3, outerRadius: 5, angle: 180, rotation: 90, fill: 'none', stroke: '#c62828', strokeWidth: 1 }));
        }
        this._staticGroup.add(new Konva.Text({ x: x1+18, y: y1+5, text: 'Lf', fontSize: 7, fill: '#c62828' }));

        // 互感（中间虚线）
        this._staticGroup.add(new Konva.Line({ points: [x2, y1+6, x2, y2-6], stroke: '#ffd54f', strokeWidth: 1, dash: [3,2] }));
        this._staticGroup.add(new Konva.Text({ x: x2+2, y: (y1+y2)/2-4, text: 'M', fontSize: 8, fill: '#ffd54f' }));

        // 输出回路（底部）
        this._staticGroup.add(new Konva.Line({ points: [x1, y2, x3, y2], stroke: '#1565c0', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: x1+2, y: y2+2, text: 'V_out', fontSize: 7, fill: '#90caf9' }));
        // 输出阻抗
        for (let i = 0; i < 3; i++) {
            this._staticGroup.add(new Konva.Line({ points: [x1+30+i*7, y2-3, x1+30+i*7+4, y2, x1+30+i*7+8, y2-3, x1+30+i*7+8, y2+3], stroke: '#1565c0', strokeWidth: 1 }));
        }
        this._staticGroup.add(new Konva.Text({ x: x1+28, y: y2-10, text: 'Ro', fontSize: 7, fill: '#1565c0' }));
        ...
        this._staticGroup.add(bg, this._circEMFLabel);
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+18, width: hw-16, text: 'AC TACHO  400Hz', fontSize: 7, fill: '#78909c', align: 'center' }));
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._ampArc, this._lcdMain, this._lcdUnit, this._lcdAmp, this._lcdFreq, this._lcdPhase);
    }
    ...
    _drawKnob() {
        ...
        this._interactGroup.add(base, this._knobRotor, kLbl);
    }
    ...
    _drawWaveformArea() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: '电压波形  ── 励磁 V_ref  ── 输出 V_out', fontSize: 8, fontStyle: 'bold', fill: '#ef9a9a', align: 'center' }));

        for (let i = 1; i < 4; i++) this._staticGroup.add(new Konva.Line({ points: [wx, wy+wh*i/4, wx+ww, wy+wh*i/4], stroke: 'rgba(239,154,154,0.07)', strokeWidth: 0.5 }));
        for (let i = 1; i < 5; i++) this._staticGroup.add(new Konva.Line({ points: [wx+ww*i/5, wy, wx+ww*i/5, wy+wh], stroke: 'rgba(239,154,154,0.05)', strokeWidth: 0.5 }));

        // 双通道中线
        this._wavMidRef = wy + wh * 0.25;
        this._wavMidOut = wy + wh * 0.75;
        [this._wavMidRef, this._wavMidOut].forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [wx+2, my, wx+ww-2, my], stroke: 'rgba(200,200,200,0.12)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+16, text: 'V_ref (励磁)', fontSize: 8, fill: '#c62828' }));
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+wh/2+4, text: 'V_out (输出)', fontSize: 8, fill: '#ef9a9a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineRef, this._wLineOut, this._wRefLbl, this._wOutLbl, this._wFreqLbl);
    }
    ...
    _drawLissajous() {
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+2, y: ly+2, width: lw-4, text: 'V_ref vs V_out', fontSize: 8, fontStyle: 'bold', fill: '#ffd54f', align: 'center' }));

        // 十字基准
        const lmx = lx + lw/2, lmy = ly + lh/2;
        this._staticGroup.add(new Konva.Line({ points: [lx+2, lmy, lx+lw-2, lmy], stroke: 'rgba(200,200,200,0.10)', strokeWidth: 0.5 }));
        this._staticGroup.add(new Konva.Line({ points: [lmx, ly+14, lmx, ly+lh-2], stroke: 'rgba(200,200,200,0.10)', strokeWidth: 0.5 }));
        this._staticGroup.add(new Konva.Text({ x: lx+2, y: lmy+2, text: 'X:V_ref', fontSize: 7, fill: '#546e7a' }));
        this._staticGroup.add(new Konva.Text({ x: lmx+2, y: ly+16, text: 'Y:V_out', fontSize: 7, fill: '#546e7a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._lissLine, this._lissPhiLbl);
    }
