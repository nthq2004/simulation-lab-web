...
export class ThreePhaseTransformer extends BaseComponent {
    ...
    _drawCoreYokes() {
        ...
        // 高光
        this._staticGroup.add(new Konva.Rect({ x: cx+2, y: cy+2, width: cw-4, height: 4, fill: 'rgba(255,255,255,0.12)', cornerRadius: 1 }));
        this._staticGroup.add(topYoke, botYoke);
        this._staticGroup.add(new Konva.Text({ x: cx, y: cy-18, width: cw, text: '三柱芯式铁芯  Three-Column Core', fontSize: 9, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawCoreLegs() {
        ...
        this._legXs.forEach((lx, i) => {
            ...
            this._staticGroup.add(leg, inner);
            // 相序标注（铁芯柱顶）
            this._staticGroup.add(new Konva.Text({ x: this._legCXs[i]-8, y: this._legY-18, text: phNames[i]+'相', fontSize: 9, fontStyle: 'bold', fill: cols[i] }));
        });
    }
    ...
    _drawCoreLaminations() {
        ...
        for (let i = 2; i < yk-1; i += 3) {
            this._staticGroup.add(new Konva.Line({ points: [cx+2,cy+i,cx+cw-2,cy+i], stroke: 'rgba(0,0,0,0.14)', strokeWidth: 0.7 }));
        }
        for (let i = 2; i < yk-1; i += 3) {
            this._staticGroup.add(new Konva.Line({ points: [cx+2,cy+this._coreH-yk+i,cx+cw-2,cy+this._coreH-yk+i], stroke: 'rgba(0,0,0,0.14)', strokeWidth: 0.7 }));
        }
        // 柱叠片
        this._legXs.forEach(lx => {
            for (let i = 2; i < this._legH-1; i += 3) {
                this._staticGroup.add(new Konva.Line({ points: [lx+2,this._legY+i,lx+this._legW-2,this._legY+i], stroke: 'rgba(0,0,0,0.16)', strokeWidth: 0.7 }));
            }
        });
    }
    ...
    _drawWindings() {
        ...
        this._legXs.forEach((lx, i) => {
            ...
            this._staticGroup.add(hiGrp, loGrp);

            // 绕组标注
            this._staticGroup.add(new Konva.Text({ x: lx-hiCoilW/2-30, y: windY+windH/2-5, text: `HV\n${i===0?'A':i===1?'B':'C'}`, fontSize: 7.5, fill: '#ff8f00', lineHeight: 1.3, align: 'center', width: 18 }));
            this._staticGroup.add(new Konva.Text({ x: lx+lw+loCoilW/2+12, y: windY+windH/2-5, text: `LV\n${i===0?'a':i===1?'b':'c'}`, fontSize: 7.5, fill: '#42a5f5', lineHeight: 1.3, align: 'center', width: 18 }));
        });

        // 绕组标注（顶部）
        const midLegX = this._legCXs[1];
        this._staticGroup.add(new Konva.Rect({ x: this._legCXs[0]-30, y: this._coreY-1, width: 24, height: 10, fill: '#1a0800', cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: this._legCXs[0]-30, y: this._coreY, text: '─ 高压绕组', fontSize: 7, fill: '#ff8f00' }));
        this._staticGroup.add(new Konva.Rect({ x: this._legCXs[0]-30, y: this._coreY+10, width: 24, height: 10, fill: '#00081a', cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Text({ x: this._legCXs[0]-30, y: this._coreY+11, text: '─ 低压绕组', fontSize: 7, fill: '#42a5f5' }));
    }

    // ── 磁通流动层（三路，相位差120°）─────
    _drawFluxLayer() {
        this._fluxGroups = [new Konva.Group(), new Konva.Group(), new Konva.Group()];
        this._fluxGroups.forEach(g => this._staticGroup.add(g));
    }

    // ── 漏磁通层 ─────────────────────────────
    _drawLeakageLayer() {
        this._leakGroups = [new Konva.Group(), new Konva.Group(), new Konva.Group()];
        this._leakGroups.forEach(g => this._staticGroup.add(g));
    }
    ...
    _drawConnectionDiagram() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+2, y: cy2+2, width: cw-4, text: '接线方式', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        this._connDiagGroup = new Konva.Group({ x: cx2, y: cy2 });
        this._staticGroup.add(bg, titleBg, this._connDiagGroup);
    ...
    _drawEquivCircuitSmall() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ex+2, y: ey+2, width: ew-4, text: `等效电路：R₁=${this.R1.toFixed(3)} X₁=${this.X1.toFixed(3)} Uk=${this.Uk}%  I₀=${this.I0pct}%  k=${this.k.toFixed(1)}`, fontSize: 7.5, fontFamily: 'Courier New, monospace', fill: '#546e7a', align: 'center' }));
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(ring, this._lcdBg, this._loadArc, this._lcdMain, this._lcdUnit, this._lcdEta, this._lcdV1l, this._lcdV2l, bg, titleBg);
        ...
        params.forEach(({label,id,unit,color},i)=>{
            ...
            this._staticGroup.add(new Konva.Text({x:cx3+2,y:cy3+cellH-9,width:cellW-4,text:unit,fontSize:6.5,fill:'#1a252f',align:'right'}));
    ...
    _drawLoadPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: px+4, y: py+2, width: pw-8, text: '负载调节', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        this._staticGroup.add(bg, titleBg, this._loadText, this._oilTempText);
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+2,width:ww-8,text:'高压三相电压 V1_a/b/c ── 低压三相电压 V2_a/b/c',fontSize:8,fontStyle:'bold',fill:'#80cbc4',align:'center'}));
        ...
        for(let i=0;i<3;i++){
            ...
            const phN=['A','B','C'];
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+13+h6*i+4,text:'V1'+phN[i],fontSize:8,fill:hiCols[i]}));
            this._staticGroup.add(new Konva.Text({x:wx+4,y:wy+13+h6*(i+3)+4,text:'V2'+phN[i],fontSize:8,fill:loCols[i]}));
        }
        ...
        this._staticGroup.add(bg,titleBg,...this._wLinesV1,...this._wLinesV2,this._wV1Lbl,this._wV2Lbl);
        this._wavH6=h6;
    }
