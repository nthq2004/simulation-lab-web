...
export class TurbineFlowmeter extends BaseComponent {
    ...
    _drawPipe() {
        ...
        [[px-11, 0], [px+pw-3, 1]].forEach(([fx, s]) => {
            ...
            [0.2, 0.5, 0.8].forEach(r => this._staticGroup.add(new Konva.Circle({
        ...
        this._staticGroup.add(outer, this._flowChan);
    }
    ...
    _drawTurbineGroup() {
        ...
        this._staticGroup.add(support, this._turbGroup, hub);
    }
    ...
    _drawSensorProbe() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-16, y: topY-30, text: '霍尔探头', fontSize: 8, fill: '#f57f17' }));
        this._staticGroup.add(body, stem, this._hallLed);
    }
    ...
    _drawFlowArrows() {
        ...
        [[this._fcX + 10, 0.35], [this._fcX + this._fcW * 0.72, 0.2]].forEach(([ax, op]) => {
            this._staticGroup.add(new Konva.Line({ points: [ax, cy, ax+16, cy], stroke: `rgba(79,195,247,${op+0.2})`, strokeWidth: 1.8 }));
            this._staticGroup.add(new Konva.Line({ points: [ax+10, cy-4, ax+16, cy, ax+10, cy+4], stroke: `rgba(79,195,247,${op+0.2})`, strokeWidth: 1.8, lineJoin: 'round' }));
        });
    }
    ...
    _drawInstrHead() {
        ...
        this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+18, width: hw-16, text: 'TURBINE  RS-485', fontSize: 7.5, fill: '#78909c', align: 'center' }));

        // 端子标签
        [['V+', '#e53935', 44], ['OUT', '#ff8f00', 62], ['485A', '#26c6da', 82], ['485B', '#00838f', 98]].forEach(([lbl, col, ty]) => {
            this._staticGroup.add(new Konva.Rect({ x: hx+5, y: hy+ty-8, width: hw-10, height: 13, fill: 'rgba(255,255,255,0.03)', cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: hx+8, y: hy+ty-5, text: lbl, fontSize: 9, fontStyle: 'bold', fill: col }));
        });
        ...
        this._staticGroup.add(jbox, sheen, plate, this._idText, this._ledPulse, this._led485, lblPulse, lbl485);
    }
    ...
    _drawLCD() {
        ...
        this._staticGroup.add(new Konva.Circle({ x: cx, y: this._lcdCY, radius: R+4, fill: '#1a252f', stroke: '#0d1520', strokeWidth: 1 }));
        // 橙色外环（涡轮工业感）
        this._staticGroup.add(new Konva.Circle({ x: cx, y: this._lcdCY, radius: R+2, fill: '#e65100', stroke: '#ff6d00', strokeWidth: 2.5 }));
        ...
        this._staticGroup.add(this._lcdBg, this._rpmArc, this._lcdMain, this._lcdUnit, this._lcdFreq, this._lcdCurr, this._lcdRpm);
    }
    ...
    _drawModbusPanel() {
        ...
        regs.forEach(([addr, name, col], i) => {
            ...
            this._staticGroup.add(addrTxt, nameTxt, valTxt);
        });

        // 分隔线
        this._staticGroup.add(new Konva.Line({ points: [mx+colW, my+18, mx+colW, my+mh], stroke: '#0d2030', strokeWidth: 1 }));
        ...
        this._staticGroup.add(bg, titleBg, title, logTitle, this._busLine, this._busActivity);
    }

    // ═══════════════════════════════════════════════
    //  动画主循环（由 consys._tickAll 在 20fps 调用）
    // ═══════════════════════════════════════════════
    tick(dt) {
