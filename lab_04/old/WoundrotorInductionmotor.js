...
export class WoundRotorInductionMotor extends BaseComponent {
    ...
    _drawStatorIron() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-this._statorRo, y: cy-this._statorRo-22, width: this._statorRo*2, text: '绕线式三相异步电动机（截面图）', fontSize: 9, fontStyle: 'bold', fill: '#546e7a', align: 'center' }));
    }
    ...
    _drawStatorWindings() {
        ...
        phases.forEach(ph => {
            ...
            this._staticGroup.add(new Konva.Text({ x: cx+(Ro+18)*Math.cos(a0)-8, y: cy+(Ro+18)*Math.sin(a0)-6, text: ph.name, fontSize: 10, fontStyle: 'bold', fill: ph.color }));
        });
    }
    ...
    _drawWoundRotor() {
        ...
        // 轴端引线标注
        this._staticGroup.add(new Konva.Text({ x: cx-this._rotorRo+2, y: cy+this._rotorRo+6, text: '转子绕组 R/S/T', fontSize: 8.5, fontStyle: 'bold', fill: '#90caf9' }));
    ...
    _drawShaft() {
        ...
        this._staticGroup.add(new Konva.Ellipse({ x: cx+Ro+6, y: cy, radiusX: 8, radiusY: this._statorRi*0.44, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Rect({ x: cx+Ro+8, y: cy-5, width: 22, height: 10, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Ellipse({ x: cx-Ro-6, y: cy, radiusX: 8, radiusY: this._statorRi*0.44, fill: '#607d8b', stroke: '#37474f', strokeWidth: 1.5 }));
    }

    // ── 动态图层 ─────────────────────────────
    _drawMagFieldLayer()    { this._magFieldGroup = new Konva.Group(); this._staticGroup.add(this._magFieldGroup); }
    _drawForceLayer()       { this._forceGroup    = new Konva.Group(); this._staticGroup.add(this._forceGroup); }
    _drawCurrentFlowLayer() { this._flowGroup     = new Konva.Group(); this._staticGroup.add(this._flowGroup); }
    ...
    _drawSlipRingAssembly() {
        ...
        this._staticGroup.add(new Konva.Text({ x: sx+2, y: sy+2, width: sw-4, text: '集电环装置', fontSize: 8.5, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        // 转轴（中心横线）
        this._staticGroup.add(new Konva.Rect({ x: sx+4, y: cy2-4, width: sw-8, height: 8, fill: '#78909c', stroke: '#546e7a', strokeWidth: 1, cornerRadius: 2 }));
        ...
        ringPositions.forEach((ry, i) => {
            ...
            this._staticGroup.add(ringGroup);
            ...
            // 引线到外接电阻
            this._staticGroup.add(new Konva.Line({ points: [sx+sw-2, ringY, sx+sw+2, ringY], stroke: brushCol, strokeWidth: 2 }));
            ...
            // 标注
            this._staticGroup.add(new Konva.Text({ x: sx+4, y: ringY-12, text: ringLabels[i], fontSize: 9, fontStyle: 'bold', fill: brushCol }));
            this._staticGroup.add(brushLeft, brushRight, sparkDot, sparkDot2);
        });
        ...
        this._staticGroup.add(bg, titleBg, this._srRotArrow);
    }
    ...
    _drawExternalResistor() {
        ...
        this._staticGroup.add(new Konva.Text({ x: rx+2, y: ry+2, width: rw-4, text: '外接转子电阻调节', fontSize: 8.5, fontStyle: 'bold', fill: '#ffa726', align: 'center' }));
        ...
        phColors.forEach((col, i) => {
            ...
            // 电阻体（横条）
            this._staticGroup.add(new Konva.Text({ x: rx+8, y: rhY-1, text: phNames[i], fontSize: 7.5, fill: col }));
            ...
            this._staticGroup.add(rBar, rFill, slider, valTxt);
        });

        // 竖向滑块（联动三相）
        const sliderPanelY = ry + rh - 58;
        this._staticGroup.add(new Konva.Rect({ x: rx+6, y: sliderPanelY, width: rw-12, height: 14, fill: '#0d2030', cornerRadius: 2 }));
        ...
        this._interactGroup.add(bg, titleBg, this._rextMainBar, this._rextMainSlider, this._rextLbl, this._sMArcLbl, this._rextStatus, mmDown, shortBtn, shortLbl, maxBtn, maxLbl);
    }
    ...
    _drawTorqueSpeedCurves() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2+4, y: cy2+2, width: cw-8, text: 'T-n 特性曲线族（R_ext 变化，最大转矩不变）', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        [0, this.ratedTorque, this.maxTorque].forEach(T => {
            ...
            this._staticGroup.add(new Konva.Text({ x: ox-24, y: ty-4, width: 22, text: Math.round(T)+'', fontSize: 6, fill: '#37474f', align: 'right' }));
        });
        ...
        rextValues.forEach((Rx, ci) => {
            ...
            if (pts.length > 2) {
                this._staticGroup.add(new Konva.Line({ points: pts, stroke: curveCols[ci], strokeWidth: ci===0?2:1.2, lineJoin: 'round', opacity: ci===0?0.85:0.4 }));
            }
            // 标注
            if (ci > 0 && pts.length >= 2) {
                this._staticGroup.add(new Konva.Text({ x: pts[0]+2, y: pts[1]-9, text: `R${ci}`, fontSize: 6.5, fill: curveCols[ci], opacity: 0.55 }));
            }
        });
        // T_max 水平虚线（最大转矩不变）
        const tmY = oy - (this.maxTorque/tMax)*(ah-4);
        this._staticGroup.add(new Konva.Line({ points: [ox, tmY, ox+aw-2, tmY], stroke: 'rgba(255,167,38,0.35)', strokeWidth: 1, dash: [5,3] }));
        this._staticGroup.add(new Konva.Text({ x: ox+4, y: tmY-10, text: 'T_max（不变）', fontSize: 7, fill: 'rgba(255,167,38,0.6)' }));
        // 额定点
        const nRx = ox+(this.ratedSpeed/nMax)*(aw-2), tRy = oy-(this.ratedTorque/tMax)*(ah-4);
        this._staticGroup.add(new Konva.Circle({ x: nRx, y: tRy, radius: 4, fill: '#66bb6a', stroke: '#2e7d32', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: nRx+4, y: tRy-10, text: 'N', fontSize: 7, fill: '#66bb6a' }));
        ...
        this._staticGroup.add(bg, titleBg, this._activeCurveLine, this._workPoint, this._workHLine, this._workVLine);
    }
    ...
    _drawLCDPanel() {
        ...
        this._staticGroup.add(new Konva.Text({ x: lx+4, y: ly+2, width: lw-8, text: '运行参数监控', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));
        ...
        params.forEach(({ label, id, unit, color }, i) => {
            ...
            this._staticGroup.add(new Konva.Rect({ x: cx2, y: cy2, width: cellW, height: cellH, fill: '#0d1520', cornerRadius: 2 }));
            this._staticGroup.add(new Konva.Text({ x: cx2+3, y: cy2+3, text: label, fontSize: 7, fill: '#37474f' }));
            ...
            this._staticGroup.add(new Konva.Text({ x: cx2+3, y: cy2+cellH-10, width: cellW-6, text: unit, fontSize: 7, fill: '#263238', align: 'center' }));
    ...
    _drawWaveform() {
        ...
        this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+2, width: ww-8, text: 'Iu  Iv  Iw 定子三相电流 ── n(rpm)转速 ── R_ext转子电阻', fontSize: 8, fontStyle: 'bold', fill: '#80cbc4', align: 'center' }));

        const h5 = (wh-14)/5;
        this._wavMids = Array.from({length:5}, (_,i) => wy+14+h5*(i+0.5));
        this._wavMids.forEach(my => {
            this._staticGroup.add(new Konva.Line({ points: [wx+2,my,wx+ww-2,my], stroke: 'rgba(200,200,200,0.07)', strokeWidth: 0.5, dash: [4,3] }));
        });
        ...
        lbls.forEach((l, i) => this._staticGroup.add(new Konva.Text({ x: wx+4, y: wy+14+h5*i+4, text: l, fontSize: 8, fill: colorsW[i] })));
        ...
        this._staticGroup.add(bg, titleBg, this._wLineIu, this._wLineIv, this._wLineIw, this._wLineN, this._wLineRext, this._wNLbl, this._wRextLbl);
        this._wavH5 = h5;
    }
