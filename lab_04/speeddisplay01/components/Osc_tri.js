...
export class Oscilloscope_tri extends BaseComponent {
    ...
    initVisuals() {
        ...
        // 为每个通道创建 Konva.Line 对象（后续通过 points 更新波形）
        this.channels.forEach(ch => { ch.line = new Konva.Line({ stroke: ch.color, strokeWidth: 2, lineJoin: 'round', tension: 0.1 }); });
        ...
        this._interactGroup.add(body, screenRect, this.title, this.gridGroup, this.statusText, tBtn.group, cBtn.group, trigBtn.group);
        // 通道档位按钮（每通道一个，切换 vScales）
        ...
        this.channels.forEach(ch => this._staticGroup.add(ch.line));
        this.updateStatus();
    }
