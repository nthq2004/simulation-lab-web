...
export class ProcessCalibrator extends BaseComponent {
    ...
    _buildLCD(cx, s, W) {
        ...
        this._staticGroup.add(this.lcdBg);
    ...
    _buildButtons(cx, s, W, H) {
        ...
        const addBtn = (x, y, w, h, label, fill, tc, action, fontSize2line, customDraw) => {
            ...
            rect.on('mousedown touchstart', (e) => {
                ...
                // 按钮在 _interactGroup（未缓存），变色只需触发重绘，无需缓存重建
        ...
        bigRect.on('mousedown touchstart', (e) => {
            ...
            // bigRect 在 _interactGroup（未缓存），变色只需触发重绘
        ...
        arrowBtns.forEach(a => {
            ...
            arrowRect.on('mousedown touchstart', (e) => {
                ...
                // 箭头在 _interactGroup（未缓存），变色只需触发重绘
                arrowTxt.fill('#ffcc44');
                if (this.sys && typeof this.sys.requestRedraw === 'function') this.sys.requestRedraw();
                setTimeout(() => {
                    arrowTxt.fill('#ffffff');
                    if (this.sys && typeof this.sys.requestRedraw === 'function') this.sys.requestRedraw();
                }, 50);
            });
    ...
    _handleButtonAction(action) {
        switch (action) {
            ...
            case 'POWER':
                ...
                if (!this.isPowered) {
                    ...
                    this._refreshCache();  // lcdBg 在 _staticGroup 中，需刷新缓存
                }
                break;
        ...
        // 注意：按钮视觉反馈（亮/暗）在 addBtn 中仅调用 requestRedraw，因为按钮在 _interactGroup（未缓存）
        // POWER/BACKLIGHT 等涉及 _staticGroup 的操作已在各自分支内部调用 _refreshCache()
    }
