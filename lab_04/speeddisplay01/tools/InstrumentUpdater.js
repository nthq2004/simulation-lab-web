...
export class InstrumentUpdater {
    ...
    detectFrequency(dev, voltage, nowMs) {
        ...
        // ── 电平判断（含 5% 迟滞，防止噪声引发误触发）─────────────────
        //   高电平门限：> 2.5 + 0.05 = 2.55V  → 确认为高
        //   低电平门限：< 2.5 - 0.05 = 2.45V  → 确认为低
        //   中间区域：保持上次电平（Schmitt Trigger 效果）
        const HIGH_TH = 2.5 + 0.05;
        const LOW_TH = 2.5 - 0.05;
        ...
        // ── 超时检测：长时间无上升沿 → 频率归零 ──────────────────────
        if (st.lastRiseAt >= 0 && (nowMs - st.lastRiseAt) > 2000) {
            st.frequency = 0;
    ...
    _updateCalibrator(dev) {
        ...
        // 仪表隐藏时不更新显示，减少主循环额外开销
        if (dev.group && !dev.group.visible()) return;
