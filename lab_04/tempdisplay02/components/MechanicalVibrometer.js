...
export class MechanicalVibrometer extends BaseComponent {
    ...
    _drawLeverArm() {
        ...
        this._dynamicGroup.add(new Konva.Line({
            ...
            shadowOffsetY: 1, shadowOpacity: 0.3,
            listening: false,
        }));
    ...
    _drawMassBlock() {
        ...
        this._dynamicGroup.add(new Konva.Rect({
            ...
            stroke: '#504018', strokeWidth: 1.2,
            cornerRadius: 3,
            shadowColor: '#000', shadowBlur: 6,
            shadowOpacity: 0.4, shadowOffsetY: 2,
        }));
    ...
    tick(dt) {
        ...
        // 底座加速度激励 ÿ_base = -A·wf²·sin(wf·t)  (mm/s²)
        const baseAcc = -A * wf * wf * Math.sin(wf * t);

        // 方程 ẍ + 2ζωn·ẋ + ωn²·x = -ÿ_base
        const acc = -baseAcc - 2 * z * wn * this._massVel - wn * wn * this._massDisp;

        // 积分
        this._massVel  += acc * dt;
        this._massDisp += this._massVel * dt;
