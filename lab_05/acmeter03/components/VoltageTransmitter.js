...
/**
 * VoltageTransmitter — 电压变送器（两线制）组件
 *
 * 说明：
 * - 模拟常见的 4-20mA 两线制电压/电参数变送器外观与显示逻辑；
 * - 将输入电压映射并显示在 LCD 上，支持零点/量程微调与开路故障显示；
 * - 使用 Konva 绘制外壳与旋钮，静态元素加入 `_staticGroup` 以启用缓存。
 */
export class VoltageTransmitter extends BaseComponent {
    ...
    _drawEnclosure() {
        ...
        const labelText = new Konva.Text({ x: 22, y: -6, width: this.w, text: '通用变送器', fontSize: 14, align: 'center', fill: '#2c3e50', fontStyle: 'bold' });
    ...
    _drawLCD() {
        ...
        const unit = new Konva.Text({
            x: centerX - 8, y: this.lcdCenterY + 12,
            text: 'mv', fontSize: 14, fill: '#1a1a1a', opacity: 0
        });
        this.unitText = unit;
    ...
    getConfigFields() {
        return [
            ...
            { label: '电压值', key: 'voltage', type: 'number' },
        ];
    }
