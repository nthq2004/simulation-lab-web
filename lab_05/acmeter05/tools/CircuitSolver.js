...
export class CircuitSolver {
    ...
    _solve() {
        ...
        // ---总的方程数量。
        const totalSize = mSize + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + ctEqCount + ptEqCount;
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // - 23 交流电流表（0V 电压源注入）
            const acAmmVIdx = totalSize - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount;
            DeviceStamps.stampACAmmeters(ctx, G, B, acAmmeterDevs, acAmmVIdx);

            // - 23.2 功率表（0V 电压源注入电流线圈）
            const wattmeterVIdx = totalSize - wattmeterEqCount - ctEqCount - ptEqCount;
