 ...
 *
 * 该类提供所有仿真组件的通用基础：
 *  - 三层视觉分组：`_staticGroup`（静态、可缓存）、`_dynamicGroup`（动态）、`_interactGroup`（交互）
 *  - 端口管理（`addPort`）用于创建 wire/pipe 类型的连接点
 *  - Konva 缓存刷新控制（`_refreshIfDirty` / `_forceCacheFlush`）以减少重绘开销
 *  - 通用 UI：配置对话、右键菜单、高亮效果、显示/隐藏
 *
 * 设计原则：尽量将不常变的视觉元素放入静态组并缓存，提高性能；
 * 动态元素放入动态组按需重建；交互元素放入交互组以便响应事件。
 */
export class BaseComponent {
    constructor(config, sys) {
        ...
        // 根据 config.scale 初始化缩放
        if (this.scale !== 1) {
            this.group.scale({ x: this.scale, y: this.scale });
        }

        // ── 脏标记优化 ──
        // _cacheDirty: true 时 _refreshIfDirty() 才实际刷新 Konva cache
        // 避免 tick() 中每帧对稳态组件做无意义的 clearCache + cache()
        this._cacheDirty = true; // 首帧需要刷新
    }

    /**
     * 标记下次 _refreshIfDirty() 需要实际刷新 Konva cache
     * 组件在 tick() 中视觉状态变化时调用此方法
     */
    markDirty() {
    ...
    /**
     * tick() 末尾调用 — 仅当 markDirty() 被调用过才执行实际的 Konva cache 刷新
     * 替代在 tick() 中直接调用 _refreshCache()（后者总是刷新）
     */
    _refreshIfDirty() {
    ...
    // 添加端口：在组件上创建可交互的连接点（wire 或 pipe）
    // 参数：x,y - 相对坐标；id - 端口短 id（如 'p','n','u'）；
    // type - 'wire'（电气）或 'pipe'（气/液管）；polarity - 可选的极性/方向标识；opacity - 显示透明度
    ...
    // 返回端口在舞台上的绝对坐标（用于连线绘制）
    // 支持端口使用 Konva 节点（node.getAbsolutePosition）或基于 group 变换手动计算
    ...
    // 在页面中心弹出配置对话框，依据 getConfigFields() 构建表单
    // 保存时调用 onConfigUpdate(newConfig)
    showConfigDialog() {
    ...
    // 返回配置字段定义（编辑器 / 对话框使用），子类可覆盖以提供更多字段
    getConfigFields() {
        return [
    ...
    // 当配置对话框保存后回调，默认仅同步 id，子类可覆盖实现更复杂的同步
    onConfigUpdate(newConfig) {
        console.log('\u914D\u7F6E\u5DF2\u66F4\u65B0:', newConfig);
        this.id = newConfig.id;
    }

    // 在指定屏幕坐标处显示组件右键菜单（旋转 / 配置等项）
    showContextMenu(evt) {
    ...
    // 旋转组件（增量角度），并同步 config.rotation，通知系统更新连线与重绘
    rotate(deltaDeg) {
    ...
    // 高亮效果：在组件底部添加一层发光矩形并进行呼吸动画
    // active: 开/关；color: 发光颜色
