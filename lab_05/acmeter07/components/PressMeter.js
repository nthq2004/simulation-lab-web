 ...
 *
 * 功能概述：
 * - 在画布上绘制一个模拟压力表（机械表盘样式），包含刻度、指针、LCD 数显与管口；
 * - 支持自定义量程（`min` / `max`，单位 MPa），并将输入值线性映射到表盘角度；
 * - 提供 `update(inP)` 方法供仿真器/求解器调用以更新显示；
 * - 提供配置接口 `getConfigFields()` 与 `onConfigUpdate()` 以供编辑器修改属性。
 *
 * 设计说明：
 * - 视觉元素使用 Konva 构建并添加到 `_staticGroup`，通过 `this._refreshCache()` 刷新缓存；
 * - 指针直接设置 `rotation` 属性以获得即时响应（在高频帧率下较为高效）；
 */
export class PressMeter extends BaseComponent {
    /**
     * 构造器：初始化参数与视觉容器
     * @param {Object} config - 场景配置（支持 radius, min, max, title 等）
    ...
    /**
     * 构建视觉元素的入口函数
     * - 在 `this._staticGroup` 下创建 `viewGroup`，并按层级绘制外壳、刻度、指针、LCD 等
     */
    initVisuals() {
        // 创建视觉容器，中心点对齐
        ...
        // 按层级绘制子元素（方法按职责分离，便于维护）
        this._drawShell();   // 表壳与光泽
        this._drawPipe();    // 下方的进气接头
        this._drawZones();   // 颜色区间（例如正常/危险区）
        this._drawTicks();   // 刻度与数字
        this._drawPointer(); // 指针（旋转表示读数）
        this._drawCenter();  // 指针轴心
        this._drawLcd();     // 中央小 LCD 数显
        this._drawName();    // 名称文字
    }

    /**
     * 将物理量（MPa）线性映射为表盘角度（度）
     * @param {number} value - 输入压力值（MPa）
     * @returns {number} 对应的角度（度，适用于 Konva rotation）
     */
    valueToAngle(value) {
     ...
     */
    update(inP = 0) {
        // 1) 数据限幅：允许短暂超量程显示（最多到 max * 1.1）以提示溢出
