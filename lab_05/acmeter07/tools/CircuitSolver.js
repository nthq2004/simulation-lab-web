...
export class CircuitSolver {
    ...
    _solve() {
        ...
        // ---总的方程数量。
        const totalSize = mSize + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount;
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // - 23 交流电流表（0V 电压源注入）
            const acAmmVIdx = totalSize - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount;
            DeviceStamps.stampACAmmeters(ctx, G, B, acAmmeterDevs, acAmmVIdx);

            // - 23.2 功率表（0V 电压源注入电流线圈）
            const wattmeterVIdx = totalSize - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount;
             ...
             */
            const ctVIdx = totalSize - ctEqCount - ptEqCount - ammEqCount;
            DeviceStamps.stampCurrentTransformers(ctx, G, B, ctDevs, ctVIdx);

            // ─ 23.7 电压互感器（PT）受控电压源注入 ────────────────────
            const potVIdx = totalSize - ptEqCount - ammEqCount;
