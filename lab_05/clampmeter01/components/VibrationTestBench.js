...
export class VibrationTestBench extends BaseComponent {
    ...
    _drawMeterZoneShell() {
        ...
        // 标签
        const labels = ['频率 FREQ', '幅值 AMP'];
        [this._meterFreq, this._meterAmp].forEach((m, i) => {
            this._staticGroup.add(new Konva.Text({
                x: m.cx - m.r, y: m.cy + m.r * 0.75,
                width: m.r * 2,
                text: labels[i],
                fontSize: Math.min(10, this.width * 0.0115),
                fontFamily: 'Courier New', fill: '#f90404', align: 'center',
            }));
        });
    }
    ...
    _drawNeedles() {
        ...
        { key: 'amp', max: 500, meter: this._meterAmp }].forEach(({ key, max, meter: m }) => {
            ...
            const readout = new Konva.Text({
                ...
                width: m.r * 1.10, text: '',
                fontSize: Math.max(16, this.width * 0.015),
    ...
    _updateAnimShapes() {
        ...
        if (this._statusNodes) {
            const statusValues = {
                ...
                curAmp: this._running ? `${this.amplitude.toFixed(1)} μm` : '--- μm',
    ...
    _drawStatusLEDs() {
        ...
        leds.forEach((led, i) => {
            ...
            this._dynamicGroup.add(new Konva.Text({
                x: lx - W * 0.027, y: ledY + ledR + 1,
                width: W * 0.060,
                text: led.label,
                fontSize: Math.max(11, W * 0.009),
                fontFamily: 'Arial',
    ...
    _drawMeterMountHint(tableY, tableW, tableH, tableCx) {
        ...
        this._dynamicGroup.add(new Konva.Rect({
            ...
            fill: this._running ? 'rgba(30,70,100,0.20)' : 'rgba(209, 214, 218, 0.12)',
            stroke: '#3a68a0',
            strokeWidth: 1.5,
            dash: [10, 6],
            cornerRadius: 8,
