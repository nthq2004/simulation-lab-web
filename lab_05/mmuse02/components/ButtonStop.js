...
export class NormallyClosedPushButton extends BaseComponent {
    ...
    _drawContactMechanism() {
        ...
        // 固定触头（COM，左侧）
        const comX = cx2 - 20;
        this._staticGroup.add(new Konva.Rect({ x: comX - colW/2, y: cy2 + 4, width: colW, height: 8, fill: '#c0a020', stroke: '#8a7010', strokeWidth: 0.8, cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [comX, cy2+12, comX, cy2+ch-4], stroke: '#c0a020', strokeWidth: 2.5, lineCap: 'round' }));

        // 固定触头（NC，右侧）
        const ncX  = cx2 + 20;
        this._staticGroup.add(new Konva.Rect({ x: ncX - colW/2, y: cy2 + 4, width: colW, height: 8, fill: '#c0a020', stroke: '#8a7010', strokeWidth: 0.8, cornerRadius: 1 }));
        this._staticGroup.add(new Konva.Line({ points: [ncX, cy2+12, ncX, cy2+ch-4], stroke: '#c0a020', strokeWidth: 2.5, lineCap: 'round' }));
        ...
        this._staticGroup.add(this._contactBridge, this._springGroup, this._dotCom, this._dotNc);
    ...
    _drawIndicatorLed() {
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx2-10, y: cy2+7, width: 20, text: 'NC', fontSize: 7, fill: '#546e7a', align: 'center' }));
    ...
    _drawTerminals() {
        ...
        this._staticGroup.add(tNcBlock);
        this._staticGroup.add(new Konva.Line({ points: [ncX+4, this._contactY + this._contactH - 16, ncX+4, ty+4], stroke: '#c0a020', strokeWidth: 1.5, dash: [3,2] }));
