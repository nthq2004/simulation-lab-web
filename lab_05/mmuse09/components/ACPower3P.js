...
export class ACPower3P extends BaseComponent {
    constructor(config, sys) {
        ...
        this.vRms = config.vRms || 220;        // 相电压有效值（V），默认 220V
        this.freq = config.freq || 50;         // 工作频率（固定 50Hz）
        this.rOn = 0.1;         // 内阻（仿真用）
    ...
    getPhaseVoltage(phase, time) {
        ...
        const peak = this.vRms * Math.sqrt(2);  // 峰值 = RMS * sqrt(2)
        const omega = 2 * Math.PI * this.freq;   // 角频率
