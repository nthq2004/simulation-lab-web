...
export class RealControlTransformer extends BaseComponent {
    constructor(config, sys) {
        ...
        this._primaryResistance   = config.primaryResistance   || 0.2;
        ...
        const Xm = Vp / (0.005 * Ip);
        this._magnetizingInductance = Xm / (2 * Math.PI * f);
    ...
    onConfigUpdate(cfg) {
        ...
        if (cfg.primaryResistance !== undefined) this._primaryResistance   = parseFloat(cfg.primaryResistance) || 0.2;
    ...
    tick(dt) {
        ...
        if (sv) {
            ...
            const dt = sv.deltaTime || 0.1e-3;
            const Rc = Math.max(0.001, this._coreResistance || 50000);
            const LmVal = Math.max(0.001, this._magnetizingInductance);
            const GmEq = 1 / (Rc + LmVal / dt);

            if (this.physCurrent !== undefined) {
                this.I_primary = this.physCurrent;
            }

            const R1e = this._primaryResistance + this._primaryLeakage / dt;
            const vMag = this.V_primary - this.I_primary * R1e + (this._primaryLeakage / dt) * this._iL1Prev;

            const iLmNow = GmEq * vMag + GmEq * (LmVal / dt) * this._iLmPrev;
            this._iLmPrev = iLmNow;

            const R2e = this._secondaryResistance + this._secondaryLeakage / dt;
            this.I_secondary = (this.V_secondary - vMag / Math.max(0.001, this._turnsRatio)
                + (this._secondaryLeakage / dt) * this._iL2Prev) / R2e;

            this._iL1Prev = this.I_primary;
            this._iL2Prev = this.I_secondary;
        }
