...
export class CircuitSolver {
    ...
    _solve() {
        ...
        // ── 控制变压器方程数（每台 1 个内部节点 + 1 个 VCVS 方程）──
        ...
        const totalSize = nodeVarCount + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount + ctVcvsEqCount;
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // - 23 交流电流表（0V 电压源注入）
            const acAmmVIdx = totalSize - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount;
            DeviceStamps.stampACAmmeters(ctx, G, B, acAmmeterDevs, acAmmVIdx);

            // - 23.2 功率表（0V 电压源注入电流线圈）
            const wattmeterVIdx = totalSize - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount;
             ...
             */
            const ctVIdx = totalSize - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount;
            DeviceStamps.stampCurrentTransformers(ctx, G, B, ctDevs, ctVIdx);

            // ─ 23.7 电压互感器（PT）受控电压源注入 ────────────────────
            const potVIdx = totalSize - ptEqCount - ammEqCount - ctVcvsEqCount;
            DeviceStamps.stampPotentialTransformers(ctx, G, B, potentialTransformerDevs, potVIdx);

            // ─ 23.75 控制变压器（控制变压器）内部节点 + VCVS ──────────
            const ctCtrlVIdx = totalSize - ctVcvsEqCount;
            const ctCtrlIntNodeIdx = mSize;
            DeviceStamps.stampControlTransformers(ctx, G, B, ctCtrlDevs, ctCtrlVIdx, ctCtrlIntNodeIdx);

            // - 24 MF47 万用表等效注入
            DeviceStamps.stampMF47(ctx, G, B, mf47Devs);

            // ─ 25. 数字万用表（MA 档分流电阻；RES 档恒流/恒压迭代切换）──
            const mmResVIdx = totalSize - mmResEqCount - ctVcvsEqCount;
            DeviceStamps.stampMultimeters(ctx, G, B, multimeterDevs, results, mmResVIdx);
            // ─ 26. 普通电流表（0V 电压源注入 p-n 之间）──
            const ammVIdx = totalSize - ammEqCount - ctVcvsEqCount;
    ...
    _updateDeviceCurrents(devices, results) {
        ...
        ctCtrlDevs.forEach(dev => {
            if (dev._vcvsIdx !== undefined) {
                const i2 = results[dev._vcvsIdx];
                dev.I_secondary = -(i2 || 0);
                const vP1 = this.nodeVoltages.get(this.portToCluster.get(`${dev.id}_wire_p1`)) || 0;
                const vA = (dev._intNodeIdx !== undefined ? results[dev._intNodeIdx] : undefined) || 0;
                const R1 = Math.max(0.001, dev._primaryResistance || 0.2);
                const L1 = Math.max(0, dev._primaryLeakage || 0.01);
                const dt = this.deltaTime || 0.1e-3;
                const G1 = 1 / (R1 + L1 / dt);
                const iL1 = dev._iL1Prev || 0;
                dev.I_primary = G1 * (vP1 - vA) + G1 * (L1 / dt) * iL1;
                dev.physCurrent = dev.I_primary;
            }
        });
