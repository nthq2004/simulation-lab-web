...
export const DeviceStamps = {
     ...
     */
    stampControlTransformers(ctx, G, B, ctDevs, vcvsStart, intStart) {
        let vrow = vcvsStart;
        let intIdx = intStart;

        ctDevs.forEach(dev => {
            const c_p1 = ctx.portToCluster.get(`${dev.id}_wire_p1`);
            const c_p2 = ctx.portToCluster.get(`${dev.id}_wire_p2`);
            const c_s1 = ctx.portToCluster.get(`${dev.id}_wire_s1`);
            const c_s2 = ctx.portToCluster.get(`${dev.id}_wire_s2`);

            if (c_p1 === undefined || c_p2 === undefined || c_s1 === undefined || c_s2 === undefined) {
                G[vrow][vrow] = 1;
                dev._vcvsIdx = vrow++;
                intIdx++;
                return;
            ...
            const R1  = Math.max(0.001, dev._primaryResistance   || 0.2);
            const R2  = Math.max(0.001, dev._secondaryResistance || 0.3);
            const L1  = Math.max(0,     dev._primaryLeakage   || 0.01);
            const L2  = Math.max(0,     dev._secondaryLeakage || 0.001);
            ...
            const G1  = 1 / (R1 + L1 / dt);
            const GmEq = 1 / (Rc + Lm / dt);
            const iL1 = dev._iL1Prev || 0;
            const iL2 = dev._iL2Prev || 0;
            const iLm = dev._iLmPrev || 0;
            ...
            const isGnd = (c) => ctx.gndClusterIndices.has(c);
            const vKnown = (c) => ctx.vPosMap.get(c);

            const getM = (c) => {
                if (c === undefined) return { type: 'none' };
                if (isGnd(c)) return { type: 'gnd' };
                const vk = vKnown(c);
                if (vk !== undefined) return { type: 'known', v: vk };
                const m = ctx.nodeMap.get(c);
                return { type: 'node', m };
            };

            const p1 = getM(c_p1), p2 = getM(c_p2), s1 = getM(c_s1), s2 = getM(c_s2);

            const gAdd = (r, c, val) => { if (r !== undefined && c !== undefined && r >= 0 && c >= 0) G[r][c] += val; };
            const bAdd = (r, val) => { if (r !== undefined && r >= 0) B[r] += val; };

            // ═══ 1. R1 between p1 and V_a ═══
            if (p1.type === 'node') {
                gAdd(p1.m, p1.m, G1);
                gAdd(p1.m, m_a, -G1);
                gAdd(m_a, p1.m, -G1);
                ...
                gAdd(m_a, m_a, G1);
            } else if (p1.type === 'known') {
                gAdd(m_a, m_a, G1);
                bAdd(m_a, G1 * p1.v);
            }

            // L1 leakage current source: I = G1 * (L1/dt) * iL1 from p1 to V_a
            if (L1 > 0 && p1.type === 'node') {
                const iL1src = G1 * (L1 / dt) * iL1;
                bAdd(p1.m, -iL1src);
                bAdd(m_a, iL1src);
            }

            // ═══ 2. Rc + Lm companion between V_a and p2 ═══
            if (p2.type === 'node') {
                gAdd(m_a, m_a, GmEq);
                gAdd(m_a, p2.m, -GmEq);
                gAdd(p2.m, m_a, -GmEq);
                ...
                gAdd(m_a, m_a, GmEq);
            } else if (p2.type === 'known') {
                gAdd(m_a, m_a, GmEq);
                bAdd(m_a, GmEq * p2.v);
            }

            // Rc+Lm current source: I_hist = GmEq * (Lm/dt) * iLmPrev from V_a to p2
            const iLmHist = GmEq * (Lm / dt) * iLm;
            bAdd(m_a, -iLmHist);
            if (p2.type === 'node') bAdd(p2.m, iLmHist);

            // ═══ 3. CCCS: -I_VCVS/N from V_a to p2 (MNA: I_VCVS < 0 when loaded) ═══
            const invN = 1 / N;
            gAdd(m_a, vCol, -invN);
            if (p2.type === 'node') gAdd(p2.m, vCol, invN);

            // ═══ 4. VCVS + R2 + L2 equation: V_s1 - V_s2 - I*(R2+L2/dt) - V_a/N + V_p2/N + (L2/dt)*iL2 = 0 ═══
            if (s1.type === 'node') {
                gAdd(vrow, s1.m, 1);
                gAdd(s1.m, vCol, 1);
            } else if (s1.type === 'known') {
                bAdd(vrow, -s1.v);
            }
            // s1 known as node: column contribution is handled above
            // s1 known voltage: B[vrow] -= V_s1 (brought to RHS)

            if (s2.type === 'node') {
                gAdd(vrow, s2.m, -1);
                gAdd(s2.m, vCol, -1);
            } else if (s2.type === 'known') {
                bAdd(vrow, s2.v);
            }

            gAdd(vrow, vCol, -(R2 + L2 / dt));
            if (L2 > 0) bAdd(vrow, -(L2 / dt) * iL2);
            gAdd(vrow, m_a, -invN);
            ...
            // dummy diagonal for safety
            gAdd(vrow, vrow, 1e-12);

            // ═══ 5. Additional noise sources to aid convergence ═══
            gAdd(m_a, m_a, 1e-12);
