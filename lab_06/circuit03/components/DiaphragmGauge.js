...
export class DiaphragmGauge extends BaseComponent {
    constructor(config, sys) {
        ...
        // 膜盒内腔：上下各留壁厚
        this._wallT = this._capsuleH * 0.20;        // 上下盖壁厚
    ...
    _drawMechanismLayer() {
        ...
        this._staticGroup.add(this._mechGroup);
    ...
    _calcGeometry(frac) {
        ...
        return {
            defl, topMemY, botMemY, rodTopY, linkTopX, linkTopY,
    ...
    _drawComponentLabels() {
        ...
        const items = [
            { text: '膜片', tx: cx-20, ty: this._memTopY0, lx: 4, ly: this._capsuleY + 8, color: '#2196F3' },
            { text: '推杆', tx: this._memCx, ty: this._memTopY0 + this._rodLen * 0.5, lx: W - 66, ly: this._capsuleY - 8, color: '#4CAF50' },
            { text: '连杆', tx: cx, ty: this._cy + R * 0.8, lx: 4, ly: this._cy + R * 0.02, color: '#FF9800' },
            { text: '扇形齿轮', tx: this._sectorPivotX + 6, ty: this._sectorPivotY, lx: W - 74, ly: this._sectorPivotY - 8, color: '#9C27B0' },
            { text: '指针', tx: cx , ty: cy , lx: W - 82, ly: cy - R * 0.68, color: '#F44336' },
        ];

        items.forEach(item => {
            // 目标标记点
            this._staticGroup.add(new Konva.Circle({
                x: item.tx, y: item.ty, radius: 5,
                fill: item.color, listening: false,
            }));
            // 标签
            const tag = new Konva.Tag({
                fill: item.color, cornerRadius: 3, opacity: 0.02,
            });
            const text = new Konva.Text({
                text: item.text, fontSize: 12, fill: item.color,
            ...
            if (dx * dx + dy * dy > 400) {
                this._staticGroup.add(new Konva.Line({
                    ...
                    stroke: item.color, strokeWidth: 1.6, opacity: 0.35,
                    dash: [6, 8], listening: false,
                }));
            }
        });
        ...
        this._staticGroup.add(new Konva.Text({
            ...
            fontSize: 12, fill: '#14c20a', align: 'center',
        }));
    }
    ...
    _drawStatusIndicator() {
        ...
        this._staticGroup.add(this._readout);
    }
