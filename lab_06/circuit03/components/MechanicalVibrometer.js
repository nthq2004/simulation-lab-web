...
export class MechanicalVibrometer extends BaseComponent {
    ...
    _updateDynamic() {
        ...
        if (this._readoutValText) {
            this._readoutValText.text(Math.abs(this._massDisp*1000).toFixed(2)+ ' μm');
        }
    ...
    _drawShellBack() {
        ...
        this._staticGroup.add(new Konva.Text({
            ...
            text: '机械振动计',
            fontSize: Math.max(12, this.width * 0.042),
            fontFamily: 'Arial',
            fontStyle: 'bold',
            fill: '#8a6a30',
            align: 'center',
    ...
    _drawDialScale() {
        ...
        for (let i = 0; i <= majorSteps; i++) {
            ...
            if (isMajor) {
                ...
                const val     = (valFrac * 2 * this.rangeMax).toFixed(2);
                const txtR    = R * 0.62;
                this._staticGroup.add(new Konva.Text({
                    x: cx + txtR * cos - 14,
                    y: cy + txtR * sin - 7,
                    ...
                    fill: isMid ? 'transparent' : '#222',//如果isMid为true，则填充颜色透明色，否则为#222
        ...
        this._staticGroup.add(new Konva.Text({
            ...
            width: 60,
            text: 'mm',
            fontSize: Math.max(12, this.width * 0.040),
            fontFamily: 'Arial',
            fontStyle: 'bold',
            fill: '#333',
            align: 'center',
    ...
    _drawLabel() {
        this._staticGroup.add(new Konva.Text({
            ...
            y: this._labelPos.y - 15,
            width: 80, height: 14,
            text: this.label,
            fontSize: Math.max(12, this.width * 0.046),
            fontFamily: 'Arial',
            fontStyle: 'bold',
            fill: '#8a6a30',
            align: 'center',
    ...
    _drawReadout() {
        ...
        // 当前读数（实际测量位移）
        const dispMM = Math.abs(this._massDisp*1000).toFixed(2);
        this._dynamicGroup.add(new Konva.Text({
            x: rx + 2, y: ry + rh * 0.08,
            width: rw - 4, height: rh * 0.84,
            text: `${dispMM} μm`,
            fontSize: Math.max(12, W * 0.050),
