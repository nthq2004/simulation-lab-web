...
export class ProcessCalibrator extends BaseComponent {
    ...
    _createUI() {
        ...
        this._staticGroup.add(brandText, modelText, subtitleText);

        // ── LCD 文本层（不缓存，避免每200ms全量重绘） ──────────────────
        this._lcdGroup = new Konva.Group({ name: '_lcdGroup' });
    ...
    _buildButtons(cx, s, W, H) {
        ...
        const addBtn = (x, y, w, h, label, fill, tc, action, fontSize2line, customDraw) => {
            ...
            rect.on('mousedown touchstart', (e) => {
                ...
                Promise.resolve().then(() => {
                    ...
                    setTimeout(() => { rect.fill(fill); this.sys.requestRedraw(); }, 50);
                });
            });
            rect.on('dblclick', e => { e.cancelBubble = true; });
            this._interactGroup.add(rect);
        ...
        bigRect.on('mousedown touchstart', (e) => {
            ...
            Promise.resolve().then(() => {
                ...
                this.sys.requestRedraw();
                setTimeout(() => { bigRect.fill('#1a4e8a'); this.sys.requestRedraw(); }, 50);
            });
        });
        bigRect.on('dblclick', e => { e.cancelBubble = true; });
        this._interactGroup.add(bigRect);
        ...
        this._interactGroup.add(measTxt, bigDiv, srcTxt);
        ...
        arrowBtns.forEach(a => {
            ...
            arrowRect.on('mousedown touchstart', (e) => {
                ...
                Promise.resolve().then(() => {
                    ...
                    this.sys.requestRedraw();
                    setTimeout(() => { arrowTxt.fill('#ffffff'); this.sys.requestRedraw(); }, 50);
                });
            });
            arrowRect.on('dblclick', e => { e.cancelBubble = true; });
            this._interactGroup.add(arrowTxt, arrowRect);
        });
        ...
        this._staticGroup.add(dpadCenter);
    ...
    _buildPressurePort(cx, s, W) {
        ...
        // 管道连接处 - 上方导管
        const tubePipeL = new Konva.Rect({
            x: cx - 35 * s, y: -36 * s,
        ...
        const tubePipeR = new Konva.Rect({
            x: cx + 25 * s, y: -36 * s,
            width: 10 * s, height: 20 * s,
            fill: '#888',
            stroke: '#444',
        ...
        this._staticGroup.add(tubePipeL, tubePipeR);

        // 内部孔 - 红色(输入)或蓝色(输出)
        const pressurePortL = new Konva.Circle({
            x: cx - 35, y: portY - 2 * s,
            radius: portRadius,
        ...
        const pressurePortR = new Konva.Circle({
            x: cx + 35, y: portY - 2 * s,
            radius: portRadius,
            fill: '#090df4',  // 默认红色（输入模式）
            stroke: '#000',
            strokeWidth: 1.5 * s
        });
        this._staticGroup.add(pressurePortL, pressurePortR);
    ...
    _buildJackLabels(s, W, H) {
        ...
        this._staticGroup.add(srcAreaLabel, measAreaLabel);
        ...
        ports.forEach(p => {
            ...
            this._staticGroup.add(jack, inner, lbl);
        });
    ...
    _addPorts() {
        ...
        // 顶部气路接口（模拟压力模块）
        const pressurePortY = -40 * s;  // 品牌条下方
        this.addPort(W / 2 - 30 * s, pressurePortY, 'i', 'pipe', 'in');
        this.addPort(W / 2 + 30 * s, pressurePortY, 'o', 'pipe', 'out');
    }
    ...
    _getSourceRange() {
        switch (this.sourceMode) {
            ...
            case 'SRC_PRESSURE': return { lo: 0.0, hi: 1000.0 }; // kPa
        ...
        if (elapsed < this._displayThrottle) {
            ...
            if (!this._pendingTimer) {
                ...
                this._pendingTimer = setTimeout(() => {
                    ...
                    this._pendingValue1 = null;
                    this._pendingValue2 = null;
                    if (this.isPowered) this._refreshDisplay();
                }, wait);
            }
            return;
        }
    ...
    tick(dt) {
        ...
        if (this.activePanel === 'MEASURE') {
            if (this.measureMode === 'MEAS_MA' && cSCom !== undefined && cSMa !== undefined) {
                const diffV = (s.nodeVoltages.get(cSMa) || 0) - (s.nodeVoltages.get(cSCom) || 0);
                downValue = diffV * 4;
            } else if (this.measureMode === 'MEAS_LOOP' && cSCom !== undefined && cSMa !== undefined) {
                const vSCom = s.nodeVoltages.get(cSCom) || 0;
                downValue = vSCom * 4;
            } else if (this.measureMode === 'MEAS_V' && cSV !== undefined && cSCom !== undefined) {
                downValue = (s.nodeVoltages.get(cSV) || 0) - (s.nodeVoltages.get(cSCom) || 0);
            } else if (this.measureMode === 'MEAS_RES' && cSV !== undefined && cSCom !== undefined) {
                downValue = s._getEquivalentResistance(s.clusters[cSV], s.clusters[cSCom], s.clusters);
            } else if (this.measureMode === 'MEAS_TC' && cSTc !== undefined && cSCom !== undefined) {
                const tcValue = (s.nodeVoltages.get(cSTc) || 0) - (s.nodeVoltages.get(cSCom) || 0);
                downValue = tcValue / 0.000041;
            } else if (this.measureMode === 'MEAS_RTD' && cSV !== undefined && cSCom !== undefined) {
                const ptValue = s._getEquivalentResistance(s.clusters[cSV], s.clusters[cSCom], s.clusters);
                downValue = (ptValue - 100) / 0.3851;
            } else if (this.measureMode === 'MEAS_HZ' && cSV !== undefined && cSCom !== undefined) {
                const voltage = (s.nodeVoltages.get(cSV) || 0) - (s.nodeVoltages.get(cSCom) || 0);
                downValue = this._detectFrequency(voltage, s.currentTime);
            }
        }
        if (this.measureMode !== 'MEAS_HZ') this._resetHzState();
        if (this.upMode === 'MEAS_PRESSURE') upValue = this.upPressureValue || 0;
        if (this.activePanel === 'MEASURE' && this.measureMode === 'MEAS_PRESSURE') downValue = this.downPressureValue || 0;
        this.update(upValue, downValue);
    }

    _ensureHzState() {
        if (!this._hzState) {
            this._hzState = { lastLevel: false, lastRiseAt: -1, frequency: 0 };
        }
    }
    ...
    _detectFrequency(voltage, nowMs) {
        this._ensureHzState();
        const st = this._hzState;
        const HIGH_TH = 0.2, LOW_TH = 0.0;
        let currentLevel = st.lastLevel;
        ...
        if (isRisingEdge) {
            if (st.lastRiseAt >= 0) {
                ...
                if (periodMs > 0) {
                    const alpha = 0.9;
                    const newFreq = 1 / periodMs;
                    st.frequency = st.frequency > 0 ? alpha * newFreq + (1 - alpha) * st.frequency : newFreq;
                }
            }
            st.lastRiseAt = nowMs;
        }
