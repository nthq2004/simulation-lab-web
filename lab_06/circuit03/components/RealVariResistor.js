...
/**
 * RealVariResistor — 真实风格可变电阻组件（带角度/滑动模型）
 *
 * 说明：
 * - 提供带指针/轨道的可变电阻物理化视觉表示，支持旋转角度映射到阻值；
 * - 内部维护 `totalResistance`、`currentResistance` 与角度 `currentDeg`，并提供 `updateResistors()` 更新阻值；
 * - 静态视觉可缓存，端口按固定排布暴露。
 */
export class RealVariResistor extends BaseComponent {
    ...
    initVisuals() {
        ...
        // 组合所有元素
        this._interactGroup.add(this.title,internalLeadA, internalLeadC, this.track, this.fTrack, this.knobGroup, internalLeadB, cPort);
    }
