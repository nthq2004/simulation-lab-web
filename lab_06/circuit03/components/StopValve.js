...
/**
 * StopValve — 截止阀（Stop Valve）可视化组件
 *
 * 说明：
 * - 表示一个有开/关状态的被动管件（截止阀），提供 `isOpen` 状态与端口；
 * - 通常不需要 `_startLoop`，因为为无源被动组件，状态由上层网络/用户操作驱动；
 * - 静态部分缓存以减少重绘。
 */
export class StopValve extends BaseComponent {
    constructor(config, sys) {
        ...
        this._initGroups();
        // 1. 基础设置与缩放
        this.scale = config.scale * (config.internalScale || 1.35);
    ...
    update() {
        ...
        // 颜色：导通绿色（#4caf50），截止红色（#e74c3c）
        if (this._knob) {
            const cOpen  = '#4caf50', cClose = '#e74c3c';
            const sOpen  = '#388e3c', sClose = '#c0392b';
            this._knob.fill(this.isOpen ? cOpen : cClose);
            this._knob.stroke(this.isOpen ? sOpen : sClose);
        }
