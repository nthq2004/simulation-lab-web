...
/**
 * AuxNCContact — 接触器常闭辅助触头（NC）
 *
 * 界面与常开辅助触头（AuxNOContact）统一：
 *  左/右端子 → 左/右静触头圆点，动触刀绕右端子旋转。
 *  无外框、无发热元件、无虚线推杆（区别于热继电器常闭触点 ThermalNCContact）。
 *
 * 工作逻辑（由 ContactorDevice 驱动）：
 *  未吸合：常闭触点闭合（刀片水平，接触左静触头）
 *  吸合  ：常闭触点断开（刀片下倾，脱离左静触头）
 *
 * ── 端口 ─────────────────────────────────────────────────────
 *  com — 公共端（左）
 *  nc  — 常闭端（右）
