...
export class Timer555 extends BaseComponent {
    ...
    tick(dt) {
        ...
        if (!s) return;
        const cOUT = s.portToCluster.get(`${this.id}_wire_out`);
        if (cOUT !== undefined) {
            this._outVoltage = s.nodeVoltages.get(cOUT) || 0;
        }
        const port = (name) => {
            const c = s.portToCluster.get(`${this.id}_wire_${name}`);
            return c !== undefined ? (s.nodeVoltages.get(c) || 0) : 0;
        };
        this._thVoltage = port('th');
        this._trVoltage = port('tr');
        this._vccVoltage = port('vcc');
    }
