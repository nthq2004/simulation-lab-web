...
export const componentConfigs = [
    { Class: ACPower, id: 'ac_power', x: 30, y: 60, voltageRMS: 50, frequency: 50, isOn: false },
    ...
    if (!sys._hallclampPollTimer) {
        sys._hallclampPollTimer = setInterval(() => {
            ...
            if (_circuitMode === 'ac') {
                ...
                if (sl) sl.value = Math.min(220, Math.max(0, (sys.comps['ac_power'] && sys.comps['ac_power'].voltageRMS) || 0));
            } else {
                const power = sys.comps['dc_power'];
                const sl = document.getElementById('dcVoltageSlider');
                if (power && vd) {
