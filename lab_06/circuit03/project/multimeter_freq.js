    ...
    'freq-basic': {
        ...
        steps: [
            ...
            },
            {
                ...
                act() {
                    const ac = this.sys.comps['ac'];
                    if (ac) { ac.frequency = 45; ac.update(); }
                },
                check() {
                    const ac = this.sys.comps['ac'];
                    return ac && Math.abs(ac.frequency - 45) < 0.1;
                },
            },
            {
                ...
                act() {
                    const ac = this.sys.comps['ac'];
                    if (ac) { ac.frequency = 55; ac.update(); }
                },
                check() {
                    const ac = this.sys.comps['ac'];
                    return ac && Math.abs(ac.frequency - 55) < 0.1;
                },
            },
            {
                msg: '5. 测试题：频率表的工作原理',
...
export const componentConfigs = [
    { Class: ACPower, id: 'ac', x: 10, y: 450, voltageRMS: 100, frequency: 50, isOn: false },
