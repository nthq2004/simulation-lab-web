...
export class Renderer {
    ...
    constructor(sys) {
        this.sys = sys;
        this._pendingPipeDraw = false;
        this._pendingWireDraw = false;
    }
    ...
    updateLinePositions() {
        ...
        const getPosByPort = (portId) => {
            const did = portId.includes('_wire_') ? portId.split('_wire_')[0]
                : portId.includes('_pipe_') ? portId.split('_pipe_')[0]
                : portId.split('_')[0];
    ...
    _drawPipe(conn, p1, p2, sys, nodesRef) {
        ...
        line.on('contextmenu', (evt) => { evt.evt.preventDefault(); removeHandler(); });
        flow.on('contextmenu', (evt) => { evt.evt.preventDefault(); removeHandler(); });
        handle.on('contextmenu', (evt) => { evt.evt.preventDefault(); removeHandler(); });
    ...
    _calcWirePoints(allConns, conn, p1, p2) {
        ...
        const devA = conn.from.split('_wire_')[0] || conn.from.split('_')[0];
        const devB = conn.to.split('_wire_')[0] || conn.to.split('_')[0];
        const siblings = allConns.filter(c => c.type === 'wire' && (() => {
            const ca = c.from.split('_wire_')[0] || c.from.split('_')[0];
            const cb = c.to.split('_wire_')[0] || c.to.split('_')[0];
