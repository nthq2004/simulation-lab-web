...
export class CircuitTopology {
     ...
     */
    build(rawDevices, connections, poorContactPorts = new Set()) {
        const parent = {};
        ...
        rawDevices.forEach(dev => {
            ...
            // 起动按钮（NO常开）：按下时短接
            if (dev.type === 'PUSHBUTTON' && dev.special === 'START-BTN' && dev._isPressed)
                internalUnion(`${id}_wire_no1`, `${id}_wire_no2`);

            // 停止按钮（NC常闭）：未按下时短接
            if (dev.type === 'PUSHBUTTON' && dev.special === 'STOP-BTN' && !dev._isPressed)
            ...
            // 汇流排：同一铜条（相）上任意端口接线，则该相全部已接线端口视为同一导体
            if (dev.type === 'busbar_3p') {
                ['l1', 'l2', 'l3'].forEach(phase => {
                    const wired = [];
                    for (let k = 0; k < dev._tapsPerPhase; k++) {
                        const pid = `${id}_wire_${phase}_${k}`;
                        if (activePorts.has(pid)) wired.push(pid);
                    }
                    for (let k = 1; k < wired.length; k++) {
                        union(wired[0], wired[k]);
                    }
                });
            }

            // 船用框架式空气断路器（发电机主开关）：
            // 主触头：由 DeviceStamps.stampMainsSwitch 注入接触电阻（仅「连接位+合闸」导通）
            // 辅触头：NO 合闸时闭合、NC 分闸时闭合（零电阻短接）
            if (dev.type === 'gen_acb') {
                if (dev._state === 'closed') {
                    internalUnion(`${id}_wire_no_a`, `${id}_wire_no_b`);
                } else {
                    internalUnion(`${id}_wire_nc_a`, `${id}_wire_nc_b`);
                }
        ...
        rawDevices.forEach(dev => {
            ...
            if (dev.type === 'source_3p' || dev.type === 'gen_3p') {
                ['u', 'v', 'w', 'n'].forEach(p =>
                    activePorts.add(`${dev.id}_wire_${p}`));
            }
            if (dev.type === 'ACB') {
            ...
            // 汇流排：强制纳入全部接线端子，避免仅有部分端口接线时产生孤立节点
            if (dev.type === 'busbar_3p') {
                ['l1', 'l2', 'l3'].forEach(phase => {
                    for (let k = 0; k < (dev._tapsPerPhase || 8); k++) {
                        activePorts.add(`${dev.id}_wire_${phase}_${k}`);
                    }
                });
            }
            // 发电机主开关：强制纳入主触头、辅触头与控制回路端口
            if (dev.type === 'gen_acb') {
                ['l1','l2','l3','t1','t2','t3','mp','mn','x','y',
                 'uv1','uv2','sh1','sh2','oc1','oc2','no_a','no_b','nc_a','nc_b'].forEach(p =>
                    activePorts.add(`${dev.id}_wire_${p}`));
            }
        });
