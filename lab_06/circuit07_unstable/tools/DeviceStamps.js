...
export const DeviceStamps = {
    ...
    // ─── 船用框架式空气断路器（发电机主开关）────────────────────────
    // 主触头接触电阻（仅「连接位 + 合闸」导通，试验位合闸不送电）
    // 控制回路线圈/电机固定电阻（储能电机、合闸线圈、失压/分励/过流脱扣线圈）
    stampMainsSwitch(ctx, G, B, mainsDevs) {
        mainsDevs.forEach(dev => {
            // 1. 主触头接触电阻
            if (dev._rackPos === 'connected' && dev._state === 'closed') {
                const Rcontact = 0.001;
                [['l1','t1'],['l2','t2'],['l3','t3']].forEach(([a, b]) => {
                    const c1 = ctx.portToCluster.get(`${dev.id}_wire_${a}`);
                    const c2 = ctx.portToCluster.get(`${dev.id}_wire_${b}`);
            ...
            // 2. 控制回路固定电阻（始终存在，保证电压检测有明确负载）
            const pairs = [
                ['mp', 'mn', dev._motorR ?? 10],
                ['x',  'y',  dev._closeCoilR ?? 50],
                ['uv1','uv2',dev._uvCoilR ?? 120],
                ['sh1','sh2',dev._shuntCoilR ?? 50],
                ['oc1','oc2',dev._ocCoilR ?? 200],
            ];
            pairs.forEach(([a, b, R]) => {
                const c1 = ctx.portToCluster.get(`${dev.id}_wire_${a}`);
                const c2 = ctx.portToCluster.get(`${dev.id}_wire_${b}`);
                if (c1 !== undefined && c2 !== undefined)
                    this._fill(ctx, G, B, c1, c2, 1 / Math.max(R, 0.01));
            });
        });
    },
