...
export class SyncGenerator3P extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        this._zeta      = parseFloat(config.freqZeta)  || 0.6; // 阻尼比（<1 有过冲）
    }
    ...
    tick(dt) {
        ...
        if (this.isOn) {
            ...
            const fTarget = this.freq - (this.ratedPower > 0 ? (Pkw / this.ratedPower) * this.freqDroop : 0);
    ...
    getConfigFields() {
        return [
            ...
            { label: '初始状态', key: 'isOn', type: 'select', options: [
    ...
    onConfigUpdate(cfg) {
        ...
        if (cfg.freqZeta  !== undefined) this._zeta     = parseFloat(cfg.freqZeta)  || 0.6;
