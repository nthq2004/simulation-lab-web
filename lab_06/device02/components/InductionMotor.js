...
export class InductionMotor extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // MNA 列索引
        this._iUCol  = undefined;
        this._iVCol  = undefined;
        this._iWCol  = undefined;
        this._iRUCol = undefined;
        this._iRVCol = undefined;
        this._iRWCol = undefined;
    ...
    _drawRotorStatic() {
        ...
        // 定子齿槽（6 槽，U 红 / V 绿 / W 蓝，每相 2 槽）
        const slotColors = ['#f30101','#1dfe1d','#4f9bf9','#fa0909','#0dfb0d','#358efb'];
        for (let i = 0; i < 6; i++) {
            const a  = (i / 6) * Math.PI * 2;
            const a1 = a - Math.PI / 6 * 0.25;
            const a2 = a + Math.PI / 6 * 0.25;
    ...
    _createFieldGroup() {
        ...
        const rInner = this._rotorR * 0.69;  // 定子铁芯中部 (r×0.55~0.95 的中间)
        ...
        this._fieldGroup.add(new Konva.Arc({
            ...
            angle: 180, fill: '#e03030', stroke: '#c02020',
            strokeWidth: 1, rotation: 0, opacity: 0.85,
        }));
        ...
        this._fieldGroup.add(new Konva.Arc({
            x: 0, y: 0,
            innerRadius: rInner, outerRadius: rOuter,
            angle: 180, fill: '#303030', stroke: '#202020',
            strokeWidth: 1, rotation: 180, opacity: 0.85,
        }));
    ...
    _updateDynamic() {
        ...
        // 1. 旋转磁场（相序决定方向）
        if (hasVoltage) {
            this._fieldAngle += this._omega_sync*2 * (this.sys?.voltageSolver?.deltaTime || 0.5e-3);
        ...
        const TeVal = this._Te.toFixed(4);
        ...
        this._statusTexts.seq.text(`相序: ${this._phaseSeq > 0 ? '正序(UVW)' : '逆序(UWV)'}`);
    ...
    _preSolve(dt) {
        ...
        const accel = (this._Te - this.loadTorque - this.B * this._omega_m) / this.J;
    ...
    _postSolve(iu, iv, iw, iru, irv, irw) {
        const IM_MAX = 300; // 最大相电流幅值 (A)，防止数值爆裂
        ...
        this._irvPrev = clamp(irv);
        this._irwPrev = clamp(irw);
