...
export class ACPower extends BaseComponent {
    constructor(config, sys) {
        ...
        this.rOn = 0.1;       // 内阻（仿真用，可用于求解器）

        // 可调参数（默认值）
        this.voltageRMS = config.voltageRMS || 24; // V（有效值，面板显示）
        this.frequency = config.frequency || 50;  // Hz
        this.phase = config.phase || 0;       // 相位（弧度）
    ...
    // 名牌与厂商信息（静态）
    _drawNameplate() {
        const title = new Konva.Text({ x: 10, y: 5, text: `AC ${this.voltageRMS}V`, fontSize: 12, fontStyle: 'bold' });
    ...
    getValue(currentTime) {
        ...
        if (this.frequency === 0) return peak * Math.sin(this.phase);
        return peak * Math.sin(2 * Math.PI * this.frequency * currentTime + this.phase);
    }
    ...
    update() {
        ...
        this._vUpdatePointer?.(this.voltageRMS);
    ...
    getConfigFields() {
        return [
            { label: '电压有效值 V', key: 'voltageRMS', type: 'number' },
    ...
    onConfigUpdate(cfg) {
        let changed = false;
        if (cfg.voltageRMS !== undefined) {
            this.voltageRMS = parseFloat(cfg.voltageRMS) || 24;
            changed = true;
        }
        if (cfg.frequency !== undefined) {
            this.frequency = parseFloat(cfg.frequency) || 50;
            changed = true;
        }
        if (changed) {
            this.update();
