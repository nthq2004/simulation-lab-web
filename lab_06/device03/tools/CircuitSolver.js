...
export class CircuitSolver {
    ...
    _solve() {
        ...
        // ── 三相异步电动机方程数（每台 6 个电流变量）──
        const imDevs = this._deviceCache.inductionMotorDevs || [];
        const imFullEq = imDevs.filter(d => !d.simpleModel).length * 6;
        const imSimpleEq = 0;  // 简化模型使用诺顿等效，不占用额外方程（暂未使用）
        const imEqCount = imFullEq + imSimpleEq;

        const nodeVarCount = mSize + ctInternalCount;
        const totalSize = nodeVarCount + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount + ctVcvsEqCount + mwEqCount + imEqCount;
    ...
    _updateDeviceCurrents(devices, results) {
        ...
        (inductionMotorDevs || []).forEach(dev => {
            if (dev._iUCol === undefined && !dev._nortonGeq) {
            ...
            if (dev.simpleModel) {
                ...
                // ── 电磁转矩 T_e = p * i_s^T * dL_sr/dθ_r * i_r ──
                const theta_r = dev._theta_r || 0;
                const Lm = dev.Lm || 0.078;
                const p = dev.polePairs || 2;
                const TWO_PI_3 = 2 * Math.PI / 3;
                const si     = Math.sin(theta_r);
                const si120  = Math.sin(theta_r - TWO_PI_3);
                const siM120 = Math.sin(theta_r + TWO_PI_3);
                ...
                // 保存当前电角度供下帧 stamp 计算 L_sr(θ_r_prev)
                dev._theta_r_prev = theta_r;
            }
        });
    }
    /**
