...
export class InductionMotor extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        this.simpleModel = config.simpleModel !== undefined ? config.simpleModel : false;
        ...
        // dq0 状态变量（上帧值）
        // 预填稳态锁转子 dq 电流，使 MNA 首次求解即得到正确转矩。
        // 原理：dq 模型在同步旋转坐标系下，锁转子稳态时 dq 电流为直流量，
        // 由 4×4 线性系统确定。初始化为稳态值后，H 项 (λ_prev/dt) 立即建磁，
        // 避免从零磁链开始长达 ~0.5s 的转矩爬升。
        {
            const _omega_e = this._omega_e || (2 * Math.PI * 50);
            const _Ls = this.Lsigma1 + this.Lm;
            const _Lr = this.Lsigma2 + this.Lm;
            const _Rs = this.R1 || 0.50;
            const _Rr = this.R2 || 0.46;
            const _V_peak = 220 * Math.SQRT2;   // 默认相电压峰值 311V

            // 转子电流表达式系数：iqr = A·iqs + B·ids,  idr = C·iqs + D·ids
            const Zr2 = _Rr * _Rr + _omega_e * _omega_e * _Lr * _Lr;
            const A_rot = -_omega_e * _omega_e * _Lr * this.Lm / Zr2;
            const B_rot = -_omega_e * this.Lm * _Rr / Zr2;
            const C_rot =  _omega_e * this.Lm * _Rr / Zr2;
            const D_rot = -_omega_e * _omega_e * _Lr * this.Lm / Zr2;

            // 定子 2×2 矩阵
            const M11 = _Rs + _omega_e * this.Lm * C_rot;
            const M12 = _omega_e * _Ls + _omega_e * this.Lm * D_rot;
            const M21 = -_omega_e * _Ls - _omega_e * this.Lm * A_rot;
            const M22 = _Rs - _omega_e * this.Lm * B_rot;

            const det = M11 * M22 - M12 * M21;
            if (Math.abs(det) > 1e-12) {
                const _iqs = (M22 * _V_peak) / det;
                const _ids = (-M21 * _V_peak) / det;
                this._iqsPrev = _iqs;
                this._idsPrev = _ids;
                this._iqrPrev = A_rot * _iqs + B_rot * _ids;
                this._idrPrev = C_rot * _iqs + D_rot * _ids;
                console.log('[IM] init iqs_prev='+_iqs.toFixed(2)+' ids_prev='+_ids.toFixed(2)+' iqr_prev='+this._iqrPrev.toFixed(2)+' idr_prev='+this._idrPrev.toFixed(2));
            } else {
                this._iqsPrev = 0;
                this._idsPrev = 0;
                this._iqrPrev = 0;
        ...
        // abc 相电流（由 dq 逆变换得到，用于显示）
        this._iuPrev  = 0;
        this._ivPrev  = 0;
        this._iwPrev  = 0;

        // dq0 MNA 列索引
        this._iqsCol = undefined;
        this._idsCol = undefined;
        this._iqrCol = undefined;
        this._idrCol = undefined;

        // 电角度（开环 Park 变换，θ_e = ω_e·t）
        this._omega_e = 2 * Math.PI * 50;   // 默认 50Hz
        this._theta_e = 0;
    ...
    _updateDynamic() {
        ...
        this._fieldGroup.visible(hasVoltage);
        ...
        const speedRPM = Math.round(rpm);
        const slipPct = (this.slip * 100).toFixed(2);
        // 显示平滑：EMA 滤除 100Hz 欠采样噪声（仅 UI 层，不影响机械积分）
        if (this._teDisp === undefined) this._teDisp = this._Te;
        this._teDisp += 0.05 * (this._Te - this._teDisp);
        const TeVal = this._teDisp.toFixed(1);
    ...
    _preSolve(dt) {
        ...
        // 电角度（开环 Park 变换，θ_e = ∫ω_e·dt）
        this._omega_e  = 2 * Math.PI * sysFreq;
        this._theta_e += this._omega_e * dt;
        if (this._theta_e > 2 * Math.PI) this._theta_e -= 2 * Math.PI;
        if (this._theta_e < 0) this._theta_e += 2 * Math.PI;
        ...
        let brakeT = 0;
        this._dcBraking = false;
        const _dc = this.sys?.comps?.['dc24v'];
        if (_dc?.isOn && !this.sys?.comps?.['ac']?.isOn) {
            const _ic = (a, b) => this.sys?.isPortConnected(a, b);
            if (_ic && _ic('dc24v_wire_p', 'im01_wire_u1') && _ic('dc24v_wire_n', 'im01_wire_v1')) {
        ...
        // 堵转保护：正序时不允许持续反转。
        // 阈值设为 -5 rad/s（≈ -48 RPM），允许启动过程转矩脉动引起的短时负速，
        // 电机惯性 J=0.12 会自然平滑 100Hz 脉动，极低转速下正平均转矩会推动正向加速。
        if (this._phaseSeq > 0 && this._omega_m < -5.0) this._omega_m = 0;
    ...
    /** 求解后将回读的 dq 电流存入（含限幅防止 MNA 数值发散） */
    _postSolve(iqs, ids, iqr, idr) {
        const IM_MAX = 300; // 最大电流幅值 (A)，防止数值爆裂
        const clamp = (v) => Math.max(-IM_MAX, Math.min(IM_MAX, v));
        this._iqsPrev = clamp(iqs);
        this._idsPrev = clamp(ids);
        this._iqrPrev = clamp(iqr);
        this._idrPrev = clamp(idr);

        // 逆 Park 变换：dq → abc（用于显示和相序检测）
        const ct = Math.cos(this._theta_e);
        const st = Math.sin(this._theta_e);
        const ct120  = Math.cos(this._theta_e - 2 * Math.PI / 3);
        const st120  = Math.sin(this._theta_e - 2 * Math.PI / 3);
        const ctM120 = Math.cos(this._theta_e + 2 * Math.PI / 3);
        const stM120 = Math.sin(this._theta_e + 2 * Math.PI / 3);
        this._iuPrev = clamp(ct * iqs + st * ids);
        this._ivPrev = clamp(ct120 * iqs + st120 * ids);
        this._iwPrev = clamp(ctM120 * iqs + stM120 * ids);
        this.phaseCurrents = { u: this._iuPrev, v: this._ivPrev, w: this._iwPrev };
    }

    _setTorque(te) {
        // 原始瞬时转矩直接注入机械积分。
        // 电机惯性 J=0.12 在 100Hz 脉动下的速度响应幅值仅 ±2 rad/s，
        // 且正平均转矩保证净加速；过冲 2%（1530 RPM）是真实物理现象。
        this._Te = te;
    }
    ...
    _onButtonClick(btnId) {
        ...
        if (btnId === 'btnClr') {
            ...
            this._iqsPrev = 0; this._idsPrev = 0; this._iqrPrev = 0; this._idrPrev = 0;
