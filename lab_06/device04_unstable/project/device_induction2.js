...
export const PROJECT_WORKFLOWS = {
    ...
    'characteristic-test': {
        ...
        steps: [
            ...
            {
                msg: '第 5 步：过载能力测试。将负载转矩设为 165 N·m（略小于最大转矩约 170 N·m），观察异步电机仍能稳定运行，体现其过载能力。',
                mode: 'check',
                async act() {
                    await new Promise(r => setTimeout(r, 500));
                    const motor = this.sys.comps['im01'];
                    if (motor) motor.loadTorque = 165;
                    const slider = document.getElementById('torqueSlider');
                    if (slider) {
                        slider.value = '165';
                        slider.dispatchEvent(new Event('input'));
                ...
                check() {
                    const motor = this.sys.comps['im01'];
                    return motor && Math.abs(motor.loadTorque - 165) < 3;
                },
            },
            {
...
export const componentConfigs = [
    ...
    { Class: InductionMotor, id: 'im01', x: 150, y: 320, visible: true,
        ...
        simpleModel: false, loadTorque: 0 },
...
function calcFanK(motor) {
    ...
    const rs = motor.ratedSpeed || 1440;
