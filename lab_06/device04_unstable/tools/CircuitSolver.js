...
export class CircuitSolver {
    ...
    _solve() {
        ...
        const {
            ...
            wattmeterDevs, reg7805Devs, fuseDevs, batteryDevs,
            mwDevs, inductionMotorDevs,
        } = this._deviceCache;
        ...
        // ── 三相异步电动机方程数（每台 4 个 dq 电流变量）──
        const imDevs = this._deviceCache.inductionMotorDevs || [];
        const imFullEq = imDevs.filter(d => !d.simpleModel).length * 4;
        const imSimpleEq = 0;  // 简化模型使用诺顿等效，不占用额外方程（暂未使用）
        const imEqCount = imFullEq + imSimpleEq;

        const nodeVarCount = mSize + ctInternalCount;
        const totalSize = nodeVarCount + pidEqCount + opAmps.length + oscDevs.length + 
        lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + 
        acAmmEqCount + wattmeterEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount + 
        ctVcvsEqCount + mwEqCount + imEqCount;
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // - 23 交流电流表（0V 电压源注入）
            const acAmmVIdx = totalSize - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampACAmmeters(ctx, G, B, acAmmeterDevs, acAmmVIdx);

            // - 23.2 功率表（0V 电压源注入电流线圈）
            const wattmeterVIdx = totalSize - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampWattmeters(ctx, G, B, wattmeterDevs, wattmeterVIdx);

            /**
             * ─ 23.5 电流互感器（CT）：原边 0V 电压源 + 副边受控源 ─────
             */
            const ctVIdx = totalSize - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampCurrentTransformers(ctx, G, B, ctDevs, ctVIdx);

            // ─ 23.7 电压互感器（PT）受控电压源注入 ────────────────────
            const potVIdx = totalSize - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampPotentialTransformers(ctx, G, B, potentialTransformerDevs, potVIdx);

            // ─ 23.75 控制变压器（互感耦合电感伴随模型，2 电流变量/台）──────────
            const ctCtrlVIdx = totalSize - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampControlTransformers(ctx, G, B, ctCtrlDevs, ctCtrlVIdx);

            // ─ 23.76 三相电机绕组（三耦合电感器，3 电流变量/台）─────────────
            const mwVIdx = totalSize - mwEqCount - imEqCount;
            DeviceStamps.stampMotorWindings(ctx, G, B, mwDevs, mwVIdx);

            // ─ 23.77 三相异步电动机（4 dq 电流变量/台，Park 变换接口）──
            const imVIdx = totalSize - imEqCount;
            DeviceStamps.stampInductionMotors(ctx, G, B, inductionMotorDevs, imVIdx);

            // - 24 MF47 万用表等效注入
            DeviceStamps.stampMF47(ctx, G, B, mf47Devs);

            // ─ 25. 数字万用表（MA 档分流电阻；RES 档恒流/恒压迭代切换）──
            const mmResVIdx = totalSize - mmResEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampMultimeters(ctx, G, B, multimeterDevs, results, mmResVIdx);
            // ─ 26. 普通电流表（0V 电压源注入 p-n 之间）──
            const ammVIdx = totalSize - ammEqCount - ctVcvsEqCount - mwEqCount;
    ...
    _updateDeviceCurrents(devices, results) {
        ...
        // ─ 23.77 三相异步电动机电流回读 + 转矩更新 ────────────────
        (inductionMotorDevs || []).forEach(dev => {
            if (dev._iqsCol === undefined && !dev._nortonGeq) {
                // 既非全模型也无 Norton 参数 → 跳过
                return;
            }
            if (dev.simpleModel) {
                // 简化模型（诺顿等效）：从节点电压回算相电流
                ...
                const i_qs = results[dev._iqsCol];
                const i_ds = results[dev._idsCol];
                const i_qr = results[dev._iqrCol];
                const i_dr = results[dev._idrCol];
                dev._postSolve(i_qs, i_ds, i_qr, i_dr);

                // ── dq 坐标系瞬时转矩 T_e = 1.5·p·Lm·(iqs·idr − ids·iqr) ──
                // dq 模型在同步旋转坐标系下，转矩为直流量，不存在 abc 模型中的
                // 100Hz 脉动，因此可以直接注入机械积分。
                const p  = dev.polePairs || 2;
                const Lm = dev.Lm || 0.078;
                let te_raw = 1.5 * p * Lm * (i_qs * i_dr - i_ds * i_qr);

                // 调试日志：输出前 10 帧的 dq 电流和转矩
                if ((window._imDqDebugCount = (window._imDqDebugCount||0) + 1) <= 10) {
                    console.log('[IM] frame='+window._imDqDebugCount+' iqs='+i_qs.toFixed(2)+' ids='+i_ds.toFixed(2)
                        +' iqr='+i_qr.toFixed(2)+' idr='+i_dr.toFixed(2)
                        +' te='+te_raw.toFixed(4)+' omega_m='+dev._omega_m.toFixed(6));
                }

                // ── 转矩限幅（基于等效电路的最大转矩公式，防止数值尖峰） ──
                const R1_lim  = dev.R1 || 2.15;
                const L1s_lim = dev.Lsigma1 || 0.004;
                const L2s_lim = dev.Lsigma2 || 0.005;
                const omegaSync_lim = dev._omega_sync || 157.08;
                const X_total_lim = 2 * Math.PI * (this._systemFreq || 50) * (L1s_lim + L2s_lim);
                const sqrt_term_lim = Math.sqrt(R1_lim * R1_lim + X_total_lim * X_total_lim);
                const absOmega_lim = Math.abs(omegaSync_lim);
                const vu1 = getV(dev.id, 'u1'), vu2 = getV(dev.id, 'u2');
                const vv1 = getV(dev.id, 'v1'), vv2 = getV(dev.id, 'v2');
                const vw1 = getV(dev.id, 'w1'), vw2 = getV(dev.id, 'w2');
                const v_mag_sq_lim = (vu1!==undefined&&vu2!==undefined?vu1-vu2:0)**2
                                   + (vv1!==undefined&&vv2!==undefined?vv1-vv2:0)**2
                                   + (vw1!==undefined&&vw2!==undefined?vw1-vw2:0)**2;
                const Vrms_sq_lim = v_mag_sq_lim / 3;
                if (absOmega_lim > 1e-6 && Vrms_sq_lim > 1) {
                    const T_mag_lim = (3 * Vrms_sq_lim) / (2 * absOmega_lim * (R1_lim + sqrt_term_lim));
                    const limit_lim = T_mag_lim * 1.5;
                    te_raw = Math.max(-limit_lim, Math.min(limit_lim, te_raw));
                }

                // dq 转矩为直流量，直接注入机械积分
                dev._setTorque(te_raw);
            }
        });
    }
    /**
