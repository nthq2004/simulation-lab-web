...
export const DeviceStamps = {
    ...
    // ─── 28. 三相异步电动机（4 变量 dq0 模型，替代原 6 变量 abc）─────────
    /**
     * 为 InductionMotor 注入 4 电流变量 dq0 MNA 模型：
     *   4 个 dq0 电流变量：i_qs, i_ds, i_qr, i_dr
     *   Park 变换接口连接 abc 端子，转矩为直流量无 100Hz 脉动
     *
     * 若 dev.simpleModel === true 则退化为诺顿等效简化模型。
     */
    ...
    _stampIMFull(ctx, G, B, motorDevs, vIdx) {
        ...
        motorDevs.forEach(dev => {
            ...
            // dq0 同步旋转坐标系参数
            const omega_e = dev._omega_e || (2 * Math.PI * (ctx.systemFreq || 50));
            const theta_e = dev._theta_e || 0;
            const omega_r = (dev.polePairs || 2) * (dev._omega_m || 0);
            const omega_slip = omega_e - omega_r;

            // Park 变换系数（幅值不变形式）
            const twoThirds = 2 / 3;
            const ct  = Math.cos(theta_e);
            const st  = Math.sin(theta_e);
            const ct120  = Math.cos(theta_e - TWO_PI_3);
            const st120  = Math.sin(theta_e - TWO_PI_3);
            const ctM120 = Math.cos(theta_e + TWO_PI_3);
            const stM120 = Math.sin(theta_e + TWO_PI_3);
            ...
            // 上帧 dq 电流
            const iqs_p = dev._iqsPrev || 0;
            const ids_p = dev._idsPrev || 0;
            const iqr_p = dev._iqrPrev || 0;
            const idr_p = dev._idrPrev || 0;
            ...
            // 历史项（B = -lam/dt）
            const H_qs = lam_qs_p * invDt;
            const H_ds = lam_ds_p * invDt;
            const H_qr = lam_qr_p * invDt;
            const H_dr = lam_dr_p * invDt;
            ...
            // ════════════════════════════════════════════════
            // 1. qs 方程：v_qs - (Rs+Gs)·iqs - ω_e·Ls·ids - Gm·iqr - ω_e·Lm·idr = -lam_qs_p/dt
            // ════════════════════════════════════════════════
            if (cU1 !== undefined && cU2 !== undefined) {
                eqVStamp(col_qs, cU1,  twoThirds * ct);
                eqVStamp(col_qs, cU2, -twoThirds * ct);
            ...
            G[col_qs][col_qs] -= (R1 + Gs);
            G[col_qs][col_ds] -= omega_e * Ls;
            G[col_qs][col_qr] -= Gm;
            G[col_qs][col_dr] -= omega_e * Lm;
            B[col_qs] -= H_qs;

            // ════════════════════════════════════════════════
            // 2. ds 方程：v_ds + ω_e·Ls·iqs - (Rs+Gs)·ids + ω_e·Lm·iqr - Gm·idr = -lam_ds_p/dt
            // ════════════════════════════════════════════════
            if (cU1 !== undefined && cU2 !== undefined) {
                eqVStamp(col_ds, cU1,  twoThirds * st);
                eqVStamp(col_ds, cU2, -twoThirds * st);
                eqVStamp(col_ds, cV1,  twoThirds * st120);
                eqVStamp(col_ds, cV2, -twoThirds * st120);
                eqVStamp(col_ds, cW1,  twoThirds * stM120);
                eqVStamp(col_ds, cW2, -twoThirds * stM120);
            }
            ...
            // ════════════════════════════════════════════════
            // 3. qr 方程（转子短接）：
            //    -Gm·iqs - ω_slip·Lm·ids - (Rr+Gr)·iqr - ω_slip·Lr·idr = -lam_qr_p/dt
            // ════════════════════════════════════════════════
            G[col_qr][col_qs] -= Gm;
            ...
            // ════════════════════════════════════════════════
            // 4. dr 方程（转子短接）：
            //    +ω_slip·Lm·iqs - Gm·ids + ω_slip·Lr·iqr - (Rr+Gr)·idr = -lam_dr_p/dt
            // ════════════════════════════════════════════════
            ...
            // ════════════════════════════════════════════════
            // KCL 贡献：逆 Park 变换注入 abc 相电流
            //   i_a = cos(θ_e)·iqs + sin(θ_e)·ids
            //   i_b = cos(θ_e-120)·iqs + sin(θ_e-120)·ids
            //   i_c = cos(θ_e+120)·iqs + sin(θ_e+120)·ids
            // ════════════════════════════════════════════════
            if (cU1 !== undefined && cU2 !== undefined) {
                kclStamp(cU1, col_qs, ct);
                kclStamp(cU1, col_ds, st);
                kclStamp(cU2, col_qs, -ct);
                kclStamp(cU2, col_ds, -st);
            }
            if (cV1 !== undefined && cV2 !== undefined) {
                kclStamp(cV1, col_qs, ct120);
                kclStamp(cV1, col_ds, st120);
                kclStamp(cV2, col_qs, -ct120);
                kclStamp(cV2, col_ds, -st120);
            }
            if (cW1 !== undefined && cW2 !== undefined) {
                kclStamp(cW1, col_qs, ctM120);
                kclStamp(cW1, col_ds, stM120);
                kclStamp(cW2, col_qs, -ctM120);
                kclStamp(cW2, col_ds, -stM120);
            }
