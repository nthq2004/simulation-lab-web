...
export class CircuitSolver {
    ...
    _buildDeviceCache() {
        ...
        return {
            ...
            wattmeterDevs:    raw.filter(d => d.special === 'WATTMETER'),
    ...
    _solve() {
        ...
        const {
            ...
            wattmeterDevs, reg7805Devs, fuseDevs,             batteryDevs, leadAcidBatteryDevs, singleLeadAcidBatteryDevs, chargeBoardDevs,
        ...
        const imEqCount = 0;  // 全模型已移除，仅用诺顿等效
...
const totalSize = nodeVarCount + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount + ctVcvsEqCount + mwEqCount + imEqCount + rlEqCount;
        ...
        let _lastIter = 0;
        let rawResults = null;  // 保存最后一次高斯消元的原始结果（阻尼前），用于 KCL 一致的电流回读
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // - 23 交流电流表（0V 电压源注入）
            const acAmmVIdx = adjustedSize - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount - imEqCount;
            DeviceStamps.stampACAmmeters(ctx, G, B, acAmmeterDevs, acAmmVIdx);

            // - 23.2 功率表（0V 电压源注入电流线圈）
            const wattmeterVIdx = adjustedSize - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount - imEqCount;
            DeviceStamps.stampWattmeters(ctx, G, B, wattmeterDevs, wattmeterVIdx);

            /**
             * ─ 23.5 电流互感器（CT）：原边 0V 电压源 + 副边受控源 ─────
             */
            const ctVIdx = adjustedSize - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount - imEqCount;
            DeviceStamps.stampCurrentTransformers(ctx, G, B, ctDevs, ctVIdx);

            // ─ 23.7 电压互感器（PT）受控电压源注入 ────────────────────
            const potVIdx = adjustedSize - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount - imEqCount;
            DeviceStamps.stampPotentialTransformers(ctx, G, B, potentialTransformerDevs, potVIdx);

            // ─ 23.75 控制变压器（互感耦合电感伴随模型，2 电流变量/台）──────────
            const ctCtrlVIdx = adjustedSize - ctVcvsEqCount - mwEqCount - imEqCount;
            ...
            const mmResVIdx = adjustedSize - mmResEqCount - ctVcvsEqCount - mwEqCount - imEqCount;
            DeviceStamps.stampMultimeters(ctx, G, B, multimeterDevs, results, mmResVIdx);
            // ─ 26. 普通电流表（0V 电压源注入 p-n 之间）──
            const ammVIdx = adjustedSize - ammEqCount - ctVcvsEqCount - mwEqCount - imEqCount;
            ...
            // 保存原始 MNA 结果（阻尼运算会覆盖 nextResults，但原始值满足 KCL）
            ...
            // 同样对扩展变量（电压源/电流源等）做阻尼，保持与节点电压一致
            for (let i = nodeVarCount; i < totalSize; i++) {
                const oldV = results[i] || 0;
                const rawNewV = nextResults[i];
                const damping = Math.min(0.6, 0.30 + iter * 0.04);
                let nextV = oldV + damping * (rawNewV - oldV);
                const MAX_STEP = Math.max(100, Math.abs(oldV) * 0.5, Math.abs(rawNewV) * 0.5);
                const delta = nextV - oldV;
                if (Math.abs(delta) > MAX_STEP) nextV = oldV + MAX_STEP * Math.sign(delta);
                nextResults[i] = nextV;
            }
        ...
        this._updateDeviceCurrents(this._cachedDevs, results, rawResults, nodeMap);
    }

    // ── 统一计算所有设备的电流，进行状态回填（取代原 CurrentReadback 阶段）────────────
    // 序号对应 DeviceStamps 中各 stamp 方法的顺序
    _updateDeviceCurrents(devices, results, rawResults, nodeMap) {
        const {
...
inductionMotorDevs, contactorDevs, thermalRelayDevs, rlSeriesDevs,
        ...
        // --- 内部辅助：安全获取端口电压 ---
        // 优先从原始 MNA 结果读取（满足 KCL），仅当 rawResults 不可用时回退到 nodeVoltages
        const getV = (devId, portName) => {
            const clusterId = this.portToCluster.get(`${devId}_wire_${portName}`);
            if (clusterId === undefined) return undefined;
            const mIdx = nodeMap ? nodeMap.get(clusterId) : undefined;
            if (mIdx !== undefined && mIdx !== -1 && rawResults) {
                const v = rawResults[mIdx];
                if (v !== undefined && v !== null) return v;
            }
            return this.nodeVoltages.get(clusterId) || 0;
        };
        // --- 从原始 MNA 结果获取电压（满足 KCL，用于电流回读） ---
        const getVRaw = (devId, portName) => {
            const clusterId = this.portToCluster.get(`${devId}_wire_${portName}`);
            if (clusterId === undefined) return undefined;
            const mIdx = nodeMap ? nodeMap.get(clusterId) : undefined;
            if (mIdx === undefined || mIdx === -1) return this.nodeVoltages.get(clusterId) || 0;
            return rawResults ? (rawResults[mIdx] ?? 0) : (this.nodeVoltages.get(clusterId) || 0);
        };
        ...
        // ─ 23.77 三相异步电动机电流回读 + 转矩更新（诺顿等效）──────
        (inductionMotorDevs || []).forEach(dev => {
            if (!dev._nortonGeq) return;
            // 使用原始 MNA 结果（rawResults/rawNodeMap），这些值满足 KCL
            const vu1 = getVRaw(dev.id, 'u1'), vu2 = getVRaw(dev.id, 'u2');
            const vv1 = getVRaw(dev.id, 'v1'), vv2 = getVRaw(dev.id, 'v2');
            const vw1 = getVRaw(dev.id, 'w1'), vw2 = getVRaw(dev.id, 'w2');
            const Geq  = dev._nortonGeq;
            const Ldt  = dev._nortonLdt || 0;
            const iup  = dev._iuPrev || 0;
            const ivp  = dev._ivPrev || 0;
            const iwp  = dev._iwPrev || 0;
            // _addI(B, cPos=c2(u2), cNeg=c1(u1), Ih) → B[c2] += Ih, B[c1] -= Ih
            // 源电流从 c1(u1) 流向 c2(u2)，与电导方向一致。
            // 则端口 u1→u2 总电流 = Geq·V + Ldt·Geq·iup  ✓
            ...
            // magIh 同样经 _addI(B, c2, c1, magIh) 注入，方向同向
            ...
            dev._postSolve(i_u, i_v, i_w, 0, 0, 0);

            // 磁化支路状态存储（用于下一帧的梯形积分），也使用原始电压保持 KCL 一致
            const RL_Geq   = dev._nortonRL_Geq   || (this.deltaTime || 0.5e-3) / (2 * (dev.Lm || 0.078));
            const V_u_raw = (vu1 !== undefined && vu2 !== undefined) ? vu1 - vu2 : 0;
            const V_v_raw = (vv1 !== undefined && vv2 !== undefined) ? vv1 - vv2 : 0;
            const V_w_raw = (vw1 !== undefined && vw2 !== undefined) ? vw1 - vw2 : 0;
            dev._magVuPrev = V_u_raw;
            dev._magVvPrev = V_v_raw;
            dev._magVwPrev = V_w_raw;
            // _magIxPrev 存储梯形积分用的磁化支路总电流（电导+历史源）
            dev._magIuPrev = RL_Geq * V_u_raw + magIhu;
            dev._magIvPrev = RL_Geq * V_v_raw + magIhv;
            dev._magIwPrev = RL_Geq * V_w_raw + magIhw;
            let slip = dev.slip || 1;
            ...
            const v_mag_sq = v_u*v_u + v_v*v_v + v_w*v_w;
            const Vrms_sq = v_mag_sq / 3;
        ...
        (devices.acbDevs || []).forEach(dev => {
            ...
            const i1 = (vL1 !== undefined && vT1 !== undefined) ? (vL1 - vT1) / R : 0;
            const i2 = (vL2 !== undefined && vT2 !== undefined) ? (vL2 - vT2) / R : 0;
            const i3 = (vL3 !== undefined && vT3 !== undefined) ? (vL3 - vT3) / R : 0;
            dev.phaseCurrents = { l1: i1, l2: i2, l3: i3 };
            // 存储进线端电压（供 TEST 按钮检测）
