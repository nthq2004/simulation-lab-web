...
export const DeviceStamps = {
     ...
     */
    stampInductionMotors(ctx, G, B, motorDevs, vIdx) {
        // ── 先筛选出需要简化模型的设备 ──
        const fullDevs = motorDevs.filter(d => !d.simpleModel);
        const simpleDevs = motorDevs.filter(d => d.simpleModel);
        let idx = vIdx;
        // 全模型（6 变量/台，占用额外电流方程）
        idx = this._stampIMFull(ctx, G, B, fullDevs, idx);
        // 简化模型（诺顿等效直接嵌入节点矩阵，不占用额外方程）
        this._stampIMSimple(ctx, G, B, simpleDevs);
        return idx;
    },

    /** 全 6 变量模型：定转子耦合（原 stampInductionMotors 本体） */
    _stampIMFull(ctx, G, B, motorDevs, vIdx) {
        const dt = ctx.deltaTime || 0.1e-3;
        ...
        motorDevs.forEach(dev => {
            ...
            const theta_r       = dev._theta_r || 0;
            const theta_r_prev  = dev._theta_r_prev !== undefined ? dev._theta_r_prev : theta_r;

            const Lss_diag = L1s + Lm;
            const Lss_off  = -Lm / 2;

            const ct     = Math.cos(theta_r);
            const ct120  = Math.cos(theta_r + TWO_PI_3);
            const ctM120 = Math.cos(theta_r - TWO_PI_3);

            const ctp     = Math.cos(theta_r_prev);
            const ct120p  = Math.cos(theta_r_prev + TWO_PI_3);
            const ctM120p = Math.cos(theta_r_prev - TWO_PI_3);

            // 伴随电导（L/dt）
            const invDt = 1 / dt;
            const Gss_diag = Lss_diag * invDt;
            const Gss_off  = Lss_off  * invDt;
            const Grr_diag = (L2s + Lm) * invDt;
            const Grr_off  = Lss_off * invDt;

            // Gsr[i][j] = Lm * cos(θr + φ_ij) / dt
            const Gsr_uu = Lm * ct     * invDt;
            const Gsr_uv = Lm * ctM120 * invDt;
            const Gsr_uw = Lm * ct120  * invDt;
            const Gsr_vu = Gsr_uw;
            const Gsr_vv = Gsr_uu;
            const Gsr_vw = Gsr_uv;
            const Gsr_wu = Gsr_uv;
            ...
            // 上帧 Gsr_prev（用 theta_r_prev）
            const Gsr_prev_uu = Lm * ctp     * invDt;
            const Gsr_prev_uv = Lm * ctM120p * invDt;
            const Gsr_prev_uw = Lm * ct120p  * invDt;
            const Gsr_prev_vu = Gsr_prev_uw;
            const Gsr_prev_vv = Gsr_prev_uu;
            const Gsr_prev_vw = Gsr_prev_uv;
            const Gsr_prev_wu = Gsr_prev_uv;
            const Gsr_prev_wv = Gsr_prev_vw;
            const Gsr_prev_ww = Gsr_prev_vv;

            // 上帧电流
            const iup  = dev._iuPrev  || 0;
            const ivp  = dev._ivPrev  || 0;
            const iwp  = dev._iwPrev  || 0;
            const irup = dev._iruPrev || 0;
            const irvp = dev._irvPrev || 0;
            const irwp = dev._irwPrev || 0;

            // 历史项（6×1）
            const Hu  = Gss_diag*iup + Gss_off*ivp + Gss_off*iwp
                      + Gsr_prev_uu*irup + Gsr_prev_uv*irvp + Gsr_prev_uw*irwp;
            const Hv  = Gss_off*iup + Gss_diag*ivp + Gss_off*iwp
                      + Gsr_prev_vu*irup + Gsr_prev_vv*irvp + Gsr_prev_vw*irwp;
            const Hw  = Gss_off*iup + Gss_off*ivp + Gss_diag*iwp
                      + Gsr_prev_wu*irup + Gsr_prev_wv*irvp + Gsr_prev_ww*irwp;
            const Hru = Grr_diag*irup + Grr_off*irvp + Grr_off*irwp
                      + Gsr_prev_uu*iup + Gsr_prev_vu*ivp + Gsr_prev_wu*iwp;
            const Hrv = Grr_off*irup + Grr_diag*irvp + Grr_off*irwp
                      + Gsr_prev_uv*iup + Gsr_prev_vv*ivp + Gsr_prev_wv*iwp;
            const Hrw = Grr_off*irup + Grr_off*irvp + Grr_diag*irwp
                      + Gsr_prev_uw*iup + Gsr_prev_vw*ivp + Gsr_prev_ww*iwp;
            ...
            G[colRU][colV]  -= Gsr_vu;
            G[colRU][colW]  -= Gsr_wu;
            G[colRU][colRU] -= (R2 + Grr_diag);
            G[colRU][colRV] -= Grr_off;
            G[colRU][colRW] -= Grr_off;
            B[colRU] -= Hru;
            ...
            G[colRV][colV]  -= Gsr_vv;
            G[colRV][colW]  -= Gsr_wv;
            G[colRV][colRU] -= Grr_off;
            G[colRV][colRV] -= (R2 + Grr_diag);
            G[colRV][colRW] -= Grr_off;
            B[colRV] -= Hrv;
            ...
            G[colRW][colV]  -= Gsr_vw;
            G[colRW][colW]  -= Gsr_ww;
            G[colRW][colRU] -= Grr_off;
            G[colRW][colRV] -= Grr_off;
            G[colRW][colRW] -= (R2 + Grr_diag);
            B[colRW] -= Hrw;

            // ── 铁损 Rc（在各相端子间并联电导） ──
            const gCore = 1 / (3 * Math.max(Rc, 0.001));
            if (cU1 !== undefined && cU2 !== undefined) {
                this._fill(ctx, G, B, cU1, cU2, gCore);
            }
            if (cV1 !== undefined && cV2 !== undefined) {
                this._fill(ctx, G, B, cV1, cV2, gCore);
            }
            if (cW1 !== undefined && cW2 !== undefined) {
                this._fill(ctx, G, B, cW1, cW2, gCore);
            }

            // ── 剩磁注入（启动时提供初始反电动势） ──
            const slip = dev.slip !== undefined ? dev.slip : 1;
            const remanence = dev.remanenceFlux || 0.012;
            if (slip > 0.8 && remanence > 1e-6) {
                const omegaSync = dev._omega_sync || (2 * Math.PI * (ctx.systemFreq || 50) / Math.max(1, dev.polePairs || 2));
                const eMag = remanence * omegaSync * 0.3;
                B[colRU] -= eMag * Math.cos(theta_r);
                B[colRV] -= eMag * Math.cos(theta_r - TWO_PI_3);
                B[colRW] -= eMag * Math.cos(theta_r + TWO_PI_3);
            }
    ...
    _stampIMSimple(ctx, G, B, motorDevs) {
        ...
        motorDevs.forEach(dev => {
            ...
            const any3pOn = Object.values(dev.sys?.comps || {}).some(c => c.type === 'source_3p' && c.isOn);
            if (!any3pOn) {
