        ...
        steps: [
            ...
            },
            {
                ...
                check() {
                    ...
                    return has('ac_wire_u', 'r_u_wire_l')
                        && has('r_u_wire_r', 'gnd_wire_gnd')
                        && has('ac_wire_v', 'r_v_wire_l')
                        && has('r_v_wire_r', 'gnd_wire_gnd')
                        && has('ac_wire_w', 'r_w_wire_l')
            ...
            },
            {
                ...
                check() {
                    ...
                    return megger && megger.isCranking()
                        && _sameCluster(this.sys, 'megger_wire_l', 'ac_wire_u')
                        && _sameCluster(this.sys, 'megger_wire_e', 'ac_wire_n');
                },
            },

            {
                ...
                check() {
                    const megger = this.sys.comps['megger'];
                    if (megger && megger.isCranking()) return false;
                    return !_sameCluster(this.sys, 'megger_wire_l', 'ac_wire_u')
                        && !_sameCluster(this.sys, 'megger_wire_l', 'ac_wire_v')
                        && !_sameCluster(this.sys, 'megger_wire_l', 'ac_wire_w');
                },
            },
            {
                ...
                check() {
                    const megger = this.sys.comps['megger'];
                    if (!megger || !megger.isCranking()) return false;
                    return _sameCluster(this.sys, 'megger_wire_l', 'ac_wire_v')
                        && _sameCluster(this.sys, 'megger_wire_e', 'ac_wire_n');
                },
            },
            {
                ...
                check() {
                    const megger = this.sys.comps['megger'];
                    if (!megger || !megger.isCranking()) return false;
                    return _sameCluster(this.sys, 'megger_wire_l', 'ac_wire_w')
                        && _sameCluster(this.sys, 'megger_wire_e', 'ac_wire_n');
                },
            },
            {
...
function _doPresetWiring(sys) {
    sys.conns = [];
    const presetConns = [
        { from: 'ac_wire_u', to: 'r_u_wire_l', type: 'wire' },
        { from: 'r_u_wire_r', to: 'gnd_wire_gnd', type: 'wire' },
        { from: 'ac_wire_v', to: 'r_v_wire_l', type: 'wire' },
        { from: 'r_v_wire_r', to: 'gnd_wire_gnd', type: 'wire' },
        { from: 'ac_wire_w', to: 'r_w_wire_l', type: 'wire' },
...
function _connectMeggerToPhase(phase) {
    ...
    if (!megger) return;
    const phasePort = `ac_wire_${phase}`;
...
export const componentConfigs = [
    { Class: ACPower3P, id: 'ac', x: 30, y: 20, vRms: 220, freq: 50, isOn: true },
