...
/**
 * VariResistor — 可变电阻（滑动/旋转型）组件
 *
 * 说明：
 * - 提供可交互调整电阻值的视觉组件，支持水平或垂直布局；
 * - 内部保存 `currentResistance` 与 `maxResistance`，并通过 `addPort` 暴露端子；
 * - 静态部分使用缓存，交互通过旋钮/拖拽完成。
 */
export class VariResistor extends BaseComponent {
    ...
    initVisuals() {
        ...
        this._interactGroup.add(leadL, leadR, this.body, this.connectorLine, this.arrow);
