 ...
 */
    ...
    _updateACV(s) {
        ...
        if (!this._acvBuf) {
            this._acvBuf = new Float64Array(200);
            this._acvSumSq = 0;
            this._acvIdx = 0;
            this._acvCount = 0;
        }
        ...
        const rms = this._acvCount > 0 ? Math.sqrt(this._acvSumSq / this._acvCount) : 0;
    ...
    _updateMACurrent(s) {
        ...
        if (!this._acCBuf) {
            this._acCBuf = new Float64Array(200);
            this._acCSumSq = 0;
            this._acCIdx = 0;
            this._acCCount = 0;
        }
        ...
        const rms = this._acCCount > 0 ? Math.sqrt(this._acCSumSq / this._acCCount) : 0;
        this.update(rms * 1000);
    }

    _updateRES(s) {
        let iSet = 0;
        if (this.mode === 'RES200') iSet = 0.001;
        else if (this.mode === 'RES2k') iSet = 0.0001;
        else if (this.mode === 'RES200k') iSet = 0.000001;
        if (iSet === 0) return;
        const vPort = Math.abs(s.getPD(`${this.id}_wire_v`, `${this.id}_wire_com`) || 0);
        if (vPort > 0.2) {
            this.update(1000000);
        } else {
            this.update(vPort / iSet);
        }
    }
