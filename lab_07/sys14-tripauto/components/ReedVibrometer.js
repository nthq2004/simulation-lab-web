...
export class ReedVibrometer extends BaseComponent {
    ...
    _drawMountBase() {
        ...
        this._staticGroup.add(new Konva.Text({
            ...
            fontSize: Math.max(12, this.width * 0.026),
            fontFamily: 'Arial',
            fontStyle: 'bold',
            fill: '#2a1800',
            align: 'center',
    ...
    tick(dt) {
        ...
        this._reedStates.forEach(reed => {
            ...
            if (this.vibAmplitude <= 0.01 || this.vibFrequency <= 0) {
                // 无激励：直接指数衰减到零
                // 不能用 Euler 积分模拟弹簧振子，因为 wn² 对高频薄片极大，
                // 20fps 时间步长远超振荡周期，Euler 积分必然数值发散。
