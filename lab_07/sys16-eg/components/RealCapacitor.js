...
/**
 * RealCapacitor — 实物风格电容器组件
 *
 * 说明：
 * - 绘制带引脚的电容器外观，并提供 `capacitance`、`vLast`、`physCurrent` 等用于时域伴随模型的字段；
 * - 实现 `getCompanionModel` / `calculatePhysicalCurrent` 等接口以便与电路求解器集成（若存在）。
 */
export class RealCapacitor extends BaseComponent {
    constructor(config, sys) {
        ...
        this.vLast = config.vLast || 0;       
        this.physCurrent = 0; 
        // 测试标志：为真时隐藏容量标签，供考核/测试使用
        this.testFlag = config.testFlag || false;
        
        this.config = { id: this.id, capacitance: this.capacitance, testFlag: this.testFlag };
    ...
    initVisuals() {
        ...
        this._staticGroup.add(leadL, leadR, this.body, negBar, minusSign, topCap, this.label);
