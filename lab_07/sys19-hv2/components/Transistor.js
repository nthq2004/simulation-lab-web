...
/**
 * Transistor — 简化 BJT 晶体管视觉与仿真组件（NPN/PNP）
 *
 * 说明：
 * - 提供 NPN/PNP 晶体管的可视化表示与简化参数（`beta`, `vbeOn`, `vceSat`）供电路仿真器使用；
 * - 端口 ID 固定以便上层电路求解器匹配（`b`, `c`, `e`），并提供伴随模型接口（若需要）；
 * - 静态视觉使用缓存以提升性能。
 */
export class Transistor extends BaseComponent {
    ...
    initVisuals() {
        ...
        this._staticGroup.add(circle, baseBar, bLine, cLine, eLine, arrow);
    }
    ...
    getCompanionModel(vB, vC, vE) {
        ...
        // 极端情况：仅当 BE 结导通时强制超大电导顶住，截止时保持反向阻断（供指针表反接测量区分 C/E）
