...
export class Workflow {
    ...
    async _simulateAutoQuiz(quizConfig) {
        ...
        // 定位正确答案选项
        const answerIdx = quizConfig.answer;
        const targetBtn = optionNodes[Array.isArray(answerIdx) ? answerIdx[0] : answerIdx];
        if (targetBtn) {
            // 高亮正确选项
            targetBtn.style.borderColor = '#4caf50';
            targetBtn.style.background = '#e8f5e9';
            targetBtn.style.boxShadow = '0 0 12px rgba(76,175,80,.5)';

            // 创建箭头指示器
            const btnRect = targetBtn.getBoundingClientRect();
            const boxRect = box.getBoundingClientRect();
            const arrowX = boxRect.left - boxRect.left;  // 相对于 box
