...
export class Workflow {
    ...
    async _simulateAutoClick(targetId, subTarget) {
        ...
        // 停留 1.5 秒让学员看清楚（仅未使用箭头指示时）
        if (!(targets[0] && sys.comps[targets[0]] && sys.comps[targets[0]].getClickablePartCenter && subTarget)) {
            const comp = sys.comps[targets[0]];
            if (comp) {
                comp.highlight && comp.highlight(true);
