...
export class Workflow {
    ...
    async _executeSingleStep(idx) {
        ...
        if (step.mode === 'find') {
            ...
            // ── check 等有 act() 的步骤：先文字说明 + 箭头指示目标组件，再执行操作 ──
            if (step.msg) {
    ...
    /**
     * 模拟自动点击效果
     * 节奏：① 展示完整步骤说明（浮动提示 4s） → ② 箭头闪烁指向部件（3s） → ③ 停留 1.5s 让学员观察
    ...
    /**
     * 在目标部件位置绘制闪烁箭头（3 次闪烁后移除）
     * 圆圈居中圈住部件中心，箭头从左上指向中心
     */
    async _flashArrow(center) {
        const sys = this.sys;
        const absX = center.x;
        const absY = center.y;
        const pad = 40, len = 30, w = 16;
        // 从左上方向部件中心指去
        ...
        const circle = new Konva.Circle({
            x: absX, y: absY, radius: 24,
            stroke: '#e74c1c', strokeWidth: 2.8,
            dash: [7, 4], opacity: 1, listening: false,
        });
        sys.layer.add(circle);

        // ── 精确闪烁 3 次：ON 600ms → OFF 400ms，共 3 个周期 ──
        const ON_MS = 600, OFF_MS = 400;
        for (let i = 0; i < 3; i++) {
            // ON
    ...
    _flashDomElement(el, msg = '', dur = 3000) {
        return new Promise((resolve) => {
            ...
            arrow.textContent = '👉';
            arrow.style.cssText = `left:${r.left - 10}px;top:${r.top + r.height / 2 - 15}px;`;
    ...
    _ensureFlashDomStyle() {
        ...
        st.textContent = `
            .wf-dom-flash{
                position:fixed; border:3px dashed #e74c3c; border-radius:6px;
                pointer-events:none; z-index:99999; box-sizing:border-box;
                animation:wfPulse .6s ease-in-out infinite;
            }
            .wf-dom-arrow{ border:none; font-size:26px; line-height:1; animation:wfPulse .6s ease-in-out infinite; }
    ...
    async _simulateAutoQuiz(quizConfig) {
        ...
        if (correctNodes.length) {
            ...
            Object.assign(arrow.style, {
                ...
                filter: 'drop-shadow(0 0 6px rgba(76,175,80,.8))',
            });
