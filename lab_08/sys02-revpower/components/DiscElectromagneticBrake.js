...
export class DiscElectromagneticBrake extends BaseComponent {
    ...
    /** 可识别部件（工作流 find 用）：线圈、弹簧（上/下）、衔铁与摩擦片、制动盘、气隙调节旋钮 */
    _addClickableParts() {
        const k = this._knob;
        const pad = 12;   // 热区向外扩展 padding，方便选中
        this.addClickablePart('knob', k.cx - k.r - pad, k.cy - k.r - pad, (k.r + pad) * 2, (k.r + pad) * 2);

        // 螺旋复位弹簧（上、下各一根，独立可点击）
        const springLeft = this._springAnchorX;
        const springRight = this._armX + this._armOffsetMax + this._springR;
        const springH = this._springR * 2 + 4;
        this.addClickablePart('spring-top',
            springLeft, this._springY[0] - springH / 2,
            springRight - springLeft, springH);
        this.addClickablePart('spring-bottom',
            springLeft, this._springY[1] - springH / 2,
            springRight - springLeft, springH);

        // 电磁铁线圈（在弹簧之后添加，确保点击层级高于弹簧）
        const cr = this._coilRect;
        this.addClickablePart('coil', cr.x, cr.y, cr.w, cr.h);

        // 动衔铁 + 摩擦片（含导向套筒）
        const armLeft = this._armX;
        const armTop = this._diskCY - this._armH / 2;
        const armW = this._armW + this._padW;
        const armH = this._armH;
        this.addClickablePart('armature', armLeft, armTop, armW, armH);

        // 制动盘
        this.addClickablePart('disc',
            this._diskCX - this._diskR, this._diskCY - this._diskR,
            this._diskR * 2, this._diskR * 2);
    }

    /** 工作流 find 步骤箭头定位：返回部件中心（组件局部坐标） */
    getClickablePartCenter(partId) {
        ...
        switch (partId) {
            case 'coil': {
                const cr = this._coilRect;
                p = { x: cr.x + cr.w / 2, y: cr.y + cr.h / 2 };
                break;
            }
            case 'spring-top':
                p = { x: (this._springAnchorX + this._armX + this._armOffsetMax) / 2, y: this._springY[0] };
                break;
            case 'spring-bottom':
                p = { x: (this._springAnchorX + this._armX + this._armOffsetMax) / 2, y: this._springY[1] };
                break;
            case 'armature': {
                const armX = this._armX + (this._armW + this._padW) / 2;
                p = { x: armX, y: this._diskCY };
                break;
            }
            case 'disc':
                p = { x: this._diskCX, y: this._diskCY };
                break;
            case 'knob': {
                const k = this._knob;
                p = { x: k.cx, y: k.cy };
