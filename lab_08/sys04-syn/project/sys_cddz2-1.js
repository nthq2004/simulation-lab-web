    ...
    'close-interlock': {
        ...
        steps: [
            ...
            },
            {
                ...
                op: { type: 'btn', target: 'genpanel', part: 'btn-start' },
                async act() {
                    const sys = this.sys;
            ...
            },
            {
                ...
                op: { type: 'btn', target: 'genpanel2', part: 'btn-start' },
                async act() {
                    const sys = this.sys;
            ...
            },
            {
                ...
                op: { type: 'btn', target: 'genpanel2', part: 'btn-close' },
                async act() {
                    const sys = this.sys;
                    const sel = sys.comps.sync_sel;
                    if (sel.getPosition() !== 1) sel.switchTo(1);
                ...
                op: [
                    {
                        ...
                        async act() {
                            const sys = this.sys;
                            const g1 = sys.comps.gen1, g2 = sys.comps.gen2;
                            // 转动 2 号调速开关，将频率调至与汇流排一致（留 0.3Hz 微频差使同步表慢转）
                    ...
                    },
                    {
                        ...
                        async act() {
                            ...
                            // 观察同步表：等待相位差滑入 320°~340° 窗口（同步表指针指向 11 点方向）后才合闸，
                            // 避免"转完档位立即合闸"——体现操作者须等待相位接近同步点再并车
                            sys.showFloatingTip('观察同步表：等待相位差滑入 320°～340° 窗口（指针近 11 点）时按\"合闸\"', 5000);
                            let hit = false;
                            for (let i = 0; i < 400 && !hit; i++) {
                                const d = degOf();
                                if (d >= 320 && d <= 340) hit = true;
    ...
    'sync-protect': {
        ...
        steps: [
            ...
            },
            {
                ...
                op: { type: 'switch', target: 'sync_sel', part: 'sel-knob' },
                async act() {
                    const sys = this.sys;
                    const sel = sys.comps.sync_sel;
                    if (sel.getPosition() !== 3) sel.switchTo(3);
            ...
            },
            {
                ...
                op: { type: 'knob', target: 'genpanel2', part: 'knob' },
                async act() {
                    const sys = this.sys;
            ...
            {
                msg: '第 10 步：演示二（非同期保护）——微调 2 号机频率，使频差小于0.2Hz', mode: 'check',
                op: { type: 'knob', target: 'genpanel2', part: 'knob' },
                async act() {
                    const sys = this.sys;
                    const g1 = sys.comps.gen1, g2 = sys.comps.gen2;
    ...
    'reverse-power': {
        ...
        steps: [
            ...
            },
            {
                ...
                op: { type: 'fault', fault: 'gen2_coolant_temp' },
                async act() {
                    const sys = this.sys;
