...
export class ImpellerFlowIndicator extends BaseComponent {
    constructor(config, sys) {
        ...
        this._magCoupling  = 1.97;        // 磁耦合系数（标定：流量1.0时指针满偏）
    ...
    _drawBackground() {
        ...
        this._staticGroup.add(new Konva.Rect({
            ...
            fill: '#7b7b9c', stroke: '#2e2e36', strokeWidth: 1.5,
            cornerRadius: 7,
    ...
    _drawScaleTicks() {
        ...
        const nMajor = 5;   // 0,20,40,60,80,100%（即 0, Qmax/5, … Qmax）
        ...
        for (let i = 0; i <= totalTicks; i++) {
            ...
            if (isMajor) {
                const qVal = (this.Qmax * frac).toFixed(1);
                const lx = cx + (R * 0.82 + 16) * Math.cos(rad);
                const ly = cy + (R * 0.82 + 16) * Math.sin(rad);
                scaleG.add(new Konva.Text({
                    x: lx - 14, y: ly - 6, width: 28, height: 16,
                    text: qVal, fontSize: 12, fontFamily: 'monospace',
        ...
        scaleG.add(new Konva.Text({
            x: cx - 20, y: cy - R * 0.22,
            width: 40,
            text: this.unit, fontSize: 12, fontStyle: 'bold',
    ...
    _drawStaticLabels() {
        ...
        this._staticGroup.add(new Konva.Text({
            x: 0, y: -24, width: W,
            text: `${this.label}  叶轮流量指示器}`,
            fontSize: 18, fontStyle: 'bold', fill: '#070d18', align: 'center',
        }));
    }
    ...
    _initDynamicNodes() {
        ...
        // ── 叶轮组（scale 3x 使转动的叶轮更醒目）──
        this._nImpellerG = new Konva.Group({ x: cx, y: cy, scaleX: 2, scaleY: 2 });
        ...
        this._nPtrText = new Konva.Text({ x:0, y:0, width:36, text:'', fontSize:16, fontFamily:'monospace', fontStyle:'bold', fill:'#e03020', align:'center', shadowColor:'#fff', shadowBlur:2, shadowOpacity:0.7 });
    ...
    _updatePointer() {
        ...
        this._nPtrText.text(qRead.toFixed(2));
    }
