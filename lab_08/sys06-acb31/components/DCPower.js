...
export class DCPower extends BaseComponent {
     ...
     * @param {string} partId 'power' | 'knob' | 'lcd'
     */
    getClickablePartCenter(partId) {
        ...
        const rel = {
            'power': { x: (12 + 16.5) * this.scale, y: ctrlY + 10 * this.scale },  // 电源键中心
            'knob':  { x: this.width - 50 * this.scale, y: ctrlY + 10 * this.scale }, // 电压旋钮中心
            'lcd':   { x: this.width / 2, y: 33 * this.scale },                     // 液晶屏中心
        };
        const p = rel[partId];
