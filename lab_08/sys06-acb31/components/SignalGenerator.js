...
export class SignalGenerator extends BaseComponent {
    ...
    _initVisuals() {
        ...
        // ── 静态元素 → _staticGroup（由系统统一缓存） ──────────────
        this._staticGroup.add(
            body, bodyTop, brandText, modelText,
            screen, divider,
            knobOuter, knobInner, knobLabel,
        );
        // ── 动态元素（用户交互/每帧更新）→ _interactGroup（不缓存，实时渲染） ──
        this._interactGroup.add(
        ...
        portLabels.forEach(pl => {
            ...
            this._staticGroup.add(bnc, bncDot);
        });
    ...
    _buildButtons(C) {
        ...
        this._interactGroup.add(this._stepDisplayBg, this._stepDisplayText);
        // 复位
