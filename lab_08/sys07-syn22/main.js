...
// 1. 定义按钮 ID 与对应操作的映射表
// 同步表勾选状态下的接线恢复：接线/起动系统会清空连线，
// 若勾选框已勾选（同步表显示），需自动补回同步表的 3 条接线。
const syncShowWires = () => {
    if (!sys || !sys.connMgr || !sys.comps) return;
    const cb = document.getElementById('syncShow');
    if (!cb || !cb.checked) return;
    SYNC_WIRES.forEach(c => sys.connMgr.addConn(c));
};
const actionMap = {
    'btnUndo': () => sys.history.undo(),
    'btnRedo': () => sys.history.redo(),
    'btnAutoWire': () => { sys.applyAllPresets(); syncShowWires(); },
    'btnStartSys': () => { sys.applyStartSystem(); syncShowWires(); },
...
// 同步表显隐勾选框（放在「选择仪表」之后，勾选显示 / 取消隐藏 sync1 组件）
// 勾选显示时自动连接同步表接线（汇流排A相 + 待并机A相(经选择开关 COM) + 接地参考），
// 取消勾选隐藏时删除这些接线。
const SYNC_WIRES = [
...
const syncShow = document.getElementById('syncShow');
if (syncShow) {
    syncShow.addEventListener('change', () => {
        const visible = syncShow.checked;
        sys.toggleInstrumentVisibility('sync1', visible);
        if (!sys || !sys.connMgr) return;
        if (visible) {
            SYNC_WIRES.forEach(c => sys.connMgr.addConn(c));
        } else {
            SYNC_WIRES.forEach(c => sys.connMgr.removeConn(c));
        }
    });
}
