...
export class Workflow {
    ...
    async _simulateAutoClick(targetId, subTarget, msg) {
        ...
        if (subTarget) {
            var partNames = {
                ...
                'armature': '衔铁与摩擦片', 'disc': '制动盘', 'knob': '气隙调节旋钮',
            };
    ...
    _compCenter(comp) {
        ...
        try {
            const pos = comp.group.getAbsolutePosition();
            const box = comp.group.getClientRect({ relativeTo: comp.group });
            if (pos && box && box.width > 0 && box.height > 0) {
                return { x: pos.x + box.width / 2, y: pos.y + box.height / 2 };
            }
            return null;
        } catch (e) {
            return null;
    ...
    async _animateOpIntro(step, opObj) {
        ...
        if (type === 'wire') await this._introAutoWire(step);
    ...
    async _introAutoWire(step) {
        const btn = document.getElementById('btnAutoWire');
        if (btn) {
            await this._flashDomElement(btn, step.msg || '点击工具栏"自动接线"按钮', 2600);
        } else if (step.msg) {
            this.sys.showFloatingTip(step.msg, 3000);
            await new Promise(r => setTimeout(r, 2000));
        }
    ...
    async _introPointPart(step, op, actionText) {
        ...
        const part = op.part || step.part || step.subTarget;
        if (tid) {
            const comp = sys.comps[tid];
            if (comp) {
                const center = comp.getClickablePartCenter && part
                    ...
                    : this._compCenter(comp);
                const tip = step.msg || `👉 ${actionText}：${(comp.type || tid)}`;
                if (center && sys.layer) {
    ...
    /**
     * 设置故障（规则5）：指向工具栏"故障设置"按钮 → 打开故障界面 → 指向并勾选故障项 → 点击"应用设置"保存
     * @param {object} step 步骤对象
     * @param {object} op 操作元数据 { fault: 故障id }
     */
    async _introFault(step, op) {
        ...
        sys.showFloatingTip(step.msg || '设置故障', 5000);
        ...
        if (fid) {
            ...
            if (cb) {
                ...
                await this._flashDomElement(label, `勾选故障：${(cb.parentNode && cb.parentNode.textContent || fid).trim()}`, 2000);
                await new Promise(r => setTimeout(r, 1500));
                cb.checked = true;
            } else {
                await new Promise(r => setTimeout(r, 1500));
        ...
        if (applyBtn) {
            await this._flashDomElement(applyBtn, '点击"应用设置"保存故障', 1800);
            await new Promise(r => setTimeout(r, 1500));
    ...
    async _introInstrument(step, op) {
        ...
        if (instId === 'heavyload') {
            ...
            sys.showFloatingTip(step.msg || '勾选"重载询问面板"，调出重载询问面板并自动接线', 5000);
            if (!cb) {
                await new Promise(r => setTimeout(r, 2000));
                return;
            }
        ...
        sys.showFloatingTip(step.msg || '选择仪表', 5000);
