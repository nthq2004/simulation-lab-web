...
export class HeavyLoadInquiry extends BaseComponent {
     ...
     * @param {string} partId 'btn'（起动/询问）| 'btn-stop'（停止）| 'knob'（选择开关）| 'led-run' | 'led-resp'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'btn')       return { x: gx + this._btnRect.x + this._btnRect.w / 2, y: gy + this._btnRect.y + this._btnRect.h / 2 };
        if (partId === 'btn-stop')  return { x: gx + this._btnStop.x + this._btnStop.w / 2, y: gy + this._btnStop.y + this._btnStop.h / 2 };
        if (partId === 'knob')      return { x: gx + this._knobMode.x, y: gy + this._knobMode.y };
        if (partId === 'led-run')   return { x: gx + this._ledRun.x,  y: gy + this._ledRun.y };
        if (partId === 'led-resp')  return { x: gx + this._ledResp.x, y: gy + this._ledResp.y };
        return null;
    }
