...
export class ACPower extends BaseComponent {
    constructor(config, sys) {
        ...
        // 可调参数（默认值）
        this.vRms = config.vRms !== undefined ? config.vRms : 24;
        ...
        this.config = { id: this.id, vRms: this.vRms, freq: this.freq, phaseDeg: this.phase * 180 / Math.PI, isOn: this.isOn, label: this.label };

        // 按初始状态同步面板显示（LCD、电源按钮、旋钮指针），不做缓存刷新
        this._syncDisplay();
    ...
    getClickablePartCenter(partId) {
        ...
        switch (partId) {
            case 'power': return { x: gx + 27,    y: gy + 95 };    // 电源按钮中心
            case 'lcd':   return { x: gx + 72.5,  y: gy + 38 };    // 数码屏中心
            case 'p':     return { x: gx + 100,   y: gy + 145 };   // 正端接线柱
            case 'n':     return { x: gx + 45,    y: gy + 145 };   // 负端接线柱
            default:      return null;
        }
    }
