...
export class CentrifugalTachometer extends BaseComponent {
    constructor(config, sys) {
        ...
        // 端口：传动轴输入（底部中央，管道口）
        this.addPort(
            this._shaftPort.x, this._shaftPort.y+8,
            'shaft', 'pipe'
        );
    }
    ...
    /** 位号标签 */
    _drawLabel() {
        const fontSize = Math.max(12, this.width * 0.048);
        this._staticGroup.add(new Konva.Text({
            x: 0, y: this._case.y - 14, width: this.width,
            text: `${this.label}    离心式转速表`,
    ...
    _drawFlyweightMechanism(frac) {
        ...
        // 从套筒右侧引出竖向推杆
        const rodX = cx + sw + 4;
        const rodTopY = this._flyArmPivY - 6;
        const leverH = this._flyArmPivY - this._mechTop - 8;
        // 推杆
        this._dynamicGroup.add(new Konva.Line({
            points: [rodX, sleeveY + sh / 2, rodX, rodTopY],
            stroke: '#e05040', strokeWidth: 2.5, lineCap: 'round', listening: false,
        }));
        // 推杆上端铰接
        this._dynamicGroup.add(new Konva.Circle({
            x: rodX, y: rodTopY, radius: 3,
            fill: '#e05040', stroke: '#b03020', strokeWidth: 0.8,
        }));
        // 推杆下端与套筒连接
        this._dynamicGroup.add(new Konva.Circle({
            x: rodX, y: sleeveY + sh / 2, radius: 2.5,
            fill: '#e05040', stroke: '#b03020', strokeWidth: 0.8,
        }));

        // 摇臂（三角形，从推杆上端到指针转轴，角度与指针一致）
        const rockerAngle = (this._needleAngleStart + frac * this._needleAngleRange) * Math.PI / 180;
        const rockerLen = this._flyArmLen * 0.4;
        const rockerEndX = rodX + rockerLen * Math.cos(rockerAngle);
        const rockerEndY = rodTopY + rockerLen * Math.sin(rockerAngle);
        this._dynamicGroup.add(new Konva.Line({
            points: [rodX, rodTopY, rockerEndX, rockerEndY],
            stroke: '#c0392b', strokeWidth: 3, lineCap: 'round', listening: false,
        }));
        // 摇臂末端（连接指针轴）
        this._dynamicGroup.add(new Konva.Circle({
            x: rockerEndX, y: rockerEndY, radius: 3.5,
            fill: '#c0392b', stroke: '#8a2010', strokeWidth: 0.8,
        }));
