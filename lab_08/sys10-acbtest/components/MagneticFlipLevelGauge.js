...
export class MagneticFlipLevelGauge extends BaseComponent {
    constructor(config, sys) {
        ...
        this.special = 'magnetic_flip_gauge';
        this.cache   = 'fixed';
        ...
        // 端口
        const loY = this._chamY + this._chamH - 5;
        const hiY = this._chamY + 5;
        this.addPort(this._chamX + this._chamW+15, hiY,  'hi', 'pipe', 'in');
        this.addPort(this._chamX + this._chamW+15, loY,  'lo', 'pipe');
        if (this.withTransmitter) {
            this.addPort(this.width-10, this._txY + 18, 'p', 'wire', 'p');
            this.addPort(this.width-10, this._txY + 48, 'n', 'wire');
        }
    }
    ...
    _drawScaleRuler() {
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            if (isMaj) {
                this._staticGroup.add(new Konva.Text({
                    x: sx - 26, y: ly - 5, width: 24,
                    text: Math.round(value).toString(),
                    fontSize: 10, fill: '#37474f', align: 'right',
                }));
            }
        }
        // 单位
        this._staticGroup.add(new Konva.Text({
            x: sx - 20, y: sy - 14, text: 'mm', fontSize: 10, fill: '#78909c',
        }));
        ...
        this._staticGroup.add(new Konva.Text({ x: sx + tw , y: hiY - 8, text: 'HH', fontSize: 10, fill: '#ef5350' }));
        this._staticGroup.add(new Konva.Text({ x: sx + tw , y: loY + 1, text: 'LL', fontSize: 10, fill: '#ffa726' }));
    ...
    _drawIndicatorColumn() {
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx-6, y: cy - 18, width: cw+10, text: '翻板柱', fontSize: 10.5, fontStyle: 'bold', fill: '#37474f', align: 'center' }));
    ...
    _drawMainChamber() {
        ...
        // 标注
        this._staticGroup.add(new Konva.Text({ x: cx, y: cy - 22, width: cw, text: '主腔', fontSize: 10.5, fontStyle: 'bold', fill: '#37474f', align: 'center' }));
    ...
    _drawFlipChips() {
        ...
        for (let i = 0; i < this.flipCount+1; i++) {
            const y = this._colY + i * this._chipH+10;
            // 翻板本体
    ...
    _drawTransmitter() {
        ...
        // 变送器外壳
        const body = new Konva.Rect({ x: tx, y: ty, width: tw, height: th, fill: '#d6dfe8', stroke: '#0d1520', strokeWidth: 1.5, cornerRadius: 4 });
        // 顶部标牌
        const nameBg = new Konva.Rect({ x: tx, y: ty, width: tw, height: 20, fill: '#dcdde6', stroke: '#1a237e', strokeWidth: 0.5, cornerRadius: [4,4,0,0] });
        this._staticGroup.add(new Konva.Text({ x: tx+2, y: ty+4, width: tw-4, text: '差压变送器', fontSize: 10.5, fontStyle: 'bold', fill: '#12f40e', align: 'center' }));
        this._staticGroup.add(new Konva.Text({ x: tx+2, y: ty+13, width: tw-4, text: '4~20mA', fontSize: 7, fill: '#9fa8da', align: 'center' }));

        // 接线端子区
        const termY = ty + th - 50;
        this._staticGroup.add(new Konva.Rect({ x: tx+4, y: termY, width: tw-8, height: 46, fill: '#dbe5ef', stroke: '#1a3040', strokeWidth: 1, cornerRadius: 2 }));
        ...
        this._txPctText  = new Konva.Text({ x: tx+6, y: ty+41, width: tw-12, text: '0.0 %', fontSize: 12, fontFamily: 'Courier New, monospace', fill: '#02eb30', align: 'center' });
        ...
        this._staticGroup.add(body, nameBg, this._txLcd, this._txCurrText, this._txPctText);
    }
    ...
    _drawMeterDisplay() {
        ...
        this._staticGroup.add(new Konva.Text({ x: mx+2, y: my+4, width: mw-4, text: '液位读数', fontSize: 8, fontStyle: 'bold', fill: '#4fc3f7', align: 'center' }));
        ...
        this._staticGroup.add(bg, this._meterLvPct, this._meterLvMM, this._meterAlarm);
    }
    ...
    _drawDrainValve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx-16, y: cy+16, text: '排污阀', fontSize: 10.5, fill: '#f86307' }));
