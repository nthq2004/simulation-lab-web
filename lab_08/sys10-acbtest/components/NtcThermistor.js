...
export class NTCThermistor extends BaseComponent {
    ...
    _drawProbeHousing() {
        ...
        // 管体高光
        this._staticGroup.add(new Konva.Rect({ x: cx2 - tubeW / 2 + 2, y: tubeY, width: 4, height: tubeH, fill: 'rgba(255,255,255,0.18)' }));
        ...
        // 珠体颜色随温度变化（动态）
        // 珠体高光
        this._staticGroup.add(new Konva.Ellipse({ x: cx2 - beadRX * 0.28, y: beadY + beadRY * 0.5, radiusX: beadRX * 0.30, radiusY: beadRY * 0.28, fill: 'rgba(255,255,255,0.35)' }));
        // 热辉光（底部散发）
        ...
        this._staticGroup.add(head, tube, this._beadGlow, this._beadEllipse, this._typeLbl);
    }
    ...
    _drawNTCElement() {
        ...
        this._staticGroup.add(this._crystalGroup);
    ...
    _drawLeads() {
        ...
        this._staticGroup.add(new Konva.Text({ x: cx2 - 22, y: this.height - 18, text: 'R+', fontSize: 10, fill: '#ef9a9a' }));
        this._staticGroup.add(new Konva.Text({ x: cx2 + 12, y: this.height - 18, text: 'R−', fontSize: 10, fill: '#b0bec5' }));
    ...
    _drawRTCurve() {
        ...
        this._staticGroup.add(new Konva.Text({ x: ox - 16, y: cy2 + 2, text: 'R(Ω)', fontSize: 10, fill: '#048b31' }));
        this._staticGroup.add(new Konva.Text({ x: cx2 + cw - 4, y: oy + 2, text: 'T(°C)', fontSize: 10, fill: '#00bcd4' }));
        ...
        [-20, 0, 25, 50, 80, 100, 120].forEach(T => {
            ...
            if (T % 50 === 0 || T === 25) this._staticGroup.add(new Konva.Text({ x: tx - 8, y: oy + 4, width: 16, text: T.toString(), fontSize: 8, fill: '#37474f', align: 'center' }));
        });
        ...
        [10, 100, 1000, 10000, 100000, 1000000].forEach(R => {
            ...
            this._staticGroup.add(new Konva.Text({ x: ox - 28, y: ry - 4, width: 24, text: rStr, fontSize: 10, fill: '#217608', align: 'right' }));
        });
        ...
        this._staticGroup.add(new Konva.Line({ points: curvePts, stroke: '#086774', strokeWidth: 2, lineJoin: 'round', opacity: 0.85 }));

        // 25°C 参考点标注
        const r25 = this.Rref;
        const tx25 = ox + (25 - tMin) / (tMax - tMin) * (aw - 2);
        const ry25 = oy - (Math.log10(r25) - logRMin) / (logRMax - logRMin) * (ah - 4);
        this._staticGroup.add(new Konva.Circle({ x: tx25, y: ry25, radius: 3.5, fill: '#785d04', stroke: '#f9a825', strokeWidth: 1 }));
        this._staticGroup.add(new Konva.Text({ x: tx25 - 30, y: ry25 - 12, text: `25°C\n${r25 >= 1000 ? (r25 / 1000).toFixed(0) + 'kΩ' : r25 + 'Ω'}`, fontSize: 10, fill: '#977508', lineHeight: 1.3 }));
        ...
        this._staticGroup.add(this._rtPoint, this._rtHLine, this._rtVLine, this._rtLabel);
    }
    ...
    _drawMeasCircuit() {
        ...
        // Vcc → Rs → NTC → GND 电路
        this._staticGroup.add(new Konva.Text({ x: cx2 + 4, y: cy2 + 6, text: `Vcc=${this.Vcc}V`, fontSize: 10, fill: '#8b0707' }));
        this._staticGroup.add(new Konva.Line({ points: [cx2 + 40, cy2 + 18, cx2 + 60, cy2 + 18], stroke: '#ef9a9a', strokeWidth: 1.5 }));

        // Rs 电阻符号
        const rsX = cx2 + 60, rsY = cy2 + 14;
        this._staticGroup.add(new Konva.Rect({ x: rsX, y: rsY, width: 32, height: 10, fill: 'none', stroke: '#ffd54f', strokeWidth: 1.5 }));
        this._staticGroup.add(new Konva.Text({ x: rsX + 2, y: rsY - 10, text: `Rs=${this.Rseries / 1000}k`, fontSize: 10, fill: '#775d06' }));
        this._staticGroup.add(new Konva.Line({ points: [rsX + 32, cy2 + 18, rsX + 50, cy2 + 18, rsX + 50, cy2 + 28], stroke: '#ffd54f', strokeWidth: 1.5 }));
        ...
        this._staticGroup.add(new Konva.Line({ points: ntcPts, stroke: '#00bcd4', strokeWidth: 2, lineJoin: 'round' }));
        // NTC 箭头（负温度系数）
        this._staticGroup.add(new Konva.Arrow({ points: [ntcX + 2, ntcY + 14, ntcX + 12, ntcY], stroke: '#00bcd4', fill: '#00bcd4', strokeWidth: 1, pointerLength: 3, pointerWidth: 3 }));
        this._staticGroup.add(new Konva.Text({ x: ntcX + 14, y: ntcY + 4, text: 'NTC', fontSize: 7, fill: '#00bcd4' }));
        this._staticGroup.add(new Konva.Line({ points: [ntcX + 5, ntcY + 14, ntcX + 5, cy2 + ch - 8, cx2 + 8, cy2 + ch - 8], stroke: '#b0bec5', strokeWidth: 1.5 }));
        ...
        this._staticGroup.add(this._circVoutLbl, this._circADCLbl, this._selfHeatLbl);
    }
    ...
    _drawPresetSelector() {
        ...
        this._staticGroup.add(new Konva.Text({ x: selX + 4, y: selY + 2, width: selW - 8, text: '型号预设 ', fontSize: 12, fontStyle: 'bold', fill: '#00bcd4', align: 'center' }));
