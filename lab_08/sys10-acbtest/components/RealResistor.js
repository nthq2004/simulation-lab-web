...
/**
 * RealResistor — 真实风格电阻组件（带色环/引脚）
 *
 * 说明：
 * - 可视化带引脚的固定电阻体，支持 `tolerance`、`currentResistance` 等属性；
 * - 提供颜色映射表与误差带色环显示，用于教学演示色环读数与实际阻值。
 */
export class RealResistor extends BaseComponent {
    constructor(config, sys) {
        ...
        // 测试标志：为真时隐藏阻值标签，供考核/测试使用
        this.testFlag = config.testFlag || false;

        this.config ={'id':this.id,'tolerance':this.tolerance,'currentResistance':this.currentResistance,'testFlag':this.testFlag};
        ...
        this._staticGroup.add(leadL, leadR, body);
    ...
    drawBands() {
        ...
        colors.forEach((color, i) => {
            ...
            this._staticGroup.add(band);
            this._bandRefs.push(band);
        });
    }
