...
export class Rotameter extends BaseComponent {
    ...
    _recalcGeometry() {
        ...
        this._hEff    = this._tubeH;                     // 初始化为管高，initFloat后更新
        ...
        this._hEff    = this._tubeH - this._hFloat;  // 有效行程 = 管高 - 转子高
   ...
   _initParameters(config){
    ...
    _initDynamicNodes() {
        ...
        this._nWaves = [];
        for (let i = 0; i < 3; i++) {
            const w = new Konva.Line({ points: [], stroke: '', strokeWidth: 2.5, lineCap: 'round' });
            this._nWaves.push(w);
        ...
        this._nParticlePool = [];
        for (let i = 0; i < 30; i++) {
            const c = new Konva.Circle({ x: 0, y: 0, radius: 2, fill: 'transparent', visible: false });
    ...
    _updateFloat() {
        ...
        const hFrac = Math.max(0, Math.min(1, (this._tubeBot - fy - this._hFloat * 0.5) / this._hEff));
    ...
    _tickSimulation(dt) {
        ...
        // 有效行程 = 管高 - 转子高
        const effH   = this._tubeH - this._hFloat;
        const hEq    = this._flow * effH;  // 平衡高度（px，从底部算）
        ...
        // 限位：转子不能超出管端（用有效行程保证 0~1 线性对应）
        const yMin = this._tubeTop + this._hFloat * 0.5;
        const yMax = this._tubeBot - this._hFloat * 0.5;
    ...
    getReading() {
        const hFrac = Math.max(0, Math.min(1,
            (this._tubeBot - this._floatY - this._hFloat * 0.5) / this._hEff
        ));
