...
/**
 * WaterTankTwoPos —— 水箱液位两位控制示意组件
 *
 * 概要：
 * - 提供一个包含：左侧水泵控制面板（本地/远程、启停）、中间水箱、右侧差压变送器与管路的交互式示意图；
 * - 采用差压法（ΔP ∝ h）测量液位，变送器内绘制 H（底部）与 L（顶部）取压口并连回水箱右侧；
 * - 本组件对外暴露顶部接线端口（左区为水泵控制开关，右区为差压开关 COM/NC 触点）。
 *
 * 布局（默认）：整体尺寸 W=480, H=280
 * - 左：控制面板（宽约 120px）包含模式开关、启动/停止按钮及水泵图示；
 * - 中：水箱区域（宽约 170px），带液位刻度与出水阀；
 * - 右：差压变送器（宽约 100px），H/L 取压口均位于变送器左侧并通过管路连至水箱右侧的顶/底部。
 */
export class WaterTankTwoPos extends BaseComponent {
    constructor(config, sys) {
        ...
        this.panelH = this.H / 2; // 控制面板高度（视觉用，高度可与水箱不同）
        ...
        this.area = this.tankW * this.tankH; // 视觉截面积（像素^2，用于绘制与比例计算）
        ...
        // ── 差压开关 (COM/NC 触点) ──
        this.pressureSwitch = {
            lowLimit: 45,          // 下限液位 (%) - 低于此值开关闭合
            highLimit: 55,         // 上限液位 (%) - 高于此值开关断开
            closed: false,         // 开关状态：true=闭合，false=断开
            fault: null
        };

        this.displayLevel = 0;

        this.config = { id: this.id, lowLimit: this.pressureSwitch.lowLimit, highLimit: this.pressureSwitch.highLimit };
        // ── 支持整体缩放的组 ──
        ...
        this._updatePressureSwitchOutput();
    }
    ...
    _drawTransmitter() {
        ...
        // ── 开关状态指示灯 ──
        const indicatorX = txX + txW / 2;
        const indicatorY = txY + 45;
        ...
        // 状态灯（可变）
        this._switchIndicator = new Konva.Circle({
            x: indicatorX, y: indicatorY,
            radius: 16,
            fill: '#888',  // 初始灰色（断开）
            stroke: '#333', strokeWidth: 1,
        });
        this.scaleGroup.add(this._switchIndicator);

        // 状态文字
        this._switchStatusText = new Konva.Text({
            x: txX + 8, y: txY + 85,
            width: txW - 16,
            text: '断开',
            fontSize: 14, fontStyle: 'bold',
            fill: '#666', align: 'center',
        });
        this.scaleGroup.add(this._switchStatusText);
        ...
        this.scaleGroup.add(new Konva.Text({
            ...
            width: txW - 10,
            text: 'COM / NC',
            fontSize: 9, fill: '#555', align: 'center',
        }));
    ...
    _drawExternalPorts() {
        ...
        // 差压开关接口（右半区）- COM 和 NC 触点
        const swComX = (this.W - 95) * this.scale;
        const swNcX = (this.W - 35) * this.scale;
        this._addTopPort(swComX, 60 * this.scale, 'COM');  // COM 触点
        this._addTopPort(swNcX, 60 * this.scale, 'NC', 'p');         // NC 触点
    }
    ...
    _updatePhysics() {
        ...
        // 根据截止阀状态调整出水流量
        this.outletFlowRate = this.outletValve.open ? this.outletValve.nominalFlow : 0;
    ...
    _updatePressureSwitchOutput() {
        // 根据液位和上下限，更新开关状态
        if (this.pressureSwitch.fault === 'open') {
            this.pressureSwitch.closed = false;
        } else if (this.pressureSwitch.fault === 'short') {
            this.pressureSwitch.closed = true;
        } else {
            // 低于下限时闭合，高于上限时断开
            if (this.level < this.pressureSwitch.lowLimit) {
                this.pressureSwitch.closed = true;
            } else if (this.level > this.pressureSwitch.highLimit) {
                this.pressureSwitch.closed = false;
            }
            this.isEnergized = !this.pressureSwitch.closed;
            // 在上下限之间时保持原状态（滞后特性）
        }
    }
    ...
    _updateUI() {
        ...
        // ── 差压开关显示 ──
        if (this.pressureSwitch.closed) {
            this._switchIndicator.fill('#00cc00');  // 绿色 = 闭合
            this._switchStatusText.text('闭合');
            this._switchStatusText.fill('#00aa00');
        } else {
            this._switchIndicator.fill('#cccccc');  // 灰色 = 断开
            this._switchStatusText.text('断开');
            this._switchStatusText.fill('#666666');
        }
    ...
    tick(dt) {
        ...
        if (this._tickAcc >= 0.25) {
            this._tickAcc = 0;
            this._updatePhysics();
        
        this._refreshCache();
    }
        this._uiTickAcc = (this._uiTickAcc || 0) + dt;
        if (this._uiTickAcc >= 0.5) {
            this._uiTickAcc = 0;
            this._updateUI();
        }
    }
    ...
    getConfigFields() {
        return [
            ...
            { label: '上限', key: 'highLimit', type: 'number' },
            { label: '下限 (%)', key: 'lowLimit', type: 'number' },
        ];
    }

    onConfigUpdate(cfg) {
        this.id = cfg.id || this.id;
        this.pressureSwitch.highLimit = parseFloat(cfg.highLimit) ?? this.hiAlarm;
        this.pressureSwitch.lowLimit = parseFloat(cfg.lowLimit) ?? this.loAlarm;
    ...
    getPressureSwitchState() { return this.pressureSwitch.closed; }
    ...
    setPressureSwitchLimits(low, high) {
        this.pressureSwitch.lowLimit = low;
        this.pressureSwitch.highLimit = high;
    }
...
export default WaterTankTwoPos;
