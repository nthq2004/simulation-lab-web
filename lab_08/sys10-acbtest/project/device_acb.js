...
export const PROJECT_WORKFLOWS = {
    'acb-close': {
        ...
        steps: [
            {
                msg: '1. 连接主回路：三相电源 → 断路器 → 电动机，并将电动机接成 Y 形', mode: 'check',
                async act() {
                ...
                check() {
                    ...
                    return c('ac_wire_u', 'acb_wire_l1')
                        ...
                        && c('acb_wire_t1', 'im01_wire_u1')
                        && c('acb_wire_t2', 'im01_wire_v1')
                        && c('acb_wire_t3', 'im01_wire_w1')
                        && c('im01_wire_u2', 'im01_wire_v2')
                        && c('im01_wire_u2', 'im01_wire_w2');
                },
            },
            {
                msg: '2. 开启三相电源（220V/50Hz），旋转调压旋钮至 380V', mode: 'check',
                async act() {
                    await new Promise(r => setTimeout(r, 300));
                ...
                check() {
                    const ac = this.sys.comps['ac'];
                    return ac && ac.isOn && Math.abs(ac.vRms - 220) < 1;
                },
            },
            {
                msg: '3. 闭合空气断路器（点击操作手柄），观察电动机起动过程和三相电流', mode: 'check',
                async act() {
                    await new Promise(r => setTimeout(r, 300));
            ...
            {
                msg: '4. 测试题：空气断路器合闸后的状态', mode: 'quiz',
                quizConfig: {
        ...
        steps: [
            ...
            },
            {
                ...
                async act() {
                    ...
                    if (ac) ac.onConfigUpdate({ vRms: 220, freq: 50, isOn: true, phaseSeq: 'pos' });
                    const acb = this.sys.comps['acb'];
                    if (acb) { acb.close(); setTimeout(() => acb.trip(), 600); }
            ...
            },
            {
                ...
                async act() {
                    await new Promise(r => setTimeout(r, 300));
                    const acb = this.sys.comps['acb'];
                    if (acb) { acb.open(); setTimeout(() => acb.close(), 400); }
...
export const componentConfigs = [
    { Class: ACPower3P, id: 'ac', x: 280, y: 60, vRms: 220, freq: 50, isOn: false, phaseSeq: 'pos', visible: true },
    { Class: ThreePhaseACB, id: 'acb', x: 250, y: 260, initState: 'off', label: 'QF', ratedVoltage: 380, ratedCurrent: 100, tripCurrent: 10, visible: true },
    {
        Class: InductionMotor, id: 'im01', x: 180, y: 540, visible: true,
...
function _autoWire(sys) {
    ...
    const cons = [
        ...
        { from: 'acb_wire_t1', to: 'im01_wire_u1', type: 'wire' },
        { from: 'acb_wire_t2', to: 'im01_wire_v1', type: 'wire' },
        { from: 'acb_wire_t3', to: 'im01_wire_w1', type: 'wire' },
