    ...
    'auto-parallel-decouple': {
        ...
        steps: [
            ...
            // ── 步骤 2：将电站切换为自动运行模式 ──
            {
                ...
                mode: 'check',
                op: { type: 'knob', target: 'auto_ctl', part: 'knob-mode',
                    async act() {
                        const sys = this.sys;
                        const ac = sys.comps.auto_ctl;
                        if (ac && !ac._auto) {
            ...
            // ── 步骤 3：加载 50kW，观察 2# 自动起动并车 ──
            {
                ...
                mode: 'check',
                op: [
                    { type: 'knob', target: 'load3', part: 'power',
                        async act() {
                            const sys = this.sys;
                            // 先将有功功率数值改为 50
                            ...
                            if (load) {
                                load.powerKw = 50;
                                load._recalcLoad();
                                if (load._ui) load._ui.power.text('50');
                                load.config.powerKw = 50;
                                load._touchDirty();
                            }
                            await _sleep(500);
                        } },
                    { type: 'btn', target: 'load3', part: 'btn-load',
                        async act() {
                            const sys = this.sys;
                            // 再点击"加载"按钮，加载 50kW 负载
                            const load = sys.comps.load3;
                            if (load) {
                                load._loaded = true;
    ...
    'lv1-fault-auto-switch': {
        ...
        steps: [
            ...
            // ── 步骤 3：触发 1# 机Ⅰ级故障，观察自动化电站自动换机 ──
            {
                ...
                mode: 'check',
                op: { type: 'fault', fault: 'gen1_lv1_fault',
                    async act() {
    ...
    'lv2-fault-power-switch': {
        ...
        steps: [
            ...
            // ── 步骤 3：触发 1# 机Ⅱ级故障，观察跳闸与备用机组自动恢复供电 ──
            {
                ...
                mode: 'check',
                op: { type: 'fault', fault: 'gen1_undervoltage',
                    async act() {
                        // 等待用户手动触发Ⅱ级故障（故障面板操作）：
    ...
    'heavyload-inquiry-practice': {
        ...
        steps: [
            ...
            // ── 步骤 2：调出重载询问面板，转为询问模式 ──
            {
                ...
                mode: 'check',
                op: [
                    { type: 'instrument', instrument: 'heavyload',
                        async act() {
                            const sys = this.sys;
                            // 演示勾选过程：勾选"重载询问面板"复选框，调出面板并自动接线。
                            // （闪烁指示由 _introInstrument 完成，此处确保面板显示 + 接线完备）
                            _showHeavyload(sys, true);
                            await _sleep(400);
                        } },
                    { type: 'knob', target: 'heavyload', part: 'knob',
                        async act() {
                            const sys = this.sys;
                            // 再将面板选择开关切换为"重载询问"模式
            ...
            // ── 步骤 3：按询问按钮，观察并车过程（回灯黄→绿） ──
            {
                ...
                mode: 'check',
                op: { type: 'btn', target: 'heavyload', part: 'btn',
                    async act() {
                        const sys = this.sys;
                        const hv = sys.comps.heavyload;
                        if (!hv) return;
            ...
            // ── 步骤 5：按停止按钮，重载退出，观察自动解列停机 ──
            {
                ...
                mode: 'check',
                op: { type: 'btn', target: 'heavyload', part: 'btn-stop',
                    async act() {
                        const sys = this.sys;
                        const hv = sys.comps.heavyload;
