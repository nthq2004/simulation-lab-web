    ...
    'mech-trip-recovery': {
        ...
        steps: [
            ...
            // ── 步骤 2：触发任意 1 种原动机机械故障 ──
            {
                ...
                op: [
                    {
                        type: 'fault', fault: 'gen1_overspeed',
                        msg: '打开故障设置面板，勾选"1#机超速故障"并应用（3 种原动机故障触发任意 1 种均可）'
                    },
                    {
                        type: 'observe', target: 'gen1',
                        msg: '观察 1# 同步发电机：原动机机械故障保护动作，单机运行立即停机',
                        async act() {
    ...
    'rev-power-recovery': {
        ...
        steps: [
            // ── 步骤 1：接线、起动 1# 机组、合闸供电 ──
            {
                ...
                op: [
                    {
                        type: 'wire', msg: '点击工具栏"自动接线"按钮完成全船电网接线（含 QF3 照明支路），并复位到初始状态',
                        async act() {
                            const sys = this.sys;
                            _autoWire(sys);
                            await _sleep(400);
            ...
            // ── 步骤 2：起动 2# 机组、并车、转移负荷 ──
            {
                ...
                op: [
                    ...
                    {
                        type: 'observe', target: 'sync1',
                        msg: '观察数字同步表：待并机与电网相位差进入允许区（接近同相）时准备并车合闸',
                        async act() {
                            const sc = this.sys.comps.sync1;
                            const degOf = () => { const d = (sc._phaseDiff || 0) * 180 / Math.PI; return (Math.round(d % 360 + 360)) % 360; };
                            await _waitUntil(() => { const d = degOf(); return d < 60 || d > 270; }, 20000);
                    ...
                    {
                        type: 'knob', target: 'genpanel2', part: 'knob',
                        msg: '微调两机设定频率转移负荷，使两机各承担总负荷约 50%',
                        async act() {
                            const sys = this.sys;
                            const g1 = sys.comps.gen1, g2 = sys.comps.gen2;
            ...
            // ── 步骤 3：加载 70kW，观察功率分配 ──
            {
                ...
                op: [
                    {
                        type: 'load', target: 'load3', part: 'power',
                        msg: '点击"三相可调负载"功率输入框，将有功功率设定为 70kW',
                        async act() {
                            const load = this.sys.comps.load3;
                            if (!load) return;
                            // 与功率输入框内联编辑确认等效：改设定值并刷新显示
                    ...
                    {
                        type: 'load', target: 'load3', part: 'btn-load',
                        msg: '再按"加载"按钮投入负载，观察两机各分担约 35kW',
                        async act() {
                            const load = this.sys.comps.load3;
                            if (!load) return;
                            // 与"加载"按钮点击处理等效：置加载状态并刷新 LCD
            ...
            // ── 步骤 4：触发 2# 冷却水高温故障（用户手动）──
            {
                ...
                op: [
                    ...
                    {
                        type: 'observe', target: 'qf1', part: 'main-contact',
                        msg: '观察 1# 主开关：1# 机被母线拖转出现逆功率，逆功率保护延时（约 5s）跳闸',
                        async act() {
                            // 等待任一主开关跳闸（保护动作先后视负荷转移而定）
                            await _waitUntil(() => {
                                const q1 = _fcomp('qf1'), q2 = _fcomp('qf2');
                                return (!!q1 && q1.getState() === 'off') || (!!q2 && q2.getState() === 'off');
                            }, 30000);
                            await _sleep(500);
                        }
                    },
                    {
                        ...
                        msg: '观察 2# 主开关：全部负荷转移到 2# 机引发过载，过载保护延时（约 15s）跳闸、全船失电',
                        async act() {
                            // 等待两个主开关都跳闸（约 5s + 15s，共约 20s）
                            await _waitUntil(() => {
                                const q1 = _fcomp('qf1'), q2 = _fcomp('qf2');
                                return !!q1 && !!q2 && q1.getState() === 'off' && q2.getState() === 'off';
                            }, 60000);
                            await _sleep(500);
                        }
                    },
            ...
            // ── 步骤 5：迅速合上 2# 机组主开关，恢复供电 ──
            {
                ...
                op: [
                    {
                        type: 'btn', target: 'genpanel2', part: 'btn-close',
                        msg: '2# 发电机仍在运行（过载只跳开关不停机），等弹簧重新储能后按 2# 遥控面板"合闸"直接恢复供电',
                        async act() {
                            const sys = this.sys;
                            await _closeQf(sys, 'genpanel2', 'qf2');
                            await _sleep(1500);
    ...
    'under-voltage-recovery': {
        ...
        steps: [
            // ── 步骤 1：自动接线，起动 1# 发电机，合闸供电 ──
            {
                ...
                op: [
                    {
                        type: 'wire', msg: '点击工具栏"自动接线"按钮完成全船电网接线，并复位到初始状态',
                        async act() {
                            const sys = this.sys;
                            _autoWire(sys);
                            await _sleep(400);
                ...
                op: [
                    {
                        ...
                        msg: '打开故障设置面板，勾选"1#机欠压故障"并应用'
                    },
                    {
                        type: 'observe', target: 'gen1',
                        msg: '观察 1# 同步发电机：调压器（AVR）故障，输出电压跌落至约 50% 额定值',
                        async act() {
                            await _waitUntil(() => { const g = _fcomp('gen1'); return !!g && g._faultAVR; }, 5000);
                            await _sleep(1500);
                        }
                    },
                    {
                        type: 'observe', target: 'qf1', part: 'main-contact',
                        msg: '观察 1# 主开关：欠压保护检测到电压跌落，延时（约 2s）动作跳闸（发电机不停机）',
                        async act() {
                            await _waitUntil(() => { const q = _fcomp('qf1'); return !q || q.getState() === 'off'; }, 15000);
                            await _sleep(500);
                        }
            ...
            // ── 步骤 3：迅速起动 2# 机组，合闸恢复供电 ──
            {
                ...
                op: [
                    {
                        type: 'btn', target: 'genpanel2', part: 'btn-start',
                        msg: '按住 2# 发电机组遥控面板"起动"按钮，迅速起动 2# 发电机，等待合闸弹簧储能（约 3s）',
                        async act() {
                            const sys = this.sys;
                            await _pressPanelBtn(sys, 'genpanel2', '_userStartPressed', 1200);
                            await _sleep(3000); // 等待储能
