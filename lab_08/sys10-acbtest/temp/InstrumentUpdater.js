...
export class InstrumentUpdater {
    ...
    _updateAmpmeter(dev) {
        ...
        // 使用 MNA 0V 电压源注入的 physCurrent（统一处理 multimeter 和 ammeter）
    ...
    _measureDiode(dev) {
        ...
        // 查找发光二极管（LED）
        const ledDevs = (s._cachedDevs && s._cachedDevs.ledDevs) || s.rawDevices.filter(d => d.type === 'led');
        const isLed = ledDevs.find(d => {
            const dA = s.portToCluster.get(`${d.id}_wire_l`);
            const dC = s.portToCluster.get(`${d.id}_wire_r`);
            return (vIdx === dA && comIdx === dC);
        });

        if (isLed) return 2.02;
        ...
        // 查找稳压二极管（正向偏置测量）
        const zenerDevs = (s._cachedDevs && s._cachedDevs.zenerDevs) || s.rawDevices.filter(d => d.type === 'zener');
        const isZener = zenerDevs.find(d => {
            const dA = s.portToCluster.get(`${d.id}_wire_l`);
            const dC = s.portToCluster.get(`${d.id}_wire_r`);
            return (vIdx === dA && comIdx === dC);
        });
        if (isZener) return 0.6767;
        ...
        const triodeMatch = transistorDevs.find(t => {
            ...
            if (!triodeMatch) {
                ...
                const igbtDevs = (s._cachedDevs && s._cachedDevs.igbtDevs) || s.rawDevices.filter(d => d.type === 'igbt');
                const igbtCEMatch = igbtDevs.find(d => {
                    const dC = s.portToCluster.get(`${d.id}_wire_c`);
                    const dE = s.portToCluster.get(`${d.id}_wire_e`);
                    return (vIdx === dE && comIdx === dC);
                });
                if (igbtCEMatch) return 0.6767;

                const mosfetDevs = (s._cachedDevs && s._cachedDevs.mosfetDevs) || s.rawDevices.filter(d => d.type === 'mosfet');
                const mosfetDSMatch = mosfetDevs.find(d => {
                    const dD = s.portToCluster.get(`${d.id}_wire_d`);
                    const dS = s.portToCluster.get(`${d.id}_wire_s`);
                    return (vIdx === dS && comIdx === dD);
                });
                if (mosfetDSMatch) return 0.6767;
            }
