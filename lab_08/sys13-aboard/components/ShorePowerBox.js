...
export class ShorePowerBox extends BaseComponent {
     ...
     * @param {string} partId 'phase-lamps'(相序指示灯) | 'knob'(相序转换旋钮) |
     *                        'breaker'(出口空气开关) | 'btn-close'(合闸按钮) | 'btn-open'(分闸按钮)
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        const rel = {
            'phase-lamps': { x: this._lampCX[1], y: this._lampCY },
            'knob':        { x: this._knobCX, y: this._knobCY },
            'breaker':     { x: 240, y: (this._brInY + this._brOutY) / 2 },
            'btn-close':   { x: this._btnCX[0], y: this._btnYL },
            'btn-open':    { x: this._btnCX[1], y: this._btnYL },
        };
        const p = rel[partId];
