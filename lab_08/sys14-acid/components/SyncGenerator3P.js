...
export class SyncGenerator3P extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // ── Ⅰ级（降级运行）故障标志 ──
        // 滑油压力下降 / 冷却水温度异常：仅为"报警级"异常，原动机继续运行、
        // 输出电压频率正常、主开关不动作；自动化电站（ShipAutoControl）检测到后
        // 执行自动换机（备用机组并车 → 转移负荷 → 解列故障机 → 停故障机）。
        // 注意：这些标志不参与上方原动机保护停机/拖转判定，也不参与
        // ShipAutoControl._hasFault（Ⅱ级）的"不可用机组"判断。
        this._lv1OilPress    = false; // Ⅰ级：滑油压力下降
        this._lv1CoolantTemp = false; // Ⅰ级：冷却水温度异常
            ...
            if (this._parMode && this._peers.length > 0) {
                ...
                if (leader === this) {
                    ...
                    if (this._parBenchFrames > 0) {
                        ...
                        // ── 负载下降（卸载）：基准实时下移跟随当前总负载（动态校零）──
                        // 否则显示功率停留在并车基准不降：_parDP 非负截断、_parBaseP 恒定，
                        // 卸载后 _displayP = (ratio+trans)×base 依旧等于并车基准，
                        // 面板显示与物理（真实 _pwr 已随支路断开骤降）脱节。
                        // 卸载时平滑下移基准，dp 保持 0，显示功率即按 ratio 同步下降。
                        if (tot < this._parTotalBase) {
                            this._parTotalBase = this._parTotalBase * 0.8 + tot * 0.2;
                            this._parBaseP = this._parTotalBase;
                            this._parDP = 0;
                        }
                    }
                    for (const p of this._peers) {
                        ...
                        p._parBaseP = this._parBaseP;
                        p._parTotalBase = this._parTotalBase;
                    }
    ...
    // ── Ⅰ级（降级运行）故障（供自动化电站 ShipAutoControl 故障换机触发）──
    // Ⅰ级故障不触发停机/拖转/保护跳闸，发电机组继续保持原状态运行，
    // 由自动化电站检测后执行自动换机（并车→转移→解列→停机）。
    getLv1Faults() {
        return {
            oilPress:    !!this._lv1OilPress,    // 滑油压力下降
            coolantTemp: !!this._lv1CoolantTemp, // 冷却水温度异常
        };
    }

    // 是否带任一Ⅰ级故障
    hasLv1Fault() {
        return !!((this._lv1OilPress || this._lv1CoolantTemp));
    }

    // 设置Ⅰ级故障：kind ∈ 'oilPress' | 'coolantTemp'；on=true 置位故障
    setLv1Fault(kind, on) {
        const map = { oilPress: '_lv1OilPress', coolantTemp: '_lv1CoolantTemp' };
        const k = map[kind];
        if (!k) return;
        this[k] = !!on;
        this.markDirty();
    ...
    /**
     * 返回可点击部件中心的世界坐标（供工作流箭头定位）
     * @param {string} partId 'start'（机旁起动按钮）| 'stop'（停止按钮）| 'knob'（调速旋钮）
     *                        | 'mode-switch'/'switch'（控制方式旋钮）| 'lcd'（液晶显示屏）
     */
    getClickablePartCenter(partId) {
        ...
        const c = this._ctrl;
        const rel = {
            'start': { x: c.start.x + c.start.w / 2, y: c.start.y + c.start.h / 2 },
            'stop':  { x: c.stop.x + c.stop.w / 2,   y: c.stop.y + c.stop.h / 2 },
            'knob':  { x: c.knob.x, y: c.knob.y },
            'mode-switch': { x: c.sw.x, y: c.sw.y + 5 },
            'switch':      { x: c.sw.x, y: c.sw.y + 5 },
            'lcd':   { x: c.lcd.x + c.lcd.w / 2, y: c.lcd.y + c.lcd.h / 2 },
        };
        const p = rel[partId];
