...
export class MarineElectronicTrip extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // CT/PT 变比：脱扣器采集互感器副边的电流/电压，按变比还原为原边值供显示与保护
        // （未接互感器、直接采集原边时保持默认 1）
        this._ctRatio = config.ctRatio !== undefined ? Math.max(1e-6, parseFloat(config.ctRatio)) : 1;
        this._ptRatio = config.ptRatio !== undefined ? Math.max(1e-6, parseFloat(config.ptRatio)) : 1;
        ...
        // 保护已投入标志：一旦稳定过即置位，之后即使电流波动也不退出保护
        this._protEngaged = false;
        this._prevU = 0;
        this._prevI = 0;
    ...
    tick(dt) {
        if (this.sys && this.sys.voltageSolver) {
            ...
            // 采集互感器副边值，再按 CT/PT 变比还原为原边值
            const uInst = getPD('u+', 'u-') * this._ptRatio;
            // 电流由求解器 0V 电压源注入测得（参照 ElecMeter/Wattmeter）
            const iInst = ((typeof this.physCurrent === 'number' && isFinite(this.physCurrent)) ? this.physCurrent : 0) * this._ctRatio;
    ...
    _updateStability() {
        ...
        // 一旦稳定过即保持保护投入（负载投切会使电压电流短时波动，不应使已投入的保护退出、
        // 否则过载/短路计时会被反复清零而无法动作）
        if (this._stable) this._protEngaged = true;
    }

    _evaluateProtection(dt) {
        // 未供电或保护尚未投入：保护不投入
        if (!this._powered || !this._protEngaged) {
            this._active = null;
            this._timer = 0;
            return;
        }
    ...
    getConfigFields() {
        return [
            ...
            { label: '电流互感器变比 Kct（副边=原边/Kct）', key: 'ctRatio', type: 'number', step: 0.1 },
            { label: '电压互感器变比 Kpt（副边=原边/Kpt）', key: 'ptRatio', type: 'number', step: 0.1 },
        ];
    }
    ...
    onConfigUpdate(cfg) {
        ...
        if (cfg.ctRatio !== undefined) { const v = parseFloat(cfg.ctRatio); if (Number.isFinite(v)) this._ctRatio = Math.max(1e-6, v); }
        if (cfg.ptRatio !== undefined) { const v = parseFloat(cfg.ptRatio); if (Number.isFinite(v)) this._ptRatio = Math.max(1e-6, v); }

        // 回写配置，保证再次打开参数对话框时能显示当前值（用规范化后的内部值，避免空输入写入 NaN）
        this.config = { ...this.config, ...cfg, ctRatio: this._ctRatio, ptRatio: this._ptRatio };
    }

    /**
     * 返回可点击部件中心坐标（供工作流 find/箭头指示定位）
     * @param {string} partId 'lcd'(液晶屏) | 'knob'(整定旋钮区) | 'reset'(复位按钮) |
     *                        'knob1'~'knob5'(第一排整定旋钮) | 其他/省略 = 面板中心
     */
    getClickablePartCenter(partId) {
        ...
        switch (partId) {
            case 'lcd':
                rel = { x: W / 2, y: this._lcdRect ? this._lcdRect.y + this._lcdRect.height / 2 : 255 };
                break;
            case 'reset':
                rel = { x: this._resetBtn ? this._resetBtn.x : W - 32, y: this._resetBtn ? this._resetBtn.y : 32 };
                break;
            case 'knob':
                rel = { x: this._knobXs[2], y: this._knobRow1 };   // 第一排中间旋钮
                break;
            default:
                if (partId && /^knob[1-5]$/.test(partId)) {
                    const i = parseInt(partId.slice(4), 10) - 1;
                    rel = { x: this._knobXs[i], y: this._knobRow1 };
                } else {
                    rel = { x: W / 2, y: H / 2 };                   // 面板中心
                }
        }
