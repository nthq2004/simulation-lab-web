...
export class Hydrometer extends BaseComponent { // 声明比重计组件类，继承自 BaseComponent
    ...
    _findTargetBattery() {
        ...
        for (const id in comps) {
            const c = comps[id];
            if ((c.type === 'leadacid_battery' || c.type === 'single_leadacid_battery') && c._lastOpenIdx !== undefined && c._lastOpenIdx >= 0) {
                const bx = c.group.x();
                const by = c.group.y();
                const holeX = bx + c._cellCx[c._lastOpenIdx];
                const holeY = by + (c._bodyTop || 0) + ((c._bodyH || 0) / 2) + (c.type === 'single_leadacid_battery' ? 30 : 0);
    ...
    getClickablePartCenter(partId) {
        ...
        switch (partId) {
            case 'bulb':  return { x: gx + this._tubeX * s, y: gy + this._bulbY * s }; // 顶部吸球
            case 'tube':  return { x: gx + this._tubeX * s, y: gy + (this._tubeTop + this._tubeH / 2) * s }; // 玻璃管中部
            case 'scale': return { x: gx + this._tubeX * s, y: gy + (this._tubeTop + this._tubeH * 0.6) * s }; // 浮子刻度区
            default:      return null;
        }
    }
