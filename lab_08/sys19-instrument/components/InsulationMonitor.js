...
export class InsulationMonitor extends BaseComponent {
     ...
     * @param {string} partId 'btn-mute' | 'btn-flash' | 'btn-reset' | 'lcd'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId && partId.indexOf('btn-') === 0) {
            const b = this._btns && this._btns[partId.slice(4)];
            if (b) return { x: gx + b.x + b.w / 2, y: gy + b.y + b.h / 2 };
        }
        if (partId === 'lcd' && this._lcd) {
            return { x: gx + this._lcd.x + this._lcd.w / 2, y: gy + this._lcd.y + this._lcd.h / 2 };
        }
