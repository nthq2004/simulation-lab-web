...
export class MotorControlBox extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // 电源各相进线对箱体 PE 的绝缘电阻（Ω），供兆欧表读取；Infinity 表示未建模
        this._lineInsulR = (config.insulR !== undefined && config.insulR !== null)
    ...
    getClickablePartCenter(partId) {
        ...
        if (!local) {
            ...
            local = rel[partId] || null;
        }
        if (!local) return null;
        // 组件可能带位移与缩放（本工程 scale=2/3），必须换算到画布坐标
        try {
            const stagePt = this.group.getAbsoluteTransform().point({ x: local.x, y: local.y });
            const layer = this.sys && this.sys.layer;
            if (layer) return layer.getAbsoluteTransform().copy().invert().point(stagePt);
            return stagePt;
        } catch (e) {
            const s = this.group ? this.group.scaleX() : 1;
            const gx = this.group ? this.group.x() : 0;
            const gy = this.group ? this.group.y() : 0;
            return { x: gx + local.x * s, y: gy + local.y * s };
        }
    }
