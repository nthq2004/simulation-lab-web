...
- **接线动画（通用要求）**：自动演示中凡涉及接线，若本次接线数量 **少于 6 根**，一律使用动画接线（`sys.connMgr.addConnectionAnimated({ from, to, type: 'wire' })`，约 3s/根，逐根 `await`，操作函数声明为 `async`）；接线数量 ≥6 根时可用瞬时接线（`addConn`）。
