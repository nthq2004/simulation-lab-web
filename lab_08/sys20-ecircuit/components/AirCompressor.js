 ...
 * - 内部变量包括 `running`、`power`、`targetPower` 与 `crankAngle`，并支持 `tick` 周期更新；
 * - 静态视觉元素加入缓存以提升渲染性能。
 */
export class AirCompressor extends BaseComponent {
    ...
    _drawShell() {
        ...
        this._staticGroup.add(this.crankcase);
    }
    ...
    _drawTopControlPanel() {
        ...
        // 旋钮标注
        panelGroup.add(new Konva.Text({ x: startX - 35, y: centerY - 18, text: "LOC", fontSize: 10, fontStyle: 'bold' }));
        panelGroup.add(new Konva.Text({ x: startX + 15, y: centerY - 18, text: "REM", fontSize: 10, fontStyle: 'bold' }));
        ...
        this._staticGroup.add(panelGroup);
    }
