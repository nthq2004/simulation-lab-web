...
export class IRThermometer extends BaseComponent {
    constructor(config, sys) {
        ...
        // ── 模拟输出参数 ──
        // dcVoltage：组件当前输出电压（V），由测量值映射得出
        this.dcVoltage = config.dcVoltage !== undefined ? parseFloat(config.dcVoltage) : 0;
        // 测温量程（用于把温度映射到 0-5V）
        this.rangeMin = config.rangeMin !== undefined ? parseFloat(config.rangeMin) : 0;
        this.rangeMax = config.rangeMax !== undefined ? parseFloat(config.rangeMax) : 100;
        ...
        // 端口注册
        this.addPort(this._portSignal.x, this._portSignal.y, 'p', 'wire', 'p');
        this.addPort(this._portGND.x,    this._portGND.y,    'n',    'wire');
    }
    ...
    _init() {
        ...
        this._staticGroup.add(this._laserGroup);
        this._staticGroup.add(this._bodyTopGroup);
        this._staticGroup.add(this._lcdGroup);
        this._staticGroup.add(this._irGlowGroup);
        this._staticGroup.add(this._triggerGroup);
    ...
    _bindInteraction() {
        ...
        this._interactGroup.add(hitRect);
    ...
    tick(dt) {
        ...
        if (this._measuring) {
            ...
            if (this._sampleTimer >= this._sampleInterval) {
                ...
                // 计算并更新 dcVoltage：将测得温度线性映射到 0-5V（超出范围夹紧）
                const span = Math.max(1e-6, this.rangeMax - this.rangeMin);
                let v = 5 * (this._measuredTemp - this.rangeMin) / span;
                v = Math.max(0, Math.min(5, v));
                this.dcVoltage = Math.round(v * 1000) / 1000; // 保留到 mV 精度
    ...
    getConfigFields() {
        return [
            ...
            { label: 'DC 输出电压 (V, 只读)', key: 'dcVoltage',   type: 'number' },
        ];
    }
    ...
    onConfigUpdate(cfg) {
        ...
        if (cfg.rangeMin   !== undefined) this.rangeMin   = parseFloat(cfg.rangeMin);
        if (cfg.rangeMax   !== undefined) this.rangeMax   = parseFloat(cfg.rangeMax);
        if (cfg.dcVoltage  !== undefined) this.dcVoltage  = parseFloat(cfg.dcVoltage);
