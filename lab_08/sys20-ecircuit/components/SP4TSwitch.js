...
export class SP4TSwitch extends BaseComponent {
     ...
     * @param {string} partId 'sel-knob'(手柄) | 'com' | 't1'~'t4'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'sel-knob' || partId === 'knob') {
            return { x: gx + this._knobCenter.x, y: gy + this._knobCenter.y };
        }
        if (partId === 'com') {
            return { x: gx + this._schemCOM.x, y: gy + this._schemCOM.y };
        }
        if (/^t[1-4]$/.test(partId || '')) {
            const t = this._schemT[parseInt(partId[1]) - 1];
            return t ? { x: gx + t.x, y: gy + t.y } : null;
        }
        return null;
    }
