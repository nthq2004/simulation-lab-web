...
export const PROJECT_WORKFLOWS = {
    ...
    'ups-startup': {
        ...
        steps: [
            {
                msg: '第 1 步：点击工具栏"自动接线"，接通主电网。',
                mode: 'check',
                op: 'wire',
                check() {
                    const c = (a, b) => this.sys.isPortConnected(a, b);
                    // 主电网已接通：同步发电机 → 主开关 → 汇流排
                    const mainOk = c('gen1_wire_u', 'qf1_wire_t1') && c('gen1_wire_v', 'qf1_wire_t2')
                        && c('gen1_wire_w', 'qf1_wire_t3')
                        && c('qf1_wire_l1', 'bus1_wire_l1_1') && c('qf1_wire_l2', 'bus1_wire_l2_1')
                        && c('qf1_wire_l3', 'bus1_wire_l3_1');
                    // UPS 电源输入未纳入自动接线
                    const upsInputUnwired = !c('bus1_wire_l1_5', 'ups1_wire_in_p')
                        && !c('gnd1_wire_gnd', 'ups1_wire_in_n');
                    return mainOk && upsInputUnwired;
                },
                async act() {
                    // 自动演示时执行一键自动接线（不含 UPS 电源输入）
                    _autoWire(this.sys);
                },
            },
            {
                msg: '第 2 步：UPS 输入 L 端接电源 A 相（汇流排第5接口），输入 N 端接地。',
                mode: 'check',
                op: [
                    { type: 'observe', target: 'ups1', part: 'in_p', msg: 'UPS 输入 L 端：接汇流排第 5 接口 A 相',
                      async act() {
                          // 箭头指向输入 L 端口，动画连上电源 A 相
                          await this.sys.addConnectionAnimated({ from: 'bus1_wire_l1_5', to: 'ups1_wire_in_p', type: 'wire' });
                      } },
                    { type: 'observe', target: 'ups1', part: 'in_n', msg: 'UPS 输入 N 端：接地',
                      async act() {
                          // 箭头指向输入 N 端口，动画连上保护地
                          await this.sys.addConnectionAnimated({ from: 'gnd1_wire_gnd', to: 'ups1_wire_in_n', type: 'wire' });
                      } },
                ],
                check() {
                    const c = (a, b) => this.sys.isPortConnected(a, b);
                    return c('bus1_wire_l1_5', 'ups1_wire_in_p') && c('gnd1_wire_gnd', 'ups1_wire_in_n');
                },
            },
            {
                msg: '第 3 步：在发电机组遥控面板上按"起动"按钮起动发电机，待主开关储能完成后合闸；再合上 UPS 输入电源开关，观察 LCD 显示输入/输出电压与电池参数。',
                mode: 'check',
                op: [
                    { type: 'btn',    target: 'genpanel', part: 'btn-start', msg: '按遥控面板"起动"按钮，起动发电机',
                      async act() {
                          await _pressPanelBtn(this.sys, 'genpanel', '_userStartPressed', 1200);
                          await _waitFor(() => this.sys.comps.gen1.isOn, 8000);   // 等发电机运行
                      } },
                    { type: 'btn',    target: 'genpanel', part: 'btn-close', msg: '待主开关储能完成后，按"合闸"按钮合闸',
                      async act() {
                          const qf1 = this.sys.comps.qf1;
                          await _waitFor(() => qf1._charged, 8000);               // 等主开关储能完成
                          await _pressPanelBtn(this.sys, 'genpanel', '_userClosePressed', 700);
                          await _waitFor(() => qf1._state === 'on', 8000);        // 等合闸到位
                      } },
                    { type: 'switch', target: 'ups1',     part: 'inputSwitch', msg: '合上 UPS 输入电源开关',
                      async act() {
                          const ups = this.sys.comps.ups1;
                          ups.setSwitchState('inputSwitch', true);
                          await _waitFor(() => ups._inputOk, 5000);               // 等输入电压有效
                      } },
                ],
                check() {
                    const sys = this.sys;
            ...
            {
                msg: '第 4 步：合上储能电池开关，合上 UPS 输出开关（两路负载不接通）。',
                mode: 'check',
                op: [
                    { type: 'switch', target: 'ups1', part: 'batterySwitch', msg: '合上储能电池开关',
                      async act() { this.sys.comps.ups1.setSwitchState('batterySwitch', true); await _sleep(500); } },
                    { type: 'switch', target: 'ups1', part: 'outputSwitch',  msg: '合上 UPS 输出开关',
                      async act() { this.sys.comps.ups1.setSwitchState('outputSwitch', true); await _sleep(500); } },
                ],
                check() {
                    const ups = this.sys.comps.ups1;
            ...
            {
                msg: '第 5 步：长按 ON 按钮 3 秒起动 UPS。观察起动过程：先进入旁通模式（约 10s），约 30s 后切换至在线（LINE）模式。',
                mode: 'check',
                op: [
                    { type: 'btn', target: 'ups1', part: 'onButton', msg: '长按 ON 按钮 3 秒起动 UPS',
                      async act() { this.sys.comps.ups1.pressOn(); } },
                    { type: 'observe', target: 'ups1', part: 'led-bypass', msg: '已进入旁通模式（BYPASS 指示灯亮），等待旁通阶段完成…',
                      async act() {
                          // 箭头指向旁通指示灯，等待在线（LINE）模式建立
                          await _waitFor(() => this.sys.comps.ups1._mode === 'normal', 40000);
                      } },
                    { type: 'observe', target: 'ups1', part: 'led-line', msg: '已进入在线模式（LINE 指示灯亮）',
                      async act() {
                          await _sleep(3000);
                      } },
                ],
            ...
            {
                msg: '第 6 步：切断交流电源。UPS 由储能电池经逆变器供电，进入电池模式。',
                mode: 'check',
                op: [
                    { type: 'btn', target: 'genpanel', part: 'btn-open', msg: '按遥控面板"分闸"按钮，断开主开关切断交流电源',
                      async act() { await _pressPanelBtn(this.sys, 'genpanel', '_userOpenPressed', 700); } },
                    { type: 'observe', target: 'ups1', part: 'led-battery', msg: '已进入电池模式（BATTERY 指示灯亮）',
                      async act() { await _waitFor(() => this.sys.comps.ups1._mode === 'battery', 8000); } },
                ],
                check() {
            ...
            {
                msg: '第 7 步：恢复交流电源。UPS 自动返回在线（LINE）模式，负载供电不间断。',
                mode: 'check',
                op: [
                    { type: 'btn', target: 'genpanel', part: 'btn-close', msg: '按遥控面板"合闸"按钮，恢复交流电源',
                      async act() { await _pressPanelBtn(this.sys, 'genpanel', '_userClosePressed', 700); } },
                    { type: 'observe', target: 'ups1', part: 'led-line', msg: '交流恢复，自动返回在线模式（LINE 指示灯亮）',
                      async act() { await _waitFor(() => this.sys.comps.ups1._mode === 'normal', 8000); } },
                ],
                check() {
            ...
            {
                msg: '第 8 步：依次合上第1路、第2路负载开关，两路负载总功率约 3000W，超过 UPS 额定功率 2000W（过载）。过载持续约 2 秒后 UPS 自动转入旁路模式（BYPASS 灯亮，输入经旁路直通为负载供电）。',
                mode: 'check',
                op: [
                    { type: 'switch', target: 'ups1', part: 'loadSwitch1', msg: '合上第 1 路负载开关',
                      async act() { this.sys.comps.ups1.setSwitchState('loadSwitch1', true); await _sleep(500); } },
                    { type: 'switch', target: 'ups1', part: 'loadSwitch2', msg: '合上第 2 路负载开关',
                      async act() { this.sys.comps.ups1.setSwitchState('loadSwitch2', true); await _sleep(500); } },
                    { type: 'observe', target: 'ups1', part: 'led-bypass', msg: '过载，UPS 自动转入旁路模式（BYPASS 指示灯亮）',
                      async act() { await _waitFor(() => this.sys.comps.ups1._mode === 'bypass', 8000); } },
                ],
                check() {
                    const ups = this.sys.comps.ups1;
...
function _autoWire(sys) {
    ...
    const cons = [
        ...
        // ── UPS 电源输入（汇流排第6接口 A 相 + 接地 → UPS 输入端）不纳入自动接线，由学生手动连接 ──
        // ── UPS 第1路输出 → 负载灯1 ──
...
// 轮询等待条件成立（超时返回 false）：用于"等待某模式/状态完成后再进行下一步"
async function _waitFor(cond, timeoutMs = 10000, stepMs = 100) {
    const t0 = Date.now();
    while (!cond()) {
