...
export class LeadAcidBattery extends BaseComponent { // 声明组件类
    ...
    getClickablePartCenter(partId) {
        const gx = this.group.x(), gy = this.group.y(), s = this.scale || 1; // 组件位置与缩放
        const capY = this._bodyTop + this._bodyH / 2; // 注液孔盖中心 y
        if (partId && /^cap[1-6]$/.test(partId)) { // 六个注液孔盖
            const i = parseInt(partId.slice(3), 10) - 1;
            if (this._cellCx && this._cellCx[i] !== undefined) {
                return { x: gx + this._cellCx[i] * s, y: gy + capY * s };
            }
            return null;
        }
        switch (partId) {
            case 'soc':     return { x: gx + (this.width / 2 - 6) * s, y: gy + 45 * s };  // SOC 电量条
            case 'volt':    return { x: gx + (this.width / 2) * s,     y: gy + 70 * s };  // 电压读数
            case 'current': return { x: gx + (this.width / 2) * s,     y: gy + 80 * s };  // 电流读数
            case 'sg':      return { x: gx + (this.width / 2) * s,     y: gy + 110 * s }; // 比重读数
            default:     return null;
        }
    }
