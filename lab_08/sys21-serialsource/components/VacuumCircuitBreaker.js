...
export class VacuumCircuitBreaker extends BaseComponent {
    ...
    _createClickableParts() {
        ...
        // 接地开关区域（左侧操作面板下方"接地开关"栏，供工作流 find 识别 / 演练点击）
        this.addClickablePart('接地开关', 8, this._gsBarY + 2, 128, 26);
    }

    /**
     * 返回可点击部件中心坐标（供工作流 find/箭头指示定位）
     * @param {string} partId 'main-contact' | '接地开关'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        const rel = {
            'main-contact': { x: this._bottle.x + this._bottle.w / 2, y: this._bottle.y + this._bottle.h / 2 },
            '接地开关':     { x: 72, y: this._gsBarY + 15 },
        };
        const p = rel[partId];
    ...
    _bindInteraction() {
        ...
        dialHit.on('click tap', (e) => {
            e.cancelBubble = true;
            // 合闸（带负荷）期间工作位旋钮闭锁；接地期间的"防带地线摇车"由 _dialTurn 处理
    ...
    _dialTurn(dir) {
        ...
        // 五防·防带地线合闸：接地开关闭合期间禁止摇向工作位（连接位）；
        //   试验位（1）与检修位（2）主回路均已隔离，允许互相移动。
        if (this.isGrounded() && nextDet === 0) { this._clickAcc = 0; this._dialAngle = this._detent * 90; return; }
        this._clickAcc += dir;
