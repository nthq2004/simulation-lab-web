...
export class EmergencyPanel extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // 控制模式默认值：未配置时保持自动（兼容既有工程）
        this._mode   = config.mode === 'manual' ? 'manual' : 'auto';
     ...
     * @param {string} partId 'mode-knob'(控制模式旋钮) | 'tie-knob'(联络开关模式旋钮)
     *                        | 'eqf-close'(应急合闸按钮) | 'eqf-open'(应急分闸按钮) | 'lcd'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        const rel = {
            'mode-knob': { x: this._modeSwitch.x,  y: this._modeSwitch.y },
            'tie-knob':  { x: this._tieSwitch.x,   y: this._tieSwitch.y },
            'eqf-close': { x: this._eqfCloseBtn.x, y: this._eqfCloseBtn.y },
            'eqf-open':  { x: this._eqfOpenBtn.x,  y: this._eqfOpenBtn.y },
            'lcd':       { x: this._lcd.x + this._lcd.w / 2, y: this._lcd.y + this._lcd.h / 2 },
        };
        const p = rel[partId];
