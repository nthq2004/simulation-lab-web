...
export class ChargeBoard extends BaseComponent { // 导出充放电板组件类，继承基础组件
    ...
    // 返回部件中心的世界坐标（供工作流箭头精确定位）
    getClickablePartCenter(partId) { // 开始一个新的代码块
        const W = this.width, H = this.height, s = this.scale || 1; // 读取尺寸与缩放
        const gx = this.group.x(), gy = this.group.y(); // 组件左上角世界坐标
        const startX = (W - (3 * 110 + 2 * 20)) / 2; // 三个 LCD 的起始 x（与 _createDynamicNodes 一致）
        const lcdCy = this._row1Y + 18; // LCD/指示灯行中心 y
        let lx = null, ly = null; // 部件局部坐标
        switch (partId) { // 根据部件 id 计算局部坐标
            case 'meter-switch': lx = W / 2;      ly = this._row2Y + H * 0.10; break; // 仪表切换开关
            case 'ch1-float':    lx = W * 0.10;   ly = this._row3Y;            break; // CH1 浮充/均充开关
            case 'ch1-knob':     lx = W * 0.32;   ly = this._row3Y;            break; // CH1 电流旋钮
            case 'ch2-float':    lx = W * 0.90;   ly = this._row3Y;            break; // CH2 浮充/均充开关
            case 'ch2-knob':     lx = W * 0.68;   ly = this._row3Y;            break; // CH2 电流旋钮
            case 'volt-lcd':     lx = startX + 55;  ly = lcdCy;                break; // 直流电压表
            case 'amp-lcd':      lx = startX + 315; ly = lcdCy;                break; // 直流电流表
            case 'ac-indicator': lx = startX + 185; ly = lcdCy;                break; // 交流电源指示灯
            default: return null; // 未知部件
        } // 结束当前代码块
        return { x: gx + lx * s, y: gy + ly * s }; // 换算为世界坐标
    } // 结束当前代码块
