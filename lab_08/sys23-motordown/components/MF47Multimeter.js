...
export class MF47Multimeter extends BaseComponent {
    ...
    tick(dt) {
        ...
        if (group === 'DCmA') {
            ...
            if (!Array.isArray(this._acvBuf)) this._acvBuf = [];
            // 滑动窗口 RMS：窗口按仿真步数取约一个工频周期（与交流电压表一致），
            // 每帧都刷新读数，避免"每几秒才更新一次"的迟滞感
            const freq = (solver && solver._systemFreq) || 50;
            const stepDt = (solver && solver.deltaTime) || 1e-4;
            const windowSize = Math.max(10, Math.round(1 / (freq * stepDt)));
            this._acvBuf.push(diff * diff);
            if (this._acvBuf.length > windowSize) this._acvBuf.shift();
            const sumSq = this._acvBuf.reduce((a, b) => a + b, 0);
            this._inputValue = Math.min(1e7, Math.sqrt(sumSq / this._acvBuf.length));
        } else if (group === 'OHM') {
        ...
        /* MOSFET 体二极管正向检测硬编码（IGBT 已由求解器 C-E 体二极管模型驱动，无需特判） */
        ...
        /* 晶闸管 G-K 间正向电阻检测硬编码：MF47 在电阻档、黑表笔(COM)接 G、红表笔(V)接 K 时，
         * 指针偏转按档位固定：×10 档 30%，×100 档 80%，×1k 档 93%。
         * 注意：当阳极 A 也接在 COM（即 A、G 同接黑表笔，用于触发演示）时跳过，
         * 交由下面的 A-K 自锁演示处理，避免掩盖 A-K 导通。 */
        if (group === 'OHM' && this.sys) {
            const s = this.sys.voltageSolver;
            const cCOM = s?.portToCluster?.get('mf47-panel_wire_COM');
            const cV = s?.portToCluster?.get('mf47-panel_wire_v');
            const cG = s?.portToCluster?.get('scr1_wire_g');
            const cK = s?.portToCluster?.get('scr1_wire_k');
            const cA = s?.portToCluster?.get('scr1_wire_a');
            const anodeOnCom = cA !== undefined && cA === cCOM;
            if (!anodeOnCom && cCOM !== undefined && cCOM === cG && cV !== undefined && cV === cK) {
                const Rmid = this._range.multiplier * 20;
                const deflMap = { 10: 0.28, 100: 0.80, 1000: 0.97 };
                const defl = deflMap[this._range.multiplier];
                if (defl !== undefined) {
                    this._inputValue = Rmid * (1 - defl) / defl;
                }
            }
        }

        /* 晶闸管 A-K 导通/自锁演示硬编码（MF47 电阻档：黑表笔 COM 接 A、红表笔 V 接 K）：
         * - 门极 G 与 COM(即 A) 同接 → 门极正偏触发，A-K 导通；
         * - 触发后按通态小电阻显示（不显示 vOn 饱和压降），且栅极断开后仍保持导通（自锁）；
         * - 表笔离开该管的 A-K（含断开阳极/阴极接线）时解除自锁。 */
        if (group === 'OHM' && this.sys) {
            const s = this.sys.voltageSolver;
            const cCOM = s?.portToCluster?.get('mf47-panel_wire_COM');
            const cV = s?.portToCluster?.get('mf47-panel_wire_v');
            ...
            for (const id in comps) {
                const dev = comps[id];
                if (!dev || dev.type !== 'scr') continue;
                const cA = s?.portToCluster?.get(`${id}_wire_a`);
                const cK = s?.portToCluster?.get(`${id}_wire_k`);
                const cG = s?.portToCluster?.get(`${id}_wire_g`);
                // 仅在黑表笔接 A、红表笔接 K 时才视为在测该管 A-K
                const measuringAK = cCOM !== undefined && cV !== undefined &&
                    cA !== undefined && cK !== undefined && cCOM === cA && cV === cK;
                if (!measuringAK) { dev._mf47Aklatch = false; continue; }
                // 门极与黑表笔(A)同接 → 触发
                if (cG !== undefined && cG === cCOM) dev._mf47Aklatch = true;
                if (dev._mf47Aklatch) {
                    this._inputValue = dev.rOnDisp || 100; // 通态小电阻
                }
            }
        }

        /* 三极管 C/E 区分硬编码：MF47 电阻档，B 短接黑表笔(COM)。
         * 正接（黑表笔接 C、红表笔接 E）→ 指针偏转 65%；
         * 反接（黑表笔接 E、红表笔接 C）→ 指针偏转 20%。 */
        if (group === 'OHM' && this.sys) {
            const s = this.sys.voltageSolver;
            const cCOM = s?.portToCluster?.get('mf47-panel_wire_COM');
            const cV = s?.portToCluster?.get('mf47-panel_wire_v');
            if (cCOM !== undefined && cV !== undefined) {
                ...
                for (const id in comps) {
                    const comp = comps[id];
                    if (!comp || comp.type !== 'bjt') continue;
                    const cB = s?.portToCluster?.get(`${id}_wire_b`);
                    const cC = s?.portToCluster?.get(`${id}_wire_c`);
                    const cE = s?.portToCluster?.get(`${id}_wire_e`);
                    if (cB !== cCOM) continue; // 基极须短接黑表笔
                    if (cC === cCOM && cE === cV) {
                        this._inputValue = Rmid * (1 - 0.65) / 0.65;
                        break;
                    }
                    if (cE === cCOM && cC === cV) {
                        this._inputValue = Rmid * (1 - 0.2) / 0.2;
                        break;
                    }
                }
            }
    ...
    getClickablePartCenter(partId) {
        ...
        const W = this.width, H = this.height;
        const { cx, cy, r } = this._knob;
        if (partId === 'dial') return { x: gx + W * 0.5, y: gy + H * 0.35 };
        if (partId === 'knob') return { x: gx + cx, y: gy + cy };
        if (partId === 'ohm-zero') return { x: gx + this._ohmZero.x, y: gy + this._ohmZero.y };
        if (partId === 'v')    return { x: gx + W * 0.78, y: gy + this._termY };
        if (partId === 'com')  return { x: gx + W * 0.5,  y: gy + this._termY };
        if (partId === 'ma')   return { x: gx + W * 0.22, y: gy + this._termY };
        if (partId && partId.startsWith('range-')) {
            const id = partId.slice(6);
            const a = KNOB_ANGLES[id];
            if (a === undefined) return null;
            const rad = (a - 90) * Math.PI / 180;
            const lr = r * 1.15;
            return { x: gx + cx + lr * Math.cos(rad), y: gy + cy + lr * Math.sin(rad) };
        }
        return null;
    }
