...
/**
 * ThermalNOContact — 热继电器常开触点（NO）
 *
 * 界面参照起动按钮（DiagramStartButton）电路符号：
 *  左/右端子 → 左/右静触头圆点，动触刀绕右端子旋转。
 *  底部操作符号：将起动按钮的「倒山字」按钮帽替换为发热元件符号
 *  （小型折叠发热丝，铜色）。
 *
 * 工作逻辑（由 ThermalRelayDevice 驱动）：
 *  未脱扣：常开触点断开（刀片下倾）
 *  脱扣  ：常开触点闭合（刀片抬至水平，接触左静触头）
 *
 * ── 端口 ─────────────────────────────────────────────────────
 *  com — 公共端（左）
 *  no  — 常开端（右）
 */
export class ThermalNOContact extends BaseComponent {
    ...
    _drawStaticParts() {
        ...
        // 底部操作符号：发热元件（替换起动按钮的倒山字帽）
        this._drawHeatCap(this._cx, H - 18, '#c89020');

        // 标签
        this._staticGroup.add(new Konva.Text({
            x: 0, y: 18, width: W,
            text: this.label, fontSize: 14, fill: '#333', align: 'center',
        }));
    }
    ...
    _bindInteraction() {
        // 发热元件符号区域可点击（试验/复位由发热元件组件负责，触点本体仅提示）
