...
export class GeneratorRemotePanel extends BaseComponent {
     ...
     * @param {string} partId 'btn-close' | 'btn-open' | 'btn-start' | 'btn-stop' | 'knob' | 'run-led' | 'ready-led'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        const rel = {
            'btn-close': this._btnClose,
            'btn-open':  this._btnOpen,
            'btn-start': this._btnStart,
            'btn-stop':  this._btnStop,
            'knob':      this._knob,
            'run-led':   this._runLed,
            'ready-led': this._readyLed,
        };
        const p = rel[partId];
