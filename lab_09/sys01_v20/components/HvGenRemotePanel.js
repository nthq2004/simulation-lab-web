...
export class HvGenRemotePanel extends BaseComponent {
    ...
    _bindInteraction() {
        ...
        // 高压带电显示器识别热区（三只指示灯区域，供工作流 find 识别 / 演练点击；避开自检按钮）
        this.addClickablePart('带电显示器', 88, this._live.box.y, 124, this._live.box.h);
    }

    /**
     * 返回可点击部件中心坐标（供工作流 find/箭头指示定位）
     * @param {string} partId '带电显示器' | 'selftest' | 'start' | 'close' | 'mode' | 'groundled'
     */
    getClickablePartCenter(partId) {
        ...
        const rel = {
            '带电显示器': { x: 150, y: 160 },
            'selftest':   { x: this._live.test.x, y: this._live.test.y },
            'start':      { x: this._row2.swStart.x, y: this._row2.swStart.y },
            'close':      { x: this._row2.swClose.x, y: this._row2.swClose.y },
            'mode':       { x: this._row2.swMode.x,  y: this._row2.swMode.y },
            'groundled':  { x: 294, y: 22 },
        };
        const p = rel[partId];
    ...
    /** 自动模式简化状态机：电网无电 → 延时 3s 自起动 1# 机组（已运行则跳过）
     *  → 自动调频 → 延时 5s 自动合闸供电
     *  （参照船舶电站自动控制模块的自动起动/并车功能的教学化子集） */
    _runAuto(dt) {
        if (this.mode !== 'auto' || !this._powered) { this._autoState = 'idle'; this._autoTimer = 0; return; }
        ...
        switch (this._autoState) {
            case 'idle':
                // 电网有电或存在故障锁存 → 不自动起动
                if (gridLive || this._faultLatched) { this._autoTimer = 0; break; }
                if (gen.isOn) {
                    // 机组已运行：跳过起动步骤（不延时 3s），直接进入合闸延时
                    this._autoState = 'regulating';
                    this._autoTimer = 0;
                } else if (gen.mode === 'remote') {
                    // 电网无电：延时 3s 后自动起动 1# 机组
                    this._autoTimer += dt;
                    if (this._autoTimer >= 3) {
                        this._autoTimer = 0;
                ...
                break;
            case 'starting':
                // 起动命令由 tick 依据本状态输出（_startPressed），起动机组
                if (gen.isOn) { this._autoState = 'regulating'; this._autoTimer = 0; }
                break;
            case 'regulating': {
                // 自动调频
                ...
                if (vOk && !qf.isClosed()) {
                    this._autoTimer += dt;
                    if (this._autoTimer >= 5) { this._autoTimer = 0; this._autoState = 'closing'; }
                } else {
                    this._autoTimer = 0;
                }
                break;
            ...
            case 'closing':
                // 合闸命令保持到断路器合上，合上后清除（避免命令锁存导致分闸后又自动重合）
                if (qf.isClosed()) { this._autoState = 'on'; this._closeCmd = false; }
                else this._closeCmd = true;
                break;
            case 'on':
                if (!qf.isClosed()) { this._autoState = 'idle'; this._autoTimer = 0; this._closeCmd = false; }
                break;
        }
    }
