...
export class RealMegohmMeter extends BaseComponent {
     ...
     * @param {string} partId 'l'（L 接线柱）| 'e'（E 接线柱）| 'crank'（手摇柄）
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'l' && this._portL) return { x: gx + this._portL.x, y: gy + this._portL.y + 8 };
        if (partId === 'e' && this._portE) return { x: gx + this._portE.x, y: gy + this._portE.y + 8 };
        if ((partId === 'crank' || !partId) && this._crankCenter) {
            return { x: gx + this._crankCenter.x, y: gy + this._crankCenter.y };
        }
