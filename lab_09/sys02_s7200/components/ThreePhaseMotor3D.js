    ...
    _updateDynamic(dt) {
        ...
        if (powered) {
            ...
            if (this._lastAng !== undefined) {
                ...
                if (Math.abs(d) > 1e-5) this._phaseSeq = d > 0 ? -1 : 1;
            }
            this._lastAng = ang;
        ...
        this._supplyFreq = (drive && this._powered) ? Math.abs(drive._fOut || 0) : null;
        ...
        if (rotating) {
            if (this._supplyFreq !== null) {
                ...
                // 无变频器（工频直接供电）：转速由供电频率决定，
                // 50Hz 工频 → 额定转速（如 1440 r/min）。
                const sf = this.sys?.voltageSolver?._systemFreq || this.ratedFreq || 50;
                rpm = Math.round(this.ratedSpeed * (sf / (this.ratedFreq || 50)));
            }
        }
