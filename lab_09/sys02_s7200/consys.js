import {
    ...
    MCS51Solver, S7200Solver, Show, perfMonitor, EventBus, EquipmentPool, ThermalSolver,
    DeviceManager
} from './export.js';
import {
    ...
    EmergencyPanel,
    ShipAutoControl, ThreePhaseLoad, HvThreePhaseLoad, SimpleVCB, SimpleHVGenerator, HvTransformer, HvPowerOneLine, LvPowerOneLine, HvSwitchPanel, LvSwitchPanel, ImportantDistPanel, HvTester, HvGroundMonitor, HvDischargeRod, HvGroundingCable,
    Syncroscope, LampRotationSync, SP4TSwitch,
    UPS
} from './export.js';
...
import { componentConfigs, initSlider, applyAllPresets } from './project/s7200smart.js';
...
export class ControlSystem {
    ...
    init() {
        ...
        this.s7200Solver = new S7200Solver(this);
        ...
        // ── 直流瞬态检测器（变压器直流模型/脉冲期间禁止跳帧） ──
        const dcTransientComps = Object.keys(this.comps)
            .map(id => this.comps[id])
            .filter(c => c && typeof c.isDcTransientActive === 'function');
        this._ctDcActive = () => dcTransientComps.some(c => c.isDcTransientActive());
    ...
    _updatePhysics() {
        ...
        if (hasWires || this._hasCalibrator) {
            ...
            // 有交流源时不跳帧（波形需每帧更新）
            // 变压器处于直流瞬态/直流模型时也不跳帧（读数与指针需即时刷新）
            const dcTransient = this._ctDcActive && this._ctDcActive();
            const shouldSkip = !this._hasACSource && !dcTransient && stableFrame > 10 && (stableFrame % 2 === 0);
        ...
        if (this._hasDigital) {
            ...
            if (this.s7200Solver) this.s7200Solver.update(1 / 20);
