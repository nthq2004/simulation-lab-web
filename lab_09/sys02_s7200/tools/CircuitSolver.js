...
export class CircuitSolver {
    ...
    _buildDeviceCache() {
        ...
        return {
            ...
            plcDevs:       raw.filter(d => d.type === 'plc_s7200'),
    ...
    _invalidateCacheIfNeeded() {
        try {
            ...
            for (const d of this.rawDevices) {
                ...
                if (d.type === 'load_3p') {
                    resistSigs.push(`${d.id}:load=${d._loaded || false}:P=${d._Pph || 0}:L=${d._Lph || 0}:C=${d._Cph || 0}`);
                }
    ...
    _solve() {
        ...
        const {
            ...
            oscDevs, osc3Devs, diodeDevs, zenerDevs, ledDevs, photodiodeDevs, ptDevs, diacDevs, resistorDevs, load3Devs, testerDevs, hvLoad3Devs, hvTransformerDevs, pressDevs, transmitterDevs,
...
const totalSize = nodeVarCount + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + tripRelayEqCount + genRemotePanelEqCount + emergencyPanelEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount + ctVcvsEqCount + mwEqCount + imEqCount + rlEqCount + upsEqCount + transformerEqCount;
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // ─ 5-2b. S7-200 SMART PLC 内部 24V 传感器电源 ─────────
            if (plcDevs.length) {
                DeviceStamps.stampPLCAux(ctx, G, B, plcDevs);
            }
