    ...
    build(rawDevices, connections, poorContactPorts = new Set(), faultShortGroups = []) {
        ...
        rawDevices.forEach(dev => {
            ...
            if (dev.type === 'ACB') {
                ...
                if (dev.ports) {
                    const pid = (n) => `${id}_wire_${n}`;
                    const has = (n) => dev.ports.some(p => p.id === n || p.id === pid(n));
                    if (has('no1')) {
                        if (acbOn) internalUnion(pid('no1'), pid('no2'));
                    }
                    if (has('nc1')) {
                        if (!acbOn) internalUnion(pid('nc1'), pid('nc2'));
                    }
                }
            ...
            // SINAMICS V20 变频器：数字量输出触点按运行状态内部短接
            //   DO1（晶体管）运行中闭合；DO2（继电器）故障时闭合
            if (dev.type === 'v20_inverter') {
                if (dev._do1On) internalUnion(`${id}_wire_do1p`, `${id}_wire_do1n`);
                if (dev._do2On) internalUnion(`${id}_wire_do2c`, `${id}_wire_do2no`);
            }

            // S7-200 SMART PLC：晶体管/继电器输出触点按过程映像 Q 内部短接
            //   1L+ → DQx 组：Q 为 1 时把对应输出端子与 1L+/2L+ 短接（源型输出）
            if (dev.type === 'plc_s7200') {
                const q = dev._qBits || [];
                const groups = [
                    { l: '1l', from: 0, to: 7 },
                    { l: '2l', from: 8, to: 15 },
                ...
                groups.forEach(g => {
                    for (let i = g.from; i <= g.to; i++) {
                        if (q[i]) internalUnion(`${id}_wire_dq${Math.floor(i / 8)}${i % 8}`, `${id}_wire_${g.l}`);
                    }
                });
            }
        ...
        let firstGnd = null;
        rawDevices.forEach(dev => {
            if (dev.type === 'gnd' || dev.type === 'syncroscope' || dev.type === 'lampRotationSync') {
        ...
        rawDevices.forEach(dev => {
            ...
            if (dev.type === 'motor_3d') {
                // 三相电机 3D：U/V/W 与 PE 端口始终纳入拓扑，
                // 保证每相对地（PE 星点）注入的阻抗支路即使 PE 未接线也成立
                ['u', 'v', 'w', 'pe'].forEach(p =>
                    activePorts.add(`${dev.id}_wire_${p}`));
            }
            ...
            if (dev.type === 'v20_inverter') {
                // 变频器三相输出端口与内部中性点始终纳入拓扑，
                // 保证 U/V/W 输出 stamp 即使未接线也能建立（n 为内部参考）
                ['u', 'v', 'w', 'n'].forEach(p =>
                    activePorts.add(`${dev.id}_wire_${p}`));
            }
            if (dev.type === 'plc_s7200') {
                // PLC 内部辅助电源与 DO 电源端始终纳入拓扑，
                // 保证 24V stamp 与 DO 触点内部短接即使未接线也成立
                ['24v', '0v', 'mm', '1l', '2l', 'lplus'].forEach(p =>
                    activePorts.add(`${dev.id}_wire_${p}`));
            }
            if (dev.type === 'ACB') {
            ...
            if (dev.type === 'motor_control_box') {
                // 电机控制箱：12 个接线端子端口 + PE 网络 + 内部中性点 n 始终纳入拓扑，
                // 保证上下排同簇、接触器吸合左右同簇恒成立（即使端口未接线）
                ['in1','in2','in3','inU','inV','inW','outU','outV','outW','out1','out2','out3','n']
