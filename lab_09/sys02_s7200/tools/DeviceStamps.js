...
export const DeviceStamps = {
    ...
    // ─── 1b. 电磁阀（线圈 500Ω 电阻 + 按电流迟滞吸合截止阀） ──────────────
    // 线圈永久串入电路（500Ω）；用上一迭代解计算线圈电流，
    // I > 30mA 吸合、I < 20mA 释放（迟滞在组件内实现）。
    stampSolenoidValves(ctx, G, B, svDevs, results) {
        svDevs.forEach(dev => {
            const cA = ctx.portToCluster.get(`${dev.id}_wire_a`);
            const cB = ctx.portToCluster.get(`${dev.id}_wire_b`);
            if (cA === undefined || cB === undefined) { dev._updateStateByCurrent(0); return; }
            const R = Math.max(dev.coilResistance || 500, 0.01);
            this._fill(ctx, G, B, cA, cB, 1 / R);
            // 用上一迭代解结算线圈电流并更新吸合状态
            const vA = ctx.getVoltageFromResults(results, cA);
            const vB = ctx.getVoltageFromResults(results, cB);
            dev._updateStateByCurrent((vA - vB) / R);
        });
    },
    ...
    // ─── 4a2. 电位端子（special: 'pterm'，诺顿等效：对地电导 + 电位电流源） ──
    // 将端子设定的对地电压 dev.potential 以 V*g 的电流源 + 对地电导 g 注入，
    // 不占用额外电压源方程。
    stampPotentialTerminals(ctx, G, B, ptermDevs) {
        ptermDevs.forEach(dev => {
            const cP = ctx.portToCluster.get(`${dev.id}_wire_p`);
            if (cP === undefined) { dev._terminalCurrent = 0; return; }
            const { g, v } = dev.getStamp ? dev.getStamp() : { g: 100, v: dev.potential || 0 };
            this._fill(ctx, G, B, cP, -1, g);
            this._addI(ctx, B, cP, -1, v * g);
        });
    },
    ...
    // ─── 5c. 断路器保护特性测试仪 ────────────────────────────────────────
    //   · 一次电流：三相理想电流源（诺顿：并联 1Ω 防开路奇异），I = √2·I_rms·sin(ωt+φ)
    //   · 二次电流：单相理想电流源（可调幅值/相位），直接注入电子脱扣器电流绕组
    //   · 线电压  ：单相诺顿电压源（内阻 0.1Ω），相位为 0，与二次电流相位差决定功率方向
    stampBreakerTesters(ctx, G, B, testerDevs, currentTime) {
        const F = 50, w = 2 * Math.PI * F;
        testerDevs.forEach(dev => {
            // ── 一次：三相大电流（CT 采样相 out3 相位与线电压一致，避免电子脱扣器误判逆功率）──
            if (dev._primaryOutput) {
                const Ip = Math.SQRT2 * (dev.primaryI || 0);
                [['out1', 'r1', -2.0943951023931953], ['out2', 'r2', 2.0943951023931953], ['out3', 'r3', 0]]
                    .forEach(([o, r, ph]) => {
                        const cO = ctx.portToCluster.get(`${dev.id}_wire_${o}`);
                        const cR = ctx.portToCluster.get(`${dev.id}_wire_${r}`);
                        if (cO === undefined || cR === undefined) return;
                        this._fill(ctx, G, B, cO, cR, 1 / 1);
                        this._addI(ctx, B, cO, cR, Ip * Math.sin(w * currentTime + ph));
                    });
            }
            // ── 二次：模拟电流 ──
            if (dev._secondaryOutput) {
                const c1 = ctx.portToCluster.get(`${dev.id}_wire_iout1`);
                const c2 = ctx.portToCluster.get(`${dev.id}_wire_iout2`);
                if (c1 !== undefined && c2 !== undefined) {
                    const Ip = Math.SQRT2 * (dev.secondaryI || 0);
                    const ph = (dev.secondaryPhase || 0) * Math.PI / 180;
                    this._fill(ctx, G, B, c1, c2, 1 / 1e4);
                    this._addI(ctx, B, c1, c2, Ip * Math.sin(w * currentTime + ph));
                }
            }
            // ── 可调线电压：接通 24V 后一直输出（不受二次起停控制）──
            if (dev._powered) {
                const cv1 = ctx.portToCluster.get(`${dev.id}_wire_vout1`);
                const cv2 = ctx.portToCluster.get(`${dev.id}_wire_vout2`);
                if (cv1 !== undefined && cv2 !== undefined) {
                    const Vp = Math.SQRT2 * (dev.lineV || 0);
                    const rOn = 0.1;
                    this._fill(ctx, G, B, cv1, cv2, 1 / rOn);
                    this._addI(ctx, B, cv1, cv2, (Vp * Math.sin(w * currentTime)) / rOn);
                }
            }
        });
    },

    // ─── 5a. 电机控制箱（诺顿等效三相电源，受 QF 供电开关控制）────────
    // 左组下排 in1/in2/in3 为电源进线：QF 合上时注入 220V 相电压三相（相对内部中性点 n）
    // 内部中性点 n 未接线时由 CircuitSolver 钳位到地；左上下、右上下及接触器吸合
    // 的左右同簇已在 CircuitTopology 中建立。
    stampMotorControlBoxes(ctx, G, B, devs, currentTime) {
        devs.forEach(dev => {
            const qfOn = dev._qfState === 'on';
            const cN  = ctx.portToCluster.get(`${dev.id}_wire_n`);
            const cPe = ctx.portToCluster.get(`${dev.id}_wire_pe1`);
            const insul = (dev._lineInsulR !== undefined && isFinite(dev._lineInsulR) && dev._lineInsulR > 0)
                ? dev._lineInsulR : null;
            ['in1', 'in2', 'in3'].forEach((p, i) => {
                const cP = ctx.portToCluster.get(`${dev.id}_wire_${p}`);
                if (cP === undefined) return;
                if (cN !== undefined) {
                    const voltage = qfOn ? dev.getPhaseVoltage(p, currentTime) : 0;
                    const rOn = qfOn ? (dev._rOnEff || 0.01) : 10e6;
                    this._fill(ctx, G, B, cP, cN, 1 / rOn);
                    this._addI(ctx, B, cP, cN, voltage / rOn);
                }
                // 电源各相进线对箱体 PE 的绝缘电阻（与源内阻并接）
                if (insul && cPe !== undefined) this._fill(ctx, G, B, cP, cPe, 1 / insul);
            });
        });
    },

    // ─── 5a2. SINAMICS V20 变频器 10V / 24V 辅助电源 ────────────────────
    //  控制单元内部提供 +10V（模拟量给定基准）与 +24V（数字量输入电源），
    //  均以 0V 端子为参考，供外部电位器 / 按钮取用。仅在对应端子已接线时注入。
    stampV20Aux(ctx, G, B, devs) {
        devs.forEach(dev => {
            const emit = (pid, E, rOn) => {
                const cP = ctx.portToCluster.get(`${dev.id}_wire_${pid}`);
                if (cP === undefined) return;
                ['v0', 'v0b'].forEach(zero => {
                    const cN = ctx.portToCluster.get(`${dev.id}_wire_${zero}`);
                    if (cN === undefined || cN === cP) return;
                    this._fill(ctx, G, B, cP, cN, 1 / rOn);
                    this._addI(ctx, B, cP, cN, E / rOn);
                });
            };
            emit('v10', 10, 20);
            emit('v24', 24, 20);
    ...
    // ─── 5a2. S7-200 SMART PLC 内部 24V 传感器电源 ────────────────────
    //   24V 端子相对 0V/M 端子输出 24V（源阻抗 20Ω），供 DI 干接点/传感器取用。
    //   DO 触点由 CircuitTopology 按过程映像 Q 内部短接（1L+/DQx 或 DQx/2M）。
    stampPLCAux(ctx, G, B, devs) {
        devs.forEach(dev => {
            const c24 = ctx.portToCluster.get(`${dev.id}_wire_24v`);
            const zeros = ['v0', 'mm']
                .map(z => ctx.portToCluster.get(`${dev.id}_wire_${z}`))
                .filter(c => c !== undefined);
            if (c24 !== undefined) {
                zeros.forEach(cN => {
                    if (cN === c24) return;
                    const rOn = 20;
                    this._fill(ctx, G, B, c24, cN, 1 / rOn);
                    this._addI(ctx, B, c24, cN, 24 / rOn);
                });
            }

            // ── DI 输入端内部下拉电阻 ──
            // 真实 PLC 数字量输入为高阻输入且带内部下拉；接入干接点/开关时，
            // 触点断开端口不应悬空（悬空会因 MNA 阻尼迭代残留假电压，
            // 导致"开关未闭合却读数偏高被误判为 ON"）。
            // 每个 DI 对相应公共端（1M/2M）接 10kΩ 下拉：
            //   开关闭合 → 端口≈24V（读 ON）；开关断开 → 端口≈0V（读 OFF）
            const c1m = ctx.portToCluster.get(`${dev.id}_wire_1m`);
            const c2m = ctx.portToCluster.get(`${dev.id}_wire_2m`);
            const cV0 = ctx.portToCluster.get(`${dev.id}_wire_v0`);
            const cMm = ctx.portToCluster.get(`${dev.id}_wire_mm`);
            if (dev.diPins && dev.diPins.length) {
                dev.diPins.forEach((pin, idx) => {
                    const cDi = ctx.portToCluster.get(`${dev.id}_wire_${pin}`);
                    if (cDi === undefined) return;
                    const bit = idx;
                    const cRef = bit <= 11
                        ? (c1m !== undefined ? c1m : (cV0 !== undefined ? cV0 : cMm))
                        : (c2m !== undefined ? c2m : (cV0 !== undefined ? cV0 : cMm));
                    if (cRef === undefined || cRef === cDi) return;
                    this._fill(ctx, G, B, cDi, cRef, 1 / 10000);
                });
            }

            // ── DO 输出端关断时的高阻下拉 ──
            // 未接负载且 Q=0 时，输出端口同样会悬空；对 2M/0V 接 1kΩ 下拉，
            // Q=1 时拓扑把端口并入 1L+/2L+（0.01Ω 级），24V 仍能正常输出。
            if (dev.dqPins && dev.dqPins.length) {
                dev.dqPins.forEach((pin, idx) => {
                    const cDq = ctx.portToCluster.get(`${dev.id}_wire_${pin}`);
                    if (cDq === undefined) return;
                    const cRef = cMm !== undefined ? cMm : cV0;
                    if (cRef === undefined || cRef === cDq) return;
                    this._fill(ctx, G, B, cDq, cRef, 1 / 1000);
                });
            }
        });
    },
    ...
    // ─── 5b2. 三相电机 3D（按状态/负荷率对 U/V/W 每相注入对地电阻） ────
    // 电阻由组件按规则计算并写入 dev._phaseGroundR：
    //   堵转           → 2.9/5 = 0.58Ω
    //   负荷率 = 0     → 29Ω
    //   负荷率 ≠ 0     → 2.9 / 负荷率
    stampMotors3D(ctx, G, B, devs) {
        devs.forEach(dev => {
            const R = (dev._phaseGroundR !== undefined && dev._phaseGroundR > 0)
                ? dev._phaseGroundR : 1e9;
            // 各相绕组对机壳（PE）的绝缘电阻（Ω），与负载阻抗并接
            const insul = (dev._insulR !== undefined && isFinite(dev._insulR) && dev._insulR > 0)
                ? dev._insulR : null;
            const cPe = ctx.portToCluster.get(`${dev.id}_wire_pe`);
            ['u', 'v', 'w'].forEach(p => {
                const cP = ctx.portToCluster.get(`${dev.id}_wire_${p}`);
                if (cPe !== undefined && cP !== undefined) {
                    this._fill(ctx, G, B, cP, cPe, 1 / R);
                    if (insul) this._fill(ctx, G, B, cP, cPe, 1 / insul);
                }
            });
        });
    },
    ...
    stampBJTs(ctx, G, B, bjtDevs, results) {
        bjtDevs.forEach(dev => {
            ...
            if (cB !== undefined && cE !== undefined && cC === undefined) {
                // 仅 B-E 接入：发射结正向压降 VBE（NPN 电流 B→E，PNP 反号）
                const pol = (dev.subType === 'PNP') ? -1 : 1;
                const VBE = (dev.vbeOn ?? 0.7);
                const vDiff = (vB - vE) * pol;
                if (vDiff > VBE) {
                    const gOn = 2, iEq = VBE * gOn;
                    this._fill(ctx, G, B, cB, cE, gOn);
                    if (pol > 0) this._addI(ctx, B, cB, cE, iEq);
                    else this._addI(ctx, B, cE, cB, iEq);
                } else {
                    this._fill(ctx, G, B, cB, cE, 1 / 1e9);
                }
            } else if (cB !== undefined && cC !== undefined && cE === undefined) {
                // 仅 B-C 接入：集电结正向压降 VBC，略低于 VBE（便于万用表二极管档区分 C/E）
                const pol = (dev.subType === 'PNP') ? -1 : 1;
                const VBC = (dev.vbcOn ?? (dev.vbeOn ?? 0.7) - 0.08);
                const vDiff = (vB - vC) * pol;
                if (vDiff > VBC) {
                    const gOn = 2, iEq = VBC * gOn;
                    this._fill(ctx, G, B, cB, cC, gOn);
                    if (pol > 0) this._addI(ctx, B, cB, cC, iEq);
                    else this._addI(ctx, B, cC, cB, iEq);
                } else {
    ...
    stampIGBTs(ctx, G, B, igbtDevs, results) {
        igbtDevs.forEach(dev => {
            ...
            if (cC !== undefined && cE !== undefined) {
                ...
                // C-E 间固有 PN 结（体二极管）：E 为阳极、C 为阴极。
                // 栅极关断时仍存在的正向导通通道，供万用表电阻档/二极管档测量。
                // （CE 开路故障时该结一并断开）
                dev._bodyDiodeOn = false;
                if (!dev._faultCEOpen) {
                    const vF = dev.vBodyDiode ?? 0.6;
                    const gD = 1 / (dev.rOnDisp || 50);
                    if (vE - vC > vF) {
                        this._fill(ctx, G, B, cE, cC, gD);
                        this._addI(ctx, B, cE, cC, vF * gD);
                        dev._bodyDiodeOn = true;
                    } else {
                        // 反向截止：并入极高阻，避免影响 IGBT 原有的 C-E 关断电阻
                        this._fill(ctx, G, B, cE, cC, 1 / (dev.rOffBody || 1e9));
                    }
                }
            }
        });
    ...
    stampMainContacts(ctx, G, B, devs) {
        devs.forEach(dev => {
            ...
            poles.forEach(([p1, p2]) => {
                ...
                if (c1 !== undefined && c2 !== undefined) {
                    const open = dev._faultOpenPoles && dev._faultOpenPoles[p1];
                    this._fill(ctx, G, B, c1, c2, open ? 1e-10 : 1 / R);
                }
            });
        });
    },
    ...
    stampNOContacts(ctx, G, B, devs) {
        devs.forEach(dev => {
            const open = dev._faultOpen || dev._faultOpenEnd;
            const R = open ? 1e12 : dev.getValue();
    ...
    stampContactCoils(ctx, G, B, devs) {
        devs.forEach(dev => {
            const R = dev._faultCoilOpen ? 1e12 : (dev._coilResistance || 1000);
    ...
    stampMultimeters(ctx, G, B, mmDevs, results, mmResVIdx) {
        ...
        mmDevs.forEach(dev => {
            if (dev.mode === 'MA') {
                ...
                // 电容档：按下 1mA 恒流对被测电容充电（测量完成后停止充电，保持读数）
                const cV = ctx.portToCluster.get(`${dev.id}_wire_v`);
                const cCOM = ctx.portToCluster.get(`${dev.id}_wire_com`);
                if (cV === undefined || cCOM === undefined) { dev.currentIdx = undefined; return; }
                // 顺从内阻：防止开路/浮空时矩阵奇异；10MΩ 对被测支路分流极小
                this._fill(ctx, G, B, cV, cCOM, 1 / 10000000);
                if (!dev._capDone) {
                    this._addI(ctx, B, cV, cCOM, 0.001);
                }
                dev.currentIdx = undefined;
            } else if (dev.mode === 'DIODE') {
                const cV = ctx.portToCluster.get(`${dev.id}_wire_v`);
                const cCOM = ctx.portToCluster.get(`${dev.id}_wire_com`);
                if (cV === undefined || cCOM === undefined) { dev.currentIdx = undefined; idx++; return; }
                // 二极管/通断档：注入 1mA 恒流测试电流（与真实数字万用表一致）
                this._addI(ctx, B, cV, cCOM, 0.001);
                // 顺从电压限制：V-COM 间并联 1MΩ，开路时 vPort≈1000V 显示 O.L，
                // 同时避免孤立节点在 MNA 中产生奇异矩阵（电流源无回流路径时解出 NaN）。
                // 此内阻远大于被测范围（0~几十 kΩ），对测量几乎无分流影响。
                this._fill(ctx, G, B, cV, cCOM, 1 / 1000000);
                dev._resCV = false;
                dev.currentIdx = undefined;
                idx++;
            } else if (dev.mode === 'RES200' || dev.mode === 'RES2k' || dev.mode === 'RES200k') {
                const cV = ctx.portToCluster.get(`${dev.id}_wire_v`);
                const cCOM = ctx.portToCluster.get(`${dev.id}_wire_com`);
                if (cV === undefined || cCOM === undefined) { dev.currentIdx = undefined; idx++; return; }
                // 各档位恒流测试电流（与真实数字万用表一致）
                let iSet = 0.001;                                   // RES200：1mA
                if (dev.mode === 'RES2k') iSet = 0.0001;            // 2kΩ：0.1mA
                else if (dev.mode === 'RES200k') iSet = 0.000001;   // 200kΩ：1µA
                ...
                // 滞回：端口电压超过 3V（顺从电压上限）→ 切换 3V 恒压源；
                // 回落到 2.5V 以下 → 恢复恒流源。恒压源退出由 _updateDeviceCurrents 依据电流阈值复位。
                if (vPort >= 3) {
                    dev._resCV = true;
                } else if (vPort <= 2.5) {
                    dev._resCV = false;
                ...
                if (dev._resCV) {
                    ...
                    // 顺从内阻：开路时 vPort=iSet×100MΩ（>3V），保证节点可解并触发恒压源切换，
                    // 对量程内（≤200kΩ）测量分流误差 <0.2%
                    this._fill(ctx, G, B, cV, cCOM, 1 / 100000000);
                    dev.currentIdx = undefined;
                }
                idx++;
            }
    ...
    stampControlTransformers(ctx, G, B, ctDevs, ctVIdx) {
        ...
        ctDevs.forEach(dev => {
            ...
            // ── 直流工况：副边由组件给定的受控电压源驱动 ──
            // 平台时间基准会把 L/R 瞬态慢放数百倍；直流励磁时改由组件输出
            // "正偏脉冲 → 0 / 反偏脉冲 → 0" 的电压，使示波器、万用表读数一致。
            if (dev._dcModelActive) {
                const R1d = Math.max(0.001, dev._primaryResistance || 20);
                const i1Col = vIdx;
                const i2Col = vIdx + 1;
                dev._i1Col = i1Col;
                dev._i2Col = i2Col;

                // 原边：绕组电阻 R1 作直流负载，并注入 0V 电压源量测原边电流（i1 = Vp / R1）
                if (primaryConnected) {
                    MNAMatrix.fillMatrix(G, B, ctx.nodeMap, ctx.gndClusterIndices, ctx.vPosMap, cP1, cP2, 1 / R1d);
                    MNAMatrix.addVoltageSource(G, B, ctx.nodeMap, ctx.gndClusterIndices, ctx.vPosMap, cP1, cP2, 0, i1Col);
                } else {
                    G[i1Col][i1Col] += 1;
                }

                // 副边：受控电压源 v = dev._dcSecondaryVoltage（正偏脉冲→0 / 反偏脉冲→0）
                if (secondaryConnected) {
                    MNAMatrix.addVoltageSource(G, B, ctx.nodeMap, ctx.gndClusterIndices, ctx.vPosMap,
                        cS1, cS2, dev._dcSecondaryVoltage || 0, i2Col);
                } else {
                    G[i2Col][i2Col] += 1;
                }
    ...
    _stampIMSimple(ctx, G, B, motorDevs) {
        ...
        motorDevs.forEach(dev => {
            ...
            // 负滑差（超同步/发电制动）时 R2/slip 变负，Zeq 可能骤降到近零/负值，
            // 导致 Geq 爆炸使 MNA 电流尖峰达 200A+；钳制 Zeq 下限保证导纳有界为正。
            const Zeq = Math.max(R_load + L_total / dt, 3);
            const Geq = 1 / Zeq;
    ...
    stampACBs(ctx, G, B, acbDevs) {
        acbDevs.forEach(dev => {
            ...
            if (dev.special === 'MainsSwitch') {
                ...
                // 分励线圈 fla↔flb（断线 → 无穷大电阻）
                const Rfl = dev._coilR && dev._coilR.fla ? dev._coilR.fla : (dev._tripCoilR || 50);
    ...
    // ─── 负序继电器（NegativeSeqRelay）────────────────────────
    // 常闭触点：正相序（isNCClosed()=true）注入 0.001Ω 导通；负相序断开不注入。
    stampNegSeqRelays(ctx, G, B, negDevs) {
        negDevs.forEach(dev => {
            if (typeof dev.isNCClosed === 'function' && dev.isNCClosed()) {
                const c1 = ctx.portToCluster.get(`${dev.id}_wire_nc1`);
                const c2 = ctx.portToCluster.get(`${dev.id}_wire_nc2`);
                if (c1 !== undefined && c2 !== undefined)
                    this._fill(ctx, G, B, c1, c2, 1 / 0.001);
            }
        });
    },

    // ─── 低压三相配电箱（DistributionBox）───────────────────────────
    // 内部三相汇流排节点即进线端口 in1/in2/in3；
    // 3 个三相塑壳开关闭合时注入 0.01Ω 接触电阻，分励线圈 fla↔flb 恒接 200Ω
    stampPDBs(ctx, G, B, pdbDevs) {
        pdbDevs.forEach(dev => {
            const Rcontact = 0.01;
            for (let s = 1; s <= 3; s++) {
                if (dev.getSwState(s - 1) === 'on') {
                    for (let ph = 1; ph <= 3; ph++) {
                        const cIn = ctx.portToCluster.get(`${dev.id}_wire_in${ph}`);
                        const cT  = ctx.portToCluster.get(`${dev.id}_wire_sw${s}_t${ph}`);
                        if (cIn !== undefined && cT !== undefined)
                            this._fill(ctx, G, B, cIn, cT, 1 / Rcontact);
                    }
                }
            }
            // 分励线圈电阻（每开关 fla↔flb 恒存在）
            const Rcoil = dev.getTripCoilR ? dev.getTripCoilR() : 200;
            for (let s = 1; s <= 3; s++) {
                const cFla = ctx.portToCluster.get(`${dev.id}_wire_sw${s}_fla`);
                const cFlb = ctx.portToCluster.get(`${dev.id}_wire_sw${s}_flb`);
                if (cFla !== undefined && cFlb !== undefined)
                    this._fill(ctx, G, B, cFla, cFlb, 1 / Rcoil);
                // 失压线圈电阻（每开关 sw{s}_uv1↔uv2，若有；如岸电主开关）。
                // 必须注入：互锁回路中若失压线圈无负载电阻，断开侧节点电位会
                // 经 1e10 Ω 弱连接被电源拉高，导致 _checkUVRelease 误判不失压。
                const cU1 = ctx.portToCluster.get(`${dev.id}_wire_sw${s}_uv1`);
                const cU2 = ctx.portToCluster.get(`${dev.id}_wire_sw${s}_uv2`);
                if (cU1 !== undefined && cU2 !== undefined)
                    this._fill(ctx, G, B, cU1, cU2, 1 / Rcoil);
            }
            // 常闭辅助触头（若有，如岸电主开关 nc1↔nc2）：闭合 0.001Ω，断开不注入
            if (typeof dev.isNCClosed === 'function' && dev.isNCClosed()) {
                const cN1 = ctx.portToCluster.get(`${dev.id}_wire_nc1`);
                const cN2 = ctx.portToCluster.get(`${dev.id}_wire_nc2`);
                if (cN1 !== undefined && cN2 !== undefined)
                    this._fill(ctx, G, B, cN1, cN2, 1 / 0.001);
            }
            // 失压脱扣线圈（岸电主开关 sw1_uv1↔sw1_uv2，额定电阻 tripCoilR）：
            // 线圈始终存在，闭合时由对方主开关的常闭辅助触头经 24V 控制电源供电
            const Ruv = dev.getTripCoilR ? dev.getTripCoilR() : 200;
            const cUv1 = ctx.portToCluster.get(`${dev.id}_wire_sw1_uv1`);
            const cUv2 = ctx.portToCluster.get(`${dev.id}_wire_sw1_uv2`);
            if (cUv1 !== undefined && cUv2 !== undefined)
                this._fill(ctx, G, B, cUv1, cUv2, 1 / Ruv);
        });
    },

    // ─── 岸电箱（ShorePowerBox）──────────────────────────────────
    // 进线 in1/in2/in3 → 相序转换开关（K-M 换相）→ 出口断路器 → 输出 t1/t2/t3。
    // 仅当 相序开关≠OFF（_knob: 1=相序1 / 2=相序2）且 出口断路器合闸(_breakerOn)
    // 时导通，接触电阻 0.01Ω。
    //   相序1：in1→t1, in2→t2, in3→t3（直通）
    //   相序2：in1→t1, in2→t3, in3→t2（L2/L3 互换 → 反相序输出）
    stampShorePowerBoxes(ctx, G, B, shoreBoxDevs) {
        shoreBoxDevs.forEach(dev => {
            const on = dev._knob !== 0 && !!dev._breakerOn;
            if (!on) return;
            const map = dev._knob === 2
                ? [['in1', 't1'], ['in2', 't3'], ['in3', 't2']]
                : [['in1', 't1'], ['in2', 't2'], ['in3', 't3']];
            map.forEach(([a, b]) => {
                const c1 = ctx.portToCluster.get(`${dev.id}_wire_${a}`);
                const c2 = ctx.portToCluster.get(`${dev.id}_wire_${b}`);
                if (c1 !== undefined && c2 !== undefined)
                    this._fill(ctx, G, B, c1, c2, 1 / 0.01);
            });
        });
    },
    ...
    stampTripRelays(ctx, G, B, tripDevs, tripVIdx) {
        tripDevs.forEach(dev => {
            ...
            // 右侧分励脱扣输出 f1a/f1b、f2a/f2b：过载持续达标后输出 24V
            [['f1a', 'f1b', '_shuntOut1'], ['f2a', 'f2b', '_shuntOut2']].forEach(([a, b, flag]) => {
                const cA = ctx.portToCluster.get(`${dev.id}_wire_${a}`);
                const cB = ctx.portToCluster.get(`${dev.id}_wire_${b}`);
                if (cA !== undefined && cB !== undefined) {
                    const vOut = dev[flag] ? 24 : 0;
                    this._addV(ctx, G, B, cA, cB, vOut, tripVIdx);
                    tripVIdx++;
                }
            });
        });
    ...
    // ─── 应急配电板（EmergencyPanel） ───────────────────────────
    // 5 个电压源：联络合/分、应发合/分、调频。
    stampEmergencyPanels(ctx, G, B, panelDevs, vIdx) {
        panelDevs.forEach(dev => {
            const out = dev._out || {};
            const emit = (a, b, volt) => {
                const cA = ctx.portToCluster.get(`${dev.id}_wire_${a}`);
            ...
            emit('tie_close_a', 'tie_close_b', out.tieClose ? 24 : 0);
            emit('tie_open_a',  'tie_open_b',  out.tieOpen  ? 24 : 0);
            emit('eqf_close_a', 'eqf_close_b', out.eqfClose ? 24 : 0);
            emit('eqf_open_a',  'eqf_open_b',  out.eqfOpen  ? 24 : 0);
            emit('egen_freq_p', 'egen_freq_n', out.freqV || 0);
        });
        return vIdx;
    },
