...
export class HvTester extends BaseComponent {
    constructor(config, sys) {
        ...
        this._measureCount = 0;     // 验电接触计数（演练模式考核：至少测量 3 次）
        this._lastContactId = null; // 上一次接触的端口 id（用于计上升沿）
        this._measurePrefixes = null; // 计数白名单（端口 id 前缀数组，null=统计所有端口）
    ...
    /** 接触头当前接触的端口（20px 内，无论是否带电），未接触返回 null */
    _contactPort() {
        if (!this.sys || !this.sys.comps || !this._tip) return null;
        ...
        for (const c of Object.values(this.sys.comps)) {
            ...
            for (const p of c.ports) {
                ...
                if (dx * dx + dy * dy < 400) return p;   // 半径 20px
            }
        }
        return null;
    }
    ...
    tick(dt) {
        ...
        // 验电测量计数：接触头接触到一个新端口（上升沿）计一次
        const cp = this._contactPort();
        const cid = cp ? cp.id : null;
        const matched = cid && (!this._measurePrefixes
            || this._measurePrefixes.some(pre => cid.startsWith(pre)));
        if (matched && cid !== this._lastContactId) this._measureCount++;
