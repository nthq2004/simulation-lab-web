...
export class InsulationIndicator extends BaseComponent {
     ...
     * @param {string} partId 'btn' = 检测（试灯）按钮帽/刀片中部
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'btn' || !partId) {
            const hitX = this._btnHatX - 20;
            const hitW = this._cx + 20 - hitX;
            return { x: gx + hitX + hitW / 2, y: gy + (this._btnTopY + this._btnAxisY + 4) / 2 };
        }
