...
/**
 * TimeDelayNOContact — 时间继电器常开延时闭合触头（NO, delay-on）
 *
 * 界面参照接触器常开辅助触头（AuxNOContact）：
 *  左/右端子 → 左/右静触头圆点，动触刀绕右端子旋转。
 *  刀臂上叠加半圆弧「延时」符号（⌒），提示延时闭合特性。
 *
 * 工作逻辑（由 TimeRelayDevice 驱动）：
 *  未到延时（idle/timing）：常开触头断开（刀片下倾）
 *  延时到达（output）    ：常开触头闭合（刀片水平，接触左静触头）
 *
 * ── 端口 ─────────────────────────────────────────────────────
 *  com — 公共端（左）
 *  no  — 常开端（右）
 */
export class TimeDelayNOContact extends BaseComponent {
    ...
    constructor(config, sys) {
        ...
        // type 复用接触器分类，使求解器对触头端口作 MNA stamp（常开触点电阻）；
        // DeviceClass 仍为 TimeRelayDevice，由时间继电器状态机驱动闭合
    ...
    tick(dt) {
        const closed = this.deviceRef ? this.deviceRef.getNOClosed() : false;
