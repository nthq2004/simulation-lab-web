...
export class ACPower3P extends BaseComponent {
    constructor(config, sys) {
        ...
        // 可识别部件（供工作流自动演示精确圈选）；电源键点击由热区接管
        const powerHit = this.addClickablePart('power', 10, 78, 35, 25);
        if (powerHit) powerHit.on('click tap', () => { this.isOn = !this.isOn; this.update(); });
    }

    /** 部件中心（画布绝对坐标，含组件自身缩放/旋转） */
