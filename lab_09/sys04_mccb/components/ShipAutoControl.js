import { BaseComponent } from './BaseComponent.js';

/**
 * ShipAutoControl 船舶电站自动控制模块
 * 尺寸 287×233，单面板。实现自动起动、自动并车、自动解列、自动调频。
 *
 * 面板布局：
 *  - 标题 + LCD 3 行（在网机组 / 总功率 / 母线电压·频率）
 *  - 状态指示灯：电源、自动模式、各机组运行
 *  - 左旋钮：手动 / 自动转换开关（左=-45°手动，右=+45°自动）
 *  - 右旋钮：备用机组顺序选择开关（左=-45°=1-2-3，右=+45°=2-1-3）
 *
 * 端口：
 *  - 上侧：bus_a / bus_b —— 汇流排采集接口（检测母线是否有电，用于失电自动起动）
 *  - 右侧：p24_p / p24_n —— 24V 电源输入（模块通电后才工作）
 *  - 下侧：comm1_a/comm1_b、comm2_a/comm2_b、comm3_a/comm3_b —— 与各发电机遥控面板的通信接口
 *
 * 控制对象：通过 units 配置关联各发电机（genId）、主开关（qfId）与遥控面板（panelId）。
 * 自动控制模块【不直接驱动发电机/主开关】，而是经通信命令控制遥控面板，
 * 由面板通过既有电气机制（端口短接/电压源）驱动机组：
 *  - 失电自动起动：母线失电时按备用顺序起动第一台机组，建压+储能后合闸
 *  - 自动并车：单机在网且负载超限时，按顺序起动下一台，同步后并入
 *  - 自动解列：多机在网且负载过低时，退出最后并入的机组
 *  - 自动调频：在线机组频率向 50Hz 调节
 */
const PANEL_W = 300;
const PANEL_H = 240;

export class ShipAutoControl extends BaseComponent {
    constructor(config, sys) {
        super(config, sys);

        this.width  = Math.max(200, config.width  || PANEL_W);
        this.height = Math.max(200, config.height || PANEL_H);

        this.type    = 'auto_control';
        this.special = 'AutoControl';
        this.cache   = 'fixed';

        this._initGroups();
        this._recalcGeometry();
        this._initParameters(config);
        this._init();

        this.config = {
            id: this.id,
            label:       this.label,
            genIds:      this.genIds.join(','),
            qfIds:       this.qfIds.join(','),
            panelIds:    this.panelIds.join(','),
            auto:        this._auto ? 'auto' : 'manual',
            seq:         this._seq,
            parallelKw:  this.parallelKw,
            decoupleKw:  this.decoupleKw,
        };

        this._addPorts();
    }

    // ═══════════════════════════════════════════
    // 几何计算
    // ═══════════════════════════════════════════

    _recalcGeometry() {
        // LCD：y=26 起，3 行
        this._lcd = { x: 5, y: 26, w: PANEL_W - 10, h: 70 };
        this._lcdRows = [30, 53, 76];

        // 指示灯行（LCD 下方），指示灯大小恢复原尺寸（r=7）
        this._leds = {
            pwr:   { x: 30,  y: 110, r: 7 },
            auto:  { x: 73, y: 110, r: 7 },
            unit1: { x: 116, y: 110, r: 7 },
            unit2: { x: 159, y: 110, r: 7 },
            unit3: { x: 202, y: 110, r: 7 },
        };

        // 两个转换开关旋钮，旋钮大小恢复原尺寸（r=23）
        this._knobMode = { x: 93, y: 170, r: 23 };
        this._knobSeq  = { x: 193, y: 170, r: 23 };

        // 端口坐标
        this._topPorts  = { bus_a: 110, bus_b: 170 };
        this._rightPorts = { p24_p: 100, p24_n: 140, heavy_a: 185, heavy_b: 225 };
        this._bottomPorts = [30, 71, 112, 153, 194, 235];
    }

    // ═══════════════════════════════════════════
    // 参数初始化
    // ═══════════════════════════════════════════

    _initParameters(config) {
        this.label    = config.label || '船舶电站自动控制模块';
        this.function = '船舶电站自动控制模块';

        const genList = String(config.genIds || 'gen1,gen2,gen3').split(',').map(s => s.trim()).filter(Boolean);
        const qfList  = String(config.qfIds  || 'qf1,qf2,qf3').split(',').map(s => s.trim()).filter(Boolean);
        this.genIds = genList;
        this.qfIds  = qfList;
        this.panelIds = String(config.panelIds || '').split(',').map(s => s.trim()).filter(Boolean);

        // 手动/自动
        this._auto = String(config.auto || 'manual') === 'auto';
        // 备用机组顺序：'123' 或 '213'
        this._seq  = String(config.seq || '123') === '213' ? '213' : '123';

        // 并车/解列负载阈值（单机额定功率的百分比）
        this.parallelKw = parseFloat(config.parallelKw) || 80;   // 并车：负载 > 单机额定 80%
        this.decoupleKw = parseFloat(config.decoupleKw) || 30;   // 解列：负载 < 单机额定 30%

        // 供电状态：p24 电压 >1V 连续 3 帧才认为有电
        this._powered    = false;
        this._powerTimer = 0;

        // 母线采集
        this._busPeak    = 0;
        this._busPortLive = false;

        // 干线短路标记：短路期间阻塞自动模式（自动指示灯熄灭，不发起动/合闸）
        this._shortFault = false;

        // 控制计时器
        this._startTimer   = {};
        this._syncTimer    = {};
        this._decoupleTimer = 0;   // 低负载累计延时（20s 后解列）
        this._stopTimer     = 0;   // 解列后停机延时（30s 后停机）
        this._decoupleIdx  = -1;   // 正在解列的机组索引（-1 表示无）
        this._lossTimer     = 0;   // 失电自动起动延时（母线失电 3s 后才发起动命令）
        this._freqAdjTimer  = 0;   // 自动调频延时（频率偏差持续 2s 后才实施调速）

        // Ⅰ级故障自动换机状态：检测到在网机组带Ⅰ级降级故障（滑油压力下降/
        // 冷却水温度异常）后自动执行换机：备用机组并车 → 负荷全部转移 →
        // 自动解列故障机组 → 故障机组自动停机。机组与主开关不因Ⅰ级故障本身动作。
        this._switchIdx   = -1;    // 换机目标（故障机组）索引，-1 表示无换机
        this._switchPhase = '';    // '' | 'parallel' | 'transfer' | 'open' | 'stop'
        this._switchTimer = 0;     // 当前阶段超时计时（s）
        this._switchHold  = 0;     // 负荷转移稳定计时（故障机功率达标连续时长，s）
        this._switchNewIdx = -1;   // 换机并入的新机组索引（并车成功后持续可调速）

        // 重载询问（侧推器）状态：面板发起重载询问后，并联时直接应答允许；
        // 单机在网时应答等待，并强制自动并车（起动+同步+合闸），并车成功后应答允许。
        this._heavyInquiry  = false;   // 是否有重载询问请求
        this._heavyResp     = '';      // 应答状态：'' | 'waiting' | 'allow'
        this._heavyForceParallel = false;  // 单机在网询问时强制并车

        // LCD 采样
        this._sample = { online: [], p: 0, live: false, lineV: 0, freq: 0 };
    }

    // ═══════════════════════════════════════════
    // 初始化
    // ═══════════════════════════════════════════

    _init() {
        this._drawStaticParts();
        this._createDynamicNodes();
        this._bindInteraction();
    }

    // ═══════════════════════════════════════════
    // 静态部件
    // ═══════════════════════════════════════════

    _drawStaticParts() {
        const s = this._staticGroup;

        // 面板（浅灰蓝底 + 深描边）
        s.add(new Konva.Rect({
            x: 0, y: 0, width: PANEL_W, height: this.height,
            fill: '#e8eef4', stroke: '#1a252f', strokeWidth: 1, cornerRadius: 3,
        }));
        s.add(new Konva.Rect({
            x: 3, y: 3, width: PANEL_W - 6, height: this.height - 6,
            fill: '#dfe7ee', cornerRadius: 2, stroke: '#5a6a75', strokeWidth: 1,
        }));
        // 标题
        s.add(new Konva.Text({
            x: 0, y: 10, width: PANEL_W, align: 'center',
            text: '船舶电站自动控制模块', fontSize: 15, fontStyle: 'bold', fill: '#1a252f',
        }));
        // LCD 黑底
        s.add(new Konva.Rect({
            x: this._lcd.x, y: this._lcd.y,
            width: this._lcd.w, height: this._lcd.h,
            fill: '#0a0e12', cornerRadius: 2, stroke: '#3a4a55', strokeWidth: 1,
        }));
        // 指示灯底盘 + 标签（大小恢复原尺寸：底盘 r=10，标签 10px）
        const ledLabels = [
            ['电源', 30], ['自动', 73], ['1#', 116], ['2#', 159], ['3#', 202],
        ];
        ledLabels.forEach(([txt, x]) => {
            s.add(new Konva.Circle({ x, y: this._leds.pwr.y, radius: 10, fill: '#cdd8e0', stroke: '#5a6a75', strokeWidth: 1 }));
            s.add(new Konva.Text({ x: x - 16, y: this._leds.pwr.y + 12, width: 32, align: 'center', text: txt, fontSize: 12, fill: '#333' }));
        });
        // 旋钮底盘
        [this._knobMode, this._knobSeq].forEach(k => {
            s.add(new Konva.Circle({ x: k.x, y: k.y, radius: k.r + 3, fill: '#cfd8df', stroke: '#5a6a75', strokeWidth: 1 }));
        });
        // 旋钮标签（宽度保持，避免两标签重叠）
        s.add(new Konva.Text({ x: this._knobMode.x - 45, y: this._knobMode.y + this._knobMode.r + 4, width: 90, align: 'center', text: '手动← →自动', fontSize: 12, fontStyle: 'bold', fill: '#1a252f' }));
        s.add(new Konva.Text({ x: this._knobSeq.x - 45, y: this._knobSeq.y + this._knobSeq.r + 4, width: 90, align: 'center', text: '顺序1-2-3/2-1-3', fontSize: 12, fontStyle: 'bold', fill: '#1a252f' }));

        // 端口标注（均置于端口上方）
        s.add(new Konva.Text({ x: 115, y: -14, width: 50, align: 'center', text: '母线采集', fontSize: 12, fontStyle: 'bold', fill: '#1a252f' }));
        s.add(new Konva.Text({ x: this.width - 22, y: 110, width: 20, align: 'center', text: '24V', fontSize: 12, fontStyle: 'bold', fill: '#1a252f' }));
        s.add(new Konva.Text({ x: this.width - 22, y: 195, width: 20, align: 'center', text: '重', fontSize: 12, fontStyle: 'bold', fill: '#1a252f' }));
        s.add(new Konva.Text({ x: 30, y: this.height - 18, width: PANEL_W - 60, align: 'center', text: '通信接口（至各遥控面板）', fontSize: 12, fill: '#5a6a75' }));

        if (this.cache === 'fixed') {
            try {
                const r = this._staticGroup.getClientRect({ relativeTo: this._staticGroup });
                if (r && r.width > 0 && r.height > 0) {
                    this._staticGroup.cache({
                        x: r.x, y: r.y, width: Math.ceil(r.width), height: Math.ceil(r.height),
                    });
                }
            } catch (e) { /* ignore */ }
        }
    }

    // ═══════════════════════════════════════════
    // 动态节点
    // ═══════════════════════════════════════════

    _createDynamicNodes() {
        const d = this._dynamicGroup;
        const ui = {};

        // LCD 3 行文字
        const mkLcd = (y) => {
            const t = new Konva.Text({
                x: this._lcd.x + 4, y, fontSize: 16,
                fontFamily: 'monospace', fontStyle: 'bold',
                fill: '#00ff88', text: '',
            });
            d.add(t);
            return t;
        };
        ui.lcd = this._lcdRows.map(mkLcd);

        // 指示灯
        ui.leds = {};
        Object.entries(this._leds).forEach(([k, v]) => {
            const c = new Konva.Circle({ x: v.x, y: v.y, radius: v.r, fill: '#8a8a8a', stroke: '#222', strokeWidth: 1 });
            d.add(c);
            ui.leds[k] = c;
        });

        // 手动/自动 旋钮
        const mkKnob = (x, y, r, initRot) => {
            const g = new Konva.Group({ x, y });
            const disk = new Konva.Circle({ radius: r, fill: '#7f8c8d', stroke: '#4a5a63', strokeWidth: 2 });
            const ptr = new Konva.Line({
                points: [0, 0, 0, -(r - 3)], stroke: '#ffffff', strokeWidth: 3, lineCap: 'round',
            });
            ptr.rotation(initRot);
            g.add(disk, ptr);
            d.add(g);
            return { g, disk, ptr };
        };
        ui.knobMode = mkKnob(this._knobMode.x, this._knobMode.y, this._knobMode.r, this._auto ? 45 : -45);
        ui.knobSeq  = mkKnob(this._knobSeq.x, this._knobSeq.y, this._knobSeq.r, this._seq === '213' ? 45 : -45);

        this._ui = ui;
    }

    // ═══════════════════════════════════════════
    // 交互绑定
    // ═══════════════════════════════════════════

    _bindInteraction() {
        // 手动/自动 转换开关：点击切换
        this._ui.knobMode.disk.on('click tap', (e) => {
            e.cancelBubble = true;
            this._auto = !this._auto;
            this.config.auto = this._auto ? 'auto' : 'manual';
            this._tweenKnob(this._ui.knobMode.ptr, this._auto ? 45 : -45);
        });

        // 备用机组顺序选择开关：点击切换 1-2-3 / 2-1-3
        this._ui.knobSeq.disk.on('click tap', (e) => {
            e.cancelBubble = true;
            this._seq = this._seq === '213' ? '123' : '213';
            this.config.seq = this._seq;
            this._tweenKnob(this._ui.knobSeq.ptr, this._seq === '213' ? 45 : -45);
        });
    }

    _tweenKnob(ptr, angle) {
        if (ptr._tw) ptr._tw.destroy();
        ptr._tw = new Konva.Tween({ node: ptr, rotation: angle, duration: 0.15 });
        ptr._tw.play();
    }

    _touchDirty() {
        this.markDirty();
        if (this.sys && typeof this.sys.requestRedraw === 'function') this.sys.requestRedraw();
    }

    // ═══════════════════════════════════════════
    // 端口
    // ═══════════════════════════════════════════

    _addPorts() {
        const tp = this._topPorts, rp = this._rightPorts;
        // 上侧：母线采集
        this.addPort(tp.bus_a, 2, 'bus_a', 'wire', 'p');
        this.addPort(tp.bus_b, 2, 'bus_b', 'wire');
        // 右侧：24V 电源
        this.addPort(this.width, rp.p24_p, 'p24_p', 'wire', 'p');
        this.addPort(this.width, rp.p24_n, 'p24_n', 'wire');
        // 右侧：重载询问通信接口（电源接口下方，与侧推器重载询问面板通信）
        this.addPort(this.width, rp.heavy_a, 'heavy_a', 'wire', 'p');
        this.addPort(this.width, rp.heavy_b, 'heavy_b', 'wire');
        // 下侧：3 个通信接口（每面板 2 线，左侧线为 'p'）
        const h = this.height;
        this.addPort(this._bottomPorts[0], h, 'comm1_a', 'wire', 'p');
        this.addPort(this._bottomPorts[1], h, 'comm1_b', 'wire');
        this.addPort(this._bottomPorts[2], h, 'comm2_a', 'wire', 'p');
        this.addPort(this._bottomPorts[3], h, 'comm2_b', 'wire');
        this.addPort(this._bottomPorts[4], h, 'comm3_a', 'wire', 'p');
        this.addPort(this._bottomPorts[5], h, 'comm3_b', 'wire');
    }

    // ═══════════════════════════════════════════
    // 仿真主循环
    // ═══════════════════════════════════════════

    tick(dt) {
        this._sensePower();
        this._senseBus();
        this._sampleUnits();
        this._scanShort();
        this._autoControl(dt);
        this._updateLCD();
        this._updateLeds();
        this.markDirty();
        if (this.sys && typeof this.sys.requestRedraw === 'function') this.sys.requestRedraw();
    }

    // 干线短路扫描：系统中任一组件带 _faultShort 标记即视为短路（与 MarineMainsSwitch 同规约）。
    // 短路期间：阻塞自动模式（不发起动/合闸），修复（标记清除）后自动恢复。
    _scanShort() {
        const sys = this.sys;
        if (!sys || !sys.comps) return;
        let hit = false;
        for (const id in sys.comps) {
            const c = sys.comps[id];
            if (c && c._faultShort) { hit = true; break; }
        }
        this._shortFault = hit;
    }

    // 机组故障判定：AVR 欠压故障或任一发动机故障（超速/滑油低压/冷却水温高）视为不可用
    _hasFault(u) {
        if (!u || !u.gen) return false;
        const g = u.gen;
        if (g._faultAVR) return true;
        const ef = typeof g.getEngineFaults === 'function' ? g.getEngineFaults() : null;
        return !!ef && (ef.overspeed || ef.oilPress || ef.coolantTemp);
    }

    // 机组Ⅰ级降级故障判定：滑油压力下降 / 冷却水温度异常。
    // Ⅰ级故障不触发停机/跳闸，机组继续运行，仅由自动换机流程将其退出电网。
    _hasLv1Fault(u) {
        if (!u || !u.gen) return false;
        const g = u.gen;
        return typeof g.hasLv1Fault === 'function' ? g.hasLv1Fault() : false;
    }

    _sensePower() {
        const solver = this.sys && this.sys.voltageSolver;
        if (!solver) return;
        const v = this.sys.getVoltageBetween(`${this.id}_wire_p24_p`, `${this.id}_wire_p24_n`);
        if (v !== undefined && isFinite(v) && v > 1) {
            this._powerTimer = Math.min(3, this._powerTimer + 1);
        } else {
            this._powerTimer = 0;
        }
        this._powered = this._powerTimer >= 3;
    }

    _senseBus() {
        const solver = this.sys && this.sys.voltageSolver;
        if (!solver) return;
        const v = this.sys.getVoltageBetween(`${this.id}_wire_bus_a`, `${this.id}_wire_bus_b`);
        if (v !== undefined && isFinite(v)) {
            this._busPeak = Math.max(this._busPeak * 0.85, Math.abs(v));
        } else {
            this._busPeak = 0;
        }
        this._busPortLive = this._busPeak > 60;
    }

    _getUnit(genId, qfId, panelId) {
        const gen = genId && this.sys.comps[genId] ? this.sys.comps[genId] : null;
        const qf  = qfId  && this.sys.comps[qfId]  ? this.sys.comps[qfId]  : null;
        const panel = panelId && this.sys.comps[panelId] ? this.sys.comps[panelId] : null;
        if (!gen) return null;
        const qfOn = qf && typeof qf.isClosed === 'function' ? qf.isClosed() : false;
        return {
            genId, qfId, panelId, gen, qf, panel,
            on:    !!gen.isOn,
            qfOn,
            online: !!gen.isOn && qfOn,
            freq:  (gen._freqOut ?? gen.freq) || 0,
            lineV: gen.getLineVoltage ? gen.getLineVoltage() : 0,
            pwr:   gen._pwr || 0,
            charged: qf ? (typeof qf.isCharged === 'function' ? qf.isCharged() : true) : false,
            powered: panel ? (typeof panel.isPowered === 'function' ? panel.isPowered() : true) : true,
        };
    }

_sampleUnits() {
        const units = [];
        for (let i = 0; i < this.genIds.length; i++) {
            const u = this._getUnit(this.genIds[i], this.qfIds[i], this.panelIds[i]);
            if (u) units.push(u);
        }
        this._units = units;
        const online = units.filter(u => u.online);
        this._sample.online = online;
        this._sample.p = online.reduce((a, u) => a + u.pwr, 0);
        // 母线带电以"在网机组"为准（qf 分闸时母线端口会经 10e9Ω 泄漏感应出非零电压，不能作为判据）
        this._sample.live = online.length > 0;
        if (online.length > 0) {
            // 线电压取在网机组端子 RMS（_rmsV 为端子相电压，非 AVR 内部电动势 _vRmsOut）
            const g = online[0].gen;
            const termV = (g && g._rmsV > 0) ? g._rmsV : (g ? g.vRms : 230);
            this._sample.lineV = Math.sqrt(3) * termV;
            this._sample.freq  = online[0].freq;
        } else {
            this._sample.lineV = 0;
            this._sample.freq  = 0;
        }
    }

    // 备用顺序：返回机组索引数组（从 genIds 中选择）
    _seqOrder() {
        const order = this._seq === '213' ? [1, 0, 2] : [0, 1, 2];
        return order.filter(i => i < this.genIds.length);
    }

    // 单机额定功率（kW）：取第一台在线机组额定值，兜底 400
    _genRated() {
        for (const u of this._units || []) {
            if (u && u.gen && typeof u.gen.ratedPower === 'number' && u.gen.ratedPower > 0) {
                return u.gen.ratedPower;
            }
        }
        return 400;
    }

    _autoControl(dt) {
        if (!this._powered || !this._auto) return;
        // 短路期间阻塞自动模式：不发起动/合闸/调速，防止对短路点反复送电
        if (this._shortFault) {
            this._decoupleIdx = -1;
            this._startTimer = {};
            this._syncTimer = {};
            this._decoupleTimer = 0;
            this._stopTimer = 0;
            this._lossTimer = 0;
            this._freqAdjTimer = 0;
            return;
        }
        const units = this._units || [];
        const online = this._sample.online || [];
        const busLive = this._sample.live;

        // 每帧统一计算各机组命令，未显式设置的机组默认 null（清除残留命令）
        const cmds = new Array(this.genIds.length).fill(null);

        // ── 失电自动起动：母线无电 → 延迟 3s 发出起动信号，起动成功后再延迟 3s 发出合闸信号 ──
        if (!busLive) {
            // 失电延时统一计时（故障机组跳过时同样计时，保证 3s 后能转向下一台可用机组）
            this._lossTimer += dt;
            const order = this._seqOrder();
            for (const i of order) {
                const u = units[i];
                if (!u) continue;
                // 跳过故障机组（欠压/超速/滑油低压/水温高），继续尝试后续机组
                if (this._hasFault(u)) continue;
                if (u.on) {
                    // 已运行但未合闸：建压+储能完成后，再延迟 3s 发出合闸信号
                    if (!(this._startTimer[i] >= 0)) this._startTimer[i] = 0;
                    const ready = u.lineV > 100 && u.freq > 45 && u.freq < 55;
                    if (ready && u.charged) {
                        this._startTimer[i] = (this._startTimer[i] || 0) + dt;
                        if (this._startTimer[i] > 3) cmds[i] = 'close';
                    } else {
                        this._startTimer[i] = 0;
                    }
                } else {
                    // 未运行：失电累计 3s 后，经面板发出起动信号
                    if (this._lossTimer > 3) {
                        cmds[i] = 'start';
                        this._startTimer[i] = 0;
                    }
                }
                break;
            }
        } else {
            // ── 母线有电：复位失电起动延时 ──
            this._lossTimer = 0;

            // ── 重载询问（侧推器）应答状态机 ──
            // 有询问且母线有电时评估：并联（≥2 台在网）→ 直接应答允许；
            // 单机在网 → 应答等待并强制自动并车；无在网 → 保持等待（不答复允许）。
            if (this._heavyInquiry) {
                if (online.length >= 2) {
                    this._heavyResp = 'allow';
                    this._heavyForceParallel = false;
                } else if (online.length === 1) {
                    this._heavyResp = 'waiting';
                    this._heavyForceParallel = true;
                } else {
                    this._heavyResp = 'waiting';
                    this._heavyForceParallel = false;
                }
            }

            // ── Ⅰ级故障自动换机（换机中或存在带Ⅰ级故障的在网机组时独占控制）──
            // 流程：备用机组自动并车 → 负荷全部转移给新机 → 自动解列故障机组 →
            // 故障机组自动停机。换机期间短路常规并车/解列/均分/调频逻辑，避免命令冲突。
            const lv1On = online.some(u => this._hasLv1Fault(u));
            if (lv1On || this._switchIdx >= 0) {
                this._switchLogic(dt, units, online, cmds);
            } else {
                // ── 常规自动控制（并车 / 解列 / 均分 / 调频）──
                // ── 自动并车：单机在网且负载 > 单机额定 80% → 起动下一台并同步合闸 ──
                // 重载询问（单机在网）时强制并车：无视负载阈值直接起动备用机。
                if (online.length === 1 && (this._sample.p > this.parallelKw / 100 * this._genRated() || this._heavyForceParallel)) {
                const inOnline = new Set(online.map(u => u.genId));
                const order = this._seqOrder();
                for (const i of order) {
                    const u = units[i];
                    if (!u || inOnline.has(u.genId)) continue;
                    // 跳过故障机组，继续尝试后续机组
                    if (this._hasFault(u)) continue;
                    if (!u.on) {
                        cmds[i] = 'start';
                        this._syncTimer[i] = 0;
                        break;
                    }
                    // 已运行：同步频率到母线频率+0.1Hz（待并机略高于电网，确保合闸后带正功率）
                    const ref = online[0].freq;
                    const target = ref + 0.1;
                    const df = target - u.freq;
                    if (Math.abs(df) < 0.1) {
                        this._syncTimer[i] = (this._syncTimer[i] || 0) + dt;
                        if (this._syncTimer[i] > 2 && u.charged) cmds[i] = 'close';
                    } else {
                        cmds[i] = df > 0 ? 'spd-inc' : 'spd-dec';
                        this._syncTimer[i] = 0;
                    }
                    break;
                }
            }

            // ── 自动解列：多机在网且负载 < 单机额定 30%，延时 15s 解列、再延时 10s 停机 ──
            // 重载询问期间（_heavyInquiry）不解列：自动电站为侧推器等大负载"锁定"备用机组，
            // 避免并车后因基载偏低把备用机解列导致询问应答回退到 waiting、大负载无法起动。
            const decoupling = this._decoupleIdx >= 0;
            const lowLoad = online.length > 1 && this._sample.p < this.decoupleKw / 100 * this._genRated();
            if (!decoupling && lowLoad && !this._heavyInquiry) {
                this._decoupleTimer += dt;
                if (this._decoupleTimer > 15) {
                    const order = this._seqOrder().slice().reverse();
                    for (const i of order) {
                        const u = units[i];
                        if (!u || !u.online) continue;
                        // 故障机组不参与自动解列（避免把带病机组甩下后失电）
                        if (this._hasFault(u)) continue;
                        this._decoupleIdx = i;
                        this._decoupleTimer = 0;
                        this._stopTimer = 0;
                        break;
                    }
                }
            } else if (!decoupling && !lowLoad) {
                // 负载恢复/单机在网：清除解列累计
                this._decoupleTimer = 0;
            }
            if (this._decoupleIdx >= 0) {
                const u = units[this._decoupleIdx];
                if (u && u.on) {
                    if (u.qfOn) {
                        // 发电机仍在运行且已合闸：发解列（分闸）命令
                        cmds[this._decoupleIdx] = 'open';
                    } else {
                        // 已分闸：延时 10s 后停机
                        this._stopTimer += dt;
                        if (this._stopTimer > 10) cmds[this._decoupleIdx] = 'stop';
                    }
                } else {
                    // 已完成解列（机组已停机），清除状态
                    this._decoupleIdx = -1;
                }
            }

            // ── 并联负荷均分（并车成功后自动转移负荷至均分）──
            // 并车瞬间新机带小功率是正常现象（正频差并车 + _parRatio 残差）；随后自动
            // 模块通过面板调速改变各机设定频率 → 物理模型 _parTrans=(freq-fAvg)×0.2
            // 将显示功率从过载机组转移到欠载机组，直至各机显示功率与平均值偏差小于死区。
            // 注意：均分基准必须用"显示功率平均"（与比较对象 _displayP 同源）。真实功率
            // 已由内阻调差自然均分，若用真实功率作基准（_sample.p/n）会误判过载 → 持续
            // 压设定频率直到下限（死锁）→ 且 shareActive 恒真屏蔽调频 → 系统频率异常下滑。
            let shareActive = false;
            if (online.length > 1) {
                let dSum = 0;
                for (const u of online) {
                    dSum += (u.gen && isFinite(u.gen._displayP)) ? u.gen._displayP : u.pwr;
                }
                const avgP = dSum / online.length;
                const thr = this._genRated() * 0.03;   // 均分死区：单机额定 3%（基准同源后收紧，慢速调速无振荡）
                for (const u of online) {
                    const i = this.genIds.indexOf(u.genId);
                    if (i < 0 || !u.powered || i === this._decoupleIdx) continue;
                    // 以显示功率判断（LCD/面板同源），调速转移的是显示功率
                    const Pg = (u.gen && isFinite(u.gen._displayP)) ? u.gen._displayP : u.pwr;
                    const dP = Pg - avgP;
                    if (Math.abs(dP) > thr) {
                        shareActive = true;
                        // 慢速调速：每 0.1Hz 频差对应 2% 总功率转移，慢速可避免振荡
                        cmds[i] = dP > 0 ? 'spd-dec-slow' : 'spd-inc-slow';
                    }
                }
            }

            // ── 自动调频（经面板调速）──
            // 并联运行时，物理模型通过内阻调差自然均分负荷（+0.1Hz 并车确保新机带正功率）；
            // 此处仅做系统频率调节：均分未完成时不调频（避免同机命令冲突），
            // 频率偏差持续 2s 后统一调速（慢速 spd-inc-slow/spd-dec-slow 防止过冲振荡）。
            if (online.length > 1) {
                if (shareActive) {
                    this._freqAdjTimer = 0;
                } else {
                    const fAvg = online.reduce((a, u) => a + u.freq, 0) / online.length;
                    if (fAvg < 49.7 || fAvg > 50.3) this._freqAdjTimer += dt;
                    else this._freqAdjTimer = 0;
                    if (this._freqAdjTimer >= 2) {
                        const dir = fAvg < 49.7 ? 'spd-inc-slow' : 'spd-dec-slow';
                        for (const u of online) {
                            const i = this.genIds.indexOf(u.genId);
                            if (i < 0 || !u.powered || i === this._decoupleIdx) continue;
                            cmds[i] = dir;
                        }
                    }
                }
            } else if (online.length === 1) {
                // 单机在线：按单机频率调频。死区放宽到 ±0.3Hz、慢速调速，
                // 避免带载下垂（正常的负荷-频率特性）引发设定频率来回振荡。
                const u = online[0];
                const i = this.genIds.indexOf(u.genId);
                if (i < 0 || !u.powered || i === this._decoupleIdx) { this._freqAdjTimer = 0; }
                else {
                    const f = u.freq;
                    if (f < 49.7 || f > 50.3) this._freqAdjTimer += dt;
                    else this._freqAdjTimer = 0;
                    if (this._freqAdjTimer >= 2) {
                        cmds[i] = f < 49.7 ? 'spd-inc-slow' : 'spd-dec-slow';
                    }
                }
            }
            }
        }

        // 统一下发命令（面板未通电的机组不发）
        for (let i = 0; i < this.genIds.length; i++) {
            const u = units[i];
            if (!u || !u.panel || typeof u.panel.setCommCmd !== 'function') continue;
            u.panel.setCommCmd(u.powered ? cmds[i] : null);
        }
    }

    // ═══════════════════════════════════════════
    // Ⅰ级故障自动换机（并车 → 转移 → 解列 → 停机）
    // ═══════════════════════════════════════════

    // 自动切换同步表选择开关到目标面板档位（自动并车合闸联锁的前提：
    // 电网有电时面板仅当选择开关转到本机档位才允许合闸）。
    _setSyncSel(panel) {
        if (!panel) return;
        const selId = panel.syncSelId, pos = panel.selPos;
        if (!selId || !pos || !this.sys || !this.sys.comps) return;
        const sel = this.sys.comps[selId];
        if (sel && typeof sel.switchTo === 'function' && typeof sel.getPosition === 'function') {
            if (sel.getPosition() !== pos) sel.switchTo(pos);
        }
    }

    // Ⅰ级故障自动换机状态机。
    // 检测：在网机组带Ⅰ级降级故障（滑油压力下降/冷却水温度异常）。
    // 流程（各阶段均设置时延与超时保护，避免卡死）：
    //   parallel  备用机组自动并车：起动备用机 → 频率同步至在线机+0.1Hz →
    //             同步稳定且储能满后切换同步表档位并发合闸命令（主开关自动同期对齐）。
    //   transfer  负荷全部转移：故障机 spd-dec-slow / 新机 spd-inc-slow 持续调速，
    //             直至故障机显示功率 ≤ 5% 额定并稳定 3s（或超时 45s 强制）。
    //   open      自动解列故障机组：对其主开关发分闸命令。
    //   stop      故障机组自动停机：分闸后延时 5s 发停机命令，停机后复位换机状态。
    _switchLogic(dt, units, online, cmds) {
        // ── 目标选择与状态复位 ──
        const _reset = () => {
            this._switchIdx = -1; this._switchPhase = '';
            this._switchTimer = 0; this._switchHold = 0; this._switchNewIdx = -1;
        };

        // 无换机目标时：取第一台带Ⅰ级故障的在网机组发起换机
        if (this._switchIdx < 0) {
            const lv1 = online.filter(u => this._hasLv1Fault(u));
            if (lv1.length === 0) { _reset(); return; }
            // 首次进入换机：清理常规控制残留状态（解列/并车计时器），
            // 避免换机结束后残留计时导致误解列/误并车。
            this._decoupleIdx = -1;
            this._decoupleTimer = 0;
            this._stopTimer = 0;
            this._startTimer = {};
            this._syncTimer = {};
            this._switchIdx = this.genIds.indexOf(lv1[0].genId);
            this._switchPhase = 'parallel';
            this._switchTimer = 0;
            this._switchHold = 0;
            this._switchNewIdx = -1;
        }

        const fi = this._switchIdx;          // 故障机组索引
        const fu = units[fi];
        if (!fu || !this._hasLv1Fault(fu)) { // 目标失效（故障已修复等）→ 复位
            _reset();
            return;
        }

        // ── 备用机组选择：按备用顺序取首台 非故障(Ⅱ/Ⅰ级) 且 不在网 的机组 ──
        const inOnline = new Set(online.map(u => u.genId));
        let bi = -1;
        const order = this._seqOrder().filter(i => i !== fi);
        for (const i of order) {
            const u = units[i];
            if (!u || inOnline.has(u.genId)) continue;
            if (this._hasFault(u) || this._hasLv1Fault(u)) continue;
            bi = i; break;
        }

        this._switchTimer += dt;
        const phase = this._switchPhase;

        if (phase === 'parallel') {
            // 并车成功检测优先：目标新机（已记住索引）在线 → 立即进入负荷转移。
            // 注意：并车合闸后新机已属"在网"，备用机选择循环（bi）会跳过它，
            // 故不能依赖 bi/bu 判断并车结果，必须用 _switchNewIdx 持久跟踪。
            const ni0 = this._switchNewIdx;
            if (ni0 >= 0 && units[ni0] && units[ni0].online) {
                this._switchPhase = 'transfer'; this._switchTimer = 0; this._switchHold = 0;
            } else if (bi >= 0) {
                // ── 并车推进：无备用机时保持现网运行（超时后放弃）──
                this._switchNewIdx = bi;   // 记住新机：并车成功后仍可对其调速
                const bu = units[bi];
                if (!bu.on) {
                    cmds[bi] = 'start';
                    this._syncTimer[bi] = 0;
                } else {
                    // 频率同步到在线机 +0.1Hz（待并机略高，确保并闸后带正功率）
                    const leader = online.find(u => u.genId !== bu.genId) || online[0];
                    const ref = leader ? leader.freq : 50;
                    const target = ref + 0.1;
                    const df = target - bu.freq;
                    if (Math.abs(df) < 0.1) {
                        this._syncTimer[bi] = (this._syncTimer[bi] || 0) + dt;
                        if (this._syncTimer[bi] > 2 && bu.charged) {
                            // 合闸前切同步表档位（自动并车联锁前提），随后持续发合闸命令
                            this._setSyncSel(bu.panel);
                            cmds[bi] = 'close';
                        }
                    } else {
                        cmds[bi] = df > 0 ? 'spd-inc' : 'spd-dec';
                        this._syncTimer[bi] = 0;
                    }
                }
            } else {
                // 无不在网备用机可选（常规自动并车可能已提前完成并车）：
                // 若已有其它在网健康机组，直接以其作为转移对象进入负荷转移阶段，
                // 否则保持等待（超时后放弃本次换机）。
                const alt = online.find(u => u.genId !== fu.genId
                    && !this._hasLv1Fault(u) && !this._hasFault(u));
                if (alt) {
                    this._switchNewIdx = this.genIds.indexOf(alt.genId);
                    this._switchPhase = 'transfer'; this._switchTimer = 0; this._switchHold = 0;
                }
            }
            // 超时保护：长时间无法起动/合闸则放弃本次换机
            if (this._switchTimer > 60) _reset();
        } else if (phase === 'transfer') {
            // 负荷全部转移：压故障机设定频率、抬新机设定频率，
            // 物理模型 _parTrans 将显示功率从故障机逐步转移至新机
            const ni = this._switchNewIdx;
            if (ni >= 0 && units[ni]) cmds[ni] = 'spd-inc';
            cmds[fi] = 'spd-dec';
            const gl = fu.gen;
            const rated = (gl && gl.ratedPower > 0) ? gl.ratedPower : 80;
            const Pd = (gl && isFinite(gl._displayP)) ? gl._displayP : ((gl && gl._pwr) || 0);
            if (Pd <= rated * 0.05) {
                // 故障机功率已卸至 ≤5% 额定：稳定 3s 后进入解列
                this._switchHold += dt;
                if (this._switchHold >= 3) {
                    this._switchPhase = 'open'; this._switchTimer = 0; this._switchHold = 0;
                }
            } else {
                this._switchHold = 0;
            }
            // 转移超时：45s 仍未卸净则强制进入解列（避免死循环）
            if (this._switchTimer > 45) {
                this._switchPhase = 'open'; this._switchTimer = 0; this._switchHold = 0;
            }
        } else if (phase === 'open') {
            // 自动解列故障机组：分闸其主开关
            if (fu.qfOn) {
                cmds[fi] = 'open';
            } else {
                // 主开关已分闸：进入停机阶段
                this._switchPhase = 'stop'; this._switchTimer = 0;
            }
            if (this._switchTimer > 30) _reset();
        } else if (phase === 'stop') {
            // 自动停机：分闸后延时 5s 发停机命令
            if (this._switchTimer > 5) {
                cmds[fi] = 'stop';
            }
            if (!fu.on) _reset();          // 故障机组已停机：换机完成
            else if (this._switchTimer > 30) _reset();
        }
    }

    _lcdLines() {
        if (!this._powered) return ['--', '--', '--'];
        const online = this._sample.online || [];
        const labels = online.map(u => {
            const idx = this.genIds.indexOf(u.genId);
            return `${idx + 1}#`;
        });
        const live = this._sample.live;
        return [
            live ? `在网:${labels.join(' ') || '无'}` : '母线 失电',
            `总功率 ${this._sample.p > 100 ? this._sample.p.toFixed(0) : this._sample.p.toFixed(1)} kW`,
            live ? `母线 ${this._sample.lineV.toFixed(0)}V ${this._sample.freq.toFixed(1)}Hz` : '母线 失电',
        ];
    }

    _updateLCD() {
        if (!this._ui) return;
        const lines = this._lcdLines();
        this._ui.lcd.forEach((t, i) => t.text(lines[i]));
    }

    _updateLeds() {
        const ui = this._ui;
        if (!ui || !ui.leds) return;
        const on = (led, on) => led.fill(on ? '#ffffff' : '#8a8a8a');
        on(ui.leds.pwr, this._powered);
        // 短路期间阻塞自动模式：自动指示灯熄灭（即使旋钮仍在自动位）
        on(ui.leds.auto, this._auto && !this._shortFault);
        for (let i = 0; i < 3; i++) {
            const led = ui.leds['unit' + (i + 1)];
            if (!led) continue;
            const u = (this._units || [])[i];
            on(led, !!(u && u.on));
        }
    }

    // ═══════════════════════════════════════════
    // 公开 API
    // ═══════════════════════════════════════════

    isPowered() { return this._powered; }
    isAuto()    { return this._auto; }
    getSeq()    { return this._seq; }
    getBusLive(){ return this._sample.live; }

    // ── 重载询问（侧推器）通信接口 ──────────────────────────
    // 面板发起重载询问：并联 → 直接应答允许；单机 → 应答等待并强制自动并车，并车成功后改答允许。
    heavyInquiryRequest() {
        this._heavyInquiry = true;
        this._heavyResp = '';
        this._heavyForceParallel = false;
    }
    // 面板获得允许并起动负载后（或放弃询问）清除询问状态
    heavyInquiryClear() {
        this._heavyInquiry = false;
        this._heavyResp = '';
        this._heavyForceParallel = false;
    }
    // 应答状态：''（无询问）| 'waiting' | 'allow'
    getHeavyResponse() {
        if (!this._heavyInquiry) return '';
        return this._heavyResp || 'waiting';
    }

    /**
     * 返回可点击部件中心坐标（供工作流 find/箭头指示定位）
     * @param {string} partId 'knob-mode'（手动/自动）| 'knob-seq'（备用顺序）| 'lcd' | 'led-auto'
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'knob-mode') return { x: gx + this._knobMode.x, y: gy + this._knobMode.y };
        if (partId === 'knob-seq')  return { x: gx + this._knobSeq.x,  y: gy + this._knobSeq.y };
        if (partId === 'lcd')       return { x: gx + this._lcd.x + this._lcd.w / 2, y: gy + this._lcd.y + this._lcd.h / 2 };
        if (partId === 'led-auto')  return { x: gx + this._leds.auto.x, y: gy + this._leds.auto.y };
        return null;
    }

    getConfigFields() {
        return [
            { label: '发电机 ID 列表(逗号分隔)', key: 'genIds', type: 'text' },
            { label: '主开关 ID 列表(逗号分隔)', key: 'qfIds',  type: 'text' },
            { label: '遥控面板 ID 列表(逗号分隔)', key: 'panelIds', type: 'text' },
            { label: '初始状态', key: 'auto', type: 'select', get: c => this._auto ? 'auto' : 'manual', options: [
                { label: '手动', value: 'manual' },
                { label: '自动', value: 'auto' },
            ]},
            { label: '备用顺序', key: 'seq', type: 'select', get: c => this._seq, options: [
                { label: '1-2-3', value: '123' },
                { label: '2-1-3', value: '213' },
            ]},
            { label: '并车负载阈值 (单机额定 %)', key: 'parallelKw', type: 'number', step: 5 },
            { label: '解列负载阈值 (单机额定 %)', key: 'decoupleKw', type: 'number', step: 5 },
        ];
    }

    onConfigUpdate(cfg) {
        if (cfg.genIds !== undefined && typeof cfg.genIds === 'string') {
            this.genIds = cfg.genIds.split(',').map(s => s.trim()).filter(Boolean);
        }
        if (cfg.qfIds !== undefined && typeof cfg.qfIds === 'string') {
            this.qfIds = cfg.qfIds.split(',').map(s => s.trim()).filter(Boolean);
        }
        if (cfg.panelIds !== undefined && typeof cfg.panelIds === 'string') {
            this.panelIds = cfg.panelIds.split(',').map(s => s.trim()).filter(Boolean);
        }
        if (cfg.auto !== undefined) this._auto = String(cfg.auto) === 'auto';
        if (cfg.seq  !== undefined) this._seq  = String(cfg.seq) === '213' ? '213' : '123';
        if (cfg.parallelKw !== undefined) this.parallelKw = parseFloat(cfg.parallelKw);
        if (cfg.decoupleKw !== undefined) this.decoupleKw = parseFloat(cfg.decoupleKw);
        this.config = { ...this.config, ...cfg };
        this.markDirty();
    }
}
