...
export class HvGenProtection extends BaseComponent {
    ...
    _evaluateFault() {
        ...
        // 欠压：面板励磁开关 OFF（人为灭磁/停机过程）时闭锁，避免正常停机被误判为欠压故障
        const genRef = this._gen ? this._gen() : null;
        const fieldRemoved = !!(genRef && genRef._fieldOn === false);
        if (!fieldRemoved && this._V3 > 0 && this._V3 < this.uvRatio * (Un / Math.sqrt(3))) {
            return { active: 'uv', time: this.uvTime };
        }
...
this._outMaxInst = 0;
    ...
    tick(dt) {
        ...
        const qfStart = this._qf();
        const qfClosed = !!(qfStart && typeof qfStart.isClosed === 'function' && qfStart.isClosed());
        ...
        // 保护有效条件：发电机运行 + 主开关（真空断路器）已合闸 + 稳定期后。
        //   主开关断开时 CT 无电流、保护出口不动作；停机过渡的滑窗残留值也不误判。
        const protEnabled = this._powered && !this._tripped && this._startedEver
            && gOn && qfClosed && this._stableT > 3;
        if (protEnabled) {
        ...
        } else if (!this._tripped && this._phase === 'delay') {
            // 保护条件不满足（主开关断开/发电机停机等）→ 复位延时中状态
            this._phase = 'idle';
            this._active = 'normal';
            this._delayT = 0;
        }
