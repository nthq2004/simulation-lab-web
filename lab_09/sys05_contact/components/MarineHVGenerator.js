...
export class MarineHVGenerator extends SyncGenerator3P {
    ...
    _initParameters(config) {
        ...
        this._demagnetized = false;   // true = 灭磁继电器得电/励磁开关 OFF，励磁消失
        this._fieldOn = true;         // 面板励磁开关：默认 ON（正常励磁）
        this._fieldFactor = 1;        // 励磁强度系数（0~1），灭磁/建磁平滑过渡
    }
    ...
    /** 设置面板励磁开关（ON/OFF），同步拨杆视觉；供交互点击与工作流调用 */
    setFieldOn(on) {
        this._fieldOn = !!on;
        if (this._excKnob) {
    ...
    /** 面板励磁开关是否在 ON 位 */
    isFieldOn() { return !!this._fieldOn; }
    ...
    _applyDemagVisual() {
        ...
        const faded = this._fieldFactor < 0.5;
        this._poleRects.forEach((rect, i) => {
            rect.fill(faded ? '#a8adb5' : (i % 2 === 0 ? '#d03030' : '#3060c8'));
            rect.stroke(faded ? '#7a8088' : '#8a1a1a');
        });
    }

    /** 定子电动势随励磁强度系数衰减（灭磁平滑过渡到 0，发电机不发电） */
    getPhaseVoltage(phase, time) {
        if (this._fieldFactor <= 0) return 0;
        return super.getPhaseVoltage(phase, time) * this._fieldFactor;
    }
    ...
    tick(dt) {
        this._senseDemag();
        // 面板励磁开关 OFF 或灭磁继电器动作 → 目标励磁为 0
        const fieldOff = (this._fieldOn === false) || this._demagnetized;
        // 灭磁平滑过渡：避免电动势瞬时跳变。求解器 stamp 与保护采样存在帧间不一致，
        // 若电动势突变为 0，机端 CT 会算出虚假冲击电流并误触发短路保护。
        // 建磁（励磁恢复）则立即生效，保持原有起动/自动合闸时序不变。
        if (fieldOff) {
            const k = Math.min(1, (dt || 0.05) / 1.2);   // 灭磁时间常数 1.2s
            this._fieldFactor += (0 - this._fieldFactor) * k;
            if (this._fieldFactor < 0.002) this._fieldFactor = 0;
        } else {
            this._fieldFactor = 1;
        }
        this._demagnetized = fieldOff;      // 供显示/继电器状态使用
        super.tick(dt);
