...
export class Multimeter extends BaseComponent {
    ...
    _createUI() {
        ...
        // ── 不缓存 this.group（端口在 _createUI 之后添加，缓存会阻断端口事件）
        //    _staticGroup 的缓存由 consys._applyStaticCaching 统一管理
    ...
    /**
     * 切档后立即用当前档位真实测量值刷新显示。
     * 不沿用上一档位的旧读数（例如 RES 档 O.L 哨兵 1000000），
     * 否则切到 ACV 档会错误地显示 O.L（直到下一帧 tick 才被真实电压覆盖）。
     */
    _measureNow() {
        // 绕过节流：清除 pending 并强制下一次 update 立即渲染，
        // 避免切档后仍沿用上一档位屏幕上的旧读数（如 RES 档 O.L）
    ...
    _updateModeByAngle(angle) {
        ...
        // 档位变化时重置测量缓冲：避免 ACV/电流档的 RMS 环形缓冲残留旧档位数据，
        // 导致切档后移动表笔仍显示上一点的大电压（如断点电压残留为 O.L/大数）
        if (this.mode !== this._lastMode) {
            this._resetMeasureBuffers();
        }
    }

    /**
     * 档位变化时重置 RMS 环形缓冲（ACV 电压 / mA 电流）
     */
    _resetMeasureBuffers() {
        this._lastMode = this.mode;
        if (this.mode.startsWith('ACV')) {
            this._acvBuf = null;
            this._acvSumSq = 0;
            this._acvIdx = 0;
            this._acvCount = 0;
        ...
        // 电容档测量状态复位（切换档位后重新充电测量）
        this._capKey = null;
        this._capDone = false;
        this._capTimer = 0;
        this._capSimTime = 0;
        this._capV0 = undefined;
        this._capIter = undefined;
        this._capValue = null;
    }
    /**
 ...
 */
    ...
    triggerBeep(isBeeping) {
        ...
        if (isBeeping) {
            // 本次测量已响过 3 声则不再响，避免持续鸣叫
            if (this._beepFired) return;
            this._beepFired = true;
            this.isBeepingNow = true;
            this._beepSequence = 0;
            this._playBeepTone();
        } else {
            // 立即停止蜂鸣并清理，复位计数，等待下一次导通重新响 3 声
            this._stopBeepTone();
            this._beepFired = false;
            try { this.lcdText.fill('#222'); } catch (e) { }
        }
    }

    /** 播放一声短促蜂鸣（130ms），间隔 220ms 播下一声，共 3 声 */
    _playBeepTone() {
        if (this._beepSequence >= 3) {
        ...
        // 本声结束后释放资源
        setTimeout(() => {
            if (this._osc === osc) this._osc = null;
            if (this._gain === gain) this._gain = null;
        ...
        this._beepSequence++;
        this._beepTimer = setTimeout(() => this._playBeepTone(), 220);
    }
     ...
     */
    update(val) {
        if (val == null || isNaN(val)) val = 1000000;
        this.value = val;
        ...
        // 清除未触发的 pending timer，避免其回调读取已重置的 _pendingValue 造成 update(null)
        if (this._pendingTimer) {
            clearTimeout(this._pendingTimer);
            this._pendingTimer = null;
        }
        ...
        switch (this.mode) {
            ...
            // --- 二极管/通断档（1mA 恒流注入，见 stampMultimeters）---
            //  <0.3V：电压很小 → 按导通电阻显示（R = V/1mA = 1000·V），<5Ω 蜂鸣
            //  0.3~3V：显示管压降
            //  >3V ：视为反向/开路 → O.L
            case 'DIODE':
    ...
    tick(dt) {
        const s = this.sys?.voltageSolver;
        if (!s) return;
        const mode = this.mode || 'OFF';
        if (mode === 'OFF') return;
        if (mode.startsWith('DCV')) {
            this.update(s.getPD(`${this.id}_wire_v`, `${this.id}_wire_com`) || 0);
        } else if (mode === 'MA') {
            this._updateMACurrent(s);
        } else if (mode.startsWith('RES')) {
            this._updateRES(s);
        } else if (mode === 'DIODE') {
            this._updateDiode(s);
        } else if (mode === 'C') {
            this._updateCapacitance(s, dt);
    ...
    _updateDiode(s) {
        const vPort = Math.abs(s.getPD(`${this.id}_wire_v`, `${this.id}_wire_com`) || 0);
        this.update(vPort);
    }

    /**
     * 电容档测量：恒流充电法
     * 以 1mA 恒流对被测电容充电 3s，读取电容两端电压增量 ΔV，
     * 由 C = I·t / ΔV 计算电容量（t 取求解器实际积分的仿真时间，
     * 避免平台"隔帧跳步"导致的时间偏差）。
     * 充电结束后停止注入电流并保持读数；更换表笔/档位后重新测量。
     */
    _updateCapacitance(s, dt) {
        const CAP_CURRENT = 0.001;   // 1mA 恒流
        const CAP_TIME = 3.0;        // 充电 3s

        const cV = s.portToCluster.get(`${this.id}_wire_v`);
        const cCOM = s.portToCluster.get(`${this.id}_wire_com`);
        if (cV === undefined || cCOM === undefined) {
            this._capKey = null;
            this._capDone = false;
            this.update(1000000);    // 未接线 → O.L
            return;
        }

        const key = `${cV}|${cCOM}`;
        const v = s.getPD(`${this.id}_wire_v`, `${this.id}_wire_com`) || 0;
        ...
        if (this._capKey !== key) {
            ...
            this._capSimTime = 0;
            this._capV0 = v;
            this._capIter = s.globalIterCount;
            this._capValue = null;
        }

        if (this._capDone) {
            if (this._capValue != null) this.update(this._capValue);
            return;
        }

        // 累计求解器真正推进的仿真时间（仅在 globalIterCount 变化时累加）
        if (this._capIter === undefined) this._capIter = s.globalIterCount;
        if (s.globalIterCount !== this._capIter) {
            this._capSimTime += s.deltaTime * (s.globalIterCount - this._capIter);
            this._capIter = s.globalIterCount;
        }

        this._capTimer += dt;
        if (this._capTimer < CAP_TIME) return;   // 充电中，暂不刷新读数

        const dV = Math.abs(v - this._capV0);
        if (this._capSimTime <= 0 || dV < 1e-9) {
            this._capValue = 1000000;            // 电压不上升：短路或超出量程
        } else {
            const cUF = CAP_CURRENT * this._capSimTime / dV * 1e6;
            this._capValue = (!isFinite(cUF) || cUF > 1e6) ? 1000000 : cUF;
        }
        this._capDone = true;                    // 停止充电，保持读数
        this.update(this._capValue);
    }

    _updateACV(s) {
        const vDiff = s.getPD(`${this.id}_wire_v`, `${this.id}_wire_com`);
        if (!this._acvBuf) {
            this._acvBuf = new Float64Array(40);
            this._acvSumSq = 0;
            this._acvIdx = 0;
            this._acvCount = 0;
        }
        const buf = this._acvBuf;
        const idx = this._acvIdx;
        this._acvSumSq -= buf[idx];
        // 求解器异常时 vDiff 可能为 NaN/Infinity，按 0 处理避免污染 RMS 缓冲导致永久 O.L
        const sq = Number.isFinite(vDiff) ? vDiff * vDiff : 0;
        buf[idx] = sq;
        this._acvSumSq += sq;
        this._acvIdx = (idx + 1) % buf.length;
        if (this._acvCount < buf.length) this._acvCount++;
        // 浮点误差可能使 _acvSumSq 累积为微小负数，sqrt(负数)=NaN → update(NaN)→O.L
        const rms = this._acvCount > 0 ? Math.sqrt(Math.max(0, this._acvSumSq) / this._acvCount) : 0;
        this._displayRMS = rms < 0.01 ? 0 : rms;
        this.update(this._displayRMS);
    }

    _updateMACurrent(s) {
        const current = this.physCurrent || 0;
        if (!this._acCBuf) {
            this._acCBuf = new Float64Array(40);
            this._acCSumSq = 0;
            this._acCIdx = 0;
            this._acCCount = 0;
        }
        const buf = this._acCBuf;
        const idx = this._acCIdx;
        this._acCSumSq -= buf[idx];
        const sq = Number.isFinite(current) ? current * current : 0;
        buf[idx] = sq;
        this._acCSumSq += sq;
        this._acCIdx = (idx + 1) % buf.length;
        if (this._acCCount < buf.length) this._acCCount++;
        const rms = this._acCCount > 0 ? Math.sqrt(Math.max(0, this._acCSumSq) / this._acCCount) : 0;
        this.update(rms * 1000);
    }

    _updateRES(s) {
        // 恒流源注入测量：R = 端口电压 / 档位测试电流
        const vPort = Math.abs(s.getPD(`${this.id}_wire_v`, `${this.id}_wire_com`) || 0);
        let iSet = 0.001;                                   // RES200：1mA
        if (this.mode === 'RES2k') iSet = 0.0001;            // 2kΩ：0.1mA
        else if (this.mode === 'RES200k') iSet = 0.000001;   // 200kΩ：1µA
        const R = vPort / iSet;
        if (!isFinite(R) || R >= 1e9) {
            this.update(1000000);
        } else {
    ...
    /**
     * 按档位标识设置旋钮档位（供工作流自动演示切档用）。
     * 角度与 _drawScaleLabels 的 scales 一致：OFF=0，DCVmv=-30，DCV20=-60，DCV200=-90，
     * ACV200=-120，ACV500=-150，DIODE=30，RES200=60，RES2k=90，RES200k=120，MA=150，C=180。
     * @param {string} mode 档位标识
     */
    setMode(mode) {
        const angleMap = {
            'OFF': 0, 'DCVmv': -30, 'DCV20': -60, 'DCV200': -90,
            'ACV200': -120, 'ACV500': -150, 'DIODE': 30, 'RES200': 60,
            'RES2k': 90, 'RES200k': 120, 'MA': 150, 'C': 180,
        };
        const angle = angleMap[mode];
        if (angle === undefined || !this.pointer) return;
        this.pointer.rotation(angle);
        this._updateModeByAngle(angle);
        this._measureNow();
        this.markDirty?.();
    ...
    getClickablePartCenter(partId) {
        ...
        const lx = this.width / 2, ly = this.height * 0.63;
        const py = this.height - 25;
        switch (partId) {
            case 'lcd':  return { x: gx + lx, y: gy + 85 };
            case 'knob': return { x: gx + lx, y: gy + ly };
            case 'v':    return { x: gx + this.width * 0.833, y: gy + py };
            case 'com':  return { x: gx + this.width * 0.5,   y: gy + py };
            case 'ma':   return { x: gx + this.width * 0.167, y: gy + py };
            default:     return null;
        }
    }
