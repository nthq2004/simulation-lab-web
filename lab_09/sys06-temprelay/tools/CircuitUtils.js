...
export const CircuitUtils = {
    ...
    getEquivalentResistance(startCluster, endCluster, allClusters, rawDevices, portToCluster, equivResCache) {
        ...
        for (const dev of rawDevices) {
            ...
            // 1.8b 三相配电箱（PDB）：进线=汇流排节点，闭合 0.01Ω；分励线圈恒 200Ω；常闭辅助触头 0.001Ω/1e9Ω
            if (type === 'PDB') {
                ...
                for (let s = 1; s <= 3; s++) {
                    ...
                    addResistor(`${id}_wire_sw${s}_uv1`, `${id}_wire_sw${s}_uv2`, Rcoil);
                }
                // 常闭辅助触头（若有）：闭合 0.001Ω，断开 1e9Ω
                if (typeof dev.isNCClosed === 'function') {
                    addResistor(`${id}_wire_nc1`, `${id}_wire_nc2`, dev.isNCClosed() ? 0.001 : 1e9);
                }
                void R;
                continue;
            }

            // 1.8c 岸电箱（ShorePowerBox）：相序开关 + 出口断路器串联，闭合 0.01Ω / 断开 1e9Ω
            if (type === 'ShorePowerBox') {
                const on = dev._knob !== 0 && !!dev._breakerOn;
                const Rp = on ? 0.01 : 1e9;
                const map = dev._knob === 2
                    ? [['in1', 't1'], ['in2', 't3'], ['in3', 't2']]
                    : [['in1', 't1'], ['in2', 't2'], ['in3', 't3']];
                map.forEach(([a, b]) => addResistor(`${id}_wire_${a}`, `${id}_wire_${b}`, Rp));
                continue;
            }

            // 1.8d 负序继电器（NegSeqRelay）：常闭触点 0.001Ω / 1e9Ω
            if (type === 'NegSeqRelay') {
                const nc = typeof dev.isNCClosed === 'function' && dev.isNCClosed();
                addResistor(`${id}_wire_nc1`, `${id}_wire_nc2`, nc ? 0.001 : 1e9);
                continue;
            }
            ...
            // 7.8b 实物电感（RealInductor）：线圈直流电阻，供万用表/兆欧表电阻档测量
            if (type === 'inductor' && dev.coilR !== undefined) {
                addResistor(`${id}_wire_l`, `${id}_wire_r`, dev.coilR);
            }
            ...
            // 10. 电子脱扣器电流采样端 I+/I-（0V 电压源）视为短路（0Ω）。
            //     否则电流互感器副边接脱扣器采样端时，等效电阻计算会误判为开路，
            //     导致 CT 切换到 1000V 顺从电压模式、副边电压/电流异常。
            if (type === 'tripRelay' || special === 'ETUnit') {
                addResistor(`${id}_wire_i+`, `${id}_wire_i-`, 0);
            }

            // 11. 绝缘电阻（模型注入，非独立器件）：
            //     三相电机 3D —— 各相绕组对机壳 PE；电机控制箱 —— 电源各相进线对箱体 PE。
            //     供兆欧表 getResistanceBetweenPorts 读取。
            if (type === 'motor_3d' && isFinite(dev._insulR) && dev._insulR > 0) {
                ['u', 'v', 'w'].forEach(ph => addResistor(`${id}_wire_${ph}`, `${id}_wire_pe`, dev._insulR));
            }
            if (type === 'motor_control_box' && isFinite(dev._lineInsulR) && dev._lineInsulR > 0) {
                ['in1', 'in2', 'in3'].forEach(ph => addResistor(`${id}_wire_${ph}`, `${id}_wire_pe1`, dev._lineInsulR));
            }
        }
