...
export class ClampMeter extends BaseComponent {
    constructor(config, sys) {
        ...
        this.width  = Math.max(this.constructor.minWidth  || 300, config.width  || this.constructor.defaultWidth  || 380);
        this.height = Math.max(this.constructor.minHeight || 360, config.height || this.constructor.defaultHeight || 480);
    ...
    _initParameters(config) {
        ...
        // 穿心导线电流感知（senseJaw=true 时启用：钳口闭合自动读取穿心导线电流）
        this._senseJaw  = !!config.senseJaw;
        this._frequency = config.frequency !== undefined ? parseFloat(config.frequency) : 50;
        this._rmsBuffer = [];
        this._clampedConn = null;
        this._sensedInstant = 0;
    }
    ...
    _portAbs(portId) {
        ...
        const did = portId.split('_wire_')[0];
        const comp = this.sys && this.sys.comps ? this.sys.comps[did] : null;
        return comp && typeof comp.getAbsPortPos === 'function'
            ? comp.getAbsPortPos(portId) : null;
    }

    /** 查找穿过钳口内孔的导线连接（取最靠近孔心的一根） */
    _senseClampedWire() {
        const sys = this.sys;
        if (!sys || !sys.conns || !this._jaw) return null;
        const hole = this._toAbs(this._jaw.cx, this._jaw.topY + this._jaw.h * 0.5);
        const innerR = this._jaw.innerR;
        let best = null, bestD = Infinity;
        for (const c of sys.conns) {
            if (c.type !== 'wire' || c.custom) continue;
            const p1 = this._portAbs(c.from);
            const p2 = this._portAbs(c.to);
            if (!p1 || !p2) continue;
            const d = _pointSegDist(hole.x, hole.y, p1.x, p1.y, p2.x, p2.y);
            if (d <= innerR && d < bestD) { bestD = d; best = c; }
        }
        return best;
    }

    /** 指定连接当前是否仍穿过钳口内孔（用于判断导线是否一直处于感测范围） */
    _isConnInJaw(conn) {
        if (!conn || !this._jaw) return false;
        const hole = this._toAbs(this._jaw.cx, this._jaw.topY + this._jaw.h * 0.5);
        const p1 = this._portAbs(conn.from);
        const p2 = this._portAbs(conn.to);
        if (!p1 || !p2) return false;
        return _pointSegDist(hole.x, hole.y, p1.x, p1.y, p2.x, p2.y) <= this._jaw.innerR;
    }

    // ═══════════════════════════════════════════════════════
    // 穿心三维效果（实际穿心导线与铁芯上半部分相交处被上钳口遮挡）
    // ═══════════════════════════════════════════════════════

    /**
     * 穿心效果：不新画导线，而是让"实际穿心而过的黑导线"呈现从铁芯中穿过的立体感——
     * 在导线所在图层（lineLayer）之上补绘一份与可动上钳口完全一致的上钳口图形，
     * 从而把与铁芯上半部分相交的那段导线遮住，使其看似从铁芯后面穿过。
     * 仅在打开钳口再合上（开始测量）后呈现。
     */
    _ensureJawOccluder() {
        if (this._occluderGroup) return;
        const lineLayer = this.sys && this.sys.lineLayer;
        if (!lineLayer) return;
        const g = new Konva.Group({ listening: false });
        this._occluderArc = new Konva.Arc({
            x: 0, y: 0,
            innerRadius: this._jaw.innerR, outerRadius: this._jaw.outerR,
            angle: 180, rotation: -180,
            fillLinearGradientStartPoint: { x: -this._jaw.outerR, y: 0 },
            fillLinearGradientEndPoint:   { x:  this._jaw.outerR, y: 0 },
        ...
        g.add(this._occluderArc);
        g.visible(false);
        lineLayer.add(g);
        this._occluderGroup = g;
    }

    _updateJawOccluder() {
        this._ensureJawOccluder();
        const g = this._occluderGroup;
        if (!g || !this._jawGroup || !this._jaw) return;

        // 仅在钳口闭合、开始测量后呈现
        if (!this._measuring) { g.visible(false); return; }

        // 用上钳口的绝对变换对齐遮挡图形（位置/旋转/缩放）
        const t = this._jawGroup.getAbsoluteTransform().decompose();
        g.position({ x: t.x, y: t.y });
        g.rotation(t.rotation);
        g.scale({ x: t.scaleX, y: t.scaleY });

        // 同步半径与渐变范围（组件尺寸变化时）
        this._occluderArc.innerRadius(this._jaw.innerR);
        this._occluderArc.outerRadius(this._jaw.outerR);
        this._occluderArc.fillLinearGradientStartPoint({ x: -this._jaw.outerR, y: 0 });
        this._occluderArc.fillLinearGradientEndPoint({ x: this._jaw.outerR, y: 0 });

        g.visible(true);
        g.moveToTop();   // 位于导线之上，遮住与铁芯上半部分相交的穿心段
    }

    /** 读取穿心导线的瞬时电流（向导线两端设备查询） */
    _readWireInstantCurrent(conn) {
        if (!conn) return null;
        for (const pid of [conn.from, conn.to]) {
            const did = pid.split('_wire_')[0];
            const comp = this.sys && this.sys.comps ? this.sys.comps[did] : null;
            if (comp && typeof comp.getWireCurrentInstant === 'function') {
                const v = comp.getWireCurrentInstant(pid);
    ...
    /**
     * 钳口闭合后更新目标读数（穿心导线瞬时电流 → 有效值 RMS）。
     * 只有当合口时锁定的那根导线"一直处于感测范围内"才持续测量；
     * 导线一旦离开感测范围即停止测量（需重新开合钳口），
     * 避免导线一穿过铁芯就自动开始测量。
     */
    _updateSensedCurrent() {
        if (!this._measuring) return;
        if (!this._isConnInJaw(this._clampedConn)) {
            // 导线离开感测范围 → 停止测量，需重新"开-合"钳口
            this._measuring = false;
            this._clampedConn = null;
            this._rmsBuffer.length = 0;
            this._sensedInstant = 0;
            this._targetI = 0;
            return;
        }
        const inst = this._readWireInstantCurrent(this._clampedConn);
        ...
        this._sensedInstant = inst;
        this._rmsBuffer.push(inst * inst);
        const solverDt = (this.sys && this.sys.voltageSolver && this.sys.voltageSolver.deltaTime) || 1e-4;
        const win = Math.max(10, Math.round(1 / (Math.max(1, this._frequency) * solverDt)));
        if (this._rmsBuffer.length > win) {
            this._rmsBuffer.splice(0, this._rmsBuffer.length - win);
        }
        const sumSq = this._rmsBuffer.reduce((a, b) => a + b, 0);
        const rms = Math.sqrt(sumSq / this._rmsBuffer.length);
        // 数字显示真实测量值（可超量程），指针由刻度映射自然限位
        this._targetI = Math.min(1e7, rms);
    }
    ...
    _bindInteraction() {
        ...
        trigHit.on('click tap', () => {
            ...
            if (this._senseJaw) {
                // 穿心感知模式：必须在"张开→合上"的瞬间导线处于感测范围内才开始测量
                if (!wasOpen && this._jawOpen) {
                    // 张开钳口 → 停止测量
                    this._measuring = false;
                    this._targetI = 0;
                } else if (wasOpen && !this._jawOpen) {
                    // 合上钳口 → 锁定当前穿心导线，锁定成功才测量
                    this._clampedConn = this._senseClampedWire();
                    this._measuring = !!this._clampedConn;
                    this._rmsBuffer.length = 0;
    ...
    tick(dt) {
        // 钳口穿心感知：自动读取被钳入导线的电流（有效值）
        if (this._senseJaw) this._updateSensedCurrent();
        // 穿心三维效果：遮挡导线与铁芯上半部分相交段（合口测量后呈现）
        this._updateJawOccluder();
    ...
    setCurrent(i) {
        ...
        // 数字显示真实测量值（可超量程），指针由刻度映射自然限位
        this._targetI = Math.max(0, Math.min(1e7, parseFloat(i) || 0));
    }
    ...
    destroy() {
        if (this._occluderGroup) {
            try { this._occluderGroup.destroy(); } catch (e) { /* ignore */ }
            this._occluderGroup = null;
        }
        super.destroy?.();
    }
}
