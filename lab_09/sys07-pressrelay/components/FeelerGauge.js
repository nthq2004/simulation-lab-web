...
export class FeelerGauge extends BaseComponent {
     ...
     * @param {string} partId 'measure'（测量按钮）| 'blade_i'（第 i 片塞尺片）
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'measure') {
            return { x: gx + this._btnX + this._btnW / 2, y: gy + this._btnY + this._btnH / 2 };
        }
        if (typeof partId === 'string' && partId.indexOf('blade_') === 0) {
            const i = parseInt(partId.slice(6), 10);
            if (!isNaN(i) && i >= 0 && i < this._blades.length) {
                const angle = this._bladeAngle(i);
                const a = angle * Math.PI / 180;
                const len = this._bladeLen * (i === this._bladeIndex ? 1.0 : 0.90);
                return {
                    x: gx + this._pivotX + Math.cos(a) * (len * 0.52),
                    y: gy + this._pivotY + Math.sin(a) * (len * 0.52),
                };
            }
        }
        return null;
