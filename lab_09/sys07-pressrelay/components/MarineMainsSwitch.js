...
export class MarineMainsSwitch extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // ── 无电子脱扣器时的简化保护（特殊标志，仅特定教学工程启用）──
        // 本工程（sys_ljdq5-2）主开关未配置电子脱扣器（MarineElectronicTrip），
        // 欠压 / 过载 / 干线短路三种保护由主开关内部"特殊实现"承担：
        //   · 欠压：本机发电机输出线电压 < 额定 85% 持续 uvTime → 延时跳闸
        //   · 过载：本机发电机输出功率 > 额定 × overloadRatio(120%) 持续 overloadTime → 延时跳闸
        //   · 干线短路：母线短路标记（_faultShort）存在 → 瞬时跳闸
        this.faultSimpleProtect = config.faultSimpleProtect === true; // 特殊保护启用标志
        this.uvThreshRatio      = config.uvThreshRatio !== undefined ? config.uvThreshRatio : 0.85; // 欠压阈值（额定电压比例）
        this.uvTime             = config.uvTime        !== undefined ? config.uvTime        : 2;   // 欠压动作延时 s
        this.overloadRatio      = config.overloadRatio !== undefined ? config.overloadRatio : 1.2; // 过载倍数（120%）
        this.overloadTime       = config.overloadTime  !== undefined ? config.overloadTime  : 15;  // 过载动作延时 s
        this._uvTimer      = 0;     // 欠压超阈值持续时间（s）
        this._overloadTimer = 0;    // 过载超阈值持续时间（s）
        this._uvTrip       = false; // 简化欠压保护是否已动作（供教学 check 用）
        this._overloadTrip = false; // 简化过载保护是否已动作（供教学 check 用）
    ...
    _logic(dt) {
        ...
        if (this._state === 'on') {
            ...
            // ── 无电子脱扣器的简化保护（特殊标志 faultSimpleProtect，仅特定工程启用）──
            // 本工程主开关未配置电子脱扣器，欠压 / 过载 / 干线短路保护由主开关内部特殊实现：
            if (this.faultSimpleProtect) {
                // 干线短路：母线短路标记（bus1._faultShort 等）存在 → 瞬时跳闸
                if (this._isBusShort()) {
                    this._uvTimer = 0;
                    this._overloadTimer = 0;
                    this.tryTrip();
                    if (typeof console !== 'undefined') console.warn(`[简化短路保护] ${this.id} 汇流排干线短路，瞬时跳闸`);
                    return;
                }
                // 欠压：发电机实际输出端子电压（_rmsV，非调压器目标 _vRmsOut）
                // 低于额定相电压×uvThreshRatio 持续 uvTime → 延时跳闸
                // （机械故障停机、发电机停转等均会导致汇流排失压）
                const busV = (gen._rmsV !== undefined) ? gen._rmsV
                    : (gen._vRmsOut !== undefined ? gen._vRmsOut : gen.vRms);
                const uvCond = busV < gen.vRms * this.uvThreshRatio;
                if (uvCond) {
                    this._uvTimer += dt;
                    if (this._uvTimer >= this.uvTime) {
                        this._uvTimer = 0;
                        this._uvTrip = true;
                        this.tryTrip();
                        if (typeof console !== 'undefined') console.warn(`[简化欠压保护] ${this.id} 欠压 ${(busV * Math.sqrt(3)).toFixed(0)}V 持续 ${this.uvTime}s，跳闸`);
                        return;
                    }
                } else {
                    this._uvTimer = 0;
                }
                // 过载：本机发电机输出功率（真实 _pwr，自然加载触发）> 额定×overloadRatio
                // 持续 overloadTime → 延时跳闸
                if (gen && gen._pwr !== undefined
                    && gen._pwr > gen.ratedPower * this.overloadRatio) {
                    this._overloadTimer += dt;
                    if (this._overloadTimer >= this.overloadTime) {
                        this._overloadTimer = 0;
                        this._overloadTrip = true;
                        this.tryTrip();
                        if (typeof console !== 'undefined') console.warn(`[简化过载保护] ${this.id} 过载 ${gen._pwr.toFixed(1)}kW（额定 ${gen.ratedPower}kW）持续 ${this.overloadTime}s，跳闸`);
                        return;
                    }
                } else {
                    this._overloadTimer = 0;
    ...
    // 干线短路检测（简化保护专用）：扫描系统中是否存在短路标记（bus1._faultShort 等）。
    // 短路故障由故障配置设置标记并触发瞬时跳闸；此处保证"短路期间合闸 → 立即再次跳闸"。
    _isBusShort() {
        const sys = this.sys;
    ...
    isRevTrip()   { return this._revTrip; }        // 逆功率保护是否已动作
    isUvTrip()    { return this._uvTrip; }         // 简化欠压保护是否已动作
    isOverloadTrip() { return this._overloadTrip; } // 简化过载保护是否已动作

    /**
     * 返回可点击部件中心坐标（供工作流 find/箭头指示定位）
     * @param {string} partId 'store-spring' | 'close-coil' | 'uv-trip' | 'shunt-coil' |
     *                        'main-contact' | 'aux-contact' | 'open-spring' |
     *                        'store-ind'(储能指示牌) | 'handle'(储能手柄) | 'dial'(工作位圆盘) |
     *                        'btn-close'(手动合闸) | 'btn-open'(手动分闸)
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        const rel = {
            'store-spring': { x: 450, y: 41 },   // 储能弹簧中点（355+190/2=450）
            'close-coil':   { x: 500, y: 70 },   // 合闸线圈中心（475+50/2=500, 58+25/2=70.5）
            'uv-trip':      { x: 432, y: 180 },  // 失压脱扣器磁轭中心（400+65/2=432.5, 145+70/2=180）
            'shunt-coil':   { x: 432, y: 260 },  // 分励线圈中心（412+40/2=432, 248+24/2=260）
            'main-contact': { x: 312, y: 119 },  // 主触头中心（250+125/2=312.5, 92+55/2=119.5）
            'aux-contact':  { x: 310, y: 27 },   // 辅助触头中心（170+280/2=310, 16+22/2=27）
            'open-spring':  { x: 205, y: 120 },  // 分闸弹簧中心（150+110/2=205, 100+40/2=120）
            'store-ind':    { x: 111, y: 58 },   // 储能指示牌（弹簧图标）中心（084+138/2=111, 044+072/2=58）
            'handle':       { x: 75, y: 210 },   // 储能手柄区域中心（39+72/2=75）
            'dial':         { x: 75, y: 252 },   // 工作位圆盘中心（39+72/2=75, 218+68/2≈252）
            'btn-close':    { x: 39, y: 103 },   // 手动合闸按钮中心（6+66/2=39, 90+26/2=103）
            'btn-open':     { x: 111, y: 103 },  // 手动分闸按钮中心（78+66/2=111, 90+26/2=103）
        };
        const p = rel[partId];
