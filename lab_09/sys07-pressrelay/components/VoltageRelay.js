...
/**
 * VoltageRelay — 电压继电器（Voltage Relay）仿真组件
 *
 * 说明：
 * - 模拟以电压/电流驱动的继电器线圈与触点机构，包含吸合判定、线圈电阻和触点可视化；
 * - 提供 `pickupCurrent` / `releaseCurrent` 等参数以定义吸合/释放阈值，并能展示线圈与触点动画；
 * - 静态视觉元素启用缓存以提升渲染性能。
 */
export class VoltageRelay extends BaseComponent {
    ...
    _drawCoil() {
        ...
        this._staticGroup.add(this.coilLeftLead, this.coilRightLead, this.coilShape);
    }
    ...
    update() {
        ...
        // 真实电流计算
        if(this.coilFault)this.currentResistance =1e9;
        else this.currentResistance =120;
