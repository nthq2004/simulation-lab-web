// 圆盘式电磁制动器（失电制动器）仿真工程 — 2 个操作项目
// 电路：DC + → 制动器线圈A1；DC - → 制动器线圈A2
// 原理：通电吸合衔铁松闸（转轴可自由转动）；断电弹簧推衔铁压紧制动盘抱闸（转速减速停止）

import { DCPower } from '../components/DCPower.js';
import { DiscElectromagneticBrake } from '../components/DiscElectromagneticBrake.js';
import { FeelerGauge } from '../components/FeelerGauge.js';
import { Multimeter } from '../components/Multimeter.js';
import { MF47Multimeter } from '../components/MF47Multimeter.js';
import { Oscilloscope_tri } from '../components/Osc_tri.js';
import { SignalGenerator } from '../components/SignalGenerator.js';
import { ProcessCalibrator } from '../components/ProcessCalibrator.js';
import { ElecMeter } from '../components/ElecMeter.js';

export const FAULT_CONFIGS = {
    coil_open: {
        id: 'coil_open', name: '线圈断线', system: '制动器',
        check()  { const c = window.sys && window.sys.comps && window.sys.comps.brk1; return c && c._faultCoilOpen; },
        trigger() { const c = window.sys && window.sys.comps && window.sys.comps.brk1; if (c) c._faultCoilOpen = true; },
        repair() { const c = window.sys && window.sys.comps && window.sys.comps.brk1; if (c) c._faultCoilOpen = false; },
    },
    stuck: {
        id: 'stuck', name: '衔铁卡死', system: '制动器',
        check()  { const c = window.sys && window.sys.comps && window.sys.comps.brk1; return c && c._faultStuck; },
        trigger() { const c = window.sys && window.sys.comps && window.sys.comps.brk1; if (c) c._faultStuck = true; },
        repair() { const c = window.sys && window.sys.comps && window.sys.comps.brk1; if (c) c._faultStuck = false; },
    },
    pad_wear: {
        id: 'pad_wear', name: '摩擦片磨损0.5mm', system: '制动器',
        check()  { const c = window.sys && window.sys.comps && window.sys.comps.brk1; return c && c._faultPadWear; },
        trigger() {
            const c = window.sys && window.sys.comps && window.sys.comps.brk1;
            if (!c) return;
            c._faultPadWear = true;
            // 记录故障前气隙；磨损使动衔铁带摩擦片右移、工作气隙自动增大（磁轭不动）
            if (c._airGapBeforePadWear === undefined) c._airGapBeforePadWear = c.getAirGap();
            c.setPadWear(0.5);
        },
        repair() {
            const c = window.sys && window.sys.comps && window.sys.comps.brk1;
            if (!c) return;
            c._faultPadWear = false;
            // 更换摩擦片：磨损量归零、恢复故障前气隙
            const restore = c._airGapBeforePadWear !== undefined ? c._airGapBeforePadWear : 0.8;
            c._airGapBeforePadWear = undefined;
            c.setPadWear(0);
            c.setAirGap(restore);
        },
    },
};

export const PROJECT_WORKFLOWS = {

    // ============================================================
    // 项目1：识别圆盘式电磁制动器的组件
    // ============================================================
    'brake-identify': {
        id: 'brake-identify',
        name: '1. 识别圆盘式电磁制动器的组件',
        steps: [
            {
                msg: '第 1 步：识别电磁铁线圈。电磁铁线圈绕在 E 形静铁心的中柱上，通电后产生电磁力吸合衔铁。',
                mode: 'find',
                target: 'brk1',
                subTarget: 'coil',
            },
            {
                msg: '第 2 步：识别螺旋复位弹簧。上下各有一根螺旋弹簧，安装在铁心背面与衔铁之间。断电时弹簧推动衔铁右移实现抱闸。',
                mode: 'find',
                target: 'brk1',
                subTarget: 'spring-top',
            },
            {
                msg: '第 3 步：识别衔铁与摩擦片。动衔铁可沿导向杆轴向滑动，其右侧面贴有摩擦片。通电时衔铁被吸向铁心，摩擦片脱离制动盘（松闸）。',
                mode: 'find',
                target: 'brk1',
                subTarget: 'armature',
            },
            {
                msg: '第 4 步：识别制动盘。制动盘固定在转轴上，与转轴同步旋转。抱闸时摩擦片压紧制动盘产生摩擦力矩，实现制动。',
                mode: 'find',
                target: 'brk1',
                subTarget: 'disc',
            },
            {
                msg: '第 5 步：识别工作气隙调节装置。磁轭左侧的旋钮可调节工作气隙（0.3~1.5mm），向上拖动或滚轮调节气隙大小。',
                mode: 'find',
                target: 'brk1',
                subTarget: 'knob',
            },
            {
                msg: '第 6 步：测试题',
                mode: 'quiz',
                quizConfig: {
                    question: '电磁制动器的工作气隙和释放间隙分别是什么？',
                    options: [
                        '工作气隙是制动盘与固定压板之间的间隙；释放间隙是衔铁与铁芯极面之间的间隙',
                        '工作气隙是抱闸时衔铁左端面与铁芯极面之间的间隙（约0.8mm）；释放间隙是衔铁吸合后摩擦片与制动盘之间的距离',
                        '工作气隙和释放间隙是同一个概念的不同称呼',
                        '工作气隙是衔铁吸合后摩擦片与制动盘之间的距离；释放间隙是抱闸时衔铁与铁芯极面之间的间隙',
                    ],
                    answer: 1,
                    analysis: '工作气隙是衔铁处于抱闸位置时，衔铁左端面与铁芯极面之间的间隙（气隙调节旋钮调节的就是这个间隙，正常值约 0.8mm）。释放间隙是衔铁吸合后（松闸状态），摩擦片与制动盘之间分离的距离，其大小约等于工作气隙。工作气隙过大会导致电磁吸力不足，松闸不彻底；过小则释放间隙不足，摩擦片可能与制动盘发生剐蹭。',
                },
            },
        ],
    },
    // ============================================================
    // 项目2：松闸与抱闸原理
    // ============================================================
    'brake-principle': {
        id: 'brake-principle',
        name: '2. 电磁制动器松闸/抱闸原理',
        steps: [
{
                msg: '第 1 步：接线：直流电源+→制动器线圈A1；直流电源-→制动器线圈A2。接通直流电源（DC24V），接通后观察：衔铁被吸向铁心（压缩弹簧），摩擦片离开制动盘，制动器松闸。',
                mode: 'check',
                async act() {
                    const sys = this.sys;
                    const dc = sys.comps['dc1'];
                    const brk = sys.comps['brk1'];
                    // 清旧接线，并确保初始断电（LCD 显示 OFF）
                    sys.conns.length = 0;
                    if (dc) { dc.isOn = false; dc.update(); }
                    sys.redrawAll();
                    await new Promise(r => setTimeout(r, 500));
                    // ── ① 接线动画：先电源+ → 制动器A1，再电源- → 制动器A2 ──
                    await sys.connMgr.addConnectionAnimated(
                        { from: 'dc1_wire_p', to: 'brk1_wire_a1', type: 'wire' });
                    await sys.connMgr.addConnectionAnimated(
                        { from: 'dc1_wire_n', to: 'brk1_wire_a2', type: 'wire' });
                    sys.redrawAll();
                    // ── ② 箭头指示电源键 → 动态点击接通电源 ──
                    if (dc && typeof dc.getClickablePartCenter === 'function') {
                        const c = dc.getClickablePartCenter('power');
                        if (c) await this._flashArrow(c);
                    }
                    if (dc) { dc.isOn = true; dc.update(); }   // 按键压下、LCD 变 24.0V 绿色
                    sys.redrawAll();
                    await new Promise(r => setTimeout(r, 1000));
                    // 保险：若电路检测稍有延迟，确保衔铁吸合演示正确
                    if (brk && !brk.isEnergized()) brk.energize();
                    // ── ③ 延时 5s，观察电磁铁吸合 → 压缩弹簧 → 松闸 ──
                    await new Promise(r => setTimeout(r, 4000));
                },
                check() {
                    const c = (a, b) => this.sys.isPortConnected(a, b);
                    const brk = this.sys.comps['brk1'];
                    const dc = this.sys.comps['dc1'];
                    return brk && brk.isEnergized()
                        && dc && dc.isOn
                        && c('dc1_wire_p', 'brk1_wire_a1')
                        && c('dc1_wire_n', 'brk1_wire_a2');
                },
            },
            {
                msg: '第 2 步：通电松闸状态下，点击转轴（或用鼠标拖住转轴区域拨动），转轴被拨动后可以保持自由转动，转速稳定不衰减。',
                mode: 'check',
                target: 'brk1',
                subTarget: 'disc',
                async act() {
                    const sys = this.sys;
                    const brk = sys.comps['brk1'];
                    // 确保松闸状态（通电吸合完成后再拨动转轴）
                    if (brk && !brk.isEnergized()) brk.energize();
                    if (brk) brk.handSpin(120);   // 演示转速适中，视觉旋转平滑、持续可见
                    // 延时 5s，观察转子持续自由旋转、转速稳定不衰减
                    await new Promise(r => setTimeout(r, 5000));
                },
                check() {
                    const brk = this.sys.comps['brk1'];
                    return brk && brk.isEnergized() && brk.getSpeed() > 400;
                },
            },
            {
                msg: '第 3 步：关闭直流电源，观察：弹簧推动衔铁右移，摩擦片压紧制动盘，制动盘压向固定压板 → 抱闸。若转轴原先在转动，将减速停止。',
                mode: 'check',
                target: 'dc1',
                subTarget: 'power',
                async act() {
                    const dc = this.sys.comps['dc1'];
                    if (dc) { dc.isOn = false; dc.update(); }   // 按键弹起、LCD 变 OFF
                    this.sys.redrawAll();
                    // 延时 5s，观察抱闸过程（衔铁复位、摩擦片压紧、转轴减速停止）
                    await new Promise(r => setTimeout(r, 5000));
                },
                check() {
                    const brk = this.sys.comps['brk1'];
                    return brk && brk.isBraking() && brk.getSpeed() < 5;
                },
            },
            {
                msg: '第 4 步：测试题',
                mode: 'quiz',
                quizConfig: {
                    question: '圆盘式电磁制动器属于失电制动器，其工作原理是？',
                    options: [
                        '通电时抱闸制动，断电时松闸',
                        '通电时衔铁吸合松闸，断电时弹簧推衔铁压紧制动盘抱闸',
                        '依靠永久磁铁抱闸，与通电无关',
                        '断电时摩擦片自动脱离制动盘',
                    ],
                    answer: 1,
                    analysis: '圆盘式电磁制动器是失电（spring-set）制动器：通电时电磁力克服弹簧力使衔铁吸合、摩擦片脱离制动盘（松闸）；断电时电磁力消失，弹簧推动衔铁使摩擦片压紧制动盘，制动盘压向固定压板产生摩擦力矩（抱闸），实现安全停车。',
                },
            },
        ],
    },
    // ============================================================
    // 项目2：气隙调节与塞尺测量
    // ============================================================
    'brake-gap-measure': {
        id: 'brake-gap-measure',
        name: '3. 气隙调节与塞尺测量',
        steps: [
            {
                msg: '第 1 步：点击工具栏「故障设置」，勾选「摩擦片磨损0.5mm」故障并应用，工作气隙自动增大到 1.3mm。',
                mode: 'check',
                async act() {
                    const sys = this.sys;
                    const brk = sys.comps['brk1'];
                    // ── ① 箭头指示工具栏「故障设置」按钮 ──
                    await this._flashDomElement(
                        document.getElementById('faultBtn'), '👉 请点击：故障设置');
                    // ── ② 弹出故障设置界面 ──
                    const modal = document.getElementById('faultModal');
                    if (modal) modal.style.display = 'flex';
                    await new Promise(r => setTimeout(r, 900));
                    // ── ③ 呈现勾选动作：箭头指示复选框 → 勾选「摩擦片磨损0.5mm」 ──
                    const cb = document.getElementById('fault_check_pad_wear');
                    if (cb) {
                        const label = cb.closest('label') || cb;
                        await this._flashDomElement(label, '👉 勾选：摩擦片磨损0.5mm');
                        cb.checked = true;
                    }
                    await new Promise(r => setTimeout(r, 400));
                    // ── ④ 点击「应用设置」触发故障：气隙 0.80 → 1.30mm ──
                    const applyBtn = document.getElementById('applyBtn');
                    if (applyBtn) {
                        await this._flashDomElement(applyBtn, '👉 点击：应用设置');
                        applyBtn.click();
                    }
                    await new Promise(r => setTimeout(r, 700));
                    // ── ⑤ 箭头重点标出工作气隙的变化（0.80 → 1.30mm） ──
                    const gapC = this._airGapCenter(brk);
                    if (gapC) await this._flashArrow(gapC);
                    sys.showFloatingTip('工作气隙变化：0.80mm → 1.30mm（摩擦片磨损 +0.5mm）', 4000);
                    await new Promise(r => setTimeout(r, 1200));
                },
                check() {
                    const brk = this.sys.comps['brk1'];
                    return brk && brk._faultPadWear === true
                        && typeof brk.getAirGap === 'function' && Math.abs(brk.getAirGap() - 1.3) < 0.02;
                },
            },
            {
                msg: '第 2 步：制动器默认断电抱闸（气隙张开），用塞尺测量：1.2mm 塞尺片可顺利插入，1.5mm 塞尺片卡阻（气隙在 1.2~1.5mm 之间）。',
                mode: 'check',
                async act() {
                    const sys = this.sys;
                    const feeler = sys.comps['feeler'];
                    const brk = sys.comps['brk1'];
                    const dc = sys.comps['dc1'];
                    // 兜底：确保断电抱闸（默认已断开，此处仅防残留状态，不展示动作）
                    if (dc && dc.isOn) { dc.isOn = false; dc.update(); }
                    await new Promise(r => setTimeout(r, 400));
                    if (brk) brk.handSpin(0);
                    if (!feeler) return;
                    // ── ① 1.2mm：箭头选中塞尺片 → 箭头按下测量按钮 ──
                    const c12 = feeler.getClickablePartCenter('blade_5');
                    if (c12) { await this._flashArrow(c12); feeler.setBlade(5); sys.requestRedraw(); }
                    await new Promise(r => setTimeout(r, 700));
                    const cm = feeler.getClickablePartCenter('measure');
                    if (cm) { await this._flashArrow(cm); feeler.measure(); sys.requestRedraw(); }
                    // ── ② 延时 3s，指出测量结果：顺利插入 ──
                    await new Promise(r => setTimeout(r, 3000));
                    await this._pointToFeelerResult(feeler, '✅ 1.2mm 塞尺片顺利插入 → 气隙 > 1.2mm');
                    // ── ③ 1.5mm：同样流程 ──
                    const c15 = feeler.getClickablePartCenter('blade_6');
                    if (c15) { await this._flashArrow(c15); feeler.setBlade(6); sys.requestRedraw(); }
                    await new Promise(r => setTimeout(r, 700));
                    if (cm) { await this._flashArrow(cm); feeler.measure(); sys.requestRedraw(); }
                    await new Promise(r => setTimeout(r, 3000));
                    await this._pointToFeelerResult(feeler, '❌ 1.5mm 塞尺片卡阻 → 气隙 ≤ 1.5mm');
                    sys.showFloatingTip('结论：工作气隙在 1.2 ~ 1.5mm 之间', 4000);
                    await new Promise(r => setTimeout(r, 1000));
                },
                check() {
                    const feeler = this.sys.comps['feeler'];
                    const log = feeler && feeler._measureLog;
                    return Array.isArray(log) && log.length === 2
                        && log[0] && log[0].blade === 1.2 && log[0].insert === true
                        && log[1] && log[1].blade === 1.5 && log[1].insert === false;
                },
            },
            {
                msg: '第 3 步：用制动器磁轭左侧的「气隙调节」旋钮（滚轮或上下拖动），把工作气隙调回到 0.8mm。',
                mode: 'check',
                target: 'brk1',
                subTarget: 'knob',
                async act() {
                    const sys = this.sys;
                    const brk = sys.comps['brk1'];
                    if (!brk) return;
                    // 分步调节 1.3 → 1.1 → 0.9 → 0.8，每步箭头指出气隙数值变化过程
                    const steps = [1.1, 0.9, 0.8];
                    for (const v of steps) {
                        brk.setAirGap(v);
                        sys.requestRedraw();
                        const gapC = this._airGapCenter(brk);
                        if (gapC) await this._flashArrow(gapC);
                        sys.showFloatingTip(`气隙调节中：${v.toFixed(2)}mm`, 2200);
                        await new Promise(r => setTimeout(r, 1400));
                    }
                    sys.showFloatingTip('✅ 工作气隙已调回 0.80mm', 4000);
                    await new Promise(r => setTimeout(r, 1200));
                },
                check() {
                    const brk = this.sys.comps['brk1'];
                    return brk && typeof brk.getAirGap === 'function' && Math.abs(brk.getAirGap() - 0.8) < 0.02;
                },
            },
            {
                msg: '第 4 步：再用塞尺测量：0.6mm 塞尺片可顺利插入，0.8mm 塞尺片卡阻（气隙已恢复正常 0.8mm 左右）。',
                mode: 'check',
                async act() {
                    const sys = this.sys;
                    const feeler = sys.comps['feeler'];
                    if (!feeler) return;
                    // ── ① 0.6mm：箭头选中塞尺片 → 箭头按下测量按钮 ──
                    const c06 = feeler.getClickablePartCenter('blade_2');
                    if (c06) { await this._flashArrow(c06); feeler.setBlade(2); sys.requestRedraw(); }
                    await new Promise(r => setTimeout(r, 700));
                    const cm = feeler.getClickablePartCenter('measure');
                    if (cm) { await this._flashArrow(cm); feeler.measure(); sys.requestRedraw(); }
                    // ── ② 延时 3s，指出测量结果：顺利插入 ──
                    await new Promise(r => setTimeout(r, 3000));
                    await this._pointToFeelerResult(feeler, '✅ 0.6mm 塞尺片顺利插入 → 气隙 > 0.6mm');
                    // ── ③ 0.8mm：同样流程 ──
                    const c08 = feeler.getClickablePartCenter('blade_3');
                    if (c08) { await this._flashArrow(c08); feeler.setBlade(3); sys.requestRedraw(); }
                    await new Promise(r => setTimeout(r, 700));
                    if (cm) { await this._flashArrow(cm); feeler.measure(); sys.requestRedraw(); }
                    await new Promise(r => setTimeout(r, 3000));
                    await this._pointToFeelerResult(feeler, '❌ 0.8mm 塞尺片卡阻 → 气隙 ≤ 0.8mm');
                    sys.showFloatingTip('结论：工作气隙已恢复到 0.8mm 左右', 4000);
                    await new Promise(r => setTimeout(r, 1000));
                },
                check() {
                    const feeler = this.sys.comps['feeler'];
                    const log = feeler && feeler._measureLog;
                    return Array.isArray(log) && log.length === 2
                        && log[0] && log[0].blade === 0.6 && log[0].insert === true
                        && log[1] && log[1].blade === 0.8 && log[1].insert === false;
                },
            },
            {
                msg: '第 5 步：测试题',
                mode: 'quiz',
                quizConfig: {
                    question: '摩擦片磨损 0.5mm 使制动器工作气隙增大到 1.3mm，其直接后果是？',
                    options: [
                        '制动力矩增大，制动更灵敏',
                        '磁轭磁阻增大、吸合力下降，抱闸变松，制动不可靠',
                        '线圈电流增大，制动力矩不变',
                        '气隙增大对制动器性能没有影响',
                    ],
                    answer: 1,
                    analysis: '摩擦片磨损使工作气隙增大：磁路磁阻增大、通电吸合力下降（松闸不彻底），同时弹簧压缩量减小使抱闸力下降，严重时制动器无法可靠制动。发现气隙超限应及时重新调整气隙或更换摩擦片。',
                },
            },
        ],
    },
};

export const componentConfigs = [
    { Class: DCPower, id: 'dc1', x: 20, y: 120, voltage: 24, isOn: false, visible: true },
    { Class: DiscElectromagneticBrake, id: 'brk1', x: 430, y: 120, visible: true, initState: 'off', coilResistance: 500, ratedCoilVoltage: 24, airGapMM: 0.8 },
    { Class: FeelerGauge, id: 'feeler', x: 560, y: 600, visible: true },

    { Class: Multimeter, id: 'multimeter', x: 940, y: 90, visible: false },
    { Class: MF47Multimeter, id: 'mf47-panel', x: 1050, y: 150, visible: false },
    { Class: Oscilloscope_tri, id: 'osc', x: 50, y: 50, visible: false },
    { Class: SignalGenerator, id: 'sg', x: 50, y: 50, visible: false },
    { Class: ProcessCalibrator, id: 'cali', x: 50, y: 50, visible: false },
    { Class: ElecMeter, id: 'elecmeter', x: 700, y: 100, visible: false },
];

function _wireBase(sys) {
    sys.conns.length = 0;
    const cons = [
        { from: 'dc1_wire_p', to: 'brk1_wire_a1', type: 'wire' },
        { from: 'dc1_wire_n', to: 'brk1_wire_a2', type: 'wire' },
    ];
    cons.forEach(c => sys.connMgr.addConn(c));
    sys.redrawAll();
}

export function initSlider(sys) {
}

export function applyAllPresets() {
    const sys = this && this.sys ? this.sys : window.sys;
    if (!sys) return;
    _wireBase(sys);
    const dc = sys.comps['dc1'];
    // 失电制动器默认断电抱闸（气隙张开、可测量）；流程2第1步会自行接线+通电
    if (dc) { dc.isOn = false; dc.update(); }
}

export async function applyStartSystem() {
    const sys = this && this.sys ? this.sys : window.sys;
    if (!sys) return;
    _wireBase(sys);
    const dc = sys.comps['dc1'];
    if (dc) { dc.isOn = false; dc.update(); }
}

export function fiveStep() {}
