...
export class CoolingSys extends BaseComponent {
    constructor(config, sys) {
        ...
        this.power = 0;
        this.targetPower = 0;
        this.powered = false;   // 交流电源是否已接通（右侧接口）
        this.ambientTemp = config.ambientTemp !== undefined ? config.ambientTemp : 10;  // 环境温度（℃）
        this.coolRate = config.coolRate !== undefined ? config.coolRate : 0.2;          // 制冷降温速率（℃/s）
        this.warmRate = config.warmRate !== undefined ? config.warmRate : 0.1;          // 自然回温速率（℃/s）
        this.temperature = config.initTemp !== undefined ? config.initTemp : this.ambientTemp;
        this._crankAngle = 0;
    ...
    tick(dt) {
        ...
        // 电源未接通时，压缩机无法运行
        this.powered = this._isPowered();

        if (this.mode === 'remote') {
            const connected = this.sys.isPortConnected(`${this.id}_wire_l`, `${this.id}_wire_r`);
            this.targetPower = (connected && this.powered) ? 1.0 : 0;
        ...
        if (this.running) {
            ...
            this.temperature += this.warmRate * dt;
            this.temperature = Math.min(this.ambientTemp, this.temperature);
        }

        this._lcdTemp.text(this.powered ? this.temperature.toFixed(1) + '°C' : '--.-°C');
    ...
    _updateVisuals() {
        ...
        // 液晶屏：仅电源接通时点亮
        this._lcdTemp.fill(this.powered ? '#39ff39' : '#1f5a1f');
        ...
        onGroup.on('click', () => {
            if (!this._isPowered()) {
                if (this.sys && typeof this.sys.showFloatingTip === 'function') {
                    this.sys.showFloatingTip('请先接通右侧单相交流电源，压缩机才能起动', 2000);
                }
                return;
            }
            if (this.mode === 'local') {
    ...
    _drawPorts() {
        // 电气端口改到左侧边缘（控制信号：接温度继电器 NO/COM）
        this.addPort(0, 70, 'l', 'wire','p');
        this.addPort(0, 120, 'r', 'wire');

        // 右侧新增：单相交流电源接口（接交流电源 L/N）
        this.addPort(this.W, 70, 'pwl', 'wire', 'p');
        this.addPort(this.W, 120, 'pwn', 'wire');

        // 接口标注（静态）
        this._staticGroup.add(new Konva.Text({
            x: this.W - 62, y: 145,
            text: 'AC 220V', fontSize: 11, fontStyle: 'bold', fill: '#c0392b'
        }));
        this._staticGroup.add(new Konva.Text({
            x: this.W - 16, y: 62,
            text: 'L', fontSize: 11, fontStyle: 'bold', fill: '#c0392b'
        }));
        this._staticGroup.add(new Konva.Text({
            x: this.W - 16, y: 112,
            text: 'N', fontSize: 11, fontStyle: 'bold', fill: '#c0392b'
        }));
    }

    /**
     * 判断右侧单相交流电源接口是否已接通电源。
     * 条件：本机电源端子（pwl/pwn）分别连到同一个交流电源的 p/n 端子，且该电源处于开启状态。
     */
    _isPowered() {
        const sys = this.sys;
        if (!sys || typeof sys.isPortConnected !== 'function') return false;

        const a = `${this.id}_wire_pwl`;
        const b = `${this.id}_wire_pwn`;
        const comps = sys.comps || {};

        for (const key in comps) {
            const c = comps[key];
            if (!c || c === this || c.type !== 'ac_source' || !c.isOn) continue;
            const cp = `${c.id}_wire_p`;
            const cn = `${c.id}_wire_n`;
            if (sys.isPortConnected(a, cp) && sys.isPortConnected(b, cn)) return true;
            if (sys.isPortConnected(a, cn) && sys.isPortConnected(b, cp)) return true;
        }
        return false;
    }
    ...
    getClickablePartCenter(partId) {
        ...
        switch (partId) {
            case 'lcd':    return { x: gx + this.W / 2, y: gy + 115 };          // 液晶屏中心
            case 'mode':   return { x: gx + this.W / 2 - 85, y: gy + 45 };      // 模式旋钮
            case 'start':  return { x: gx + this.W / 2, y: gy + 45 };           // 起动按钮
            case 'stop':   return { x: gx + this.W / 2 + 85, y: gy + 45 };      // 停止按钮
            case 'pwl':    return { x: gx + this.W, y: gy + 70 };               // 电源 L 端子
            case 'pwn':    return { x: gx + this.W, y: gy + 120 };              // 电源 N 端子
            default:       return null;
        }
    }
