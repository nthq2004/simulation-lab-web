...
export class JSZ3 extends BaseComponent {
    ...
    _drawStaticParts() {
        ...
        this._staticGroup.add(new Konva.Text({
            x: 28, y: -18, text: 'JSZ3 时间继电器', fontSize: 16, fontStyle: 'bold', fill: '#202838',
        }));
        this._staticGroup.add(new Konva.Rect({
    ...
    _updateLEDs() {
        ...
        const blink = output || (this._state === 'timing' && Math.sin(this._animTick * 6) > 0);
    ...
    _updateDynamic() {
        ...
        if (_dirty) {
            ...
            this._progressArc.visible(st === 'timing');
            if (st === 'timing' && this.delayTime > 0) {
...
_updateContactVisual() {
    ...
    for (let i = 0; i <= segments; i++) {
        ...
        const offset = Math.sin(Math.PI * k) * bend-6;
    ...
    /**
     * 部件中心的世界坐标（供工作流自动演示箭头/圈选精确定位）
     * 支持部件：coil-l / coil-r（线圈端子 2、7）、nc（端子5）、com（端子8）、
     *          no（端子6）、dial（延时刻度盘旋钮）、led-power（电源灯）、led-output（输出灯）
     */
    _partLocalXY(partId) {
        const cx = this._termCircleCx, cy = this._termCircleCy, R = this._termCircleR;
        const term = (portName) => {
            const def = this._termDefs.find(d => d[1] === portName);
            if (!def) return null;
            const rad = def[2] * Math.PI / 180;
            return { x: cx + R * Math.cos(rad), y: cy + R * Math.sin(rad) };
        };
        switch (partId) {
            case 'coil-l': case 'term-2': return term('l');     // 端子 2
            case 'coil-r': case 'term-7': return term('r');     // 端子 7
            case 'nc':  case 'term-5': return term('nc_a');     // 端子 5（常闭）
            case 'com': case 'term-8': return term('com_a');    // 端子 8（公共）
            case 'no':  case 'term-6': return term('no_a');     // 端子 6（常开）
            case 'dial': return { x: this._dialCx, y: this._dialCy };
            case 'led-power':  return { x: this._ledPowerX, y: this._ledY };
            case 'led-output': return { x: this._ledOutputX, y: this._ledY };
            default: return null;
        }
    }

    getClickablePartCenter(partId) {
        const local = this._partLocalXY(partId);
        if (!local) return null;
        try {
            const p = this.group.getAbsoluteTransform().point({ x: local.x, y: local.y });
            return { x: p.x, y: p.y };
        } catch (e) {
