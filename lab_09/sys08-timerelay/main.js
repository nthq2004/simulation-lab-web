...
// 断电延时继电器（JSZ3N）显隐勾选框：勾选显示 JSZ3N、隐藏 JSZ3；取消则相反。
// 切换时被隐藏组件的连线一并移除（暂存），被显示组件的历史连线自动恢复，避免出现悬空连线。
(function bindShowTZNCheckbox() {
    const cb = document.getElementById('chkShowTZN');
    if (!cb) return;
    // 各时间继电器的连线暂存（隐藏时移出 conns，重新显示时恢复）
    const connMemory = { tz: [], tzn: [] };

    /** 移除某组件的全部连线并暂存（返回移出的连线数） */
    const takeConnsOf = (compId) => {
        if (!sys || !Array.isArray(sys.conns)) return 0;
        const prefix = compId + '_wire_';
        const keep = [];
        const moved = [];
        for (const c of sys.conns) {
            const hit = (typeof c.from === 'string' && c.from.startsWith(prefix))
                || (typeof c.to === 'string' && c.to.startsWith(prefix));
            (hit ? moved : keep).push(c);
        }
        if (moved.length) {
            connMemory[compId] = moved;
            sys.conns.length = 0;
            keep.forEach(c => sys.conns.push(c));
        }
        return moved.length;
    };

    /** 恢复某组件暂存的连线（去重；返回恢复的连线数） */
    const restoreConnsOf = (compId) => {
        const saved = connMemory[compId] || [];
        if (!sys || !Array.isArray(sys.conns) || !saved.length) return 0;
        let added = 0;
        for (const c of saved) {
            const dup = sys.conns.some(x => x.type === c.type
                && ((x.from === c.from && x.to === c.to) || (x.from === c.to && x.to === c.from)));
            if (!dup) { sys.conns.push(c); added++; }
        }
        connMemory[compId] = [];
        return added;
    };
    ...
    const apply = () => {
        const tzn = sys && sys.comps ? sys.comps.tzn : null;
        const tz = sys && sys.comps ? sys.comps.tz : null;
        const showTZN = cb.checked;
        const showComp = showTZN ? tzn : tz;
        const hideComp = showTZN ? tz : tzn;
        const showId = showTZN ? 'tzn' : 'tz';
        const hideId = showTZN ? 'tz' : 'tzn';
        let changed = false;

        // ① 隐藏被切走的组件，并移除其连线（暂存以便下次恢复）
        if (hideComp && hideComp.group && hideComp.group.visible()) {
            takeConnsOf(hideId);
            hideComp.group.visible(false);
            changed = true;
        }
        // ② 显示目标组件，并恢复其历史连线
        if (showComp && showComp.group && !showComp.group.visible()) {
            showComp.group.visible(true);
            restoreConnsOf(showId);
            changed = true;
        }

        if (changed) {
            if (typeof sys.redrawAll === 'function') sys.redrawAll();
            else if (typeof sys.requestRedraw === 'function') sys.requestRedraw();
        }
    };

    // 供工作流自动演示/演练统一调用（保证复选框状态与实际显隐一致）
    window.applyShowTZN = apply;
    cb.addEventListener('change', apply);
    apply();    // 启动时按默认未勾选状态同步（显示 JSZ3、隐藏 JSZ3N）
})();
