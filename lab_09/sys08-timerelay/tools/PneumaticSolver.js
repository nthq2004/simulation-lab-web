...
export class PneumaticSolver {
    ...
    solve() {
        ...
        // 压力源（气瓶压力等）每帧可能变化，需与连接拓扑一起决定是否重算压力场
        const sourceKey = this._computeSourceKey();
        const pressKey = currentTopologyKey + '||' + sourceKey;

        // ── 压力场重算：连接拓扑或压力源变化时 ──
        if (pressKey !== this._cachedPressKey) {
            this._cachedPressKey = pressKey;
            ...
            // ── 压力源播种（气瓶 / 泵 / 执行器 / 校验仪，统一收集）──
            this._collectSources(allComps).forEach(src => {
                terminalPressures[src.portId] = src.pressure;
                queue.push(src.portId);
            });
            ...
            while (queue.length > 0) {
                ...
                this.sys.conns.forEach(conn => {
                    ...
                    if (nextPortId) {
                        ...
                        const compId = nextPortId.includes('_pipe_') ? nextPortId.split('_pipe_')[0]
                            : nextPortId.includes('_wire_') ? nextPortId.split('_wire_')[0]
                            : nextPortId.split('_')[0];
            ...
            // ── 仅在连接/开关拓扑变化时重建流量区域（压力变化不必重建）──
            if (currentTopologyKey !== this._cachedFlowKey) {
                this._cachedFlowKey = currentTopologyKey;
        ...
        // ── 压缩机充气：运转中且排气口接至气瓶时，逐帧为气瓶充气 ──
        this._updateCompressors();
    }
    ...
    _syncDevices() {
        ...
        const targetTypes = ['pressSwitch', 'pressMeter', 'regulator',
            'pressure_sensor', 'pressure_transducer', 'calibrator'];
        ...
        allComps.forEach(device => {
            if (targetTypes.includes(device.type)) {
                ...
                if (device.type === 'calibrator') {
                    // 校验仪气压测量：求解器用 MPa，显示用 kPa
                    const kpa = currentP * 1000;
                    if (device.upMode === 'MEAS_PRESSURE') {
                        device.upPressureValue = kpa;
                    }
                    if (device.activePanel === 'MEASURE' && device.measureMode === 'MEAS_PRESSURE') {
                        device.downPressureValue = kpa;
                    }
                    return;
                }
            ...
            } else if (device.special === 'pressrelay' && device.update) {
                // 压力继电器：取进气口压力，驱动触点动作（isEnergized → NC/NO 通断）
                const inPortId = `${device.id}_pipe_i`;
                device.update(pressures[inPortId] || 0);
            }
        });
    }
    ...
    _processInternalTransfer(device, inputPortId, terminalPressures, queue) {
        ...
        // 电磁阀：吸合时上下管路连通，截止时不通
        if (device.type === 'solenoid_valve') {
            if (inputPortId.startsWith(deviceId + '_pipe_')) {
                const otherPort = device.ports.find(p => p.type === 'pipe' && p.id !== inputPortId);
                if (otherPort) {
                    terminalPressures[otherPort.id] = device.isOpen ? inP : 0;
                    queue.push(otherPort.id);
                }
            }
        }
    ...
    /**
     * 收集全场压力源（统一出口：气瓶 / 泵 / 执行器 / 气泡液位 / 校验仪）。
     * 供压力场 BFS 播种与源状态键共用，避免重复枚举。
     * @returns {{portId:string, pressure:number}[]}
     */
    _collectSources(allComps) {
        const list = allComps || this.__allComps || (this.__allComps = Object.values(this.sys.comps));
        const sources = [];
        list.forEach(dev => {
            if (!dev || !dev.id) return;
            if (dev.type === 'airBottle') {
                sources.push({ portId: `${dev.id}_pipe_o`, pressure: dev.pressure || 0 });
            } else if (dev.type === 'Pump' && dev.pumpOn) {
                sources.push({ portId: `${dev.id}_pipe_o`, pressure: 0.4 });
            } else if (dev.special === 'actuator') {
                sources.push({ portId: `${dev.id}_pipe_o`, pressure: dev.outPress || 0 });
            } else if (dev.special === 'bubble_level') {
                sources.push({ portId: `${dev.id}_pipe_o`, pressure: dev.backPress / 1000 || 0 });
            } else if (dev.type === 'calibrator' && dev.activePanel === 'SOURCE' && dev.sourceMode === 'SRC_PRESSURE') {
                sources.push({ portId: `${dev.id}_pipe_o`, pressure: dev.sourceValue / 1000 || 0 });
            }
        });
        return sources;
    }
    ...
    /**
     * 压力源状态键：源端口与压力值组成的签名。
     * 与连接拓扑键拼接后用于判断是否需要重算压力场。
     */
    _computeSourceKey() {
        return this._collectSources()
            .map(s => `${s.portId}=${s.pressure.toFixed(5)}`)
            .sort()
            .join(';');
    }

    /**
     * 压缩机充气：运转中的压缩机，其排气口（_pipe_o）经管路（可穿过三通等
     * 无源管件）连到某气瓶时，逐帧为该气瓶充气（充气量取设备 `refillRate`）。
     */
    _updateCompressors() {
        const allComps = this.__allComps || (this.__allComps = Object.values(this.sys.comps));
        for (const dev of allComps) {
            if (!dev || dev.type !== 'airCompressor' || !dev.running) continue;
            const rate = dev.refillRate !== undefined ? dev.refillRate : 0.001;
            if (rate <= 0) continue;
            const bottle = this._findConnectedBottle(`${dev.id}_pipe_o`);
            if (bottle) bottle.refill(rate);
        }
    }

    /**
     * 从起始端口沿管路搜索相连的气瓶（允许经三通等无源管件间接连接）。
     * @returns {object|null} 气瓶组件
     */
    _findConnectedBottle(startPort) {
        const visited = new Set();
        const queue = [startPort];
        while (queue.length) {
            ...
            const devId = this._getDeviceIdFromPort(cur);
            const dev = this.sys.comps[devId];

            // 到达气瓶即为目标
            if (dev && dev.type === 'airBottle' && typeof dev.refill === 'function') return dev;
            ...
            (this.sys.conns || []).forEach(conn => {
                ...
                else if (conn.to === cur) next = conn.from;
                if (next && !visited.has(next)) queue.push(next);
            });

            // 无源管件（三通）内部导通：把同设备其它 pipe 端口入队
            if (dev && dev.type === 'teeConnector' && Array.isArray(dev.ports)) {
                dev.ports.forEach(p => {
                    if (p.type === 'pipe' && !visited.has(p.id)) queue.push(p.id);
                });
            }
        }
        return null;
    ...
    /** 公共接口：查询某气路端口当前压力（MPa） */
    getPressAt(portId) {
        return this.terminalPressures ? (this.terminalPressures[portId] || 0) : 0;
    }
    ...
    _computeTopologyKey() {
        ...
        Object.values(this.sys.comps || {}).forEach(dev => {
            ...
            } else if (dev.type === 'solenoid_valve') {
                parts.push(`${dev.id}:sv:open=${!!dev.isOpen}`);
