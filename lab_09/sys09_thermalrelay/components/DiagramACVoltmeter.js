// 交流电压表（简化外观 / Diagram 版）
//
// 电气模型完全继承 ACVoltmeter：type='ac_volt'、special='AC_VOLTMETER'、
// 端口 vp/vn、参数 maxVoltage/frequency/rampTime、tick 中的 RMS 计算逻辑
// 均与原型一致，求解器无需任何改动。
//
// 本类只重写"界面绘制"：删除右侧原理界面，仅保留左侧表盘操作界面，
// 两个接线柱贴在表盘圆周边缘（左下 / 右下对称）。

import { ACVoltmeter } from './ACVoltmeter.js';

export class DiagramACVoltmeter extends ACVoltmeter {
    ...
    _recalcGeometry() {
        ...
        this._termVp = { x: this._face.cx + outerR * Math.cos(aL), y: this._face.cy + outerR * Math.sin(aL) };
        this._termVn = { x: this._face.cx + outerR * Math.cos(aR), y: this._face.cy + outerR * Math.sin(aR) };
    ...
    _drawLeftTerminals() {
        ...
        const defs = [
            { pos: this._termVp, color: '#c83020' },
            { pos: this._termVn, color: '#3068c0' },
        ];
        defs.forEach(td => {
            // 黄铜接线柱
    ...
    _updateDynamic() {
        ...
        // 指针限位在满量程（_voltageToAngle 内部 clamp），超量程不再偏转
        this._needleAngle = this._voltageToAngle(v);
        this._needleGroup.rotation(this._needleAngle);

        // 液晶显示真实测量值：即使超量程也照常显示（超量程红色警示）
        const over = v > this.maxVoltage;
        const low  = v < this.maxVoltage * 0.20;
        this._voltText.text(`${v.toFixed(1)} V`);
        this._voltText.fill(over ? '#d02020' : (low ? '#e08030' : '#40c870'));
    }
}
