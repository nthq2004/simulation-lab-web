...
export class ThreePhaseLoad extends BaseComponent {
    ...
    // 有功功率（kW）访问器：无论通过界面、配置对话框还是代码直接赋值，
    // 都会同步重算负载阻抗并刷新面板上的功率显示值。
    get powerKw() { return this._powerKw; }
    set powerKw(v) {
        const nv = parseFloat(v);
        this._powerKw = Number.isFinite(nv) ? nv : 0;
        // 参数尚未初始化完（cosPhi/reactive 未就绪）时仅记录数值，避免重算用到未定义参数
        if (this.cosPhi === undefined || this.reactive === undefined) return;
        this._recalcLoad();
        // 面板已创建则同步刷新显示
        if (this._ui && this._ui.power) {
            this._ui.power.text(this._powerKw.toFixed(0));
            this._touchDirty();
        }
    }
    ...
    _initParameters(config) {
        ...
        // 联接方式：'star' 星形（默认，带中性点 N）/ 'delta' 角形（无中性点）
        this.connection = config.connection === 'delta' ? 'delta' : 'star';
        this._loaded  = config.loaded === true || config.loaded === 'true';
        // 电网失电时自动卸载：汇流排电压消失后自动断开负载，防止恢复供电时冲击过大
        this.noBusUnload = config.noBusUnload !== false;  // 默认 true
    ...
    _recalcLoad() {
        ...
        const w = 2 * Math.PI * F;
        const UL = UPH * Math.sqrt(3);                    // 额定线电压 ≈ 398V

        if (this.connection === 'delta') {
            // ── 角形（Δ）联接：无中性点，每个元件跨线电压 UL ──
            // 三相总有功 P = 3·UL²/RΔ  →  RΔ = 3·UL²/P
            this._Rd = 3 * UL * UL / P;
            this._Pph = P / 3;                            // 每元件 W（显示用）
            const Qph = (P / 3) * tan;                    // 每元件无功 var
            if (this.reactive === 'ind') {
                this._Ld = UL * UL / (w * Qph);           // 每元件电感 H
                this._Cd = 0;
            } else {
                this._Ld = 0;
                this._Cd = Qph / (w * UL * UL);           // 每元件电容 F
            }
            // 星形字段清零，避免误用
            this._Rph = 0; this._Qph = 0; this._Lph = 0; this._Cph = 0;
        } else {
            // ── 星形（Y）联接：每相到中性点，跨相电压 UPH ──
            this._Pph = P / 3;                            // 每相 W
            ...
            // 角形字段清零
            this._Rd = 0; this._Ld = 0; this._Cd = 0;
        }
    }
    ...
    _drawStaticParts() {
        ...
        if (this.connection === 'delta') {
            // 角形联接：标注 Δ，无中性点
            s.add(new Konva.Text({ x: this.width / 2 - 20, y: this.height - 16, text: 'Δ接', fontSize: 12, fontStyle: 'bold', fill: '#1a252f' }));
        } else {
    ...
    _addPorts() {
        ...
        // 角形联接无中性点
        if (this.connection !== 'delta') this.addPort(this._portN, this.height, 'n', 'wire');
    }
    ...
    tick(dt) {
        ...
        // 元件两端端口：星形跨相电压(l↔n)，角形跨线电压(l↔l)
        const pairs = this.connection === 'delta'
            ? [['l1', 'l2'], ['l2', 'l3'], ['l3', 'l1']]
            : [['l1', 'n'], ['l2', 'n'], ['l3', 'n']];
        const elemV = (i) => {
            const [a, b] = pairs[i];
            return this.sys.getVoltageBetween(`${this.id}_wire_${a}`, `${this.id}_wire_${b}`) || 0;
        };

        // ── 电网失电自动卸载 ──
        // 汇流排任意一相电压消失 → 自动断开负载（防止恢复供电时冲击过大）
        if (this._loaded && this.noBusUnload && this.sys && typeof this.sys.getVoltageBetween === 'function') {
            let busLost = true;
            for (let i = 0; i < 3; i++) {
                if (Math.abs(elemV(i)) > 10) { busLost = false; break; }
            }
            if (busLost) {
                this._loaded = false;
            }
        ...
        if (this._loaded && this.sys && typeof this.sys.getVoltageBetween === 'function') {
            ...
            const Le = this.connection === 'delta' ? this._Ld : this._Lph;
            const Ce = this.connection === 'delta' ? this._Cd : this._Cph;
            for (let i = 0; i < 3; i++) {
                const v = elemV(i);
                if (this.reactive === 'ind' && Le > 0) {
                    const gEq = sDt / Le;
     ...
     * @param {string} partId 'power'（有功功率输入框）| 'btn-load'（加载按钮）| 'btn-unload'（卸载按钮）
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        if (partId === 'power')      return { x: gx + 100 + 36, y: gy + 118 + 10 };      // 有功功率输入框 (100,118) w72 h20
        if (partId === 'btn-load')   return { x: gx + 18 + 50,  y: gy + 188 + 13 };      // 加载按钮 (18,188) w100 h26
        if (partId === 'btn-unload') return { x: gx + 164 + 50, y: gy + 188 + 13 };      // 卸载按钮 (164,188) w100 h26
        return null;
    }
