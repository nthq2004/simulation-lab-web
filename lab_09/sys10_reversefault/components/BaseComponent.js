export class BaseComponent {
    ...
    `;
        ...
        // ── 字段禁用支持：f.disabled 可为布尔，或 (comp, vals) => boolean ──
        // vals 为对话框中各字段的当前值，可据此联动启用/禁用其它字段。
        const currentVals = {};
        const applyDisabled = () => {
            fields.forEach(f => {
                const el = document.getElementById(`diag_${f.key}`);
                if (el) currentVals[f.key] = el.value;
            });
            fields.forEach(f => {
                if (f.disabled === undefined) return;
                const el = document.getElementById(`diag_${f.key}`);
                if (!el) return;
                const dis = typeof f.disabled === 'function'
                    ? !!f.disabled(this, currentVals) : !!f.disabled;
                el.disabled = dis;
                el.style.background = dis ? '#f0f0f0' : 'white';
                el.style.cursor = dis ? 'not-allowed' : '';
            });
        };
        content.addEventListener('change', applyDisabled);
        content.addEventListener('input', applyDisabled);
        applyDisabled();
    ...
    addClickablePart(partId, x, y, w, h, onTop = false) {
        ...
        hit.on('click tap', function(e) {
            ...
            var flash = new Konva.Rect({
                ...
                opacity: 1,
                // 纯视觉反馈，必须不参与命中，否则 0.6s 闪光期间会吞掉后续点击
                listening: false,
            });
            group.add(flash);
            flash.to({
