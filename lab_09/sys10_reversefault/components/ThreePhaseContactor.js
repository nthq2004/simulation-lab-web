...
export class ThreePhaseContactor extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // ── 线圈电压检测：峰值保持（快起慢衰）+ 连续零电压判定 ──
        // 仿真中交流源的模拟时间每帧仅推进 0.5ms，一个 50Hz 周期约需 40 帧（2s），
        // 若用整周期滑窗 RMS，吸合/释放会滞后约 2s。改用峰值包络：
        //   稳态时 peak/√2 = 交流有效值（与整周期 RMS 等价且无相位误差）；
        //   上电时包络在半个周期内即升至峰值 → 快速吸合；
        //   断电时端口电压近似为 0，连续若干帧判为真掉电 → 包络立即塌陷 → 快速释放。
        this._peakV          = 0;      // 电压峰值包络
        this._peakDecay      = 0.995;  // 每帧衰减系数（跨过正弦零点时不误塌陷）
        this._zeroEps        = 5;      // 判定"无电压"的电压阈值 (V)
        this._zeroRunNeed    = 2;      // 连续零电压帧数阈值（区分真掉电与正弦过零）
        this._zeroRun        = 0;
    ...
    _bindMomentaryPress() {
        ...
        this._momentaryHit = hit;
        this._onWindowUp = () => this._momentaryUp();

        hit.on('mousedown touchstart', (e) => this._momentaryDown(e));
    ...
    // ═══════════════════════════════════════════
    // 可识别部件（工作流 find 步骤）：电磁线圈、静铁芯、动衔铁、
    // 反作用弹簧、主触头、辅助常开触头、辅助常闭触头
    // ═══════════════════════════════════════════

    _addClickableParts() {
        if (!this._interactGroup) return;

        // ── 左侧电磁机构 ──
        // 静铁芯（E 形铁心：垂直磁轭 + 三条水平铁心腿）
        const coreX = this._yoke.x;
        const coreRight = this._legs[0].x + this._legs[0].w;
        this.addClickablePart('core', coreX, this._yoke.y, coreRight - coreX, this._yoke.h);

        // 电磁线圈（绕在中柱上，层级高于静铁芯）
        const cr = this._coilRect;
        this.addClickablePart('coil', cr.x - 2, cr.y - 2, cr.w + 4, cr.h + 4);

        // 动衔铁（热区覆盖吸合/释放全行程）
        const armTop = this._yoke.y + 4;
        this.addClickablePart('armature', this._armX, armTop,
            this._armW + this._armOffsetMax, this._armH);

        // 反作用弹簧（上方螺旋弹簧）
        const springLeft = this._armX + this._armW;
        const springRight = this._springAnchorRight + 4;
        this.addClickablePart('spring', springLeft, this._springTopY - 12,
            springRight - springLeft, 24);

        // ── 右侧触点组 ──
        const slots = this._contactSlots;
        const slotW = slots[1].cx - slots[0].cx;
        const boxOf = (list) => {
            const min = Math.min(...list.map(s => s.cx));
            const max = Math.max(...list.map(s => s.cx));
            return { x: min - slotW / 2, w: (max - min) + slotW };
        };
        const cY = this._shaftY - 40, cH = 80;

        const main = boxOf(slots.filter(s => s.isMain));
        this.addClickablePart('main-contact', main.x, cY, main.w, cH);

        // 辅助常开（NO1 13-14）、辅助常闭（NC1 31-32）
        this.addClickablePart('aux-no', slots[0].cx - slotW / 2, cY, slotW, cH);
        this.addClickablePart('aux-nc', slots[1].cx - slotW / 2, cY, slotW, cH);

        // 左侧热区与整体按压区重叠：在部件上按下/松开同样触发电磁机构吸合/释放
        ['core', 'coil', 'armature', 'spring'].forEach(pid => {
            const node = this.getClickablePartNode(pid);
            if (!node) return;
            node.on('mousedown touchstart', (e) => this._momentaryDown(e));
            node.on('mouseleave', () => this._momentaryUp());
        });
    }

    /**
     * 部件中心（画布绝对坐标）：供工作流自动演示箭头/圈选定位。
     * - 结构类部件按几何中心计算（动衔铁随当前吸合位置动态变化）；
     * - 端口 id（a1/a2/l1/t1/no1a/no1b/nc1a/nc1b…）返回端口绝对坐标，
     *   便于接线演示时箭头指向接线端子（不注册热区，避免遮挡端口点击）。
     */
    getClickablePartCenter(partId) {
        const t = this.group && this.group.getAbsoluteTransform
            ? this.group.getAbsoluteTransform()
            : null;
        const abs = (lx, ly) => {
            if (!t) return { x: lx, y: ly };
            const p = t.point({ x: lx, y: ly });
            return { x: p.x, y: p.y };
        };

        // 接线端口（origId 匹配，如 'a1'、'l1'）
        const port = this.ports && this.ports.find(p => p.origId === partId);
        ...
        switch (partId) {
            case 'coil': {
                const cr = this._coilRect;
                return abs(cr.x + cr.w / 2, cr.y + cr.h / 2);
            }
            case 'core':
                // 指向垂直磁轭（避免与绕在中柱上的线圈指示重叠）
                return abs(this._yoke.x + this._yoke.w / 2, this._yoke.y + this._yoke.h / 2);
            case 'armature':
                return abs(this._armX + this._armOffsetCur + this._armW / 2, this._shaftY);
            case 'spring':
                return abs((this._armX + this._armW + this._springAnchorRight + 4) / 2, this._springTopY);
            case 'main-contact':
                return abs((this._contactSlots[2].cx + this._contactSlots[4].cx) / 2, this._shaftY);
            case 'aux-no':
                return abs(this._contactSlots[0].cx, this._shaftY);
            case 'aux-nc':
                return abs(this._contactSlots[1].cx, this._shaftY);
            default:
    ...
    tick(dt) {
        if (this._autoSense && this.sys.getVoltageBetween && !this._momentaryHeld) {
            ...
            if (vRaw !== undefined && isFinite(vRaw)) {
                ...
                if (vAbs < this._zeroEps) {
                    // 端口近似为 0：可能是正弦过零，也可能是真掉电。
                    // 连续若干帧为 0 才判定为真掉电，避免过零瞬间误释放。
                    this._zeroRun++;
                    if (this._zeroRun >= this._zeroRunNeed) this._peakV = 0;
                } else {
                    this._zeroRun = 0;
                    // 峰值保持（快起慢衰）：稳态时 peak/√2 即交流有效值
                    this._peakV = Math.max(vAbs, this._peakV * this._peakDecay);
                }
                this._vRms = this._peakV / Math.SQRT2;
            }
            if (!this._animating) {
