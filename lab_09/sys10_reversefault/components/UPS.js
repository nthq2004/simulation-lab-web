...
export class UPS extends BaseComponent {
    ...
    /**
     * 返回可点击部件/关键模块的中心世界坐标，供工作流箭头定位。
     * @param {string} partId 部件 ID：
     *   in_p / in_n / out1_p / out1_n / out2_p / out2_n —— 接线端口；
     *   led-line / led-battery / led-bypass / led-fault —— 状态指示灯；
     *   inputSwitch / batterySwitch / outputSwitch / loadSwitch1 / loadSwitch2 —— 五个操作开关；
     *   onButton / offButton / entButton —— 面板按钮；
     *   rectifier / inverter / battery / staticSwitch —— 关键模块识别区。
     * @returns {{x:number,y:number}|null}
     */
    getClickablePartCenter(partId) {
        const gx = this.group ? this.group.x() : 0;
        const gy = this.group ? this.group.y() : 0;
        const rect = (r) => ({ x: r.x + r.w / 2, y: r.y + r.h / 2 });
        const ledAt = (k) => { const l = this._leds.find(x => x.key === k); return l ? { x: l.x, y: l.y } : null; };
        const rel = {
            // 接线端口
            'in_p':   { x: this._portIn.p.x,   y: this._portIn.p.y },
            'in_n':   { x: this._portIn.n.x,   y: this._portIn.n.y },
            'out1_p': { x: this._portOut1.p.x, y: this._portOut1.p.y },
            'out1_n': { x: this._portOut1.n.x, y: this._portOut1.n.y },
            'out2_p': { x: this._portOut2.p.x, y: this._portOut2.p.y },
            'out2_n': { x: this._portOut2.n.x, y: this._portOut2.n.y },
            // 状态指示灯
            'led-line':    ledAt('line'),
            'led-battery': ledAt('bat'),
            'led-bypass':  ledAt('byp'),
            'led-fault':   ledAt('fault'),
            // 五个操作开关
            'inputSwitch':   { x: this._swInput.cx,  y: this._swInput.cy },
            'batterySwitch': { x: this._swBatt.cx,   y: this._swBatt.cy },
            'outputSwitch':  { x: this._swOutput.cx, y: this._swOutput.cy },
            'loadSwitch1':   { x: this._swLoad1.cx,  y: this._swLoad1.cy },
            'loadSwitch2':   { x: this._swLoad2.cx,  y: this._swLoad2.cy },
            // 面板按钮
            'onButton':      rect(this._btns.on),
            'offButton':     rect(this._btns.off),
            'entButton':     rect(this._btns.ent),
            // 关键模块识别区
            'rectifier':     rect(this._rectifier),
            'inverter':      rect(this._inverter),
            'battery':       rect(this._battery),
            'staticSwitch':  { x: (this._staticSw.top.x + this._staticSw.bot.x) / 2,
                               y: (this._staticSw.top.y + this._staticSw.bot.y) / 2 },
        };
        const p = rel[partId];
