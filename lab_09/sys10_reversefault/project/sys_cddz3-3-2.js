    ...
    'auto-mech-trip-recovery': {
        ...
        steps: [
            // ── 步骤 1：自动接线，起动 1# 发电机，合闸供电 ──
            {
                ...
                op: [
                    ...
                    },
                    {
                        ...
                        msg: '按住 1# 发电机组遥控面板"起动"按钮，起动 1# 发电机，随后等待建压与合闸弹簧储能（约 3s）',
                        async act() {
                            const sys = this.sys;
                            await _pressPanelBtn(sys, 'genpanel', '_userStartPressed', 1200);
                            await _sleep(3000); // 等待建压与储能
                        }
                    },
                    {
                        type: 'btn', target: 'genpanel', part: 'btn-close',
                        msg: '建压储能到位后按住 1# 遥控面板"合闸"按钮，1# 主开关合闸向汇流排供电',
                        async act() {
                            const sys = this.sys;
            ...
            // ── 步骤 2：将电站投入自动运行模式（备用顺序默认 123）──
            {
                ...
                op: [
                    {
                        type: 'knob', target: 'auto_ctl', part: 'knob-mode',
                        msg: '将"船舶电站自动控制模块"的模式旋钮从"手动"切换到"自动"档（备用顺序旋钮保持默认 1-2-3）',
                        async act() {
                            const sys = this.sys;
                            const ac = sys.comps.auto_ctl;
                            if (ac && !ac._auto) {
                    ...
                    {
                        type: 'observe', target: 'auto_ctl', part: 'led-auto',
                        msg: '观察"船舶电站自动控制模块"："自动"指示灯点亮，自动电站进入自动运行状态，开始实时监测母线与各机组状态',
                        async act() {
                            await _sleep(1500);
                        }
                    },
                ...
                op: [
                    {
                        ...
                        msg: '打开故障设置面板，勾选"1#机冷却水温高故障"并应用（演练模式下请手动操作）'
                    },
                    {
                        type: 'observe', target: 'gen1',
                        msg: '观察 1# 同步发电机：冷却水温高保护动作，单机运行立即停机',
                        async act() {
            ...
            // ── 步骤 4：观察自动电站失电自动起动 2# 机组并自动合闸恢复 ──
            {
                ...
                op: [
                    {
                        type: 'observe', target: 'auto_ctl', part: 'lcd',
                        msg: '观察自动控制模块：检测到母线失电（全船失电），按备用顺序 1-2-3 查找可用机组——识别 1# 机处于故障状态自动跳过，选中 2# 机并发出起动命令',
                        async act() {
                            // 自动等待：失电 3s 检测 → 起动 2#（约 10~16s 完成恢复）
                            await _sleep(500);
                        }
                    },
                    {
                        type: 'observe', target: 'gen2',
                        msg: '观察 2# 同步发电机：自动电站经通信接口自动发出起动命令，2# 机自动起动、建压并等待合闸弹簧储能',
                        async act() {
                            await _waitUntil(() => { const g = _fcomp('gen2'); return !!g && g.isOn; }, 40000);
                            await _sleep(500);
                        }
                    },
                    {
                        type: 'observe', target: 'qf2', part: 'main-contact',
                        msg: '观察 2# 主开关：2# 机建压储能完成后，自动电站延时自动发出合闸命令，主开关合闸恢复全船供电',
                        async act() {
                            await _waitUntil(() => { const q = _fcomp('qf2'); return !!q && q.getState() === 'on'; }, 20000);
                            await _sleep(500);
                        }
                    },
                ],
    ...
    'auto-uv-trip-recovery': {
        ...
        steps: [
            ...
            // ── 步骤 3：触发 1# 机欠压故障（用户手动），观察主开关欠压保护跳闸 ──
            {
                ...
                op: [
                    {
                        type: 'fault', fault: 'gen1_undervoltage',
                        msg: '打开故障设置面板，勾选"1#机欠压故障"并应用（演练模式下请手动操作）'
                    },
                    {
                        type: 'observe', target: 'qf1', part: 'main-contact',
                        msg: '观察 1# 主开关：欠压保护检测到电压跌落，延时（约 2s）动作跳闸、全船失电',
                        async act() {
            ...
            // ── 步骤 4：观察自动电站失电自动起动 2# 机组并自动合闸恢复 ──
            {
                ...
                op: [
                    {
                        type: 'observe', target: 'auto_ctl', part: 'lcd',
                        msg: '观察自动控制模块：检测到母线失电（全船失电），按备用顺序 1-2-3 查找可用机组——识别 1# 机带欠压故障自动跳过，选中 2# 机并发出起动命令',
                        async act() {
                            // 自动等待：失电 3s 检测 → 起动 2#（约 10~16s 完成恢复）
                            await _sleep(3500);
                        }
        ...
        steps: [
                ...
                op: [
                    {
                        ...
                        msg: '打开故障设置面板，勾选"汇流排干线短路故障"并应用（演练模式下请手动操作）'
                    },
                    {
                        type: 'observe', target: 'qf1', part: 'main-contact',
                        msg: '观察 1# 主开关：短路电流巨大，短路保护瞬时动作跳闸（全船失电）',
                        async act() {
                            await _waitUntil(() => { const q = _fcomp('qf1'); return !q || q.getState() === 'off'; }, 15000);
                            await _sleep(500);
                        }
            ...
            // ── 步骤 4：观察短路期间自动模式被阻塞，随后将控制模式转回手动 ──
            {
                ...
                op: [
                    {
                        type: 'observe', target: 'auto_ctl', part: 'led-auto',
                        msg: '观察自动控制模块：检测到短路标记，自动模式被阻塞（"自动"指示灯熄灭）——不自动起动任何机组、不自动合闸，防止对短路点反复送电',
                        async act() {
                    ...
                    {
                        type: 'knob', target: 'auto_ctl', part: 'knob-mode',
                        msg: '确认自动模式已阻塞后，将"船舶电站自动控制模块"的模式旋钮拨回"手动"档，退出自动运行',
                        async act() {
                            const sys = this.sys;
                            const ac = sys.comps.auto_ctl;
