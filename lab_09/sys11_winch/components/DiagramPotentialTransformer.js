// 电压互感器（简化原理图 / Diagram 版）
//
// 电气模型完全继承 PotentialTransformer：type='potential_transformer'、
// special='POTENTIAL_TRANSFORMER'、端口 s1/s2/p1/p2、参数 turnsRatio/primaryRated/
// secondaryRated、回读字段 V_primary/V_secondary 等均与原型一致，
// 求解器（DeviceStamps.stampPotentialTransformers / CircuitSolver）无需任何改动。
//
// 本类只重写"界面绘制"：横置原理图，左侧原边（多而细，10 匝）、右侧副边（少而粗，4 匝）。

import { PotentialTransformer } from './PotentialTransformer.js';

export class DiagramPotentialTransformer extends PotentialTransformer {
    ...
    _drawWindings() {
        ...
        this._staticGroup.add(new Konva.Path({
            data: this._windingPath(c.x, true, 10, 9),
            stroke: '#c0392b', strokeWidth: 1.4, lineCap: 'round', fillEnabled: false,
        }));
        // 副边（右）：少而粗 —— 4 匝
        this._staticGroup.add(new Konva.Path({
            data: this._windingPath(c.x + c.w, false, 4, 11),
            stroke: '#2471a3', strokeWidth: 4.5, lineCap: 'round', fillEnabled: false,
        }));
    }
    ...
    _updateDynamic() {
        const v1 = this.V_primary || 0, v2 = this.V_secondary || 0;
