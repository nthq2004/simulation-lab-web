...
export class InductionMotor2 extends BaseComponent {
    ...
    _updateDynamic(dt = 1 / 20) {
        ...
        // 缺相运行判定：用各相电流的短窗 RMS（瞬时值会在每周期过零，不能直接比较），
        // 电机已在旋转（|ω|>5）且某相 RMS 持续近零 → 单相运行。
        // 断相绕组的感应电动势仍会产生端电压（hasAllPhases 可能成立），会干扰
        // 相序判别而把相序判成负序 → 同步速反号 → 电机制动堵转（而非缺相过载）。
        // 故缺相运行时锁存相序（保持故障前方向），直至修复并稳定后再解除。
        if (!this._iRmsWin) this._iRmsWin = [[], [], []];
        const iAbs = [Math.abs(this.phaseCurrents?.u || 0), Math.abs(this.phaseCurrents?.v || 0), Math.abs(this.phaseCurrents?.w || 0)];
        for (let i = 0; i < 3; i++) {
            this._iRmsWin[i].push(iAbs[i]);
            if (this._iRmsWin[i].length > 12) this._iRmsWin[i].shift();
        }
        const iRms = this._iRmsWin.map(arr => Math.sqrt(arr.reduce((a, b) => a + b * b, 0) / Math.max(1, arr.length)));
        const iRmsMin = Math.min(iRms[0], iRms[1], iRms[2]);
        const iRmsMax = Math.max(iRms[0], iRms[1], iRms[2]);
        // 单相运行判据：一相电流远小于其余相（且确有电流），比"某相<1A"更稳，不受波动影响
        const phaseMissing = iRmsMax > 2 && iRmsMin < Math.max(1, iRmsMax * 0.05);
        const running = Math.abs(this._omega_m) > 5;
        if (running && phaseMissing) {
            if (!this._phaseLost) this._phaseLockedSeq = this._phaseSeq || 1;  // 记录故障前相序
            this._phaseLost = true;
            this._phaseGood = 0;
        } else if (this._phaseLost && running && !phaseMissing) {
            // 缺相修复后需连续多帧三相都有电流才解除锁存，避免瞬时误清
            this._phaseGood = (this._phaseGood || 0) + 1;
            if (this._phaseGood > 20) { this._phaseLost = false; this._phaseGood = 0; }
        } else if (!running) {
            this._phaseLost = false;
            this._phaseGood = 0;
        }
        // 缺相期间强制沿用故障前相序（防止断相绕组感应电动势使相序翻负 → 反转/堵转）
        if (this._phaseLost) this._phaseSeq = this._phaseLockedSeq || 1;

        if (hasAllPhases && solver && !this._phaseLost) {
            ...
            // 默认保持初始相序（正转），需连续 3 帧一致才改变，避免起动/缺相等
            // 瞬态下首次误判即锁定错误方向（反转）而堵转。
            if (cur === 0 || cur === det) {
                this._seqVotes += det;
        ...
        } else if (!hasVoltage && !this._phaseLost) {
            // 强制正序：失电时也保持正序，保证所有计算按正序进行
            // 失电时回到默认正序（正转），保证下次通电按默认方向起转
            this._phaseSeq = 1;
            this._seqVotes = 0;
        ...
        const energized = hasVoltage || this._dcBraking;
        this._fieldGroup.visible(energized);

        // 单相运行（缺相）时气隙磁场为脉动磁场：不旋转，幅值周期性脉动（一闪一闪）。
        // 用显式单相判定 + 固定频率脉动，保证现象稳定一致，而不受交流瞬时电压过零影响。
        const singlePhasing = energized && phaseMissing;
        this._fieldSinglePhase = singlePhasing;

        if (energized) {
            if (singlePhasing) {
                // 脉动磁场：以约 3Hz 的固定频率明暗闪动，不旋转
                this._fieldPulse = (this._fieldPulse || 0) + dt * 2 * Math.PI * 3;
                if (this._fieldPulse > 1e6) this._fieldPulse = 0;
                const p = 0.5 + 0.5 * Math.sin(this._fieldPulse);
                this._fieldGroup.opacity(0.12 + 0.88 * p);
            } else {
    ...
    _preSolve(dt) {
        ...
        // 机械时间常数约 10ms，求解器每帧按子步细分调用本函数。
        // 先按当前转速算出滑差与电磁转矩，再做积分，使转矩能随转速变化，
        // 避免细分积分因转矩滞后而发散（转速越过同步速）。
        this._updateSlip();
        this._Te = this._computeTe();

        const accel = (this._Te - loadT - brakeT - this.B * this._omega_m) / this.J;
        this._omega_m += accel * dt;

        // 缺相单相运行：电机仍以约 3% 转差继续运行（不应因不平衡转矩而堵转）。
        // 本仿真电机为三相等效模型、未含负序磁场，故显式维持转速，使现象表现为
        // "电机继续运行、健康两相电流增大（过载）"，而非直接堵转。
        if (this._phaseLost && !this._dcBraking && Math.abs(this._omega_sync) > 1) {
            this._omega_m = this._omega_sync * 0.97;
        }
    ...
    /**
     * 稳态等效电路电磁转矩（与 CircuitSolver 一致），
     * 用本帧实测端电压 _Vrms 与当前滑差计算，供细分积分逐次调用。
     */
    _computeTe() {
        const omegaSync = this._omega_sync || 0;
        if (Math.abs(omegaSync) < 1e-6) return 0;
        const Vrms = this._Vrms || 0;
        const Vrms_sq = Vrms * Vrms;
        let slip = this.slip || 1;
        if (Math.abs(slip) < 0.0001) slip = slip >= 0 ? 0.0001 : -0.0001;

        const R1 = this.R1 || 2.15;
        const R2 = this.R2 || 0.42;
        const sysFreq = (this.sys?.voltageSolver?._systemFreq) || 50;
        const X_total = 2 * Math.PI * sysFreq * ((this.Lsigma1 || 0.004) + (this.Lsigma2 || 0.005));
        ...
        // 缺相且电机处于起动状态 → 强制转矩为 0（三相缺一相无法产生起动转矩）
        if (this._hasAllPhase === false && Math.abs(this._omega_m) < 5) te = 0;

        // 反接制动转矩增强（|s| > 1.2 时乘 2·(|s|-0.7)，模拟集肤效应）
        if (Math.abs(slip) > 1.2) te *= 2 * (Math.abs(slip) - 0.7);

        // 限幅到物理可实现的最大转矩 T_max = 3·V²/(2·|ω_sync|·(R₁+√(R₁²+X²)))
