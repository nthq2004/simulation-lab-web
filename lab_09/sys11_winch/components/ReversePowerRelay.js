...
export class ReversePowerRelay extends BaseComponent {
    ...
    /**
     * 返回可点击部件中心的世界坐标（供工作流 find / 自动演示箭头定位）
     * @param {string} partId 'dir'|'power'|'cd'|'trip-led'|'reset-btn'|'contact'|'no'|'nc'
     */
    getClickablePartCenter(partId) {
        const p = this._pane;
        const rel = {
            'dir':       { x: p.x + p.w / 2, y: p.y + 35 },                          // 方向指示文字
            'power':     { x: p.x + p.w / 2, y: p.y + 74 + 24 },                     // 功率数值
            'cd':        { x: p.x + p.w / 2, y: p.y + 150 + 20 },                    // 延时倒计时
            'trip-led':  { x: this._led.trip.x, y: this._led.trip.y },               // TRIP 指示灯
            'reset-btn': { x: this._resetBtn.x + this._resetBtn.w / 2, y: this._resetBtn.y + this._resetBtn.h / 2 }, // 复位按钮
            'contact':   { x: this._contactCx, y: this._contactCy },                 // 触点机构
            'no':        { x: this._portOutput.no.x, y: this._portOutput.no.y },     // 常开触点 NO
            'nc':        { x: this._portOutput.nc.x, y: this._portOutput.nc.y },     // 常闭触点 NC
        };
        const r = rel[partId];
        if (!r) return null;
        const pt = this.group.getAbsoluteTransform().point({ x: r.x, y: r.y });
