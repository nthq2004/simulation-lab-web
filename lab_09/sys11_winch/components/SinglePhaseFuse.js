...
export class SinglePhaseFuse extends BaseComponent {
    constructor(config, sys) {
        ...
        this.width  = Math.max(20, config.width  || 28);
    ...
    tick(dt) {
        // ── 电流计算 + 过流自保护 ──
        // 电流由求解器按熔断器两端 0.01Ω 压降回填（physCurrent）；不能用
        // getVoltageBetween 直接读，因 0.01Ω 与回路其它阻抗并联时压降近零。
        if (this._states[0] === 'ok') {
            const solver = this.sys?.voltageSolver;
            if (solver) {
                if (this.physCurrent === undefined) {
                    const cL = solver.portToCluster.get(`${this.id}_wire_l`);
                    const cT = solver.portToCluster.get(`${this.id}_wire_t`);
                    if (cL !== undefined && cT !== undefined) {
                        this.physCurrent = ((solver.nodeVoltages.get(cL) || 0) - (solver.nodeVoltages.get(cT) || 0)) / 0.01;
                    }
                }
                // 过流熔断：正常过流按物理电流判别；此外，被保护回路存在"近零阻抗
                // 短路元件"（如接触器线圈被注入 0.001Ω）时，短路电流受变压器内阻抗
                // 限制、熔断器压降不显著，故补充按回路短路判据直接熔断。
                const overcurrent = Math.abs(this.physCurrent) > this.ratedCurrent * 1.2;
                const shortInLoop = this._loopHasShort();
                if (overcurrent || shortInLoop) {
                    this.blow(0);
                }
            }
        }
    ...
    /** 被保护的控制回路中是否存在近零阻抗短路元件（接触器线圈电阻被注入为 0.001Ω） */
    _loopHasShort() {
        // 仅副边熔断器（FU5）承担"控制回路短路"保护职责：短路电流由 FU5 切断。
        // 一次侧熔断器 FU4 不参与该判据，避免级联误熔断。
        if (this.id !== 'fu5') return false;
        const comps = this.sys?.comps;
        if (!comps) return false;
        for (const k in comps) {
            const c = comps[k];
            if (c && c.type === 'ContactorDevice' && c.special === 'contactcoil'
                && c._coilResistance !== undefined && c._coilResistance < 0.01) {
                // 线圈回路得电（线圈两端有电压）时才熔断
                const v = this.sys.getVoltageBetween?.(`${c.id}_wire_a1`, `${c.id}_wire_a2`) || 0;
                if (Math.abs(v) > 20) return true;
            }
        }
        return false;
    }
    ...
    showContextMenu(evt) {
        ...
        const isBlown = this._states[0] === 'blown';
        menu.appendChild(createItem(isBlown ? '更换熔断器' : '模拟熔断', () => {
            isBlown ? this.replace(0) : this.blow(0);
        }));
