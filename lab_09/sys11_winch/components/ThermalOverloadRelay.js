...
export class ThermalOverloadRelay extends BaseComponent {
    ...
    constructor(config, sys) {
        ...
        // 注册可识别部件（供工作流自动演示箭头精确圈选 RESET/TEST/整定旋钮）
        this._addClickableParts();
    }

    /**
     * 注册可识别部件，仅用于自动演示的箭头/圈选定位：
     *   reset — 红色 RESET 复位按钮
     *   knob  — 整定电流旋钮
     *   test  — TEST 机械测试按钮
     * 命中节点一律关闭监听，避免遮挡旋钮拖动、RESET 点击等原有交互。
     */
    _addClickableParts() {
        const add = (id, cx, cy, r) => {
            const hit = this.addClickablePart(id, cx - r, cy - r, r * 2, r * 2);
            if (hit && typeof hit.listening === 'function') hit.listening(false);
        };
        add('reset', this._resetBtnX, this._resetBtnY, this._resetBtnR);
        add('knob',  this._knobX,     this._knobY,     this._knobR);
        add('test',  this._testBtnX,  this._testBtnY,  this._testBtnR);
    }

    /** 部件中心（画布绝对坐标，计入组件自身位移/旋转/缩放） */
    ...
    _initParameters(config) {
        ...
        this.ratedCurrent = config.ratedCurrent !== undefined ? config.ratedCurrent : 9; // 整定电流 A（指针目标值）
        this._defaultRated = this.ratedCurrent;   // 刻度盘中心值（物理刻度，出厂固定，不随整定值移动）
        this._pointerRatedCur = this.ratedCurrent; // 指针当前显示值（用于“转动指针”的平滑动画）
    ...
    _createBimetals() {
        ...
        this._poleData.forEach((p, i) => {
            ...
            const lo = new Konva.Shape({
                ...
                listening: false,
                opacity: 0.35,
                // 关键性能项：opacity<1 且同时有 fill+stroke 时，Konva 默认的
                // perfectDrawEnabled 会为每个双金属片各建一块离屏缓冲画布（每帧昂贵）；
                // 关闭后直接以 globalAlpha 绘制，外观不变、整层绘制耗时降至约 1/30。
            ...
            const hi = new Konva.Shape({
                ...
                listening: false,
                opacity: 0.35,
                perfectDrawEnabled: false,   // 见低膨胀层说明：避免每帧离屏缓冲
            });
            hi._bend = initBend;
            g.add(hi);
    ...
    _updateKnobPointer() {
        ...
        const kx = this._knobX, ky = this._knobY;
        const defR = this._defaultRated;
        // 指针按“当前显示值”定位（平滑转动过程中的中间值），刻度盘固定不变
        const cur = (this._pointerRatedCur !== undefined) ? this._pointerRatedCur : this.ratedCurrent;
        const ratio = defR > 0 ? cur / defR : 1;
    ...
    _updateDynamic() {
        ...
        // 状态标签：仅在文字/颜色真正变化时更新（避免每帧 measureText 与多余重绘）
        const stText = this._getStateLabelText();
        if (this._lastStateText !== stText) { this._stateLabel.text(stText); this._lastStateText = stText; changed = true; }
        const stColor = this._getStateLabelColor();
        if (this._lastStateColor !== stColor) { this._stateLabel.fill(stColor); this._lastStateColor = stColor; changed = true; }

        // 整定旋钮指针：向目标整定值平滑转动（改整定值时可见“指针转动”而非刻度跳动）
        if (this._pointerRatedCur === undefined) this._pointerRatedCur = this.ratedCurrent;
        const ptrDiff = this.ratedCurrent - this._pointerRatedCur;
        if (Math.abs(ptrDiff) > 1e-3) {
            const dt = this._lastDt || 0.05;
            this._pointerRatedCur += ptrDiff * Math.min(1, dt * 6);   // 指数趋近，约 0.5s 到位
            if (Math.abs(this.ratedCurrent - this._pointerRatedCur) < 1e-3) {
                this._pointerRatedCur = this.ratedCurrent;
            }
            this._updateKnobPointer();
            changed = true;
        }
        // “整定 xxA”文字：随目标值更新
        if (this._knobPaintedRated !== this.ratedCurrent) {
            if (this._ratedLabel) this._ratedLabel.text(`整定 ${this.ratedCurrent.toFixed(1)}A`);
            this._knobPaintedRated = this.ratedCurrent;
            changed = true;
        }
    ...
    tick(dt) {
        ...
        const changed = this._updateDynamic();
        this._refreshIfDirty();   // 缓存脏时内部会自行 requestRedraw
        if (changed && this.sys && typeof this.sys.requestRedraw === 'function') {
    ...
    getConfigFields() {
        return [
            ...
            { label: '整定电流 (A)',   key: 'ratedCurrent', type: 'number',
                min: Math.max(0.1, this._defaultRated * 0.5),
                max: this._defaultRated * 1.5,
    ...
    onConfigUpdate(cfg) {
        ...
        if (cfg.ratedCurrent !== undefined) {
            // 只更新整定目标值 → 指针随之转动到对应刻度；
            // 刻度盘中心值 _defaultRated 为物理刻度，出厂固定，不随整定值移动。
        ...
        this._addClickableParts();   // 几何重建后重新注册可识别部件（尺寸/位置可能已变）
        this._refreshCache();
