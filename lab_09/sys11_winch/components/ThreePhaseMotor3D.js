...
export class ThreePhaseMotor3D extends BaseComponent {
    constructor(config, sys) {
        ...
        this.height = Math.max(240, config.height || 305);
        ...
        this.config = {
            ...
            cosphi:       this.cosphi,
            status:       this._stalled ? 'stall' : 'normal',
            loadRate:     this._loadRate,
            insulR:       this._insulR,
        };
    ...
    _recalcGeometry() {
        ...
        this._head = H * 0.185;                  // 顶部转速显示带高度
        this._speedPanel = { x: W * 0.028, y: H * 0.017, w: W * 0.944, h: H * 0.165 };
        this._cylR  = Math.max(42, Math.min(H * 0.27, W * 0.24));
        this._cylCY = 58 + this._head + this._cylR;   // 机身中心 y（顶部留显示带 + 接线盒空间）
        ...
        // 接线盒（顶部小 3D 盒，置于圆柱顶面）
        // 接线盒宽度加倍；三根接线端子间距同步加倍（原 23 → 46）
        const tbW = 156, tbH = 30;
        ...
        // PE 端子（机壳右上角，避开右端盖风扇区域，黄绿）
        this._peX = this._cylX2 - this._cylR * 0.60;
        this._peY = this._cylCY - this._cylR + 8;
    ...
    _initParameters(config) {
        ...
        this.ratedPower   = config.ratedPower   !== undefined ? parseFloat(config.ratedPower) : 5.0;   // kW
        this.ratedVoltage = config.ratedVoltage !== undefined ? config.ratedVoltage : 380;
        this.ratedFreq    = config.ratedFreq    !== undefined ? parseFloat(config.ratedFreq)  : 50;    // Hz
        ...
        // 以铭牌额定功率为准（纯阻性星形等效，cosφ≈1），推算：
        //   每相 100% 负荷注入电阻 R_100 = U_line² / P_N
        //   额定电流 I_N = P_N / (√3·U_line) = U_ph / R_100
        // 参数 r100 可显式覆盖注入电阻（不再由功率反推）。
        this._r100Explicit = config.r100 !== undefined;
        this._R100 = this._r100Explicit
            ? parseFloat(config.r100)
            : this.ratedVoltage * this.ratedVoltage / (this.ratedPower * 1000);
        this.ratedCurrent = config.ratedCurrent !== undefined
            ? parseFloat(config.ratedCurrent)
            : +(this.ratedPower * 1000 / (Math.sqrt(3) * this.ratedVoltage)).toFixed(2);   // A

        // ── 状态 / 负荷率（决定 U/V/W 每相对地注入电阻）──
        this._stalled  = (config.status || 'normal') === 'stall';
        const lr = config.loadRate !== undefined ? parseFloat(config.loadRate) : 1;
        this._loadRate = [0, 0.25, 0.5, 0.75, 1, 1.25].includes(lr) ? lr : 1;
        this._phaseGroundR = this._computePhaseGroundR();

        // ── 各相绕组对机壳（PE）的绝缘电阻（Ω），供兆欧表/绝缘检测读取 ──
        // Infinity 表示未建模（不影响负载阻抗）
        this._insulR = (config.insulR !== undefined && config.insulR !== null)
            ? parseFloat(config.insulR) : Infinity;

        // 旋转动画状态
        this._rotorAngle = 0;
        this._powered    = false;
        this._phaseSeq   = 0;      // 1 正转(CW) / -1 反转(CCW) / 0 无
        this._lastAng    = undefined;   // 空间矢量角（相序检测用）
        this._supplyFreq = null;        // 变频器输出频率（Hz），用于转速显示
        this._linePeak   = 0;           // 工频线电压峰值保持
    }
     ...
     *   堵转        → R_100 / 5
     *   负荷率 = 0  → R_100 × 10
     *   负荷率 ≠ 0  → R_100 / 负荷率（100% → R_100）
     */
    _computePhaseGroundR() {
        const R = this._R100 || 28.88;
        if (this._stalled) return R / 5;
        if (this._loadRate === 0) return R * 10;
        return R / this._loadRate;
    }
    ...
    getClickablePartCenter(partId) {
        ...
        const p = rel[partId];
        if (!p) return null;
        // 用绝对变换换算（计入组件自身位移/缩放/旋转），否则缩放后箭头会指偏
        const pt = this.group.getAbsoluteTransform().point({ x: p.x, y: p.y });
    ...
    /**
     * 供钳形电流表读取"穿心导线"的瞬时电流（A）。
     * 三相导线电流即各相对地注入电阻中的电流 I = (V_phase − V_pe) / R。
     * @param {string} portId 端口 id，如 `m-3d_wire_u`
     * @returns {number|null}
     */
    getWireCurrentInstant(portId) {
        const m = /_wire_(u|v|w)$/.exec(portId);
        if (!m || !this._phaseCurrent) return null;
        return this._phaseCurrent[m[1]];
    }
    ...
    _drawStaticParts() {
        ...
        this._drawLabel();
        this._drawSpeedPanel();
    }

    /** 顶部转速显示面板（静态框体，数字由动态层刷新） */
    _drawSpeedPanel() {
        const p = this._speedPanel, W = this.width;
        this._staticGroup.add(new Konva.Rect({
            x: p.x, y: p.y, width: p.w, height: p.h, cornerRadius: 5,
            fillLinearGradientStartPoint: { x: 0, y: p.y },
            fillLinearGradientEndPoint: { x: 0, y: p.y + p.h },
            fillLinearGradientColorStops: [0, '#12331b', 0.5, '#0b2412', 1, '#071a0d'],
            stroke: '#2f6b3a', strokeWidth: 1.4, listening: false,
        }));
        this._staticGroup.add(new Konva.Text({
            x: p.x + 8, y: p.y + 3, width: p.w * 0.5,
            text: '转速', fontSize: Math.max(10, W * 0.044), fontStyle: 'bold',
            fontFamily: 'Arial, sans-serif', fill: '#7fce90', listening: false,
        }));
        this._staticGroup.add(new Konva.Text({
            x: p.x + p.w * 0.5, y: p.y + p.h - Math.max(15, W * 0.058),
            width: p.w * 0.5 - 8, text: 'r/min',
            fontSize: Math.max(11, W * 0.052), fontStyle: 'bold',
            fontFamily: 'Arial, sans-serif', fill: '#7fce90', align: 'right', listening: false,
        }));
    }
    ...
    _drawLabel() {
        ...
        s.add(new Konva.Text({
            x, y,
            text: `${this.ratedPower}kW  ${this.ratedVoltage}V  ${this.ratedCurrent}A  ${this.ratedSpeed}rpm`,
    ...
    _createDynamicNodes() {
        ...
        this._createShaft();
        this._createStatus();
        this._createRotationIndicators();
    ...
    _createStatus() {
        ...
        this._speedText = new Konva.Text({
            x: p.x + 6, y: p.y + p.h * 0.26, width: p.w - 12, height: p.h * 0.72,
            text: '0', fontSize: Math.max(20, W * 0.118), fontStyle: 'bold',
            fontFamily: 'Consolas, monospace', fill: '#39e06a',
    ...
    /**
     * 旋转方向标志：围绕右端盖（转动部分）画一段顺时针弧 + 一段逆时针弧，
     * 各带箭头指示转向，并随变频器输出相序（_phaseSeq）激活/熄灭。
     */
    _createRotationIndicators() {
        const cx = this._bellCX, cy = this._bellCY, r = this._bellR + 12;
        // 顺时针（正转）：线/字绿色
        this._cwMark = this._makeRotArrow(cx, cy, r, -72, 22, true, '正转', '#22c55e', '#2f5a37');
        // 逆时针（反转）：线/字红色
        this._ccwMark = this._makeRotArrow(cx, cy, r, 252, 158, false, '反转', '#ef4444', '#5a2b2b');
    }

    /**
     * 生成一段带箭头的旋转弧标志。
     * @param {number} cx,cy 圆心；r 半径
     * @param {number} aStart,aEnd 起止角（度，顺时针为正）
     * @param {boolean} cw 箭头指向是否为顺时针
     * @param {string} label 文字标签（正转/反转）
     * @param {string} colOn 激活颜色；colOff 熄灭颜色
     */
    _makeRotArrow(cx, cy, r, aStart, aEnd, cw, label, colOn, colOff) {
        const D2R = Math.PI / 180;
        const lo = Math.min(aStart, aEnd), hi = Math.max(aStart, aEnd);
        const group = new Konva.Group({ x: cx, y: cy, listening: false });

        const arc = new Konva.Shape({
            sceneFunc: (ctx, shape) => {
                ctx.beginPath();
                ctx.arc(0, 0, r, lo * D2R, hi * D2R, false);
                ctx.strokeShape(shape);
            },
            stroke: colOff, strokeWidth: 5.5, lineCap: 'round', listening: false,
        });
        group.add(arc);

        // 箭头（切向）：顺时针在 hi 端、逆时针在 lo 端
        const a = (cw ? hi : lo) * D2R;
        const px = r * Math.cos(a), py = r * Math.sin(a);
        const dx = cw ? -Math.sin(a) : Math.sin(a);
        const dy = cw ? Math.cos(a) : -Math.cos(a);
        const nx = -dy, ny = dx, L = 15, Wd = 11;
        const head = new Konva.Line({
            points: [px + dx * L, py + dy * L, px + nx * Wd / 2, py + ny * Wd / 2, px - nx * Wd / 2, py - ny * Wd / 2],
            closed: true, fill: colOff, listening: false,
        });
        group.add(head);

        // 标签：置于弧的外侧中部
        const mid = ((lo + hi) / 2) * D2R;
        const lr = r + 20;
        const txt = new Konva.Text({
            x: lr * Math.cos(mid) - 28, y: lr * Math.sin(mid) - 12, width: 56,
            text: label, fontSize: Math.max(13, this.width * 0.06), fontStyle: 'bold',
            fontFamily: 'Arial, sans-serif', fill: colOff, align: 'center', listening: false,
        });
        group.add(txt);

        this._dynamicGroup.add(group);
        return { group, arc, head, txt, colOn, colOff };
    }

    /** 按变频器输出相序激活/熄灭转向标志（正转绿、反转红） */
    _updateRotationIndicators() {
        const fwd = this._powered && this._phaseSeq > 0;
        const rev = this._powered && this._phaseSeq < 0;
        if (this._cwMark) {
            const c = fwd ? this._cwMark.colOn : this._cwMark.colOff;
            this._cwMark.arc.stroke(c); this._cwMark.head.fill(c); this._cwMark.txt.fill(c);
        }
        if (this._ccwMark) {
            const c = rev ? this._ccwMark.colOn : this._ccwMark.colOff;
            this._ccwMark.arc.stroke(c); this._ccwMark.head.fill(c); this._ccwMark.txt.fill(c);
        }
    }
    ...
    _updateDynamic(dt) {
        ...
        // 各相对地（PE 星点）电流：I = (V_phase − V_pe) / R，供钳形电流表读取
        const Vpe = getV('pe');
        const Rg = Math.max(0.001, this._phaseGroundR);
        this._phaseCurrent = {
            u: (Vu - Vpe) / Rg,
            v: (Vv - Vpe) / Rg,
            w: (Vw - Vpe) / Rg,
        ...
        const lineInst = Math.max(Vuv, Vvw, Vwu);

        // 线电压峰值保持（工频直供时避免过零瞬间误判通断）
        this._linePeak = Math.max(lineInst, (this._linePeak || 0) * 0.85);

        // 供电判定：
        //   接变频器时直接依据变频器运行状态 —— 低频时 V/f 输出幅值很低，
        //   若仍用瞬时线电压阈值判断，会在波形过零处频繁判定“失电”，
        //   导致转子“一转一停”；故此处不再用电压阈值。
        //   无变频器（工频直供）时才用线电压峰值判断。
        const drive = this._findSupplyDrive();
        const powered = drive ? !!drive._outputActive : (this._linePeak > 25);

        // 相序检测：由定子电压空间矢量角的旋转方向判定正/反转
        //   正转 → _phaseSeq = +1（转子顺时针旋转，面板 CW 标志点亮）
        //   反转 → _phaseSeq = -1（转子逆时针旋转，面板 CCW 标志点亮）
        if (powered) {
            const alpha = (2 / 3) * (Vu - 0.5 * Vv - 0.5 * Vw);
            const beta  = (Vv - Vw) / Math.sqrt(3);
            if (Math.hypot(alpha, beta) > 1e-6) {
                const ang = Math.atan2(beta, alpha);
                if (this._lastAng !== undefined) {
                    let d = ang - this._lastAng;
                    if (d > Math.PI) d -= 2 * Math.PI;
                    else if (d < -Math.PI) d += 2 * Math.PI;
                    if (Math.abs(d) > 1e-6) this._phaseSeq = d > 0 ? -1 : 1;
                }
                this._lastAng = ang;
            }
        } else {
            this._phaseSeq = 0;
            this._lastAng = undefined;
        }

        this._powered = powered;

        // 堵转时转子不转（即使三相已通电）
        const rotating = powered && !this._stalled;

        // 读取所接变频器输出频率，使转速与之对应：n = 额定转速 × (f / 额定频率)
        this._supplyFreq = (drive && powered) ? Math.abs(drive._fOut || 0) : null;

        let rpm = 0;
        if (rotating) {
            if (this._supplyFreq !== null) {
                rpm = Math.round(this.ratedSpeed * (this._supplyFreq / (this.ratedFreq || 50)));
            } else {
                // 无变频器（工频直接供电）时按线电压幅值估算
        ...
        // 转子/风扇旋转角（快速旋转：通电且非堵转时高速）
        const visualOmega = rotating ? Math.min(this.ratedSpeed, 3000) * 2 * Math.PI / 60 : 0;
        if (rotating) {
        ...
        // 转速显示（堵转时显示"堵转"）
        if (this._stalled && powered) {
            this._speedText.text('堵转');
            this._speedText.fill('#e04030');
        } else {
            this._speedText.text(String(rpm));
            this._speedText.fill(powered ? '#39e06a' : '#2f7a48');
        }

        // 转向标志随相序激活
        this._updateRotationIndicators();

        // 活动黄绿导线随组件移动自动重绘
        this._updateDynamicWire();
    }

    /** 查找与电动机 U/V/W 同节点的变频器（用于获取输出频率/转向） */
    _findSupplyDrive() {
        const solver = this.sys && this.sys.voltageSolver;
        if (!solver || !solver.portToCluster || !this.sys.comps) return null;
        const myU = solver.portToCluster.get(`${this.id}_wire_u`);
        if (myU === undefined) return null;
        for (const c of Object.values(this.sys.comps)) {
            if (!c || c.type !== 'v20_inverter') continue;
            const du = solver.portToCluster.get(`${c.id}_wire_u`);
            if (du !== undefined && du === myU) return c;
        }
        return null;
    }
    ...
    getConfigFields() {
        return [
            ...
            { label: '额定电流 (A)（由功率推算，只读）', key: 'ratedCurrent', type: 'number', disabled: true, get: c => c.ratedCurrent },
            { label: '额定转速 (r/min)', key: 'ratedSpeed',   type: 'number' },
            { label: '功率因数',         key: 'cosphi',       type: 'number' },
            { label: '状态', key: 'status', type: 'select', options: [
                { value: 'normal', label: '正常' },
                { value: 'stall',  label: '堵转' },
            ] },
            { label: '负荷率', key: 'loadRate', type: 'select',
              // 堵转时不可设置负荷率
              disabled: (comp, vals) => vals.status === 'stall',
              options: [
                { value: '0',    label: '0' },
                { value: '0.25', label: '25%' },
                { value: '0.5',  label: '50%' },
    ...
    onConfigUpdate(cfg) {
        ...
        if (cfg.status       !== undefined) this._stalled     = cfg.status === 'stall';
        // 额定功率/电压变化 → 重新推算注入电阻（未显式指定时）与额定电流
        if (!this._r100Explicit) {
            this._R100 = this.ratedVoltage * this.ratedVoltage / (this.ratedPower * 1000);
        }
        this.ratedCurrent = +(this.ratedPower * 1000 / (Math.sqrt(3) * this.ratedVoltage)).toFixed(2);
        // 仅正常状态可修改负荷率
        if (!this._stalled && cfg.loadRate !== undefined) {
            const lr = parseFloat(cfg.loadRate);
            if ([0, 0.25, 0.5, 0.75, 1, 1.25].includes(lr)) this._loadRate = lr;
        }
        this._phaseGroundR = this._computePhaseGroundR();
