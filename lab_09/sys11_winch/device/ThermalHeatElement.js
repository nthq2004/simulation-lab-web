 ...
 *    → tick() 短窗口滑动 RMS 采集三相电流 → deviceRef.setCurrent(max)
...
export class ThermalHeatElement extends BaseComponent {
    ...
    _initParameters(config) {
        ...
        // 双金属片最小动作时间（s）：热惯性的体现。电流再大也不会快于此时间脱扣，
        // 保证短路电流由上级断路器先切断时热继电器来不及动作（见 ThermalRelayDevice.preUpdate）。
    ...
    showContextMenu(evt) {
        ...
        if (dev) {
            if (dev.isTripped()) {
                menu.appendChild(createItem('复位', () => {
                    if (dev.getManualTrip()) {
                        dev.setManualTrip(false);
                        dev.requestReset();
                    } else {
                        dev.requestReset();
                    }
                ...
                menu.appendChild(createItem('模拟脱扣', () => {
                    dev.setManualTrip(!dev.getManualTrip());
                }));
            }
        }
    ...
    _sampleCurrents() {
        ...
        // 电流反而偏小。实际单相运行时健康两相电流约为额定的 1.7~2.3 倍，此处按
        // 2.2×额定 计，使热继电器能按"过载"正确动作（而非依赖电机堵转电流）。
        const hasSinglePhasingMotor = this._hasSinglePhasingMotor();
        if (hasSinglePhasingMotor) {
            maxI = Math.max(maxI, this.ratedCurrent * 2.2);
        }
    ...
    /** 系统中是否存在"运行中缺相（单相运行）"的电机 */
    _hasSinglePhasingMotor() {
        const comps = this.sys?.comps;
        if (!comps) return false;
        for (const k in comps) {
            const c = comps[k];
            if (c && c.type === 'induction_motor' && c._phaseLost && Math.abs(c._omega_m) > 5) {
                return true;
            }
        }
        return false;
