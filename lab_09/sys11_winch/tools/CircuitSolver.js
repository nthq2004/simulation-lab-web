...
export class CircuitSolver {
    ...
    _buildDeviceCache() {
        ...
        return {
            ...
            potDevs:          raw.filter(d => d.type === 'potentiometer'),
            svDevs:           raw.filter(d => d.type === 'solenoid_valve'),
            load3Devs:        raw.filter(d => d.type === 'load_3p'),
            testerDevs:       raw.filter(d => d.type === 'breaker_tester'),
            ...
            ptermDevs:        raw.filter(d => d.type === 'pterm'),
            ...
            motor3dDevs:      raw.filter(d => d.type === 'motor_3d'),
            ...
            syncroscopeDevs:  raw.filter(d => d.type === 'syncroscope' || d.type === 'lampRotationSync'),
            ...
            pdbDevs:          raw.filter(d => d.type === 'PDB'),
            shorePowerBoxDevs: raw.filter(d => d.type === 'ShorePowerBox'),
            negSeqRelayDevs: raw.filter(d => d.type === 'NegSeqRelay'),
            tripRelayDevs: raw.filter(d => d.type === 'tripRelay'),
            genRemotePanelDevs: raw.filter(d => d.type === 'gen_remote_panel'),
            emergencyPanelDevs: raw.filter(d => d.type === 'emergency_panel'),
            ...
            v20Devs:       raw.filter(d => d.type === 'v20_inverter'),
    ...
    _buildTopology() {
        ...
        const faultShorts = this.sys._faultShortGroups || [];
        const result = this._topology.build(this.rawDevices, this.connections, poorSet, faultShorts);
        ...
        for (const d of this.rawDevices) {
            switch (d.type) {
                case 'source_3p':
                    ...
                    break;
                case 'breaker_tester':
                    if (d._primaryOutput) { seed(`${d.id}_wire_out1`); seed(`${d.id}_wire_out2`); seed(`${d.id}_wire_out3`); }
                    if (d._secondaryOutput) { seed(`${d.id}_wire_iout1`); seed(`${d.id}_wire_vout1`); }
                    break;
                case 'motor_control_box':
                    if (d._qfState === 'on') { seed(`${d.id}_wire_in1`); seed(`${d.id}_wire_in2`); seed(`${d.id}_wire_in3`); }
                    break;
                case 'v20_inverter':
                    if (d.isOn) { seed(`${d.id}_wire_u`); seed(`${d.id}_wire_v`); seed(`${d.id}_wire_w`); }
                    break;
    ...
    _invalidateCacheIfNeeded() {
        try {
            ...
            for (const d of this.rawDevices) {
                ...
                if (d.type === 'potentiometer') resistSigs.push(`${d.id}:pos=${d.position}:R=${d.totalResistance}`);
                if (d.type === 'load_3p') {
                    resistSigs.push(`${d.id}:load=${d._loaded || false}:conn=${d.connection || 'star'}:P=${d._Pph || 0}:L=${d._Lph || 0}:C=${d._Cph || 0}:Rd=${d._Rd || 0}:Ld=${d._Ld || 0}:Cd=${d._Cd || 0}`);
                }
                ...
                if (d.type === 'switch' || (d.type === 'relay' && d.special !== 'voltage' && d.special !== 'time' && d.special !== 'pressrelay')) {
                ...
                if (d.type === 'relay' && d.special === 'pressrelay') {
                    resistSigs.push(`${d.id}:on=${d.isEnergized}`);
                }
                if (d.type === 'relay' && d.special === 'voltage') {
                ...
                if (d.type === 'ContactorDevice') {
                    const fPoles = d._faultOpenPoles
                        ? Object.entries(d._faultOpenPoles).filter(([, v]) => v).map(([k]) => k).sort().join(',')
                        : '';
                    resistSigs.push(`${d.id}:sp=${d.special}:v=${d.getValue()}:coilOpen=${!!d._faultCoilOpen}:noOpen=${!!d._faultOpen}:noEnd=${d._faultOpenEnd || ''}:poles=${fPoles}`);
                }
                ...
                if (d.type === 'PDB') {
                    ...
                    resistSigs.push(`${d.id}:s1=${d.getSwState(0)}:s2=${d.getSwState(1)}:s3=${d.getSwState(2)}:nc=${ncSt}`);
                }
                if (d.type === 'ShorePowerBox') {
                    resistSigs.push(`${d.id}:knob=${d._knob}:br=${d._breakerOn ? 1 : 0}`);
                }
                if (d.type === 'NegSeqRelay') {
                    resistSigs.push(`${d.id}:nc=${typeof d.isNCClosed === 'function' && d.isNCClosed() ? 1 : 0}`);
                }
                if (d.type === 'PUSHBUTTON') {
                    resistSigs.push(`${d.id}:pressed=${!!d._isPressed}:man=${!!d._manualPressed}:faultOpen=${!!d._faultOpen}`);
                }
                if (d.type === 'ups') {
                    resistSigs.push(`${d.id}:o1=${d.isOutputActive('out1') ? 1 : 0}:o2=${d.isOutputActive('out2') ? 1 : 0}:v=${d.getOutputVoltageRms() || 0}`);
                }
                if (d.type === 'tripRelay') {
            ...
            resistSigs.sort();
            const poorPorts = this.sys?._poorContactPorts;
            const poorSig = poorPorts && poorPorts.size ? [...poorPorts].sort().join(',') : '';
            const faultShortGroups = this.sys?._faultShortGroups;
            const faultSig = faultShortGroups && faultShortGroups.length
                ? faultShortGroups.map(g => g.join('+')).sort().join(',')
                : '';
            const sig = connKeys.join('|') + '|' + poorSig + '|' + faultSig + '|' + resistSigs.join('|');
    ...
    _solve() {
        ...
        for (const src of power3Devs) {
            if (src.isOn) { this._systemFreq = (src._freqOut ?? src.freq) || 50; break; }
        }
        ...
        if (this._systemFreq === 50) {
            for (const src of v20Devs) {
                const f = Math.abs(src._fOut || 0);
                if (src.isOn && f > 0.5) { this._systemFreq = f; break; }
            }
        }
        ...
        // ---电机控制箱内部中性点 n：未接线时钳位到地（三相电源注入参考）---
        mcBoxDevs.forEach(p => {
            const nIdx = this.portToCluster.get(`${p.id}_wire_n`);
            if (nIdx === undefined) return;
            const nPort = `${p.id}_wire_n`;
        ...
        // ── 电子脱扣器方程数：I+/I- 电流采样 0V 电压源 + t1/t2 脱扣输出电压源
        //    + 右侧两路分励脱扣输出电压源（f1a/f1b、f2a/f2b，接线才计数）──
        let tripRelayEqCount = 0;
        tripRelayDevs.forEach(dev => {
            const cIp = this.portToCluster.get(`${dev.id}_wire_i+`);
            const cIm = this.portToCluster.get(`${dev.id}_wire_i-`);
            if (cIp !== undefined && cIm !== undefined) tripRelayEqCount++;
            const cT1 = this.portToCluster.get(`${dev.id}_wire_t1`);
            const cT2 = this.portToCluster.get(`${dev.id}_wire_t2`);
            if (cT1 !== undefined && cT2 !== undefined) tripRelayEqCount++;
            [['f1a', 'f1b'], ['f2a', 'f2b']].forEach(([a, b]) => {
                if (this.portToCluster.has(`${dev.id}_wire_${a}`) && this.portToCluster.has(`${dev.id}_wire_${b}`))
                    tripRelayEqCount++;
            });
        });

        // ── 发电机组遥控面板方程数：调速/合闸/分闸 3 个电压源（端口接线才计数）──
        let genRemotePanelEqCount = 0;
        genRemotePanelDevs.forEach(dev => {
            [['spd_p', 'spd_n'], ['close_a', 'close_b'], ['open_a', 'open_b']].forEach(([a, b]) => {
                if (this.portToCluster.has(`${dev.id}_wire_${a}`) && this.portToCluster.has(`${dev.id}_wire_${b}`))
                    genRemotePanelEqCount++;
            });
        });

        // ── 应急配电板方程数：联络合/分、应发合/分、调频 5 个电压源（端口接线才计数）──
        let emergencyPanelEqCount = 0;
        emergencyPanelDevs.forEach(dev => {
            [['tie_close_a', 'tie_close_b'], ['tie_open_a', 'tie_open_b'], ['eqf_close_a', 'eqf_close_b'], ['eqf_open_a', 'eqf_open_b'], ['egen_freq_p', 'egen_freq_n']].forEach(([a, b]) => {
                if (this.portToCluster.has(`${dev.id}_wire_${a}`) && this.portToCluster.has(`${dev.id}_wire_${b}`))
                    emergencyPanelEqCount++;
            });
        });
...
// ── UPS 输出方程数（每个接线的输出通道一个电压源方程，索引计数与 stampUPSs 严格一致）──
let upsEqCount = 0;
upsDevs.forEach(dev => {
    ['out1', 'out2'].forEach(ch => {
        if (this.portToCluster.has(`${dev.id}_wire_${ch}_p`) && this.portToCluster.has(`${dev.id}_wire_${ch}_n`))
            upsEqCount++;
    });
});
...
const totalSize = nodeVarCount + pidEqCount + opAmps.length + oscDevs.length + lvdtDevs.length + aiEqCount + pcEqCount + dcEqCount + aoEqCount + doEqCount + acAmmEqCount + wattmeterEqCount + tripRelayEqCount + genRemotePanelEqCount + emergencyPanelEqCount + ctEqCount + ptEqCount + ammEqCount + mmResEqCount + ctVcvsEqCount + mwEqCount + imEqCount + rlEqCount + upsEqCount + transformerEqCount;
        ...
        const frameDt = this._frameDt || (1 / 20);

        // ── 预求解：更新感应电机机械状态（每帧积分满一个真实帧时间） ──
        // 电机机械时间常数约 10ms，一帧 50ms 若只积分一次会数值发散（转速冲过同步速）。
        // 故按 2ms 子步细分积分，使电磁转矩能随转速逐子步更新；
        // 每帧必须积分满 frameDt(1/20s)，否则机械动态会被放慢、起动过程长达数十秒。
        const MOTOR_STEP = 0.002;
        (inductionMotorDevs || []).forEach(dev => {
            let remaining = frameDt;
            while (remaining > 1e-9) {
                const step = Math.min(MOTOR_STEP, remaining);
                dev._preSolve(step);
                remaining -= step;
            }
        });

        // ── 预求解：控制变压器直流瞬态状态机（保证本帧 stamp 使用最新状态）──
        // 使用真实帧时间（1/20 s），而非求解步长 deltaTime（0.1ms），
        // 否则瞬态保持时长会被放慢 500 倍，读数刷新严重滞后。
        (this._deviceCache.controlTransformerDevs || []).forEach(dev => {
            if (dev && typeof dev._preSolve === 'function') dev._preSolve(frameDt);
        });
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            // ─ 4a. UPS 不间断电源（输出端电压源方程注入） ────────────
            const upsVIdx = totalSize - upsEqCount;
            DeviceStamps.stampUPSs(ctx, G, B, upsDevs, upsVIdx, currentTime);
            // ─ 4b. 镍氢电池（诺顿等效） ──────────────────────────────────
            ...
            // ─ 5-2. SINAMICS V20 变频器三相输出 + 10V/24V 辅助电源 ─────────
            if (v20Devs.length) {
                DeviceStamps.stampPower3Sources(ctx, G, B, v20Devs, currentTime);
                DeviceStamps.stampV20Aux(ctx, G, B, v20Devs);
            }
            // ─ 5c. 断路器保护特性测试仪（一次大电流 / 二次模拟电流 / 可调线电压） ──
            DeviceStamps.stampBreakerTesters(ctx, G, B, testerDevs, currentTime);
            // ─ 5a. 电机控制箱（QF 供电开关控制的三相电源注入） ─────────
            DeviceStamps.stampMotorControlBoxes(ctx, G, B, mcBoxDevs, currentTime);
            ...
            // ─ 15i2 低压三相配电箱（3 开关接触电阻 + 分励线圈） ────────
            DeviceStamps.stampPDBs(ctx, G, B, pdbDevs);
            // ─ 15i2b 岸电箱（相序开关 + 出口断路器，接触电阻注入） ──────
            DeviceStamps.stampShorePowerBoxes(ctx, G, B, shorePowerBoxDevs);
            // ─ 15i2c 负序继电器（常闭触点注入） ──────────────────
            DeviceStamps.stampNegSeqRelays(ctx, G, B, negSeqRelayDevs);
            // ─ 15j 主触头（3极电阻注入） ────────────────────────────
            ...
            const rlVIdx = totalSize - rlEqCount - upsEqCount;
            ...
            // ── 电位端子注入（诺顿等效，无额外方程）─
            DeviceStamps.stampPotentialTerminals(ctx, G, B, ptermDevs);
            // ─ 1b2. 电磁阀（线圈电阻 + 迟滞吸合）─
            DeviceStamps.stampSolenoidValves(ctx, G, B, svDevs, results);
            // ── 充放电板注入（每路输出一个电压源方程）
            DeviceStamps.stampChargeBoards(ctx, G, B, chargeBoardDevs);
            // ── 三相异步电动机对地绝缘电阻 ──
            DeviceStamps.stampMotors(ctx, G, B, motorDevs);
            // ── 三相电机 3D：按状态/负荷率对 U/V/W 注入对地电阻 ──
            DeviceStamps.stampMotors3D(ctx, G, B, motor3dDevs);
            // ─ 19 AO 模块 ────────────────────────────────────────────────
            ...
            // - 23b 应急配电板（联络/应发合分、调频电压源）
            const emergencyPanelVIdx = adjustedSize - emergencyPanelEqCount - tripRelayEqCount - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampEmergencyPanels(ctx, G, B, emergencyPanelDevs, emergencyPanelVIdx);

            // - 23 电子脱扣器电流采样（0V 电压源注入，索引最前）
            const tripVIdx = adjustedSize - tripRelayEqCount - acAmmEqCount - wattmeterEqCount - ctEqCount - ptEqCount - ammEqCount - ctVcvsEqCount - mwEqCount;
            DeviceStamps.stampTripRelays(ctx, G, B, tripRelayDevs, tripVIdx);
...
inductionMotorDevs, contactorDevs, thermalRelayDevs, rlSeriesDevs,
        ...
        // ─ 1a. 熔断器电流（对应 stampFuses，R=0.01Ω）：供熔断器过流自熔断判别
        fuseDevs.forEach(dev => {
            const vL = getV(dev.id, 'l') ?? 0;
            const vT = getV(dev.id, 't') ?? 0;
            const R = dev.getState && dev.getState() === 'ok' ? 0.01 : 1e9;
            dev.physCurrent = (vL - vT) / R;
        });
        ...
        igbtDevs.forEach(dev => {
            ...
            // 叠加 C-E 体二极管（PN 结）电流：正向由 E→C，折算为 C→E 为负
            if (!dev._faultCEOpen) {
                const vF = dev.vBodyDiode ?? 0.6;
                let iD; // E→C 方向
                if (dev._bodyDiodeOn) {
                    iD = ((vE - vC) - vF) / Math.max(0.001, dev.rOnDisp || 50);
                } else {
                    iD = (vE - vC) / Math.max(0.001, dev.rOffBody || 1e9);
                }
                dev.physCurrent -= iD;
            }
        });
        ...
        // ─ 4a UPS 输出电流（对应 stampUPSs，从电压源支路电流直接读取）──
        upsDevs.forEach(dev => {
            ['out1', 'out2'].forEach(ch => {
                const idx = dev._upsVIdx ? dev._upsVIdx[ch] : -1;
                const iCh = (idx >= 0 && idx < results.length) ? (results[idx] || 0) : 0;
        ...
        // ─ 23.6b 电子脱扣器采样电流（对应 stampTripRelays，参照功率表）──
        const tripRelayDevs = devices.tripRelayDevs || [];
        tripRelayDevs.forEach(dev => {
            if (dev.currentIdx !== undefined) dev.physCurrent = results[dev.currentIdx];
        });
        ...
        (inductionMotorDevs || []).forEach(dev => {
            ...
            // 从节点电压回算相电流
                ...
                dev._hasAllPhase = _hasAllPhaseV;
        ...
        (devices.acbDevs || []).forEach(dev => {
            ...
            if (vFla !== undefined && vFlb !== undefined) {
                ...
                // 船用主开关（MainsSwitch）由自身 _logic 检测分励电流跳闸，不走通用 isTripped 分支
                if (Math.abs(iCoil) > 0.01 && dev._state === 'on' && dev.special !== 'MainsSwitch' && !dev.isTripped()) {
                    dev.trip();
                }
            } else {
        ...
        // ─ 三相配电箱触头电流回读 + 分励脱扣 ─────────────────────
        (devices.pdbDevs || []).forEach(dev => {
            const R = 0.01;
            const pc = { sw1: {}, sw2: {}, sw3: {} };
            for (let s = 1; s <= 3; s++) {
                const sw = pc[`sw${s}`];
                // 关键：触头未闭合（OFF/TRIP）时不得回读电流。
                // 触头断开呈 1e9Ω，此时若仍用闭合电阻 0.01 计算 (vIn-vT)/0.01，
                // 母线带电而出线端悬空时会算出数万安虚假电流，污染 RMS 窗口，
                // 导致"先送电再合闸"时合闸瞬间误判短路脱扣。
                const closed = dev.getSwState(s - 1) === 'on';
                for (let ph = 1; ph <= 3; ph++) {
                    const vIn = getV(dev.id, `in${ph}`);
                    const vT  = getV(dev.id, `sw${s}_t${ph}`);
                    sw[`l${ph}`] = (closed && vIn !== undefined && vT !== undefined) ? (vIn - vT) / R : 0;
                }
                // 分励脱扣器线圈电流检测（每开关独立）
                const vFla = getV(dev.id, `sw${s}_fla`);
                const vFlb = getV(dev.id, `sw${s}_flb`);
                if (vFla !== undefined && vFlb !== undefined) {
                    const Rcoil = dev.getTripCoilR ? dev.getTripCoilR() : 200;
                    const iCoil = (vFla - vFlb) / Rcoil;
                    dev[`_shuntTripCurrent${s}`] = iCoil;
                    if (Math.abs(iCoil) > 0.01 && dev.getSwState(s - 1) === 'on' && !dev.isTripped(s - 1)) {
                        dev.tripSwitch(s - 1);
                    }
                } else {
                    dev[`_shuntTripCurrent${s}`] = 0;
                }
            }
            dev.phaseCurrents = pc;
        });
    }
    /**
     * PID 模块通道详细更新逻辑
