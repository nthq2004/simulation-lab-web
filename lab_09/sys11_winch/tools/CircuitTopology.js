...
export class CircuitTopology {
    ...
    build(rawDevices, connections, poorContactPorts = new Set(), faultShortGroups = []) {
        ...
        rawDevices.forEach(dev => {
            ...
            // 压力继电器：励磁（压力低于下限）时 NC-COM 闭合（压缩机控制触点）；
            // 释放（压力高于上限）时 NO-COM 闭合。用于压力开关控制压缩机启停。
            if (dev.type === 'relay' && dev.special === 'pressrelay') {
                if (dev.isEnergized) internalUnion(`${id}_wire_NC`, `${id}_wire_COM`);
                else internalUnion(`${id}_wire_NO`, `${id}_wire_COM`);
            }
            ...
            // 停止按钮（NC常闭）：未按下（含模拟按下 _manualPressed）且未故障时短接
            if (dev.type === 'PUSHBUTTON' && dev.special === 'STOP-BTN'
                && !dev._isPressed && !dev._manualPressed && !dev._faultOpen)
