...
export const DeviceStamps = {
    ...
    // ─── 1b0. 三端电位器（分压器：l–w 与 w–r 两段电阻）──────────────
    stampPotentiometers(ctx, G, B, devs) {
        devs.forEach(dev => {
            const cL = ctx.portToCluster.get(`${dev.id}_wire_l`);
            const cW = ctx.portToCluster.get(`${dev.id}_wire_w`);
            const cR = ctx.portToCluster.get(`${dev.id}_wire_r`);
            const { rUp, rDown } = (typeof dev.getResistances === 'function')
                ? dev.getResistances()
                : { rUp: 1e9, rDown: 1e9 };
            if (cL !== undefined && cW !== undefined) this._fill(ctx, G, B, cL, cW, 1 / Math.max(rUp, 0.001));
            if (cW !== undefined && cR !== undefined) this._fill(ctx, G, B, cW, cR, 1 / Math.max(rDown, 0.001));
        });
    },
    ...
    stampLoad3p(ctx, G, B, devs, deltaTime) {
        devs.forEach(dev => {
            // ── 角形（Δ）联接：无中性点，每个元件跨线电压 ──
            if (dev.connection === 'delta') {
    ...
    stampV20Aux(ctx, G, B, devs) {
        devs.forEach(dev => {
            if (!dev._powered) return;   // 主电源失电时内部 10V/24V 无输出
    ...
    stampContactCoils(ctx, G, B, devs) {
        devs.forEach(dev => {
            const R = dev._faultCoilOpen ? 1e12 : 1000;
