...
export class ControlSystem {
    ...
    init() {
        ...
        const componentConfigs = [
            { Class: WaterBath, id: 'temptest', x: 50, y: 665 },
            { Class: DCPower, id: 'dcpower', x: 50, y: 65, voltage: 5 },
            { Class: Resistor, id: 'r1', x: 180, y: 300, direction: 'vertical' },
            { Class: Resistor, id: 'r2', x: 300, y: 300, direction: 'vertical' },

            { Class: RealPT100, id: 'pt100', x: 220, y: 620 },
            { Class: VariResistor, id: 'pt', value: 385.1, cvalue: 100, x: 80, y: 450, direction: 'vertical' },
            { Class: VariResistor, id: 'varires', value: 200, x: 320, y: 450, direction: 'vertical' },
            { Class: Ground, id: 'gnd0', x: 250, y: 665 },
            { Class: Resistor, id: 'r5k', x: 450, y: 480, value: 5000, direction: 'vertical' },
            { Class: VariResistor, id: 'r10kv', x: 460, y: 250, value: 10000, direction: 'vertical' },
            { Class: OpAmp, id: 'amp1', x: 580, y: 200 },
            { Class: OpAmp, id: 'amp2', x: 580, y: 600 },
            { Class: Resistor, id: 'r106k1', x: 600, y: 320, value: 101500 },
            { Class: Resistor, id: 'r106k2', x: 600, y: 450, value: 101500 },
            ...
            { Class: VariResistor, id: 'r1k', x: 750, y: 700, value: 2000, direction: 'vertical' },
            { Class: Ground, id: 'gnd', x: 740, y: 890 },
            { Class: OpAmp, id: 'amp4', x: 850, y: 650 },
            ...
            { Class: DCPower, id: 'dcpower2', x: 1650, y: 130 },
            ...
            { Class: Multimeter, id: 'multimeter', x: 980, y: 530 },
            { Class: Multimeter, id: 'multimeter2', x: 300, y: 630 },
        ];
    ...
    initSteps() {
        ...
        const projectConfigs = [
            { id: 0, name: "1. 温度变送器功能的验证" },
            // { id: 1, name: "2. PT100测温电桥的测试" },
        ...
        const conns = [
            // --- 1. 桥式电路部分 (电桥测量) ---
            // 电源正极驱动电桥顶端 (R1, R2 上端)
            { from: 'dcpower_wire_p', to: 'r1_wire_l', type: 'wire' },
            { from: 'r1_wire_l', to: 'r2_wire_l', type: 'wire' },
            // 电桥下端 (PT100, VariRes 下端) 接地形成回路
            { from: 'pt_wire_r', to: 'gnd0_wire_gnd', type: 'wire' },
            { from: 'varires_wire_r', to: 'gnd0_wire_gnd', type: 'wire' },
            // 桥臂连接：左臂 R1-PT100，右臂 R2-VariRes
            { from: 'r1_wire_r', to: 'pt_wire_l', type: 'wire' },
            { from: 'r2_wire_r', to: 'varires_wire_l', type: 'wire' },

            // --- 2. 仪表放大器部分 (三运放结构：amp1, amp2 为输入级，amp3 为差分级) ---
            // 输入级：将电桥左右中点信号接入 amp1 和 amp2 的同相输入端
            { from: 'r2_wire_r', to: 'amp1_wire_p', type: 'wire' }, // 右桥压 -> amp1
            { from: 'r1_wire_r', to: 'amp2_wire_p', type: 'wire' }, // 左桥压 -> amp2
        ...
        this.stepsArray[0] = [
            // 1. 电桥供电基础回路
            {
                ...
                act: async () => {
                    ...
                    this.showComp.showTooltip('pt', '1. PT100测温电桥', { color: '#2ecc71' });
                    await new Promise(r => setTimeout(r, 3000));
                },
                check: () => {
                    const required = [
                        { from: 'dcpower_wire_p', to: 'r1_wire_l' },
                        { from: 'r1_wire_l', to: 'r2_wire_l' },
                        { from: 'pt_wire_r', to: 'gnd0_wire_gnd' },
                        { from: 'varires_wire_r', to: 'gnd0_wire_gnd' },
                        { from: 'r1_wire_r', to: 'pt_wire_l' },
                        { from: 'r2_wire_r', to: 'varires_wire_l' }
                    ];
            ...
            // 2. 仪表放大器前级输入与增益网络
            {
                ...
                check: () => {
                    const required = [
                        { from: 'r1_wire_r', to: 'amp2_wire_p' },
                        { from: 'r2_wire_r', to: 'amp1_wire_p' },
            ...
            // 修正后的步骤 13
            {
                msg: "步骤 13：模拟 50℃ 环境。观察输出电流（预期 12mA）及 PID PV显示值（50）。",
                act: async () => {
                    // 1. 设置状态
                    this.comps.pt.currentResistance = 119.25;
                    this.comps.pt.update();

                    // 2. 显示反馈
                    this.showComp.showStatusRing('pt', '#f1c40f');
                    // 确保 ID 传递准确
                    this.showComp.removeTooltip('pt');
                    this.showComp.showTooltip('pt', '模拟 50 度 (Rt=119.25Ω)', { color: '#e82f0e' });
                ...
                check: () => Math.abs(this.comps.pt.currentResistance - 119.25) < 0.5
            },

            // 14. 100度性能验证
            {
                msg: "步骤 14：模拟 100℃ 环境。观察输出电流（预期 20mA）及 PID PV显示值(100)。",
                act: async () => {
                    await new Promise(r => setTimeout(r, 500));
                    this.comps.pt.currentResistance = 138.51;
                    this.comps.pt.update(); // 触发组件状态更新
                    this.showComp.showTooltip('pt', '模拟100度(Rt=138.51Ω)', { color: '#e82f0e' });
                    await new Promise(r => setTimeout(r, 4000));
                    this.showComp.clearAllTooltips(); // 演示结束，恢复视角
                },
                check: () => Math.abs(this.comps.pt.currentResistance - 138.51) < 0.5
            }
        ];
    ...
    initFault() {
        ...
        this.FAULT_CONFIG = {
            1: {
                ...
                check: () => { if (this.comps['valve2']) this.comps['valve2'].isLeaking === true; },
