...
export class PressSwitch extends BaseComponent {
    constructor(config, sys) {
        ...
        this.lowSet = 0.02*this.setPoint;
        this.highSet = this.lowSet+1+0.02*this.differential;
    ...
    initVisuals() {
        ...
        // 限位螺钉 (作用螺钉) - 位于杠杆下方
        this.limitBolt = new Konva.Rect({ x: -122.5, y: 0, width: 15, height: 39, fill: '#999', stroke: '#333' });
    ...
    update(p, sp, diff) {
        ...
        // 双位滞后逻辑
        this.lowSet = 0.02*this.setPoint;
        this.highSet = 1+0.02*this.differential+this.lowSet;
        this.config.lowSet = this.lowSet;
        this.config.highSet = this.highSet;
        ...
        const percent = Math.min(200,Math.max(0,p*50));
        const forceEffect = (percent - this.setPoint) * 0.06;
        const targetRot = Math.max(-20, Math.min(20, forceEffect));
