...
export class CircuitSolver {
    ...
    _solve() {
        ...
        const ptDevs = raw.filter(d => d.type === 'pressure_transducer');
        ...
        // 5. 统计额外的电压源方程数量 (PID 的 pi1 配电端 和 PWM 输出端,热电偶)
        let extraEqCount = 0;
        pidDevs.forEach(pid => {
        ...
        const totalSize = mSize + extraEqCount + opAmps.length + oscDevs.length + ptDevs.length+2*sgDevs; // 总方程数量 = 节点电压方程 + 额外电压源方程 + 运放模式方程 + 示波器约束方程 + 压力变送器方程+信号发生器*2。
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            sgDevs.forEach(sg => {
                ...
                const voltOutputs = sg.update(this.currentTime);
                ...
                [
                    ...
                    if (ch.enabled && portN !== undefined && portP !== undefined) {
                        ...
                        const Rs = 50;
                        const Gs = 1 / Rs; // 电导
                        const Vs = voltOutputs[chCfg.key]; // 理想电压 (包含偏置)
                    ...
                    dev.chCurrent[chCfg.idx] =sgVIdx++;
                });
            });
            // 注入 GMIN 防奇异
        ...
        pressDevs.forEach(dev => {
            const port1L = `${dev.id}_wire_r1l`;
            const port1R = `${dev.id}_wire_r1r`;
            const v1L = this.nodeVoltages.get(this.portToCluster.get(port1L)) || 0;
            const v1R = this.nodeVoltages.get(this.portToCluster.get(port1R)) || 0;
            // 规定一个标准方向：从 left 流向 right 为正
            dev.r1Current = (v1L - v1R) / dev.r1;
            const port2L = `${dev.id}_wire_r2l`;
            const port2R = `${dev.id}_wire_r2r`;
            const v2L = this.nodeVoltages.get(this.portToCluster.get(port2L)) || 0;
            const v2R = this.nodeVoltages.get(this.portToCluster.get(port2R)) || 0;
            // 规定一个标准方向：从 left 流向 right 为正
            dev.r2Current = (v2L - v2R) / dev.r2;
        });        
        ...
        // 10. 在循环结束后，提取信号发生器电流
        sgDevs.forEach(dev => {
            if (dev.chCurrent[0] !== undefined) {
                // 这里的电流是从 outP 流向 outN 的内部等效电流
                dev.ch1Current = results[dev.chCurrent[0]];
            }
            if (dev.chCurrent[1] !== undefined) {
                // 这里的电流是从 outP 流向 outN 的内部等效电流
                dev.ch2Current = results[dev.chCurrent[1]];
            } 
            console.log(dev.ch1Current,dev.ch2Current);           
        });
    ...
    */
    ...
    _getPhysicalFlowIntoPort(dev, extPort) {
        ...
        if (dev.type === 'press_sensor') {
            let returnCurrent;
            const r1Current = dev.r1Current || 0;
            const r2Current = dev.r2Current || 0;
            // 如果查询的是左端口，流入为正 (vL > vR 时 current 为正)
            // 如果查询的是右端口，流入为正 (vR > vL 时 current 为负，所以取反)
            if (extPort.endsWith('_r1l')) returnCurrent = -r1Current;
            else if (extPort.endsWith('_r1r')) returnCurrent = r1Current;
            else if (extPort.endsWith('_r2l')) returnCurrent = -r2Current;
            else if (extPort.endsWith('_r2r')) returnCurrent = r2Current;
        ...
        if (dev.type === 'njfet') {
            const current = dev.physCurrent || 0;
            // 如果没有计算过电流，返回 0
            if (extPort.endsWith('_g')) {
                return 0;
            }
            return extPort.endsWith('_d') ? -current : current;;
        }
        // ---情况8. 更新示波器电流
        if (dev.type === 'oscillicope' || dev.type === 'oscillicope_tri') {
            const current = dev.physCurrent || 0;
            // 如果没有计算过电流，返回 0
            if (extPort.endsWith('_p') || extPort.endsWith('_n')) {
                return 0;
        ...
        if (dev.type === 'pressure_transducer') {
            const current = dev.physCurrent || 0;
            // 如果没有计算过电流，返回 0
            if (extPort.endsWith('wire_p') || extPort.endsWith('wire_n')) {
                return 0;
            }
            return extPort.endsWith('_outp') ? -current : current;;
        }
        // ---情况10. 信号发生器两路输出电流        
        if (dev.type === 'pressure_transducer') {
            let returnCurrent;
